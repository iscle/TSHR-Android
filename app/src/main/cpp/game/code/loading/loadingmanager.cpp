...
void
LoadingManager::AddCallback(LoadingManager::ProcessRequestsCallback *pCallback, void *pUserData) {
    if (mCancellingLoads) {
        return;
    }
...
void LoadingManager::AddRequest
...
#ifndef FINAL
    rReleaseAssertMsg(static_cast<int>(newTail) != mRequestHead,
                      "Too many load requests already!\n");
    ...
    mRequests[mRequestTail].pFileHandler = FileHandlerFactory::CreateFileHandler(handlerType,
                                                                                 sectionName);
...
void LoadingManager::CancelPendingRequests() {
    if (mRequestHead != mRequestTail) {
        mRequestTail =
                (mRequestHead + 1) % MAX_REQUESTS;  //Catch up and lose the rest of the loads.
    }
...
CementFileHandle LoadingManager::RegisterCementLibrary(const char *filename) {
    ...
    if (newTail != mRequestHead) {
        ...
        mRequests[mRequestTail].pFileHandler = new(GMA_TEMP) CementFileHandler(
                &(mCementLibraries[i]));
        mRequests[mRequestTail].pFileHandler->AddRef();
