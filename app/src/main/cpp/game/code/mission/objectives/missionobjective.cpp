...
void MissionObjective::Initialize() {
    ...
    //
    GetEventManager()->TriggerEvent(EVENT_MISSION_OBJECTIVE_NEW,
    ...
    for (i = 0; i < mNumNPCs; ++i) {
        if (mNPCs[i].driver) {
            ...
            character = GetCharacterManager()->AddCharacter(CharacterManager::NPC, name,
                                                            mNPCNames[i], "npd", NULL);
            ...
            if (!mNPCs[i].npc) {
                ...
                mNPCs[i].npc = GetCharacterManager()->AddCharacter(CharacterManager::NPC, name,
                                                                   mNPCNames[i], "npd", NULL);
...
void MissionObjective::AddNPC(const char *name, const char *loc, bool driver) {
    ...
    unsigned int len =
            strlen(name) < MAX_NPC_NAME_LENGTH - 1 ? strlen(name) : MAX_NPC_NAME_LENGTH - 2;
...
//////////////////////////////////////////////////////////////////////////
void
MissionObjective::LightPathRightWay(SwapArray <RoadManager::PathElement> &orPathRoute, int iStart,
                                    int iPastEnd) {
    int i;
...
//////////////////////////////////////////////////////////////////////////
void
MissionObjective::LightPathWrongWay(SwapArray <RoadManager::PathElement> &orPathRoute, int iStart,
                                    int iPastEnd) {
    int i;
...
//
//========================================================================
int MissionObjective::FindIntersectionInPath(SwapArray <RoadManager::PathElement> &irPath,
                                             Intersection *ipIntToFind) {
    //
...
//========================================================================
int
MissionObjective::FindRoadInPath(SwapArray <RoadManager::PathElement> &irPath, Road *ipRoadToFind) {
    //
    // Determine whether the players on the path that we want to light.
    //
    for (int i = irPath.mUseSize - 1; i > -1; i--) {
        if (irPath[i].type == RoadManager::ET_INTERSECTION)
            continue;
        if ((((Road * )(irPath[i].elem))->GetSourceIntersection() ==
             ipRoadToFind->GetSourceIntersection()
             && ((Road * )(irPath[i].elem))->GetDestinationIntersection() ==
                ipRoadToFind->GetDestinationIntersection())
            ||
            (((Road * )(irPath[i].elem))->GetSourceIntersection() ==
             ipRoadToFind->GetDestinationIntersection()
             && ((Road * )(irPath[i].elem))->GetDestinationIntersection() ==
                ipRoadToFind->GetSourceIntersection())
                ) //found matching path
...
void MissionObjective::UpdateLongRoadArrows(PathStruct &orPathStruct) {
    ...
    for (int i = orPathRoute.mUseSize - 1; i > -1; i--) {
        if (orPathRoute[i].type == RoadManager::ET_INTERSECTION)
            continue;
        if ((((Road * )(orPathRoute[i].elem))->GetSourceIntersection() ==
             ((Road * )(lastPathElem.elem))->GetSourceIntersection()
             && ((Road * )(orPathRoute[i].elem))->GetDestinationIntersection() ==
                ((Road * )(lastPathElem.elem))->GetDestinationIntersection())
            ||
            (((Road * )(orPathRoute[i].elem))->GetSourceIntersection() ==
             ((Road * )(lastPathElem.elem))->GetDestinationIntersection()
             && ((Road * )(orPathRoute[i].elem))->GetDestinationIntersection() ==
                ((Road * )(lastPathElem.elem))->GetSourceIntersection())
                ) //found matching path
        {
            ...
            RoadManager::FindClosestPointOnRoad(pRoad, tempVect, tempVect2, distPlayerFromRoad,
                                                playerRoadSegIndex);
            ...
            if (i > 0) {
                ...
                if (orPathRoute.mUseSize ==
                    1) //return for now, since there's only one road, and we don't have origin, destination info
                    return;
            ...
            for (int j = 5; j > 0; j--) {
                if (j <= numArrowsToPlace) {
                    ...
                    if (pRoad == orPathStruct.mpTargetRoadSeg->GetRoad()) {
                        ...
                        {
                            if (pRoadSegmentPlacement->GetSegmentIndex() >
                                orPathStruct.mpTargetRoadSeg->GetSegmentIndex())
                                continue;
                    ...
                    UpdateAnimPosition(GetAnimEntityDSGManager()->mpFloatingRightWayArrows[j],
                                       tempVect);
                    UpdateAnimPosition(GetAnimEntityDSGManager()->mpFloatingWrongWayArrows[j],
                                       tempVect);
                    ...
                    if (distPlayerFromRoad > 120.0f &&
                        (scaleFactor == 1.0f &&
                         ((int) pRoadSegmentPlacement->GetSegmentIndex()) < playerRoadSegIndex
                         || scaleFactor == -1.0f &&
                            ((int) pRoadSegmentPlacement->GetSegmentIndex()) > playerRoadSegIndex)
                            ) {
                        //we're lighting a road piece further away from the goal than the user
                        GetAnimEntityDSGManager()->mpFloatingRightWayArrows[j]->SetVisibility(
                                false);
                        ...
                        GetAnimEntityDSGManager()->mpFloatingWrongWayArrows[j]->SetVisibility(
                                false);
                    }
                    ...
                    GetAnimEntityDSGManager()->mpFloatingRightWayArrows[j]->SetAnimRootHeadingYUp(
                            tempVect);
                    GetAnimEntityDSGManager()->mpFloatingWrongWayArrows[j]->SetAnimRootHeadingYUp(
                            tempVect);
                    /*
...
bool MissionObjective::FindNearestRoadIconAhead
    ...
    if (DirectionalArrowEnum::INTERSECTION & mArrowType) {
        ...
        for (i = orPathRoute.mUseSize - 1; i > -1; i--) {
            if (orPathRoute[i].type == RoadManager::ET_INTERSECTION) {
                ...
                if (temp < nearestInter) {
                    nearestInter = temp;
                    ((Intersection * )(orPathRoute[i].elem))->AnimEntity(0)->GetAnimRootHeading(
                            orIconFacing);
                }
            }
        }
        ...
        for (i = GetAnimEntityDSGManager()->mpFloatingRightWayArrows.mUseSize - 1; i > 0; i--) {
            ...
            if (temp < nearestInter) {
                nearestInter = temp;
                GetAnimEntityDSGManager()->mpFloatingRightWayArrows[i]->GetAnimRootHeading(
                        orIconFacing);
            }
        }
        //
        ...
        for (i = GetAnimEntityDSGManager()->mpFloatingWrongWayArrows.mUseSize - 1; i > 0; i--) {
            ...
            if (temp < nearestInter) {
                nearestInter = temp;
                GetAnimEntityDSGManager()->mpFloatingWrongWayArrows[i]->GetAnimRootHeading(
                        orIconFacing);
            }
        }
    }
    ...
    if (DirectionalArrowEnum::NEAREST_ROAD & mArrowType) {
        ...
        if (temp < nearestInter) {
            nearestInter = temp;
            GetAnimEntityDSGManager()->mpFloatingRightWayArrows[0]->GetAnimRootHeading(
                    orIconFacing);
        }
    }
...
//    rReleasePrintf("LightPath. 0x%x\n", orPathRoute.mpData);
    ...
    SourceTVal = pRoadManager->DetermineRoadT(pSourceRoadSeg,
                                              pRoadManager->DetermineSegmentT(closestPosn,
                                                                              pSourceRoadSeg));

    pRoadManager->FindClosestPointOnRoad((Road *) targetPath.elem, targetPosn,
                                         closestPosn, closestDist, closestSegIndex);

    TargetTVal = pRoadManager->DetermineRoadT(pTargetRoadSeg,
                                              pRoadManager->DetermineSegmentT(closestPosn,
                                                                              pTargetRoadSeg));
            ...
            if (orPathRoute[i].type == RoadManager::ET_INTERSECTION) {
                    ...
                    //There's definitely some(other intersection) beyond this intersection
                {
                    if (orPathRoute[i + 1].type == RoadManager::ET_INTERSECTION) {
                        ...
                        ((Road * )(orPathRoute[i + 1].elem))->GetRoadSegment(0)->GetPosition(0.0f,
                                                                                             0.0f,
                                                                                             &pathHeading);
                        ((Road * )(orPathRoute[i + 1].elem))->GetRoadSegment(
                                ((Road * )(orPathRoute[i + 1].elem))->GetNumRoadSegments() -
                                1)->GetPosition(0.0f, 0.0f, &pathTemp);
                        if ((pathHeading - sourcePosn).MagnitudeSqr() >
                            (pathTemp - sourcePosn).MagnitudeSqr()) {
                ...
                } else //No(other intersection) beyond this intersection
                {
                    ...
                    ((Road * )(orPathRoute[i + 1].elem))->GetRoadSegment(0)->GetPosition(0.0f, 0.0f,
                                                                                         &pathHeading);
                    ((Road * )(orPathRoute[i + 1].elem))->GetRoadSegment(
                            ((Road * )(orPathRoute[i + 1].elem))->GetNumRoadSegments() -
                            1)->GetPosition(0.0f, 0.0f, &pathTemp);
...
*/
        ...
        if (i + 1 < orPathRoute.mUseSize) //There's something in this path
        {
            if (orPathRoute[i + 1].type == RoadManager::ET_INTERSECTION) {
                ...
                ((Road * )(orPathRoute[i].elem))->GetRoadSegment(0)->GetPosition(0.0f, 0.0f,
                                                                                 &pathHeading);
                ((Road * )(orPathRoute[i].elem))->GetRoadSegment(
                        ((Road * )(orPathRoute[i].elem))->GetNumRoadSegments() - 1)->GetPosition(
                        0.0f, 0.0f, &pathTemp);
                if ((pathHeading - sourcePosn).MagnitudeSqr() >
                    (pathTemp - sourcePosn).MagnitudeSqr()) {
                    pathHeading = pathTemp;
...
//    rmt::Vector pathTemp2, pathTemp;
    ...
    RoadSegment *pRoadEnd = ipSourceRoadSeg->GetRoad()->GetRoadSegment(
            ipSourceRoadSeg->GetRoad()->GetNumRoadSegments() - 1);
