...
GoToObjective::~GoToObjective() {
    if (mAnimatedIcon) {
        ...
        WalkerCam *wc = static_cast<WalkerCam *>(GetSuperCamManager()->GetSCC(0)->GetSuperCam(
                SuperCam::WALKER_CAM));
        if (wc) {
...
void GoToObjective::OnInitialize() {
    ...
    if (eventLocator != NULL) {
        GetEventManager()->AddListener(this,
                                       (EventEnum)(EVENT_LOCATOR + eventLocator->GetEventType()));
    }
    ...
    if (mActionTrigger) {
        ...
        ActionButton::GenericEventButtonHandler *pABHandler = dynamic_cast<ActionButton::GenericEventButtonHandler *>(ActionButton::GenericEventButtonHandler::NewAction(
                eventLocator, (EventEnum)(EVENT_LOCATOR + mOldEvent), gma));
        rAssert(pABHandler);
    ...
    if (strcmp(mDestinationDrawableName, "") != 0) {
        ...
        SphereTriggerVolume *sphTrig = dynamic_cast<SphereTriggerVolume *>(eventLocator->GetTriggerVolume(
                0));
    ...
    if (GetInteriorManager()->IsInside()) {
        ...
        if (entrance) {
            ...
            if (dist.Magnitude() < 20.0f) {
                ...
                GetGameplayManager()->GetCurrentMission()->GetCurrentStage()->ShowStageComplete(
                        false);
            }
        }
    }
...
void GoToObjective::OnFinalize() {
    ...
    EventLocator *eventLocator = dynamic_cast<EventLocator *>(mDestinationLocator);
    if (eventLocator != NULL) {
        GetEventManager()->RemoveListener(this, (EventEnum)(
...
void GoToObjective::OnUpdate(unsigned int elapsedTime) {
    ...
    if (!GetFinished() && mDestinationLocator && !mActionTrigger) {
        ...
        for (i = 0; i < mDestinationLocator->GetNumTriggers(); ++i) {
            if (reinterpret_cast<TriggerLocator *>(mDestinationLocator)->GetTriggerVolume(
                    i)->IsPlayerTracking(0)) {
                SetFinished(true);
            }
        }
    }
            ...
            if (locator == mDestinationLocator && !mWaitingForEffect) {
                if (mActionTrigger && mOldData != -1) {
                    ...
                    GetCharacterManager()->GetCharacter(0)->RemoveActionButtonHandler(
                            GetActionButtonManager()->GetActionByIndex(id));
                }
