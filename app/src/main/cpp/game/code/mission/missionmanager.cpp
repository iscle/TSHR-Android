...
void MissionManager::Initialize() {
    ...
    //
    defaultReward = GetRewardsManager()->GetReward(this->GetCurrentLevelIndex(),
                                                   Reward::eDefaultCar);
    ...
    if (mLoadingState != STATE_INVALID) {
        ...
        if (currentMission->IsSundayDrive()) {
            ...
            GetEventManager()->TriggerEvent(EVENT_MISSION_START,
                                            reinterpret_cast<void *>(currentMission));
    ...
    if (mLoadingState == STATE_WAIT_FOR_DYNALOAD && id == EVENT_FIRST_DYNAMIC_ZONE_END) {
        ...
        if (!GetCurrentMission()->IsSundayDrive() && CommandLineOptions::Get(CLO_SKIP_SUNDAY) ||
            ...
            if (mIsDemo) {
                ...
                rTuneAssert(GetGameFlow()->GetCurrentContext() != CONTEXT_PAUSE &&
                            GetGameFlow()->GetNextContext() != CONTEXT_PAUSE &&
                            GetGameFlow()->GetNextContext() != CONTEXT_GAMEPLAY);
    ...
    switch (mLoadingState) {
        ...
        case STATE_MISSION_LOADING: {
            if (GetLoadingManager()->IsLoading() ||
                GetVehicleCentral()->GetCurrentVehicleUnderConstruction() != NULL) {
                break;
            }
        ...
        case STATE_MISSION_LOAD: {
            ...
            sprintf(mLastFileName, "scripts\\missions\\level0%d\\%sl.mfk", levelnum,
                    mission->GetName());
        ...
        case STATE_MISSION_INIT: {
            ...
            if (mShouldLoadDefaultVehicle) {
                ...
                PlaceVehicleAtLocatorName(GetVehicleInSlot(eDefaultCar),
                                          mDefaultLevelVehicleLocator);
        ...
        case STATE_MISSION_DYNALOAD: {
            ...
            if (
                    (GetGuiSystem()->GetBackendManager()->GetCurrentScreen() ==
            ...
            //
            if (mission->GetNumStages() == 0 ||
                !mResetting) // || !mission->GetStage(0)->LoadMissionStart())
            {
                mLoadingState = STATE_MISSION_START;
            }
                ...
                if (GetGameFlow()->GetCurrentContext() == CONTEXT_LOADING_DEMO) {
                    ...
                    if (GetGameFlow()->GetCurrentContext() != CONTEXT_PAUSE ||
                        (GetGameFlow()->GetCurrentContext() == CONTEXT_PAUSE &&
                         GetPauseContext()->IsWaitingForContextSwitch())) {
                        GetGameFlow()->SetContext(CONTEXT_GAMEPLAY);
                ...
                if (pCurrentMission->IsForcedCar() == true) {
                    ...
                    if (mPlayerAndCarInfo.mbDirtyFlag != true) {
                        GetAvatarManager()->GetAvatarForPlayer(0)->GetCharacter()->GetPosition(
                                mPlayerAndCarInfo.mPlayerPosition);
                        pCurrentMission->GetVehicleRestart()->GetLocation(
                                &mPlayerAndCarInfo.mForceLocation);
...
/*
  ...
  */
        GetRenderManager()->pWorldRenderLayer()->DumpDynaLoad(name,
                                                              GetRenderManager()->mEntityDeletionList);
    }
