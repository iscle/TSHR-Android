...
int RewardsManager::BindReward(char *name, char *filename, Reward::eRewardType rtype,
                               Reward::eQuestType qtype, int level) {
    NameCheck(name);
...
int
RewardsManager::SetCarAttributes(char *carname, float speed, float acceleration, float toughness,
                                 float stability) {
...
float RewardsManager::ComputeOverallCarRating(CarAttributeRecord *carStats) {
    ...
    //
    return (carStats->mAcceleration + carStats->mSpeed + carStats->mStability +
            carStats->mToughness) / 4.0f;
}
...
int RewardsManager::AddMerchandise(char *name, char *filename, Reward::eRewardType rtype, int level,
                                   int cost, Merchandise::eSellerType sellerType) {

    //check if the array of merchandise is full
    if (mLevelTokenStoreList[level].mCounter < MAX_INVENTORY) {
        mLevelTokenStoreList[level].mInventoryList[mLevelTokenStoreList[level].mCounter]->SetName(
                name);
        mLevelTokenStoreList[level].mInventoryList[mLevelTokenStoreList[level].mCounter]->SetLevel(
                level);
        mLevelTokenStoreList[level].mInventoryList[mLevelTokenStoreList[level].mCounter]->SetFilename(
                filename);
        mLevelTokenStoreList[level].mInventoryList[mLevelTokenStoreList[level].mCounter]->SetRewardType(
                rtype);
        mLevelTokenStoreList[level].mInventoryList[mLevelTokenStoreList[level].mCounter]->SetCost(
                cost);
        mLevelTokenStoreList[level].mInventoryList[mLevelTokenStoreList[level].mCounter]->SetSellerType(
                sellerType);
...
RewardsManager::FindNextMerchandise(int level, Merchandise::eSellerType sellerType) {
    for (int i = mLevelTokenStoreSearchIndex[level];
         i < mLevelTokenStoreList[level].mCounter; i++) {
...
bool RewardsManager::BuyMerchandise(int level, const char *name) {
    ...
    if (GetCharacterSheetManager()->GetNumberOfTokens((RenderEnums::LevelEnum) level) <
        pMerchandise->GetCost()) {
        //player doesn't have enough tokens to buy the item 
        return false;
    } else {
        pMerchandise->UnlockReward();
        GetCharacterSheetManager()->SubtractTokens((RenderEnums::LevelEnum) level,
                                                   pMerchandise->GetCost());
