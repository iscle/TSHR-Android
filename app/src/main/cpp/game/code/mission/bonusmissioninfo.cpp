...
//
//=============================================================================
void BonusMissionInfo::SetPositions(CarStartLocator *pos1, CarStartLocator *pos2,
                                    CarStartLocator *carPos) {
...
void BonusMissionInfo::Update(unsigned int milliseconds) {
    ...
    if (!mIsCompleted || (mIsCompleted && !mIsOneShot)) {
        ...
        if (distSquared < 5.0f * 5.0f) {
            ...
            GetEventManager()->TriggerEvent(EVENT_BONUS_MISSION_CHARACTER_APPROACHED,
                                            reinterpret_cast<void *>(missionNum));
        }
