...
void CharacterSheetManager::SetMissionComplete(RenderEnums::LevelEnum level, char *name,
                                               bool completed_secondary_objective, int seconds) {
    ...
    if (GetMissionRecord(level, name) != NULL) {
        ...
        if (seconds > 0) {
            ...
            if (p_missionrecord->mBestTime < 0) {  //default case so seconds becomes new best time
...
void CharacterSheetManager::SetMissionSkipped(RenderEnums::LevelEnum level,
...
//get a ptr to the record, you shouldn't  modify the record though, use the set methods to modify instead
MissionRecord *
CharacterSheetManager::QueryStreetRaceStatus(RenderEnums::LevelEnum level, char *name) {
    return (GetStreetRaceRecord(level, name));
}

MissionRecord *
CharacterSheetManager::QueryBonusMissionStatus(RenderEnums::LevelEnum level, char *name) {
...
void CharacterSheetManager::SaveData(GameDataByte *dataBuffer, unsigned int numBytes) {
    if (CommandLineOptions::Get(CLO_MEMCARD_CHEAT)) {
        ...
        for (int i = 0; i < MAX_LEVELS; i++) {
            ...
            // unlock wasps
            tempCharacterSheet->mLevelList[i].mWaspsDestroyed = GetRewardsManager()->GetTotalWasps(
                    i);
        }
...
void CharacterSheetManager::SetCurrentMission(RenderEnums::LevelEnum level,
                                              RenderEnums::MissionEnum mission_number) {
    ...
    if (static_cast<int>(level) >
        static_cast<int>(spInstance->mCharacterSheet.mHighestMissionPlayed.mLevel)) {
        ...
        if (static_cast<int>(mission_number) >
            static_cast<int>(spInstance->mCharacterSheet.mHighestMissionPlayed.mMissionNumber)) {
            spInstance->mCharacterSheet.mHighestMissionPlayed.mMissionNumber = mission_number;
        }
    }
}
...
MissionRecord *CharacterSheetManager::GetMissionRecord(RenderEnums::LevelEnum level, char *name) {
    ...
    for (int i = 0; i < MAX_MISSIONS; i++) {
        ...
        if (strcmp(spInstance->mCharacterSheet.mLevelList[level].mMission.mList[i].mName, name) ==
            0) {
...
MissionRecord *
CharacterSheetManager::GetStreetRaceRecord(RenderEnums::LevelEnum level, char *name) {
    //check to see if name array is too long
    if (strlen(name) > 16) {
    ...
    for (int i = 0; i < MAX_STREETRACES; i++) {
        ...
        if (strcmp(spInstance->mCharacterSheet.mLevelList[level].mStreetRace.mList[i].mName,
                   name) == 0) {
...
MissionRecord *
CharacterSheetManager::GetBonusMissionRecord(RenderEnums::LevelEnum level, char *name) {
    if (strlen(name) > 16) {
        //Name is too long
        rAssert(0);
    }
...
Record *CharacterSheetManager::GetCollectableRecord(RenderEnums::LevelEnum level,
                                                    CharacterSheetManager::eCollectableType type,
                                                    char *name) {
    if (strlen(name) > 16) {
    ...
    switch (type) {
        case eCard: {
            for (int i = 0; i < MAX_CARDS; i++) {
                if (strcmp(name,
                           spInstance->mCharacterSheet.mLevelList[level].mCard.mList[i].mName)) {
...
int CharacterSheetManager::QueryNumMissionsCompleted(RenderEnums::LevelEnum level) {
    ...
    for (int i = 0; i < MAX_MISSIONS; i++) {
        if (strcmp(spInstance->mCharacterSheet.mLevelList[level].mMission.mList[i].mName, "m0") ==
            0) {
...
float CharacterSheetManager::QueryPercentLevelCompleted(RenderEnums::LevelEnum level) const {
    ...
    level_completed = (
            (missions + bonusmission + streetraces + skins + cars + cards + wasps + gags) / 8);
...
int CharacterSheetManager::UpdateRewardsManager() {
    for (int i = 0; i < MAX_LEVELS; i++) {
        ...
        if (static_cast<int>(QueryHighestMission().mLevel) >= i) {
            GetRewardsManager()->GetReward((RenderEnums::LevelEnum) i,
                                           Reward::eDefaultCar)->UnlockReward();
        } else {
            GetRewardsManager()->GetReward((RenderEnums::LevelEnum) i,
                                           Reward::eDefaultCar)->LockReward();
        }

        //update the cards unlockable
        if (spInstance->QueryAllCardsCollected((RenderEnums::LevelEnum) i) == true) {
            GetRewardsManager()->GetReward((RenderEnums::LevelEnum) i,
                                           Reward::eCards)->UnlockReward();
        } else {
            GetRewardsManager()->GetReward((RenderEnums::LevelEnum) i,
                                           Reward::eCards)->LockReward();
        }

        //update the streetraces
        if (spInstance->QueryAllStreetRacesCompleted((RenderEnums::LevelEnum) i) == true) {
            GetRewardsManager()->GetReward((RenderEnums::LevelEnum) i,
                                           Reward::eStreetRace)->UnlockReward();
        } else {
            GetRewardsManager()->GetReward((RenderEnums::LevelEnum) i,
                                           Reward::eStreetRace)->LockReward();
        }

        //update the bonus mission
        if (spInstance->QueryBonusMissionCompleted((RenderEnums::LevelEnum) i) == true) {
            GetRewardsManager()->GetReward((RenderEnums::LevelEnum) i,
                                           Reward::eBonusMission)->UnlockReward();
        } else {
            GetRewardsManager()->GetReward((RenderEnums::LevelEnum) i,
                                           Reward::eBonusMission)->LockReward();
        }
