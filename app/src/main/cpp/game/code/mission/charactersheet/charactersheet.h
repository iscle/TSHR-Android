...
static const int NUM_BYTES_PER_SECTOR = (NUM_PERSISTENT_OBJECTS + 7)
        >> 3; // We'll bit pack the object states so this is the number of bytes we'll need per zone/rail.
static const int NUM_BYTES_FOR_PERSISTENT_STATES = NUM_BYTES_PER_SECTOR *
                                                   NUM_PERSISTENT_SECTORS; // And this is the number of bytes we'll need for the entire game.
