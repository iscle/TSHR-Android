...
TractorBeam::LoadSetup(const char *statePropDSGName) {
    ...
    if (mpDrawable != NULL) {
        ...
        WorldRenderLayer *pLayer = static_cast<WorldRenderLayer *>(GetRenderManager()->mpLayer(
...
TractorBeam::Update(float timeInMS) {
    if (mpDrawable != NULL) {
        ...
        rmt::Matrix targetOrientation;
        {
            ...
            rmt::Quaternion orientation = Slerp(targetOrientation, mTransform, timeInMS,
                                                BEAM_ANGULAR_SPEED);
...
rmt::Quaternion
TractorBeam::Slerp(const rmt::Matrix m1, const rmt::Matrix m2, float deltaTime,
