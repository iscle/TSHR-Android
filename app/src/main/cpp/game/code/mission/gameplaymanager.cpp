...
bool
GameplayManager::TestPosInFrustrumOfPlayer(const rmt::Vector &pos, int playerID, float radius) {
...
//
//=============================================================================
bool GameplayManager::TestProximityToUsersCarAndNudgeUpIfNecessaryDamnUglyHack(rmt::Vector &pos,
                                                                               Vehicle *playersCar) {
    if (playersCar) {
...
Vehicle *GameplayManager::AddLevelVehicle(char *vehicleName, eCarSlots slot, char *confile) {
    ...
    if (mIsDemo) {
        ...
        if (slot == GameplayManager::eDefaultCar) {
            ...
            v = GetVehicleCentral()->InitVehicle(vehicleName, true, confile, VT_USER,
                                                 VehicleCentral::ALLOW_DRIVER, true);
        } else {

            // ai and other slot always have driver, even if supressed
            v = GetVehicleCentral()->InitVehicle(vehicleName, true, confile, VT_USER,
                                                 VehicleCentral::FORCE_DRIVER);
        }
...
Vehicle *GameplayManager::AddMissionVehicle(char *vehiclename, char *confile, char *driver) {
    ...
    for (i = 0; i < MAX_MISSION_VEHICLE_SLOTS; i++) {
        if (strcmp(vehiclename, mMissionVehicleSlots[i].name) == 0) {
            ...
            rTuneAssertMsg(vehicleSavedFromExecution == 0,
                           "you are trying to use two of the same car in a mission");
    ...
    rTuneAssertMsg(
            (strcmp(GetGameplayManager()->GetVehicleSlotVehicleName(GameplayManager::eOtherCar),
                    vehiclename) != 0), "you can't add the OTHER car to a mission");
    ...
    for (i = 0; i < MAX_MISSION_VEHICLE_SLOTS; i++) {
        if (mMissionVehicleSlots[i].vehicle == 0) {
            ...
            GetEventManager()->TriggerEvent(EVENT_MISSION_VEHICLE_CREATED,
                                            mMissionVehicleSlots[i].vehicle);

            //update the AI vehicle slot so It points to the correct vehicle.
            if (strcmp(GetGameplayManager()->GetVehicleSlotVehicleName(GameplayManager::eAICar),
                       "NULL") != 0) {
                if (strcmp(mMissionVehicleSlots[i].vehicle->GetName(),
                           GetGameplayManager()->GetVehicleSlotVehicleName(
                                   GameplayManager::eAICar)) == 0) {
...
void GameplayManager::EmptyMissionVehicleSlots() {
    ...
    for (i = 0; i < MAX_MISSION_VEHICLE_SLOTS; i++) {
        if (mMissionVehicleSlots[i].vehicle != NULL) {
            ...
            GetEventManager()->TriggerEvent(EVENT_MISSION_VEHICLE_RELEASED,
                                            mMissionVehicleSlots[i].vehicle);
    ...
    if (carData->mp_vehicle->mVehicleDestroyed) {
        if (carData->usingHusk) {
            ...
            if (avatar->IsInCar()) {
                ...
                rAssert(GetVehicleCentral()->mHuskPool.IsHuskType(
                        avatar->GetVehicle()->mVehicleID));
    ...
    switch (id) {
            ...
            for (int i = 0; i < MAX_VEHICLE_SLOTS; i++) {
                ...
                if (mVehicleSlots[i].mp_vehicle == v &&
                    ...
                    if (husk) {
                        ...
                        GetVehicleCentral()->RemoveVehicleFromActiveList(
                                mVehicleSlots[i].mp_vehicle);
                        ...
                        GetEventManager()->TriggerEvent(EVENT_VEHICLE_DESTROYED_SYNC_SOUND,
                                                        (void *) husk);
        ...
        case EVENT_USER_CANCEL_MISSION_BRIEFING: {
            if (GetCurrentMission()->IsForcedCar() == true) {
                ...
                GetCharacterManager()->RemoveCharacter(
                        mVehicleSlots[eOtherCar].mp_vehicle->GetDriver());
        ...
        case EVENT_USER_CANCEL_PAUSE_MENU: {
            ...
            if (GetCurrentMission()->IsForcedCar() ==
                true) {  //we are quiting from forced car mission we need to lock the car so user cannot drive it.
                mVehicleSlots[eOtherCar].mp_vehicle->TransitToAI();
...
void GameplayManager::SetCurrentVehicle(Vehicle *vehicle) {
    ...
    if (currentHud != NULL) {
        ...
        if (mCurrentVehicle != NULL && mCurrentVehicle->mVehicleType == VT_USER) {
            ...
            mCurrentVehicleIconID = currentHud->GetHudMap(0)->RegisterIcon(
                    HudMapIcon::ICON_PLAYER_CAR,
...
void GameplayManager::MakeSureHusksAreReverted(Vehicle *pvehicle) {
    ...
    for (i = 0; i < MAX_MISSION_VEHICLE_SLOTS; i++) {
        if ((mMissionVehicleSlots[i].usingHusk == true) &&
            (mMissionVehicleSlots[i].vehicle == pvehicle)) {
...
void GameplayManager::SetBonusMissionInfo(const char *NPCname,
    ...
    rAssert(missionNum >= 0 && missionNum >= MAX_MISSIONS &&
            missionNum < MAX_BONUS_MISSIONS + MAX_MISSIONS);

    if (missionNum >= MAX_MISSIONS) {
        if (strcmp(missionName, "bm1") == 0) {
            if (GetCharacterSheetManager()->QueryBonusMissionCompleted(
                    GetMissionManager()->GetCurrentLevelIndex()) == true) {
        ...
        int id = -1;
        ActionButton::GenericEventButtonHandler *pABHandler = static_cast<ActionButton::GenericEventButtonHandler *>(ActionButton::GenericEventButtonHandler::NewAction(
                evtLoc, EVENT_BONUS_MISSION_DIALOGUE, HeapMgr()->GetCurrentHeap()));
...
Mission *GameplayManager::GetCurrentMission() {
    if (mIsInBonusMission) {
        if (mCurrentBonusMission >= MAX_MISSIONS &&
            mCurrentBonusMission < MAX_MISSIONS + MAX_BONUS_MISSIONS) {
...
int GameplayManager::CreateChaseManager(char *hostvehiclename, char *confile, int spawnrate) {
    for (int i = 0; i < MAX_CHASEMANAGERS; i++) {
        if (strcmp(m_ChaseManager_Array[i].hostvehicle, "NULL") == 0 &&
            m_ChaseManager_Array[i].mp_chasemanager == NULL) {
...
int GameplayManager::UpdateVDU() {
    if (mVDU.mCounter == 0) {
        ...
        GetAvatarManager()->GetAvatarForPlayer(0)->GetCharacter()->GetPosition(
                CharacterPositionStart);
        ...
        for (int i = 0; i < MAX_VDU_CARS; i++) {
            if (mVDU.mpVehicle[i] != NULL) {
                ...
                if ((spInstance->TestPosInFrustrumOfPlayer(position, 0) == false) &&
                    (rmt::Sqrt(CarPositionDifference.MagnitudeSqr()) > 5.0f)) {
