...
void GetOutOfCarCondition::Update(unsigned int elapsedTime) {
    if (mbIsConditionActive == true) {
        ...
        if ((mTimeRemainingmilliseconds <= 0 ||
             GetInteriorManager()->IsInside())  //If you go inside you fail.
            &&
...
void GetOutOfCarCondition::HandleEvent(EventEnum id, void *pEventData) {
    if (id == EVENT_GETOUTOFVEHICLE_END) {
        ...
        if (mbIsConditionActive ==
            false) //This is the first time and not me cheating by jumping in a traffic car
        {
        ...
        CGuiScreenHud *currentHud = GetCurrentHud();
        if (currentHud != NULL) {
            currentHud->DisplayMessage(true,
                                       299); //299 is the magic number in the textbible for the gET IN CAR message.
        }
