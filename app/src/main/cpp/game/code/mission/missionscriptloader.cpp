...
*/
    ...
    // Level methods
    //
    console->AddFunction("InitLevelPlayerVehicle", InitLevelPlayerVehicle, "", 3,
                         4); //Inits a player vehicle and puts it at the locator (carname, locatorname, slot, [.con])
    console->AddFunction("PlacePlayerCar", PlacePlayerCar, "", 2,
                         2); //Places a Player car at the locator (carname, locatorname)
    /*
	...
	console->AddFunction("DisableTraffic", DisableTraffic,
		"No Traffic", 0, 0);
    */
    console->AddFunction("AddPurchaseCarReward", AddPurchaseCarReward, "", 5,
                         6); //Add purchasable car reward (name), (character name), (choreo), (location), (float scale), [carstart]
    console->AddFunction("SetPostLevelFMV", SetPostLevelFMV, "", 1,
                         1); //FMV to play before loading next level.
    ...
    console->AddFunction("CreateTrafficGroup", CreateTrafficGroup, "", 1,
                         1); //Creates a Traffic Group
    console->AddFunction("AddTrafficModel", AddTrafficModel, "", 2,
                         3); //Add a Traffic Model to a Group
    console->AddFunction("CloseTrafficGroup", CloseTrafficGroup, "", 0, 0); //Closes a Traffic Group

    console->AddFunction("SetCarAttributes", SetCarAttributes, "", 5,
                         5); //Set the Car Attributes displayed in the Phone Booth
    console->AddFunction("SetTotalGags", SetTotalGags, "", 2,
                         2); //Set the total number of gags for a level
    console->AddFunction("SetTotalWasps", SetTotalWasps, "", 2,
                         2); //Set the total number of wasps for a level
    console->AddFunction("AddGlobalProp", AddGlobalProp, "", 1,
                         1); //Adds a Prop the Master Global Prop Hash Table

    //Chase/HitnRun Methods
    console->AddFunction("CreateChaseManager", CreateChaseManager, "", 3,
                         3); //Creates a ChaseManager for a host car
    console->AddFunction("DisableHitAndRun", DisableHitAndRun, "", 0,
                         0); //Disables Hit and Run Feature
    console->AddFunction("EnableHitAndRun", EnableHitAndRun, "", 0,
                         0); //Enables Hit and Run Feature
    console->AddFunction("SetHitAndRunMeter", SetHitAndRunMeter, "", 1,
                         1); //Set the value of the HitnRun meter
    console->AddFunction("SetNumChaseCars", SetNumChaseCars, "", 1,
                         1); //Set the Max Number of Chase Cars
    console->AddFunction("SetChaseSpawnRate", SetChaseSpawnRate, "", 2,
                         2); //Sets the Spawn rate  for a host car
    console->AddFunction("KillAllChaseAI", KillAllChaseAI, "", 1,
                         1); //Kills all Chase AI for a host car
    console->AddFunction("ResetHitAndRun", ResetHitAndRun, "", 0, 0); //Resets HitnRun system
    console->AddFunction("SetHitAndRunDecay", SetHitAndRunDecay, "", 1,
                         1); //Sets decay rate for HitnRun meter
    console->AddFunction("SetHitAndRunDecayInterior", KillAllChaseAI, "", 1,
                         1); //Sets interior decay rate for HitnRun meter
    //
    // Mission methods
    //
    console->AddFunction("AddMission", AddMission, "", 1,
                         1); //Adds a new mission and names it (Name)

    console->AddFunction("AddBonusMission", AddBonusMission, "", 1,
                         1); //Adds a new bonus mission and names it (Name)

    console->AddFunction("SetMissionNameIndex", SetMissionNameIndex, "", 1,
                         1); //Sets the index in the text bible (index)

    console->AddFunction("SelectMission", SelectMission, "", 1,
                         1); //Selects a mission by name (Name)
    ...
    console->AddFunction("SetMissionResetPlayerInCar", SetMissionResetPlayerInCar, "", 1,
                         1); //This is where the player will go (in car) when the mission resets due to failure [locator]

    console->AddFunction("SetMissionResetPlayerOutCar", SetMissionResetPlayerOutCar, "", 2,
                         2); //This is where the player will go (out of car) when the mission resets due to failure [player locator, vehicle locator]
    console->AddFunction("SetDynaLoadData", SetDynaLoadData, "", 1,
                         2); //This is the string that defines what to load and dump..

    console->AddFunction("AddBonusObjective", AddBonusObjective, "", 1,
                         2); //This adds a bonus objective to the current mission (type).

    console->AddFunction("SetForcedCar", SetForcedCar, "", 0,
                         0); //This makes a mission a forced car mission

    console->AddFunction("CloseMission", CloseMission, "", 0, 0); //Closes the current mission

    console->AddFunction("SetDemoLoopTime", SetDemoLoopTime, "", 1,
                         1); //Sets the demo loop time only for demo loops

    console->AddFunction("StreetRacePropsLoad", StreetRacePropsLoad, "", 1,
                         1); //String that defines which file that contains the props for streetraces to load

    console->AddFunction("StreetRacePropsUnload", StreetRacePropsUnload, "", 1,
                         1); //String that defines which file that contains the props for streetraces to unload

    console->AddFunction("UseElapsedTime", UseElapsedTime, "", 0,
                         0); //Make this Mission timer count UP

    console->AddFunction("AttachStatePropCollectible", AttachStatePropCollectible, "", 2,
                         2); //Attaches a collectible to the vehicle (vehicle name , statepropname)

    console->AddFunction("ShowHUD", ShowHUD, "", 1, 1); //Sets visibility of in-game HUD

    console->AddFunction("SetNumValidFailureHints", SetNumValidFailureHints, "", 1,
                         1); //Sets number of valid failure hints to be randomly chosen

    //
    // Stage methods
    //
    console->AddFunction("AddStage", AddStage, "", 0,
                         7); //Adds a new stage to the current mission ([final], [locked, skin|car, name], [locked, skin|car, name])
    console->AddFunction("SetStageMessageIndex", SetStageMessageIndex, "", 1,
                         2); //Sets the index in the text bible (index)

    console->AddFunction("SetStageTime", SetStageTime, "", 1,
                         1); //Adds time to the current stage ()
    console->AddFunction("AddStageTime", AddStageTime, "", 1,
                         1); //Sets the time of the current stage ()

    console->AddFunction("AddStageVehicle", AddStageVehicle,
                         "", //Adds an AI vehicle to the stage (name, startlocator, aitype, [.con], [driver name])
                         3, 5);
    console->AddFunction("MoveStageVehicle", MoveStageVehicle,
                         "", //Moves an AI vehicle in the stage (name, startlocator, aitype)
                         3, 3);
    console->AddFunction("ActivateVehicle", ActivateVehicle, "", 3,
                         4); //Activates AI vehicle that has no AI in the stage (name, startlocator, aitype, [driver name])
    console->AddFunction("AddStageWaypoint", AddStageWaypoint, "", 1,
                         1); //Adds a waypoint to the stage (locatorname)
    console->AddFunction("AddStageCharacter", AddStageCharacter, "", 3,
                         5); //Adds a character to the stage (name, locatorname, dynaload string, [vehiclename], [carlocator])

    console->AddFunction("AddStageMusicChange", AddStageMusicChange, "", 0,
                         0); //Triggers check for change of music tension
    console->AddFunction("SetStageMusicAlwaysOn", SetStageMusicAlwaysOn, "", 0,
                         0); //Locks music on for walking out of car in stage
    console->AddFunction("SetCompletionDialog", SetCompletionDialog, "", 1,
                         2); //Plays dialog with given event name at end of stage
    console->AddFunction("StageStartMusicEvent", SetStageStartMusicEvent, "", 1,
                         1); //Triggers a music event when stage begins
    console->AddFunction("SetMusicState", SetMusicState, "", 2,
                         2); //Sets a radMusic state using the given state and event names

    console->AddFunction("SetStageCamera", SetStageCamera, "", 3,
                         3); //Sets the current camera mode for this stage (mode, cut, quick transition)

    console->AddFunction("RESET_TO_HERE", ResetToThisStage, "", 0,
                         0); //Sets the desred stage for resseting to... hehe hehehshshehahrheh

    console->AddFunction("SetMaxTraffic", SetTrafficDensity, "", 1,
                         1); //Sets the max traffic density.

    console->AddFunction("AddSafeZone", AddSafeZone, "", 2,
                         2); //Adds a Safezone at locatorname,radius in meters ()

    console->AddFunction("SetBonusMissionStart", SetBonusMissionStart, "", 0,
                         0); //Makes this stage the start of a bonus mission.

    console->AddFunction("ShowStageComplete", ShowStageComplete, "", 0,
                         0); //Show stage complete presentation.
    ...
    console->AddFunction("SetVehicleAIParams", SetVehicleAIParams, "", 3,
                         3); //Sets params for an existing stage vehicle AI (STAGEVEHICLENAME, MINSHORTCUTSKILL, MAXSHORTCUTSKILL)

    console->AddFunction("PlacePlayerAtLocatorName", PlacePlayerAtLocatorName, "", 1,
                         1); //On Stage Reset/Init places the player at a Locator
    console->AddFunction("msPlacePlayerCarAtLocatorName", msPlacePlayerCarAtLocatorName, "", 1,
                         1); //Place the players current car at locator at the start of stage

    //Chuck:Commands for Swapping Default for forced cars As per the Agreed template
    console->AddFunction("SwapInDefaultCar", SwapInDefaultCar, "", 0,
                         0); //At the End of this Stage We Swap In the Default during the Iris Wipe
    console->AddFunction("SetSwapPlayerLocator", SetSwapPlayerRespawnLocatorName, "", 1,
                         1); //Sets the Locator to spawn the player after we swap cars
    console->AddFunction("SetSwapDefaultCarLocator", SetSwapDefaultCarRespawnLocatorName, "", 1,
                         1); //Set the Locator to spawn the default car after we swap cars
    console->AddFunction("SetSwapForcedCarLocator", SetSwapForcedCarRespawnLocatorName, "", 1,
                         1); //Set the Locator to spawn the forced car after we swap cars

    console->AddFunction("NoTrafficForStage", NoTrafficForStage, "", 0,
                         0); //Sets No Traffic For a Stage

    console->AddFunction("ClearTrafficForStage", ClearTrafficForStage, "", 0,
                         0); //Clear traffic at the beginning of the stage

    console->AddFunction("SetStageAIRaceCatchupParams", SetStageAIRaceCatchupParams, "", 5,
                         5); //Sets Race AI catchup params.

    console->AddFunction("SetStageAIEvadeCatchupParams", SetStageAIEvadeCatchupParams, "", 3,
                         3); //Sets Evade AI catchup params.

    console->AddFunction("SetStageAITargetCatchupParams", SetStageAITargetCatchupParams, "", 3,
                         3); //Sets Target AI catchup params.


    console->AddFunction("SetCharacterToHide", SetCharacterToHide, "", 1,
                         1); //Set a Character To Be Hidden for duration at start of stage

    console->AddFunction("SetLevelOver", SetLevelOver, "", 0,
                         0); //Explictly Command to End Level at Stage Finalize
    console->AddFunction("SetGameOver", SetGameOver, "", 0, 0); //Explicitly end the game.

    console->AddFunction("StayInBlack", StayInBlack, "", 0,
                         0); //Makes Scree black for duration of stage
    ...
    //
    console->AddFunction("SetParTime", SetParTime, "", 1,
                         1); //Sets Par Time for Gambling Races ONLY!

    console->AddFunction("SetRaceEnteryFee", SetRaceEnteryFee, "", 1,
                         1); //Sets The Amount of Coins Required to Enter Race

    console->AddFunction("PutMFPlayerInCar", PutMFPlayerInCar, "", 0,
                         0); //Forces the player to respawn in their car at the beginning of this stage


    console->AddFunction("SetStatepropShadow", SetStatepropShadow, "", 2,
                         2); //Sets which element of a stateprop is the shadow and projects it onto the ground


    //
    // Objective methods
    //
    console->AddFunction("AddObjective", AddObjective, "", 1,
                         3); //Adds a new objective to the current stage (objtype)
    console->AddFunction("CloseObjective", CloseObjective, "", 0,
                         0); //Closes the current objective ()

    console->AddFunction("SetDestination", SetDestination, "", 1,
                         3); //Sets the destination of the current objective (locator name, [p3d name], [float scale factor])

    console->AddFunction("AddNPC", AddNPC, "", 2,
                         3); //Adds an NPC (name) to the position (locator name) beginning at this objective

    console->AddFunction("RemoveNPC", RemoveNPC, "", 1, 1); //Remove an NPC (name) in this objective

    console->AddFunction("AddDriver", AddDriver, "", 2,
                         2); //Adds an driver (name) int the car (other name) in this objective

    console->AddFunction("RemoveDriver", RemoveDriver, "", 1,
                         1); //Remove an driver (name) in this objective

    console->AddFunction("SetTalkToTarget", SetTalkToTarget, "", 1,
                         4); //Sets the target of the objective's conversation (name, [0 - exclamation (default), 1 - gift, 2 - Door], [y offset], [trigger radius]

    console->AddFunction("SetDialogueInfo", SetDialogueInfo, "", 4,
                         4); //Sets the members of the dialogue, the name of the dialogue sound and the length of pause

    console->AddFunction("SetDialoguePositions", SetDialoguePositions, "", 2,
                         4); //Sets the positions of the characters and the car for the dialogue

    console->AddFunction("SetRaceLaps", SetRaceLaps, "", 1,
                         1); //Sets the number of laps in the race.

    console->AddFunction("TurnGotoDialogOff", TurnGotoDialogOff, "", 0,
                         0); //Turns arrival dialog off for goto objectives.

    console->AddFunction("MustActionTrigger", MustActionTrigger, "", 0,
                         0); //Must use action button for goto objectives.

    //Chuck Coin Objectives

    console->AddFunction("SetCoinFee", SetCoinFee, "", 1,
                         1); //Set The Amount of Coins to get past a Stage

    //Timer Objectives

    console->AddFunction("SetDurationTime", SetPauseDuration, "", 1,
                         1); //Set the amount of time for the timer objective

    //
    // Collection objective methods
    //
    console->AddFunction("AddCollectible", AddCollectible, "", 1,
                         4); //Adds a new collectible to the current objective (locator name, [p3d name], [dialog event], [float scale factor])
    console->AddFunction("SetCollectibleEffect", SetCollectibleEffect, "", 1,
                         1); //Sets the collectible effect (effect name)
    console->AddFunction("BindCollectibleTo", BindCollectibleToWaypoint, "", 2,
                         2); //Binds the collectible <num> to the waypoint <num>(collect num, waypoint num)
    console->AddFunction("AllowUserDump", AllowUserDump, "", 0,
                         0); //Allows the player to lose collected object when struck by ai vehicles.

    console->AddFunction("SetVehicleToLoad", SetVehicleToLoad, "", 3,
                         3); //Starts an async load of a new player vehicle

    //
    // Follow objective methods
    //
    console->AddFunction("SetObjTargetVehicle", SetObjTargetVehicle, "", 1,
                         1); //Sets the target vehicle for a follow objective (name)
    console->AddFunction("SetObjDistance", SetObjDistance, "", 1,
                         1); //Sets the distance needed to lose the tail (distance)

    // Boss objective
    console->AddFunction("SetObjTargetBoss", SetObjTargetBoss, "", 1,
                         1); //Sets the target boss for a destroy objective

    console->AddFunction("AddCollectibleStateProp", AddCollectibleStateProp, "", 3,
                         3); //Creates a new collectible stateprop into world

    console->AddFunction("SetPickupTarget", SetPickupTarget, "", 1,
                         1); //Sets the target for a pickupitem objective


    console->AddFunction("AllowRockOut", AllowRockOut, "", 0,
                         0); //Have the PC 'rock out' when idling during this objective

    //
    // Mission Condition methods
    //
    console->AddFunction("AddCondition", AddCondition, "", 1,
                         2); //Adds a new condition to the stage (type)
    console->AddFunction("CloseCondition", CloseCondition, "", 0, 0); //Closes the current condition

    console->AddFunction("SetFollowDistances", SetFollowDistances, "", 2,
                         2); //Sets the follow condition distances (min, max)

    //
    // Follow Condition methods
    //
    console->AddFunction("SetCondTargetVehicle", SetCondTargetVehicle, "", 1,
                         1); //Sets the target vehicle for a follow condition (name)

    //
    // Position Condition methods
    console->AddFunction("SetConditionPosition", SetConditionPosition, "", 1,
                         1); //Sets the position required for the position condition on a race.

    console->AddFunction("SetCondTime", SetCondTime, "", 1,
                         1); //Sets the timeout for the out of car condition.

    console->AddFunction("SetHitNRun", SetHitNRun, "", 0,
                         0); //Turns on Hit N Run on timeout condition.

    //
    // Presentation Functions
    //
    console->AddFunction("EnableTutorialMode", EnableTutorialMode, "", 1,
                         1); //Should tutorial mode be enabled?
    console->AddFunction("SetConversationCamName", SetConversationCamName, "x", 1,
                         1); //Should tutorial mode be enabled?
    console->AddFunction("SetConversationCamPcName", SetConversationCamPcName, "", 1,
                         1); //Sets which of the conversation cameras will be used when the PC is talking
    console->AddFunction("SetConversationCamNpcName", SetConversationCamNpcName, "", 1,
                         1); //Sets which of the conversation cameras will be used when the NPC is talking
    console->AddFunction("SetConversationCam", SetConversationCam, "", 2,
                         3); //Sets the conversation cmaera for a particular line of dialog
    console->AddFunction("SetConversationCamDistance", SetConversationCamDistance, "", 2,
                         2); //Sets which of the conversation cameras will be used when the NPC is talking
    console->AddFunction("AmbientAnimationRandomize", AmbientAnimationRandomize, "", 2,
                         2); //should the animation for a particular character be randomized?
    console->AddFunction("ClearAmbientAnimations", ClearAmbientAnimations, "", 1, 1);
    console->AddFunction("AddAmbientPcAnimation", AddAmbientPcAnimation, "", 1,
                         2); //Adds an animation that will be chosen at random during conversations
    console->AddFunction("AddAmbientNpcAnimation", AddAmbientNpcAnimation, "", 1,
                         2); //Adds an animation that will be chosen at random during conversations
    console->AddFunction("CharacterIsChild", CharacterIsChild, "", 1,
                         1); //Tells the conversation camera that this character is a child, and to look down
    console->AddFunction("SetPresentationBitmap", SetPresentationBitmap, "", 1,
                         1); //this is the bitmap to use on the mission breifing screen
    console->AddFunction("SetAnimCamMulticontName", SetAnimatedCameraMulticontrollerName, "", 1,
                         1); //This is the multicontroller for the animated camera that we should use
    console->AddFunction("SetAnimatedCameraName", SetAnimatedCameraName, "", 1,
                         1); //This is the animated camera that we should use
    console->AddFunction("SetMissionStartMulticontName", SetMissionStartMulticontrollerName, "", 1,
                         1); //This is the multicontroller for the animated camera that we should use
    console->AddFunction("SetMissionStartCameraName", SetMissionStartCameraName, "", 1,
                         1); //This is the animated camera that we should use
    console->AddFunction("SetCamBestSide", SetCamBestSide, "", 1,
                         2); //This is the name of the locator that corresponds to the best side for the camera
    console->AddFunction("SetFMVInfo", SetFMVInfo, "", 1, 2); //Play the FMV specified by file name
    console->AddFunction("StartCountdown", StartCountdown, "", 1,
                         2); //start the 321 go countdown, specify dialog
    console->AddFunction("AddToCountdownSequence", AddToCountdownSequence, "", 1,
                         2); //Add message and dialog to countdown sequence
    console->AddFunction("SetCarStartCamera", SetCarStartCamera, "", 1,
                         1); //Sets the camera that will be used when a car is loaded from a phone booth
    console->AddFunction("GoToPsScreenWhenDone", GoToPattyAndSelmaScreenWhenDone, "", 0,
                         0); //go to patty and selma screen when stage done?

    //SuperSprint methods
    console->AddFunction("SetPlayerCarName", SetPlayerCarName, "", 2,
                         2); //This sets the car name for a given player

    console->AddFunction("SetCondMinHealth", SetCondMinHealth, "", 1,
                         1); //This sets the min health before failing the condition

    //
    // Loading script methods
    //
    console->AddFunction("LoadP3DFile", LoadP3DFile, "", 1,
                         3); //Loads a Pure3D file (filename, [heap name], [inventory section name])
    console->AddFunction("LoadDisposableCar", LoadDisposableCar, "", 3,
                         3); //Loads a Pure3D CAR file (filename,name,slot)


    //Setting the respawn time for wrenches and wasps and nitro etc

    console->AddFunction("SetRespawnRate", SetRespawnRate, "", 2,
                         2); //Set RespawnRate for Wrenches and Nitro in seconds,etc

    console->AddFunction("AddCharacter", AddCharacter, "", 2,
                         2); //Adds a character to the game world.  All files must still be loaded (p3d, choreo)
    console->AddFunction("AddNPCCharacterBonusMission", AddNPCCharacterBonusMission, "", 7,
                         8); //Adds an NPC character to the game world.  All files must still be loaded (p3d, choreo, loc, mission name, icon name, dialogue name, <bool>isRace, alternate icon name)
    console->AddFunction("SetBonusMissionDialoguePos", SetBonusMissionDialoguePositions, "", 3,
                         4); //Sets the positions of the characters and the car for the bonus mission dialogue (mission name, char 1 loc, char 2 loc, car loc)

    console->AddFunction("AddAmbientCharacter", AddAmbientCharacter, "", 2,
                         3); //Adds an ambient NPC character to the game world.  All files must still be loaded (p3d, loc)

    console->AddFunction("AddBonusMissionNPCWaypoint", AddBonusMissionNPCWaypoint, "", 2,
                         2); //Adds a waypoint for specified bonus mission NPC (NPCNAME, LOCATOR)
    console->AddFunction("AddObjectiveNPCWaypoint", AddObjectiveNPCWaypoint, "", 2,
                         2); //Adds a waypoint for specified mission objective NPC (NPCNAME, LOCATOR)
    console->AddFunction("AddAmbientNPCWaypoint", AddAmbientNPCWaypoint, "", 2,
                         2); //Adds a waypoint for specified ambient NPC (NPCNAME, LOCATOR)
    console->AddFunction("AddPurchaseCarNPCWaypoint", AddPurchaseCarNPCWaypoint, "", 2,
                         2); //Adds a waypoint for specified Purchase Car Reward NPC (NPCNAME, LOCATOR)

    console->AddFunction("ActivateTrigger", ActivateTrigger, "", 1,
                         1); //Activate a locator (locator name)
    console->AddFunction("DeactivateTrigger", DeactivateTrigger, "", 1,
                         1); //Deactivate a locator (locator name)

    //
    // Oh, that's a creative name. Like, duh.
    //
    console->AddFunction("CreateAnimPhysObject", CreateAnimPhysObject, "", 2,
                         2); //Creates an animated physics object (name, animname)
    console->AddFunction("CreateActionEventTrigger", CreateActionEventTrigger, "", 5,
                         5); //Creates an action event trigger CreateActionEventTrigger(const char* triggerName, rmt::Vector& pos, float r)
    console->AddFunction("LinkActionToObjectJoint", LinkActionToObjectJoint, "", 5,
                         5); //Links the Action Trigger to an object joint. LinkActionToObjectJoint(const char* objectName, const char* jointName, const char* triggerName, const char* typeName, const char* buttonName)
    console->AddFunction("LinkActionToObject", LinkActionToObject, "", 5,
                         5); //Links the Action Trigger to an object. LinkActionToObject(const char* objectName, const char* jointName, const char* triggerName, const char* typeName, const char* buttonName)
    console->AddFunction("SetCoinDrawable", SetCoinDrawable, "", 1,
                         1); //Name the drawable for the coins
    console->AddFunction("SetParticleTexture", SetParticleTexture, "", 2,
                         2); //Name of texture used for particle
...
void MissionScriptLoader::LoadScriptAsync(char *filename,
...
void MissionScriptLoader::InitLevelPlayerVehicle(int argc, char **argv) {
    ...
    if (GetGameplayManager()->mVehicleSlots[eslot].mp_vehicle == NULL) {
        ...
        if (strcmp(GetGameplayManager()->mVehicleSlots[eslot].mp_vehicle->GetName(), vehicleName) ==
            0) {
...
void MissionScriptLoader::PlacePlayerCar(int argc, char **argv) {
    ...
    if (strcmp(vehicleName, "current") == 0) {
        ...
        rTuneAssertMsg(0,
                       "was this ever getting called, 'cause it wouldn't have worked - see greg \n");
...
void MissionScriptLoader::AddPurchaseCarReward(int argc, char **argv) {
    ...
    Character *c = GetCharacterManager()->AddCharacterDeferedLoad(CharacterManager::NPC, uniqueName,
...
void MissionScriptLoader::AddMission(int argc, char **argv) {
    ...
    if (GetGameplayManager()->mIsDemo == false) {
        ...
        if (strcmp(argv[1], "m8") != 0) {
            ...
            if (GetCharacterSheetManager()->AddMission(GetGameplayManager()->GetCurrentLevelIndex(),
                                                       spInstance->mpMission->GetName()) != 0) {
...
//its a bonus mission
    if (strcmp(argv[1], "bm1") == 0) {
        if (GetCharacterSheetManager()->AddBonusMission(
                GetGameplayManager()->GetCurrentLevelIndex(), spInstance->mpMission->GetName()) !=
            0) {
            //asserted cause something failed adding mission to charactersheet.
            rAssert(0);
        }
        ...
        if (GetCharacterSheetManager()->AddGambleRace(GetGameplayManager()->GetCurrentLevelIndex(),
                                                      spInstance->mpMission->GetName()) != 0) {
            rTuneAssert(0);
        }
        ...
        if (GetCharacterSheetManager()->AddStreetRace(GetGameplayManager()->GetCurrentLevelIndex(),
                                                      spInstance->mpMission->GetName()) != 0) {
            //problems adding the streetrace to the charactersheet
            //rAssert(0);
            rReleasePrintf(
                    "Warning: Could NOT add %S into the CharacterSheet, All 3 StreetRace Slots are Full, Double Check this Mission's Name\n",
...
void MissionScriptLoader::SetMissionResetPlayerOutCar(int argc, char **argv) {
    ...
    if (playerLoc) {
        bool loading = ((GetGuiSystem()->GetBackendManager()->GetCurrentScreen() ==
...
void MissionScriptLoader::AddObjective(int argc, char **argv) {
    ...
    switch (spInstance->mObjType) {
        ...
        case MissionObjective::OBJ_BUY_CAR: {
            ...
            rTuneAssertMsg(argc == 3,
                           "You need to specify which car to buy for BuyCar Objectives\n");
        ...
        case MissionObjective::OBJ_BUY_SKIN: {
            ...
            rTuneAssertMsg(argc == 3,
                           "You need to specify which skin to buy for BuSkinr Objectives\n");
...
void MissionScriptLoader::SetTalkToTarget(int argc, char **argv) {
    ...
    reinterpret_cast<TalkToObjective *>(spInstance->mpObjective)->SetTalkToTarget(argv[1], icon,
                                                                                  yOffset, trigRad);
}
...
void MissionScriptLoader::SetDialoguePositions(int argc, char **argv) {
    ...
    static_cast<DialogueObjective *>(spInstance->mpObjective)->SetPositions(char1Pos, char2Pos,
                                                                            carPos, dontReset);
}
...
void MissionScriptLoader::SetCollectibleEffect(int argc, char **argv) {
    if (spInstance->mpObjective->GetObjectiveType() == MissionObjective::OBJ_GOTO) {
        ...
        rTuneAssertMsg(dynamic_cast<CollectibleObjective *>(spInstance->mpObjective) != NULL,
                       "The current objective cannot accept collectibles.");
        CollectibleObjective *pCol = static_cast<CollectibleObjective *>(spInstance->mpObjective);
        rAssert(pCol);
...
void MissionScriptLoader::TurnGotoDialogOff(int argc, char **argv) {
    rAssert(spInstance->mpObjective->GetObjectiveType() == MissionObjective::OBJ_GOTO ||
            spInstance->mpObjective->GetObjectiveType() == MissionObjective::OBJ_GO_OUTSIDE);
...
void MissionScriptLoader::MustActionTrigger(int argc, char **argv) {
    rAssertMsg(spInstance->mpObjective->GetObjectiveType() == MissionObjective::OBJ_GOTO,
               "MustActionTrigger can only be applied to GOTO objectives!\n");
...
*/
    ...
    if (argc >= 5) {
        ...
        if (strcmp(argv[4], "current") == 0) {
            spInstance->mpStage->AddCharacter(argv[1], locator, carlocator, dynaloadString,
                                              argv[4]);
            return;
        }
...
void MissionScriptLoader::AddCondition(int argc, char **argv) {
    ...
    rTuneAssertMsg(spInstance->mCondType != MissionCondition::COND_INVALID,
                   "This is not a valid Condition type\n");
...
void MissionScriptLoader::ClearAmbientAnimations(int argc, char **argv) {
    ...
    const int index = GetGameplayManager()->GetMissionNumByName(bonusMissionName) -
                      GameplayManager::MAX_MISSIONS;  //the bonus missions come after the regular missions
    rAssert(index >= 0);
...
//
//=============================================================================
bool MissionScriptLoader::GetDirectionalArrowType(const char *string,
                                                  DirectionalArrowEnum::TYPE *outArrowType) {
    if (string == NULL)
        return false;
...
void MissionScriptLoader::LoadP3DFile(int argc, char **argv) {
    ...
    if (argc == 4) {
        GetLoadingManager()->AddRequest(spInstance->mFileHandler, argv[1], heap, argv[3], argv[1],
                                        0, name);
    } else {
...
void MissionScriptLoader::LoadDisposableCar(int argc, char **argv) {
    ...
    if (strcmp(argv[3], "DEFAULT") == 0) {
        ...
        if (GetGameplayManager()->mVehicleSlots[GameplayManager::eDefaultCar].mp_vehicle != NULL) {
            ...
            GetLoadingManager()->AddRequest(spInstance->mFileHandler, argv[1], GMA_LEVEL_MISSION,
                                            argv[1], "Default");
            strcpy(GetGameplayManager()->mVehicleSlots[GameplayManager::eDefaultCar].filename,
                   argv[1]);
            strcpy(GetGameplayManager()->mVehicleSlots[GameplayManager::eDefaultCar].name, argv[2]);
        }

    } else if (strcmp(argv[3], "OTHER") == 0) {
        if (strcmp(GetGameplayManager()->mVehicleSlots[GameplayManager::eOtherCar].name, argv[2]) ==
            0) {
            ...
            if (GetGameplayManager()->mVehicleSlots[GameplayManager::eOtherCar].mp_vehicle !=
                ...
                strcpy(GetGameplayManager()->GetVehicleSlotFilename(GameplayManager::eOtherCar),
                       argv[1]);
                strcpy(GetGameplayManager()->GetVehicleSlotFilename(GameplayManager::eOtherCar),
                       argv[2]);
                //load pure3d car into the inventory under its filename
                GetLoadingManager()->AddRequest(spInstance->mFileHandler, argv[1],
                                                GMA_LEVEL_MISSION, argv[1], "Default");

            } else {
                strcpy(GetGameplayManager()->mVehicleSlots[GameplayManager::eOtherCar].filename,
                       argv[1]);
                strcpy(GetGameplayManager()->mVehicleSlots[GameplayManager::eOtherCar].name,
                       argv[2]);
                //load pure3d car into the inventory under its filename
                GetLoadingManager()->AddRequest(spInstance->mFileHandler, argv[1],
                                                GMA_LEVEL_MISSION, argv[1], "Default");
...
void MissionScriptLoader::AddNPCCharacterBonusMission(int argc, char **argv) {
    ...
    if (!GetGameplayManager()->mIsDemo) {
        ...
        const MissionRecord *missionRecord = GetCharacterSheetManager()->QueryStreetRaceStatus(
                GetGameplayManager()->GetCurrentLevelIndex(), argv[4]);
        if (missionRecord == NULL)
            GetCharacterSheetManager()->QueryBonusMissionStatus(
                    GetGameplayManager()->GetCurrentLevelIndex(), argv[4]);
        ...
        if (argc == 8) {
            GetGameplayManager()->SetBonusMissionInfo(uniqueName, argv[4], argv[5], argv[6], isRace,
                                                      NULL, wasMissionAlreadyCompleted);
        } else if (argc > 8) {
            GetGameplayManager()->SetBonusMissionInfo(uniqueName, argv[4], argv[5], argv[6], isRace,
                                                      argv[8], wasMissionAlreadyCompleted);
        }
    }
}
...
void MissionScriptLoader::SetBonusMissionDialoguePositions(int argc, char **argv) {
    ...
    rTuneAssertMsg(missionNum >= GameplayManager::MAX_MISSIONS,
                   "Trying to set bonus mission positions on regular mission\n");
    ...
    if (missionNum != -1 && missionNum >= GameplayManager::MAX_MISSIONS) {
        ...
        BonusMissionInfo *bmi = GetGameplayManager()->GetBonusMissionInfo(
                missionNum - GameplayManager::MAX_MISSIONS);
...
void MissionScriptLoader::AddAmbientCharacter(int argc, char **argv) {
    ...
    Character *c = GetCharacterManager()->AddCharacterDeferedLoad(CharacterManager::NPC, argv[1],
                                                                  argv[1], "npd", argv[2]);
...
// You have been assimilated.
    rTuneAssertMsg(false,
                   "No longer neccessary; loading being done uniformly now.  You have been assimilated.\n");
}
...
void MissionScriptLoader::LinkActionToObject(int argc, char **argv) {
    ...
    GetActionButtonManager()->LinkActionToObject(argv[1], argv[2], argv[3], argv[4], argv[5],
                                                 false);
}
...
void MissionScriptLoader::AddSafeZone(int argc, char **argv) {
    ...
    rTuneAssertMsg(spInstance->mpStage != NULL,
                   "This Command: AddSafeZone needs to be in a stage \n");
...
void MissionScriptLoader::AddPed(int argc, char **argv) {
    ...
    if (spInstance->mp_ModelGroup->numModels == PedestrianManager::MAX_MODELS) {
        ...
        spInstance->mp_ModelGroup->models[spInstance->mp_ModelGroup->numModels].Init(name,
                                                                                     max_number);
...
void MissionScriptLoader::ClosePedGroup(int argc, char **argv) {
    ...
    PedestrianManager::GetInstance()->SetModelGroup(spInstance->mPedGroupID,
                                                    *(spInstance->mp_ModelGroup));
...
void MissionScriptLoader::UseElapsedTime(int argc, char **argv) {
    rTuneAssertMsg(spInstance->mpStage != NULL,
                   "This Command: UseElapsedTime needs to be in a stage \n");
...
void MissionScriptLoader::SetParTime(int argc, char **argv) {
    ...
    if (spInstance->mObjType == MissionObjective::OBJ_RACE) {
        ...
        if (pRaceObjective->QueryIsGambleRace() == true) {
            ...
            rTuneAssertMsg(0,
                           "ERROR: You CANNOT set a par time  for a  NON GAMBLE Race Objective!!!!\n");
        }
    } else {
...
void MissionScriptLoader::SetRaceEnteryFee(int argc, char **argv) {
    ...
    if (enteryfee < 0) {
        rTuneAssertMsg(0,
                       "ERROR: Entery Fee MUST be at least GREATER than 1 coin, Double Check your Script!!\n");
    } else {
...
void MissionScriptLoader::PutMFPlayerInCar(int argc, char **argv) {
    rTuneAssertMsg(spInstance->mpStage != NULL,
                   "This Command: PutMFPlayerInCar needs to be in a stage \n");
...
void MissionScriptLoader::DisableHitAndRun(int argc, char **argv) {
    rTuneAssertMsg(spInstance->mpStage != NULL,
                   "This Command: DisableHitAndRun needs to be in a stage \n");
...
void MissionScriptLoader::SwapInDefaultCar(int argc, char **argv) {
    rTuneAssertMsg(spInstance->mpStage != NULL,
                   "This Command: SwapInDefaultCar needs to be in a stage \n");
...
void MissionScriptLoader::SetSwapDefaultCarRespawnLocatorName(int argc, char **argv) {
    rTuneAssertMsg(spInstance->mpStage != NULL,
                   "This Command: SetSwapDefaultCarLocator needs to be in a stage \n");
...
void MissionScriptLoader::SetSwapForcedCarRespawnLocatorName(int argc, char **argv) {
    rTuneAssertMsg(spInstance->mpStage != NULL,
                   "This Command: SetSwapForcedCarLocator needs to be in a stage \n");
...
void MissionScriptLoader::SetSwapPlayerRespawnLocatorName(int argc, char **argv) {
    rTuneAssertMsg(spInstance->mpStage != NULL,
                   "This Command: SetSwapPlayerLocator needs to be in a stage \n");
...
void MissionScriptLoader::NoTrafficForStage(int argc, char **argv) {
    rTuneAssertMsg(spInstance->mpStage != NULL,
                   "This Command:NoTrafficForStage needs to be in a stage \n");
...
void MissionScriptLoader::ClearTrafficForStage(int argc, char **argv) {
    rTuneAssertMsg(spInstance->mpStage != NULL,
                   "This Command: ClearTrafficForStage needs to be in a stage \n");
...
void MissionScriptLoader::PlacePlayerAtLocatorName(int argc, char **argv) {
    rTuneAssertMsg(spInstance->mpStage != NULL,
                   " You must USE: PlacePlayerAtLocatorName inside the Stage Command!! \n");
...
void MissionScriptLoader::msPlacePlayerCarAtLocatorName(int argc, char **argv) {
    rTuneAssertMsg(spInstance->mpStage != NULL,
                   " You must USE: msPlacePlayerCarAtLocatorName inside the Stage Command!! \n");
...
void MissionScriptLoader::SetPauseDuration(int argc, char **argv) {
    if (spInstance->mpObjective != NULL) {
        ...
        rTuneAssertMsg(0,
                       "Timer Objective has not been declared, You must use AddObjective(timer); first \n");
    }
}
...
void MissionScriptLoader::StayInBlack(int argc, char **argv) {
    rTuneAssertMsg(spInstance->mpStage != NULL,
                   "You cant use this command to a Stage that doesnt exist, Use AddStage() before this command! \n");
