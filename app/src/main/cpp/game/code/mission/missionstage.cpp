...
void
MissionStage::SetAIRaceCatchupParams(Vehicle *vehicle, const VehicleAI::RaceCatchupParams &params) {
    rAssert(vehicle);
...
void MissionStage::SetAIEvadeCatchupParams(Vehicle *vehicle,
                                           const VehicleAI::EvadeCatchupParams &params) {
    rAssert(vehicle);
...
void MissionStage::SetAITargetCatchupParams(Vehicle *vehicle,
                                            const VehicleAI::TargetCatchupParams &params) {
    rAssert(vehicle);
...
void MissionStage::AddCharacter(char *name,
    ...
    if (strcmp(dynaloadString, "") != 0) {
        mCharacters[mNumCharacters].pZoneEventLocator = new(
                GetGameplayManager()->GetCurrentMissionHeap()) ZoneEventLocator;
...
void MissionStage::Finalize() {
    ...
    if (mCountdownEnabled) {
        ...
        if (currentHud != NULL) {
            HudEventHandler *hudEventHandler = currentHud->GetEventHandler(
                    CGuiScreenHud::HUD_EVENT_HANDLER_COUNTDOWN);
...
void MissionStage::Reset() {
        ...
        for (int j = 0; j < MAX_VEHICLES; j++) {
            if (mVehicles[j].vehicle != NULL) {
                gameplayManager->MakeSureHusksAreReverted(
                        mVehicles[j].vehicle);  // make sure the same car (eg. CellA) used in multiple stages isn't left as a husk...
            }
        }
    }
    ...
    if (mbPutPlayerInCar) {
        ...
        if (GetGameplayManager()->GetCurrentMission()->IsWagerMission() ||
            GetGameplayManager()->GetCurrentMission()->IsRaceMission()) {
        } else {   //if you assert here, tell chuck.
    ...
    for (i = 0; i < MAX_CHASE_STRUCTS; i++) {
        ...
        if (strcmp(m_ChaseData_Array[i].vehiclename, "NULL") != 0) {
            ...
            p_chasemanager = GetGameplayManager()->GetChaseManager(
                    m_ChaseData_Array[i].vehiclename);
        ...
        if (mCharacters[i].vehicle != NULL) {
            ...
            if (mCharacters[i].locator == NULL) {
                ...
                if (mCharacters[i].character->IsInCarOrGettingInOut()) {
                    GetAvatarManager()->PutCharacterOnGround(mCharacters[i].character,
                                                             GetGameplayManager()->GetCurrentVehicle());
                }
    ...
    if (mNumCharacters == 0 && GetGameplayManager()->ShouldPutPlayerInCar()) {
        ...
        GetGameplayManager()->PlaceVehicleAtLocation(veh, pos,
                                                     -rotation);  //The car and characters are reversed.
    ...
    for (j = 0; j < mNumConditions; j++) {
        ...
        if (mConditions[j]->GetType() == MissionCondition::COND_PLAYER_OUT_OF_VEHICLE) {
            if (GetGameplayManager()->GetCurrentMission()->GetCarryOverOutOfCarCondition() ==
                true) {
        ...
        if (mConditions[j]->GetType() == MissionCondition::COND_FOLLOW_DISTANCE) {
            GetGuiSystem()->HandleMessage(GUI_MSG_SHOW_HUD_OVERLAY, HUD_PROXIMITY_METER,
                                          1 /* 1 = green bar */);
        }
    }
...
//        GetGuiSystem()->HandleMessage(GUI_MSG_SHOW_HUD_OVERLAY, HUD_COUNT_DOWN);
    ...
    CGuiScreenHud *currentHud = GetCurrentHud();
    if (currentHud != NULL) {
        if (mb_UseElapsedTime &&
            this->GetObjective()->GetObjectiveType() == MissionObjective::OBJ_RACE) {
            ...
            currentHud->SetTimerBlinkingInterval(parTimeInMilliseconds - 10000,
                                                 parTimeInMilliseconds);
        } else {
    ...
    if (strcmp(mPlayerRespawnLocatorName, "NULL") != 0) {
        ...
        if (pCharacter->IsInCarOrGettingInOut() == true) {
            ...
            rTuneAssertMsg(playerLoc != NULL,
                           "Cant Find Locator to Player for PlacePlayerAtLocatorName!\n");
    ...
    if (strcmp(mPlayerCarRespawnLocatorName, "NULL") != 0) {
        ...
        pVehicle = GetGameplayManager()->GetCurrentVehicle();
        rTuneAssertMsg(pVehicle != NULL,
                       "Error: with msPlacePlayerCarAtLocatorName(); Player does NOT currently have a vehicle! \n");
    ...
    for (i = 0; i < mNumVehicles; i++) {
        ...
        if (mVehicles[i].vehicle != NULL) {
            ...
            if (!GetGameplayManager()->TestForContinuityErrorWithCar(mVehicles[i].vehicle, false)) {
                mVehicles[i].vehicleAINum = GetVehicleCentral()->AddVehicleToActiveList(
                        mVehicles[i].vehicle);
    ...
    if (mObjective->GetObjectiveType() == MissionObjective::OBJ_RACE) {
        ...
        if (pRaceObjective->QueryIsGambleRace() == true && !mbRacePaidOut) {
            ...
            if ((elapsedtime <= partime) && mObjective->IsFinished()) {
                ...
                GetCoinManager()->AdjustBankValue((int) (rmt::LtoF(mRaceEnteryFee) + (rmt::FtoL(
                        GetGameplayManager()->GetCurrentVehicle()->GetGamblingOdds() *
                        rmt::LtoF(mRaceEnteryFee)))));
                ...
                int currentBestTime = GetCharacterSheetManager()->GetGambleRaceBestTime(
                        currentLevel);
    ...
    if (p_vehicle != NULL) {
        ...
        for (int i = 0; i < MAX_SAFEZONES; i++) {
            ...
            if (m_SafeZone_Array[i] != NULL) {
                //if true and false
                if (m_SafeZone_Array[i]->InsideZone(player_position) &&
                    (mb_InsideSafeZone == false)) {
                ...
                //if false and true
                if ((m_SafeZone_Array[i]->InsideZone(player_position) == false) &&
                    (mb_InsideSafeZone)) {
    ...
    switch (id) {
        case EVENT_GUI_MISSION_START: {
            ...
            if (GetGameplayManager()->GetCurrentVehicle() != NULL) {
                ...
                VehicleController *controller = GetVehicleCentral()->GetVehicleController(
                        GetVehicleCentral()->GetVehicleId(
                                GetGameplayManager()->GetCurrentVehicle()));
                if (controller) {
                    rAssert(dynamic_cast<HumanVehicleController *>(controller));
                    static_cast<HumanVehicleController *>(controller)->SetDisableReset(false);
...
//=============================================================================
void
MissionStage::SetLockRequirement(unsigned int which, LockRequirement::Type type, const char *name) {
    ...
    strncpy(mRequirement[which].mName, name,
            (nameLen < LockRequirement::MAX_NAME_LEN) ? nameLen : LockRequirement::MAX_NAME_LEN);
...
void MissionStage::SetCountdownEnabled(radKey32 dialogID, tUID secondSpeakerUID) {
    ...
    mCountdownSequenceUnits = new(
            GMA_LEVEL_MISSION) CountdownSequenceUnit[MAX_NUM_COUNTDOWN_SEQUENCE_UNITS];
    rAssert(mCountdownSequenceUnits != NULL);
...
void MissionStage::SwapInDefaultCarStart() {
    ...
    if (pVehicle == GetGameplayManager()->mVehicleSlots[GameplayManager::eOtherCar].mp_vehicle) {
        ...
        if (pCharacter->IsInCarOrGettingInOut() == true) {
            ...
            rTuneAssertMsg(playerLoc != NULL,
                           "Cant Find Locator to Spawn Player after Car SWAP !\n");
            GetGameplayManager()->PlaceCharacterAtLocator(pCharacter, playerLoc);
        }
...
void MissionStage::OnProcessRequestsComplete(void *pUserData) {
    ...
    GetGameplayManager()->mVehicleSlots[GameplayManager::eOtherCar].mp_vehicle->SetUserDrivingCar(
            false);
    ...
    Vehicle *vehicle;
    char conName[64];
    sprintf(conName, "%s.con",
            GetGameplayManager()->mVehicleSlots[GameplayManager::eDefaultCar].name);
    vehicle = GetGameplayManager()->AddLevelVehicle(
            GetGameplayManager()->mVehicleSlots[GameplayManager::eDefaultCar].name,
            GameplayManager::eDefaultCar, conName);
