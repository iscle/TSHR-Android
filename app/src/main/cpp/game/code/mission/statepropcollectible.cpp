...
StatePropCollectible::StatePropCollectible() :
    ...
    if (wasAdded == false) {
        radDbgWatchAddFloat(&STATEPROP_SHAKE_FORCE, "Stateprop shake force", "Stateprop", NULL,
                            NULL, 0, 400.0f);
        wasAdded = true;
    }
#endif
...
sim::Solving_Answer
StatePropCollectible::PreReactToCollision(sim::SimState *pCollidedObj,
    ...
    if (pCollidedObj->mAIRefIndex == PhysicsAIRef::redBrickVehicle) {
        ...
        if (vehicle->IsUserDrivingCar()) {
            ...
            if (linearVelocity.MagnitudeSqr() < 100.0f) {
                if (vehicle->AttachCollectible(this)) {   // It attached ok.
                    rmt::Matrix origin;
                    origin.Identity();
...
void StatePropCollectible::LoadSetup(CStatePropData *statePropData,
    ...
    StatePropDSG::LoadSetup(statePropData, startState, transform, ipCollAttr, isDynaLoaded,
                            ipSearchStore, useSharedtPose, collisionObject, physicsObject);
...
void StatePropCollectible::AddToDSG() {
    if (m_IsInDSG == false) {
        WorldRenderLayer *wrl = static_cast<WorldRenderLayer *>(GetRenderManager()->mpLayer(
...
void StatePropCollectible::EnableHudIcon(bool enable) {
    ...
    if (enable) {
        if (m_HudIcon == -1)
            m_HudIcon = currentHud->GetHudMap(0)->RegisterIcon(HudMapIcon::ICON_COLLECTIBLE,
                                                               mTransform.Row(3), this, true);
    } else {
        if (m_HudIcon != -1) {
