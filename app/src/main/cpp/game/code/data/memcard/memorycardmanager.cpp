...
MemoryCardManager::LoadMemcardInfo(IMemoryCardInfoLoadCallback *callback) {
    ...
    if (m_memcardInfoLoadState == MEMCARD_INFO_NOT_LOADED) {
        ...
        rTuneWarningMsg(false,
                        "WARNING: *** Memory card info already loaded! Ignoring request to load again.\n");
    }
#endif
}
...
MemoryCardManager::UnloadMemcardInfo() {
    if (m_memcardInfoLoadState == MEMCARD_INFO_LOAD_COMPLETED) {
        ...
        rTuneWarningMsg(false,
                        "WARNING: *** Memory card info not completely loaded yet! Ignoring request to unload.\n");
    }
}
...
MemoryCardManager::IsCurrentDrivePresent(unsigned int currentDriveIndex) {
    ...
    if (static_cast<int>(currentDriveIndex) != -1) {
        isMissing = (m_mediaInfos[currentDriveIndex].m_MediaState ==
                     IRadDrive::MediaInfo::MediaNotPresent);
    }
    return !isMissing;
}

bool
MemoryCardManager::IsCurrentDriveReady(bool forceSyncUpdate, bool *unformatted,
                                       IRadDrive::MediaInfo::MediaState *errorOut) {
    bool isReady = false;
    ...
    if (currentDriveIndex != -1) {
        ...
        if (m_mediaInfos[currentDriveIndex].m_MediaState ==
            IRadDrive::MediaInfo::MediaNotFormatted) {
        ...
        if (m_mediaInfos[ currentDriveIndex ].m_MediaState==IRadDrive::MediaInfo::MediaEncodingErr)
            card_unformatted = true;
#endif
        isReady = (m_mediaInfos[currentDriveIndex].m_MediaState ==
                   IRadDrive::MediaInfo::MediaPresent);
        if (unformatted)
...
MemoryCardManager::DetermineSavedGameCreationSize(unsigned int driveIndex) {
    ...
    {
        rAssert(driveIndex < NUM_SAVE_GAME_DRIVES);
        m_savedGameCreationSize = m_pDrives[driveIndex]->GetCreationSize(m_memcardInfo,
                                                                         GetGameDataManager()->GetGameDataSize());
    ...
    for (i = 0; i <pal->numDescriptors; i++)
    {
        if(pal->descriptorArray[ i ].textureHeader)
        {
            ...
            pal->descriptorArray[ i ].textureHeader =
                reinterpret_cast<TEXHeaderPtr>(((u32) pal->descriptorArray[i].textureHeader) +
                    ((u32) pal)
);
...
)
            {
                pal->descriptorArray[i].textureHeader->data =
                    reinterpret_cast<Ptr>(((u32) pal->descriptorArray[ i ].textureHeader->data) +
                        ((u32) pal)
);
