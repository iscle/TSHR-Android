...
short PersistentWorldManager::GetPersistentObjectID(tUID Sector, tUID ObjectName) {
    ...
    int i = 0;
    for (; i < NUM_SECTORS; ++i) {
        if (sSectorMap[sectorIndex].Sector ==
            (unsigned short) (Sector & static_cast<tUID>(0xFFFF))) {
            mCurSectorLoad = sectorIndex;
            if (CheckObject(sectorIndex, sSectorMap[sectorIndex].CurIndex)) {
                rAssert(static_cast<unsigned char>(sSectorMap[sectorIndex].CurIndex) <
                        NUM_PERSISTENT_OBJECTS); // Out of bits for this zone/rail.
    ...
    rAssert(i <
            NUM_SECTORS); // Oh-oh. We requested an ID for an object in a sector we aren't tracking.
    return rv;
}
...
=============================================================================*/
bool
PersistentWorldManager::CheckObject(unsigned char SectorIndex, unsigned char ObjectIndex) const {
    rAssert(SectorIndex < NUM_PERSISTENT_SECTORS);
    rAssert(ObjectIndex < NUM_PERSISTENT_OBJECTS);
    unsigned char bitMask = 1 << (ObjectIndex & 0x7);
