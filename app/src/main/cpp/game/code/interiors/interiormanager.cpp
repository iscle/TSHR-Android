...
//            sphere->radius += 10.0f;
    ...
    void SetPosition(bool u, const rmt::Vector &v) {
        ...
        if (useTranslate && mpSimStateObj) {
            ...
            if (mPose) {
                ...
                mPose->GetJoint(0)->SetWorldTranslation(
                        mPose->GetJoint(0)->GetWorldTranslation() + mPosn);
            }
    ...
    void SimUpdate(float timeins) {
        if (mPose) {
            mPose->Assign(((tCompositeDrawable *) mpDrawstuff)->GetPose());
            mPose->GetJoint(0)->SetObjectTranslation(
                    mPose->GetJoint(0)->GetObjectTranslation() + mPosn);
            mpSimStateObj->GetCollisionObject()->SetHasMoved(true);
            mpSimStateObj->GetCollisionObject()->Update();
        }
    }
...
class Gag
        : public ActionButton::ButtonHandler,
          public AnimationPlayer::LoadDataCallBack,
          public EventListener,
          public NISSoundPlaybackCompleteCallback {
public:
    void Render(void) {
        gagPlayer->Render();
    }
    ...
    void Unload(void) {
        if (draw) {
            ((WorldRenderLayer *) GetRenderManager()->mpLayer(
                    RenderEnums::LevelSlot))->pWorldScene()->Remove(draw);
        }
    ...
    void Update(unsigned int elapsedTime) {
        ...
        bool finished = (binding->gagFMVFileName[0] == 0) ? gagPlayer->IsFinished()
                                                          : !GetPresentationManager()->GetFMVPlayer()->IsPlaying();
...
protected:
    ...
    void PlayDialog(radKey32 DialogID) {
        rAssert(strlen(binding->dialogChar1) != 0 &&
                strlen(binding->dialogChar1) < DialogueObjective::MAX_CHAR_NAME_LEN);
        rAssert(strlen(binding->dialogChar2) != 0 &&
                strlen(binding->dialogChar2) < DialogueObjective::MAX_CHAR_NAME_LEN);
        ...
        GetEventManager()->TriggerEvent(EVENT_CONVERSATION_INIT_DIALOG,
                                        (void *) (&mDialogEventData));
    ...
    void Play(void) {
        ...
        if (binding->persistIndex != -1) {
            alreadyPlayed = GetCharacterSheetManager()->QueryGagViewed(
                    GetGameplayManager()->GetCurrentLevelIndex(), binding->persistIndex);
            if (!alreadyPlayed) {
                GetCharacterSheetManager()->AddGagViewed(
                        GetGameplayManager()->GetCurrentLevelIndex(), binding->persistIndex);
...
//                    GetSoundManager()->PlayNISSound(binding->instructSoundID, &box);
            ...
            if (binding->gagFMVFileName[0] != 0) {
                GetPresentationManager()->PlayFMV(binding->gagFMVFileName, NULL, true, false,
                                                  false);
            }
        }
        ...
        if (binding->cameraShake) {
            mTimeToCameraShake = (binding->shakeDelay == 0.0f) ? 1 : int(
                    binding->shakeDelay * 1000.0f);
            mTimeToCameraShakeEnd = (binding->shakeDuration == 0.0f) ? 0 : int(
                    (binding->shakeDelay + binding->shakeDuration) * 1000.0f);
        }
...
public:
    ...
    void OnLoadDataComplete(void) {
        ...
        ((WorldRenderLayer *) GetRenderManager()->mpLayer(
                RenderEnums::LevelSlot))->pWorldScene()->Add(draw);
...
GagDrawable::GagDrawable(Gag *g) : gag(g), mPose(NULL) {
    ...
    if (g->collObj) {
        ...
        if (cd && gag->binding->animBV) {
            ...
            SetSimState(
                    sim::SimStateArticulated::CreateSimStateArticulated(mPose, g->collObj, NULL));
...
void GagDrawable::Display() {
    ...
    if ((sparkleStrength > 0.0f) && gag->sparkle) {
        ...
        GetSparkleManager()->AddGagSparkle(gag->binding->triggerPos, size, sparkleStrength,
                                           (unsigned int) this);
    }
}
...
void InteriorManager::OnBootupStart() {
    ...
    GetEventManager()->AddListener(this, static_cast<EventEnum>(EVENT_LOCATOR +
                                                                LocatorEvent::INTERIOR_EXIT));
...
void
InteriorManager::Enter(InteriorEntranceLocator *entry, Character *pCharacter, Sequencer *pSeq) {
    //
    ...
    pAction = new TriggerEventAction(EVENT_ENTER_INTERIOR_TRANSITION_START,
                                     (void *) &mLoadedInteriorUID);
    pSeq->AddAction(pAction);
    pSeq->EndSequence();
...
void InteriorManager::ConsoleGagSetTrigger(int argc, char **argv) {
    ...
    if (argc == 4) {
        ...
        GetInteriorManager()->GetBuildBinding()->triggerPos.Set((float) atof(argv[2]),
                                                                (float) atof(argv[3]),
                                                                (float) atof(argv[4]));
    }
...
void InteriorManager::ConsoleGagSetPosition(int argc, char **argv) {
    ...
    if (argc == 2) {
        ...
        GetInteriorManager()->GetBuildBinding()->gagPos.Set((float) atof(argv[1]),
                                                            (float) atof(argv[2]),
                                                            (float) atof(argv[3]));
    }
}
...
void InteriorManager::ConsoleGagSetCameraShake(int argc, char **argv) {
    ...
    if (argc == 4) {
        ...
        GetInteriorManager()->GetBuildBinding()->shake.looping =
                GetInteriorManager()->GetBuildBinding()->shakeDuration != 0.0f;
    }
}
...
void InteriorManager::ConsoleGagCheckCollCards(int argc, char **argv) {
    ...
    unsigned int len =
            strlen(argv[1]) < DialogueObjective::MAX_CHAR_NAME_LEN - 1 ? strlen(argv[1]) :
            DialogueObjective::MAX_CHAR_NAME_LEN - 2;
    strncpy(GetInteriorManager()->GetBuildBinding()->dialogChar1, argv[1], len);
    GetInteriorManager()->GetBuildBinding()->dialogChar1[len] = '\0';

    len = strlen(argv[2]) < DialogueObjective::MAX_CHAR_NAME_LEN - 1 ? strlen(argv[2]) :
          DialogueObjective::MAX_CHAR_NAME_LEN - 2;
...
void InteriorManager::SetupLightsAndAnims() {
    ...
    if (pLightGroup) {
        ...
        for (int i = 0; i < pLightGroup->GetNumLights(); ++i) {
            ...
            rAssert(light != NULL);
            if (light != NULL) {
                GetRenderManager()->mpLayer(RenderEnums::LevelSlot)->pView(PLAYER_ONE)->AddLight(
                        light);
            }
        }
    }
...
void InteriorManager::AttemptEntry() {
    ...
    } else if (mEntryRequested && mInteriorLoaded && mLoadedGags &&
