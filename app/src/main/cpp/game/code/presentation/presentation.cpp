...
PresentationManager::~PresentationManager() {
    ...
    delete mNISPool;
    mNISPool = NULL;
    delete mTransitionPool;
    mTransitionPool = NULL;
    delete mpNISPlayer;
    mpNISPlayer = NULL;
    delete mpCameraPlayer;
    mpCameraPlayer = NULL;
    delete mpTransitionPlayer;
    mpTransitionPlayer = NULL;
    delete mpFMVPlayer;
    mpFMVPlayer = NULL;
...
void PresentationManager::Finalize() {
    ...
    delete mp_PCAnimator;
    mp_PCAnimator = NULL;
    delete mp_NPCAnimator;
    mp_NPCAnimator = NULL;
}
...
void PresentationManager::PlayFMV(const char *FileName,
    ...
    if (mOverlay) {
        ...
        // Hide the HUD
        //
        CGuiScreenHud *hud = dynamic_cast<CGuiScreenHud *>(GetGuiSystem()->GetInGameManager()->FindWindowByID(
...
//
//=============================================================================
void PresentationManager::QueueFMV(FMVEvent **pFMVEvent,
                                   PresentationEvent::PresentationEventCallBack *pCallback) {
...
//
//=============================================================================
void PresentationManager::QueueNIS(NISEvent **pNISEvent,
                                   PresentationEvent::PresentationEventCallBack *pCallback) {
...
//=============================================================================
const tName
PresentationManager::GetCameraTargetForLineOfDialog(const unsigned int lineOfDialog) const {
...
*/
        ...
        case EVENT_CONVERSATION_INIT: {
            ...
            //
            SuperCam *camera = GetSuperCamManager()->GetSCC(0)->GetSuperCam(
                    SuperCam::CONVERSATION_CAM);
            ...
            if (GetSuperCamManager()->GetSCC(PLAYER_ONE)->AllowCameraToggle()) {
                if (GetGameplayManager()->GetCurrentMission()->DialogueCharactersTeleported()) {
                    GetSuperCamManager()->GetSCC(PLAYER_ONE)->SelectSuperCam(
                            SuperCam::CONVERSATION_CAM, SuperCamCentral::CUT, 0);
                } else {
                    GetSuperCamManager()->GetSCC(PLAYER_ONE)->SelectSuperCam(
                            SuperCam::CONVERSATION_CAM, 0);
                }
                GetSuperCamManager()->GetSCC(PLAYER_ONE)->AddTarget(
                        mp_NPCAnimator->GetCharacter()->GetTarget());
            }
            ...
            rAssert(npc);
            static_cast<NPCController *>(npc->GetController())->TransitToState(
        ...
        case EVENT_CONVERSATION_DONE: {
            ...
            if (GetGameplayManager()->GetCurrentMission()->DialogueCharactersTeleported()) {
                GetSuperCamManager()->GetSCC(PLAYER_ONE)->SelectSuperCam(mOldCamIndexNum,
