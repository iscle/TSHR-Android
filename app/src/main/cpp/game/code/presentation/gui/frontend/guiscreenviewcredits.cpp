                ...
                for (int i = 0; i < NUM_USER_INPUT_HANDLERS; i++) {
                    ...
                    if (userInputHandler != NULL) {
...
#endif
                        {
                            ...
                            //
                            isScrollingDone = this->ScrollCredits(
                                    -(1.0f + CREDITS_MANUAL_SCROLL_FACTOR) * currentScrollAmount);
                        }
                        ...
                        if (!m_playKKDialog) // only allow speed-up if dialog is not playing
                        {
...
#endif
                            {
                                ...
                                //
                                isScrollingDone = this->ScrollCredits(
                                        (CREDITS_MANUAL_SCROLL_FACTOR - 1.0f) *
                                        currentScrollAmount);
                            }
                        }
                    }
...
void CGuiScreenViewCredits::InitIntro() {
    ...
    m_playKKDialog = GetCheatInputSystem()->IsCheatEnabled(CHEAT_ID_PLAY_CREDITS_DIALOG) ||
                     GetCharacterSheetManager()->QueryMissionStatus(RenderEnums::L7,
                                                                    RenderEnums::M7)->mCompleted;
