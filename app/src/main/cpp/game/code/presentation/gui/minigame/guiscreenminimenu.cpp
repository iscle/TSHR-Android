...
//    m_optionsMenu(NULL),
    ...
    if (m_numUnlockedVehicles == 0) {
        m_unlockedVehicles[0] = GetRewardsManager()->GetReward(RenderEnums::L1,
                                                               Reward::eDefaultCar);
    ...
    for (int p = 0; p < SuperSprintData::NUM_PLAYERS; p++) {
        ...
        m_playerMenus[p].m_pMenu[PlayerMenu::MENU_VEHICLES]->SetHighlightColour(false,
                                                                                tColour(0, 0, 0));
//        m_playerMenus[ p ].m_pMenu[ PlayerMenu::MENU_VEHICLES ]->SetSelectionMadeOutlineColour(tColour(0, 0, 0, 192));
        ...
        m_playerMenus[p].m_pMenu[PlayerMenu::MENU_VEHICLES]->SetSelectionValueCount(0,
                                                                                    m_numUnlockedVehicles);
                ...
                if (m_numActivePlayers == 0) // track selection state
                {
                    ...
                    if (m_trackNumLapsArrowU != NULL && m_trackNumLapsArrowD != NULL) {
                        ...
                        for (int i = 0; i < numUserInputHandlers; i++) {
                            CGuiUserInputHandler *userInputHandler = GetGuiSystem()->GetUserInputHandler(
                                    i);
            ...
            case GUI_MSG_CONTROLLER_UP: {
                if (m_numActivePlayers == 0) // track selection state
                {
                    ...
                    if (currentNumLaps < SuperSprintData::MAX_NUM_LAPS) {
                        ...
                        if (currentNumLaps == SuperSprintData::MAX_NUM_LAPS &&
                            m_trackNumLapsArrowU != NULL) {
                            m_trackNumLapsArrowU->SetAlpha(
                                    NUM_LAPS_ARROW_GREY_OUT_ALPHA); // grey out arrow
                        }
...
void CGuiScreenMiniMenu::InitIntro() {
    ...
    if (!CommandLineOptions::Get(CLO_SKIP_FE)) {
        for (int i = 0; i < NUM_TRACKS; i++) {
            bool isTrackUnlocked = GetCardGallery()->IsCardDeckComplete(
        ...
        if (m_playerMenus[p].IsActive()) {
            for (int i = 0; i < NUM_CHARACTERS; i++) {
                ...
                if (i != currentSelection) {
                    ...
                    m_playerMenus[p].m_pMenu[PlayerMenu::MENU_CHARACTERS]->SetMenuItemEnabled(i,
                                                                                              isEnabled);
                }
            }
        }
...
CGuiScreenMiniMenu::OnTrackSelected() {
    ...
    //
    RenderEnums::LevelEnum levelIndex = static_cast<RenderEnums::LevelEnum>(RenderEnums::B01 +
                                                                            currentTrack);
...
CGuiScreenMiniMenu::OnCharacterSelected(int playerID, bool isSelected) {
    ...
    m_playerMenus[playerID].m_pMenu[PlayerMenu::MENU_CHARACTERS]->SetSelectionValue(characterIndex,
                                                                                    isSelected
                                                                                    ? CHARACTER_SELECTED
...
CGuiScreenMiniMenu::PrepareVehicleSelections() {
    for (int level = 0; level < RenderEnums::numLevels; level++) {
        ...
        for (pReward = GetRewardsManager()->FindFirstMerchandise(level,
                                                                 Merchandise::SELLER_SIMPSON);
             pReward != NULL;
             pReward = GetRewardsManager()->FindNextMerchandise(level,
                                                                Merchandise::SELLER_SIMPSON)) {
            this->AddRewardVehicle(pReward);
        }
    }
...
CGuiScreenMiniMenu::UpdateVehicleRating(int playerID) {
    if (m_playerMenus[playerID].m_vehicleRating != NULL) {
        int vehicleIndex = m_playerMenus[playerID].m_pMenu[PlayerMenu::MENU_VEHICLES]->GetSelectionValue(
                0);

        rAssert(m_unlockedVehicles[vehicleIndex] != NULL);
        CarAttributeRecord *carStats = GetRewardsManager()->GetCarAttributeRecord(
                m_unlockedVehicles[vehicleIndex]->GetName());
        if (carStats != NULL) {
            int rating = (int) (GetRewardsManager()->ComputeOverallCarRating(carStats));

            rAssertMsg(rating >= 0 &&
                       rating < m_playerMenus[playerID].m_vehicleRating->GetNumOfImages(),
