...
void CGuiScreenMultiHud::HandleMessage
    ...
    switch (message) {
        ...
        case GUI_MSG_DEATH_VOLUME_START: {
            ...
            //
            if (g_TimeSinceLastDeathVolume > WHITE_WASH_IN_TIME + 1000.0 &&
...
//        m_speedDigital[ i ].SetColour(tColour(255, 255, 255));
        ...
        for (unsigned int j = 0; j < MAX_NUM_ICONS_PER_TYPE; j++) {
            ...
            m_hudMap[i]->AddIconToInventory(HudMapIcon::ICON_MISSION,
                                            missions->GetSprite(objectName));

            sprintf(objectName, "IconCollectible%d_%d", i, j);
            m_hudMap[i]->AddIconToInventory(HudMapIcon::ICON_COLLECTIBLE,
                                            collectables->GetSprite(objectName));

            sprintf(objectName, "IconPlayerCar%d_%d", i, j);
            m_hudMap[i]->AddIconToInventory(HudMapIcon::ICON_PLAYER_CAR,
                                            playerCars->GetSprite(objectName));
            ...
            m_hudMap[i]->AddIconToInventory(HudMapIcon::ICON_AI_HIT_N_RUN,
                                            aiHarassCars->GetSprite(objectName));

            sprintf(objectName, "IconWaypoint%d_%d", i, j);
            m_hudMap[i]->AddIconToInventory(HudMapIcon::ICON_FLAG_WAYPOINT,
                                            waypointFlags->GetSprite(objectName));

            sprintf(objectName, "IconChecker%d_%d", i, j);
            m_hudMap[i]->AddIconToInventory(HudMapIcon::ICON_FLAG_CHECKERED,
                                            checkeredFlags->GetSprite(objectName));

            sprintf(objectName, "BonusMission%d_%d", i, j);
            m_hudMap[i]->AddIconToInventory(HudMapIcon::ICON_BONUS_MISSION,
                                            bonusMissions->GetSprite(objectName));

            sprintf(objectName, "StreetRace%d_%d", i, j);
            m_hudMap[i]->AddIconToInventory(HudMapIcon::ICON_STREET_RACE,
                                            streetRaces->GetSprite(objectName));

            sprintf(objectName, "WagerRace%d_%d", i, j);
            m_hudMap[i]->AddIconToInventory(HudMapIcon::ICON_WAGER_RACE,
                                            wagerRaces->GetSprite(objectName));

            sprintf(objectName, "PhoneBooth%d_%d", i, j);
            m_hudMap[i]->AddIconToInventory(HudMapIcon::ICON_PHONE_BOOTH,
                                            phoneBooths->GetSprite(objectName));

            sprintf(objectName, "PurchaseCentre%d_%d", i, j);
            m_hudMap[i]->AddIconToInventory(HudMapIcon::ICON_PURCHASE_CENTRE,
                                            purchaseCentres->GetSprite(objectName));
        }
