...
CGuiScreenMemoryCard::UpdateDeviceList(bool forceUpdate) {
    ...
    if (m_numAttachedDevices > 0) {
        currentSelectedDrive = m_availableDrives[m_pMenu->GetSelectionValue(
                MENU_ITEM_MEMORY_DEVICE)];
    }
...
//    this->SetButtonVisible(BUTTON_ICON_BACK, !memoryDevicesAvailable);
    ...
    for (int i = 0; i < numAvailableDrives; i++) {
        ...
        Scrooby::Text *memoryDeviceText = dynamic_cast<Scrooby::Text *>(m_pMenu->GetMenuItem(
                MENU_ITEM_MEMORY_DEVICE)->GetItemValue());
    ...
    if (memoryDevicesAvailable) {
        ...
        m_pMenu->GetMenuItem(MENU_ITEM_MEMORY_DEVICE)->m_attributes &= ~SELECTABLE;
        m_pMenu->SetSelectionValueCount(MENU_ITEM_MEMORY_DEVICE,
                                        1); // to disable left/right scrolling
    }
...
CGuiScreenMemoryCard::UpdateFreeSpace(unsigned int currentDriveIndex) {
    ...
    if (((int) currentDriveIndex < m_numAttachedDevices) &&
        m_mediaInfos[currentDriveIndex]->m_MediaState != IRadDrive::MediaInfo::MediaPresent) {
        m_memStatusText->SetVisible(true);
        m_memStatusText->SetIndex(
                (m_mediaInfos[currentDriveIndex]->m_MediaState -
                 IRadDrive::MediaInfo::MediaNotFormatted) * 3
                + PLATFORM_TEXT_INDEX);
        bool disable = true;
    ...
    } else // no error
    {
        if (((int) currentDriveIndex < m_numAttachedDevices) &&
            ...
            int memcard_driveindex = GetMemoryCardManager()->GetDriveIndex(
                    m_availableDrives[currentDriveIndex]);
            rTuneAssert(memcard_driveindex >= 0);
            if (GetMemoryCardManager()->EnoughFreeSpace(memcard_driveindex) == false
                && GetGameDataManager()->DoesSaveGameExist(m_availableDrives[currentDriveIndex],
                                                           false) ==
                   false) // check for full without save game
            {
