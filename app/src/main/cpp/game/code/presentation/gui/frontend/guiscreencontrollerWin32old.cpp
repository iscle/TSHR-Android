...
const tColour DEFAULT_INPUTMAPPED_ITEM_COLOUR(255, 0,
                                              0);   // the same as in guiscreenmemorycard.cpp
...
CGuiScreenController::CGuiScreenController
    ...
    // Add main menu items
    SetSingleItemMenuPage(m_pMenuLabels, NUM_MAINMENU_LABELS, szMainControllerPage, "Menu", "Label",
                          ALL_ATTRIBUTES_ON);
    m_numControllerGroups = 1; // The first page is the MENU_PAGE_MAIN with an index of 0.

    // Add master list items
    SetSingleItemMenuPage(m_pKMLabels[MASTER_LIST], NUM_KEYBOARDMOUSE_LABELS, szKeyboardMousePage,
                          "Menu", "Label", ALL_ATTRIBUTES_OFF);
    SetSingleItemMenuPage(m_pGamepadLabels[MASTER_LIST], NUM_GAMEPAD_LABELS, szGamepadPage, "Menu",
                          "Label", ALL_ATTRIBUTES_OFF);
    // Add map items
    SetSingleItemMenuPage(m_pKMLabels[MAP_PRIMARY], NUM_KEYBOARDMOUSE_LABELS, szKeyboardMousePage,
                          "Map1", "Map1Label");
    SetSingleItemMenuPage(m_pKMLabels[MAP_SECONDARY], NUM_KEYBOARDMOUSE_LABELS, szKeyboardMousePage,
                          "Map2", "Map2Label");
    SetSingleItemMenuPage(m_pGamepadLabels[MAP_PRIMARY], NUM_GAMEPAD_LABELS, szGamepadPage, "Map1",
                          "Map1Label");
    SetSingleItemMenuPage(m_pGamepadLabels[MAP_SECONDARY], NUM_GAMEPAD_LABELS, szGamepadPage,
                          "Map2", "Map2Label");
    ...
    if (m_state == GUI_WINDOW_STATE_RUNNING) {
        switch (message) {
                ...
                switch (m_currentPage) {
                    ...
                    case MENU_PAGE_KM: {
                        ...
                        if (menuItem == m_menuGroupStartIndex[m_currentPage]) {
                            m_pMenu->Reset(m_menuGroupStartIndex[m_currentPage] +
                                           (NUM_KEYBOARDMOUSE_LABELS - 1));
                            GetEventManager()->TriggerEvent(EVENT_FE_MENU_UPORDOWN);
                            return;
                        } else if (menuItem == m_menuGroupStartIndex[m_currentPage] +
                                               NUM_KEYBOARDMOUSE_LABELS) {
                            m_pMenu->Reset(m_menuGroupStartIndex[m_currentPage] +
                                           (NUM_KEYBOARDMOUSE_LABELS * 2) - 1);
                            GetEventManager()->TriggerEvent(EVENT_FE_MENU_UPORDOWN);
                            return;
                        }
                    }
                    ...
                    case MENU_PAGE_GAMEPAD: {
                        ...
                        if (menuItem == m_menuGroupStartIndex[m_currentPage]) {
                            m_pMenu->Reset(m_menuGroupStartIndex[m_currentPage] +
                                           (NUM_GAMEPAD_LABELS - 1));
                            GetEventManager()->TriggerEvent(EVENT_FE_MENU_UPORDOWN);
                            return;
                        } else if (menuItem ==
                                   m_menuGroupStartIndex[m_currentPage] + NUM_GAMEPAD_LABELS) {
                            m_pMenu->Reset(m_menuGroupStartIndex[m_currentPage] +
                                           (NUM_GAMEPAD_LABELS * 2) - 1);
                            GetEventManager()->TriggerEvent(EVENT_FE_MENU_UPORDOWN);
                            return;
                        }
                    }
            ...
            case GUI_MSG_CONTROLLER_DOWN: {
                ...
                switch (m_currentPage) {
                    case MENU_PAGE_MAIN:
                        if (m_pMenu->GetSelection() ==
                            m_menuGroupStartIndex[m_currentPage] + (NUM_MAINMENU_LABELS - 1)) {
                            m_pMenu->Reset(m_menuGroupStartIndex[m_currentPage]);
                            GetEventManager()->TriggerEvent(EVENT_FE_MENU_UPORDOWN);
                            return;
                        }
                    ...
                    case MENU_PAGE_KM: {
                        ...
                        // On the first element on keyboard Map 1
                        if (menuItem ==
                            m_menuGroupStartIndex[m_currentPage] + (NUM_KEYBOARDMOUSE_LABELS - 1)) {
                            m_pMenu->Reset(m_menuGroupStartIndex[m_currentPage]);
                            GetEventManager()->TriggerEvent(EVENT_FE_MENU_UPORDOWN);
                            return;
                        } else if (menuItem == m_menuGroupStartIndex[m_currentPage] +
                    ...
                    case MENU_PAGE_GAMEPAD: {
                        ...
                        // On the first element on gamepad Map 1
                        if (menuItem ==
                            m_menuGroupStartIndex[m_currentPage] + (NUM_GAMEPAD_LABELS - 1)) {
                            m_pMenu->Reset(m_menuGroupStartIndex[m_currentPage]);
                            GetEventManager()->TriggerEvent(EVENT_FE_MENU_UPORDOWN);
                            return;
                        } else if (menuItem ==
                                   m_menuGroupStartIndex[m_currentPage] + (NUM_GAMEPAD_LABELS * 2) -
                                   1) {
                            m_pMenu->Reset(
                                    m_menuGroupStartIndex[m_currentPage] + NUM_GAMEPAD_LABELS);
                            GetEventManager()->TriggerEvent(EVENT_FE_MENU_UPORDOWN);
            ...
            case GUI_MSG_CONTROLLER_RIGHT: {
                ...
                switch (m_currentPage) {
                    case MENU_PAGE_KM: {
                        ...
                        // On Map 1
                        if (menuItem <
                            m_menuGroupStartIndex[m_currentPage] + NUM_KEYBOARDMOUSE_LABELS) {
...
void
CGuiScreenController::RemapButton(eControllerColumn column, eControllerType type, int menuItem) {
    switch (m_currentPage) {
        case MENU_PAGE_KM:
...
void CGuiScreenController::UpdatePageLabels(eMenuPages page) {
    ...
    switch (page) {
        case MENU_PAGE_KM: {
            ...
            for (int column = MAP_PRIMARY, mapCount = 0;
                 column < NUM_COLUMNS; column++, mapCount++) {
                for (int kmItem = 0; kmItem < NUM_KEYBOARDMOUSE_LABELS; kmItem++) {
                    ...
                    if (vKey != Input::INVALID_CONTROLLERID) {
                        pInputPoint = pRadController->GetInputPointByTypeAndIndex("Button",
                                                                                  VirtualKeyToIndex[vKey]);
