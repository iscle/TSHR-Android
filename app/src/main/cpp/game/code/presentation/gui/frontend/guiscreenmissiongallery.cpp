...
CGuiScreenMissionGallery::OnProcessRequestsComplete(void *pUserData) {
    ...
    for (int i = 0; i < NUM_MISSIONS_PER_LEVEL; i++) {
        if ((m_pMenu->GetMenuItem(i)->m_attributes & SELECTION_ENABLED) > 0) {
            ...
            if (pSprite != NULL) {
                Scrooby::Sprite *missionImage = dynamic_cast<Scrooby::Sprite *>(m_pMenu->GetMenuItem(
                        i)->GetItem());
    ...
    if (m_screenState == SCREEN_STATE_NORMAL) {
        ...
        if (m_elapsedTime < (unsigned int) MISSION_VIEW_TRANSITION_TIME) {
            ...
            pDrawable->ScaleAboutCenter(
                    MISSION_IMAGE_SCALE + percentageDone * (1.0f - MISSION_IMAGE_SCALE));
        ...
        if (m_elapsedTime < (unsigned int) MISSION_VIEW_TRANSITION_TIME) {
            ...
            pDrawable->ScaleAboutCenter(
                    MISSION_IMAGE_SCALE + (1.0f - percentageDone) * (1.0f - MISSION_IMAGE_SCALE));
...
CGuiScreenMissionGallery::OnMenuSelectionMade(int selection) {
    ...
    int missionTitleIndex =
            scrapBookContents->GetCurrentLevel() * NUM_MISSIONS_PER_LEVEL + selection;
    rAssert(m_missionTitle != NULL);
    ...
    m_projectileVelocity.y = (MISSION_VIEW_POS_Y - startPosY -
                              0.5f * MISSION_VIEW_PROJECTILE_GRAVITY *
                              MISSION_VIEW_TRANSITION_TIME * MISSION_VIEW_TRANSITION_TIME) /
                             MISSION_VIEW_TRANSITION_TIME;
}
    ...
    for (int i = 0; i < NUM_MISSIONS_PER_LEVEL; i++) {
        ...
        if (GetCheatInputSystem()->IsCheatEnabled(CHEAT_ID_UNLOCK_MISSIONS)) {
            ...
            if (i == NUM_MISSIONS_PER_LEVEL - 1) {
                isUnlocked = GetCharacterSheetManager()->QueryBonusMissionCompleted(
                        static_cast<RenderEnums::LevelEnum>(scrapBookContents->GetCurrentLevel()));
            } else {
                int missionIndex = scrapBookContents->GetCurrentLevel() == 0 ? i + 1 : i;
                MissionRecord *pMissionRecord = GetCharacterSheetManager()->QueryMissionStatus(
                        static_cast<RenderEnums::LevelEnum>(scrapBookContents->GetCurrentLevel()),
                        missionIndex);
