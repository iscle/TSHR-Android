...
CGuiScreenSkinGallery::OnProcessRequestsComplete(void *pUserData) {
    ...
    for (int i = 0; i < m_numSelections; i++) {
        if ((m_pMenu->GetMenuItem(i)->m_attributes & SELECTION_ENABLED) > 0) {
            ...
            if (pSprite != NULL) {
                Scrooby::Sprite *skinImage = dynamic_cast<Scrooby::Sprite *>(m_pMenu->GetMenuItem(
                        i)->GetItem());
        ...
        case SCREEN_STATE_BACK_VIEW: {
            ...
            if (m_elapsedTime < (unsigned int) SKIN_VIEW_TRANSITION_TIME) {
                ...
                pDrawable->ScaleAboutCenter(
                        SKIN_BASE_SCALE + (1.0f - percentageDone) * SKIN_IMAGE_SCALE);
...
CGuiScreenSkinGallery::OnMenuSelectionMade(int selection) {
    if (m_numSelections > 0) {
        ...
        m_projectileVelocity.y = (SKIN_VIEW_POS_Y - startPosY -
                                  0.5f * SKIN_VIEW_PROJECTILE_GRAVITY * SKIN_VIEW_TRANSITION_TIME *
                                  SKIN_VIEW_TRANSITION_TIME) / SKIN_VIEW_TRANSITION_TIME;
...
CGuiScreenSkinGallery::Load2DImages() {
    ...
    for (Reward *pReward = GetRewardsManager()->FindFirstMerchandise(pScreen->GetCurrentLevel(),
