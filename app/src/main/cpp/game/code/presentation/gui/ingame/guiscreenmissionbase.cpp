...
//
//===========================================================================
CGuiScreenMissionBase::CGuiScreenMissionBase(Scrooby::Screen *pScreen, CGuiEntity *pParent,
                                             eGuiWindowID id) :
    ...
    //
    m_Page = m_pScroobyScreen->GetPage("MissionLoad");
    rAssert(m_Page != NULL);
    m_LetterboxPage = m_pScroobyScreen->GetPage("LetterBox");
    rAssert(m_LetterboxPage != NULL);
    m_FgLayer = m_Page->GetGroup("foreground");
    rAssert(m_FgLayer != NULL);
    m_loadCompleted = m_Page->GetGroup("LoadCompleted");
    rAssert(m_loadCompleted != NULL);
    m_Line0 = m_FgLayer->GetText("MissionInfo");
    rAssert(m_Line0 != NULL);
    m_missionTitle = m_FgLayer->GetText("MissionTitle");
    rAssert(m_missionTitle != NULL);
    m_BgLayer = m_Page->GetLayer("Background");
    rAssert(m_BgLayer != NULL);
    m_missionStartBitmap = m_Page->GetSprite("MissionStartBitmap");
    rAssert(m_missionStartBitmap != NULL);
    overlay = m_Page->GetLayer("Overlay");
    rAssert(overlay != NULL);
    m_darkenPolys = overlay->GetGroup("dark_polys");
    rAssert(m_darkenPolys != NULL);
    m_darkenTop = m_LetterboxPage->GetGroup("TopBar");
    rAssert(m_darkenTop != NULL);
    m_darkenBottom = m_LetterboxPage->GetGroup("BottomBar");
    rAssert(m_darkenBottom != NULL);
    m_backgroundPoly = m_Page->GetPolygon("background");
    rAssert(m_backgroundPoly != NULL);
    m_textOverlays = m_Page->GetGroup("Text");
    rAssert(m_textOverlays != NULL);
    m_Flag = m_Page->GetSprite("flag");
    rAssert(m_Flag != NULL);
    m_ClipLeftGroup = m_Page->GetGroup("ClipLeftGroup");
    rAssert(m_ClipLeftGroup != NULL);
    m_ClipRightGroup = m_Page->GetGroup("ClipRightGroup");
    rAssert(m_ClipRightGroup != NULL);
    m_ClipLeft = m_Page->GetSprite("clipLeft");
    rAssert(m_ClipLeft != NULL);
    m_ClipRight = m_Page->GetSprite("clipRight1");
    rAssert(m_ClipRight != NULL);
    m_ClipLeftArm = m_Page->GetSprite("clipArmLeft1");
    rAssert(m_ClipLeftArm != NULL);
    m_ClipRightArm = m_Page->GetSprite("clipArmRight1");
    rAssert(m_ClipRightArm != NULL);
    m_Foreground = m_Page->GetLayer("Background");
    rAssert(m_Foreground != NULL);
    m_LetterboxLayer = m_LetterboxPage->GetLayer("Background");
    rAssert(m_LetterboxLayer != NULL);
...
void CGuiScreenMissionBase::UpdateAnimatedBitmap(const float deltaT) {
    if (s_AnimatedBitmapSprite == NULL) {
        ...
        if (s_AnimatedBitmapSprite == NULL) {
            tRefCounted::Assign(s_AnimatedBitmapSprite,
                                p3d::find<tSprite>(s_AnimatedBitmapShortName));
        }
        if (s_AnimatedBitmapSprite == NULL) {
...
CGuiScreenMissionBase::UpdateGamblingInfo() {
    ...
    CoinObjective *coinObjective = dynamic_cast<CoinObjective *>(currentMission->GetStage(
            0)->GetObjective());
    ...
    RaceObjective *raceObjective = dynamic_cast<RaceObjective *>(currentMission->GetStage(
            1)->GetObjective());
