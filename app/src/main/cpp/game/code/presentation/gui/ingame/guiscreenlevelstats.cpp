...
CGuiScreenLevelStats::UpdateLevelStats() {
    ...
    sprintf(buffer, "%d / %d", GetCharacterSheetManager()->QueryNumMissionsCompleted(currentLevel),
            7);
    m_levelStats[STAT_STORY_MISSIONS]->SetString(0, buffer);

    sprintf(buffer, "%d / %d",
            GetCharacterSheetManager()->QueryBonusMissionCompleted(currentLevel) ? 1 : 0, 1);
    m_levelStats[STAT_BONUS_MISSIONS]->SetString(0, buffer);

    sprintf(buffer, "%d / %d",
            GetCharacterSheetManager()->QueryNumStreetRacesCompleted(currentLevel), 3);
    ...
    sprintf(buffer, "%.1f %%",
            GetCharacterSheetManager()->QueryPercentLevelCompleted(currentLevel));
