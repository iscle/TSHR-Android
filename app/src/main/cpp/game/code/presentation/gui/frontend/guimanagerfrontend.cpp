...
// On PC never show the splash screen.. it is very console-ish...
    ...
    if (s_memcardCheckState != MEM_CARD_CHECK_COMPLETED) {
        CGuiScreen *messageScreen = static_cast<CGuiScreen *>(this->FindWindowByID(
        ...
        CGuiScreen *errorPromptScreen = static_cast<CGuiScreen *>(this->FindWindowByID(
                CGuiWindow::GUI_SCREEN_ID_ERROR_PROMPT));
        if (errorPromptScreen != NULL) {
            errorPromptScreen->SetScroobyScreen(m_pScroobyProject->GetScreen("ErrorPrompt"));
        }

        CGuiScreen *genericPromptScreen = static_cast<CGuiScreen *>(this->FindWindowByID(
...
*/
                ...
                if (m_quittingToDemo) {
                    if (CommandLineOptions::Get(CLO_SEQUENTIAL_DEMO)) {
                        ...
                        if (whichLevel == 0) {
                            ...
                            unsigned int whichMission =
                                    (oldMisssion + 1) % RenderEnums::numMissions;
                            GetGameplayManager()->SetMissionIndex(
                                    (RenderEnums::MissionEnum) whichMission);
                        }
                    } else {
...
#else
                        unsigned int whichLevel =
                                rand() % RenderEnums::numLevels;  //All seven levels
                        GetGameplayManager()->SetLevelIndex((RenderEnums::LevelEnum) whichLevel);
                    ...
                    GetGameplayManager()->SetMissionIndex(
                            static_cast<RenderEnums::MissionEnum>(currentMission.mMissionNumber));
        ...
        default: {
            ...
            if (m_state != GUI_FE_UNINITIALIZED &&
                ...
                //
                CGuiScreen *pScreen = static_cast<CGuiScreen *>(this->FindWindowByID(
                        m_currentScreen));
                rAssert(pScreen);
                if (pScreen->IsIgnoringControllerInputs() &&
                    pScreen->IsControllerMessage(message)); // don't pass to screen
                else
...
#endif
                if (controllerID >= 0 &&
                    !GetInputManager()->GetController(controllerID)->IsConnected()) {
