        ...
        switch (message) {
            ...
            case GUI_MSG_MENU_SELECTION_VALUE_CHANGED: {
                if (param1 == MENU_ITEM_CAMERA) {
                    ...
                    GetSuperCamManager()->GetSCC(0)->SelectSuperCam(superCamType,
...
void CGuiScreenPauseSettings::InitIntro() {
    ...
    if (GetTutorialManager()->IsTutorialModeEnabled()) {
        ...
        bool isTutorialToggleAllowed = (guiManagerIngame->GetResumeGameScreenID() !=
...
CGuiScreenPauseSettings::UpdateCameraSelections() {
    ...
    m_currentCameraSelectionMode = isPlayerInCar ? CAMERA_SELECTION_FOR_DRIVING
    ...
    for (int i = 0; i < m_numCameraSelections[m_currentCameraSelectionMode]; i++) {
        ...
        SuperCam *superCam = GetSuperCamManager()->GetSCC(0)->GetSuperCam(
                m_cameraSelections[m_currentCameraSelectionMode][i]);
        rAssert(superCam);
