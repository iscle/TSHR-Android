...
CGuiScreenLoadingFE::GetCurrentMemoryUsage(ContextEnum currentContext) const {
    ...
    if (totalMemoryFree > 0) {
        switch (currentContext) {
            case CONTEXT_LOADING_GAMEPLAY: {
                currentMemoryUsage =
                        (m_startingMemoryAvailable - totalMemoryFree) / TOTAL_INGAME_MEMORY_USAGE;

                break;
            }
            case CONTEXT_LOADING_DEMO: {
                currentMemoryUsage =
                        (m_startingMemoryAvailable - totalMemoryFree) / TOTAL_DEMO_MEMORY_USAGE;

                break;
            }
            case CONTEXT_LOADING_SUPERSPRINT: {
#ifndef RAD_WIN32
                currentMemoryUsage = (m_startingMemoryAvailable - totalMemoryFree) /
                                     TOTAL_SUPERSPRINT_MEMORY_USAGE;
#else
            ...
            case CONTEXT_FRONTEND: {
                currentMemoryUsage =
                        (m_startingMemoryAvailable - totalMemoryFree) / TOTAL_FE_MEMORY_USAGE;
