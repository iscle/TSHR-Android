...
void CGuiScreenLoadGame::HandleMessage
    ...
    if (message == GUI_MSG_MESSAGE_UPDATE) {
        if (m_formatState) {
            ...
            if (m_elapsedFormatTime > m_minimumFormatTime && m_formatDone) {
                ...
                if (m_formatResult == Success) {
                    m_guiManager->DisplayPrompt(PROMPT_FORMAT_SUCCESS_GC + PLATFORM_TEXT_INDEX,
                                                this, PROMPT_TYPE_CONTINUE); // format success
                    m_formatState = false;
                } else {
                    m_guiManager->DisplayPrompt(PROMPT_FORMAT_FAIL_GC + PLATFORM_TEXT_INDEX, this,
                                                PROMPT_TYPE_CONTINUE); // format fail
                    m_formatState = false;
                }
            }
        }
        ...
        if (m_StatusPromptShown ==
            false) { // check for user unplugging memcard if not showing status
        ...
        switch (param1) {
            ...
            case PROMPT_LOAD_DELETE_CORRUPT_XBOX_HD: {
                if (param2 == CGuiMenuPrompt::RESPONSE_NO) {
                    ...
                    if (err == Success)
                        m_guiManager->DisplayPrompt(
                                PROMPT_DELETE_CORRUPT_SUCCESS_GC + PLATFORM_TEXT_INDEX, this,
                                PROMPT_TYPE_CONTINUE);
                    else
                        m_guiManager->DisplayPrompt(
                                PROMPT_DELETE_CORRUPT_FAIL_GC + PLATFORM_TEXT_INDEX, this,
                                PROMPT_TYPE_CONTINUE);
            ...
            case PROMPT_FORMAT_CONFIRM2_XBOX: // really format
            {
                if (param2 == CGuiMenuPrompt::RESPONSE_YES) {
                    ...
                    m_guiManager->DisplayMessage(
                            CGuiScreenMessage::MSG_ID_FORMATTING_GC + PLATFORM_TEXT_INDEX, this);
                } else {
                    this->GotoMemoryCardScreen(true);
                }
            ...
            case PROMPT_LOAD_SUCCESSFUL: {
                CGuiScreen *promptScreen = (CGuiScreen *) m_guiManager->FindWindowByID(
                ...
                CGuiScreenMainMenu *pScreen =
                        static_cast<CGuiScreenMainMenu *>(m_guiManager->FindWindowByID(
    ...
    if (m_state == GUI_WINDOW_STATE_RUNNING) {
        switch (message) {
            case GUI_MSG_MENU_SELECTION_MADE: {
                ...
                bool saveGameExists = GetGameDataManager()->GetSaveGameInfo(currentDrive,
                                                                            m_currentSlot,
                                                                            &saveGameInfo,
                                                                            &corrupt);
...
void CGuiScreenLoadGame::InitIntro() {
    ...
    if (s_forceGotoMemoryCardScreen ||
        !GetMemoryCardManager()->IsCurrentDriveReady(true, &unformatted, &mediaState)) {
    ...
    //
    for (unsigned int i = 0; i < NUM_GAME_SLOTS; i++) {
        bool saveGameExists = GetGameDataManager()->GetSaveGameInfo(currentDrive, i, &saveGameInfo,
                                                                    &file_corrupt);
//        saveGameExists = saveGameExists && saveGameInfo.CheckData();

        Scrooby::Text *slotText = dynamic_cast<Scrooby::Text *>(m_pMenu->GetMenuItem(
                i)->GetItem());
