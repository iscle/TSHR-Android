...
*/
    ...
    // wrap main menu text
    //
    Scrooby::Text *mainMenuText = dynamic_cast<Scrooby::Text *>(m_pMenu->GetMenuItem(
            MENU_ITEM_MAIN_MENU)->GetItem());
        ...
        if (param1 == PROMPT_CONFIRM_NEW_GAME) {
            switch (param2) {
                case CGuiMenuPrompt::RESPONSE_YES: {
                    ...
                    CGuiScreen *pScreen = static_cast<CGuiScreen *>(m_guiManager->FindWindowByID(
                ...
                if (param1 == MENU_ITEM_MAIN_MENU) {
                    ...
                    if (param2 == MAIN_MENU_MINI_GAME) // special case for mini-game
                    {
                        if (!GetCharacterSheetManager()->IsMiniGameUnlocked() &&
                            ...
                            // grey out selection
                            //
                            m_pMenu->GetMenuItem(MENU_ITEM_MAIN_MENU)->GetItemValue()->SetColour(
                                    tColour(128, 128, 128));
        ...
        if (multiController != NULL) {
            ...
            if (currentMenuSelection == MAIN_MENU_MINI_GAME) {
                ...
                if (currentFrame < 1.0f) {
                    ...
                    int currentRaceFrame = (int) raceMultiController->GetFrame() %
                                           (int) raceMultiController->GetNumFrames();
                    float numRemainingRaceFrames =
                            raceMultiController->GetNumFrames() - currentRaceFrame;
                    raceMultiController->SetRelativeSpeed(
                            numRemainingRaceFrames / NUM_RACE_CAR_FRAMES_FOR_BLENDING);
...
void CGuiScreenMainMenu::UpdateGags(unsigned int elapsedTime) {
    ...
    if (m_gagsElapsedTime > (m_gagsCycleTime - FE_GAG_PREP_TIME)) {
        if (m_nextGagIndex == -1) {
...
#endif
            if (m_nextGagIndex != 0) {
                GetEventManager()->TriggerEvent(EVENT_FE_GAG_INIT,
                                                reinterpret_cast<void *>(FE_GAGS[m_nextGagIndex]));
            } else // multiple homer gags
            {
                ...
                GetEventManager()->TriggerEvent(EVENT_FE_GAG_INIT,
                                                reinterpret_cast<void *>(FE_GAGS_FOR_HOMER[m_nextHomerGagIndex]));
            }
        }
    }
    ...
    if (m_gagsElapsedTime > m_gagsCycleTime) {
        if (m_gags != NULL) {
            ...
            if (m_currentGagIndex == 0) {
                ...
                if (m_homer != NULL && multiController != NULL) {
                    ...
                    tMultiController *homerMultiController = m_homer->GetMultiController();
                    if (homerMultiController != NULL) {
                        homerMultiController->SetFrameRange(
                                HOMER_GAG_ANIMATION[m_nextHomerGagIndex].startFrame,
        ...
        if (CommandLineOptions::Get(CLO_FE_GAGS_TEST)) {
            ...
            m_gagsCycleTime = 1000 * (MIN_GAGS_CYCLE_TIME +
                                      rand() % (MAX_GAGS_CYCLE_TIME - MIN_GAGS_CYCLE_TIME + 1));
        }
...
void CGuiScreenIntroTransition::InitIntro() {
    ...
    CGuiScreenMainMenu *pScreen = static_cast<CGuiScreenMainMenu *>(m_guiManager->FindWindowByID(
