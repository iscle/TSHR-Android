...
IGuiScreenRewards::InsertPreviewObject(PreviewObject *previewObjects,
    ...
    if (isSorted) {
        ...
        for (int i = 0; i < numPreviewObjects; i++) {
            ...
            if (static_cast<int>(pReward->RewardStatus()) >
                static_cast<int>(previewObjects[i].isUnlocked)) {
                insertHere = true;
...
IGuiScreenRewards::UpdateVehicleStats() {
    const PreviewObject *currentPreviewObject = this->GetCurrentPreviewObject();
    CarAttributeRecord *carAttributes = GetRewardsManager()->GetCarAttributeRecord(
            const_cast<char *>(currentPreviewObject->name));
