...
CGuiScreenController::CGuiScreenController(Scrooby::Screen *pScreen, CGuiEntity *pParent)
    ...
    // Add main menu items
    SetGroups(m_pMenuLabels, NUM_MAINMENU_LABELS, szMainControllerPage, "Menu", "Label",
              ALL_ATTRIBUTES_ON);
    ...
    SetGroups(m_pCCLabels[MAP_SECONDARY], NUM_CHARACTERCONTROL_LABELS, szCCPage, "Map2",
              "Map2Label");
            ...
            case GUI_MSG_MENU_SELECTION_VALUE_CHANGED: {
                ...
                switch (m_currentPage) {
                    case MENU_PAGE_GAMESETTINGS: {
                        ...
                        int menuItem =
                                param1 - NUM_CHARACTERCONTROL_LABELS - NUM_VEHICLECONTROL_LABELS;
                        UserController *pController = GetInputManager()->GetController(
                                m_currentControllerID);

                        switch (menuItem) {
                            case GS_MOUSELOOK: {
                                pController->SetMouseLook(
                                        m_pMenu->GetSelectionValue(param1) == PROMPT_YES);

                                GetSuperCamManager()->GetSCC(0)->SelectSuperCam(
                                        SuperCam::ON_FOOT_CAM, SuperCamCentral::CUT, 0);
                                GetSuperCamManager()->GetSCC(0)->Update(0);
                            }
                                break;
                            case GS_MOUSESENSITIVITY_X: {
                                pController->SetMouseSensitivityX(
                                        m_pMenu->GetMenuItem(param1)->m_slider.m_value);
                            }
                                break;
                            case GS_MOUSESENSITIVITY_Y: {
                                pController->SetMouseSensitivityY(
                                        m_pMenu->GetMenuItem(param1)->m_slider.m_value);
                            }
                                break;
                            case GS_INVERTMOUSEX: {
                                pController->SetInvertMouseX(
                                        m_pMenu->GetSelectionValue(param1) == PROMPT_YES);
                            }
                                break;
                            case GS_INVERTMOUSEY: {
                                pController->SetInvertMouseY(
                                        m_pMenu->GetSelectionValue(param1) == PROMPT_YES);
                            }
                                break;
                            case GS_FORCEFEEDBACK: {
                                pController->SetForceFeedback(
                                        m_pMenu->GetSelectionValue(param1) == PROMPT_YES);
                            }
                                break;
                            case GS_WHEELSENSITIVITY: {
                                pController->SetWheelSensitivityX(
                                        m_pMenu->GetMenuItem(param1)->m_slider.m_value);
                            }
                                break;
                            default:
            ...
            case GUI_MSG_CONTROLLER_UP: {
                ...
                switch (m_currentPage) {
                    ...
                    case MENU_PAGE_VEHICLECONTROLS: {
                        ...
                        if (menuItem == m_menuGroupStartIndex[m_currentPage]) {
                            m_pMenu->Reset(m_menuGroupStartIndex[m_currentPage] +
                                           (NUM_VEHICLECONTROL_LABELS - 1));
                            GetEventManager()->TriggerEvent(EVENT_FE_MENU_UPORDOWN);
                            return;
                        } else if (menuItem == m_menuGroupStartIndex[m_currentPage] +
                                               NUM_VEHICLECONTROL_LABELS) {
                            m_pMenu->Reset(m_menuGroupStartIndex[m_currentPage] +
                                           (NUM_VEHICLECONTROL_LABELS * 2) - 1);
                            GetEventManager()->TriggerEvent(EVENT_FE_MENU_UPORDOWN);
                            return;
                        }
                    }
            ...
            case GUI_MSG_CONTROLLER_DOWN: {
                ...
                switch (m_currentPage) {
                    ...
                    case MENU_PAGE_GAMESETTINGS: {
                        ...
                        // On the first element on gamepad Map 1
                        if (menuItem ==
                            m_menuGroupStartIndex[m_currentPage] + NUM_GAMESETTING_LABELS - 1) {
                            m_pMenu->Reset(m_menuGroupStartIndex[m_currentPage]);
                            GetEventManager()->TriggerEvent(EVENT_FE_MENU_UPORDOWN);
                            return;
                        }
            ...
            case GUI_MSG_CONTROLLER_RIGHT: {
                ...
                switch (m_currentPage) {
                    case MENU_PAGE_CHARACTERCONTROLS: {
                        ...
                        // On Map 1
                        if (menuItem <
                            m_menuGroupStartIndex[m_currentPage] + NUM_CHARACTERCONTROL_LABELS) {
                    ...
                    case MENU_PAGE_VEHICLECONTROLS: {
                        ...
                        // On Map 1
                        if (menuItem <
                            m_menuGroupStartIndex[m_currentPage] + NUM_VEHICLECONTROL_LABELS) {
                    ...
                    case MENU_PAGE_MAIN: {
                        switch (param1) {
                            ...
                            case MENU_ITEM_RESTOREALLDEFAULTS: {
                                ...
                                GetInputManager()->GetFEMouse()->SetSelectable(true);
                                m_guiManager->DisplayPrompt(PROMPT_CONFIRM_RESTOREALLDEFAULTS,
                                                            this);
                                break;
                            }
                            default: {
    ...
    for (int i = 0; i < NUM_GAMESETTING_LABELS; i++, menuItem++, relMenuItem++) {
        switch (relMenuItem) {
            ...
            case GS_INVERTMOUSEY: {
                ...
                break;
            case GS_FORCEFEEDBACK: {
                m_pMenu->SetSelectionValue(menuItem,
                                           pController->IsForceFeedbackOn() == PROMPT_YES);
            }
                break;
            case GS_WHEELSENSITIVITY: {
                pMenuItem = m_pMenu->GetMenuItem(menuItem);
...
//===========================================================================
void
CGuiScreenController::OnButtonMapped(const char *szNewInput, eControllerType cont, int num_dirs,
