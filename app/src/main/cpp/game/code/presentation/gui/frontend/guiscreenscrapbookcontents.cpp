...
CGuiScreenScrapBookContents::OnLevelSelectionChange(int delta) {
    ...
    // missions
    int numMissionsCompleted = GetCharacterSheetManager()->QueryNumMissionsCompleted(
            static_cast<RenderEnums::LevelEnum>(m_currentLevel));
    ...
    unlockedText = dynamic_cast<Scrooby::Text *>(m_pMenu->GetMenuItem(
            MENU_STORY_MISSIONS)->GetItemValue());
    rAssert(unlockedText != NULL);
    unlockedText->SetString(0, buffer);

    // clothing
    sprintf(buffer, "%d / %d",
            GetCharacterSheetManager()->QueryNumSkinsUnlocked(
                    static_cast<RenderEnums::LevelEnum>(m_currentLevel)),
            3);
    unlockedText = dynamic_cast<Scrooby::Text *>(m_pMenu->GetMenuItem(
            MENU_CHARACTER_CLOTHING)->GetItemValue());
    rAssert(unlockedText != NULL);
    unlockedText->SetString(0, buffer);

    // vehicles
    int numVehiclesUnlocked = GetCharacterSheetManager()->QueryNumCarUnlocked(
            static_cast<RenderEnums::LevelEnum>(m_currentLevel));
    ...
    sprintf(buffer, "%d / %d",
            ...
            6);
    unlockedText = dynamic_cast<Scrooby::Text *>(m_pMenu->GetMenuItem(
            MENU_VEHICLES)->GetItemValue());
    rAssert(unlockedText != NULL);
    unlockedText->SetString(0, buffer);

    // cards
    sprintf(buffer, "%d / %d",
            GetCharacterSheetManager()->QueryNumCardsCollected(
                    static_cast<RenderEnums::LevelEnum>(m_currentLevel)),
            NUM_CARDS_PER_LEVEL);
    unlockedText = dynamic_cast<Scrooby::Text *>(m_pMenu->GetMenuItem(
            MENU_COLLECTOR_CARDS)->GetItemValue());
    rAssert(unlockedText != NULL);
    unlockedText->SetString(0, buffer);
