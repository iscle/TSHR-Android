...
*/
            case GUI_MSG_MENU_SELECTION_MADE: {
                Scrooby::Sprite *currentCardSelection = dynamic_cast<Scrooby::Sprite *>(m_pMenu->GetMenuItem(
                        param1)->GetItem());
                ...
                if (cardID != -1) {
                    ...
                    m_cardVelocity.y = (m_viewCardDistY -
                                        0.5f * CARD_PROJECTILE_GRAVITY * CARD_TRANSITION_TIME *
                                        CARD_TRANSITION_TIME) / CARD_TRANSITION_TIME;
...
CGuiScreenCardGallery::UpdateCards(unsigned int currentLevel) {
    ...
    for (unsigned int i = 0; i < NUM_CARDS_PER_LEVEL; i++) {
        Scrooby::Sprite *cardSelection = dynamic_cast<Scrooby::Sprite *>(m_pMenu->GetMenuItem(
                i)->GetItem());
