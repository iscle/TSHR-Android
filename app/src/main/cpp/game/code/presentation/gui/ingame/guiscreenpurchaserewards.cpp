            ...
            case GUI_MSG_CONTROLLER_RIGHT: {
                ...
                if (m_currentType == Merchandise::SELLER_INTERIOR) {
                    if (m_numPreviewClothing > 1) {
                        ...
                        m_currentPreviewClothing =
                                (m_currentPreviewClothing + m_numPreviewClothing + selectionDelta) %
                                m_numPreviewClothing;

                        this->On3DModelSelectionChange(
                                &m_previewClothing[m_currentPreviewClothing]);
                    }
                } else {
                    if (m_numPreviewVehicles > 1) {
                        previousSelection = m_currentPreviewVehicle;

                        m_currentPreviewVehicle =
                                (m_currentPreviewVehicle + m_numPreviewVehicles + selectionDelta) %
...
CGuiScreenPurchaseRewards::OnProcessRequestsComplete(void *pUserData) {
    if (reinterpret_cast<Scrooby::Pure3dObject *>(pUserData) == m_previewBgd) {
        ...
        if (drawable != NULL) {
            ...
            tMultiController *multiController = p3d::find<tMultiController>(
    ...
    if (m_isPurchasingReward) {
        ...
        if (m_elapsedCoinDecrementTotalTime < COINS_DECREMENT_DURATION) {
            ...
            if (m_elapsedCoinDecrementTime > COINS_DECREMENT_PERIOD) {
                ...
                int bankValue = m_bankValueBeforePurchase - (int) (m_elapsedCoinDecrementTotalTime /
                                                                   (float) COINS_DECREMENT_DURATION *
                                                                   deltaCoins);
...
CGuiScreenPurchaseRewards::UpdateRewardPrice() {
    ...
    if (currentPreviewObject->isUnlocked) {
        ...
        Merchandise *pMerchandise = GetRewardsManager()->GetMerchandise(
        ...
        if (pMerchandise != NULL) {
            ...
            if(CGuiTextBible::GetCurrentLanguage() == Scrooby::XL_GERMAN)
            {
                ...
                //
                sprintf(buffer, "F�r %d %s", rewardCost, coinsText);
            }
            else
#endif // PAL
            {
            ...
            this->SetButtonVisible(BUTTON_ICON_ACCEPT,
                                   GetCoinManager()->GetBankValue() >= rewardCost);
...
CGuiScreenPurchaseRewards::PurchaseReward() {
    ...
    if (currentPreviewObject->isUnlocked) {
        ...
        bool boughtReward = GetRewardsManager()->BuyMerchandise(
                GetGameplayManager()->GetCurrentLevelIndex(),
                currentPreviewObject->name);
        ...
        if (boughtReward) {
            ...
            if (currentHud != NULL) {
                HudCoinCollected *hudCoinCollected = static_cast<HudCoinCollected *>(currentHud->GetEventHandler(
                        CGuiScreenHud::HUD_EVENT_HANDLER_COIN_COLLECTED));
...
CGuiScreenPurchaseRewards::LoadSelectedReward() {
    ...
    if (m_currentType == Merchandise::SELLER_INTERIOR) {
        ...
        GetCharacterSheetManager()->SetCurrentSkin(GetGameplayManager()->GetCurrentLevelIndex(),
                                                   m_previewClothing[m_currentPreviewClothing].name);
        ...
        ActionButton::SummonVehiclePhone::LoadVehicle(
                m_previewVehicles[m_currentPreviewVehicle].name,
