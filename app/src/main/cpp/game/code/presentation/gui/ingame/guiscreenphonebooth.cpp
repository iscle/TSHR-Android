            ...
            switch (message) {
                case GUI_MSG_CONTROLLER_SELECT: {
                    ...
                    if (currentPreviewObject->isUnlocked) {
                        ...
                        //
                        int carIndex = GetCharacterSheetManager()->GetCarIndex(
                                currentPreviewObject->name);
                        if (carIndex >= 0) {
                            bool isVehicleDamagedOut = (
                                    GetCharacterSheetManager()->GetCarDamageState(carIndex) ==
                                    eHusk);
...
*/
                        ...
                        GetEventManager()->TriggerEvent(EVENT_PHONE_BOOTH_RIDE_REQUEST,
                                                        reinterpret_cast<void *>(pWalkerCharacter));

                        bool isBonusVehicle = (
                                m_previewVehicles[m_currentPreviewVehicle].pReward->GetQuestType() ==
                                Reward::eBonusMission);
                        if (isBonusVehicle) {
                            ...
                            bool isNewVehicle = (strcmp(pCurrentVehicle->GetName(),
                                                        currentPreviewObject->name) != 0);
                            if (!isNewVehicle) {
                                //
                ...
                case GUI_MSG_CONTROLLER_LEFT: {
                    m_currentPreviewVehicle = (m_currentPreviewVehicle - 1 + m_numPreviewVehicles) %
...
void CGuiScreenPhoneBooth::InitIntro() {
    ...
    //
    Reward *pDefaultCar = GetRewardsManager()->GetReward(
...
void CGuiScreenPhoneBooth::InitOutro() {
    if (m_isLoadingReward) {
        ...
        tCameraAnimationController *camController = p3d::find<tCameraAnimationController>(
                "CAM_phonecameraShape");
