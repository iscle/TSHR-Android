...
void
CGuiScreenMessage::GetControllerDisconnectedMessage(int controller_id, char *str_buffer,
                                                    int max_char) {
    ...
    for(int i = 0; stringBuffer[ i ] != '\0'; i++)
    {
        ...
        if(stringBuffer[ i ] == SLOT_WILDCARD_CHARACTER)
        {
            ...
            const IRadDrive::MediaInfo* mediaInfo =
                GetMemoryCardManager()->GetMediaInfo(static_cast<unsigned int>(CGuiScreenMemoryCard::s_currentMemoryCardSlot));
            char textBibleEntry[ 32 ];
            sprintf(textBibleEntry, "XBOX_%s",
                GetMemoryCardManager()->GetDriveName(static_cast<unsigned int>(CGuiScreenMemoryCard::s_currentMemoryCardSlot)));
