...
//            s_registeredIcons[ i ].m_iconImage->SetAlpha(1.0f);
            ...
            if (s_registeredIcons[i].m_dynamicLocator != NULL) {
                s_registeredIcons[i].m_dynamicLocator->GetPosition(
                        &s_registeredIcons[i].m_location);
            ...
            if (static_cast<int>(i) == s_fpIconID) // for focal point icon ...
            {
                ...
                m_radarCone->RotateAboutCenter(
                        rmt::RadianToDeg(this->CalculatRadarConeAngle(iconLoc)));
...
CHudMap::UnregisterIcon(int iconID) {
    if (iconID != -1) {
        ...
        rAssertMsg(false,
                   "This should only happen if you didn't get a valid icon ID upon registration!");
    }
}
...
CHudMap::ChangeIconType(int iconID, HudMapIcon::eIconType type) {
    ...
    if (iconID != -1) {
        ...
        //
        newIconID = this->RegisterIcon(type, oldIcon->m_location, oldIcon->m_dynamicLocator,
                                       isFocalPointIcon);
    }
...
CHudMap::RestoreAllRegisteredIcons() {
    if (s_numRegisteredIcons > 0) {
        ...
        for (unsigned int i = 0; i < MAX_NUM_REGISTERED_ICONS; i++) {
            if (s_registeredIcons[i].m_iconImage != NULL) {
                s_registeredIcons[i].m_iconImage = this->RemoveIconFromInventory(
                        s_registeredIcons[i].m_type);
        ...
        rAssertMsg(numIconsRestored == s_numRegisteredIcons,
                   "Not all registered icons were restored!");
    }
}
...
CHudMap::DetermineOnRoadLocation(rmt::Vector &source, rmt::Vector &target, float visibleRadius) {
    ...
    isPathElementFound = VehicleAI::FindClosestPathElement(source,
                                                           sourceElem, sourceSeg, sourceSegT,
                                                           sourceRoadT,
                                                           false); // don't want shortcuts on hudmap

    if (!isPathElementFound) {
        GetIntersectManager()->FindClosestRoad(source, searchRadius, sourceSeg,
                                               dummy); // ignore shortcuts
    ...
    isPathElementFound = VehicleAI::FindClosestPathElement(target,
                                                           targetElem, targetSeg, targetSegT,
                                                           targetRoadT,
                                                           false); // don't want shortcuts on hudmap
    ...
    if (!isPathElementFound) {
        ...
        if (m_lastRoadSeg) {
            ...
            rAssertMsg(closestRoadSeg,
                       "Should always find a nearby road seg! Increase search radius!");
            if (!closestRoadSeg) {
                return;
            }
    ...
    if (pathElements.mUseSize == 1) {
        ...
        } else // the only element is a road
        {
            ...
            if (sourceElem != targetElem) {
                ...
                RoadManager::FindClosestPointOnRoad(theRoad, source, closestPos, closestDist,
                                                    closestSegIndex);

                rAssert(0 <= closestSegIndex &&
                        closestSegIndex < (int) theRoad->GetNumRoadSegments());
        ...
        for (int i = 0; i < pathElements.mUseSize; i++) {
            ...
            switch (currentPathElement->type) {
                case RoadManager::ET_NORMALROAD: {
                    ...
                    if (i == 0) // special case if first path element is a road
                    {
                        for (int j = 1; j < pathElements.mUseSize; j++) {
                            ...
                            if (pathElement->type == RoadManager::ET_INTERSECTION) {
                                isRoadBackwards = (road->GetSourceIntersection() ==
                                                   static_cast<Intersection *>(pathElement->elem));
                    ...
                    while (currentRoadSegmentIndex >= 0 &&
                           currentRoadSegmentIndex < numRoadSegments) {
                        RoadSegment *currentRoadSegment = road->GetRoadSegment(
                                currentRoadSegmentIndex);
                        rmt::Vector vec0, vec1, vec2, vec3, start, end;
                        ...
                        int numIntersectPoints = IntersectLineSphere(start, end, visibleSphere,
                                                                     intersectPoints);
                        if (numIntersectPoints > 0) {
                            ...
                            if (numIntersectPoints > 1) {
                                rTuneWarningMsg(false,
                                                "Multiple intersection points not yet handled properly, but this should not happen here!");
                            }
                            ...
                            {
                                float roadSegT = roadManager->DetermineSegmentT(
                                        closestIntersectPoint,
                                        const_cast<RoadSegment *>(currentRoadSegment));
                                float intersectRoadT = roadManager->DetermineRoadT(
                                        const_cast<RoadSegment *>(currentRoadSegment), roadSegT);
