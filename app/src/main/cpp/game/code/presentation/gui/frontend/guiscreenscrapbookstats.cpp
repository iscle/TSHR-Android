...
void CGuiScreenScrapBookStats::InitIntro() {
    ...
    for (int i = 0; i < RenderEnums::numLevels; i++) {
        numUnlocked[STAT_STORY_MISSIONS] += GetCharacterSheetManager()->QueryNumMissionsCompleted(
                static_cast<RenderEnums::LevelEnum>(i));
        numUnlocked[STAT_BONUS_MISSIONS] += GetCharacterSheetManager()->QueryBonusMissionCompleted(
                static_cast<RenderEnums::LevelEnum>(i)) ? 1 : 0;
        numUnlocked[STAT_STREET_RACES] += GetCharacterSheetManager()->QueryNumStreetRacesCompleted(
                static_cast<RenderEnums::LevelEnum>(i));
        numUnlocked[STAT_CARDS] += GetCardGallery()->GetCollectedCards(i)->m_numCards;
        numUnlocked[STAT_CLOTHING] += GetCharacterSheetManager()->QueryNumSkinsUnlocked(
                static_cast<RenderEnums::LevelEnum>(i));
        numUnlocked[STAT_VEHICLES] += GetCharacterSheetManager()->QueryNumCarUnlocked(
                static_cast<RenderEnums::LevelEnum>(i));
        numUnlocked[STAT_WASPS] += GetCharacterSheetManager()->QueryNumWaspsDestroyed(
                static_cast<RenderEnums::LevelEnum>(i));
        numUnlocked[STAT_GAGS] += GetCharacterSheetManager()->QueryNumGagsViewed(
                static_cast<RenderEnums::LevelEnum>(i));
        numUnlocked[STAT_MOVIES] += GetCharacterSheetManager()->QueryFMVUnlocked(
                static_cast<RenderEnums::LevelEnum>(i)) ? 1 : 0;
