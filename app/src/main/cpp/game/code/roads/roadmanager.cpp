...
void RoadManager::PopulateConnectivityData(bool useMultiplier, Intersection *intersections,
                                           int numInts) {
        ...
        if (srcRd->GetShortCut() && sourceElem != targetElem) {
            ...
            */
            ...
            totalDist += distToDestInt + FindPathElementsBetween(useMultiplier,
                                                                 sourceElem, 0.0f, intPos,
                                                                 targetElem, targetT, targetPos,
                                                                 tmpElems);
        ...
        if (srcSrcInt == targDestInt && srcDestInt == targSrcInt) {
            ...
            */
            ...
            if (distFromClosestPtOnTargSqr < CLOSE_ENOUGH_TO_BE_ADJACENT_ROAD_SQR) {
                ...
                float closestTargSegT = RoadManager::DetermineSegmentT(closestPtOnTarg,
                                                                       closestTargSeg);
                float closestTargRoadT = RoadManager::DetermineRoadT(closestTargSeg,
                                                                     closestTargSegT);
    ...
    if (sourceElem.type == ET_INTERSECTION) {
        ...
        if (sourceInt->mBigIntersection || shortestRoads->mUseSize == 1) {
            ...
            distViaTargetInt = FindDistToTargetInOneDirection(useMultiplier,
                                                              targetInt, sourceInt, sourceInt, NULL,
                                                              elemsToTargetInt,
                                                              firstShortRoad, lastShortRoad,
                                                              errVal) +
                               distFromTargetToInt;
            ...
            if (targetOtherInt) {
                ...
                distViaTargetOtherInt = FindDistToTargetInOneDirection(useMultiplier,
                                                                       targetOtherInt, sourceInt,
                                                                       sourceInt, NULL,
                                                                       elemsToTargetOtherInt,
                                                                       firstShortRoad,
                                                                       lastShortRoad, errVal) +
                ...
                if (targetElem.type == ET_NORMALROAD) {
                    if (lastShortRoad) {
                        ...
                        distViaTargetOtherInt += GetTraversalDistance(lastShortRoad,
                                                                      &targetShortRoad);
                    }
                }
        ...
        if (sourceElem.type == ET_NORMALROAD) {
            ...
            if (!sourceRoad->GetShortCut()) // shortcuts only go one way!!
            {
                ...
                distToTargetIntInOneDir = FindDistToTargetInOneDirection(useMultiplier,
                                                                         targetInt, sourceSrcInt,
                                                                         sourceDestInt, NULL,
                                                                         elemsToTargetIntInOneDir,
                                                                         firstShortRoad,
                                                                         lastShortRoad,
                                                                         targetIntOneErr) +
                                          distToSrcInt + distFromTargetToInt;
                if (firstShortRoad && targetIntOneErr != DEAD_END) {
                    distToTargetIntInOneDir += GetTraversalDistance(&sourceShortRoad,
                                                                    firstShortRoad);
                }
                if (targetElem.type == ET_NORMALROAD && targetIntOneErr != DEAD_END) {
                    rAssert(lastShortRoad);
                    targetShortRoad.isOutRoad = true;
                    distToTargetIntInOneDir += GetTraversalDistance(lastShortRoad,
                                                                    &targetShortRoad);
                }
            ...
            distToTargetIntInOtherDir = FindDistToTargetInOneDirection(useMultiplier,
                                                                       targetInt, sourceDestInt,
                                                                       sourceSrcInt, NULL,
                                                                       elemsToTargetIntInOtherDir,
                                                                       firstShortRoad,
                                                                       lastShortRoad,
                                                                       targetIntOtherErr) +
            ...
            if (targetOtherInt) {
                if (!sourceRoad->GetShortCut()) // shortcuts only go one way!!
                {
                    ...
                    distToTargetOtherIntInOneDir = FindDistToTargetInOneDirection(useMultiplier,
                                                                                  ...
                                                                                  sourceDestInt,
                                                                                  NULL,
                                                                                  elemsToTargetOtherIntInOneDir,
                                                                                  firstShortRoad,
                                                                                  lastShortRoad,
                                                                                  targetOtherIntOneErr) +
                                                   distToSrcInt + distFromTargetToOtherInt;
                    if (firstShortRoad && targetOtherIntOneErr != DEAD_END) {
                        distToTargetOtherIntInOneDir += GetTraversalDistance(&sourceShortRoad,
                                                                             firstShortRoad);
                    }
                    if (targetElem.type == ET_NORMALROAD && targetOtherIntOneErr != DEAD_END) {
                        rAssert(lastShortRoad);
                        targetShortRoad.isOutRoad = false;
                        distToTargetOtherIntInOneDir += GetTraversalDistance(lastShortRoad,
                                                                             &targetShortRoad);
                    }
                }
                ...
                distToTargetOtherIntInOtherDir = FindDistToTargetInOneDirection(useMultiplier,
                                                                                ...
                                                                                sourceDestInt,
                                                                                sourceSrcInt, NULL,
                                                                                elemsToTargetOtherIntInOtherDir,
                                                                                firstShortRoad,
                                                                                lastShortRoad,
                                                                                targetOtherIntOtherErr) +
                                                 distToDestInt + distFromTargetToOtherInt;
                if (firstShortRoad && targetOtherIntOtherErr != DEAD_END) {
                    distToTargetOtherIntInOtherDir += GetTraversalDistance(&sourceShortRoad,
                                                                           firstShortRoad);
                }
                if (targetElem.type == ET_NORMALROAD && targetOtherIntOtherErr != DEAD_END) {
                    rAssert(lastShortRoad);
                    targetShortRoad.isOutRoad = false;
                    distToTargetOtherIntInOtherDir += GetTraversalDistance(lastShortRoad,
                                                                           &targetShortRoad);
                }
            }
        } else {
            ...
            if (targetElem.type == ET_NORMALROAD && targetIntOtherErr != DEAD_END) {
                if (lastShortRoad) {
                    targetShortRoad.isOutRoad = true;
                    distToTargetIntInOtherDir += GetTraversalDistance(lastShortRoad,
                                                                      &targetShortRoad);
                }
            }
