...
void Intersection::PopulateShortestRoads(SwapArray <RoadManager::ShortestRoad> &roadsToAdjacentInts,
                                         bool useMultiplier) {
    ////////////
...
void
Intersection::FindGoodTrafficLane(const Road &road, unsigned int preferredLaneIndex, Lane *&outLane,
    ...
    if (mnRoadsOut == 1) {
        ...
        for (i; i < mnRoadsOut; ++i) {
            ...
            if (stillLookingForFirstRoad || (cosAlpha <= bestCosAlpha)) {
                ...
                FindGoodTrafficLane(*mRoadListOut[i], preferredLaneIndex, laneCandidate,
                                    laneIndexCandidate);
