...
public:  // methods
    ...
    //
    const Intersection *GetSourceIntersection(void) const { return mpSourceIntersection; }

    // return the intersection at the end of this road.
    //
    const Intersection *GetDestinationIntersection(void) const { return mpDestinationIntersection; }

    // set the intersection at the beginning of this road.
    //
    void
    SetSourceIntersection(Intersection *pIntersection) { mpSourceIntersection = pIntersection; }

    // set the intersection at the end of this road.
    //
    void SetDestinationIntersection(
            Intersection *pIntersection) { mpDestinationIntersection = pIntersection; }
    ...
    unsigned int GetNumRoadSegments(void) const { return mnRoadSegments; }

    unsigned int GetMaxRoadSegments(void) const { return mnMaxRoadSegments; }

    RoadSegment *GetRoadSegment(unsigned int index) const;

    int GetRoadSegmentAtPoint(const rmt::Vector &point, RoadSegment **ppOutRoadSegment, float &in,
                              float &lateral, int hint) const;
