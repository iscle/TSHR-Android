...
void SuperCam::SetCameraValues(unsigned int milliseconds,
    ...
    rAssertMsg(!(GetFlag((Flag) SHAKE) && GetFlag((Flag) TRANSITION)),
               "Do not shake the camera while transitioning!");
    ...
    if (GetFlag((Flag) SHAKE) || GetFlag((Flag) TRANSITION)) {
        ...
        if (GetFlag((Flag) SHAKE)) {
            ...
            if (mTransitionTime >= mTransitionTimeLimit ||
                ...
                if (GetFlag((Flag) EASE_IN)) {
                    //Do the Ease in
                    EaseIn(rmt::LtoF(mTransitionTime) / rmt::LtoF(mTransitionTimeLimit), &pos,
                           &targ, milliseconds);
                    TransitionFOV(rmt::LtoF(mTransitionTime) / rmt::LtoF(mTransitionTimeLimit),
                                  &mSCFOV, milliseconds);
                } else {
                    //Do the Transition
                    TransitionCamera(rmt::LtoF(mTransitionTime) / rmt::LtoF(mTransitionTimeLimit),
                                     &pos, &targ, &mSCFOV, milliseconds,
                                     GetFlag((Flag) QUICK_TRANSITION));
                }
            }
        }
    }
...
void SuperCam::InitMyController(int controllerID) {
    ...
    if (mControllerHandle == -1) {
        ...
        if (controllerID != -1) {
            mControllerHandle = im->RegisterMappable(static_cast<unsigned int>(controllerID),
                                                     mController);
        }
    }
...
void SuperCam::SphericalMotion(const rmt::Vector &target,
    ...
    rmt::CartesianToSpherical(virtTargToPos.x, virtTargToPos.z, virtTargToPos.y, &actMag, &actRot,
                              &actElev);
    rmt::CartesianToSpherical(virtTargToVirtPos.x, virtTargToVirtPos.z, virtTargToVirtPos.y,
                              &virtMag, &virtRot, &virtElev);
...
void SuperCam::TransitionCamera(float timeLeftPct,
    ...
    float posInterval =
            (((1.0f - mTransitionPositionRate) * timeLeftPct) + mTransitionPositionRate) * timeMod;
    CLAMP_TO_ONE(posInterval);

    float targInterval =
            (((1.0f - mTransitionTargetRate) * timeLeftPct) + mTransitionTargetRate) * timeMod;
    ...
    if (!quick) {
        ...
        SphericalMotion(mCameraVirtualTarget, actPos, mCameraVirtualPosition,
                        mCameraVirtualPositionDelta, posInterval);

        actPos.Add(actPos, mCameraVirtualTarget);
    } else {
        //Do it quick.
        MotionCubic(&actPos.x, &mCameraVirtualPositionDelta.x, mCameraVirtualPosition.x,
                    posInterval);
        MotionCubic(&actPos.y, &mCameraVirtualPositionDelta.y, mCameraVirtualPosition.y,
                    posInterval);
        MotionCubic(&actPos.z, &mCameraVirtualPositionDelta.z, mCameraVirtualPosition.z,
                    posInterval);
    }
...
void SuperCam::TransitionFOV(float timeLeftPct, float *fov, unsigned int milliseconds) {
    ...
    float fovInterval =
            (((1.0f - mFOVTransitionRate) * timeLeftPct) + mFOVTransitionRate) * timeMod;
