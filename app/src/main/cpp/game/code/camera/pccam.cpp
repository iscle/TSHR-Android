...
void PCCam::Update(unsigned int milliseconds) {
    ...
    if (mController) {
        ...
        float mouseSense = static_cast<Mouse *>(GetInputManager()->GetController(
                0)->GetRealController(MOUSE))->getSensitivityY();

        gDefaultElevation +=
                gElevationIncrement * m_fYawVelocity * m_fAngularSpeed * mouseSense * Y_SENSE_MOD;
    ...
    if (player->mPCCamFacing == 2) {
        ...
        float mouseSenseX = static_cast<Mouse *>(GetInputManager()->GetController(
                0)->GetRealController(MOUSE))->getSensitivityX();
    ...
    */
    ...
    if (GetFlag((Flag) CUT)) {
        FilterFov(zoom, SUPERCAM_DEFAULT_MIN_FOV, SUPERCAM_DEFAULT_MAX_FOV, FOV, mFOVDelta, 1.0f,
                  timeMod);
    } else {
        FilterFov(zoom, SUPERCAM_DEFAULT_MIN_FOV, SUPERCAM_DEFAULT_MAX_FOV, FOV, mFOVDelta,
                  SUPERCAM_DEFAULT_FOV_LAG, timeMod);
    }
...
*/
    ...
    if (!iList.LineOfSight(lookFrom, lookTo, 0.01f, true)) {
        if (iList.TestIntersection(lookFrom, lookTo, &intersection, 1.0f)) {
            ...
            rmt::Vector offSettedDest = intersection + ((intersection - newPos) /
                                                        ((intersection - newPos).Magnitude()) *
                                                        (NEARCLIP_OFFSET));
...
//
//=============================================================================
void PCCam::SetCollisionOffset(const rmt::Vector *offset, unsigned int numCollisions,
