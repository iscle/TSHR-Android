...
//
//=============================================================================
float &RailCam::ProjectPointOnLine(const rmt::Vector &A, const rmt::Vector &B, const rmt::Vector &O,
                                   float &t) const {
    ...
    if (A == B)
        ...
        t = -((B.x - A.x) * (A.x - O.x) + (B.y - A.y) * (A.y - O.y) + (B.z - A.z) * (A.z - O.z)) /
...
//
//=============================================================================
RailCam::SolutionType
RailCam::IntersectLineCylinder(const int segment, const rmt::Vector &origin, const float radius,
    ...
    float b = 2.0f *
              (-A.x * A.x - A.z * A.z + A.x * B.x + A.z * B.z + A.x * origin.x - B.x * origin.x +
...
//    }    
    ...
    if (mCandidates[EXACT].segment != -1 && mCandidates[EXACT].pDist > mStep) {
        ...
        if (mCandidates[APPROX].pDist <= mStep) {
            ...
            SolutionType index = (mCandidates[EXACT].pDist <= mCandidates[APPROX].pDist) ? EXACT
                                                                                         : APPROX;
...
rmt::Vector RailCam::FindCameraPosition_Projection(const rmt::Vector &target, const float pOffset) {
    ...
    for (i = 0; i < mQ.GetNumSegments(); i++) {
        ...
        SolutionType index = IntervalClamp(
                ProjectPointOnLine(mQ.GetKnot(i), mQ.GetKnot(i + 1), target, segT));
