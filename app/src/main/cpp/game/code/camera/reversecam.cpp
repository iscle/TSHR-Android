...
void ReverseCam::Update(unsigned int milliseconds) {
    ...
    if (GetFlag((Flag) FIRST_TIME)) {
        ...
        if (collisionPlane.Intersect(mTargetPosition, rmt::Vector(0.0f, 1.0f, 0.0f),
                                     &intersection)) {
...
bool ReverseCam::CanSwitch() {
    ...
    if (GetFlag((Flag) CUT)) {
        GetSuperCamManager()->GetSCC(GetPlayerID())->SelectSuperCam(SuperCam::FOLLOW_CAM,
                                                                    SuperCamCentral::CUT |
                                                                    SuperCamCentral::FORCE |
                                                                    SuperCamCentral::NO_TRANS, 0);
    } else if (mTarget) {
