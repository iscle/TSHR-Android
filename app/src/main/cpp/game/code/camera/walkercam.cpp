...
void WalkerCam::Update(unsigned int milliseconds) {
    ...
    if (!mTarget->IsCar()) {
        ...
        if (GetFlag((Flag) CUT)) {
            FilterFov(zoom, mData.GetMinFOV(), mData.GetMaxFOV(), FOV, mFOVDelta, 1.0f, timeMod,
                      offset);
        } else {
            FilterFov(zoom, mData.GetMinFOV(), mData.GetMaxFOV(), FOV, mFOVDelta, mData.GetFOVLag(),
                      timeMod, offset);
        }
...
void WalkerCam::UpdateForPhysics(unsigned int milliseconds) {
    ...
    if (mNumCollisions || hasGroundOffset || GetFlag((Flag) CUT)) {
        ...
        if (mMagnitude > (mData.GetMaxMagnitude() * 1.2f) ||
            ...
            IntersectionList &iList = GetSuperCamManager()->GetSCC(
                    GetPlayerID())->GetIntersectionList();
            ...
            if (iList.LineOfSight(lookFrom, lookTo, 0.1f, true)) {
                ...
                if (rmt::Epsilon(intersection.MagnitudeSqr(), 0.0f, 0.01f) ||
                    fromToIntersection.MagnitudeSqr() <
                    (mData.GetMinMagnitude() * mData.GetMinMagnitude())) {
                    //Trouble.
...
void WalkerCam::UpdatePositionNormal(unsigned int milliseconds, float timeMod) {
    ...
    if (!GetFlag((Flag) CUT)) {
        ...
        if (!mTarget->IsCar() &&
            ...
            rmt::Vector vup;
            GetCameraUp(&vup);
            camMat.FillHeading(heading,
                               vup);  //I do this because the tCamera may not be the same as I think it is.
...
//
//=============================================================================
bool WalkerCam::UpdatePositionOneCollsion(unsigned int milliseconds, float timeMod,
...
float WalkerCam::IsTargetNearPed(unsigned int milliseconds) {
    ...
    if (mLastCharacter != static_cast<tUID>(0)) {
        ...
        if (c) {
            ...
            float distNormal =
                    targetToPed.Magnitude() / (PED_MAX_DIST - PED_MIN_DIST);  //Square root!
