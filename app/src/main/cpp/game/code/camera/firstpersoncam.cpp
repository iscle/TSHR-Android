...
void FirstPersonCam::Update(unsigned int milliseconds) {
    ...
    if (GetInputManager()->GetGameState() == Input::ACTIVE_GAMEPLAY ||
        GetInputManager()->GetGameState() == Input::ACTIVE_ALL) {
    ...
    desiredRot =
            mRotation + (MAX_ROT_ANGLE * -(mController->GetAxisValue(SuperCamController::stickX)));
    ...
    desiredElev =
            MAX_ELEV_ANGLE * (invert * mController->GetAxisValue(SuperCamController::stickY)) +
            DEFAULT_ELEVATION;
    ...
    rmt::SphericalToCartesian(DEFAULT_MAGNITUDE, mRotation, mElevation, &target.x, &target.z,
                              &target.y);
    ...
    float FOV = GetFOV();
    FilterFov(zoom, FIRST_PERSON_CAM_MIN_FOV, FIRST_PERSON_CAM_MAX_FOV, FOV, mFOVDelta,
