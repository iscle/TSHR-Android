...
CollisionAttributes *
ATCManager::CreateCollisionAttributes(unsigned int classtypeid, unsigned int physpropid,
                                      float volume) {
    ...
    CollisionAttributes *p_collisionattributes = new(GMA_LEVEL_OTHER) CollisionAttributes(
            mp_ATCTable[physpropid].mSound, mp_ATCTable[physpropid].mParticle,
            mp_ATCTable[physpropid].mAnimation,
            mp_ATCTable[physpropid].mFriction, mp_ATCTable[physpropid].mMass,
            mp_ATCTable[physpropid].mElasticity,
            classtypeid, volume);
...
void ATCManager::Init(void) {
    GetLoadingManager()->AddRequest(FILEHANDLER_PURE3D, "art\\atc\\atc.p3d", GMA_PERSISTENT,
                                    "Default");
}
