...
bool Skidmark::IsVisible() const {
    ...
    if (GetSuperCamManager()->GetSCC(0)->GetCamera()->SphereVisible(boundingSphere.centre,
...
void Skidmark::Display() {
    ...
    pddiPrimStream *pStream = p3d::pddi->BeginPrims(mpShader->GetShader(), PDDI_PRIM_TRISTRIP,
                                                    PDDI_V_CT, m_NumSegments * 2);
    for (int i = 0; i < m_NumSegments; i++) {
        tColour colour(m_Segments[i].intensity, m_Segments[i].intensity, m_Segments[i].intensity,
                       m_Segments[i].intensity);
...
void Skidmark::Extend(const rmt::Vector &position, const rmt::Vector &forward,
                      const rmt::Vector groundNormal, float halfWidth, float intensity) {
    ...
    if (m_NumSegments > 0) {
        ...
        if (FindTextureBreakPosition(elevatedPosition, &textureBreakPosition, &breakPointU,
                                     &distanceToTextureBreakPoint, &distanceToGivenPoint)) {
            float distanceFromStart =
                    distanceToTextureBreakPoint + m_Segments[m_NumSegments - 1].distanceFromStart;
            ...
            float currentU = breakPointU +
                             distanceToTextureBreakPoint * TEX_UNITS_PER_METER * m_TextureDirection;
            if (currentU > 1.0f)
                currentU = 1.0f;
            else if (currentU < 0)
                currentU = 0;
            ...
            distanceFromStart =
                    distanceToGivenPoint + m_Segments[m_NumSegments - 1].distanceFromStart;
            WriteVertices(elevatedPosition, right, halfWidth, ucIntensity, currentU,
                          distanceFromStart);
            WriteVertices(elevatedPosition, right, halfWidth, ucIntensity, currentU,
                          distanceFromStart);
            ...
            ExtendVertices(elevatedPosition, right, halfWidth, ucIntensity, currentU,
                           distanceFromStart);
            WriteVertices(elevatedPosition, right, halfWidth, ucIntensity, currentU,
                          distanceFromStart);
            m_LastPositionWritten = elevatedPosition;
...
void Skidmark::FadeInVertices() {
    ...
    for (int i = 0; i < m_NumSegments; i++) {
        int intensity = static_cast<int>(m_Segments[i].distanceFromStart / DISTANCE_TO_FADE_IN *
                                         255.0f);
...
bool Skidmark::FindTextureBreakPosition(const rmt::Vector &position, rmt::Vector *texBreakPosition,
                                        float *u, float *distanceToTextureBreakPoint,
                                        float *distanceToGivenPoint) {
