            ...
            for (int i = 0; i < orList.mUseSize; i++) {
                ...
                for (unsigned int j = 0; j < numLanes; j++) {
                    ...
                    if (lane->mTrafficVehicles.mUseSize < (int) (nDesiredDensity)) {
                        ...
                        rmt::Vector intPts[2];
                        int numIntersections = IntersectLineSphere(startPos, endPos, pSphere,
                                                                   intPts);
                        ...
                        for (int k = 0; k < numIntersections; k++) {
                            ...
                            if (mNumTraffic < maxTrafficToUse) {
                                ...
                                minDistSqr = TrafficManager::REMOVE_RADIUS *
                                ...
                                if (distVec.MagnitudeSqr() < minDistSqr) {
                                    ...
                                    v->mTrafficLocomotion->InitSegment(segment,
                                    ...
                                    RoadSegment *lastSeg = road->GetRoadSegment(
                                            road->GetNumRoadSegments() - 1);
                                    lastSeg->GetLaneLocation(1.0f, 0, segEnd, segFacing);
                                    const float MIN_DIST_FROM_ROAD_END_SQR = 100.0f;
                                    if ((cPos - segEnd).MagnitudeSqr() >
                                        MIN_DIST_FROM_ROAD_END_SQR) {
...
void TrafficManager::UpdateIntersection(unsigned int milliseconds, Vehicle *v,
