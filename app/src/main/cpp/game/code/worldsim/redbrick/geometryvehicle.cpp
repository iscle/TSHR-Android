...
static const GeometryVehicle::VehicleFrameController FRAME_CONTROLLER_DATA[] =
        {
                ...
                {"PTRN_ship",                   NULL, false, eNone,     0,    0,    1.0f /
                                                                                    25.0f, 0},
...
const rmt::Vector DUNE_COLLECTIBLE_ROT(rmt::DegToRadian(-90), rmt::DegToRadian(0),
                                       rmt::DegToRadian(0));
...
static const int NUM_FRAME_CONTROLLERS =
        sizeof(FRAME_CONTROLLER_DATA) / sizeof(FRAME_CONTROLLER_DATA[0]);
...
GeometryVehicle::GeometryVehicle() :
    ...
    */
    ...
    if (!refractiveIndexAdded) {
        radDbgWatchAddFloat(&refractiveIndex, "RefractionShader", "RefraciveIndex", 0, 0, -20.0f,
                            20.0f);
...
GeometryVehicle::~GeometryVehicle() {
    ...
    if (mVariableEmissionParticleSystem) {
        GetRenderManager()->mEntityDeletionList.Add(
                (tRefCounted *&) mVariableEmissionParticleSystem);
        mVariableEmissionParticleSystem = 0;
    }
...
void GeometryVehicle::Display() {
    ...
    if (mVehicleOwner->mVehicleType == VT_USER) {
        ...
        if (!fucked) {
            ...
            if (rmt::Fabs(distance) > 0.01f && rmt::Fabs(distance) <
                                               50.0f)  // Car must move at least 1 cm for an adjustment to be made
            {
                ...
                tShaderVectorBroadcast cbPos(PDDI_SP_ROTVEC,
                                             rmt::Vector(mCurEnvMapRotation, 0.0f, -angle));
                int count = mCompositeDrawable->GetNumDrawableElement();
                for (int i = 0; i < count; i++) {
                    tCompositeDrawable::DrawableElement *pElement = mCompositeDrawable->GetDrawableElement(
                            i);
    ...
    if (mEngineParticleAttr.mType != ParticleEnum::eNull) {
        ...
        if ((mEngineParticleAttr.mType == ParticleEnum::eEngineSmokeLight) ||
            ...
            GetSparkleManager()->AddSmoke(mVehicleOwner->mTransform,
                                          mVehicleOwner->GetSmokeOffset(), movement, ((0.4f -
                                                                                       mVehicleOwner->GetVehicleLifePercentage(
                                                                                               mVehicleOwner->mHitPoints)) *
                                                                                      2.5f));
        } else {
            // draw some particles
...
//    if(mVehicleOwner->mVehicleType == VT_USER)
    ...
    for (int i = 0; i < NUM_BRAKELIGHT_BBQS; i++) {
        if (mBrakeLights[i] != NULL) {
            brakecolour.Set(
                    rmt::Clamp((int) (mOriginalBrakeLightColours[i].Red() * fadeFactor * dayReduce),
                               0, 255),
                    rmt::Clamp(
                            (int) (mOriginalBrakeLightColours[i].Green() * fadeFactor * dayReduce),
                            0, 255),
                    rmt::Clamp(
                            (int) (mOriginalBrakeLightColours[i].Blue() * fadeFactor * dayReduce),
                            0, 255),
                    rmt::Clamp(
                            (int) (mOriginalBrakeLightColours[i].Alpha() * fadeFactor * dayReduce),
                            0, 255)
            );
    ...
    for (int i = 0; i < VehicleCentral::NUM_HEADLIGHT_BBQGS; i++) {
        if (GetVehicleCentral()->mHeadLights[i] != NULL) {
            for (int j = 0; j < GetVehicleCentral()->mHeadLights[i]->GetNumQuads(); j++) {
                headlightcolour.Set(
                        rmt::Clamp(
                                (int) (GetVehicleCentral()->mOriginalHeadLightColours[count].Red() *
                                       fadeFactor * mHeadLightScale), 0, 255),
                        rmt::Clamp(
                                (int) (GetVehicleCentral()->mOriginalHeadLightColours[count].Green() *
                                       fadeFactor * mHeadLightScale), 0, 255),
                        rmt::Clamp(
                                (int) (GetVehicleCentral()->mOriginalHeadLightColours[count].Blue() *
                                       fadeFactor * mHeadLightScale), 0, 255),
                        rmt::Clamp(
                                (int) (GetVehicleCentral()->mOriginalHeadLightColours[count].Alpha() *
                                       fadeFactor * mHeadLightScale), 0, 255)
                );
            ...
            // We set it back.
            GetVehicleCentral()->mHeadLights[i]->GetShader()->SetInt(PDDI_SP_BLENDMODE,
                                                                     PDDI_BLEND_ADD);
        }
    }
        ...
        for (int i = 0; i < NUM_GHOSTGLOW_BBQGS; i++) {
            if (mGhostGlows[i] != NULL) {
                for (int j = 0; j < mGhostGlows[i]->GetNumQuads(); j++) {
                    glowcolour.Set(
                            rmt::Clamp((int) (mOriginalGhostGlowColours[count].Red() * fadeFactor),
                                       0, 255),
                            rmt::Clamp(
                                    (int) (mOriginalGhostGlowColours[count].Green() * fadeFactor),
                                    0, 255),
                            rmt::Clamp((int) (mOriginalGhostGlowColours[count].Blue() * fadeFactor),
                                       0, 255),
                            rmt::Clamp(
                                    (int) (mOriginalGhostGlowColours[count].Alpha() * fadeFactor),
                                    0, 255)
                    );
        ...
        for (int i = 0; i < NUM_NUKEGLOW_BBQGS; i++) {
            if (mNukeGlows[i] != NULL) {
                for (int j = 0; j < mNukeGlows[i]->GetNumQuads(); j++) {
                    glowcolour.Set(
                            rmt::Clamp((int) (mOriginalNukeGlowColours[count].Red() * fadeFactor),
                                       0, 255),
                            rmt::Clamp((int) (mOriginalNukeGlowColours[count].Green() * fadeFactor),
                                       0, 255),
                            rmt::Clamp((int) (mOriginalNukeGlowColours[count].Blue() * fadeFactor),
                                       0, 255),
                            rmt::Clamp((int) (mOriginalNukeGlowColours[count].Alpha() * fadeFactor),
                                       0, 255)
                    );
...
//
//=============================================================================
void GeometryVehicle::SetSkidValues(int wheel, float intensity, rmt::Vector &groundPlaneNormal,
...
void GeometryVehicle::DisplayShadow(BlobShadowParams *BlobParams) {
    ...
    pddiPrimStream *blob = p3d::pddi->BeginPrims(blobShader, PDDI_PRIM_TRIANGLES, PDDI_V_C,
                                                 6 * NumPoints);
    for (int i = 0; i < NumPoints * 2; ++i) {
        ...
        x = (Points[nextIndex][0] + mShadowPointAdjustments[nextAdjust][0]) *
            BlobParams->ShadowScale;
        y = (Points[nextIndex][1] + mShadowPointAdjustments[nextAdjust][1]) *
            BlobParams->ShadowScale;
        blob->Colour(insideColour);
    ...
    // How the soft edge.
    blob = p3d::pddi->BeginPrims(blobShader, PDDI_PRIM_TRISTRIP, PDDI_V_C,
                                 ((2 * NumPoints) + 1) * 2);
    for (int i = 0; i <= NumPoints * 2; ++i) {
        int index = i;
...
void GeometryVehicle::DisplayLights(const HeadLightParams &LightParams) {
    ...
    // Center light.
    pddiPrimStream *light = p3d::pddi->BeginPrims(lightShader, PDDI_PRIM_TRISTRIP, PDDI_V_C,
                                                  (NumPoints << 1) + 1);
    light->Colour(Colour);
    ...
    pddiPrimStream *nFade = p3d::pddi->BeginPrims(lightShader, PDDI_PRIM_TRISTRIP, PDDI_V_C,
                                                  (NumPoints + 1) << 1);
    nFade->Colour(Black);
    ...
    pddiPrimStream *pFade = p3d::pddi->BeginPrims(lightShader, PDDI_PRIM_TRISTRIP, PDDI_V_C,
                                                  (NumPoints + 1) << 1);
    pFade->Colour(Colour);
...
bool GeometryVehicle::GetArt(const char *name) {
    ...
    if (IsTrafficModel()) {
        ...
        if (poseIndex > -1 && body != NULL) {
            ...
            rDebugPrintf(
                    "Warning: Couldn't find tCompositeDrawable prop \"%s\" for traffic vehicle %s. Chassis color randomization will not apply to this vehicle.\n",
                    propname, name);
        }
    ...
    // no wheels
    if (strcmp(name, "frink_v") == 0 || strcmp(name, "honor_v") == 0 ||
        strcmp(name, "hbike_v") == 0 ||
    ...
    if (strcmp(name, "frink_v") == 0) {
        int count = 0;
        tRefCounted::Assign(mRefractionShader[count++],
                            p3d::find<tShader>("special_m")); //must be first
...
void GeometryVehicle::Update(float dt) {
    ...
    if (mVariableEmissionParticleSystem != NULL) {
        mVariableEmissionParticleSystem->SetBias(p3dParticleSystemConstants::EmitterBias::EMISSION,
                                                 animationSpeedBias);
    }
...
//=============================================================================
bool
GeometryVehicle::GetSpecialController(tUID name, VehicleFrameController *outSpecialController) {
    bool wasFound = false;
