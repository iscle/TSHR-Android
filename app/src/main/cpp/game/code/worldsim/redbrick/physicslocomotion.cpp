...
//
//=============================================================================
void PhysicsLocomotion::CompareNormalAndHeight(int index, rmt::Vector &normalPointingAtCar,
...
void PhysicsLocomotion::SetForceApplicationPoints() {
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (mVehicle->mVehicleType == VT_USER &&
            (mVehicle->mVehicleState == VS_SLIP || mVehicle->mVehicleState == VS_EBRAKE_SLIP)) {
...
void PhysicsLocomotion::ApplySuspensionForces(float dt, bool atrest) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        */
        ...
        mCachedSuspensionForceResults[i] = CalculateSuspensionForce(wheel,
                                                                    yVelocityAlongSuspensionAxis,
                                                                    dt, forceToUse);
        ...
        ((ArticulatedPhysicsObject * )(
                mVehicle->mSimStateArticulated->GetSimulatedObject(-1)))->AddForce(force,
...
//=============================================================================
float
PhysicsLocomotion::CalculateSuspensionForce(Wheel *wheel, float suspensionPointYVelocity, float dt,
                                            float &forceToUse) {
    ...
    if (wheel->mWheelInCollision &&
        tip > cos65) // minor TODO - should this number match any others?
    {
        // test
        //
        ...
        {
            if ((mVehicle->mDamperShouldNotPullDown[wheel->mWheelNum]) ||
                mVehicle->mDoingJumpBoost) {
...
void PhysicsLocomotion::LowSpeedTest() {
    ...
    if (count > 1) {
        ...
        if (mVehicle->mSpeed < lowspeed && mVehicle->mGas < 0.1f /*&& mVehicle->mReverse <0.1f*/ &&
            mVehicle->mBrake < 0.1f)  // new... brake
        {
...
void PhysicsLocomotion::ApplyAngularDrag() {
    if (mVehicle->mPercentOfTopSpeed > 0.01f) {
        ...
        sim::PhysicsObject *phobj = (sim::PhysicsObject * )(
                mVehicle->mSimStateArticulated->GetSimulatedObject());
        rAssert(phobj);
...
void PhysicsLocomotion::ApplyRollingFriction() {
    ...
    if (count > 1) {
        ...
        if (mVehicle->mSpeed > reallylowspeed && mVehicle->mGas == 0.0f) {
            ...
            force.Scale(
                    -1.0f * mVehicle->mRollingFrictionForce * mVehicle->mDesignerParams.mDpMass);
    ...
    //else
    {
        ...
        */
        ...
        if (1)//tip <start)
        {
            ...
            if (!(mVehicle->mWheels[0]->mWheelInCollision) &&
                !(mVehicle->mWheels[3]->mWheelInCollision)) {
                doit = true;

                if (mVehicle->mWheels[1]->mWheelInCollision &&
                    mVehicle->mWheels[2]->mWheelInCollision) {
            ...
            if (0)//(slowdowntipping)
            {
                ...
                float facingAngVel = (mVehicle->mSimStateArticulated->GetAngularVelocity()).DotProduct(
                        mVehicle->mVehicleFacing);
                ...
                if ((facingAngVel > 0.0f && mVehicle->mVehicleTransverse.DotProduct(up) > 0.0f) ||
                    ...
                    torque.Scale(
                            -1.0f * magicshit * mVehicle->mDesignerParams.mDpMass * facingAngVel);
        ...
        if (doit) {
            ...
            if (!(mVehicle->mWeebleOn)) {
                ...
                */
                ...
                rmt::Vector current = ((ArticulatedPhysicsObject * )(
                        mVehicle->mSimStateArticulated->GetSimulatedObject(
                                -1)))->GetExternalCMOffset();
            ...
            if (mVehicle->mWeebleOn) {
                ((ArticulatedPhysicsObject * )(mVehicle->mSimStateArticulated->GetSimulatedObject(
                        -1)))->SetExternalCMOffset(mVehicle->mCMOffset);
            }
...
void PhysicsLocomotion::Update(float dt) {
    ...
    if (mVehicle->mSimStateArticulated->GetControl() != sim::simAICtrl) {
        ...
        if (GetCheatInputSystem()->IsCheatEnabled(CHEAT_ID_NO_TOP_SPEED) &&
            mVehicle->mVehicleType == VT_USER) {
            // no drag for you!
        } else {
            if (!(mVehicle->mSteeringWheelsOutOfContact) && !(mVehicle->mDoSpeedBurst) &&
                !(mVehicle->mDoingJumpBoost)) {
                ApplyDragForce();
            }
        }
        ...
        ((ArticulatedPhysicsObject * )(
                mVehicle->mSimStateArticulated->GetSimulatedObject(-1)))->Update(dt);
        ...
        if (tip < test) {
            ...
            if (side > 0.0f) {
                ...
                if (mVehicle->mUnmodifiedInputWheelTurnAngle < 0.0f) {
                    ...
                    torque.Scale(mVehicle->mUnmodifiedInputWheelTurnAngle * 1.0f *
                                 mVehicle->mDesignerParams.mDpMass * magicshit);
...
void PhysicsLocomotion::PostUpdate() {
    ...
    mVehicle->mPercentOfTopSpeed =
            mVehicle->mSpeed / (mVehicle->mDesignerParams.mDpTopSpeedKmh / 3.6f);
    ...
    for (int i = 0; i < 4; i++) {
        int index = i ^
                    2; // We do this to change i from 0, 1, 2, 3 into 2, 3, 0, 1 so that the rear wheels are processed last.
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (foundPlane) {
...

            {
                ...
                if (penetratingDepth > 0.0f) {
                    ...
                    fixAlongSuspensionAxis = fixHeight / rmt::Fabs(
                            (mVehicle->mVehicleUp).DotProduct(fixHeightNormal));
        ...
        if (0)//foundTri)
        {
            ...
            if (trans.y < mTerrainIntersectCache[i].closestTriPosn.y) {
                ...
                float tempFixAlongSuspensionAxis =
                        fixHeight / rmt::Fabs((mVehicle->mVehicleUp).DotProduct(fixHeightNormal));
            ...
            if (checkBump) {
                ...
                // TODO - what angle?
                if (rmt::Fabs(fixHeightNormal.DotProduct(mVehicle->mVehicleUp)) <
                    sin20)// && fixHeight> wheel->mRadius)
                {
