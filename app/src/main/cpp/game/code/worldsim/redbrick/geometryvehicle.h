...
struct HeadLightParams {
    HeadLightParams(const rmt::Vector &Pos, const rmt::Vector &Normal, const rmt::Vector &Forward,
                    const rmt::Vector &Up) :
...
public:
    ...
    void SetLightsOffDueToDamage(
            bool lightsOffDueToDamage) { mLightsOffDueToDamage = lightsOffDueToDamage; }
...
private:
    ...
    bool
    GetArt(const char *name);    // a bit obscure, but use return value to say true for localized damage textures and shaders present
    ...
    void SetCollectibleHardpointTransform(float rotx, float roty, float rotz,
    ...
    bool FindDamageShadersAndTextures(
            const char *name);  // only returns true if localized damage textures and shaders are present
    //void TriggerDamage();
