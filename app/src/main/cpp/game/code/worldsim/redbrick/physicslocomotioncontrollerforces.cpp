...
void PhysicsLocomotion::ApplyControllerForces2(float dt) {
    ...
    for (i = 0; i < 4; i++) {
        ((ArticulatedPhysicsObject * )(
                mVehicle->mSimStateArticulated->GetSimulatedObject(-1)))->AddForce(totalForce[i],
                                                                                   &(mForceApplicationPoints[i]));
    }
}
...
void PhysicsLocomotion::GasForce2(rmt::Vector &forceResult, int wheelNum, float dt) {
    if (mVehicle->mWheels[wheelNum]->mDriveWheel) {
        ...
        if (mVehicle->mPercentOfTopSpeed < burnoutRange && mVehicle->mGas > gasvalue &&
            ...
            desiredForceMag = mVehicle->mDesignerParams.mDpGasScale * mVehicle->mSlipGasModifier *
                              mVehicle->mGas * mVehicle->mDesignerParams.mDpMass;// * gravFactor;

            desiredForceMag *= gasdecrease;

            //mVehicle->mBurnoutLevel = burnoutsound;
            mVehicle->mBurnoutLevel = (1.0f -
                                       (0.5f * (mVehicle->mPercentOfTopSpeed / burnoutRange)));
            ...
            if (rmt::Fabs(mVehicle->mUnmodifiedInputWheelTurnAngle) > 0.6) {
                ...
                ((ArticulatedPhysicsObject * )(
                        mVehicle->mSimStateArticulated->GetSimulatedObject(-1)))->AddTorque(
                        etorque);
                ...
                if (mVehicle->mEBrake > 0.5f) {
                    ...
                    if (mVehicle->mSpeedBurstTimer >
                        mVehicle->mDesignerParams.mDpMaxSpeedBurstTime) {
            ...
            if (mVehicle->mGas > 0.0f) {
                ...
                if (mVehicle->mVehicleState == VS_NORMAL) {
                    if (mVehicle->mVehicleType == VT_USER && mVehicle->mPercentOfTopSpeed >
                                                             mVehicle->mDesignerParams.mDpGasScaleSpeedThreshold) {
                        // use high speed gas scale
                        desiredForceMag = mVehicle->mDesignerParams.mDpHighSpeedGasScale *
                                          mVehicle->mSlipGasModifier * mVehicle->mGas *
                                          mVehicle->mDesignerParams.mDpMass;// * gravFactor;

                    } else {
                        desiredForceMag =
                                mVehicle->mDesignerParams.mDpGasScale * mVehicle->mSlipGasModifier *
                                mVehicle->mGas * mVehicle->mDesignerParams.mDpMass;// * gravFactor;
                    ...
                    desiredForceMag =
                            mVehicle->mDesignerParams.mDpSlipGasScale * mVehicle->mSlipGasModifier *
                            mVehicle->mGas * mVehicle->mDesignerParams.mDpMass;// * gravFactor;
        ...
        if (mVehicle->mPercentOfTopSpeed < mVehicle->mDesignerParams.mDpWheelieRange &&
            mVehicle->mGas > gasvalue && mVehicle->mVehicleType == VT_USER) {
            // set wheelie offsets
            ...
            if (!(mVehicle->mDoingWheelie) && !(mVehicle->mWeebleOn)) {
                ...
                ((ArticulatedPhysicsObject * )(mVehicle->mSimStateArticulated->GetSimulatedObject(
                        -1)))->SetExternalCMOffset(wheelie);
            ...
            ((ArticulatedPhysicsObject * )(
                    mVehicle->mSimStateArticulated->GetSimulatedObject(-1)))->SetExternalCMOffset(
                    mVehicle->mCMOffset);
        ...
        if (mVehicle->mDoingJumpBoost) {
            ...
            if (mVehicle->mGas > 0.5f && mVehicle->mPercentOfTopSpeed > 0.3f &&
                mVehicle->GetSpeedKmh() < 140.0f) {
                ...
                float multiplier = 1.0f +/* mVehicle->mPercentOfTopSpeed */ +(avgGasScale *
                                                                              gasscaleboosteffect);
        ...
        if (mVehicle->mGas == 0.0f &&
            (mVehicle->mDoSpeedBurst || mVehicle->mSpeedBurstTimer > 0.0f)) {
            mVehicle->mSpeedBurstTimer = 0.0f;
            mVehicle->mDoSpeedBurst = false;
        }
    ...
    if (mVehicle->mWheels[wheelNum]->mDriveWheel) {
        ...
        if (proj < 0.0f) {
            ...
            if (mVehicle->mVehicleType == VT_USER && mVehicle->mPercentOfTopSpeed >
                                                     mVehicle->mDesignerParams.mDpGasScaleSpeedThreshold) {
                desiredForceMag =
                        mVehicle->mDesignerParams.mDpHighSpeedGasScale * mVehicle->mBrake *
                        mVehicle->mDesignerParams.mDpMass;
            } else {
                desiredForceMag = mVehicle->mDesignerParams.mDpGasScale * mVehicle->mBrake *
                                  mVehicle->mDesignerParams.mDpMass;
            }
            ...
            if (mVehicle->mGas > 0.1f) {
                desiredForceMag = (mVehicle->mDesignerParams.mDpGasScale) * mVehicle->mBrake *
                                  mVehicle->mDesignerParams.mDpMass *
                                  0.75f;// magic number is gasdecrease when doing burnout
            } else {
                desiredForceMag =
                        (mVehicle->mDesignerParams.mDpBrakeScale + 3.0f) * mVehicle->mBrake *
                        mVehicle->mDesignerParams.mDpMass;// * gravFactor;
            }
        }
...
void PhysicsLocomotion::ReverseForce2(rmt::Vector &forceResult, int wheelNum) {
    if (mVehicle->mWheels[wheelNum]->mDriveWheel) {
        float desiredForceMag = mVehicle->mDesignerParams.mDpGasScale * mVehicle->mReverse *
                                mVehicle->mDesignerParams.mDpMass;
...
void PhysicsLocomotion::EBrakeEffect(rmt::Vector &forceResult, int wheelNum) {
    ...
    if (mVehicle->mWheels[wheelNum]->mDriveWheel &&
        mVehicle->mWheels[wheelNum]->mWheelTurnAngle == 0.0f) {
        ...
        float desiredForceMag = mVehicle->mEBrake *
                                defaultebrakebrakescalethatplumishappywith /*mVehicle->mDesignerParams.mDpBrakeScale*/ *
                                mVehicle->mDesignerParams.mDpMass;// * gravFactor;
    ...
    if (mVehicle->mPercentOfTopSpeed > threshold) {
        ...
        if (mVehicle->mVehicleState == VS_NORMAL) {
            desiredLateralResistanceForce =
                    (mVehicle->mDesignerParams.mDpMass * lateralVelocityProjection *
                     mVehicle->mDesignerParams.mDpTireLateralResistanceNormal) /*mCurrentSteeringForce)*/ *
                    0.25f;
        } else if (mVehicle->mVehicleState == VS_EBRAKE_SLIP) {
            // slip
            {
                ...
                desiredLateralResistanceForce =
                        (mVehicle->mDesignerParams.mDpMass * lateralVelocityProjection *
                         mVehicle->mDesignerParams.mDpTireLateralResistanceSlip) /*mCurrentSteeringForce)*/ *
                        0.25f;
        ...
        } else    // just VS_SLIP
        {
            ...
            desiredLateralResistanceForce =
                    (mVehicle->mDesignerParams.mDpMass * lateralVelocityProjection *
                     mVehicle->mDesignerParams.mDpTireLateralResistanceSlipNoEBrake /*mCurrentSteeringForce*/) *
                    0.25f;


            if (mVehicle->mWheels[wheelNum]->mSteerWheel) {
                desiredLateralResistanceForce *= (1.0f +
                                                  mVehicle->mDesignerParams.mDpSlipEffectNoEBrake);
...
*/
        ...
        if (mVehicle->mVehicleFacing.DotProduct(mVehicle->mVelocityCM) < 0.0f &&
            mVehicle->mEBrake < 0.1 && rmt::Fabs(mVehicle->mUnmodifiedInputWheelTurnAngle) > 0.5f &&
            mVehicle->mGas == 0.0f)//||
        ...
        if (mVehicle->mWheels[wheelNum]->mDriveWheel && mVehicle->mVehicleType == VT_USER &&
            mVehicle->mGas > 0.0f &&
            mVehicle->mPercentOfTopSpeed > gasmodifierthreshold &&
            rmt::Fabs(mVehicle->mWheelTurnAngleInputValue) > 0.1f &&
        ...
        if (mVehicle->mVehicleState == VS_NORMAL) {
            desiredLateralResistanceForce =
                    (mVehicle->mDesignerParams.mDpMass * lateralVelocityProjection *
                     mVehicle->mDesignerParams.mDpTireLateralResistanceNormal) *
                    0.25f;// * gravFactor;
        } else {
            // slip
            desiredLateralResistanceForce =
                    (mVehicle->mDesignerParams.mDpMass * lateralVelocityProjection *
                     mVehicle->mDesignerParams.mDpTireLateralResistanceSlip) *
                    0.25f;// * gravFactor;
    ...
    if (mVehicle->mPercentOfTopSpeed > 0.02f) {
        if (mVehicle->mEBrake >
            0.1) // note: we can be in this state without always holding the button down
        {
            if (mVehicle->mWheels[wheelNum]->mSteerWheel) {
                // increase resistance on steer wheels
                //desiredLateralResistanceForce *= (1.0f + mVehicle->mEBrake * mVehicle->mDesignerParams.mDpEBrakeEffect);
    ...
    */
    ...
    if (mVehicle->mVehicleState == VS_NORMAL) {
        ...
        if (!mVehicle->mOutOfControl) {
            ...
            if ((velDir.DotProduct(mVehicle->mVehicleFacing) > cos15 &&
                 rmt::Fabs(mVehicle->mWheelTurnAngleInputValue) < 0.1f) ||
