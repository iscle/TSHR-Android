...
void
HumanVehicleController::Create(Vehicle *pVehicle, VehicleMappable *pMappable, int controllerId) {
    SetVehicle(pVehicle);
...
void HumanVehicleController::Reset(void) {
    if (!mDisableReset) {
        ...
        GetGuiSystem()->HandleMessage(GUI_MSG_MANUAL_RESET,
                                      reinterpret_cast<unsigned int>(vehicle));
    }
}
...
void HumanVehicleController::Update(float timeins) {
    ...
    switch (vehicle->mTerrainType) {
        ...
        case TT_Dirt:   // Dirt type terrain.
        {
            ...
            {
                GetInputManager()->GetController(mControllerId)->ApplyEffect(RumbleEffect::GROUND2,
                                                                             250);
...
//                    mWheelRumble->SetPPO(120 - (80 * (speed / 80.0f)), 0, 0);
        ...
        case TT_Gravel:    // Loose gravel type terrain.
        {
            if (speed > 40.0f) //Hmmmm...  TODO: allow this to be modified
            {
                GetInputManager()->GetController(mControllerId)->ApplyEffect(RumbleEffect::GROUND4,
                                                                             250);
...
//                    mWheelRumble->SetPPO(120 - (60 * (speed / 80.0f)), 0, 0);
        ...
        case TT_Sand:    // Sand type terrain.
        {
            if (speed > 40.0f) //Hmmmm...  TODO: allow this to be modified
            {
                GetInputManager()->GetController(mControllerId)->ApplyEffect(RumbleEffect::GROUND3,
                                                                             250);
            }
        }
...
//            GetInputManager()->GetController(mControllerId)->ApplyEffect(RumbleEffect::HARD2, 250);
        ...
        case EVENT_RUMBLE_COLLISION: {
            ...
            if (rc->vehicle == GetVehicle() && mControllerId != -1 &&
                rc->vehicle->mSpeedKmh > 1.0f) {
                GetInputManager()->GetController(mControllerId)->ApplyDynaEffect(
                        RumbleEffect::COLLISION1, 333, rc->normalizedForce);
                GetInputManager()->GetController(mControllerId)->ApplyDynaEffect(
                        RumbleEffect::COLLISION2, 333, rc->normalizedForce);
