...
static const int NUM_OBJECTS_THAT_NEVER_GIVE_COINS =
        sizeof(OBJECTS_THAT_NEVER_GIVE_COINS) / sizeof(OBJECTS_THAT_NEVER_GIVE_COINS[0]);
...
#ifndef RAD_RELEASE
    radDbgWatchAddFloat(&mCurrHitnRun, "Current HitnRun Value", "HitnRun", NULL, NULL, 0.0f, 100.0f,
                        false); //false for read and write
    radDbgWatchAddFloat(&mDecayRatePerSecond, "Decay Rate", "HitnRun", NULL, NULL, 0.0f, 100.0f,
                        false); //false for read and write
    radDbgWatchAddFloat(&mDecayRateWhileSpawning, "Decay Rate while spawning", "HitnRun", NULL,
                        NULL, 0.0f, 100.0f, false); //false for read and write
    radDbgWatchAddFloat(&mDecayRateInsidePerSecond, "Interior Decay Rate", "HitnRun", NULL, NULL,
                        0.0f, 100.0f, false); //false for read and write
    radDbgWatchAddFloat(&mVehicleDestroyedDelta, "Vehicle Destroyed Delta", "HitnRun", NULL, NULL,
                        0.0f, 100.0f, false); //false for read and write
    radDbgWatchAddInt(&mVehicleDestroyedCoins, "Vehicle Destroyed Coins", "HitnRun", NULL, NULL, 0,
                      100, false);
    radDbgWatchAddFloat(&mHitBreakableDelta, "Hit Breakable Delta", "HitnRun", NULL, NULL, 0.0f,
                        100.0f, false); //false for read and write
    radDbgWatchAddFloat(&mVehicleHitDelta, "Vehicle Hit Delta", "HitnRun", NULL, NULL, 0.0f, 100.0f,
                        false); //false for read and write
    radDbgWatchAddInt(&mHitBreakableCoins, "Hit Breakable Coins", "HitnRun", NULL, NULL, 0, 100,
                      false);
    radDbgWatchAddFloat(&mHitMoveableDelta, "Hit Moveable Delta", "HitnRun", NULL, NULL, 0.0f,
                        100.0f, false); //false for read and write
    radDbgWatchAddInt(&mHitMoveableCoins, "Hit Moveable Coins", "HitnRun", NULL, NULL, 0, 100,
                      false);
    radDbgWatchAddFloat(&mHitKrustyGlassDelta, "Hit Krusty Glass Delta", "HitnRun", NULL, NULL,
                        0.0f, 100.0f, false); //false for read and write
    radDbgWatchAddInt(&mHitKrustyGlassCoins, "Hit Krusty Glass Coins", "HitnRun", NULL, NULL, 0,
                      100, false);
    radDbgWatchAddFloat(&mColaPropDestroyedDelta, "Cola Prop Destroyed Delta", "HitnRun", NULL,
                        NULL, 0.0f, 100.0f, false); //false for read and write
    radDbgWatchAddInt(&mColaPropDestroyedCoins, "Cola Prop Destroyed Coins", "HitnRun", NULL, NULL,
                      0, 100, false);
    radDbgWatchAddFloat(&mKickNPCDelta, "Kick NPC Delta", "HitnRun", NULL, NULL, 0.0f, 100.0f,
                        false); //false for read and write
    radDbgWatchAddInt(&mKickNPCCoins, "Kick NPC Coins", "HitnRun", NULL, NULL, 0, 100, false);
    radDbgWatchAddFloat(&mPlayerCarHitNPCDelta, "Vehicle Hit NPC Delta", "HitnRun", NULL, NULL,
                        0.0f, 100.0f, false); //false for read and write
    radDbgWatchAddInt(&mPlayerCarHitNPCCoins, "Vehicle Hit NPC Coins", "HitnRun", NULL, NULL, 0,
                      100, false);
    radDbgWatchAddInt(&mBustedCoins, "Coins lost when busted", "HitnRun", 0, 0, 0, 100, false);
    radDbgWatchAddFloat(&mSwitchSkinDelta, "Switch Skin Delta", "HitnRun", NULL, NULL, 0.0f,
                        -100.0f, false); //false for read and write
    radDbgWatchAddFloat(&mChangeVehicleDelta, "Change Vehicle Delta", "HitnRun", NULL, NULL, 0.0f,
                        -100.0f, false); //false for read and write
    radDbgWatchAddInt(&mNumChaseCars, "Number of Chase Vehicles", "HitnRun", NULL, NULL, 0, 5,
                      false);
    radDbgWatchAddFloat(&mDecayDelay, "Delay until meter decay", "HitnRun", NULL, NULL, 0.0f,
                        5000.0f, false); //false for read and write
    radDbgWatchAddFloat(&mDecayDelayWhileSpawning, "Decay delay while spawning", "HitnRun", NULL,
                        NULL, 0.0f, 5000.0f, false); //false for read and write
    radDbgWatchAddFloat(&STOP_HITNRUN, "Spawning Stop Percentage", "HitnRun", NULL, NULL, 0.0f,
                        100.0f, false); //false for read and write
    radDbgWatchAddFloat(&mCopTicketDistance, "Cop Ticket Distance", "HitnRun", NULL, NULL, 0.0f,
                        20.0f, false); //false for read and write
    radDbgWatchAddFloat(&mCopTicketDistanceOnFoot, "Cop Ticket Distance On Foot", "HitnRun", NULL,
                        NULL, 0.0f, 20.0f, false); //false for read and write
    radDbgWatchAddFloat(&mCopTicketSpeedThreshold, "Cop Ticket Speed Threshold", "HitnRun", NULL,
                        NULL, 0.0f, 30.0f, false); //false for read and write
    radDbgWatchAddFloat(&mCopTicketTimeThreshold, "Cop Ticket Time Threshold", "HitnRun", NULL,
                        NULL, 0.0f, 10.0f, false); //false for read and write
    radDbgWatchAddFloat(&mUnresponsiveTime, "Busted Unresponsive Time", "HitnRun", NULL, NULL, 0.0f,
                        10000.0f, false); //false for read and write
#endif
}
    ...
    if (!mHitnRunDisabled && !GetInteriorManager()->IsInside()) {
        ...
        switch (id) {
            case EVENT_VEHICLE_DESTROYED_BY_USER: {
                ...
                if (!isChaseVehicle &&
                    (vehicle->mVehicleType == VT_TRAFFIC || vehicle->mVehicleType == VT_AI) &&
                    vehicle != mImmunityVehicle) {
                    mCurrHitnRun += mVehicleDestroyedDelta;
                    mDecayDelayMS = decayDelay;
                    GetCoinManager()->SpawnCoins(mVehicleDestroyedCoins, vehicle->GetPosition(),
                                                 vehicle->GetGroundY());
                }
            }
                break;
                        ...
                        if (collAttribs) {
                            if (HasntBeenHit((void *) cedsg)) {
                                ...
                                if (id == BreakablesEnum::eKrustyGlassBreaking) {
                                    ...
                                    GetCoinManager()->SpawnCoins(mHitKrustyGlassCoins,
                                                                 simState->GetPosition());
                                    ...
                                    if (DoesObjectGiveCoins(cedsg)) {
                                        ...
                                        GetCoinManager()->SpawnCoins(mHitMoveableCoins,
                                                                     simState->GetPosition());
                                    }
                                } else {
                                    mCurrHitnRun += mHitBreakableDelta;
                                    GetCoinManager()->SpawnCoins(mHitBreakableCoins,
                                                                 simState->GetPosition());
                                }
            ...
            if (mChaseOn) {
                ...
                if (cars <= 0) {
                    ...
                    if (playerVehicle != NULL) {
                        ...
                        float distToPlayerSqr = chaseManager->GetClosestCarPosition(&playerPos,
                                                                                    &copPos);

                        if (currSpeed < mCopTicketSpeedThreshold &&
                            distToPlayerSqr < copTicketDistSqr) {
                        ...
                        if (mTicketTimer >=
                            (mCopTicketTimeThreshold * 1000.0f)) //convert from seconds to MS
                        {
                            GetEventManager()->TriggerEvent(EVENT_HIT_AND_RUN_CAUGHT, NULL);
                            mTicketTimer = 0.0f;
                        }
                    } else {
                        float copTicketDistSqr =
                                (mCopTicketDistanceOnFoot) * (mCopTicketDistanceOnFoot);
                        ...
                        if (mTicketTimer >=
                            (mCopTicketTimeThreshold * 1000.0f * 2)) //convert from seconds to MS
                        {
                            GetEventManager()->TriggerEvent(EVENT_HIT_AND_RUN_CAUGHT, NULL);
                            mTicketTimer = 0.0f;
                        }
        ...
        if (mVehicleResetTimer <= 0.0f) {
            ...
            if (player->IsInCar()) {
                ...
                GetGuiSystem()->HandleMessage(GUI_MSG_MANUAL_RESET,
                                              reinterpret_cast<unsigned int>(playerVehicle));
                GameplayManager *gameplayManager = GetGameplayManager();
                if (gameplayManager != NULL) {
                    ChaseManager *chaseManager = gameplayManager->GetChaseManager(0);
                    if (chaseManager != NULL) {
