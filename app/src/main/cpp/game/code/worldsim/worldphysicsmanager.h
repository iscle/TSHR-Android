...
public:
    ...
    void SubmitStaticsPseudoCallback(rmt::Vector &position, float radius, int collisionAreaIndex,
                                     sim::SimState *callerSimState, bool allowAutoPairing = false);

    void
    SubmitFencePiecesPseudoCallback(rmt::Vector &position, float radius, int collisionAreaIndex,
                                    sim::SimState *callerSimState, bool allowAutoPairing = false);

    //void SubmitFencePiecesPseudoCallback(rmt::Vector& position, float radius, int collisionAreaIndex);
    void SubmitDynamicsPseudoCallback(rmt::Vector &position, float radius, int collisionAreaIndex,
                                      sim::SimState *caller, bool allowAutoPairing = false);

    //bool WorldPhysicsManager::IsASubmitter(sim::SimState* testSimState);

    void
    SubmitAnimCollisionsPseudoCallback(rmt::Vector &position, float radius, int collisionAreaIndex,
                                       sim::SimState *callerSimState);

    void
    SubmitAnimCollisionsForUpdateOnly(rmt::Vector &position, float radius, int collisionAreaIndex);
    ...
    int
    GetNewGroundPlane(sim::SimState *simStateOwner);    // user refers to it with the returned index
    ...
    bool FenceSanityCheck(unsigned collisionAreaIndex, const rmt::Vector lastFrame,
...
private:
    ...
    bool UpdateFencePiece(rmt::Vector &callerPosition, sim::ManualSimState *fencePiece,
                          rmt::Vector &end0, rmt::Vector &end1, rmt::Vector &normal,
                          sim::SimState *callerSimState);
...
inline WorldPhysicsManager *
GetWorldPhysicsManager() { return (WorldPhysicsManager::GetInstance()); }
