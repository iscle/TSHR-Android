...
void Avatar::SetInCarController(void) {
    ...
    mpAvatarInputManager->mVehicleMappableHandle = InputManager::GetInstance()->RegisterMappable(
            mControllerId, pVehicleMappable);
    ...
    mpAvatarInputManager->mCharacterMappableHandle = InputManager::GetInstance()->RegisterMappable(
            mControllerId, pCharacterMappable);
...
void Avatar::SetCameraTargetToVehicle(bool cut) {
    ...
    //
    GetSuperCamManager()->GetSCC(mPlayerId)->SelectSuperCam(SuperCam::FOLLOW_CAM,
                                                            cut ? (SuperCamCentral::CUT |
...
#else
    GetSuperCamManager()->GetSCC(mPlayerId)->SelectSuperCam(SuperCam::WALKER_CAM,
                                                            cut ? SuperCamCentral::CUT : 0);
#endif
}
...
void
Avatar::GetRaceInfo(float &distToCurrCollectible, int &currCollectible, int &numLapsCompleted) {
