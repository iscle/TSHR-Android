        ...
        if (pDSG != NULL) {
            ...
            if (boundingBox.Intersects(boundingSphere)) {
                if (pDSG->IsCollisionEnabled()) {
                    ...
                    if ((pCollisionEntityList != NULL)
                        && (numObjectsAffected <
                            CollisionEntityDSGList::NUM_COLLISION_LIST_ENTITIES)) {
...
void WorldPhysicsManager::InitCollisionManager() {
    ...
    mCollisionManager->SetCollisionManagerAttributes(
            CM_DetectAll); // TODO - should be able to use IfMoving
    ...
    */
    ...
    for (i = 0; i < mNumCollisionAreas; i++) {
        ...
        for (j = 0; j < mMaxFencePerArea; j++) {
            ...
            mFences[i][j].mFenceSimState = (sim::ManualSimState * )(
                    SimState::CreateManualSimState(tempOBBox));
    ...
    mCollisionManager->GetImpulseBasedCollisionSolver()->SetCollisionSolverAgent(
            mpWorldCollisionSolverAgentManager);
...
void WorldPhysicsManager::EmptyCollisionAreaIndex(int index) {
    ...
    for (i = 0; i < mMaxDynamics; i++) {
        if (mCurrentDynamics[index][i].mDynamicPhysDSG) {
            //TODO
            if (mCurrentDynamics[index][i].mDynamicPhysDSG->GetAIRef() !=
                PhysicsAIRef::redBrickVehicle) {
    ...
    for (i = 0; i < mNumCollisionAreas; i++) {
        ...
        for (j = 0; j < mMaxDynamics; j++) {
            if (mCurrentDynamics[i][j].mDynamicPhysDSG == obj) {
                mCollisionManager->RemoveCollisionObject(obj->GetSimState()->GetCollisionObject(),
                                                         i);
                ...
                if (!(vehicle)) {
                    ...
                    {
                        sim::CollisionObject *gpCollObj = mGroundPlanePool->GetSimState(
                                groundPlaneIndex)->GetCollisionObject();
...
void WorldPhysicsManager::OnQuitLevel() {
    ...
    if (!(mGroundPlanePool->FreeAllGroundPlanes())) {
        ...
        rAssertMsg(false,
                   "not all the ground planes were free'd by their dynamic object owners!  bad dynamic objects!\n");
    }
...
//
//=============================================================================
void WorldPhysicsManager::SubmitStaticsPseudoCallback(rmt::Vector &position, float radius,
                                                      int collisionAreaIndex,
                                                      sim::SimState *callerSimState,
                                                      bool allowAutoPairing) {
    rAssert(collisionAreaIndex > WorldPhysicsManager::INVALID_COLLISION_AREA);
                ...
                if (mCurrentStatics[collisionAreaIndex][k].mStaticPhysDSG == 0) {
                    ...
                    //if(!allowAutoPairing)
                    {
                        mCollisionManager->AddPair(currCollObj,
                                                   callerSimState->GetCollisionObject(),
                                                   collisionAreaIndex);
                    }
    ...
    for (i = 0; i < mMaxStatics; i++) {
        if (mCurrentStatics[collisionAreaIndex][i].mStaticPhysDSG != 0 &&
            ...
            mCollisionManager->RemoveCollisionObject(
                    mCurrentStatics[collisionAreaIndex][i].mStaticPhysDSG->mpSimState()->GetCollisionObject(),
                    collisionAreaIndex);
...
//
//=============================================================================
void WorldPhysicsManager::SubmitAnimCollisionsPseudoCallback(rmt::Vector &position, float radius,
                                                             int collisionAreaIndex,
                                                             sim::SimState *callerSimState) {
    rAssert(collisionAreaIndex > WorldPhysicsManager::INVALID_COLLISION_AREA);
        ...
        if (!alreadyIn) {
            ...
            for (k = 0; k < mMaxAnimCollisions; k++) {
                if (mCurrentAnimCollisions[collisionAreaIndex][k].mAnimCollisionEntityDSG == 0) {
                    ...
                    currCollObj->SetAutoPair(false);
                    mCollisionManager->AddCollisionObject(currCollObj, collisionAreaIndex);
                    mCollisionManager->AddPair(currCollObj, callerSimState->GetCollisionObject(),
                                               collisionAreaIndex);
    ...
    for (i = 0; i < mMaxAnimCollisions; i++) {
        if (mCurrentAnimCollisions[collisionAreaIndex][i].mAnimCollisionEntityDSG != 0 &&
            mCurrentAnimCollisions[collisionAreaIndex][i].clean == false) {
            mCollisionManager->RemoveCollisionObject(
                    mCurrentAnimCollisions[collisionAreaIndex][i].mAnimCollisionEntityDSG->GetSimState()->GetCollisionObject(),
                    collisionAreaIndex);
...
//
//=============================================================================
void WorldPhysicsManager::SubmitAnimCollisionsForUpdateOnly(rmt::Vector &position, float radius,
                                                            int collisionAreaIndex) {
    rAssert(collisionAreaIndex > WorldPhysicsManager::INVALID_COLLISION_AREA);
    ...
    for (i = 0; i < numResults; i++) {
        ...
        if (!alreadyIn) {
            ...
            for (k = 0; k < mMaxUpdateAnimCollisions; k++) {
                if (mCurrentUpdateAnimCollisions[collisionAreaIndex][k].mAnimCollisionEntityDSG ==
                    0) {
                    // here is a slot
...
//
//=============================================================================
void WorldPhysicsManager::SubmitDynamicsPseudoCallback(rmt::Vector &position, float radius,
                                                       int collisionAreaIndex,
                                                       sim::SimState *callerSimState,
                                                       bool allowAutoPairing) {
    rAssert(collisionAreaIndex > WorldPhysicsManager::INVALID_COLLISION_AREA);
    ...
    for (i = 0; i < numResults; i++) {
        ...
        if (callerSimState->mAIRefIndex == PhysicsAIRef::redBrickVehicle &&
            curr->mpSimState()->mAIRefIndex == PhysicsAIRef::redBrickVehicle) {
            ...
            if (submittingVehicle->mVehicleType == VT_USER) {
                if (submittedVehicle->mVehicleType == VT_USER) {
                    if (submittingVehicle->mVehicleCentralIndex >
                        submittedVehicle->mVehicleCentralIndex) {
            ...
            for (k = 0; k < mMaxDynamics; k++) {
                if (mCurrentDynamics[collisionAreaIndex][k].mDynamicPhysDSG == 0) {
                    ...
                    */
                    ...
                    if (curr->GetAIRef() != PhysicsAIRef::redBrickVehicle &&
                        curr->GetAIRef() != PhysicsAIRef::PlayerCharacter) {
    ...
    for (int i = 0; i < mMaxDynamics; i++) {
        ...
        if (mCurrentDynamics[collisionAreaIndex][i].mDynamicPhysDSG != 0 &&
            ...
            if (atRest && pObject->GetCollisionAttributes()) {
                atRest = atRest && !((pObject->GetCollisionAttributes()->GetClasstypeid() ==
            ...
            if (atRest || IsBreakable(pObject)) {
                mCollisionManager->RemoveCollisionObject(
                        pObject->mpSimState()->GetCollisionObject(), collisionAreaIndex);
    ...
    for (i = 0; i < mNumCollisionAreas; i++) {
        ...
        for (j = 0; j < mMaxDynamics; j++) {
            if (mCurrentDynamics[i][j].mDynamicPhysDSG &&
                mCurrentDynamics[i][j].mDynamicPhysDSG->GetSimState()->GetControl() ==
                sim::simSimulationCtrl &&
                mCurrentDynamics[i][j].mDynamicPhysDSG->GetSimState()->mAIRefIndex !=
                PhysicsAIRef::redBrickVehicle &&
                mCurrentDynamics[i][j].mDynamicPhysDSG->GetSimState()->mAIRefIndex !=
                PhysicsAIRef::PlayerCharacter)    // TODO - vehicles use also?
...

            {
                ...
                if (foundPlane) {
                    mGroundPlanePool->UpdateGroundPlane(
                            mCurrentDynamics[i][j].mDynamicPhysDSG->GetGroundPlaneIndex(),
...
void WorldPhysicsManager::UpdateDynamicObjects(float dt, int collisionAreaIndex) {
    ...
    for (j = 0; j < mMaxDynamics; j++) {
        if (mCurrentDynamics[collisionAreaIndex][j].mDynamicPhysDSG) {
            ...
            // need to keep updating when "at rest" if it's a onetime movable that has been hit
            if (atRest &&
                mCurrentDynamics[collisionAreaIndex][j].mDynamicPhysDSG->GetCollisionAttributes()) {
                atRest = atRest &&
                         !((mCurrentDynamics[collisionAreaIndex][j].mDynamicPhysDSG->GetCollisionAttributes()->GetClasstypeid() ==
                            PROP_ONETIME_MOVEABLE)
            ...
            if ((mCurrentDynamics[collisionAreaIndex][j].mDynamicPhysDSG->GetSimState()->mAIRefIndex !=
                 PhysicsAIRef::redBrickVehicle) &&
                (mCurrentDynamics[collisionAreaIndex][j].mDynamicPhysDSG->GetSimState()->mAIRefIndex !=
                 PhysicsAIRef::PlayerCharacter) &&
                (!atRest) &&
                (mCurrentDynamics[collisionAreaIndex][j].mDynamicPhysDSG->GetLastUpdate() !=
                 updateFrame)) {
...
void WorldPhysicsManager::UpdateAnimCollisions(float dt, int collisionAreaIndex) {
    ...
    for (j = 0; j < mMaxUpdateAnimCollisions; j++) {
        if (mCurrentUpdateAnimCollisions[collisionAreaIndex][j].mAnimCollisionEntityDSG) {
            if (mCurrentUpdateAnimCollisions[collisionAreaIndex][j].mAnimCollisionEntityDSG->GetLastUpdate() !=
                updateFrame) {
                // set the update tick so we wont update this again
                mCurrentUpdateAnimCollisions[collisionAreaIndex][j].mAnimCollisionEntityDSG->SetLastUpdate(
                        updateFrame);
                mCurrentUpdateAnimCollisions[collisionAreaIndex][j].mAnimCollisionEntityDSG->Update(
                        dt);
            }
        }
...
//
//=============================================================================
void WorldPhysicsManager::SubmitFencePiecesPseudoCallback(rmt::Vector &position, float radius,
                                                          int collisionAreaIndex,
                                                          sim::SimState *callerSimState,
                                                          bool allowAutoPairing) {
    rAssert(collisionAreaIndex > WorldPhysicsManager::INVALID_COLLISION_AREA);
    ...
    for (i = 0; i < numResults; i++) {
        ...
        if (UpdateFencePiece(position, mFences[collisionAreaIndex][i].mFenceSimState,
            ...
            if (mFences[collisionAreaIndex][i].mInCollision == false) {
                ...
                if (allowAutoPairing) {
                    mFences[collisionAreaIndex][i].mFenceSimState->GetCollisionObject()->SetAutoPair(
                            true);
                } else {
                    mFences[collisionAreaIndex][i].mFenceSimState->GetCollisionObject()->SetAutoPair(
                            false);
                ...
                mCollisionManager->AddCollisionObject(
                        mFences[collisionAreaIndex][i].mFenceSimState->GetCollisionObject(),
                        collisionAreaIndex);
                mCollisionManager->AddPair(callerSimState->GetCollisionObject(),
                                           mFences[collisionAreaIndex][i].mFenceSimState->GetCollisionObject(),
                                           collisionAreaIndex);
...
//=============================================================================
bool
WorldPhysicsManager::UpdateFencePiece(rmt::Vector &callerPosition, sim::ManualSimState *fencePiece,
                                      rmt::Vector &end0,
                                      rmt::Vector &end1, rmt::Vector &normal,
                                      sim::SimState *callerSimState) {
    ...
    if (test.DotProduct(normal) < 0.0f &&
        callerSimState->mAIRefIndex != PhysicsAIRef::CameraSphere) {
...
bool WorldPhysicsManager::FenceSanityCheck(unsigned collisionAreaIndex, const rmt::Vector lastFrame,
                                           const rmt::Vector thisFrame, rmt::Vector *fixPos) {
