    ...
    switch (id) {
        case EVENT_DYNAMIC_ZONE_LOAD_ENDED: {
            ...
            if (mNumLocators > 0) {
                ...
                for (i = 0; i < mNumLocators && i < max &&
                            allocated < mNumCarTypes - mNumParkedCars; ++i) {
                    unsigned int which = rand() % mNumLocators;
                    if (!(mLocators[which]->GetFlag(Locator::ACTIVE))) {
                        //Find another one, this one is used.
                        unsigned int j;
                        for (j = (which + 1) % mNumLocators;
                             j != which; j = (j + 1) % mNumLocators) {
                    ...
                    if (CommandLineOptions::Get(CLO_PARKED_CAR_TEST) ||
                        rand() % mNumCarTypes <= 2)  //Weight it to placing the car.
                    {
                        ...
                        for (j = 0; j < mNumCarTypes; ++j) {
                            ...
                            if (mParkedCars[j].mLoadedZoneUID == static_cast<tUID>(0) &&
                                !inUseByPlayer) {
                                //We found one!
                                info = &mParkedCars[j];
        ...
        case EVENT_DUMP_DYNA_SECTION: {
            ...
            for (i = 0; i < mNumCarTypes; ++i) {
                if (mParkedCars[i].mLoadedZoneUID == sectionNameUID) {
                    ...
                    if (mParkedCars[i].mHusk) {
                        // remove husk
                        bool succeeded = GetVehicleCentral()->RemoveVehicleFromActiveList(
                                mParkedCars[i].mHusk);
                        rAssert(succeeded);
            ...
            if (
                    (sectionNameUID == mFreeCar.mLoadedZoneUID ||
                     mFreeCar.mLoadedZoneUID == static_cast<tUID>(0))
                    &&
                    (mFreeCar.mCar != NULL)
                    ) {
                ...
                if ((GetGameplayManager()->GetCurrentVehicle() == mFreeCar.mCar) ||
                    ...
                    rTuneAssertMsg(mFreeCar.mCar != NULL,
                                   "Attempting to calculate distance from Null free car to Character, Crash Imminent\n");
...
void ParkedCarManager::AddCarType(const char *name) {
    ...
    unsigned int i;
    unsigned int iterations = CommandLineOptions::Get(CLO_PARKED_CAR_TEST) ? NUM_TEST_PARKED_CARS /
                                                                             MAX_DIFFERENT_CARS : 1;
    for (i = 0; i < iterations; ++i) {
        ...
        Vehicle *car = GetVehicleCentral()->InitVehicle(mParkedCars[mNumCarTypes].mName, false,
                                                        NULL,
                                                        VT_TRAFFIC);  //TODO: Should I change the NULL to a con file?
        rAssert(car);
...
void ParkedCarManager::AddFreeCar(const char *name, CarStartLocator *loc) {
    ...
    if (mFreeCar.mCar == NULL) {
        ...
        GetLoadingManager()->AddRequest(FILEHANDLER_LEVEL, fileName, GMA_LEVEL_OTHER,
                                        FREE_CAR_SECTION, FREE_CAR_SECTION, this);
    }
}
...
void ParkedCarManager::RemoveFreeCar() {
    ...
    if (mFreeCar.mCar != NULL) {
        ...
        */
        ...
        if (currentHud != NULL) {
            HudMapIcon *playerCarIcon = currentHud->GetHudMap(0)->FindIcon(
...
void ParkedCarManager::CreateFreeCar() {
    ...
    Vehicle *car = GetVehicleCentral()->InitVehicle(mFreeCar.mName, true, NULL,
                                                    VT_TRAFFIC);  //TODO: Should I change the NULL to a con file?
    rAssert(car);
    car->mCreatedByParkedCarManager = true;
