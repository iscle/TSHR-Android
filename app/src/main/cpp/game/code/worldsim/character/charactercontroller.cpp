...
bool PhysicalController::IsButtonDown(int buttonId) const {
    if (mActive) {
        return GetCharacterMappable()->IsButtonDown(buttonId) ||
               GetIntention() == (eIntention) buttonId;
    } else {
        return false;
    }
}
...
int PhysicalController::TimeSinceChange(int buttonId) const {
    return IsButtonDown(buttonId) ? 0 : GetCharacterMappable()->GetButton(
            buttonId)->TimeSinceChange();
};
...
void CameraRelativeCharacterController::HandleEvent(int id, void *pEventData) {
    ...
    if (EVENT_CAMERA_CHANGE == eventId) {
        bool interior = GetInteriorManager()->IsEntering() || GetInteriorManager()->IsExiting() ||
    ...
    switch (mState) {
        ...
        case STOPPED: {
            ...
            if (cosAlpha < COSALPHA_QUICKTURN_TOLERANCE) {
                ...
                mSecondsSinceLastTurnAnim = SECONDS_BETW_PLAYING_TURN_ANIMS;
            } else if (COSALPHA_QUICKTURN_TOLERANCE <= cosAlpha &&
                       cosAlpha <= COSALPHA_TURN_TOLERANCE) {
            ...
            if (mSecondsInStopped > SECONDS_BACK_TO_FOLLOW_PATH) {
                ...
                //
                bool ignorePeds = (mpCharacter->GetRole() == Character::ROLE_PEDESTRIAN) ? true
                                                                                         : false;
...
void NPCController::FollowPath(float seconds) {
    ...
    if (mUseTempWaypoint) {
        ...
        if ((lastPos - mNPCWaypoints[0]).Magnitude() <
            20.0f) // were close to our path again, clear the temp waypoint
        {
