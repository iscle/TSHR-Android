...
void CharacterRenderable::DisplayShadow(tPose *pose, const BlobShadowParams *BlobParams) {
    if (BlobParams) {
        ...
        const int HalfCircle = NumBlobSlices
                >> 1; // We only keep half the circle and then mirror it. Hence the reason to keep the number of points even.
...
*/
        ...
        for (int i = 0; i < NumBlobSlices; i++) {
            ...
            float nextFadeScale = (nextScaleIndex < HalfCircle) ? BlobParams->ShadowScale
            ...
            x0 *= articulated ? pointScales[i] : (index == i) ? NonArticulatedScale
                                                              : -NonArticulatedScale;
            y0 = BlobPoints[index][1];
            y0 *= BlobParams->ShadowScale;
            y0 *= articulated ? pointScales[i] : (index == i) ? NonArticulatedScale
                                                              : -NonArticulatedScale;
            ...
            x1 *= articulated ? pointScales[nextScaleIndex] : (nextIndex == nextScaleIndex)
                                                              ? NonArticulatedScale
                                                              : -NonArticulatedScale;
            y1 = BlobPoints[nextIndex][1];
            y1 *= BlobParams->ShadowScale;
            y1 *= articulated ? pointScales[nextScaleIndex] : (nextIndex == nextScaleIndex)
                                                              ? NonArticulatedScale
                                                              : -NonArticulatedScale;
