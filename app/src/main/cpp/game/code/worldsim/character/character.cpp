...
public:
    ...
    AmbientDialogueTrigger(Character *c, float radius) : SphereTriggerVolume(rmt::Vector(0, 0, 0),
                                                                             radius),
                                                         mpCharacter(c) {
...
//#endif
...

    {
        ...
        sim::CylinderVolume *pCollisionVolume = new sim::CylinderVolume(offset, vUp, 0.55f,
                                                                        mfRadius);
        sim::CollisionObject *pCollisionObject = new sim::CollisionObject(pCollisionVolume);
        sim::PhysicsProperties *pPhysicsProperties = sim::PhysicsProperties::HardWoodProperties(
                p3d::inventory);
        sim::PhysicsObject *pSimulatedObject = new sim::PhysicsObject(pPhysicsProperties, offset,
                                                                      identity, 2.0f);
        pSimulatedObject->SetSimEnvironment(GetWorldPhysicsManager()->mSimEnvironment);
        sim::SimState *pSimState = sim::SimState::CreateSimState(pCollisionObject,
                                                                 pSimulatedObject);
        rAssert(pSimState);
...
void Character::Slam() {
    ...
    int numObjectsKicked = GetWorldPhysicsManager()->ApplyForceToDynamicsSpherical(
            mCollisionAreaIndex,
            position,
            2,
    ...
    if (numObjectsKicked > 0) {
        for (i = 0;
             i < WorldPhysicsManager::CollisionEntityDSGList::NUM_COLLISION_LIST_ENTITIES; i++) {
...
void Character::RelocateAndReset(const rmt::Vector &position, float facing, bool resetMe,
                                 bool snapToGround) {
    // TODO: Not done yet...
...
//    pPuppet->UpdateEnd();
    ...
    if (mIsNPC && mRole != ROLE_PEDESTRIAN) {
        ...
        bool bOverStatic = GetCollisionHeight(prevPosition, position, collisionPosition,
                                              collisionNormal);
        if (bOverStatic) {
...
void Character::PostSimUpdate(float timeins) {
    ...
    mbNeedChoreoUpdate = mbNeedChoreoUpdate && shouldUpdate && (!mTooFarToUpdate ||
                                                                ((frameCount & modulo) ==
                                                                 (mIntersectFrame & modulo)));
...
END_PROFILE("GetActionButtonHandler")
        */
        ...
        if (!IsInCar()) {
            if (radTimeGetMicroseconds64() - mLastInteriorLoadCheck >
                3000000) //(amortise/3s) if it's been more than 3 sec's since we last checked for volumes
            {
                rmt::Vector posn;
                GetPosition(&posn);
    ...
    if (!mIsNPC &&
        ...
        if (mCollisionAreaIndex != -1) {
            if (GetWorldPhysicsManager()->FenceSanityCheck(mCollisionAreaIndex, mLastFramePos, tmp,
                                                           &tmp)) {
                SetPosition(tmp);
    ...
    if (simState != NULL) {
        if (simState->GetControl() == sim::simAICtrl) {
            ...
            if (shouldSolveCollisions) {
                ...
                Character::eCollisionType collisionType = SolveCollisionWithStatic(desiredPos,
                                                                                   outPos);
                ...
                if (GetDesiredSpeed() > 1.0f) {
                    ...
                    */
                    ...
                    if (!GetInteriorManager()->IsInside()) {
                        rmt::Vector facing;
                        this->GetFacing(facing);
                        GetSparkleManager()->AddDash(mpPuppet->GetFootPosition(i), facing,
                                                     GetDesiredSpeed() * 0.125f);
                    }
...
void Character::AddToWorldScene(void) {
    if (!mVisible && mManaged) {
        ...
        if (mpCharacterRenderable->GetDrawable()) {
            mpPuppet->GetP3DPose()->SetSkeleton(
                    mpCharacterRenderable->GetDrawable()->GetSkeleton());
            pPuppet->UpdateEnd();
        }
        ...
        mpWorldScene = ((WorldRenderLayer *) GetRenderManager()->mpLayer(
                RenderEnums::LevelSlot))->pWorldScene();
...
void Character::MoveInWorldScene(void) {
    if (mVisible) {
        ...
        // now move!
        //
        mpWorldScene->Move(oldBox, (IEntityDSG * )
        this);
    }
}
...
*/
sim::Solving_Answer
Character::PreReactToCollision(sim::SimState *pCollidedObj, sim::Collision &inCollision) {
        ...
        if (vSpeedMps > speedThreshold) {
            if (mpSimStateObj->mAIRefIndex == PhysicsAIRef::NPCharacter) {
                ...
                if (v->mVehicleType == VT_TRAFFIC) {
                    ...
                    // Impart immediate vertical velocity! YA!
                    float randX, randY, randZ;
                    randX = ((float) (rand() % MAX_RAND_MODULUS) + BASE_SPEED_COMPONENT_MOD_MPS) *
                            speedRatio;
                    randY = ((float) (rand() % MAX_RAND_MODULUS) + BASE_SPEED_COMPONENT_MOD_MPS) *
                            speedRatio;
                    randZ = ((float) (rand() % MAX_RAND_MODULUS) + BASE_SPEED_COMPONENT_MOD_MPS) *
                            speedRatio;
                            ...
                            if (chaseManager != NULL) {
                                if (v->mName) {
                                    if (chaseManager->IsModelRegistered(v->mName)) {
                                        GetEventManager()->TriggerEvent(EVENT_HIT_AND_RUN_CAUGHT,
                                                                        NULL);
                                    }
                                }
                            }
...
//            return sim::Solving_Continue;
    ...
    if (bTestNormals) {
        for (int i = 0; i < mCurrentCollision; i++) {
            ...
            if ((mCollisionData[i].mpCollisionVolume == theCollision.mCollisionVolumeA ||
                ...
                bool store = (mCollisionData[i].mCollisionNormal.DotProduct(facing) >
                              theCollision.mNormal.DotProduct(facing));
                ...
                if (store) {
                    ...
                    mCollisionData[i].mpCollisionVolume = theCollision.mCollisionVolumeA ==
                                                          mpSimStateObj->GetCollisionObject()->GetCollisionVolume()
                                                          ? theCollision.mCollisionVolumeB
                                                          : theCollision.mCollisionVolumeA;
                }
    ...
    mCollisionData[mCurrentCollision].mpCollisionVolume = theCollision.mCollisionVolumeA ==
                                                          mpSimStateObj->GetCollisionObject()->GetCollisionVolume()
                                                          ? theCollision.mCollisionVolumeB
                                                          : theCollision.mCollisionVolumeA;
...
//=============================================================================
sim::Solving_Answer
Character::PostReactToCollision(rmt::Vector &impulse, sim::Collision &inCollision) {
    return sim::Solving_Continue;
}
...
=============================================================================
*/
Character::eCollisionType
Character::SolveCollisionWithStatic(const rmt::Vector &desiredPos, rmt::Vector &outPos) {
    ...
    for (i = 0; i < mCurrentCollision; i++) {
        if (!CanStandOnCollisionNormal(mCollisionData[i].mCollisionNormal)) {
            ...
            if (bSolve) {
                ...
                if (mCollisionData[i].mCollisionDistance < 0.0f) {
                    if (sbNegative) {
                        ...
                        if (sbOutput)
                            rDebugPrintf("collision %.4f (%.2f, %.2f, %.2f) %s\n",
                ...
                if (mCollisionData[i].mCollisionNormal.y <= -0.5f) {
                    ...
                    if (mbSurfing && (this->GetLocoVelocity().MagnitudeSqr() < 0.001f) &&
                        mpStandingCollisionVolume) {
                        ...
                        if (objSimState->mAIRefIndex == PhysicsAIRef::redBrickVehicle &&
                            (Vehicle * )(objSimState->mAIRefPointer) ==
                            (Vehicle * )(surfSimState->mAIRefPointer)) {
                            ignoreThisOne = true;
                        }
                    }
                ...
                if (sbOutput)
                    rDebugPrintf("Dot product reject. dot = %.2f, dist = %.6f, %s\n",
                                 dot,
                                 mCollisionData[i].mCollisionDistance,
                                 mCollisionData[i].mpCollisionVolume->GetCollisionObject()->GetName()
                    );
            ...
            if (sbOutput)
                rDebugPrintf("CollisionNormal reject: %.2f, %.2f, %.2f, %s\n",
                             mCollisionData[i].mCollisionNormal.x,
                             mCollisionData[i].mCollisionNormal.y,
                             mCollisionData[i].mCollisionNormal.z,
                             mCollisionData[i].mpCollisionVolume->GetCollisionObject()->GetName()
...
//        UpdateGroundHeight(prevPosition, position, mGroundY, mGroundNormal);
    ...
    if (IsNPC()) {
        ...
        if (delta > TOO_FAR_FROM_GROUND_DIST ||
            ...
            bOverStatic = GetCollisionHeight(prevPosition, position, collisionPosition,
                                             collisionNormal);
        }
        /*
    ...
    if (bOverStatic) {
        ...
        if (IsNPC() && mLastGoodPosOverStatic.Equals(position, 0.5f)) {
            if (((NPCController *) GetController())->GetState() ==
                NPCController::TALKING_WITH_PLAYER) {
    ...
    if ((mVelocity.y <= mfGroundVerticalVelocity && (delta < 0.0f)) ||
        ...
        if (!mbIsStanding && mbDoGroundIntersect && !inJump &&
            (this->mpSimStateObj->GetControl() == sim::simAICtrl)) {
            //falling.
            //
...
*/
sim::CollisionVolume *
Character::FindStandingVolume(const rmt::Vector &inPos, sim::CollisionVolume *inVolume,
    ...
    switch (inVolume->Type()) {
        ...
        case sim::WallVolumeType: {
            ...
            sim::CollisionVolume *pOutSubVolume = sim::FindClosestPointOnVolume(inPos, inVolume,
                                                                                currentNormal,
                                                                                currentDist);
        ...
        case sim::BBoxVolumeType: {
            ...
            for (int i = 0; i < inVolume->SubVolumeList()->GetSize(); i++) {
                sim::CollisionVolume *pOutSubVolume = FindStandingVolume(inPos,
                                                                         inVolume->SubVolumeList()->GetAt(
                                                                                 i), currentNormal,
                                                                         currentDist);
                if (pOutSubVolume && pOutSubVolume->GetCollisionObject()->GetCollisionEnabled()) {
...
=============================================================================
*/
bool Character::GetCollisionHeight(const rmt::Vector &prevPosition, const rmt::Vector &position,
    ...
    //
    GetWorldPhysicsManager()->mCollisionManager->DetectRayIntersection(msIntersectInfo, rayOrg,
                                                                       rayEnd, false,
                                                                       this->GetCollisionAreaIndex());
    ...
    for (i = 0; i < msIntersectInfo.GetSize(); i++) {
        if (msIntersectInfo.GetSize() > 0 && msIntersectInfo[i].mCollisionVolume) {
            ...
            sim::CollisionVolume *pOutVolume = FindStandingVolume(testPosition,
                                                                  msIntersectInfo[i].mCollisionVolume,
                                                                  outNormal, outDist);

            bool vehicleIgnore = false;
            if (this->mpTargetVehicle && pOutVolume) {
                if (pOutVolume->GetCollisionObject()->GetSimState()->mAIRefPointer ==
                    this->mpTargetVehicle) {
            ...
            if (pOutVolume &&
                ...
                (pOutVolume->GetCollisionObject()->GetSimState()->mAIRefIndex !=
                 PhysicsAIRef::redBrickPhizVehicleGroundPlane) &&
                (pOutVolume->GetCollisionObject()->GetSimState()->mAIRefIndex !=
                 PhysicsAIRef::redBrickPhizMoveableGroundPlane) &&
                (!vehicleIgnore)) {
...
void Character::AddActionButtonHandler(ActionButton::ButtonHandler *pActionButtonHandler) {
    if (!IsNPC()) {
        ...
        rAssertMsg(i < MAX_ACTION_BUTTON_HANDLERS,
                   "Need to increase the size of MAX_ACTION_BUTTON_HANDLERS");
        if (i == MAX_ACTION_BUTTON_HANDLERS) {
            return;
        }

        ActionButton::ButtonHandler *newButton = TestPriority(pActionButtonHandler,
                                                              mpCurrentActionButtonHandler);
...
void Character::RemoveActionButtonHandler(ActionButton::ButtonHandler *pActionButtonHandler) {
    if (!IsNPC()) {
        ...
        if (mpCurrentActionButtonHandler == NULL) {
            ...
            for (i = 0; i < MAX_ACTION_BUTTON_HANDLERS; ++i) {
                if (mpActionButtonHandlers[i] != NULL) {
                    mpCurrentActionButtonHandler = TestPriority(mpCurrentActionButtonHandler,
                                                                mpActionButtonHandlers[i]);
                }
            }
...
Character::TestPriority(ActionButton::ButtonHandler *bA, ActionButton::ButtonHandler *bB) {
    if (NULL == bA) {
        return bB;
    } else if (NULL == bB) {
        return bA;
    ...
    if (bAType == ActionButton::ButtonHandler::GET_IN_CAR ||
        bBType == ActionButton::ButtonHandler::GET_IN_CAR) {
    ...
    if (GetRenderLayer() == GetRenderManager()->rCurWorldRenderLayer()) {
        ...
        if (true) //!(IsInCar()))
        {
            ...
            if (collisionAreaIndex > WorldPhysicsManager::INVALID_COLLISION_AREA) {
                ...
                GetWorldPhysicsManager()->SubmitStaticsPseudoCallback(position, sfCollisionRadius,
                                                                      collisionAreaIndex,
                                                                      GetSimState(), true);

                GetWorldPhysicsManager()->SubmitFencePiecesPseudoCallback(position,
                                                                          sfCollisionRadius,
                                                                          collisionAreaIndex,
                                                                          GetSimState(), true);
            }
    ...
    if (GetRenderLayer() == GetRenderManager()->rCurWorldRenderLayer()) {
        ...
        if (true) //!(IsInCar()))
        {
            ...
            if (collisionAreaIndex > WorldPhysicsManager::INVALID_COLLISION_AREA) {
                ...
                GetWorldPhysicsManager()->SubmitAnimCollisionsPseudoCallback(position,
                                                                             sfCollisionRadius,
                                                                             collisionAreaIndex,
                                                                             GetSimState());
                static float sfUpdateRadius = 30.0f;//0.0f;
                GetWorldPhysicsManager()->SubmitAnimCollisionsForUpdateOnly(position,
                                                                            sfUpdateRadius,
                                                                            collisionAreaIndex);
            }
        }
    ...
    if (GetRenderLayer() == GetRenderManager()->rCurWorldRenderLayer()) {
        ...
        if (true) //!(IsInCar()))
        {
            ...
            if (collisionAreaIndex > WorldPhysicsManager::INVALID_COLLISION_AREA) {
                ...
                GetWorldPhysicsManager()->SubmitDynamicsPseudoCallback(position, sfCollisionRadius,
                                                                       collisionAreaIndex,
                                                                       GetSimState(), true);
            }
        }
...
void Character::Display(void) {
    ...
    if (IsMarge() &&
        ...
        if (numJoints >= 36) {
            mpPuppet->GetPose()->GetJoint(33)->SetObjectMatrix(
                    mpCharacterRenderable->GetDrawable()->GetSkeleton()->GetJoint(33)->restPose);
            mpPuppet->GetPose()->GetJoint(34)->SetObjectMatrix(
                    mpCharacterRenderable->GetDrawable()->GetSkeleton()->GetJoint(34)->restPose);
            mpPuppet->GetPose()->GetJoint(35)->SetObjectMatrix(
                    mpCharacterRenderable->GetDrawable()->GetSkeleton()->GetJoint(35)->restPose);
        }
    }
    ...
    if (npcController != NULL) {
        ...
        bool npcStateNeedsToSubmit =
                ...
                (
                        (GetStateManager()->GetState() != CharacterAi::INSIM) &&
                        (
                                (npcController->mOffPath &&
                                 state == NPCController::FOLLOWING_PATH) ||
            ...
            if (GetRenderLayer() == GetRenderManager()->rCurWorldRenderLayer()) {
                ...
                if (collisionAreaIndex != WorldPhysicsManager::INVALID_COLLISION_AREA) {
                    if (collisionAreaIndex > WorldPhysicsManager::INVALID_COLLISION_AREA) {
                        ...
                        GetWorldPhysicsManager()->SubmitStaticsPseudoCallback(position,
                                                                              sfCollisionRadius,
                                                                              collisionAreaIndex,
                                                                              GetSimState());
