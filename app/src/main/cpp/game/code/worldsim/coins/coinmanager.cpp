...
CoinManager::CoinManager() :
        ...
        mDrawAfterGui(false) {
#ifndef RAD_RELEASE
    radDbgWatchAddFloat(&COIN_SPAWN_VELOCITY, "Spawn - Initial velocity", "Coins", 0, 0, 0.01f,
                        5.0f);
    radDbgWatchAddFloat(&COIN_EXTRA_VERTICAL, "Spawn - Initial extra vertical", "Coins", 0, 0, 0.0f,
                        2.5f);
    ...
    radDbgWatchAddFloat(&BOUNCE_DAMPENING, "Spawn - Bounce energy dampening", "Coins", 0, 0, 0.0f,
                        2.0f);
    ...
    radDbgWatchAddFloat(&IN_CAR_RANGE_MULTIPLIER, "Collection - In car multiplier", "Coins", 0, 0,
                        0.0f, 5.0f);
    radDbgWatchAddFloat(&DECAY_EXTRA_SPIN, "Animation - Extra Decay spin", "Coins", 0, 0, 0.0f,
                        50.0f);
    radDbgWatchAddFloat(&SPIN_MULTIPLIER, "Animation - Spin multiplier", "Coins", 0, 0, 0.01f,
                        5.0f);
    radDbgWatchAddFloat(&COLLECT_TIME, "Animation - Collection duration", "Coins", 0, 0, 0.1f,
                        5.0f);
    ...
    radDbgWatchAddFloat(&MAX_VISIBLITY, "Visibility culling distance", "Coins", 0, 0, 10.0f,
                        100.0f);
...
the position.
=============================================================================*/
void CoinManager::SpawnCoins(int Count, const rmt::Vector &Position, const rmt::Vector *Direction,
                             bool Force) {
    SpawnCoins(Count, Position, Position.y, Direction, Force);
}

void CoinManager::SpawnCoins(int Count, const rmt::Vector &Position, float GroundY,
                             const rmt::Vector *Direction, bool Force) {
    rAssert(Count >= 0);
    if (Count <= 0) {
        return;
    }
...
Spawn a single coin.
=============================================================================*/
void CoinManager::SpawnCoin(ActiveCoin &Coin, const rmt::Vector &Start, float Ground,
                            const rmt::Vector *Direction, bool InstaCollect) {
    Coin.Position = Start;
...
void CoinManager::Update(int ElapsedMS) {
    ...
    for (int i = 0; i < NUM_COINS; ++i) {
        ...
        if (c.State != CS_Inactive) {
            ...
            if (c.State != CS_InitialSpawning && c.State != CS_SpawnToCollect &&
                c.State != CS_Collecting && c.State != CS_Collected) {
            ...
            if (c.State == CS_Collecting) {
                ...
                UpdateHUDFlying(c);
            } else if (c.State != CS_RestingIndefinitely && c.State != CS_Collecting &&
                       c.State != CS_Collected) {
                if (c.Age > LIFE_TIME + DECAY_TIME) {
                    // Coin has expired.
                    c.State = CS_Inactive;
                    --mNumActiveCoins;
                    GetSparkleManager()->AddSparkle(
                            rmt::Vector(c.Position.x, c.Position.y + mCoinBounding.radius,
                                        c.Position.z), 0.25f, 0.5f, rmt::Vector(0.0f, 0.01f, 0.0f),
                            Sparkle::SE_Vanish);
                ...
                if (c.State == CS_Spawning || c.State == CS_InitialSpawning ||
                    c.State == CS_SpawnToCollect) {
            ...
            rmt::SinCos((c.Age * SPIN_MULTIPLIER) + (decaySpin * DECAY_EXTRA_SPIN), &coinSin,
                        &coinCos);
...
=============================================================================*/
void CoinManager::CheckCollection(ActiveCoin &Coin, const rmt::Vector &AvatarPos,
    ...
    if ((x < (RANGE * RangeMultiplier)) && (z < (RANGE * RangeMultiplier)) && (y < RANGE) &&
        (x + z < (INNER_RANGE * RangeMultiplier))) {
...
void CoinManager::LoseCoins(int Count, const rmt::Vector *Position) {
    ...
    if (Position) {
        ...
        SpawnCoins(count, rmt::Vector(Position->x, Position->y + 0.25f, Position->z), Position->y,
                   0, true);
    }
#ifdef RAD_GAMECUBE
    HeapMgr()->PushHeap(GMA_GC_VMM);
...
=============================================================================*/
void
CoinManager::UpdateSpawning(ActiveCoin &Coin, const rmt::Vector &AvatarPos, float DeltaSeconds) {
    rmt::Vector vel;
    ...
    */
    if (Coin.State == CS_InitialSpawning) {
        ...
        if (DoSparkle < 0) {
            DoSparkle = Sparkle_Rate;
            Sparkle_Rate ^= SPARKLE_RATE_TOGGLE;
            GetSparkleManager()->AddSparkle(Coin.Position, 0.5f, 1.0f,
                                            rmt::Vector(Coin.Velocity.x * 0.1f,
                                                        (Coin.Velocity.y * 0.1f) + SPARKLE_FLOAT,
                                                        Coin.Velocity.z * 0.1f), Sparkle::SE_Trail);
        }
    }
...
void
CoinManager::AddGlint(ActiveCoin &Coin, const rmt::Vector &ToCamera, const rmt::Vector &CoinAxis,
        ...
        for (int i = 0; i < NUM_COINS; ++i) {
            ...
            if (c.State != CS_Inactive) {
                ...
                if ((GlintDelay <= 0) && (DoGlint == i)) {
                    if (c.State != CS_InitialSpawning && c.State != CS_Collecting &&
                        c.State != CS_Collected) {
                        if (GetGameFlow()->GetCurrentContext() != CONTEXT_PAUSE) {
                            AddGlint(c, diff, rmt::Vector(c.HeadingCos, 0.0f, -c.HeadingSin),
                                     camTrans.Row(0));
                        }
                    }
                }
    ...
    if ((mNumHUDFlying > 0) || renderHUDCoin) {
        ...
        if (renderHUDCoin) {
            ...
            if (mHUDSparkle <= 0) {
                ...
                float radius =
                        0.2f * ((coinCos < 0.0f) ? mCoinBounding.radius : -mCoinBounding.radius);
                pos.Add(rmt::Vector(coinCos * radius, 0.0f, coinSin * radius));
                GetSparkleManager()->AddSparkle(pos, 1.0f, 0.25f, rmt::Vector(0.0f, 0.0f, 0.0f),
                                                Sparkle::SE_HUDGlint);
