...
void NormalEngineState::Service(unsigned int elapsedTime) {
    ...
    if ((m_interpolateTime > 0) && !m_isAttacking) {
        ...
        if (elapsedTime > m_interpolateTime) {
            ...
            m_currentTrim += trimDiff * (1.0f -
                                         m_parameters->GetDecayFinishTrim());  // Increase at half-speed because we start at 0.5 above
...
VehicleSoundPlayer::VehicleSoundPlayer() :
    ...
    m_engineStates[ENGINE_STATE_REVERSE] = new ReverseEngineState(
            &(m_soundPlayers[CARPLAYER_BACKUP_BEEP]));
...
void VehicleSoundPlayer::UpdateSoundParameters(unsigned int elapsedTime) {
    ...
    if (m_vehicle != NULL) {
        ...
        if (!warningPrinted) {
            rTuneString(
                    "Warning: Vehicle doesn't have a sound associated with it (CarSoundParameters)\n");
            warningPrinted = true;
        }
    }
}
...
void VehicleSoundPlayer::CheckHorn() {
    ...
    if (buttonVal > 0.05f) {
        if (!m_hornPlaying) {
            ...
            //
            resource = Sound::daSoundRenderingManagerGet()->GetResourceManager()->FindResource(
                    hornClipName);
...
void VehicleSoundPlayer::checkDamage() {
    ...
    if ((lifePercentage <= damagePercentage) && (!m_playingDamage)) {
        ...
        //
        damageTrimPercentage =
                (damagePercentage - lifePercentage) / m_parameters->GetDamageVolumeRange();
