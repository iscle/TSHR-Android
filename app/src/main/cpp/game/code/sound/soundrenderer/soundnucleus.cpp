...
namespace Sound {
    ...
    void CreateStreamerResources(StreamerResources *pSi, radMemoryAllocator alloc) {
        ...
        if (STREAM_USE_BUFFERED_DATA_SOURCES && (false == CommandLineOptions::Get(CLO_FIREWIRE))) {
            ...
            pSi->m_pBufferedDataSource->Initialize(
                    ...
                    radMemorySpaceGetAllocator(STREAM_BUFFERED_DATA_SOURCE_MEMORY_SPACE,
                                               RADMEMORY_ALLOC_DEFAULT), // This is IOP Memory
    ...
    void SoundNucleusInitialize(radMemoryAllocator alloc) {
        ...
        for (unsigned int i = 0; i < SOUND_NUM_STREAM_PLAYERS; i++) {
            ...
            rTunePrintf(
                    "AUDIO: Predicting SOUND streamer will allocate: Sound: [0x%x] Buffer:[0x%x]\n",
        ...
        for (unsigned int i = 0; i < MUSIC_NUM_STREAM_PLAYERS; i++) {
            ...
            rTunePrintf(
                    "AUDIO: Predicting MUSIC streamer will allocate: Sound: [0x%x] Buffer:[0x%x]\n",
                    streamSoundMemoryNeeded,
                    streamBufferMemoryNeeded);
        ...
        unsigned int totalClipMemoryNeeded =
                (TOTAL_PS2_FREE_UNCOMPRESSED_CLIP_BYTES *
                 gClipFileAudioFormat.m_pEncoding->m_CompressionNumerator) /
        ...
        for (unsigned int i = 0; i < MUSIC_NUM_STREAM_PLAYERS; i++) {
            ...
            sgDesc[i].use_buffered_data_source = CommandLineOptions::Get(CLO_FIREWIRE) ? false
                                                                                       : STREAM_USE_BUFFERED_DATA_SOURCES;
        }
