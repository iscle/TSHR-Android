            ...
            case State_Cueing: {
                switch (m_CueingState) {
                    ...
                    case CueingState_Resource: {
                        ...
                        daSoundFileInstance *pFileInstance = m_pAllocatedResource->GetFileInstance(
                                m_AllocResInstanceID);
                    ...
                    case CueingState_Resource: {
                        ...
                        if (pFileInstance->GetState() == daSoundFileInstance::Loaded) {
                            if (NULL == m_StreamInfo.m_pRsdFileDataSource) {
                                m_pAllocatedResource->GetFileInstance(
                                        m_AllocResInstanceID)->CreateFileDataSource(
                                        &m_StreamInfo.m_pRsdFileDataSource);
                            } else if (IRadSoundRsdFileDataSource::Initialized ==
                                       m_StreamInfo.m_pRsdFileDataSource->GetState()) {
                    ...
                    case CueingState_Player: {
                        switch (state) {
                            ...
                            case IRadSoundStreamPlayer::Paused: {
                                ...
                                if (m_pStateChangeCallback) {
                                    m_pStateChangeCallback->OnSoundReady(
                                            m_pStateChangeCallbackUserData);
                                }
...
//-----------------------------------------------------------------------------
    ...
    void daSoundClipStreamPlayer::Capture(
        ...
        //
        m_pAllocatedResource = daSoundResourceManager::GetInstance()->FindAllocatedResource(
                m_pResource);
        ...
        if (m_CaptureCount > 0) {
            ...
            if (0 == m_CaptureCount) {
                ...
                switch (m_Type) {
                    ...
                    case IDaSoundResource::STREAM: {
                        if (m_StreamInfo.m_pResources != NULL) {
                            m_StreamInfo.m_pResources->m_pStitchedDataSource->SetStitchCallback(
                                    NULL, NULL);
                            ...
                            if (m_StreamInfo.m_pResources->m_pBufferedDataSource != NULL) {
                                m_StreamInfo.m_pResources->m_pBufferedDataSource->SetInputDataSource(
                                        NULL);
                            }
...
//-----------------------------------------------------------------------------
    ...
    unsigned int daSoundClipStreamPlayer::GetPlaybackTimeInSamples(void) {
        ...
        if (IsCaptured()) {
            ...
            switch (m_Type) {
                case IDaSoundResource::CLIP: {
                    pPlayer = static_cast<IRadSoundPlayer *>(m_ClipInfo.m_pClipPlayer
                    );
                    break;
                }
                case IDaSoundResource::STREAM: {
                    pPlayer = static_cast<IRadSoundPlayer *>(m_StreamInfo.m_pResources->m_pStreamPlayer
                    );
                    break;
                }
                default:
...
//-----------------------------------------------------------------------------
    ...
    void daSoundClipStreamPlayer::OnStitch(IRadSoundHalDataSource **ppHds, unsigned int frameCount,
                                           void *pUserData) {
    ...
    void daSoundClipStreamPlayer::HookUpAndCuePlayer(void) {
        ...
        switch (m_Type) {
            case IDaSoundResource::CLIP: {
                ...
                m_ClipInfo.m_pClipPlayer->SetPositionalGroup(
                        m_IsPositional ? m_pPositionalGroup : NULL);
            ...
            case IDaSoundResource::STREAM: {
                ...
                m_StreamInfo.m_pResources->m_pStreamPlayer->SetPositionalGroup(
                        m_IsPositional ? m_pPositionalGroup : NULL);

                m_StreamInfo.m_pResources->m_pStitchedDataSource->SetStitchCallback(this, NULL);
                m_StreamInfo.m_pResources->m_pStitchedDataSource->ResetAudioFormat(
                        m_StreamInfo.m_pRsdFileDataSource->GetFormat()); // GCN ADPCM HACK
                ...
                if (NULL != m_StreamInfo.m_pResources->m_pBufferedDataSource) {
                    ...
                    m_StreamInfo.m_pResources->m_pStreamPlayer->SetDataSource(
                            m_StreamInfo.m_pResources->m_pBufferedDataSource);
                } else {
                    m_StreamInfo.m_pResources->m_pStreamPlayer->SetDataSource(
                            m_StreamInfo.m_pResources->m_pStitchedDataSource);
                }
