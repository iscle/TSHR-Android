...
static unsigned int tutorialTableLength =
        sizeof(tutorialConvNames) / sizeof(TutorialConversationData);
    ...
    if (id == EVENT_DIALOG_SHUTUP) {
        ...
        if (id == EVENT_TUTORIAL_DIALOG_PLAY) {
            ...
            if (GetGameplayManager()->IsSuperSprint()) {
                ...
                dialog = m_dialogList->FindDialogForEvent(id, NULL, NULL,
                                                          tEntity::MakeUID(
                                                                  vehicleEventData->mDriverName), 0,
                                                          convName, false);
                if (dialog == NULL) {
                ...
                dialog = m_dialogList->FindDialogForEvent(id, NULL, NULL,
                                                          tEntity::MakeUID(charName), 0, 0, false);
        ...
        if ((playerAvatar != NULL)
            ...
            if ((avatarVehicle->mpDriver != NULL) && (avatarVehicle->mpDriver != character1)) {
                ...
                dialog = m_dialogList->FindDialogForEvent(id, NULL, NULL, driverUID, charUID2,
                                                          convName, false);
                if (dialog == NULL) {
                    rDebugPrintf("No driver dialog found for event %d\n", static_cast<int>(id));
                } else {
        ...
        dialog = m_dialogList->FindDialogForEvent(id, character1, character2, charUID1, charUID2,
                                                  convName, false);
...
void DialogCoordinator::queueVillainDialog(EventEnum id, void *eventData) {
    ...
    switch (id) {
        ...
        case EVENT_BIG_VEHICLE_CRASH:
            ...
            if ((eventCar != NULL)
                ...
                &&
                (gameplayMgr->GetCurrentMission()->GetCurrentStage()->GetMainAIVehicleForThisStage() !=
                 NULL)) {
                //
                // Hack!
                //
    ...
    if (AICar != NULL) {
        ...
        //
        if (villainName != NULL) {
            dialog = m_dialogList->FindDialogForEvent(villainID, NULL, NULL,
                                                      tEntity::MakeUID(villainName), 0, 0, true);
