...
void SoundFXGameplayLogic::handleCollisionEvent(SoundCollisionData *collisionData) {
    ...
    if (vehicleCrashName != NULL) {
        startCollisionPlayer(vehicleCrashName, collisionData->collObjA, collisionData->collObjB,
                             vectorPtr);
    }
    if (nonVehicleCrashName != NULL) {
        startCollisionPlayer(nonVehicleCrashName, collisionData->collObjA, collisionData->collObjB,
                             vectorPtr);
...
void SoundFXGameplayLogic::startCollisionPlayer(const char *soundName,
    ...
    if (GetGameplayManager()->IsSuperSprint()) {
        ...
        listenerPosition.Set(rsListenerPosition.m_x, rsListenerPosition.m_y,
                             rsListenerPosition.m_z);
