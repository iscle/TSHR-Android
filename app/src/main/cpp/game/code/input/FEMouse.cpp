...
void FEMouse::Move(int mouseX, int mouseY, long screenWidth, long screenHeight) {
    ...
    m_pCursor->SetPos(deltaX / WIN_TO_P3D_POINT_SCALE_FACTOR,
                      deltaY / WIN_TO_P3D_POINT_SCALE_FACTOR);
...
void FEMouse::Update() {
    ...
    if (!pCurrentMenu || (pCurrentMenu && !pCurrentMenu->HasSelectionBeenMade()))//m_bSelectable)
    {
        ...
        //{
        //
        m_hotSpotType = pCurrentWindow->CheckCursorAgainstHotspots(
                m_pCursor->XPos() * WIN_TO_P3D_POINT_SCALE_FACTOR,
...
void FEMouse::ButtonDown(eFEMouseButton buttonType) {
    ...
    if (!(!pCurrentMenu ||
          (pCurrentMenu && !pCurrentMenu->HasSelectionBeenMade())))//!m_bSelectable)
    {
        return;
    }
