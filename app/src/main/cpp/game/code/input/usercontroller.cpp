    ...
    if (!mbInputPointsRegistered && m_xIController2 != NULL) {
        ...
        for (i = 0; i < mNumButtons; i++) {
            ...
            if (pInputPoint != NULL) {
                ...
                pInputPoint->RegisterControllerInputPointCallback(
                        static_cast<IRadControllerInputPointCallback *>(this), i);
...
#else
        IRadControllerOutputPoint *p_OutputPoint = m_xIController2->GetOutputPointByName(
                "LeftMotor");
        if (p_OutputPoint) {
            mRumbleEffect.SetMotor(0, p_OutputPoint);
        }
        ...
        for (i = 0; i < NUM_INPUT_POINTS; i++) {
            ...
            if (pInputPoint != NULL) {
                pInputPoint->UnRegisterControllerInputPointCallback(
                        static_cast<IRadControllerInputPointCallback *>(this));
#ifdef DEBUGWATCH
                char name[64];
                sprintf(name, "%s Button", pInputPoint->GetName());
