    ...
    for (i = 0; i < NUM_DYNA_EFFECTS; ++i) {
        if (mCurrentDynaEffects[i].mRumbleTimeLeft <= milliseconds) {
            if (mCurrentDynaEffects[i].mRumbleTimeLeft > 0) {
                UpdateDynaEffect(static_cast<DynaEffect>(i), mCurrentDynaEffects[i].mRumbleTimeLeft,
                                 mCurrentDynaEffects[i].mMaxGain);
            }
            ...
            UpdateDynaEffect(static_cast<DynaEffect>(i), milliseconds,
                             mCurrentDynaEffects[i].mMaxGain);
        }
    }
...
void RumbleEffect::UpdateEffect(Effect effect, unsigned int milliseconds) {
    if (mCurrentEffects[effect].mRumbleTimeLeft % VALUES[effect].pulseTime <
        32) //We use 32 because we hope to only do this once a frame (or twice)
    {
...
//
//=============================================================================
void RumbleEffect::UpdateDynaEffect(DynaEffect effect, unsigned int milliseconds,
                                    float gain) //This is a pcnt
{
    if (mCurrentDynaEffects[effect].mRumbleTimeLeft % DYNA_VALUES[effect].pulseTime <
        32) //We use 32 because we hope to only do this once a frame (or twice)
    {
