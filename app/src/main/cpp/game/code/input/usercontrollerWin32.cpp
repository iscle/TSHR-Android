...
int
UserController::DeriveDirectionValues(eControllerType type, int dxKey, float value, float *values) {
    ...
    if (m_pController[type]->IsInputAxis(dxKey)) {
        ...
        ((Gamepad *) m_pController[GAMEPAD])->CalculatePOV(value, &values[DIR_UP],
                                                           &values[DIR_DOWN],
...
void UserController::ApplyDynaEffect(RumbleEffect::DynaEffect effect, unsigned int durationms,
                                     float gain) {
    if (mbIsRumbleOn && !CommandLineOptions::Get(CLO_NO_HAPTIC)) {
...
#ifdef CONTROLLER_DEBUG
    if (gRandomButtoners[m_controllerId].mRandomButtonEnable ||
        CommandLineOptions::Get(CLO_RANDOM_BUTTONS)) {
        ...
        if (rb.mButtonTimeout == 0) {
            ...
            if (rb.mLastButton[rb.mCurrentButton] != -1) {
                ...
                for (unsigned j = 0; j < Input::MaxMappables; j++) {
                    if (mMappable[j]) {
                        mMappable[j]->DispatchOnButton(m_controllerId,
                                                       rb.mLastButton[rb.mCurrentButton],
                                                       &mButtonArray[rb.mLastButton[rb.mCurrentButton]]);
                    }
                }
            }
...
void UserController::SetMap(int map, int virtualKey, eControllerType cont, int dxKey,
                            eDirectionType dir) {
...
bool UserController::GetMap(int map, int virtualKey, eControllerType &type, int &dxKey,
                            eDirectionType &dir) const {
    // Check over the inputs.
    rAssert(map >= 0 && map < Input::MaxVirtualMappings);
    rAssert(virtualKey >= 0 && virtualKey < mNumButtons);
...
const char *UserController::GetMap(int map, int virtualKey, int &numDirs, eControllerType &cont,
                                   eDirectionType &dir) const {
    int dxKey;
    ...
    if (cont == KEYBOARD &&
        ...
        for (int dir = 0; dir < ndirections; dir++) {
            if (dvalues[dir] >= MAPPING_DEADZONE) {
                ...
                mMapData.MapNext = false;
                mMapData.callback->OnButtonMapped(m_pController[cont]->GetInputName(dxKey), cont,
                                                  ndirections, (eDirectionType) dir);
