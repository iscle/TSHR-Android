...
*/
    for (i = 0; i < mTriggerCount; i++) {
        ...
        if (CommandLineOptions::Get(CLO_SHOW_DYNA_ZONES) &&
            locator->GetDataType() == LocatorType::DYNAMIC_ZONE) {
            mTriggerVolumes[i]->Render();
        } else if (!locator) {
...
//
//========================================================================
void TriggerVolumeTracker::ActivateNearestInteriorLoadZone(int iPlayerID, rmt::Vector &irPosnRef,
                                                           float iMaxDistance) {
        ...
        if (loc->GetDataType() == LocatorType::DYNAMIC_ZONE) {
            ...
            if (pZEL->IsInteriorLoad()) {
                ...
                if ((curDistSqr < nearestLocatorDistSqr)
                    && (curDistSqr <
                        ((iMaxDistance + sphere.radius) * (iMaxDistance + sphere.radius)))) {
                    pNearestZEL = pZEL;
    ...
    if (playerID >= MAX_PLAYERS) {
        ...
        if (vehicle) {
            ...
            */
            ...
            sim::CollisionVolume *vecol = vehicle->GetSimState()->GetCollisionObject()->GetCollisionVolume()->GetSubCollisionVolume(
                    0);
            sColType = vecol->Type();
...
//END_PROFILE("Added Triggers");
    ...
    if (playerID < MAX_PLAYERS) {
        ...
        for (i = 0; i < mActiveCount[playerID]; ++i) {
            ...
            // TBJ [8/23/2002] 
            //
            if ((mActiveVolumes[playerID][i]->GetLocator() &&
                 !mActiveVolumes[playerID][i]->GetLocator()->GetFlag(Locator::ACTIVE)) ||
...
void TriggerVolumeTracker::RemoveActive(int playerID, int index) {
    ...
    if (trigLoc && trigLoc->IsPlayerTracked(playerID) <=
                   1) //Some locators are screwy so they never track trigger voluemes.  Gags, mostly
    {
...
void TriggerVolumeTracker::TestVolume(TriggerVolume *vol,
    ...
    if (vol->GetLocator() != NULL) {
        if (playerID >= MAX_PLAYERS &&
            (((1 << vol->GetLocator()->GetDataType()) &
              mRegisteredVehicles[playerID - MAX_PLAYERS].mTriggerTypes) == 0)) {
            return;
    ...
    if (playerID < MAX_PLAYERS &&
        ...
        if (GetGameplayManager()->GetGameType() == GameplayManager::GT_SUPERSPRINT) {
            collision =
                    (!loc || loc->GetFlag(Locator::ACTIVE)) && vol->IntersectsSphere(center, 1.5f);
        } else {
            collision = (!loc || loc->GetFlag(Locator::ACTIVE)) &&
                        (vol->IntersectsSphere(center, 1.5f) ||
                         vol->IntersectsBox(sP1, sP2, sP4, sP3));
        }
    } else {
        collision = (!loc || loc->GetFlag(Locator::ACTIVE)) &&
