...
inline bool AnyStickMovement(Character *character, float tol = 0.0f) {
    ...
    return (rmt::Abs(character->GetController()->GetValue(CharacterController::LeftStickX)) >
            tol) ||
           (rmt::Abs(character->GetController()->GetValue(CharacterController::LeftStickY)) >
            tol) ||
           #ifdef RAD_WIN32
           (
...
void WalkerLocomotionAction::SwitchLocomotion(void) {
    tRefCounted::Assign(mpLocomotion,
                        choreo::find<choreo::Locomotion>(mpCharacter->GetPuppet()->GetBank(),
                                                         tEntity::MakeUID("walkerLoco")));
...
float WalkerLocomotionAction::SolveActualDir(float fDesiredDir, float timeins) {
    ...
    float newDir = (mpCharacter->GetFacingDir() * oldRatio) +
                   ((mpCharacter->GetFacingDir() + rotateDelta) * newRatio);
...
void WalkerLocomotionAction::Update(float timeins) {
    ...
    if (c && mpCharacter->mbSurfing &&
        ...
        mpCharacter->GetActionController()->SequenceSingleAction(
                new PlayAnimationAction(mpCharacter, "surf_in"));
        mpCharacter->GetActionController()->SequenceSingleAction(new SurfAction(mpCharacter));
        mpCharacter->GetActionController()->SequenceSingleAction(
                new PlayAnimationAction(mpCharacter, "surf_out"));
    }
}
...
void InCarAction::IWannaRock(bool whattaYouWannaDoWithYourLife) {
    ...
    if (rockinIsMyBusiness) {
        anim = choreo::find<choreo::Animation>(pPuppet->GetBank(),
                                               mIsDriver ? "in_car_victory_small_driver"
                                                         : "in_car_victory_small");
    } else {
        anim = choreo::find<choreo::Animation>(pPuppet->GetBank(),
                                               mIsDriver ? "in_car_idle_driver" : "in_car_idle");
    }
...
bool SteerAction::ShouldRelease(void) {
    ...
    float steer = GetCharacterManager()->GetCharacter(0)->GetController()->GetValue(
            CharacterController::LeftStickX);
...
=============================================================================
*/
PlayAnimationAction::PlayAnimationAction(Character *pCharacter, const char *anim,
...
=============================================================================
*/
PlayIdleAnimationAction::PlayIdleAnimationAction(Character *pCharacter, const char *anim,
                                                 float fFrameRate /* = 30.0f  */)
        :
        PlayAnimationAction(pCharacter, anim, fFrameRate) {
}

PlayIdleAnimationAction::PlayIdleAnimationAction(Character *pCharacter, const tName &anim,
                                                 float fFrameRate)
        :
        PlayAnimationAction(pCharacter, anim, fFrameRate) {
}
...
CarDoorAction::CarDoorAction(Vehicle *pVehicle, Vehicle::DoorAction action, Vehicle::Door door,
                             float delay, float time, Character *c, Sequencer *s)
        :
        mpVehicle(pVehicle),
        mAction(action),
        mDoor(door),
...
void JumpAction::Update(float timeins) {
    ...
    switch (mJumpState) {
        case InitJump: {
            tRefCounted::Assign(mpAnimationDriver,
                                mpCharacter->GetPuppet()->PlayAnimation("jump_idle_launch"));
...
//animation "jump_kick"
                ...
                if (mbJumpAgain) {
                    tRefCounted::Assign(mpAnimationDriver, mpCharacter->GetPuppet()->PlayAnimation(
                            "jump_dash_in_air"));
                } else {
                    tRefCounted::Assign(mpAnimationDriver,
                                        mpCharacter->GetPuppet()->PlayAnimation("jump_idle"));
                }
                // SetIsDriverIndefinite(true) as we will manually control the stop
                // of the animation.  Length can change based on terrain/collision objects.
                //
        ...
        case Jump: {
            ...
            if (mpCharacter->IsStanding()) {
                ...
                if (mbSlam) {
                    ...
                    mVelocity.Clear();
                    tRefCounted::Assign(mpAnimationDriver,
                                        pPuppet->PlayAnimation("jump_idle_land"));
                    // SetIsDriverIndefinite(true) as we will manually control the stop
                    // of the animation.  Length can change based on terrain/collision objects.
                    //
                    pPuppet->SetIsDriverIndefinite(mpAnimationDriver, false);
                ...
                if (mpAnimationDriver) {
                    if (mbInJumpKick) {
                        ...
                        float t1 = rmt::Fabs(.5000000000f / g *
                                             (-2.f * v - 2.f * rmt::Sqrt((v * v) + 2.f * g * d)));
...
=============================================================================
*/
void JumpAction::TestForJumpAgain(const rmt::Vector &currentPos, float fUpVelocity,
    ...
    if (theIntention == CharacterController::Jump && !mbJumpAgain) {
        ...
        if (mbJumpAgain) {
            ...
            tRefCounted::Assign(mpAnimationDriver,
                                mpCharacter->GetPuppet()->PlayAnimation("stomp_antic"));
//            mpAnimationDriver = mpCharacter->GetPuppet()->PlayAnimation("stomp_in_air");
            ...
            tRefCounted::Assign(mpAnimationDriver,
                                mpCharacter->GetPuppet()->PlayAnimation("jump_kick"));
...
DodgeAction::DodgeAction(Character *pCharacter)
        :
        mpAnimationDriver(
                NULL), // Set when we start playing the dodge animation and need to store its handle
        mpCharacter(pCharacter) {
...
CringeAction::CringeAction(Character *pCharacter)
        :
        mpAnimationDriver(
                NULL), // Set when we start playing the animation and need to store its handle
        mpCharacter(pCharacter) {
    rAssert(mpCharacter != NULL);
}
