...
public:
    ...
    bool TestIntersectionStatics(const rmt::Vector &start, const rmt::Vector &end,
                                 rmt::Vector *intersection);

    bool TestIntersectionDynamics(const rmt::Vector &start, const rmt::Vector &end,
                                  rmt::Vector *intersection, DynaPhysDSG **objectHit = NULL);

    bool TestIntersectionAnimPhys(const rmt::Vector &start, const rmt::Vector &end,
                                  rmt::Vector *intersection);

    // Performs intersection against both dynamics and statics
    bool
    TestIntersection(const rmt::Vector &start, const rmt::Vector &end, rmt::Vector *intersection,
                     float rayWidthForFences = 0.0f);

    // Perform a line of sight test against statics and dynamics. Faster than TestIntersection
    bool LineOfSight(const rmt::Vector &start, const rmt::Vector &end, float rayWidth = 1.0f,
    ...
    int FillIntersectionListDynamics(const rmt::Vector &position, float radius, bool addDynaPhys,
                                     DynaPhysDSG *source = NULL);
...
protected:
    ...
    std::vector<sim::CollisionObject *, s2alloc < sim::CollisionObject * >>
    m_StaticCollisionList;
