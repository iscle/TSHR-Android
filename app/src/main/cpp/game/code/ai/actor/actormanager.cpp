...
void
ActorManager::FireProjectile(tUID typeName, const rmt::Vector &position,
                             const rmt::Vector &direction, unsigned int ownerID) {
...
ActorManager::SetupConsoleFunctions() {
    GetConsole()->AddFunction("AddFlyingActor", AddFlyingActor, "", 5,
                              5); //Adds a new (flying actor) into the world
    GetConsole()->AddFunction("AddFlyingActorByLocator", AddFlyingActorByLocator, "", 3,
                              4); //Adds a new (flying actor) into the world
    GetConsole()->AddFunction("AddBehaviour", AddBehaviour, "", 2,
                              7); //adds a new behaviour pattern to an actor
    GetConsole()->AddFunction("SetCollisionAttributes", SetCollisionAttributes, "", 4,
                              4); //Sets friction/mass/elasticity values
    GetConsole()->AddFunction("AddSpawnPoint", AddSpawnPointScript, "", 8,
                              8); //Creates a new spawnpoint
    GetConsole()->AddFunction("AddSpawnPointByLocatorScript", AddSpawnPointByLocatorScript, "", 6,
                              6); //Creates a new spawnpoint
    GetConsole()->AddFunction("SetProjectileStats", SetProjectileStats, "", 3,
                              3); //Sets stateprop name, speed and number of instances to allocate
    GetConsole()->AddFunction("PreallocateActors", PreallocateActors, "", 2,
                              2); //Creates N stateprops for later use
    GetConsole()->AddFunction("SetActorRotationSpeed", SetActorRotationSpeed, "", 2,
                              2);    //Creates sets the rotation speed for all actors of the given type. Rotation speed is in degrees/second
    GetConsole()->AddFunction("AddShield", AddShield, "", 2,
                              2); //Creates a shield around the stateprop. First parameter = recharge time in seconds after being broken.
}
...
ActorManager::AddFlyingActor(int argc, char **argv) {
    ...
    Actor *actor = GetActorManager()->CreateActor(tName::MakeUID(statepropName),
                                                  tName::MakeUID(instanceName), transform);
    HeapMgr()->PopHeap(GMA_LEVEL_OTHER);
}
...
ActorManager::SetActorRotationSpeed(int argc, char **argv) {
    ...
    std::vector < Actor * ,
            s2alloc < Actor * >> actorList = GetActorManager()->GetActorsByType(typeName);
...
ActorManager::AddShield(int argc, char **argv) {
    ...
    std::vector < Actor * ,
            s2alloc < Actor * >> actorList = GetActorManager()->GetActorsByType(name);
    for (unsigned int i = 0; i < actorList.size(); i++) {
...
ActorManager::AddBehaviour(int argc, char **argv) {
    ...
    std::vector < Actor * ,
            s2alloc < Actor * >> actorList = GetActorManager()->GetActorsByType(targetObject);
    ...
    for (int i = 0; i < numBehavioursToAllocate; i++) {
        ...
        if (strcmp(behaviourName, "ATTACK_PLAYER") == 0) {
            ...
            AttackBehaviour *attackBehaviour = new AttackBehaviour(maxFiringRange,
                                                                   forwardFiringArc);
            ...
            EvasionBehaviour *evasionBehaviour = new EvasionBehaviour(minEvadeDistHoriz,
                                                                      maxEvadeDistHoriz,
                                                                      minEvadeDistVert,
                                                                      maxEvadeDistVert, speed);
