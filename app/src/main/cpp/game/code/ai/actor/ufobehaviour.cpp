...
//    rotationMatrix.FillRotation(rotationAxis, timeInMS * 0.001f);
    ...
    switch (aiRef) {
        case PhysicsAIRef::redBrickVehicle: {
            ...
            if (vehicle->mVehicleID != VehicleEnum::CBLBART &&
                ...
                if (scc->GetActiveSuperCam()->GetType() == SuperCam::BUMPER_CAM) {
                    ...
                    scc->SelectSuperCam(SuperCam::FAR_FOLLOW_CAM,
                                        SuperCamCentral::FORCE | SuperCamCentral::CUT, 0);
                }
...
//-            GetEventManager()->TriggerEvent((EventEnum)(EVENT_LOCATOR + LocatorEvent::DEATH) , NULL);
        ...
        case PhysicsAIRef::redBrickVehicle: {
            ...
            if (vehicle->IsUserDrivingCar() && vehicle->GetDriver() != NULL &&
                !GetAvatarManager()->IsAvatarGettingInOrOutOfCar(0)) {
...
void
UFOBehaviour::GetTargetsInTractorRange(rmt::Vector &actorPosition, float maxRange,
                                       SwapArray<DynaPhysDSG *> *targetList) {
    targetList->ClearUse();
