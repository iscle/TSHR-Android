...
/*
    ...
    */
    ...
    if (m_ActorAnimation) {
        if (m_ActorAnimation->Update(currentTransform, &newTransform,
                                     static_cast<float>(timeInMS))) {
    ...
    mp_StateProp->RecomputeShadowPositionNoIntersect(m_HighestIntersectHeight,
                                                     m_HighestIntersectNormal, 0.05f, 3.0f);
}
...
void FlyingActor::FindWaypoint(const rmt::Vector &start, const rmt::Vector &end, int depth) {
    ...
    if (hitObject == false) {
        ...
        for (int i = 0; i < NUM_MOVEMENT_RETRIES; i++) {
            ...
            if (foundPlane) {
                intermediary.y = groundPlaneIntersectionPoint.y + FLYING_HEIGHT +
                                 s_Randomizer.Float() * 2.0f;
            }
...
FlyingActor::UpdateMovement(unsigned int timeInMS, rmt::Vector *out_newPosition) {
    ...
    // Sanity check
    {
        bool cleanMove = m_IntersectionList.LineOfSight(currPosition, m_PreviousPosition,
                                                        RAYWIDTH_FOR_PATH_FINDING);
        if (cleanMove == false) {
    ...
    if (m_IsMoving) {
        ...
        if (distToTravel > (distToWaypoint - CLOSE_ENOUGH_TO_WAYPOINT_DIST)) {
            ...
            *out_newPosition =
                    currPosition + toWaypoint * m_Speed * static_cast<float>(timeInMS);
        }
    ...
    if (callback == STATEPROP_CHANGE_STATE_EVENT) {
        ...
        switch (newState) {
            ...
            case ActorEnum::eDestroyed:
                GetCharacterSheetManager()->IncNumWaspsDestroyed(
