...
bool
IntersectionList::TestIntersectionStatics(const rmt::Vector &sourceOfRay,
                                          const rmt::Vector &endOfRay, rmt::Vector *intersection) {
...
bool
IntersectionList::TestIntersection(const rmt::Vector &sourceOfRay, const rmt::Vector &endOfRay,
                                   rmt::Vector *intersection, float rayWidthForFences) {
    rAssert(m_StaticCollisionList.capacity() == SIZE_INTERSECTION_LIST_ARRAYS);
    rAssert(m_DynamicCollisionList.capacity() == SIZE_INTERSECTION_LIST_ARRAYS);
...
bool
IntersectionList::LineOfSight(const rmt::Vector &sourceOfRay, const rmt::Vector &endOfRay,
                              float rayWidth, bool staticsOnly) {
    rmt::Vector sray;
    sray.Sub(endOfRay, sourceOfRay);
    rmt::Vector ray = sray;
    float rayLen = ray.NormalizeSafe();
...
IntersectionList::FillIntersectionListStatics(const rmt::Vector &position, float radius) {
    ...
    GetIntersectManager()->FindStaticPhysElems(const_cast<rmt::Vector &>(position), radius,
                                               spList);
...
int IntersectionList::FillIntersectionListDynamics(const rmt::Vector &position, float radius,
                                                   bool addDynaPhys, DynaPhysDSG *source) {
    ClearDynamics();

    ReserveArray < DynaPhysDSG * > dpList(200);
    // Perhaps intersectmanager should be changed to accept a const rmt::Vector& instead?
    GetIntersectManager()->FindDynaPhysElems(const_cast<rmt::Vector &>(position), radius,
                                             dpList);
...
IntersectionList::FillIntersectionListAnimPhys(const rmt::Vector &position, float radius) {
    ...
    GetIntersectManager()->FindAnimPhysElems(const_cast<rmt::Vector &>(position), radius,
                                             spList);
    for (int i = 0; i < spList.mUseSize; i++) {
...
bool
IntersectionList::LineSegmentIntersection(const LineSegment2D &line1, const LineSegment2D &line2,
                                          float *t) {
