...
bool Projectile::Init(const char *statePropName, const char *instanceName) {
    ...
    if (pStatePropData) {
        ...
        CollisionAttributes *pCollAttr = GetATCManager()->CreateCollisionAttributes(PROP_MOVEABLE,
                                                                                    1, 2.2425f);
...
Projectile::Update(unsigned int timeInMS) {
    ...
    if (currState == 0) {
        ...
        if (m_IntersectionList.TestIntersectionDynamics(m_PreviousPosition, m_CurrentPosition,
                                                        &dynamicsIntersectionPoint, &objectHit)) {
            if (IsJumpingPlayer(objectHit) == false) {
                // we hit something, make it explode at the collisionpoint
                m_Speed = 0;
                rmt::Vector adjustedIntersection;
                AdjustCollisionPosition(dynamicsIntersectionPoint, m_CurrentPosition,
                                        m_PreviousPosition, &adjustedIntersection);
        ...
        if (m_WillHitStatic) {
            ...
            if (toCurr.Dot(toPrev) < 0) {
                ...
                rmt::Vector adjustedIntersection;
                AdjustCollisionPosition(m_StaticIntersectionPoint, m_CurrentPosition,
                                        m_PreviousPosition, &adjustedIntersection);
                SetPosition(adjustedIntersection);
                mp_StateProp->SetState(1);
            }
...
void Projectile::CalculateIntersections() {
    ...
    m_WillHitStatic = m_IntersectionList.TestIntersectionStatics(position, destination,
                                                                 &m_StaticIntersectionPoint);
    ...
    switch (object->GetAIRef()) {
        ...
        case PhysicsAIRef::StateProp: {
            ...
            const char *ignore_list[] =
                    {
                            "l1_crate", "l2_crate", "l3_crate", "l4_crate", "l5_crate", "l6_crate",
                            "l7_crate",
                            "l1_vending", "l2_vending", "l3_vending", "l4_vending", "l5_vending",
                            "l6_vending", "l7_vending"
                    };
