...
static CarPurchaseDialogueStuff s_carPurchaseConvNames[] =
        {
                ...
                {0,                           tEntity::MakeUID(
                        "zombie")}   // No conversation for zombies
        };
...
static const AnimSwitchSoundData ANIM_SWITCH_SOUND_TABLE[] =
        {
                ...
                {::radMakeKey32(
                        "Splatfrom7Trans"),         "platform_01", "platform_settings",      true},  // (sic)
...
*/
    bool AnimSwitch::Create(tEntityStore *inStore) {
        AnimCollisionEntityDSG *pAnimCollisionEntityDSG = p3d::find<AnimCollisionEntityDSG>(inStore,
                                                                                            mpActionEventLocator->GetObjName());
        bool bCreated = false;
        if (pAnimCollisionEntityDSG) {
            ...
            tPose::Joint *pJoint = pAnimCollisionEntityDSG->GetPoseJoint(jointIndex,
                                                                         mpActionEventLocator->GetShouldTransform());

            ActionButton::AnimCollisionEntityDSGWrapper *pGameObject = new(
                    GMA_LEVEL_OTHER) ActionButton::AnimCollisionEntityDSGWrapper;
...
*/
    void
    AnimSwitch::Init(IGameObjectWrapper *pGameObject, tPose::Joint *pJoint, bool bAttachToJoint) {
        ...
        if (mSoundName != NULL) {
            if (fDirection == 0.0f) {
                ...
                if (mIsMovingSound) {
                    ...
                    //
                    rAssert(dynamic_cast<tPoseAnimationController *>(pAnimController->GetTrack(
                            0)));
                    controller = static_cast<tPoseAnimationController *>(pAnimController->GetTrack(
                            0));
...
*/
    SummonVehiclePhone::SummonVehiclePhone(ActionEventLocator *pActionEventLocator)
        ...
        if (currentHud != NULL) {
            mHudMapIconID = currentHud->GetHudMap(0)->RegisterIcon(HudMapIcon::ICON_PHONE_BOOTH,
                                                                   pos);
        }
...
*/
    bool SummonVehiclePhone::OnButtonPressed(Character *pCharacter, Sequencer *pSeq) {
        ...
        if (sbEnabled && GetGameplayManager()->QueryPhoneBoothsEnabled()) {
            ...
            car_name[0] = '\0';
            strcpy(car_name,
                   GetGameplayManager()->GetVehicleSlotVehicleName(GameplayManager::eDefaultCar));
            ...
            if (GetGameplayManager()->GetVehicleInSlot(GameplayManager::eDefaultCar) != NULL) {
                //update
                float currentCarHitpoints = GetGameplayManager()->GetVehicleInSlot(
                        GameplayManager::eDefaultCar)->mHitPoints;
                GetCharacterSheetManager()->UpdateCarHealth(CarIndex,
                                                            GetGameplayManager()->GetVehicleInSlot(
                                                                    GameplayManager::eDefaultCar)->GetVehicleLifePercentage(
                                                                    currentCarHitpoints));
            } else {
            ...
            SuperCam *sc = GetSuperCamManager()->GetSCC(0)->GetSuperCam(
...
=============================================================================
*/
    void SummonVehiclePhone::LoadVehicle(const char *name, const char *filename,
                                         VehicleCentral::DriverInit driver) {
        //HACK: this a perpetuation of Trav's original Billboardwrappedloader hack.
        //dyna loading duriong a car load will now be BAD. -dm
        ((BillboardWrappedLoader *) GetAllWrappers()->mpLoader(
                AllWrappers::msBillboard))->OverrideLoader(true);
        ...
        GetLoadingManager()->AddRequest(FILEHANDLER_PURE3D, filename, GMA_LEVEL_MISSION, filename,
                                        NULL, &sCallback);
    }
...
//    sSelectedVehicle = selectedVehicleIndex;
        ...
        rAssert(pInfo);
        SummonVehiclePhone::LoadVehicle(pInfo->GetVehicleName(), pInfo->GetFileName(),
...
*/
    void SummonVehiclePhoneStaticCallback::OnProcessRequestsComplete(void *pUserData) {
        ((BillboardWrappedLoader *) GetAllWrappers()->mpLoader(
                AllWrappers::msBillboard))->OverrideLoader(false);
...
//    rAssert(pInfo);
        ...
        if (GetCharacterSheetManager()->GetCarIndex(SummonVehiclePhone::GetSelectedVehicleName()) ==
            -1) {
            bool bplayercar = false;
        }


        Vehicle *pVehicle = GetVehicleCentral()->InitVehicle(
                const_cast<char *>(SummonVehiclePhone::GetSelectedVehicleName()), true, NULL,
                VT_USER, mDriver, bplayercar);
        rAssert(pVehicle);
        if (pVehicle) {
            if (mpActionEventLocator) {
                CarStartLocator *pLocator = p3d::find<CarStartLocator>(
                        mpActionEventLocator->GetObjName());
                if (pLocator) {
...
*/
                    ...
                    if (GetGameplayManager()->TestForContinuityErrorWithCar(pVehicle, true)) {
                        Vehicle *errorCar = GetGameplayManager()->GetVehicleInSlot(
                                GameplayManager::eAICar);
                        if (errorCar) {
                            //This is kinda hacky.
                    ...
                    rAssertMsg(0,
                               "what now! - vehicle init'd and added to active list but not addref'd - see greg \n");
                }
                tRefCounted::Release(mpActionEventLocator);
            } else {
                rAssertMsg(0,
    ...
    bool CollectibleCard::Create(tEntityStore *inStore) {
        ...
        if (!found &&
            ...
            if (mAnimatedCollectionThing == NULL) {
                ...
                mAnimatedCollectionThing->Init("card_collect", rmt::Vector(0.0f, 0.0f, 0.0f), false,
                                               true);
            }
...
*/
    void CollectibleCard::OnEnter(Character *pCharacter) {
        if (!IsCollected()) {
            ...
            //
            Card *collectedCard = GetCardGallery()->AddCollectedCardByName(
                    tEntity::MakeUID(mpActionEventLocator->GetObjName()));
            if (collectedCard != NULL) {
                // Update collected cards in character sheet
                //
                GetCharacterSheetManager()->AddCard(
                        static_cast<RenderEnums::LevelEnum>(collectedCard->GetLevel() - 1),
            ...
            // Maybe play a sound effect.
            //
            GetEventManager()->TriggerEvent(EVENT_CARD_COLLECTED,
                                            reinterpret_cast<void *>(collectedCard));

            //Rumble the controller.
            int playerID = mpActionEventLocator->GetPlayerID();
            int controllerID = GetInputManager()->GetControllerIDforPlayer(playerID);
            if (controllerID != -1) {
                GetInputManager()->GetController(controllerID)->ApplyEffect(RumbleEffect::MEDIUM,
                                                                            250);
                GetInputManager()->GetController(controllerID)->ApplyEffect(RumbleEffect::HARD2,
                                                                            250);
            }
        }
        Collectible::OnEnter(pCharacter);
...
//=============================================================================
    bool WrenchIcon::Create(tEntityStore *inStore) {
        ...
        for (i = 0; i < ActionButtonManager::MAX_ACTIONS; ++i) {
            if (GetActionButtonManager()->GetActionByIndex(i)) {
                ...
                if (bh && bh->GetType() == ActionButton::ButtonHandler::WRENCH_ICON) {
                    ...
                    rAssert(pWrenchIcon);
                    if (tEntity::MakeUID(pWrenchIcon->GetActionEventLocator()->GetObjName()) ==
                        nameUID) {
                        found = true;
                        break;
                    }
        ...
        if (mAnimatedCollectionThing == NULL) {
            mAnimatedCollectionThing = new(GMA_LEVEL_OTHER) AnimatedIcon();
            mAnimatedCollectionThing->Init("wrench_collect", rmt::Vector(0.0f, 0.0f, 0.0f), false,
                                           true);
        }
        ...
        for (i = 0; i < ActionButtonManager::MAX_ACTIONS; ++i) {
            if (GetActionButtonManager()->GetActionByIndex(i)) {
                ...
                if (bh && bh->GetType() == ActionButton::ButtonHandler::NITRO_ICON) {
                    ...
                    rAssert(pNitroIcon);
                    if (tEntity::MakeUID(pNitroIcon->GetActionEventLocator()->GetObjName()) ==
                        nameUID) {
                        found = true;
                        break;
                    }
...
//
//=============================================================================
    GenericEventButtonHandler::GenericEventButtonHandler(EventLocator *pEventLocator,
                                                         EventEnum event) :
            mpEventLocator(NULL),
            mEvent(event),
            mEventData(NULL) {
...
//=============================================================================
    ButtonHandler *
    GenericEventButtonHandler::NewAction(EventLocator *pEventLocator, EventEnum event,
                                         GameMemoryAllocator alloc) {
...
//=============================================================================
    void TeleportAction::HandleEvent(EventEnum id, void *pEventData) {
        if (id == EVENT_GUI_IRIS_WIPE_CLOSED) {
            ...
            GetAvatarManager()->GetAvatarForPlayer(0)->GetCharacter()->RelocateAndReset(pos, facing,
                                                                                        true);
#ifdef RAD_WIN32
            GetSuperCamManager()->GetSCC(0)->SelectSuperCam(SuperCam::ON_FOOT_CAM, SuperCamCentral::CUT, 0);
#else
            GetSuperCamManager()->GetSCC(0)->SelectSuperCam(SuperCam::WALKER_CAM,
                                                            SuperCamCentral::CUT, 0);
#endif
...
//=============================================================================
    void PurchaseReward::OnUpdate(float timeins) {
        ...
        if (mIsActive && (mAllPurchased || !sbEnabled || !GetGameplayManager()->IsSundayDrive())) {
        ...
        } else if (!mIsActive && !mAllPurchased && sbEnabled &&
            ...
            if (mHudMapIconID == -1) {
                CGuiScreenHud *currentHud = GetCurrentHud();
                if (currentHud != NULL) {
                    mHudMapIconID = currentHud->GetHudMap(0)->RegisterIcon(
                            HudMapIcon::ICON_PURCHASE_CENTRE,
...
//=============================================================================
    bool PurchaseCar::OnButtonPressed(Character *pCharacter) {
        ...
        if (mpActionEventLocator->GetUID() == GIL) {
            ...
            //
            GetEventManager()->TriggerEvent(EVENT_HAGGLING_WITH_GIL,
                                            GetCharacterManager()->GetCharacterByName(
                                                    "reward_gil"));
        } else {
        ...
        // The menu will trigger the loading.
        //
        tRefCounted::Assign(SummonVehiclePhone::sCallback.mpActionEventLocator,
                            mpActionEventLocator);
...
//=============================================================================
    void PurchaseCar::OnUpdate(float timeins) {
        ...
        if (mIsActive) {
            ...
            Character *character = GetCharacterManager()->GetCharacterByName(
                    tEntity::MakeUID(charName));
            rAssert(character);
...
//=============================================================================
    bool PurchaseSkin::OnButtonPressed(Character *pCharacter) {
        ...
        GetGuiSystem()->GotoScreen(CGuiWindow::GUI_SCREEN_ID_PURCHASE_REWARDS,
                                   Merchandise::SELLER_INTERIOR);
