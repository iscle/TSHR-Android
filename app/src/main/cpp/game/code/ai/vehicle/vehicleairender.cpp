...
VehicleAIRender::VehicleAIRender() {
    ...
    radDbgWatchAddFunction("Drop Path Point", &DropPathPoint, NULL,
                           "Vehicle AI Debug\\Path Finding");
}
...
int VehicleAIRender::RegisterAI(void *ai, int type) {
    ...
    if (ra->type == TYPE_TRAFFIC_AI) {
        ...
        sprintf(name, "Vehicle AI Debug\\%s (%d)",
                ((VehicleAI * )(ra->ai))->GetVehicle()->GetNameDangerous(), rendercounter);
    } else {
        rAssert(false);
    }
...
void VehicleAIRender::Display() {
            ...
            */
            ...
            if (lastElem.elem) {
                if (lastElem.type == RoadManager::ET_INTERSECTION) {
                    ...
                    stream = p3d::pddi->BeginPrims(mAIShader->GetShader(), PDDI_PRIM_LINESTRIP,
                                                   PDDI_V_C, 4);
                    if (stream) {
            ...
            char state[64];
            char shortcut[128];
            sprintf(speed, "%.1f (%.1f) km/h", ai->GetVehicle()->GetSpeedKmh(),
                    ai->GetDesiredSpeedKmh());
            sprintf(params, "g : %.1f  b : %.1f  e : %.1f", ai->GetGas(), ai->GetBrake(),
                    ai->GetHandBrake());
            sprintf(state, "state : %d", ai->GetState());
            sprintf(shortcut, "SC MIN : %d  MAX : %d  MOD : %d",
                    ai->GetMinShortcutSkill(), ai->GetMaxShortcutSkill(),
                    ai->GetShortcutSkillMod());
            ...
            p3d::pddi->DrawString(shortcut, (int) pos2.x, (int) pos2.y + 45,
                                  tColour(255, 255, 255));
    ...
    if (sNumPathPoints) {
        for (unsigned i = 0; i < sNumPathPoints - 1; i++) {
            ...
            P3DDrawCylinder(0.3f, sPathPoints[i], sPathPoints[i + 1], *mAIShader,
                            tColour(255, 0, 255));
        }
...
void VehicleAIRender::DropPathPoint(void *) {
    ...
    */
    ...
    if (sWhichPathEndpoint == 0) {
        ...
        bool ok = true;
        ok &= VehicleAI::FindClosestPathElement(sPathEndPoints[0], sourceElem, sourceSeg, sourceT,
                                                sourceRoadT, true);
        ok &= VehicleAI::FindClosestPathElement(sPathEndPoints[1], targetElem, targetSeg, targetT,
                                                targetRoadT, true);
