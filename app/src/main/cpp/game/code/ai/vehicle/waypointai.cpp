            ...
            if (roughDistToPlayerSqr <= rmt::Sqr(DIST_KEEP_UP_WITH_PLAYER)) {
                ...
                if (miNumLapsCompleted > playerLapsCompleted) {
                    ...
                    if (miCurrentCollectible > playerCurrCheckpoint) {
                        ...
                        float distDiff = rmt::Fabs(
                                playerDistToCheckpoint - mDistToCurrentCollectible);
...
int WaypointAI::RegisterHudMapIcon() {
    ...
    if (currentHud != NULL) {
        switch (mWaypointAIType) {
            case RACE: {
                iconID = currentHud->GetHudMap(0)->RegisterIcon(HudMapIcon::ICON_AI_RACE,
                                                                initialLoc, this);

                break;
            }
            case EVADE: {
                iconID = currentHud->GetHudMap(0)->RegisterIcon(HudMapIcon::ICON_AI_EVADE,
                                                                initialLoc, this);

                break;
            }
            case TARGET: {
                iconID = currentHud->GetHudMap(0)->RegisterIcon(HudMapIcon::ICON_AI_TARGET,
                                                                initialLoc, this);
