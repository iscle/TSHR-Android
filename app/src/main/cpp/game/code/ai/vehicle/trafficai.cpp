            ...
            switch (foundSOMETHING) {
                ...
                case OT_PLAYERVEHICLE: {
                    ...
                    */
                    ...
                    if (mLastThingThatImpededMe != SOMETHING) {
                        ...
                        {
                            Vehicle *playerVehicle = GetAvatarManager()->GetAvatarForPlayer(
                                    0)->GetVehicle();
                            ...
                            if (playerVehicle->mSpeed > PLAYER_MIN_SPEED_THRESHOLD &&
                                GetVehicle()->mTrafficLocomotion->mActualSpeed >
                                MY_MIN_SPEED_THRESHOLD) {
                                /*
                ...
                case OT_ENDOFROAD: {
                    if (mState != WAITING_AT_INTERSECTION) {
                        ...
                        bool found = false;
                        SwapArray < Road * >
                        *array = &(((Intersection *) SOMETHING)->mWaitingRoads);
...
void
TrafficAI::PerhapsTriggerImpedence(ObstacleType foundSOMETHING, float distSqr, void *SOMETHING) {
...
bool TrafficAI::AttemptLaneChange(ObstacleType foundSOMETHING, float distFromSOMETHINGSqr,
                                  void *SOMETHING) {
...
void TrafficAI::StopForSomething(float seconds, ObstacleType ID, float distSqr, void *obj) {
    ...
    float secondsHistoryDelay =
            numFramesInHistory * TrafficLocomotion::SECONDS_BETW_HISTORY_UPDATES;
...
void TrafficAI::CheckForObstacles(ObstacleType &objID, float &distFromObjSqr, void *&obj,
                                  bool &objOnMyRight) {
    float lookAhead = GetLookAheadDistance();
    ...
    if (!avatar->IsInCar() &&
        WillCollide(pPos, pDir, pRightSide, span, lookAhead, playerPos, charPosLiesOnMyRight)) {
    ...
    for (int i = 0; (i < TrafficManager::MAX_CHARS_TO_STOP_FOR) && (charCount < numChars); i++) {
        ...
        if (charToStopFor != NULL) {
            ...
            charCount++;
            if (WillCollide(pPos, pDir, pRightSide, span, lookAhead, charPos,
                            charPosLiesOnMyRight)) {
    ...
    if (!mIsInIntersection && mState != LANE_CHANGING) {
        ...
        if (intersection->GetType() == Intersection::N_WAY &&
            ...
            if ((mPrevState != WAITING_AT_INTERSECTION) ||
                ...
                if (WillCollide(pPos, pDir, pRightSide, span, lookAhead, endOfRoadPos,
                                endOfRoadPosLiesOnMyRight)) {
