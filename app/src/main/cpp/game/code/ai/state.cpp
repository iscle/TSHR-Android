...
*/
    ...
    void InCar::Exit(void) {
        ...
        if (mpCharacter->GetTargetVehicle() != NULL) {
            if ((mpCharacter->GetTargetVehicle()->GetDriver() == mpCharacter) &&
                (mpCharacter->GetRole() == Character::ROLE_PEDESTRIAN)) {
    ...
    void InCar::Update(float timeins) {
...
#endif
                    {
                        m_GetOutState = (pVehicle->GetSpeedKmh() < 30.0f) ? GETOUT_COMITTED
                                                                          : GETOUT_TRYING;
                ...
                case GETOUT_TRYING : {
                    if (!actionDown) {
                        ...
                        VehicleController *controller = GetVehicleCentral()->GetVehicleController(
                                GetVehicleCentral()->GetVehicleId(pVehicle));
                        if (controller) {
                            rAssert(dynamic_cast<HumanVehicleController *>(controller));
                            static_cast<HumanVehicleController *>(controller)->GetMappable()->GetButton(
                                    VehicleMappable::Brake)->SetValue(0.0f);
                            static_cast<HumanVehicleController *>(controller)->GetMappable()->GetButton(
                                    VehicleMappable::HandBrake)->SetValue(0.0f);
                        }
                        break;
                    }
                ...
                case GETOUT_COMITTED : {
                    if (pVehicle->GetSpeedKmh() < 3.0f) {
                        ...
                        if (controller) {
                            rAssert(dynamic_cast<HumanVehicleController *>(controller));
                            static_cast<HumanVehicleController *>(controller)->GetMappable()->GetButton(
                                    VehicleMappable::Gas)->SetValue(0.0f);
                            static_cast<HumanVehicleController *>(controller)->GetMappable()->GetButton(
                                    VehicleMappable::Brake)->SetValue(0.0f);
                            static_cast<HumanVehicleController *>(controller)->GetMappable()->GetButton(
                                    VehicleMappable::HandBrake)->SetValue(1.0f);
                        }
                        ...
                        VehicleController *v = GetVehicleCentral()->GetVehicleController(
                                GetVehicleCentral()->GetVehicleId(pVehicle));
                        if (v && (v->GetGas() > 0.0f)) {
                            m_GetOutState = GETOUT_NONE;
                            mpCharacter->GetTargetVehicle()->mGeometryVehicle->HideBrakeLights();
        ...
        if (m_InCarAction->IsRunning()) {
            ...
            */
            ...
            VehicleController *v = GetVehicleCentral()->GetVehicleController(
                    GetVehicleCentral()->GetVehicleId(mpCharacter->GetTargetVehicle(), false));
            if (v) {
                bool duh;
                ...
                } else // passenger
                {
                    if ((rmt::Abs(steer) > 0.5) && (speed > 50.0f)) {
                        action = new SteerAction(mpCharacter,
                                                 (steer < 0) ? "sway_right" : "sway_left", 20.0f);
                    }
                }
        ...
        if (theIntention == CharacterController::CelebrateSmall) {
            ...
            mpCharacter->GetActionController()->SequenceSingleAction(
                    new PlayAnimationAction(mpCharacter, mIsDriver ? "in_car_victory_small_driver"
                                                                   : "in_car_victory_small"));
            return;
        }
        ...
        if (theIntention == CharacterController::CelebrateBig) {
            ...
            mpCharacter->GetActionController()->SequenceSingleAction(
                    new PlayAnimationAction(mpCharacter, mIsDriver ? "in_car_victory_large_driver"
                                                                   : "in_car_victory_large"));
            return;
        }
        ...
        if (theIntention == CharacterController::WaveHello) {
            ...
            mpCharacter->GetActionController()->SequenceSingleAction(
                    new PlayAnimationAction(mpCharacter, "wave_driver"));
            return;
        }
        ...
        if (theIntention == CharacterController::WaveGoodbye) {
            ...
            mpCharacter->GetActionController()->SequenceSingleAction(
                    new PlayAnimationAction(mpCharacter, "wave_goodbye_driver"));
            return;
        }
    }
...
*/
    ...
    void Loco::SequenceAction(void) {
        ...
        if ((theIntention == CharacterController::Attack) &&
            GetCheatInputSystem()->IsCheatEnabled(CHEAT_ID_KICK_TOGGLES_CHARACTER_MODEL)) {
        ...
        if ((theIntention == CharacterController::Attack) &&
            mpCharacter->GetActionButtonHandler()) {
        ...
        if ((mpCharacter->GetActionButtonHandler() != (ActionButton::ButtonHandler *) 0) &&
            ((mLastActionFrame + 3) < GetGame()->GetFrameCount())) {
        ...
        if (theIntention == CharacterController::Dodge) {
            ...
            if (mpCharacter->IsNPC()) // change NPC controller state when dodging is done
            {
                pSeq->BeginSequence();
                pSeq->AddAction(
                        new ChangeNPCControllerState(mpCharacter, NPCController::FOLLOWING_PATH));
                pSeq->EndSequence();
            }
            mpCharacter->GetWalkerLocomotionAction()->StopDriver();
            return;
        ...
        //
        if (mpCharacter->GetController()->IsButtonDown(CharacterController::Dash) &&
...
*/
    ...
    void GetIn::Enter(void) {
        ...
        if (!mpCharacter->GetTargetVehicle()->mIrisTransition && !mObstructed &&
            ...
            Vehicle::Door door = (isDriver && !slide) ? Vehicle::DOOR_DRIVER
                                                      : Vehicle::DOOR_PASSENGER;
            Action *pAction = 0;
            ...
            bool high = false;
            if (isDriver) {
                high = mpCharacter->GetTargetVehicle()->GetDriverLocation().y >
                       CharacterTune::sGetInHeightThreshold;
            } else {
                high = mpCharacter->GetTargetVehicle()->GetPassengerLocation().y >
                       CharacterTune::sGetInHeightThreshold;
            }
            ...
            if (mpCharacter->GetTargetVehicle()->NeedToOpenDoor(door)) {
                ...
                pSeq->BeginSequence();
                pAction = new PlayAnimationAction(mpCharacter, anims[0],
                                                  CharacterTune::sfGetInOutOfCarAnimSpeed);
                pSeq->AddAction(pAction);
                pSeq->AddAction(
                        new CarDoorAction(mpCharacter->GetTargetVehicle(), Vehicle::DOORACTION_OPEN,
                                          door, delay * scale, time * scale));
                pSeq->EndSequence();
            }
            ...
            //
            pSeq->BeginSequence();
            pAction = new PlayAnimationAction(mpCharacter, anims[1],
                                              CharacterTune::sfGetInOutOfCarAnimSpeed);
            pSeq->AddAction(pAction);
            pSeq->EndSequence();
            ...
            if (mpCharacter->GetTargetVehicle()->HasActiveDoor(door) ||
                mpCharacter->GetTargetVehicle()->NeedToCloseDoor(door)) {
                float delay = CharacterTune::sGetInCloseDelay;
                float time = CharacterTune::sGetInCloseSpeed;
                float scale = 30.0f / CharacterTune::sfGetInOutOfCarAnimSpeed;

                pSeq->BeginSequence();
                pAction = new PlayAnimationAction(mpCharacter, anims[2],
                                                  CharacterTune::sfGetInOutOfCarAnimSpeed);
                pSeq->AddAction(new CarDoorAction(mpCharacter->GetTargetVehicle(),
                                                  Vehicle::DOORACTION_CLOSE, door, delay * scale,
                                                  time * scale));
                pSeq->AddAction(pAction);
                pSeq->EndSequence();
            }
            ...
            if (slide) {
                ...
                if (mpCharacter->GetTargetVehicle()->NeedToCloseDoor(Vehicle::DOOR_DRIVER)) {
                    ...
                    pSeq->BeginSequence();
                    pAction = new PlayAnimationAction(mpCharacter, "get_into_car_close_door_driver",
                                                      CharacterTune::sfGetInOutOfCarAnimSpeed);
                    pSeq->AddAction(new CarDoorAction(mpCharacter->GetTargetVehicle(),
                                                      Vehicle::DOORACTION_CLOSE,
                                                      Vehicle::DOOR_DRIVER, delay * scale,
                                                      time * scale));
                    pSeq->AddAction(pAction);
                    pSeq->EndSequence();
                }
            ...
            CGuiScreenIrisWipe *iw = static_cast<CGuiScreenIrisWipe *>(GetGuiSystem()->GetInGameManager()->FindWindowByID(
    ...
    void GetIn::HandleEvent(EventEnum id, void *pUserData) {
        if (id == EVENT_GUI_IRIS_WIPE_CLOSED) {
            ...
            GetSuperCamManager()->GetSCC(0)->SelectSuperCam(SuperCam::FOLLOW_CAM,
                                                            SuperCamCentral::CUT,
                                                            0); //Override the auto one
...
// figure out how to get into the car from an arbitrary location
    void
    GetIn::CalcGetInPath(int *nWaypoints, rmt::Vector *waypoints, bool *doingSlide, bool *jump) {
        const float origedge = 0.75f; // how far want to be from the car at the nav points (TODO : make this tunable?)
        const float edge = origedge /
                           1.414f; // x & z need to be sacled by sqrt(2) to make the diagonal distance right ;
        ...
        *nWaypoints = 0;
        ...
        if (((characterPosition.z > box.z) || (characterPosition.z < -box.z)) &&
            (isDriver ? (characterPosition.x > -(box.x - (edge / 2))) : (characterPosition.x <
                                                                         (box.x - (edge / 2))))) {
            onside = false;
        }
    ...
    void GetIn::Update(float timeins) {
        if (mpCharacter->CollidedThisFrame() && (mCollisionFailure == 0) &&
            !mpCharacter->GetJumpLocomotionAction()->IsInJump()) {
            mObstructed = true;
            mpCharacter->GetActionController()->Clear();
            Enter();
        }
...
*/
    ...
    void GetOut::Enter(void) {
        bool destroyed = !mpCharacter->GetTargetVehicle() ||
                         mpCharacter->GetTargetVehicle()->mVehicleDestroyed;
    ...
    void GetOut::HandleEvent(EventEnum id, void *pUserData) {
        if (id == EVENT_GUI_IRIS_WIPE_CLOSED) {
            ...
            if (!mObstructed) {
                ...
                //
                GetWorldPhysicsManager()->mCollisionManager->DetectRayIntersection(sIntersectInfo,
                                                                                   rayOrg, rayEnd,
                                                                                   false,
                                                                                   car->mCollisionAreaIndex);

                for (int i = 0; i < sIntersectInfo.GetSize(); i++) {
                    if (sIntersectInfo[i].mCollisionVolume &&
                        (sIntersectInfo[i].mCollisionVolume->GetCollisionObject()->GetSimState()->mAIRefPointer !=
                         mpCharacter) &&
                        (sIntersectInfo[i].mCollisionVolume->GetCollisionObject()->GetSimState()->mAIRefPointer !=
                         car)) {
                        mObstructed = true;
            ...
            character->RelocateAndReset(getOutPosition, choreo::GetWorldAngle(facing.x, facing.z),
                                        true, true);
            ...
            if (rmt::Abs(getOutPosition.y - newPos.y) > dist) {
                car->GetPosition(&getOutPosition);
                getOutPosition.y += car->GetExtents().y + 0.2f;
                character->RelocateAndReset(getOutPosition,
                                            choreo::GetWorldAngle(facing.x, facing.z), true, true);
            }
    ...
    void GetOut::DoGetOut(void) {
        mPanic = !mpCharacter->GetTargetVehicle() ||
                 mpCharacter->GetTargetVehicle()->mVehicleDestroyed;
        bool mIsDriver;
        ...
        if (mPanic) {
            ...
            if (mpCharacter->GetTargetVehicle()->HasActiveDoor(door)) {
                ...
                PlayAnimationAction *pAction = new PlayAnimationAction(mpCharacter, anims[2],
                                                                       CharacterTune::sfGetInOutOfCarAnimSpeed);
                pAction->AbortWhenMovementOccurs(true);
                pSeq->AddAction(new CarDoorAction(mpCharacter->GetTargetVehicle(),
                                                  Vehicle::DOORACTION_CLOSE, door, delay * scale,
                                                  time * scale, mpCharacter, pSeq));
                pSeq->AddAction(pAction);
            }
            pSeq->EndSequence();
        }
