...
void HeapStack::Pop() {
    ...
    m_CurPos--;
    rAssertMsg(m_CurPos >= 0,
               "Heap Stack Underflow! Calls to Push and Pop on heap stack are mismatched. [jdy]");
}

void HeapStack::Pop(GameMemoryAllocator alloc) {
    --currentStackPointer;
    pushNumberStack[currentStackPointer] = -1;
    rAssertMsg(m_Stack[m_CurPos] == alloc,
               "HeapStack - Detected mismatch in push/pop calls - fix, or tell Ian Gipson immediately");
    m_CurPos--;
    rAssertMsg(m_CurPos >= 0,
               "Heap Stack Underflow! Calls to Push and Pop on heap stack are mismatched. [jdy]");
}
...
void HeapActivityTracker::MemoryAllocated(radMemoryAllocator allocator, void *address,
                                          unsigned int size) {
...
HeapManager *HeapManager::GetInstance() {
    ...
    if (!s_Instance) {
        ...
        s_Instance->SetValue(
                0);          // Is there a better way to initialize these values?  Perhaps radThread should do this.
    }
    ...
    if (!p) {
        ...
        p = static_cast<void *>(new(GMA_PERSISTENT) HeapManager(
                currentHeap));  // Create a heap manager.  Init the heap stack with the current heap.
    ...
    HeapInfo info[] =
            {
                    ...
                    {GMA_AUDIO_PERSISTENT, "Audio Persistent",
                     GetHeapReference(GMA_AUDIO_PERSISTENT), 0},
    ...
    if (text) {
        ...
        for (i = 0; i < static_cast<int>(sizeof(info) / sizeof(info[0])); ++i) {
            ...
            if (heap) {
                ...
                float percentUsed =
                        static_cast<float>(used) / static_cast<float>(size) * 100.0f;
                float highWaterPercentUsed =
                        static_cast<float>(highWater) / static_cast<float>(size) * 100.0f;
                ...
                sprintf(buffy, "%8s %d/%d [%2.1f%%] HW: %2.2f [%2.1f%%] %d\n", info[i].name, used,
                        size, percentUsed, hwMB, highWaterPercentUsed, numObjects);
    ...
    if (CommandLineOptions::Get(CLO_HEAP_STATS)) {
        ...
        for (i = 0; i < static_cast<int>(sizeof(info) / sizeof(info[0])); ++i) {
            ...
            if (heap) {
                ...
                char buffy[256];
                sprintf(buffy, "%8s %d/%d [%2.1f%%] HW: %2.2f [%2.1f%%] %d", info[i].name, used,
                        size, percentUsed, hwMB, highWaterPercentUsed, numObjects);
    ...
    if (CommandLineOptions::Get(CLO_ART_STATS)) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            if (heap) {
                ...
                char buffy[256];
                sprintf(buffy, "%8s %2.2f/%2.2f [%2.1f%%] HW: %2.2f [%2.1f%%]", info[i].name,
                        usedMB, sizeMB, percentUsed, hwMB, highWaterPercentUsed);
        ...
        tris = 300000;
#else
        tris = p3d::pddi->GetIntStat(PDDI_STAT_BUFFERED_PRIM) +
               p3d::pddi->GetIntStat(PDDI_STAT_BUFFERED_INDEXED_PRIM);
#endif
