...
CStateProp *
CStateProp::CreateStateProp(CStatePropData *statePropData, unsigned int state, tPose *pose) {
...
CStateProp::CStateProp(tAnimatedObject *animatedObject, CStatePropData *statePropData,
                       unsigned int state) :
...
void CStateProp::SetState(unsigned int state) {
    ...
    for (i = 0; i < numFrameControllers; i++) {
        FrameControllerData frameControllerData = m_StatePropData->GetFrameControllerData(
                m_CurrentState, i);
        tFrameController *fc = GetFrameControllerByIndex(i);
