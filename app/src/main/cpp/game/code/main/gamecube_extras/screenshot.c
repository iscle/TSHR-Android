...
static GRAB_STATUS GrabChunk(u32 chunk,
    ...
    if (chunk >= XFB_BASE && chunk < XFB_BASE + XFB_RANGE) {
        ...
        if (chunk == XFB_BASE) {    //save off XFB screenshot
        ...
        if (chunk == EFB_BASE) {    //save off EFB screenshot
            if (!g_minimize_buffer) {
    ...
    if (chunk == XFB_BASE + XFB_RANGE - 1 ||
        chunk == EFB_BASE + EFB_RANGE - 1) {    //free screenshot buffer space
        if (g_data) {
            deallocator(g_data);
            g_data = 0;
        }
    ...
    while (status == GRAB_TRANSFERRING) {
        ...
        if (g_mail_waiting) {
            if (HIOReadMailbox(&temp)) {
                ...
                if (temp >= PING_BASE && temp < PING_BASE + PING_RANGE) {    //ping message received
                    ...
                    OSReport("SCREENSHOT: Sent ping back.\n");
                           ...
                           (temp >= EFB_BASE &&
                            temp < EFB_BASE + EFB_RANGE)) {    //request for screen capture
                    escapeCount = 0;
                    ...
                    status = GrabChunk(g_received_signal, bufferXFB, allocator, deallocator);
                           ...
                           MINIMIZE_BUFFER_TRUE) {    //request to use minimal memory (100K), however game pauses
                    g_minimize_buffer = TRUE;
                } else if (temp ==
                           MINIMIZE_BUFFER_FALSE) {    //request to not use minimal memory and keep game running
                    g_minimize_buffer = FALSE;
                }
            }
        }
        if (!g_minimize_buffer) {    //Escape loop since were not pausing the game to transfer
            return;
        }
        if (g_minimize_buffer &&
            escapeCount > 150000000) {    //The PC screenshot app has died - escape
            if (g_data) {
                deallocator(g_data);
                g_data = 0;
            }
