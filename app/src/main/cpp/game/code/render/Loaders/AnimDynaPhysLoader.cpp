...
AnimDynaPhysLoader::AnimDynaPhysLoader() : tSimpleChunkHandler(
        SRR2::ChunkID::INSTA_ANIM_DYNA_PHYS_DSG) {
...
tEntity *AnimDynaPhysLoader::LoadObject(tChunkFile *file, tEntityStore *store) {
    ...
    StatePropDSG *globalStatepropEntity = static_cast<StatePropDSG *>(GetAllWrappers()->GetGlobalEntity(
            tName::MakeUID(typeName)));
        ...
        switch (file->GetCurrentID()) {
                ...
                // We are loading a stateprop
            {
                if (!haveGlobal) {
                    ...
                    pWrappedObject = (AnimDynaPhysWrapper *) pObjWrapperLoader->LoadObject(file,
                                                                                           store);
            ...
            case SRR2::ChunkID::ANIM_DSG_WRAPPER: {
                if (!haveGlobal) {
                    ...
                    pWrappedObject = (AnimDynaPhysWrapper *) pWrapperLoader->LoadObject(file,
                                                                                        store);
                    pWrappedObject->AddRef();
            ...
            case SRR2::ChunkID::OBJECT_ATTRIBUTES: {
                if (!haveGlobal) {
                    ...
                    file->GetString(
                            tempsound); //Chuck: Reading the new sound properties since we dont use the sound returned from the ATC manager.
                    ...
                    pCollAttr = GetATCManager()->CreateCollisionAttributes(classType, physPropID,
                                                                           volume);
                    pCollAttr->AddRef();
                    pCollAttr->SetSound(
                            tempsound); //Chuck: Setting the CollAttr to use the correct sound from the OTC chunk.
                }
            }
                break;
            ...
            case SRR2::ChunkID::INSTANCES: {
                ...
                while (file->ChunksRemaining()) {
                    ...
                    if (m_IsStateProp) {
                        ...
                        if (persistance && ((haveGlobal &&
                                             globalStatepropEntity->GetCollisionAttributes()->GetClasstypeid()) ||
                                            (!haveGlobal &&
                                             ((pCollAttr->GetClasstypeid() == PROP_BREAKABLE) ||
                                              (pCollAttr->GetClasstypeid() == PROP_MOVEABLE))))) {
                            if (isMissionProp == false)
                                persistentID = GetPersistentWorldManager()->GetPersistentObjectID(
                                        tEntity::MakeUID(file->GetFilename()),
                                        tEntity::MakeUID(typeName));
                            else
                                persistentID = -1;
                        }
                        if (persistentID >= -1) {
                            ...
                            if (haveGlobal) {
                                ...
                                pStatePropDSG->LoadSetup(pWrappedObject->GetStatePropData(), 0,
                                                         matrix, pCollAttr, true, store,
                                                         useSharedtPose,
                            ...
                            bool collision = store->TestCollision(pStatePropDSG->GetUID(),
                                                                  pStatePropDSG);
                            if (!collision) {
                        ...
                        rAssert(pCollAttr != NULL);
                        pAnimDSG->LoadSetUp(pCollAttr, matrix, pWrappedObject->GetDrawable(),
                                            pWrappedObject->GetController(), store);
                        mpListenerCB->OnChunkLoaded(static_cast<InstDynaPhysDSG *>(pAnimDSG),
                                                    mUserData, _id);
...
AnimDynaPhysWrapperLoader::AnimDynaPhysWrapperLoader() : tSimpleChunkHandler(
        SRR2::ChunkID::ANIM_DSG_WRAPPER) {
    ...
    while (file->ChunksRemaining()) {
        ...
        switch (file->GetCurrentID()) {
            case P3D_COMPOSITE_DRAWABLE: {
                ...
                // for our DSG objects
                rAssertMsg(wrapper->mCompDraw == NULL,
                           "There must only be one composite drawable in the wrapper!");
            ...
            case P3D_MULTI_CONTROLLER: {
                ...
                wrapper->mMultiController = (tMultiController *) mpMCLoader->LoadObject(file,
                                                                                        store);
                rAssert(wrapper->mMultiController != NULL);
                ...
                bool collision = store->TestCollision(wrapper->mMultiController->GetUID(),
                                                      wrapper->mMultiController);
                if (!collision) {
            ...
            case Pure3D::Animation::FrameControllerData::FRAME_CONTROLLER: {
                tFrameController *pFC = static_cast<tFrameController *>(mpFCLoader->LoadObject(
                        file, store));
                rAssert(pFC != NULL);
                store->Store(pFC);
            }
            ...
            case Simulation::Collision::OBJECT: {
                ...
                wrapper->mCollisionObject = static_cast<sim::CollisionObject *>(mpCollObjLoader->LoadObject(
                        file, store));
                rAssert(wrapper->mCollisionObject != NULL);
            ...
            case Pure3D::Animation::AnimationData::ANIMATION: {
                tAnimation *pAnimation = static_cast<tAnimation *>(mpAnimationLoader->LoadObject(
                        file, store));
            ...
            case Simulation::Physics::OBJECT:
                ...
                wrapper->mPhysicsObject = (sim::PhysicsObject *) mpPhysObjLoader->LoadObject(file,
                                                                                             store);
                rAssert(wrapper->mPhysicsObject != NULL);
            ...
            case P3D_SKELETON: {
                ...
                tSkeleton *pSkeleton = static_cast<tSkeleton *>(mpSkelLoader->LoadObject(file,
                                                                                         store));
...
AnimObjDSGWrapperLoader::AnimObjDSGWrapperLoader() : tSimpleChunkHandler(
        SRR2::ChunkID::ANIM_OBJ_DSG_WRAPPER) {
    ...
    while (file->ChunksRemaining()) {
        ...
        switch (id) {
            ...
            case StateProp::STATEPROP: {
                CStatePropData *pPropData = static_cast<CStatePropData *>(mpStatePropLoader->LoadObject(
                        file, store));
