...
SpatialNode &SpatialTreeIter::rSeekNode
    ...
    if (pCurNode->GetSubTreeSize() > 0) {
        ...
        if (PlaneVolResult >
            0.0f) //The Plane is greater than TVolume in Posn, so TVolume is in the LT Partitiion..
        {
            return rSeekNode(irVolume, iCurNodeOffset + pCurNode->LChildOffset());
        } else {
            if (PlaneVolResult <
                0.0f) //The Plane is less han TVolume in Posn, so TVolume is in the GT Partitiion..
            {
...
void SpatialTreeIter::MarkAllSphere
    ...
    if (intersectResult == 0.0f) //intersection
    {
        ...
        MarkAllSphereIntersect(iCurNodeOffset + pCurNode->LChildOffset(), BoxPtsLT, irSphere,
                               iMark);

        irBoxPts.CutOffLT(pCurNode->mData.mSubDivPlane);

        MarkAllSphereIntersect(iCurNodeOffset + pCurNode->RChildOffset(), irBoxPts, irSphere,
                               iMark);
    } else {
        if (intersectResult > 0.0f) //Plane falls on the right, so recurse lt side
        {
...
void SpatialTreeIter::MarkAllSphereIntersect
    ...
    if (intersectResult == 0.0f) //intersection
    {
        ...
        if (intersectResult > 0.0f) //Plane falls on the right, so recurse lt side
        {
            ...
            MarkAllSphereIntersect(iCurNodeOffset + pCurNode->LChildOffset(), irBoxPts, irSphere,
                                   iMark);
        } else //Plane falls on the left(lt), so recurse gt side
        {
            irBoxPts.CutOffLT(pCurNode->mData.mSubDivPlane);
...
void SpatialTreeIter::MarkSubTrees
    ...
    if (intersectResult == 0.0f) //intersection
    {
        ...
        if (intersectResult > 0.0f) //Plane falls on the right, so recurse lt side
        {
            ...
            MarkSubTrees(iCurNodeOffset + pCurNode->LChildOffset(), irBoxPts, irDesiredVolume,
                         iMark);
        } else //Plane falls on the left(lt), so recurse gt side
        {
            irBoxPts.CutOffLT(pCurNode->mData.mSubDivPlane);

            MarkSubTrees(iCurNodeOffset + pCurNode->RChildOffset(), irBoxPts, irDesiredVolume,
                         iMark);
        }
    }
}
