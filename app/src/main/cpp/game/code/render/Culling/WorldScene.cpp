...
struct gShaderCompare
        : public std::binary_function<const WorldScene::zSortBlah &, const WorldScene::zSortBlah &, bool> {
...
//////////////////////////////////////////////////////////////////////////
        ...
        for (i = mStaticTreeWalker.rCurrent().mSEntityElems.mUseSize - 1; i > -1; i--) {
            ...
            {
                case 1:
                    mShadowCastersPass1.Add(
                            (IEntityDSG *&) mStaticTreeWalker.rCurrent().mSEntityElems[i]);
                    break;
                case 2: {
                ...
                default:
                    if (!mStaticTreeWalker.rCurrent().mSEntityElems[i]->mTranslucent &&
                        mStaticTreeWalker.rCurrent().mSEntityElems[i]->CastsShadow() == 0) {
                        //mpZSorts.AddUse(1);
        ...
        for (i = mStaticTreeWalker.rCurrent().mAnimCollElems.mUseSize - 1; i > -1; i--) {
            ...
            switch (3)//mStaticTreeWalker.rCurrent().mAnimCollElems[i]->CastsShadow())
            {
                case 1:
                    mShadowCastersPass1.Add(
                            (IEntityDSG *&) mStaticTreeWalker.rCurrent().mAnimCollElems[i]);
                    break;
                case 2: {
                    ...
                    break;
                default:
                    if (!mStaticTreeWalker.rCurrent().mAnimCollElems[i]->mTranslucent &&
                        mStaticTreeWalker.rCurrent().mAnimCollElems[i]->CastsShadow() == 0) {
                        //mpZSorts.AddUse(1);
        ...
        for (i = mStaticTreeWalker.rCurrent().mAnimElems.mUseSize - 1; i > -1; i--) {
            ...
            switch (3)//mStaticTreeWalker.rCurrent().mAnimElems[i]->CastsShadow())
            {
                case 1:
                    mShadowCastersPass1.Add(
                            (IEntityDSG *&) mStaticTreeWalker.rCurrent().mAnimElems[i]);
                    break;
                case 2: {
                    ...
                    break;
                default:
                    if (!mStaticTreeWalker.rCurrent().mAnimElems[i]->mTranslucent &&
                        mStaticTreeWalker.rCurrent().mAnimElems[i]->CastsShadow() == 0) {
                        //mpZSorts.AddUse(1);
        ...
        for (i = mStaticTreeWalker.rCurrent().mDPhysElems.mUseSize - 1; i > -1; i--) {
            ...
            case 2:
                ...
                */
                ...
                default:
                    if (!mStaticTreeWalker.rCurrent().mDPhysElems[i]->mTranslucent &&
                        mStaticTreeWalker.rCurrent().mDPhysElems[i]->CastsShadow() == 0) {
                        // mpZSorts.AddUse(1);
        ...
        for (i = mStaticTreeWalker.rCurrent().mSPhysElems.mUseSize - 1; i > -1; i--) {
            ...
            switch (3)//mStaticTreeWalker.rCurrent().mSPhysElems[i]->CastsShadow())
            {
                case 1:
                    mShadowCastersPass1.Add(
                            (IEntityDSG *&) mStaticTreeWalker.rCurrent().mSPhysElems[i]);
                    break;
                case 2: {
                ...
                default:
                    if (!mStaticTreeWalker.rCurrent().mSPhysElems[i]->mTranslucent &&
                        mStaticTreeWalker.rCurrent().mSPhysElems[i]->CastsShadow() == 0) {
                        //mpZSorts.AddUse(1);
...
void WorldScene::MarkCameraVisible(tPointCamera *pCam, unsigned int iFilter) {
    ...
    pCam->GetTarget(&ViewVector);
    ViewVector.Sub(CamPosn);
    ViewVector.Normalize();

    SetVisCone(ViewVector, CamPosn, pCam->GetFieldOfView() /
                                    1.87f);//this tweak was done to account for the pop-in along the diagonal, especially evident in the interiors//0.8726645f); // 50 degrees 1.745329f); //100 degrees
...
//    Plane cameraPlanes[6];
    ...
    // camera parameters
    float fov, aspect;
    pCam->GetFOV(&fov, &aspect); //fov = fov/1.5f;
    ...
    rmt::Vector toFarPlane = look;
    toFarPlane.Scale(farPlane);
    rmt::Vector onFarPlane = eye;
    onFarPlane.Add(toFarPlane);  // a point on the far plane

    rmt::Vector lookInv = pCam2WorldMat->Row(2);
    lookInv.Scale(-1);
    // toNearPlane offset by a small value to avoid some artifacts
    rmt::Vector toNearPlane = look;
    toNearPlane.Scale(nearPlane - 0.00001f);
    rmt::Vector onNearPlane = eye;
    onNearPlane.Add(toNearPlane);

    // find surface normals for left and right clipping pplanes
    // first get camera space vectors
    rmt::Vector tmpr(rmt::Cos(fov / 2), 0,
                     -rmt::Sin(fov / 2));  // right vector is 'rotated' by fov/2
    rmt::Vector tmpl = tmpr;
    tmpl.Scale(-1, 1, 1);   // left vector is inverse of right
