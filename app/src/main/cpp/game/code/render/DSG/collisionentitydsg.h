...
public:
    ...
    CollisionAttributes(char *p_sound, char *p_particle, char *p_animation, float friction,
                        float mass, float elasticity, unsigned int classtypeid, float volume);
...
public:
    ...
    PreReactToCollision(sim::SimState *pCollidedObj, sim::Collision &inCollision) = 0;
