...
StatePropDSG::LoadSetup(CStatePropData *statePropData,
    ...
    if (type == PROP_MOVEABLE || type == PROP_ONETIME_MOVEABLE || type == PROP_BREAKABLE) {
        ...
        if (IsFullArticulatedPhysicsObject(physicsObject)) {
            poser::PoseEngine *poseEngine = new poser::PoseEngine(p3dPose, 1,
                                                                  p3dPose->GetNumJoint());
            poseEngine->AddRef();
            ...
            sim::SimStateArticulated *pSimStateArt = sim::SimStateArticulated::CreateSimStateArticulated(
                    statePropData->GetUID(), poserPose, sim::SimStateAttribute_Default,
                    ipSearchStore);
            if (pSimStateArt != NULL) {
                ...
                // Fallback path
                pSimState = sim::SimState::CreateSimState(statePropData->GetUID(),
                                                          sim::SimStateAttribute_Default,
                                                          ipSearchStore);
            }
...
StatePropDSG::Clone(const char *Name, const rmt::Matrix &iMatrix) const {
    ...
    pClone->LoadSetup(spData, START_STATE, iMatrix, pCollAttr, true, NULL, usingSharedPose, NULL,
                      NULL);
    pClone->SetName(Name);
...
StatePropDSG::Update(float timeInSec) {
    ...
    if (mIsHit && (GetCollisionAttributes()->GetClasstypeid() == PROP_ONETIME_MOVEABLE)) {
        if (!GetGameplayManager()->TestPosInFrustrumOfPlayer(mPosn, 0, mSphere.radius)) {
            this->AddRef();
            ((WorldRenderLayer *) GetRenderManager()->mpLayer(RenderEnums::LevelSlot))->RemoveGuts(
                    this);
...
StatePropDSG::PreReactToCollision(sim::SimState *pCollidedObj, sim::Collision &inCollision) {
    ...
    if (this->GetSimState()->GetControl() == sim::simAICtrl &&
        pCollidedObj->GetCollisionObject()->IsStatic() == true) {
...
StatePropDSG::RecieveEvent(int callback, CStateProp *stateProp) {
    ...
    switch (callback) {
        ...
        case StatePropEnum::eRemoveFromWorld:
            GetBreakablesManager()->RemoveBrokenObjectFromWorld(this, RenderEnums::LevelSlot,
                                                                m_IsDynaLoaded);
