                ...
                if (nextCheck == 1 && !previouslyCheated)  //This is the finish line.
                {
                    ...
                    if (static_cast<int>(mPlayers[playerID].mNumLaps) == mNumLaps - 1) {
                        ...
                        if (!mVehicleSlots[playerID].mIsHuman) {
                            ...
                            mNumHumansFinished++;
                            int controllerID = GetInputManager()->GetControllerIDforPlayer(
                                    playerID);
                            if (controllerID != -1) {
                                GetInputManager()->GetController(controllerID)->SetGameState(
            ...
            if (mCountDownTime > 2000) {
                ...
                for (i = 0; i < SuperSprintData::NUM_PLAYERS; ++i) {
                    ...
                    if (controllerID != -1) {
                        GetInputManager()->GetController(controllerID)->SetGameState(
                                Input::ACTIVE_ALL);
                        GetInputManager()->GetController(controllerID)->SetRumble(
                                GetInputManager()->IsRumbleEnabled());
                    } else {
                ...
                for (int j = 0; j < numAIs; j++) {
                    if (numAIsDoingTurbo < MAX_AIS_DOING_TURBO) {
                        ...
                        if (coinflip == 0 || coinflip == 1 || coinflip == 2) {
                            rAssert(0 <= aiIndices[j] &&
                                    aiIndices[j] < SuperSprintData::NUM_PLAYERS);
                ...
                if (numAIs > 1 && numAIsDoingTurbo == 0) {
                    ...
                    rAssert(0 <= aiIndices[randAI] &&
                            aiIndices[randAI] < SuperSprintData::NUM_PLAYERS);
...
void SuperSprintManager::LoadScriptData() {
    HeapMgr()->PushHeap(
            GMA_LEVEL_OTHER);  // I choose other because this stuff will exist longer than a single mission
...
void SuperSprintManager::OnButtonDown(int controllerId, int buttonId, const Button *pButton) {
    switch (mCurrentState) {
        case RACING: {
            ...
            if (buttonId == CamSelect && mVehicleSlots[playerID].mIsHuman &&
                !GetGameFlow()->GetContext(CONTEXT_SUPERSPRINT)->IsSuspended()) {
                ...
                if (playerCount == 1) {
                    ...
                    if (GetSuperCamManager()->GetSCC(0)->GetActiveSuperCam()->GetType() ==
...
char SuperSprintManager::GetNumCheckpoints() {
    if (mNumCheckPoints == 0) {
        if ((RenderEnums::LevelEnum)(GetCurrentLevelIndex() + RenderEnums::B00) ==
            RenderEnums::B02 ||
            (RenderEnums::LevelEnum)(GetCurrentLevelIndex() + RenderEnums::B00) ==
            RenderEnums::B07) {
            if (mGoLeft) {
        ...
        } else if ((RenderEnums::LevelEnum)(GetCurrentLevelIndex() + RenderEnums::B00) ==
                   RenderEnums::B05) {
            if (mGoLeft) {
...
void SuperSprintManager::SetUpCars() {
    ...
    for (int i = 0; i < SuperSprintData::NUM_PLAYERS; ++i) {
        if (mVehicleSlots[i].mState == SuperSprintData::CarData::SELECTED) {
            ...
            if (mVehicleSlots[i].mIsHuman) {
                mNumHumanPlayers++;
                mVehicleSlots[i].mVehicle = GetVehicleCentral()->InitVehicle(carName, true, NULL,
                                                                             VT_USER,
                ...
                mVehicleSlots[i].mVehicle = GetVehicleCentral()->InitVehicle(carName, true,
                                                                             scriptName, VT_AI,
                                                                             VehicleCentral::FORCE_NO_DRIVER,
                                                                             false,
...
void SuperSprintManager::PositionAI() {
    ...
    for (i = 0; i < SuperSprintData::NUM_PLAYERS; ++i) {
        ...
        if (!mVehicleSlots[i].mIsHuman) {
            if (mVehicleSlots[i].mVehicleAI == NULL) {
                mVehicleSlots[i].mVehicleAI = new WaypointAI(mVehicleSlots[i].mVehicle, false,
                                                             smallTriggerRadius,
                                                             true);  // indicate false to segment optimization
                mVehicleSlots[i].mVehicleAI->SetUseMultiplier(
                        false); // no traffic in supersprint... no need for multiplier
    ...
    for (i = 0; i < mNumCheckPointLocators; i++) {
        ...
        bool succeeded = VehicleAI::FindClosestPathElement(locPos, closestElem, seg, segT, roadT,
                                                           true);
        if (!succeeded) {
            char msg[512];
    ...
    for (i = 0; i < SuperSprintData::NUM_PLAYERS; ++i) {
        if (mVehicleSlots[i].mState == SuperSprintData::CarData::SELECTED) {
            GetAvatarManager()->PutCharacterInCar(
                    GetAvatarManager()->GetAvatarForPlayer(i)->GetCharacter(),
                    mVehicleSlots[i].mVehicle);
            if (!mVehicleSlots[i].mIsHuman) {
...
void SuperSprintManager::LoadCars() {
    ...
    for (int i = 0; i < SuperSprintData::NUM_PLAYERS; ++i) {
        ...
        if (mVehicleSlots[i].mState == SuperSprintData::CarData::WAITING) {
            ...
            while (strcmp(SuperSprintData::VEHICLE_NAMES[carNum].name,
                          mVehicleSlots[(i + 1) % SuperSprintData::NUM_PLAYERS].mCarName) == 0 ||
                   strcmp(SuperSprintData::VEHICLE_NAMES[carNum].name,
                          mVehicleSlots[(i + 2) % SuperSprintData::NUM_PLAYERS].mCarName) == 0 ||
                   strcmp(SuperSprintData::VEHICLE_NAMES[carNum].name,
                          mVehicleSlots[(i + 3) % SuperSprintData::NUM_PLAYERS].mCarName) == 0) {
...
void SuperSprintManager::LoadCharacters() {
    ...
    for (i = 0; i < SuperSprintData::NUM_PLAYERS; ++i) {
        ...
        GetCharacterManager()->AddPCCharacter(
                SuperSprintData::CHARACTER_NAMES[mPlayers[i].mCharacterIndex], "npd");
    }
}
    ...
    for (i = 0; i < SuperSprintData::NUM_PLAYERS; ++i) {
        ...
        if (mNumHumanPlayers == 1) {
            ...
            if (pressed) {
                ...
                if (!mVehicleSlots[i].mIsHuman ||
                    GetSuperCamManager()->GetSCC(0)->GetActiveSuperCam()->GetType() ==
                    SuperCam::SUPER_SPRINT_CAM ||
                    GetSuperCamManager()->GetSCC(0)->GetActiveSuperCam()->GetType() ==
...
void SuperSprintManager::PositionCharacters() {
    ...
    for (i = 0; i < SuperSprintData::NUM_PLAYERS; ++i) {
        ...
        if (loc) {
            Character *character = GetCharacterManager()->GetCharacterByName(tEntity::MakeUID(
                    SuperSprintData::CHARACTER_NAMES[mPlayers[i].mCharacterIndex]));
