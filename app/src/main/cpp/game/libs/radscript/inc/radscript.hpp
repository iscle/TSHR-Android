...
struct IRadScript
    ...
    void Load(const char *pFileName, ScriptLoadCallback *pCallback, void *pCallbackUserData,
              radMemoryAllocator allocator = RADMEMORY_ALLOC_DEFAULT);
    ...
    if (strcmp(pToken, "DEFAULT") == 0) {
        ...
        *pnOptionField = 0;
        ...
        if (pToken[0] == '|' || pToken[nTokenLength] == '|') {
            rDebugString(
                    "IRadScript: Invalid syntax after \'option\' keyword, \'|\' cannot be leading or trailing character.\n");
            return -1;
        }
        ...
        for (int i = 0; i < nTokenLength; i++) {
            if (pToken[i] == '|') {
                pszOption[nOptionCount] = &pToken[i + 1];
                nOptionCount++;
            }
        }
...
inline bool IRadScript::IsCurrentPlatForm() const {
    ...
    for (int i = 0; i < m_nCurrPlatformOptionStackSize; i++) {
        if (m_PlatformOptionStack[i] != ParserConst::PT_DEFAULT) {
            nReturnOption = ((ParserConst::PlatformType)(
                    (int) nReturnOption | (int) m_PlatformOptionStack[i]));
        }
    }
...
inline bool
IRadScript::OnBeginObject(const char *pObjectName, const char *pClassName, const bool bCreate) {
    ...
    if (bCreate) {
        ...
        if (m_xCurrObjectInstance != NULL) {
            if (false ==
                m_xIRadNameSpace_Context->AddInstance(pObjectName, m_xCurrObjectInstance)) {
...
inline bool IRadScript::ParseMethod(
    ...
    while (!AttemptMatch(ParserConst::TOK_RPAREN)) {
        ...
        if (nParamSize >= MAX_PARAM_SIZE) {
            rDebugString(
                    "IRadScript : (Parser error) parameters exceeds 31, possible corrupted script file.\n");
            rAssert(0);
        }
    }
