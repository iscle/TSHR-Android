...
/* static */ void radSoundHalMemoryRegion::Initialize
    ...
    s_pRadSoundMemoryRegion_Root = new("radSoundHalMemoryRegion (Root)",
                                       allocator) radSoundHalMemoryRegion();
...
IRadSoundHalMemoryRegion *radSoundHalMemoryRegion::CreateChildRegion
    ...
    if (size > 0) {
        ...
        if (xIRadMemoryObject != NULL) {
            ...
            pRadSoundMemoryRegion = new("radSoundHalMemoryRegion",
                                        GetThisAllocator()) radSoundHalMemoryRegion();
...
/* virtual */ IRadSoundHalMemoryRegion *radSoundHalMemoryRegion::GetNextChild
    ...
    return static_cast<radSoundHalMemoryRegion *>(pIRadSoundHalMemoryRegion_LastChild)->m_pMemoryRegion_Next;
}
...
/* static */ void radSoundHalMemoryRegion::GetStats_Recurse
    ...
    if (recurse == true) {
        radSoundHalMemoryRegion *pRsmr = static_cast<radSoundHalMemoryRegion *>(pRadSoundMemoryRegion->GetFirstChild());

        while (pRsmr != NULL) {
            GetStats_Recurse(pRsmr, pFreeMemory, pLargestBlock, pNumObjects, recurse);

            pRsmr = static_cast<radSoundHalMemoryRegion *>(pRadSoundMemoryRegion->GetNextChild(
                    pRsmr));
        }
    }
}
...
unsigned int radSoundHalBufferCalculateMemorySize
    ...
    sizeInBytes = ::radMemoryRoundUp(sizeInBytes,
                                     ...
                                     pIRadSoundHalAudioFormat->GetNumberOfChannels());
