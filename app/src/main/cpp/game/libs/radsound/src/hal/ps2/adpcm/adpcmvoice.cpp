        ...
        if (m_xRadSoundBufferAdpcmPs2 != NULL) {
            ...
            if (m_HaveVoices == true) {
                ...
                if (m_pHardwareVoiceNumberArrayDry[0] != NULL_VOICE) {
                    radSoundIopPs2::GetInstance()->GetWriteCommandPacket()->m_KeyOnMask[m_DryCore] |= GetVoiceSwitchMask(
                            m_pHardwareVoiceNumberArrayDry);
                    radSoundIopPs2::GetInstance()->GetWriteCommandPacket()->m_KeyOffMask[m_DryCore] &= ~GetVoiceSwitchMask(
                            m_pHardwareVoiceNumberArrayDry);
                    radSoundIopPs2::GetInstance()->GetWriteCommandPacket()->m_VoiceDryMaskLeft[m_DryCore] |= GetVoiceSwitchMask(
                            m_pHardwareVoiceNumberArrayDry);
                    radSoundIopPs2::GetInstance()->GetWriteCommandPacket()->m_VoiceDryMaskRight[m_DryCore] |= GetVoiceSwitchMask(
                            m_pHardwareVoiceNumberArrayDry);
                }

                for (unsigned int core = 0; core < 2; core++) {
                    if (m_pHardwareVoiceNumberArrayCoreEffect[core][0] != NULL_VOICE) {
                        radSoundIopPs2::GetInstance()->GetWriteCommandPacket()->m_KeyOnMask[core] |= GetVoiceSwitchMask(
                                m_pHardwareVoiceNumberArrayCoreEffect[core]);
                        radSoundIopPs2::GetInstance()->GetWriteCommandPacket()->m_KeyOffMask[core] &= ~GetVoiceSwitchMask(
                                m_pHardwareVoiceNumberArrayCoreEffect[core]);
                        radSoundIopPs2::GetInstance()->GetWriteCommandPacket()->m_VoiceEffectMaskLeft[core] |= GetVoiceSwitchMask(
                                m_pHardwareVoiceNumberArrayCoreEffect[core]);
                        radSoundIopPs2::GetInstance()->GetWriteCommandPacket()->m_VoiceEffectMaskRight[core] |= GetVoiceSwitchMask(
                                m_pHardwareVoiceNumberArrayCoreEffect[core]);

                        if (m_AuxModes[core] == radSoundAuxMode_PostFaderDirect) {
                            radSoundIopPs2::GetInstance()->GetWriteCommandPacket()->m_VoiceDryMaskLeft[core] |= GetVoiceSwitchMask(
                                    m_pHardwareVoiceNumberArrayCoreEffect[core]);
                            radSoundIopPs2::GetInstance()->GetWriteCommandPacket()->m_VoiceDryMaskRight[core] |= GetVoiceSwitchMask(
                                    m_pHardwareVoiceNumberArrayCoreEffect[core]);
                        }
...
void radSoundVoiceAdpcmPs2::SetVoiceLoopAddresses(void) {
    if (m_HaveVoices == true) {
        for (unsigned int voice = 0;
             voice < m_xRadSoundBufferAdpcmPs2->GetFormat()->GetNumberOfChannels(); voice++) {
            unsigned int loopBackPoint = m_xRadSoundBufferAdpcmPs2->GetStartAddress() + (voice *
                                                                                         (m_xRadSoundBufferAdpcmPs2->GetSizeInBytes() /
                                                                                          m_xRadSoundBufferAdpcmPs2->GetFormat()->GetNumberOfChannels()));
...
unsigned int radSoundVoiceAdpcmPs2::GetPlaybackPositionInSamples
    ...
    if (m_HaveVoices == true && m_ReadSynch == true) {
        ...
        unsigned int bufferEnd =
                bufferStart + (m_xRadSoundBufferAdpcmPs2->GetSizeInBytes() / channels);
...
void radSoundVoiceAdpcmPs2::SetPlaybackPositionInSamples
    ...
    if (m_xRadSoundBufferAdpcmPs2 != NULL) {
        rAssert(samples < m_xRadSoundBufferAdpcmPs2->GetFormat()->FramesToSamples(
                m_xRadSoundBufferAdpcmPs2->GetSizeInFrames()));
...
void radSoundVoiceAdpcmPs2::SetStartAddressInSamples
    ...
    if (m_HaveVoices == true) {
        ...
        unsigned int chunkSize = m_xRadSoundBufferAdpcmPs2->GetSizeInBytes() /
                                 m_xRadSoundBufferAdpcmPs2->GetFormat()->GetNumberOfChannels();
        unsigned int startAddress = m_xRadSoundBufferAdpcmPs2->GetStartAddress();
        unsigned int offsetIntoChunk =
                m_xRadSoundBufferAdpcmPs2->GetFormat()->SamplesToBytes(samples) /
                m_xRadSoundBufferAdpcmPs2->GetFormat()->GetNumberOfChannels();
        ...
        for (unsigned int voice = 0;
            ...
            if (m_pHardwareVoiceNumberArrayDry[0] != NULL_VOICE) {
                radSoundIopPs2::GetInstance()->GetWriteCommandPacket()->m_SpuVoiceParams[m_DryCore][m_pHardwareVoiceNumberArrayDry[voice]].m_StartAddress =
                        chunkStartAddress + offsetIntoChunk;
            ...
            for (unsigned int core = 0; core < 2; core++) {
                if (m_pHardwareVoiceNumberArrayCoreEffect[core][0] != NULL_VOICE) {
                    radSoundIopPs2::GetInstance()->GetWriteCommandPacket()->m_SpuVoiceParams[core][m_pHardwareVoiceNumberArrayCoreEffect[core][voice]].m_StartAddress =
                            chunkStartAddress + offsetIntoChunk;
...
void radSoundVoiceAdpcmPs2::FreeHardwareVoices
    ...
    for (unsigned int voiceNumber = 0; voiceNumber <
                                       m_xRadSoundBufferAdpcmPs2->GetFormat()->GetNumberOfChannels(); voiceNumber++) {
        if (m_pHardwareVoiceNumberArrayDry[voiceNumber] != NULL_VOICE) {
            g_UsedCoreVoiceBitMask[m_DryCore] &= (~(0x01
                    << m_pHardwareVoiceNumberArrayDry[voiceNumber]));
        ...
        for (unsigned int core = 0; core < 2; core++) {
            if (m_pHardwareVoiceNumberArrayCoreEffect[core][voiceNumber] != NULL_VOICE) {
                g_UsedCoreVoiceBitMask[core] &= (~(0x01
                        << m_pHardwareVoiceNumberArrayCoreEffect[core][voiceNumber]));
...
bool radSoundVoiceAdpcmPs2::GetCoreVoices
    ...
    for (unsigned int currentUserVoice = 0; currentUserVoice < numVoices; currentUserVoice++) {
        for (unsigned int checkVoice = 0; checkVoice < 24; checkVoice++) {
            ...
            if (((g_UsedCoreVoiceBitMask[core] & checkVoiceMask) == 0) &&
                ((foundVoiceMask & checkVoiceMask) == 0)) {
...
bool radSoundVoiceAdpcmPs2::GetHardwareVoices
    ...
    rWarningMsg(foundDry == true,
                "radSound: Out of voices trying to capture dry voices, and priority kill failed");
    ...
    if (foundDry == true) {
        for (unsigned int core = 0; core < 2; core++) {
            if (m_AuxModes[core] != radSoundAuxMode_Off) {
                ...
                rWarningMsg(foundWet,
                            "radSound: Out of voices trying to assign effect, and priority kill failed");
            }
        }
    }
...
void radSoundVoiceAdpcmPs2::SetVoiceVolumes(void) {
    if (m_HaveVoices == true) {
        ...
        if (m_xRadSoundHalPositionalGroup != NULL) {
            occl = OcclusionAnalogToMultiplier(
                    m_xRadSoundHalPositionalGroup->m_RadSoundHalPositionalInformation.m_Occlusion);
            obst = ObstructionAnalogToMultiplier(
                    m_xRadSoundHalPositionalGroup->m_RadSoundHalPositionalInformation.m_Obstruction);
        }
        ...
        short leftSpuVolume = ::radSoundVolumeAmplitudeToHardwareAdpcmPs2(
                postFaderLAmp * occl * obst * lSurround);
        short rightSpuVolume = ::radSoundVolumeAmplitudeToHardwareAdpcmPs2(
                postFaderRAmp * occl * obst * rSurround);
                ...
                if (m_AuxModes[core] == radSoundAuxMode_PostFaderDirect) {
                    ...
                    if (m_xRadSoundHalPositionalGroup == NULL) {
                        ...
                        if (m_AuxModes[core] == radSoundAuxMode_PreFader) {
                            leftEffectVolume = ::radSoundVolumeAmplitudeToHardwareAdpcmPs2(
                                    preFaderLAmp * coreAuxGainAmp);
                            rightEffectVolume = ::radSoundVolumeAmplitudeToHardwareAdpcmPs2(
                                    preFaderRAmp * coreAuxGainAmp);
                        } else {
                            rAssert(m_AuxModes[core] == radSoundAuxMode_PostFader);

                            leftEffectVolume = ::radSoundVolumeAmplitudeToHardwareAdpcmPs2(
                                    postFaderLAmp * coreAuxGainAmp);
                            rightEffectVolume = ::radSoundVolumeAmplitudeToHardwareAdpcmPs2(
                                    postFaderRAmp * coreAuxGainAmp);
                        }
                    } else {
                        if (core !=
                            m_xRadSoundHalPositionalGroup->m_RadSoundHalPositionalInformation.m_EnvAuxSend) {
...
void radSoundVoiceAdpcmPs2::SetVoicePitches(void) {
    ...
    if (m_xRadSoundHalPositionalGroup == NULL) {
        ...
        pitchAdjust =
                m_xRadSoundHalPositionalGroup->m_RadSoundHalPositionalInformation.m_PitchAdjust *
                m_Pitch;
    }
...
void radSoundVoiceAdpcmPs2::CaculateSurroundPanAmpFactor(float pan, float span, float *lFactorAmp,
                                                         float *rFactorAmp) {
