    ...
    for (unsigned int currentCore = 0; currentCore < 2; currentCore++) {
        ...
        sceSdRemote
                (
                        ...
                        StartOfSoundMemory + SizeOfSoundMemory - (SizeOfEffect * currentCore) -
                        SizeOfEffect
                );
        ...
        for (unsigned int currentVoice = 0; currentVoice < 24; currentVoice++) {
            ...
            sceSdRemote(1, rSdSetParam, currentCore | (currentVoice << 0x01) | SD_VP_PITCH,
                        0x0000); // 1000
            sceSdRemote(1, rSdSetParam, currentCore | (currentVoice << 0x01) | SD_VP_ADSR1,
                        0x000f); // magic numbers adsr == NULL
            sceSdRemote(1, rSdSetParam, currentCore | (currentVoice << 0x01) | SD_VP_ADSR2,
                        0x1fc0); // taken from sony sample code
...
void radSoundHalSystem::Initialize(const SystemDescription &systemDescription) {
    rAssertMsg(systemDescription.m_NumAuxSends <= 2,
               "radSound: ps2 only supports a maximum of 2 aux sends");

    m_NumAuxSends = systemDescription.m_NumAuxSends;

    ::radMemoryMonitorDeclareAllocation((void *) StartOfSoundMemory,
                                        SizeOfSoundMemory - (m_NumAuxSends * SizeOfEffect),
                                        MM_MAX_CALLSTACK_DEPTH, radMemorySpace_Sound);
    ...
    radSoundPoolObject<radSoundBufferAsyncLoaderAdpcmPs2>::Initialize(GetThisAllocator(),
                                                                      LOAD_POOL_SIZE,
                                                                      LOAD_POOL_SIZE);
    radSoundPoolObject<radSoundHalAdpcmBufferClearer>::Initialize(GetThisAllocator(),
                                                                  CLEAR_POOL_SIZE, CLEAR_POOL_SIZE);
}
...
void radSoundHalSystem::SetOutputMode(radSoundOutputMode mode) {
    if (m_OutputMode != mode) {
        ...
        ::radSoundHalListener::GetInstance()->SetSupportSurround(
                mode == radSoundOutputMode_Surround);
