...
void radSoundHalBufferWin::ClearAsync
    ...
    while (pBufferIter != NULL) {
        ClearDirectSoundBuffer(pBufferIter->m_refIDirectSoundBuffer, startPositionInFrames,
                               numberOfFrames);
...
void radSoundHalBufferWin::CreateDirectSoundBuffer
    ...
    HRESULT hr = radSoundHalSystem::GetInstance()->GetDirectSound()->CreateSoundBuffer(&bd,
                                                                                       ppIDirectSoundBuffer,
                                                                                       NULL);
...
void radSoundHalBufferWin::CreateBufferData(BufferData **ppBufferData,
                                            IDirectSoundBuffer *pIDirectSoundBuffer) {
    ...
    if (numFreePoolAllocations == 0) {
        ...
        rAssertMsg(*ppBufferData != NULL,
                   "radSound: Buffer data memory pool empty.  Increase max allocations with IRadSoundHalSystem::InitializeMemory()");
    } else {
...
void radSoundHalBufferWin::InitializeBufferDataPool(unsigned int maxCount,
                                                    radMemoryAllocator allocator) {
    rAssert(s_refIRadMemoryPool == NULL);

    ::radMemoryCreatePool(&s_refIRadMemoryPool, sizeof(BufferData), maxCount, 0, false, None,
                          allocator, "radSoundHalBufferDataPool");
    rAssert(s_refIRadMemoryPool != NULL);
}
