    ...
    if (m_refIRadFile->IsOpen() == true) {
        ...
        if (m_refIRadSoundHalAudioFormat == NULL) {
            ...
            m_StateInfo.m_ReadingHeaderInfo.m_pRadSoundHalFileHeader =
                    static_cast<radSoundHalFileHeader *>(::radMemoryAllocAligned(
                            RADMEMORY_ALLOC_TEMP,
                            ::radMemoryRoundUp(sizeof(radSoundHalFileHeader),
                                               radFileMaxSectorSize),
...
void radSoundRsdFileDataSource::_StateReadingHeader(void) {
    ...
    ::radMemoryFreeAligned(RADMEMORY_ALLOC_TEMP,
                           m_StateInfo.m_ReadingHeaderInfo.m_pRadSoundHalFileHeader);
...
void radSoundRsdFileDataSource::InitFile(void) {
    ...
    rAssert(m_refIRadSoundHalAudioFormat->BytesToFrames(initialPlaybackOffsetInBytes) <
            m_FramesLeftInFile);
    ...
    unsigned int fileDataSize =
            m_refIRadFile->GetSize() - RSD_FILE_DATA_OFFSET - initialPlaybackOffsetInBytes;
...
void radSoundRsdFileDataSource::_StateReadingData(void) {
    ...
    xIRadSoundHalDataSourceCallback->OnDataSourceFramesLoaded(
            m_StateInfo.m_ReadingDataInfo.m_ReadSizeInFrames);
}
