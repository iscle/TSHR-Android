...
void radSoundHalVoiceWin::SetBuffer(IRadSoundHalBuffer *pIRadSoundHalBuffer) {
    ...
    if (m_pBufferData != NULL) {
        ...
        m_xRadSoundHalBufferWin->FreeBufferData(m_xRadSoundHalPositionalGroup != NULL,
                                                m_pBufferData);
        m_pBufferData = NULL;
    }
    ...
    if (pIRadSoundHalBuffer != NULL) {
        ...
        m_xRadSoundHalBufferWin->GetBufferData(m_xRadSoundHalPositionalGroup != NULL,
                                               &m_pBufferData);
...
void radSoundHalVoiceWin::Play() {
    if (m_xIDirectSoundBuffer != NULL && IsHardwarePlaying() == false) {
        ...
        if (m_xRadSoundHalBufferWin->IsStreaming() == false) {
            flags |= (m_xIDirectSound3dBuffer != NULL) ? DSBPLAY_TERMINATEBY_DISTANCE
        ...
        HRESULT hr = m_xIDirectSoundBuffer->Play(
                0,
                m_xRadSoundHalBufferWin->IsStreaming() ? 0
                                                       : m_Priority, // Must be 0 for buffers without DSBCAPS_LOCDEFER defined during creation
                flags);
        ...
        if (m_xRadSoundHalBufferWin->IsStreaming() == false &&
            (status & DSBSTATUS_LOCHARDWARE) == 0) {
...
bool radSoundHalVoiceWin::IsHardwarePlaying(void) {
    if (m_xIDirectSoundBuffer != NULL) {
        ...
        rWarningMsg((status & DSBSTATUS_BUFFERLOST) == 0,
                    "radSoundHalVoiceWin: IDirectSoundBuffer lost");
...
// rReleasePrintf("%d\n", ::radSoundVolumeDbToHardwareWin(::radSoundVolumeAnalogToDb(/* 0.0f */ volume * ::radSoundVolumeAmplitudeToAnalog(rollOffFactor))));
        ...
        m_xIDirectSoundBuffer->SetVolume(::radSoundVolumeDbToHardwareWin(
                ::radSoundVolumeAnalogToDb(/* 0.0f */
                        volume * ::radSoundVolumeAmplitudeToAnalog(rollOffFactor))));
    }
}
...
void radSoundHalVoiceWin::SetPitchInternal(void) {
    if (m_xIDirectSoundBuffer != NULL) {
        if (m_xIRadSoundHalAudioFormat != NULL) {
            HRESULT hr = m_xIDirectSoundBuffer->SetFrequency(
                    ::radSoundPercentageToHardwarePitchWin(m_Pitch,
                                                           m_xIRadSoundHalAudioFormat->GetSampleRate()));
...
void radSoundHalVoiceWin::SetPanInternal(void) {
    if (m_xIDirectSoundBuffer != NULL) {
        ...
        HRESULT hr = m_xIDirectSoundBuffer->QueryInterface(IID_IDirectSound3DBuffer,
                                                           (void **) &xIDirectSound3DBuffer);
