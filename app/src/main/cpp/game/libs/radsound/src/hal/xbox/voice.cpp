...
void radSoundVoiceXBox::ReleaseVoiceToPool(ref <IDirectSoundBuffer> &refIDsb) {
    if (refIDsb != NULL) {
        ...
        if (dwStatus & DSBSTATUS_PLAYING) {
            ...
            radSoundReleasingVoiceNode *pRsrvn = new(
                    "radSoundReleasingVoiceNode")radSoundReleasingVoiceNode();
    ...
    if (m_refIDirectSoundBuffer != NULL) {
        if (m_Playing == false) {
            ...
            if (SUCCEEDED(hr)) {
                ...
                rTuneString(
                        "radSoundVoiceXBox: IDirectSoundBuffer::Play() failed.  You are probably out of voices.\n");
            }
        }
    }
}
...
void radSoundVoiceXBox::Stop
    ...
    if (m_Playing == true) {
        ...
        hr = m_refIDirectSoundBuffer->GetCurrentPosition((LPDWORD) & m_LastPlaybackPositionInBytes,
                                                         NULL);
...
void radSoundVoiceXBox::SetVolume
    ...
    if (m_Volume != volume) {
        radSoundVerifyChangeThreshold(m_Playing, "Volume", m_Volume, volume,
                                      radSoundVolumeChangeThreshold);
...
void radSoundVoiceXBox::SetTrim(float trim) {
    ...
    if (m_Trim != trim) {
        radSoundVerifyChangeThreshold(m_Playing, "Trim", m_Trim, trim,
                                      radSoundVolumeChangeThreshold);
    ...
    if (m_refIDirectSoundBuffer != NULL) {
        if (m_refIRadSoundHalAudioFormat != NULL) {
            HRESULT hr = m_refIDirectSoundBuffer->SetFrequency(
                    ::radSoundPercentageToHardwarePitchXBox(m_Pitch,
                                                            (unsigned int) m_refIRadSoundHalAudioFormat->GetSampleRate()));
...
/* virtual */ void radSoundVoiceXBox::SetAuxMode(unsigned int aux, radSoundAuxMode mode) {
    ...
    rAssertMsg(m_refRadSoundHalPositionalGroup == NULL,
...
/* virtual */ void radSoundVoiceXBox::SetAuxGain(unsigned int aux, float gain) {
    ...
    if (m_refOutputSendState[aux].m_Gain != gain) {
        radSoundVerifyChangeThreshold(m_Playing, "AuxGain", m_refOutputSendState[aux].m_Gain, gain,
                                      radSoundVolumeChangeThreshold);
...
/* virtual */ void radSoundVoiceXBox::SetPositionalGroup
    ...
    radSoundHalPositionalGroup *pRadSoundHalPositionalGroup
            = dynamic_cast<radSoundHalPositionalGroup *>(pIRadSoundHalPositionalGroup);
...
void
radSoundVoiceXBox::OnSetAuxEffect(unsigned int auxNumber, IRadSoundHalEffect *pIRadSoundEffect) {
    if (m_refRadSoundHalPositionalGroup == NULL) {
        rAssert(auxNumber < RSDVOICE_OUTPUT_MAX);

        if (pIRadSoundEffect != NULL &&
            m_refOutputSendState[auxNumber].m_Mode != radSoundAuxMode_Off) {
...
void radSoundVoiceXBox::CreateDirectSoundBuffer(void) {
    ...
    if (m_refIRadSoundHalAudioFormat->GetEncoding() == IRadSoundHalAudioFormat::PCM) {
        ...
        CreateImaAdpcmFormat(m_refIRadSoundHalAudioFormat->GetNumberOfChannels(),
                             (unsigned int) m_refIRadSoundHalAudioFormat->GetSampleRate(),
                             XBOX_ADPCM_SAMPLES_PER_BLOCK, &wfx);
    } else {
        rAssert(0);
    }
    ...
    hr = m_refIDirectSoundBuffer->SetBufferData
            (
                    ...
                    m_refRadSoundBufferXBox->GetFormat()->FramesToBytes(
                            m_refRadSoundBufferXBox->GetSizeInFrames())
            );
...
void radSoundVoiceXBox::PopulateDsMixBins(DSMIXBINS *pDsMixBins,
                                          DSMIXBINVOLUMEPAIR *pDsMixBinVolumePairs) {
        ...
        if (m_refOutputSendState[i].m_Mode == radSoundAuxMode_PreFader) {
            ...
            if (DSMIXBIN_FXSEND_13 != m_refOutputSendState[i].m_pDsMixBin[0]) {
                pDsMixBinVolumePairs[bin].lVolume = radSoundVolumeDbToHardwareXBox(
                        radSoundVolumeAnalogToDb(
                                m_refOutputSendState[i].m_Gain * m_Trim * m_MuteFactor * panL));
            ...
            if (DSMIXBIN_FXSEND_13 != m_refOutputSendState[i].m_pDsMixBin[1] &&
                ...
                pDsMixBinVolumePairs[bin].lVolume = radSoundVolumeDbToHardwareXBox(
                        radSoundVolumeAnalogToDb(
                                m_refOutputSendState[i].m_Gain * m_Trim * m_MuteFactor * panR));
            ...
            if (DSMIXBIN_FXSEND_13 != m_refOutputSendState[i].m_pDsMixBin[0]) {
                pDsMixBinVolumePairs[bin].lVolume = radSoundVolumeDbToHardwareXBox(
                        radSoundVolumeAnalogToDb(
                                m_refOutputSendState[i].m_Gain * m_Volume * m_Trim * m_MuteFactor *
                                panL));
                pDsMixBinVolumePairs[bin].dwMixBin = m_refOutputSendState[i].m_pDsMixBin[0];
