...
void radSoundHalPositionalGroup::UpdatePositionalSettings(radSoundVector *pListenerPosition,
                                                          float listenerRolloffFactor) {
    // Calculate the current volume rolloff factor

    float distToListener = pListenerPosition->GetDistanceBetween(
            *((radSoundVector * ) & (m_Ds3dBuffer.vPosition)));
    m_VolumeRolloffFactor = ::radSoundHalCalcualteRollOff(m_Ds3dBuffer.flMinDistance,
                                                          m_Ds3dBuffer.flMaxDistance,
                                                          distToListener);
