...
unsigned int radSoundVoiceGcn::GetSilentBufferStartInLogicalUnits() {
    ...
    if (
            ...
            (m_xRadSoundBufferGcn->GetFormat()->GetEncoding() ==
    ...
    } else if (m_xRadSoundBufferGcn->GetFormat()->GetEncoding() ==
               IRadSoundHalAudioFormat::GCNADPCM) {
        silentBufferInSamples =
                (unsigned int) s_xIRadMemoryObject_SilentBuffer->GetMemoryAddress();
    ...
    if (m_apVoiceParamBlock[0] != NULL) {
        ...
        if (m_xRadSoundBufferGcn->GetFormat()->GetNumberOfChannels() == 1) {
            ...
            if (calculateSurround) {
                mix[0].vS = atoh(positionalAnalogSurroundAmp * postFaderMonoAmp * obstructionMult *
                                 occlusionMult);
            }
            ...
            if (m_xRadSoundHalPositionalGroup != NULL) {
                ...
                if (envAux == 0) {
                    ...
                    mix[0].vAuxAL = atoh(positionalAnalogAmp * postFaderLAmp *
                                         ::radSoundVolumeAnalogToAmplitude(
                                                 s_MasterAuxGains[envAux] * occlusionMult));
                    mix[0].vAuxAR = atoh(positionalAnalogAmp * postFaderRAmp *
                                         ::radSoundVolumeAnalogToAmplitude(
                                                 s_MasterAuxGains[envAux] * occlusionMult));
                    ...
                    mix[0].vAuxBL = atoh(positionalAnalogAmp * postFaderLAmp *
                                         ::radSoundVolumeAnalogToAmplitude(
                                                 s_MasterAuxGains[envAux] * occlusionMult));
                    mix[0].vAuxBR = atoh(positionalAnalogAmp * postFaderRAmp *
                                         ::radSoundVolumeAnalogToAmplitude(
                                                 s_MasterAuxGains[envAux] * occlusionMult));
                ...
                if
                    ...
                    if (m_AuxModes[0] == radSoundAuxMode_PreFader) {
                        mix[0].vAuxAL = atoh(positionalAnalogAmp * preFaderLAmp *
                                             ::radSoundVolumeAnalogToAmplitude(
                                                     s_MasterAuxGains[0] * m_AuxGains[0]));
                        mix[0].vAuxAR = atoh(positionalAnalogAmp * preFaderRAmp *
                                             ::radSoundVolumeAnalogToAmplitude(
                                                     s_MasterAuxGains[0] * m_AuxGains[0]));
                ...
                if
                    ...
                    if (m_AuxModes[1] == radSoundAuxMode_PreFader) {
                        mix[0].vAuxBL = atoh(positionalAnalogAmp * preFaderLAmp *
                                             ::radSoundVolumeAnalogToAmplitude(
                                                     s_MasterAuxGains[1] * m_AuxGains[1]));
                        mix[0].vAuxBR = atoh(positionalAnalogAmp * preFaderRAmp *
                                             ::radSoundVolumeAnalogToAmplitude(
                                                     s_MasterAuxGains[1] * m_AuxGains[1]));

                        if (calculateSurround) {
                            mix[0].vAuxBS = atoh(positionalAnalogSurroundAmp * preFaderMonoAmp *
                                                 ::radSoundVolumeAnalogToAmplitude(
                                                         s_MasterAuxGains[1] * m_AuxGains[1]));
                        }
            ...
            if
                ...
                if (m_AuxModes[0] == radSoundAuxMode_PreFader) {
                    ...
                    mix[1].vAuxAL = 0;
                    mix[1].vAuxAR = atoh(positionalAnalogAmp * preFaderRAmp *
                                         ::radSoundVolumeAnalogToAmplitude(
                                                 s_MasterAuxGains[0] * m_AuxGains[0]));

                    if (calculateSurround) {
                        mix[0].vAuxAS = atoh(positionalAnalogSurroundAmp * preFaderMonoAmp *
                                             ::radSoundVolumeAnalogToAmplitude(
                                                     s_MasterAuxGains[0] * m_AuxGains[0]));
                    ...
                    mix[0].vAuxAR = 0;
                    mix[1].vAuxAL = 0;
                    mix[1].vAuxAR = atoh(positionalAnalogAmp * postFaderRAmp *
                                         ::radSoundVolumeAnalogToAmplitude(
                                                 s_MasterAuxGains[0] * m_AuxGains[0]));

                    if (calculateSurround) {
                        mix[0].vAuxAS = atoh(positionalAnalogSurroundAmp * postFaderMonoAmp *
                                             ::radSoundVolumeAnalogToAmplitude(
                                                     s_MasterAuxGains[0] * m_AuxGains[0]));
                        mix[1].vAuxAS = mix[0].vAuxAS;
                    }
            ...
            if
                ...
                if (m_AuxModes[1] == radSoundAuxMode_PreFader) {
                    ...
                    mix[1].vAuxBL = 0;
                    mix[1].vAuxBR = atoh(positionalAnalogAmp * preFaderRAmp *
                                         ::radSoundVolumeAnalogToAmplitude(
                                                 s_MasterAuxGains[1] * m_AuxGains[1]));
                    ...
                    mix[1].vAuxBL = 0;
                    mix[1].vAuxBR = atoh(positionalAnalogAmp * postFaderRAmp *
                                         ::radSoundVolumeAnalogToAmplitude(
                                                 s_MasterAuxGains[1] * m_AuxGains[1]));

                    if (calculateSurround) {
                        mix[0].vAuxBS = atoh(positionalAnalogSurroundAmp * postFaderMonoAmp *
                                             ::radSoundVolumeAnalogToAmplitude(
                                                     s_MasterAuxGains[1] * m_AuxGains[1]));
                        mix[1].vAuxBS = mix[0].vAuxBS;
                    }
...
void radSoundVoiceGcn::SetVoicePosition_Di(void) {
    rAssert(m_LastKnownSamplePosition < m_xRadSoundBufferGcn->GetFormat()->FramesToSamples(
    ...
    for (unsigned int channel = 0; channel < totalChannelCount; channel++) {
        ...
        unsigned int currentAddress = m_xRadSoundBufferGcn->GetAramSampleStartInLogicalUnits(
                channel, m_LastKnownSamplePosition);
...
/* virtual */ void radSoundVoiceGcn::SetPlaybackPositionInSamples(unsigned int position) {
    if (m_xRadSoundBufferGcn != NULL) {
        rAssert(position < m_xRadSoundBufferGcn->GetFormat()->FramesToSamples(
                m_xRadSoundBufferGcn->GetSizeInFrames()));
    }
...
/* virtual */ void radSoundVoiceGcn::GetVoicePosition_Di(void) {
    ...
    if (HWPos < aramStartPos) {
        ...
        // that information
        //
        m_LastKnownSamplePosition = m_xRadSoundBufferGcn->GetFormat()->FramesToSamples(
                m_xRadSoundBufferGcn->GetSizeInFrames()) - 1;
    } else {
...
/* virtual */ void radSoundVoiceGcn::SetPositionalGroup(
    ...
    if (m_xRadSoundHalPositionalGroup !=
        static_cast<radSoundHalPositionalGroup *>(pIRadSoundHalPositionalGroup)) {
        if (m_xRadSoundHalPositionalGroup != NULL) {
            m_xRadSoundHalPositionalGroup->RemovePositionalEntity(this);
        }

        m_xRadSoundHalPositionalGroup = static_cast<radSoundHalPositionalGroup *>(pIRadSoundHalPositionalGroup);
...
bool radSoundVoiceGcn::CaptureVoices_Di(void) {
    ...
    float ratio = (m_Pitch * (float) m_xRadSoundBufferGcn->GetFormat()->GetSampleRate()) /
                  s_HardwarePlaybackRate;
    ...
    for (unsigned int channel = 0;
         channel < m_xRadSoundBufferGcn->GetFormat()->GetNumberOfChannels(); channel++) {
