...
void radSoundBufferAsyncLoaderGcn::OnDataSourceFramesLoaded(unsigned int framesActuallyRead) {
    if (m_xRadSoundBufferGcn->GetFormat()->FramesToBytes(framesActuallyRead) < m_ChunkBytesToLoad) {
        void *pMissedBuffer = ((char *) s_pMainMemoryTransferBuffer) +
                              m_xRadSoundBufferGcn->GetFormat()->FramesToBytes(framesActuallyRead);
        unsigned int missedBufferSize = m_ChunkBytesToLoad -
                                        m_xRadSoundBufferGcn->GetFormat()->FramesToBytes(
                                                framesActuallyRead);
...
void radSoundBufferAsyncLoaderGcn::UnInterleaveData(void) {
    ...
    unsigned int framesToUnInterleave = m_xRadSoundBufferGcn->GetFormat()->BytesToFrames(
            m_ChunkBytesToLoad);
    ...
    if (m_xRadSoundBufferGcn->GetFormat()->GetEncoding() == IRadSoundHalAudioFormat::PCM) {
        for (unsigned int channel = 0; channel < numberOfChannels; channel++) {
            signed short *pTransferBuffer =
                    ((signed short *) s_pMainMemoryTransferBuffer) + channel;
            ...
            for (unsigned int i = 0; i < framesToUnInterleave; i++) {
                *pInterleaveBuffer = __lhbrx(pTransferBuffer,
                                             0); // gamecube has hardware endian support
                pInterleaveBuffer++;
        ...
        for (unsigned int channel = 0; channel < numberOfChannels; channel++) {
            char *pTransferBuffer =
                    ((char *) s_pMainMemoryTransferBuffer) + channel * GCN_ADPCM_BYTES_PER_FRAME;
