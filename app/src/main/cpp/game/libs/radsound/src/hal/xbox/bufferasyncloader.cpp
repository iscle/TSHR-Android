...
/* virtual */ void
radSoundBufferAsyncLoaderXBox::OnDataSourceFramesLoaded(unsigned int actualFramesRead) {
    if (!m_Cancelled) {
        //
        ...
        if (actualFramesRead < m_FramesToLoad) {
            unsigned int offsetInBytes = m_xIRadSoundHalDataSource->GetFormat()->FramesToBytes(
                    m_BufferOffset + actualFramesRead);
            unsigned int sizeInBytes = m_xIRadSoundHalDataSource->GetFormat()->FramesToBytes(
                    m_FramesToLoad - actualFramesRead);
            unsigned char fillChar = (m_xIRadSoundHalDataSource->GetFormat()->GetBitResolution() ==
                                      8) ? 128 : 0;
