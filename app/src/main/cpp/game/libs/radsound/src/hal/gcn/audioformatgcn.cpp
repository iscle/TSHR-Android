...
void radSoundHalAudioFormatGcn::Initialize
    ...
    if (pRadSoundHalAdpcmInfoGcn != NULL) {
        m_pRadSoundHalAdpcmInfoGcn = (radSoundHalAdpcmInfoGcn * )
        ::radMemoryAlloc(
                GetThisAllocator(), sizeof(radSoundHalAdpcmInfoGcn) *
                                    m_pIRadSoundHalAudioFormat->GetNumberOfChannels());

        ::memcpy(
                m_pRadSoundHalAdpcmInfoGcn,
                pRadSoundHalAdpcmInfoGcn,
                sizeof(radSoundHalAdpcmInfoGcn) *
                m_pIRadSoundHalAudioFormat->GetNumberOfChannels());
    }
