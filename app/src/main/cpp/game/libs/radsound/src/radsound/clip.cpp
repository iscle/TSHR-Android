...
void radSoundClip::Initialize
    ...
    rAssertMsg(pIdentifier != NULL,
               "You MUST name all of your sound clips so we can track memory usage");
...
/* virtual */ void radSoundClip::Update(unsigned int elapsedTime) {
    ...
    if (m_xIRadSoundHalDataSource->GetState() == IRadSoundHalDataSource::Initialized) {
        ...
        if (m_xIRadSoundHalDataSource->GetRemainingFrames() >= MIN_FRAMES) {
            ...
            initialBufferMemorySizeInBytes = ::radSoundHalBufferCalculateMemorySize(
                    IRadSoundHalAudioFormat::Bytes,
                    initialBufferSizeInFrames, IRadSoundHalAudioFormat::Frames,
            ...
            initialBufferSizeInFrames = xIRadSoundHalAudioFormat->BytesToFrames(
        ...
        if (xIRadMemoryObject != NULL) {
            ...
            if (m_xIRadSoundHalDataSource->GetRemainingFrames() >= MIN_FRAMES) {
                ...
                m_xIRadSoundHalBuffer->ClearAsync(0, m_xIRadSoundHalBuffer->GetSizeInFrames(),
                                                  this);
            }
        } else {
            char buf[128];
    ...
    while (pSearch != NULL) {
        ...
        if (pIRadSoundHalBuffer != NULL) {
            sprintf(
                    buf,
                    "Size: [%d] File: [%s]\n",
                    pIRadSoundHalBuffer->GetFormat()->FramesToBytes(
                            pIRadSoundHalBuffer->GetSizeInFrames()),
                    "Unknown");
