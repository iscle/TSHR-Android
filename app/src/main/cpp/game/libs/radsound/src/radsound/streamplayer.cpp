...
void radSoundStreamPlayer::InitializeAsync
    ...
    rAssertMsg(pIdentifier != NULL,
               "You MUST name all of your stream players so we can track memory usage");
...
void radSoundStreamPlayer::AllocateResources(IRadSoundHalAudioFormat *pIRadSoundHalAudioFormat) {
    ...
    if (xIRadMemoryObject != NULL) {
        ref <IRadSoundHalBuffer> xIRadSoundHalBuffer = ::radSoundHalBufferCreate(
                GetThisAllocator());
        xIRadSoundHalBuffer->Initialize(pIRadSoundHalAudioFormat, xIRadMemoryObject, sizeInFrames,
                                        true, true);
...
bool radSoundStreamPlayer::ServicePlay(void) {
    ...
    if (playbackPositionInSamples >= m_LastPlaybackPositionInSamples) {
        ...
        samplesPlayedThisFrame =
                bufferSizeInSamples - (m_LastPlaybackPositionInSamples - playbackPositionInSamples);
    }
...
#ifndef RAD_DEBUG
                rTunePrintf(
                        "radsound: TRC Violation: Streamer: [%s] is skipping (loading latency).\n",
                        m_xIRadString_Name->GetChars());
#endif
            }
        } else {
...
void radSoundStreamPlayer::ServiceLoad(void) {
    ...
    unsigned int frameSizeInSamples =
            xIRadSoundHalAudioFormat->BytesToSamples(
                    xIRadSoundHalAudioFormat->GetFrameSizeInBytes());
    ...
    if (m_OutstandingLoadSize == 0 && m_OutstandingClearSize == 0) {
        unsigned int lowWaterMarkInFrames =
                radSoundFloatToUInt(
                        radSoundUIntToFloat(bufferSizeInFrames) * (1.0f - m_float_LowWaterMark));
        ...
        if (currentPlaybackPositionInFrames == m_WritePositionInFrames) {
            ...
            if (currentPlaybackPositionInFrames > m_WritePositionInFrames) {
                ...
                framesNeeded = bufferSizeInFrames -
                               (m_WritePositionInFrames - currentPlaybackPositionInFrames);
            }
        }

        framesNeeded = ::radMemoryRoundDown(framesNeeded,
                                            m_xIRadSoundHalVoice->GetBuffer()->GetMinTransferSize(
                                                    IRadSoundHalAudioFormat::Frames));
        ...
        if (framesNeeded >= lowWaterMarkInFrames || m_EndFrameCounter != 0xFFFFFFFF) {
            ...
            if ((m_WritePositionInFrames + framesNeeded) > bufferSizeInFrames) {
                ...
                framesNeeded = framesNeeded -
                               ((m_WritePositionInFrames + framesNeeded) % bufferSizeInFrames);
            } else {
...
IRadSoundHalAudioFormat *radSoundStreamPlayer::GetFormat(void) {
    rAssertMsg(m_xIRadSoundHalVoice->GetBuffer() != NULL,
               "Can't get format of streamer until it is initilized");
