...
radSoundHalListener::radSoundHalListener
    ...
    ::radSoundHalCreateRollOffTable(STD_ROLL_OFF_TABLE_MAX_DIST_VOL, m_pRollOffTable,
    ...
    if (radSoundHalSystem::GetInstance()->GetNumAuxSends() > 0) {
        ...
        if (refIDirectSoundBuffer != NULL) {
            hr = refIDirectSoundBuffer->QueryInterface(IID_IKsPropertySet,
                                                       (void **) &m_xIKsPropertySet);
...
void radSoundHalListener::UpdatePositionalSettings
    ...
    while (pRadSoundHalPositionalGroup != NULL) {
        pRadSoundHalPositionalGroup->UpdatePositionalSettings(
                (radSoundVector * ) & m_Ds3dListener.vPosition, m_RolloffFactor);
