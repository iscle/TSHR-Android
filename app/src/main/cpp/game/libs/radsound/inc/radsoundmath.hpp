...
#if !defined(RAD_GAMECUBE) && !defined(RAD_PS2) && !defined(RAD_XBOX) && !defined(RAD_WIN32) && !defined(RAD_ANDROID)
#error 'FTech requires definition of RAD_GAMECUBE, RAD_PS2, RAD_XBOX, RAD_WIN32, or RAD_ANDROID'
#endif
...
inline void
radSoundVerifyChangeThreshold(bool doCheck, const char *pValueName, float oldVal, float newVal,
                              float threshold) {
#ifndef RAD_RELEASE
    ...
    if (doCheck) {
        if (fabsf(oldVal - newVal) > (radSoundVolumeChangeThreshold + 0.0001f)) {
            rTunePrintf(
                    "radsound: TRC Violation: delta [%s] change exceeded [%d%%] was:[%d%%] now:[%d%%]\n",
...
inline void
radSoundLeftRightVolumeToPan(float *pPan, float *pVolume, float leftVol, float rightVol) {
