...
//----------------------------------------------------------------------
    ...
    public:
        ...
        PURE3D_ANIMATION *GetAnimation() const { return m_Animation; }

        tUID GetUID() const { return m_Animation->GetUID(); }

        const char *GetName() const { return m_Animation->GetName(); }

        bool IsCyclic() const { return m_Animation->GetCyclic(); }

        float GetFramesPerSecond() const { return m_Animation->GetSpeed(); }

        float GetFrameCount() const { return m_Animation->GetNumFrames(); }
