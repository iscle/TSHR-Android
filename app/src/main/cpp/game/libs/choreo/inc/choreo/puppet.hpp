...
namespace choreo {
    ...
    public:
        ...
        // rig accessors
        Rig *GetRig() const { return m_Bank->GetRig(); }

        // skeleton accessors
        tSkeleton *GetSkeleton() const { return m_Bank->GetRig()->GetSkeleton(); }

        // position & orientation
        const poser::Transform &GetRootTransform() const { return m_Engine->GetRootTransform(); }
        ...
        const poser::Transform &
        GetPrevRootTransform() const { return m_Engine->GetPrevRootTransform(); }

        const rmt::Vector &GetPrevPosition() const { return m_Engine->GetPrevPosition(); }

        const rmt::Vector &GetPosition() const { return m_Engine->GetPosition(); }

        void SetPosition(const rmt::Vector &pos) { m_Engine->SetPosition(pos); }

        float GetPrevOrientation() const { return m_Engine->GetPrevOrientation(); }

        float GetOrientation() const { return m_Engine->GetOrientation(); }

        void SetOrientation(float radians) { m_Engine->SetOrientation(radians); }

        rmt::Vector GetPrevFacingVector() const { return m_Engine->GetPrevFacingVector(); }

        rmt::Vector GetFacingVector() const { return m_Engine->GetFacingVector(); }
        ...
        const rmt::Vector &GetGroundPoint() const { return m_Engine->GetGroundPoint(); }
        ...
        // velocity & turning
        const rmt::Vector &GetVelocity() const { return m_Engine->GetVelocity(); }

        float GetTurningVelocity() const { return m_Engine->GetTurningVelocity(); }

        // LOD partition
        LODPartition *GetLODPartition() const { return m_Engine->GetLODPartition(); }
        ...
        // foot positioning
        int GetLegCount() const { return m_Engine->GetLegCount(); }

        bool IsLegIKEnabled(int leg) const { return m_Engine->IsLegIKEnabled(leg); }

        void SetIsLegIKEnabled(int leg, bool isEnabled) const {
            m_Engine->SetIsLegIKEnabled(leg, isEnabled);
        }

        bool IsFootPlanted(int foot) const { return m_Engine->IsFootPlanted(foot); }

        rmt::Vector GetFootPosition(int foot) const { return m_Engine->GetFootPosition(foot); }
        ...
        // replay accessors
        bool IsReplayEnabled() const { return m_Engine->IsReplayEnabled(); }
        ...
        bool RemoveAttachedProp(tPose *p3dPose) { return m_Engine->RemoveAttachedProp(p3dPose); }

        bool HasAttachedProp(tPose *p3dPose) const { return m_Engine->HasAttachedProp(p3dPose); }
        ...
        bool IsDriverPlaying(PuppetDriver *driver) const { return !IsDriverFinished(driver); }
        ...
        int GetDriverCount() const { return m_DriverCount; }
