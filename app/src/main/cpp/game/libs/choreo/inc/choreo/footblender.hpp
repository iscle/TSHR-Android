...
namespace choreo {
    ...
    public:
        ...
        RigLeg *GetRigLeg() const { return m_RigLeg; }

        bool IsIKEnabled() const { return m_IsIKEnabled; }

        void SetIsIKEnabled(bool isIKEnabled) { m_IsIKEnabled = isIKEnabled; }

        bool IsFootPlanted() const { return m_IsFootPlanted; }

        const poser::Transform &GetEffector() const { return m_Effector; }

        poser::Transform &GetEffector() { return m_Effector; }

        const rmt::Vector &GetPoleVector() const { return m_PoleVector; }

        rmt::Vector &GetPoleVector() { return m_PoleVector; }

        float GetGroundFixupWeight() const { return m_GroundFixupWeight; }

        bool Push(FootBlendDriver *footDriver);

        virtual int GetMinimumJointIndex() const { return m_MinJointIndex; }
