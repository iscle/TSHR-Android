...
//---------------------------------------------------------------------------
    ...
    public:
        ...
        PURE3D_ANIMATION *GetP3DAnimation() const { return m_Animation.GetAnimation(); }
        ...
        GroundFixupEnum GetGroundFixupEnum() const { return m_GroundFixupEnum; }

        float GetGroundFixup(float frame) const;

        bool IsCyclic() const { return m_IsCyclic; }

        void SetIsCyclic(bool isCyclic) { m_IsCyclic = isCyclic; }

        float GetFramesPerSecond() const { return m_Animation.GetFramesPerSecond(); }

        float GetFrameCount() const { return m_Animation.GetFrameCount(); }
        ...
        void SetStartFrame(float startFrame) { m_StartFrame = startFrame; }

        float GetEndFrame() const { return m_EndFrame; }

        void SetEndFrame(float endFrame) { m_EndFrame = endFrame; }

        static float MakeValidFrame(float frame, float startFrame, float endFrame, bool cyclic);

        float GetRootBlendInTime() const { return m_RootBlendInTime; }

        void SetRootBlendInTime(float rootBlendInTime) { m_RootBlendInTime = rootBlendInTime; }

        float GetRootBlendOutTime() const { return m_RootBlendOutTime; }

        void SetRootBlendOutTime(float rootBlendOutTime) { m_RootBlendOutTime = rootBlendOutTime; }

        float GetBlendInTime() const { return m_BlendInTime; }

        void SetBlendInTime(float blendInTime) { m_BlendInTime = blendInTime; }

        float GetBlendOutTime() const { return m_BlendOutTime; }

        void SetBlendOutTime(float blendOutTime) { m_BlendOutTime = blendOutTime; }
        ...
        bool HasGroupRotation(tUID group) const { return m_Animation.HasGroupRotation(group); }
...
//---------------------------------------------------------------------------
    ...
    public:
        ...
        bool GetHoldEndFrame() const { return m_HoldEndFrame; }

        void SetHoldEndFrame(bool holdEndFrame) { m_HoldEndFrame = holdEndFrame; }
