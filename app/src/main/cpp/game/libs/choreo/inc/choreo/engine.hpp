...
namespace choreo {
    ...
    public:
        ...
        // pose accessors
        poser::Pose *GetPose() { return m_PoseEngine->GetPose(); }

        const poser::Pose *GetPose() const { return m_PoseEngine->GetPose(); }

        tPose *GetP3DPose() const { return m_PoseEngine->GetP3DPose(); }

        // rig accessors
        Rig *GetRig() const { return m_Rig; }

        // component accessors
        poser::PoseEngine *GetPoseEngine() const { return m_PoseEngine; }

        RootBlender *GetRootBlender() const { return m_RootBlender; }

        JointBlender *GetJointBlender() const { return m_JointBlender; }

        ReplayBlender *GetReplayBlender() const { return m_ReplayBlender; }
        ...
        // replay accessors
        bool IsReplayEnabled() const { return (m_ReplayBlender != 0); }

        // leg blender accessors
        FootBlender *GetFootBlender(int i) const;

        FootPlanter *GetFootPlanter() const { return m_FootPlanter; }

        // position & orientation
        const poser::Transform &
        GetRootTransform() const { return m_RootBlender->GetRootTransform(); }
        ...
        const poser::Transform &
        GetPrevRootTransform() const { return m_RootBlender->GetPrevRootTransform(); }

        const rmt::Vector &GetPrevPosition() const { return m_RootBlender->GetPrevPosition(); }

        const rmt::Vector &GetPosition() const { return m_RootBlender->GetPosition(); }

        void SetPosition(const rmt::Vector &pos) { m_RootBlender->SetPosition(pos); }

        float GetPrevOrientation() const { return m_RootBlender->GetPrevOrientation(); }

        float GetOrientation() const { return m_RootBlender->GetOrientation(); }

        void SetOrientation(float radians) { m_RootBlender->SetOrientation(radians); }

        rmt::Vector GetPrevFacingVector() const { return m_RootBlender->GetPrevFacingVector(); }

        rmt::Vector GetFacingVector() const { return m_RootBlender->GetFacingVector(); }
        ...
        const rmt::Vector &GetGroundPoint() const { return m_RootBlender->GetGroundPoint(); }
        ...
        // velocity & turning
        const rmt::Vector &GetVelocity() const { return m_RootBlender->GetVelocity(); }

        float GetTurningVelocity() const { return m_RootBlender->GetTurningVelocity(); }
        ...
        // pose update
        int GetPassCount() const { return m_PoseEngine->GetPassCount(); }
