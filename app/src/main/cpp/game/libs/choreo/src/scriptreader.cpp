    ...
    void ScriptReader::LoadDefault() {
        ...
        switch (m_TokenType) {
            case TOKEN_FLOAT: {
                ...
                if (index >= 0) {
                    SetErrorFormat(
                            "Inconsistent type for default \"%s\"; see previous declaration on line %d.",
    ...
    void ScriptReader::LoadUntilStruct() {
        ...
        if (!m_IsInScope) {
            switch (m_TokenType) {
                ...
                default:
                    SetErrorFormat("Unexpected token \"%s\", expected opening brace or semicolon.",
                                   m_Token);
                    return;
            }
        }
...
//----------------------------------------------------------------------
    ...
    void ScriptReader::RequiredParamFloat(const char *identifier,
        ...
        if (!m_IsInScope) {
            ...
            if (m_TokenType != TOKEN_BRACE_OPEN) {
                SetErrorFormat("Unexpected token \"%s\", expected float or opening brace.",
                               m_Token);
                return;
            }
            NextToken();
    ...
    void ScriptReader::RequiredParamString(const char *identifier,
        ...
        if (!m_IsInScope) {
            ...
            if (m_TokenType != TOKEN_BRACE_OPEN) {
                SetErrorFormat("Unexpected token \"%s\", expected string or opening brace.",
                               m_Token);
                return;
            }
            NextToken();
