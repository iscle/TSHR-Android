                ...
                if (strcmp(script->GetNextStructIdentifier(), TOKEN_JOINT) == 0) {
                    ...
                    if (!script->IsError()) {
                        ...
                        if (jointIndex < 0) {
                            ...
                            if (!partition->AddEntry(jointIndex, jointWeight,
                                                     (int) jointPriority)) {
                                script->SetErrorFormat(
                                        "Failed to add joint named \"%s\", possible duplicate.",
                                        jointName);
                            }
                        }
                    }
                    ...
                    if (!script->IsError()) {
                        ...
                        } else if (!partition->AddEntry(iJointIndex, jointWeight,
                                                        (int) jointPriority)) {
                            script->SetErrorFormat(
                                    "Failed to add joint index %d, possible duplicate.",
                                    iJointIndex);
                        }
                    }
                    ...
                    if (!script->IsError()) {
                        ...
                        if ((iMin < 0) || (iMax < iMin)) {
                            script->SetErrorFormat("Invalid index range values of %d to %d.", iMin,
                                                   iMax);
                        } else if (!partition->AddRangeEntry(iMin, iMax, jointWeight,
                                                             (int) jointPriority)) {
                            script->SetErrorFormat("Failed to add joint range %d to %d.", iMin,
                                                   iMax);
                        }
                    }
                } else {
                ...
                if (strcmp(script->GetNextStructIdentifier(), TOKEN_JOINT) == 0) {
                    ...
                    if (!script->IsError()) {
                        ...
                        if (jointIndex < 0) {
                            ...
                            script->SetErrorFormat(
                                    "Failed to add joint \"%s\", possible duplicate.", jointName);
                        }
                    }
                } else if (strcmp(script->GetNextStructIdentifier(), TOKEN_JOINTINDEX) == 0) {
                    script->BeginStruct(TOKEN_JOINTINDEX);
                    ...
                    if (!script->IsError()) {
                        if (!partition->AddEntry((int) jointIndex)) {
                            script->SetErrorFormat("Failed to add joint %d, possible duplicate.",
                                                   (int) jointIndex);
                        }
                    }
                } else if (strcmp(script->GetNextStructIdentifier(), TOKEN_JOINTRANGE) == 0) {
...
//---------------------------------------------------------------------------
    ...
    bool IntersectPartition::ReadFromScript(ScriptReader *script, BaseBank *bank) {
        ...
        script->RequiredParamString(TOKEN_NAME, intersectPartitionName,
                                    sizeof(intersectPartitionName));
...
//---------------------------------------------------------------------------
    ...
    bool CompletePartition::AddJointRange(tUID jointMinUID, tUID jointMaxUID, float weight,
                                          int priority) {
                ...
                if (strcmp(script->GetNextStructIdentifier(), TOKEN_JOINT) == 0) {
                    ...
                    if (!script->IsError()) {
                        ...
                        if (jointIndex < 0) {
                            ...
                            if (!partition->AddJoint(jointIndex, jointWeight,
                                                     (int) jointPriority)) {
                                script->SetErrorFormat("Failed to add joint named \"%s\".",
                                                       jointName);
                            }
                        }
                    }
                    ...
                    if (!script->IsError()) {
                        ...
                        if ((iJointIndex < 0) || (iJointIndex >= rig->GetJointCount())) {
                            script->SetErrorFormat("Joint index %d out of range.", iJointIndex);
                        } else if (!partition->AddJoint(iJointIndex, jointWeight,
                                                        (int) jointPriority)) {
                    ...
                    if (!script->IsError()) {
                        ...
                        if ((jointMinIndex < 0) || (jointMaxIndex < jointMinIndex) ||
                            (jointMaxIndex >= rig->GetJointCount())) {
                            script->SetErrorFormat("Invalid joint range values of %s to %s.",
                                                   jointMin, jointMax);
                        } else if (!partition->AddJointRange(jointMinIndex, jointMaxIndex,
                                                             jointWeight, (int) jointPriority)) {
                            script->SetErrorFormat("Failed to add joint range %s to %s.", jointMin,
                                                   jointMax);
                        }
                    }
                    ...
                    if (!script->IsError()) {
                        ...
                        if ((iMin < 0) || (iMax < iMin) || (iMax >= rig->GetJointCount())) {
                            script->SetErrorFormat("Invalid index range values of %d to %d.", iMin,
                                                   iMax);
                        } else if (!partition->AddJointRange(iMin, iMax, jointWeight,
                                                             (int) jointPriority)) {
                            script->SetErrorFormat("Failed to add joint range %d to %d.", iMin,
                                                   iMax);
                        }
...
//---------------------------------------------------------------------------
    ...
    float LODPartition::GetLOD() const {
        return (m_ActiveJointCount - 1.0f + m_LastActiveJointWeight) /
               (float) GetRig()->GetJointCount();
    }
