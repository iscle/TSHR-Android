...
namespace choreo {
    ...
    InternalAnimation::InternalAnimation(tSkeleton *skeleton, PURE3D_ANIMATION *anim) {
        ...
        if (m_JointMax > 0) {
            ...
            for (i = m_JointMin; i <= m_JointMax; ++i) {
                ...
                if (animGroup != 0) {
                    jointChannels->vectorChannel = animGroup->GetVectorChannel(
                            Pure3DAnimationChannels::Pose::TRANSLATION_TRAN);
                    jointChannels->rotationChannel = animGroup->GetRotationChannel(
    ...
    void InternalAnimation::GetJointTranslation(int jointIndex, float frame,
                                                rmt::Vector &translation) const {
    ...
    void InternalAnimation::GetJointRotation(int jointIndex, float frame,
                                             rmt::Quaternion &rotation) const {
        m_JointChannels[jointIndex - m_JointMin].rotationChannel->GetQuaternion(frame, &rotation);
    }

    void InternalAnimation::GetJointRotationEuler(int jointIndex, float frame,
                                                  rmt::Vector &rotation) const {
    ...
    void InternalAnimation::GetGroupTranslation(tUID groupUID, float frame,
                                                rmt::Vector &translation) const {
        const tAnimationGroup *animGroup = m_Animation->GetGroupByUID(groupUID);
        const tVectorChannel *vectorChannel = animGroup->GetVectorChannel(
                Pure3DAnimationChannels::Pose::TRANSLATION_TRAN);
    ...
    void InternalAnimation::GetGroupRotation(tUID groupUID, float frame,
                                             rmt::Quaternion &rotation) const {
        const tAnimationGroup *animGroup = m_Animation->GetGroupByUID(groupUID);
        const tRotationChannel *rotationChannel = animGroup->GetRotationChannel(
                Pure3DAnimationChannels::Pose::ROTATION_ROT);
        rotationChannel->GetQuaternion(frame, &rotation);
    }

    void InternalAnimation::GetGroupRotationEuler(tUID groupUID, float frame,
                                                  rmt::Vector &rotation) const {
        const tAnimationGroup *animGroup = m_Animation->GetGroupByUID(groupUID);
        const tRotationChannel *rotationChannel = animGroup->GetRotationChannel(
                Pure3DAnimationChannels::Pose::ROTATION_ROT);
