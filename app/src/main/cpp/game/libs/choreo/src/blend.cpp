        ...
        for (f = 0; f < m_FootCount; ++f) {
            ...
            if ((extraAttribIndex >= 0) &&
                ...
                for (int iFrame = 0; iFrame < (int) (frameCount - 1.0f); ++iFrame) {
                    ...
                    if (rmt::Epsilon(info, plantValue, FOOT_EPSILON) &&
                        !rmt::Epsilon(prevInfo, plantValue, FOOT_EPSILON)) {
                        if (foot->plant < 0.0f) {
                            foot->plant = frame;
                        }
                    } else if (rmt::Epsilon(info, liftValue, FOOT_EPSILON) &&
                               !rmt::Epsilon(prevInfo, liftValue, FOOT_EPSILON)) {
                        if (foot->lift < 0.0f) {
                            foot->lift = frame;
                        }
                    }
...
//---------------------------------------------------------------------------
    ...
    public:
        ...
        Blend *GetBlend() { return m_Blend; }

        // blending params
        Partition *GetPartition() const { return m_Partition; }

        void SetPartition(Partition *partition) { tRefCounted::Assign(m_Partition, partition); }
        ...
        int GetBlendSlotCount() const { return m_BlendSlotCount; }
        ...
        void SetBlendThreshold(float blendThreshold) { m_BlendThreshold = blendThreshold; }

        int GetMaxBlendedSlotCount() const { return m_MaxBlendedSlotCount; }

        void SetMaxBlendedSlotCount(int maxSlotCount) { m_MaxBlendedSlotCount = maxSlotCount; }
    ...
    void BlendPhase::UpdateLegPhase() {
        ...
        for (i = 0; i < legCount; ++i) {
            ...
            if (((footLift > footPlant) && (GetPhase() >= footPlant) && (GetPhase() < footLift)) ||
                ((footLift < footPlant) &&
                 ((GetPhase() >= footPlant) || (GetPhase() < footLift)))) {
                // foot is planted
                foot->isPlanted = true;
