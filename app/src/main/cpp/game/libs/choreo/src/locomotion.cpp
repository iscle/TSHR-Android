    ...
    LocomotionSlot::LocomotionSlot(Rig *rig, PURE3D_ANIMATION *animation) :
        ...
        if (motionRootIndex >= 0) {
            if (m_Animation.HasJointTranslation(motionRootIndex)) {
                ...
                m_Animation.GetJointTranslation(motionRootIndex, m_Animation.GetFrameCount() - 1.0f,
                                                dist);
                dist.Sub(initDist);
            }
        }
...
//---------------------------------------------------------------------------
                ...
                if (strcmp(script->GetNextStructIdentifier(), TOKEN_ANIMATION) == 0) {
                    ...
                    if (!script->IsError()) {
                        if (strlen(p3dAnimName) == 0) {
                            ...
                            PURE3D_ANIMATION *p3dAnim = choreo::find<PURE3D_ANIMATION>(bank,
                                                                                       p3dAnimName);
                            if (p3dAnim == 0) {
                                script->SetErrorFormat(
                                        "Could not find Pure3D animation named \"%s\".",
                                        p3dAnimName);
                            } else {
    ...
    bool Locomotion::ReadFromScript4(ScriptReader *script, BaseBank *bank) {
        ...
        if (!script->IsError()) {
            while (script->IsMoreStructs()) {
                if (strcmp(script->GetNextStructIdentifier(), TOKEN_ANIMATION) == 0) {
                    ...
                    if (i >= DIR_COUNT) {
                        script->SetErrorFormat("Incorrect direction specification \"%s\".",
                                               dirName);
                    } else {
                        dirIndex = i;
                    }
                    ...
                    if (animIndex > MAX_ANIMATIONS) {
                        ...
                        script->SetErrorFormat(
                                "Direction \"%s\" and speed %d animation already specified.",
                                dirName, speedIndex);
                    }
        ...
        velCount += 2;
        if (velCount < 2) {
            script->SetErrorFormat("Degenerate locomotion, velocity count of %d is less than 2.",
                                   velCount);
        }
...
//---------------------------------------------------------------------------
    ...
    public:
        ...
        float GetMotionAngle() const { return m_MotionAngle; }

        void SetMotionAngle(float motionAngle) { m_MotionAngle = motionAngle; }
...
//---------------------------------------------------------------------------
    ...
    float LocomotionDriver::GetActualAnimMaxVelocity() const {
        ...
        float relAngle = Wrap(0.0f, rmt::PI_2,
                              GetSmallestArc(actualMotionAngle, actualFacingAngle));
        return m_Locomotion->GetAnimMaxVelocity(relAngle);
    }

    float LocomotionDriver::GetDesiredAnimMaxVelocity() const {
        float relAngle = Wrap(0.0f, rmt::PI_2,
                              GetSmallestArc(m_DesiredMotionAngle, m_DesiredFacingAngle));
        return m_Locomotion->GetAnimMaxVelocity(relAngle);
    }
