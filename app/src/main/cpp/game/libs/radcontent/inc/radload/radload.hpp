...
// Typedefs
enum radLoadState {
    CREATED, QUEUED, LOADING, COMPLETE, CANCELED
};
...
template<class base>
base *radLoadFind(radLoadInventory *inventory, const radKey id);

template<class base>
base *radLoadFind(radLoadInventory *inventory, const char *name); //@}
...
struct radLoadInit {
    ...
    radLoadInit() : fileLoaderListSize(64), dataLoaderListSize(256), loadQueueSize(32),
                    loadThreadStackSize(65536) {};
...
struct radLoadOptions {
    ...
    radLoadOptions() : filename(NULL), stream(NULL), resolverInventory(NULL),
                       allocator(RADLOAD_ALLOC_DEFAULT), syncLoad(false), yieldMillis(0.0f),
                       yieldSize(0) {}
