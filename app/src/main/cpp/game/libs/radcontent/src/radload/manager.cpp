...
radLoadManager::radLoadManager(radLoadInit &init)
    ...
    m_pFileLoaders = new RefHashTable<radLoadFileLoader>(init.fileLoaderListSize, 200,
                                                         init.fileLoaderListSize);
    m_pFileLoaders->AddRef();
    m_pDataLoaders = new RefHashTable<radLoadDataLoader>(init.dataLoaderListSize, 200,
                                                         init.dataLoaderListSize);
    ...
    m_pMutex->Lock();
    ::radThreadCreateThread(&m_pThread, radLoadManager::LoadThreadEntry, static_cast<void *>(this),
                            IRadThread::PriorityNormal, init.loadThreadStackSize);
            ...
            if (callback) {
                ...
                if (item) {
                    ...
                    */
                    ...
                    radLoadFileLoader *loader = m_pFileLoaders->Find(
                            radMakeCaseInsensitiveKey(filename + i));
                    rAssert(loader);
                    radMemoryAllocator old = ::radMemorySetCurrentAllocator(
                            item->GetOptions()->allocator);

                    loader->LoadFile(item->GetOptions(),
                                     static_cast<radLoadUpdatableRequest *>(item));
