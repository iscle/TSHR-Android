...
PtrHashTable::PtrHashTable(unsigned int tableSize, unsigned int maxDensity,
                           unsigned int maxDeletedItems)
        :
...
void *PtrHashTable::Iterator::PtrFirst(radKey key) {
    ...
    while (((m_hashTable->m_elements[m_hash] != NULL) && (m_hashTable->m_keys[m_hash] != m_key)) ||
           (m_hashTable->m_elements[m_hash] == &m_hashTable->m_deleteMarker)) {
        nSearch++;
...
void *PtrHashTable::RawIterator::PtrNext() {
    do {
        m_index++;
    } while ((static_cast<unsigned int>(m_index) < m_hashTable->m_tableSize) &&
             ((!m_hashTable->m_elements[m_index]) ||
              (m_hashTable->m_elements[m_index] == &m_hashTable->m_deleteMarker)));
...
void *PtrHashTable::RawIterator::PtrPrev() {
    do {
        m_index--;
    } while ((m_index >= 0) && (!m_hashTable->m_elements[m_index]) &&
             (m_hashTable->m_elements[m_index] != &m_hashTable->m_deleteMarker));
