...
void radMovieVideoDecoderPs2::Initialize(unsigned int maxWidthPixels, unsigned int maxHeightPixels,
                                         unsigned int codedVideoBufferBytes) {
    rAssert((codedVideoBufferBytes / PS2_VIDEO_DECODE_NUM_PACKETS) % radFileMaxSectorSize ==
            0);  // proper size for double buffering
    ...
    m_pDecodedVideoBuffer = (char *) ::radMemoryAllocAligned(GetThisAllocator(),
                                                             m_DecodedVideoBufferSize,
                                                             RAD_IPU_FILE_READMULTIPLE);
    ...
    // HACK HACK HACK --joel
    radMemoryAllocator heaps[] = {7, 1,
                                  9};  // Zone/Rail, Temp, then HUD (trust me, don't change this)
...
void radMovieVideoDecoderPs2::SetSource(IRadFile *pIRadFile, unsigned int widthPixels,
...
void radMovieVideoDecoderPs2::AddDataToDecoder(void) {
    ...
    if (isInputRequired == true) {
        ...
        if (m_DecoderInputPending == false &&
            ...
            radMovieIpuPs2::GetInstance()->InputAsync(m_pCodedVideoPacket[m_CurrentDecodePacket],
                                                      m_CodedVideoPacketSize);
        }
    }
}
...
void *radMovieVideoDecoderPs2::GetLockedVideoFrame(unsigned int *pPresentationTime) {
    ...
    if (m_State == IRadMovieVideoDecoder::Ready) {
        ...
        *pPresentationTime =
                (currentFrame / m_FrameRate) * 100000 +
                (currentFrame % m_FrameRate) * 100000 / m_FrameRate;
        ...
        if ((unsigned int) pUserData == RAD_IPU_FILE_READ_MARKER) {
            ...
            if (m_FileBytesRemaining == 0) {
                rDebugChannelPrintf(radMovieDebugChannel2,
                                    "radMovieVideoDecoderPs2: Reached end of file\n");
            }
...
void radMovieVideoDecoderPs2::DecodeNextFrame(void) {
    rAssert(m_State == IRadMovieVideoDecoder::Ready ||
            m_State == IRadMovieVideoDecoder::DataPending);
