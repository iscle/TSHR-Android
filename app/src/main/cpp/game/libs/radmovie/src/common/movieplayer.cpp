...
void radMoviePlayer::Initialize
    ...
    m_refIRadSoundBufferedDataSource->Initialize(radMemorySpace_Local,
                                                 ::radMemoryGetAllocator(GetThisAllocator()),
                                                 audioPrimaryBufferSize, audioSizeType, NULL,
                                                 "radMoviePlayer::m_refIRadSoundBufferedDataSource");
    ...
    m_refIRadSoundStreamPlayer->InitializeAsync(audioSecondaryBufferSize, audioSizeType,
                                                ...
                                                "radMoviePlayer::m_refIRadSoundStreamPlayer");
...
void radMoviePlayer::OnFileOperationsComplete(void *pUserData) {
    if (m_refIRadFile_Stale != NULL) {
        ...
        if (m_refIRadFile_Current->IsOpen()) {
            ...
            m_pTempHeaderBuffer = ::radMemoryAllocAligned(RADMEMORY_ALLOC_TEMP,
                                                          RAD_MOVIE_FILE_DATA_POSITION,
            ...
            m_refIRadFile_Current->AddCompletionCallback(this,
                                                         (void *) RAD_MOVIE_PLAYER_HEADER_READ);
        } else {
            m_refIRadFile_Current = NULL;
        }
        ...
        m_refIRadMovieRenderStrategy->SetParameters(pRadMovieFileHeader->m_Width,
                                                    pRadMovieFileHeader->m_Height);
        m_refIRadMovieVideoDecoder->SetSource(m_refIRadFile_Current, m_VideoFrameInfo.Width,
                                              m_VideoFrameInfo.Height,
                                              pRadMovieFileHeader->m_DataPosition,
                                              pRadMovieFileHeader->m_FrameRate);
        ...
        if (m_AudioTrackIndex != radMovie_NoAudioTrack) {
            ...
            if (m_AudioTrackIndex < pRadMovieFileHeader->m_NumAudioTracks) {
                ...
                ref <radMovieAudioDataSource> refSoundSource = ::radMovieAudioDataSourceCreate(
                        GetThisAllocator());
...
void radMoviePlayer::Service(void) {
    ...
    if (m_State == IRadMoviePlayer2::NoData) {
        ...
        rAssert((!(m_refIRadFile_Current == NULL && m_refIRadFile_Stale != NULL &&
                   m_refIRadString_PendingFileName == NULL)));
        rAssert((!(m_refIRadFile_Current != NULL && m_refIRadFile_Stale == NULL &&
                   m_refIRadString_PendingFileName != NULL)));
        rAssert((!(m_refIRadFile_Current != NULL && m_refIRadFile_Stale != NULL &&
        ...
        if (m_refIRadFile_Stale != NULL) {
            ...
            m_refIRadFile_Current->AddCompletionCallback(this,
                                                         (void *) RAD_MOVIE_PLAYER_FIRST_ACCESS);
            ...
            if
                    ...
                    (m_refIRadSoundStreamPlayer->GetState() == IRadSoundStreamPlayer::Paused ||
        ...
        if (m_VideoFrameState == VideoFrame_Unlocked) {
            ...
            if ((m_AudioTrackIndex == radMovie_NoAudioTrack ||
                 streamPlayerState == IRadSoundStreamPlayer::Playing) &&
                (videoDecoderState == IRadMovieVideoDecoder::Ready ||
                 videoDecoderState == IRadMovieVideoDecoder::Decoding)) {
                //
                // Try locking the next video frame
                //

                m_pVideoFrame = m_refIRadMovieVideoDecoder->GetLockedVideoFrame(
                        &m_PresentationTime);
                ...
                rAssert(streamPlayerState != IRadSoundStreamPlayer::Playing &&
                        m_AudioTrackIndex != radMovie_NoAudioTrack ||
                ...
                rDebugChannelPrintf(radMovieDebugChannel2,
                                    "radMoviePlayer: Out of data at [%dms] m_CurrentFrameNumber [%d]\n",
                                    GetCurrentTime());
...
unsigned int radMoviePlayer::GetCurrentTime(void) {
    ...
    if (m_AudioTrackIndex != radMovie_NoAudioTrack &&
        m_refIRadSoundStreamPlayer->GetFormat() != NULL) {
        //
        ...
        if (samples > 0) {
            m_pSoundClockFixupTaps[7] =
                    m_refIRadSoundStreamPlayer->GetFormat()->SamplesToMilliseconds(samples) /
                    (float) m_refIRadStopwatch->GetElapsedTime();
        }
    }
    ...
    return rmt::FtoL(m_refIRadStopwatch->GetElapsedTime() *
                     (m_pSoundClockFixupTaps[0] + m_pSoundClockFixupTaps[1] +
                      m_pSoundClockFixupTaps[2] + m_pSoundClockFixupTaps[3] +
                      m_pSoundClockFixupTaps[4] + m_pSoundClockFixupTaps[5] +
                      m_pSoundClockFixupTaps[6] + m_pSoundClockFixupTaps[7])) / 8;
}
