...
void radMovieRenderStrategyPs2::PrepareImageTags
    ...
    destBasePointer = ((pddiExtPS2Control * )(
            p3d::pddi->GetExtension(PDDI_EXT_PS2_CONTROL)))->GetBackBufferVRAM();
    ...
    for (j = 0; j < mby; j++) {
        for (i = 0; i < mbx; i++) {
            ...
            if (destY + j * MOVIE_UTIL_PIXEL_BLOCK_EDGE > displayHeight) {
                ...
                setTRXPOS(p, 0, destX + i * MOVIE_UTIL_PIXEL_BLOCK_EDGE,
                          destY + j * MOVIE_UTIL_PIXEL_BLOCK_EDGE);
            } else {
