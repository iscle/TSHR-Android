...
void radMovieRenderStrategyXBox::SetParameters(unsigned int width, unsigned int height) {
    ...
    if (width != m_Width || height != m_Height) {
        ...
        m_pD3dTexture = new d3dTexture(((d3dContext *) p3d::context->GetContext()),
                                       m_pIDirect3DTexture8);
...
bool radMovieRenderStrategyXBox::Render(void) {
    ...
    ((d3dDisplay *) p3d::context->GetDisplay())->GetD3DDevice()->SetRenderState(D3DRS_YUVENABLE,
                                                                                true);

    // Use a pddi prim stream to render the movie.  We need 4 vertices.

    pddiPrimStream *pStream = p3d::context->GetContext()->BeginPrims(m_pPddiShader,
...
*/
    ...
    float bottom =
            (float) (p3d::context->GetDisplay()->GetHeight() - desc.Height) / 2.0f + desc.Height;
