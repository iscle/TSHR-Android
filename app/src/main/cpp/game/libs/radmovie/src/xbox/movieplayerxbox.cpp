...
void radMoviePlayerXBox::Initialize(IRadMovieRenderLoop *pIRadMovieRenderLoop,
...
void radMoviePlayerXBox::Load(const char *pVideoFileName, unsigned int audioTrackIndex) {
    ...
    if (audioTrackIndex != radMovie_NoAudioTrack) {
        ...
        HRESULT hr = ::XMVDecoder_CreateDecoderForFile(XMVFLAG_NONE, pVideoFileName,
                                                       &m_pXMVDecoder);
...
void radMoviePlayerXBox::SetVolume(float volume) {
    volume = 1.0f;
    if (m_Volume != volume) {
        ::radSoundVerifyChangeThreshold(m_CheckAudio, "Volume", m_Volume, volume,
