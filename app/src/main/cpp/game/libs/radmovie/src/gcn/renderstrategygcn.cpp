...
radMovieSimpleFullScreenRenderStrategyCreate(radMemoryAllocator allocator) {
...
0x0C(r12) //  1.596
psq_l f29,
0x10(r12), 0, 0 //  -0.391 2.018
psq_l f30,
0x18(r12), 0, 0 //  -0.813 0.000
...
srwi r11, r11,
2     // divide pixel count by four
add r9, r8, r11     // V pointer is w*h/4 bytes after U pointer

srwi r10, r4,
2      // divide h by four to convert to row tile count
mtctr r10             // move the row tile count into the count register

srwi r10, r3,
1      // divide w by two to convert to UV Pitch

// Done with the source pointer, stomp it with the WGP address
lis r5,
0xCC00      // upper 16 bits
ori r5, r5,
0x8000  // lower 16 bits

RowLoop:

srwi r4, r3,
2               // Divide the width by four for the number of tiles per scanline
...
psq_l f0,
0x00(r7),  0, 4     // Load Y00 Y10
psq_l f2,
0x00(r11), 0, 4     // Load Y01 Y11
psq_l f4,
0x00(r12), 0, 4     // Load Y02 Y12

subi r4, r4,
1               // decrement the tile count

psq_l f1,
0x02(r7),  0, 4     // Load Y20 Y30
psq_l f3,
0x02(r11), 0, 4     // Load Y21 Y31
psq_l f5,
0x02(r12), 0, 4     // Load Y22 Y32

addi r7, r7,
4               // Move pointer to the start of the next tile
...
psq_l f10,
0x00(r9), 0, 4     // Load V00 U10
psq_lx f11, r9, r10,
0, 4      // Load V01 U11

addi r8, r8,
2               // Move U pointer to start of next tile
ps_sub f8, f8, f26           // f8  = (U-128)   0,0 & 1,0
ps_sub f9, f9, f26           // f9  = (U-128)   0,1 & 1,1

addi r9, r9,
2               // Move V pointer to start of next tile
...
psq_st f12,
0x00(r5), 0, 4     // Store GB Pixel 00
psq_st f13,
0x00(r5), 0, 4     // Store GB Pixel 10
psq_st f14,
0x00(r5), 0, 4     // Store GB Pixel 20
psq_st f15,
0x00(r5), 0, 4     // Store GB Pixel 30
psq_st f16,
0x00(r5), 0, 4     // Store GB Pixel 01
psq_st f17,
0x00(r5), 0, 4     // Store GB Pixel 11
psq_st f18,
0x00(r5), 0, 4     // Store GB Pixel 21
psq_st f19,
0x00(r5), 0, 4     // Store GB Pixel 31
...
cmpwi cr0, r4,
0              // test the tile counter

psq_st f12,
0x00(r5), 0, 4     // Store GB Pixel 02
psq_st f13,
0x00(r5), 0, 4     // Store GB Pixel 12
psq_st f14,
0x00(r5), 0, 4     // Store GB Pixel 22
psq_st f15,
0x00(r5), 0, 4     // Store GB Pixel 32
psq_st f16,
0x00(r5), 0, 4     // Store GB Pixel 03
psq_st f17,
0x00(r5), 0, 4     // Store GB Pixel 13
psq_st f18,
0x00(r5), 0, 4     // Store GB Pixel 23
psq_st f19,
0x00(r5), 0, 4     // Store GB Pixel 33
...
void radMovieRenderStrategyGcn::ConvertTexRgba8(char *src, char *dest, unsigned int width,
                                                unsigned int height) {
