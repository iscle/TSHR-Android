...
FeProject *FeProjectFileHandler::LoadProject(XMLTree *loadTree) {
    ...
    while (subTree->SetCurrentElementByIndex(i)) {
        if (subTree->GetAttribute(TAG_File, *m_tempString) != 0) {
            ...
            temp += *m_tempString;
            lastScreen = FeApp::GetInstance()->GetFeResourceManager().AddResource(temp, rValue,
                                                                                  RT_SCREEN,
                                                                                  m_tempString->FullFilename());
        }
        ++i;
    }
        ...
        if (subTree->GetAttribute(TAG_File, *m_tempString) != 0) {
            if (subTree->GetAttribute(TAG_Export, &isExport)) {
                if (isExport) {
                    ...
                    temp += *m_tempString;
                    FeApp::GetInstance()->GetFeResourceManager().AddResource(temp, rValue, RT_PAGE,
                                                                             m_tempString->FullFilename());
                }
            }
        }
    ...
    if (pLayersSubTree) {
        ...
        while (NULL != pCurrentLayerTree) {
            if (pCurrentLayerTree->GetAttribute(TAG_AttrName, *m_tempString) > 0) {
                ...
                if (pCurrentLayerTree->GetAttribute(TAG_AttrVisible, *m_tempString) &&
                    (*m_tempString == "true")) {
                    visibility = true;
                } else {
                    visibility = false;
            ...
            if (pImageSubTree->GetAttribute(TAG_AttrName, alias)) {
                if (pImageSubTree->GetAttribute(TAG_AttrData, filename)) {
                    ...
                    *m_tempString += filename;
                    ...
                    if (!FeApp::GetInstance()->UseFullImageNames() ||
                        filename.JustExtension().EqualsInsensitive("p3d")) {
                        filename = filename.FullFilename();
                    } else {
                        filename = *m_tempString;
                    }
                    int index = FeApp::GetInstance()->GetFeResourceManager().AddResource(
                            *m_tempString, NULL, RT_IMAGE, filename);
                    pOwningPage->AddResource(index);
                    FeApp::GetInstance()->GetFeResourceManager().AddAlias(
                            static_cast<const char *>(alias), index);
                }
            }
            ++i;
        }
            ...
            if (pTextBibleSubTree->GetAttribute(TAG_AttrName, alias)) {
                if (pTextBibleSubTree->GetAttribute(TAG_AttrData, filename)) {
                    ...
                    *m_tempString += filename;
                    ...
                    int index = FeApp::GetInstance()->GetFeResourceManager().AddResource(
                            *m_tempString, NULL, RT_TEXT_BIBLE, inventoryName);
                    pOwningPage->AddResource(index);
                    FeApp::GetInstance()->GetFeResourceManager().AddAlias(
                            static_cast<const char *>(alias), index);
                }
            ...
            if (pTextStyleSubTree->GetAttribute(TAG_AttrName, alias)) {
                if (pTextStyleSubTree->GetAttribute(TAG_AttrData, filename)) {
                    ...
                    *m_tempString += filename;
                    ...
                    int index = FeApp::GetInstance()->GetFeResourceManager().AddResource(
                            *m_tempString, NULL, RT_TEXT_STYLE, inventoryName);
                    pOwningPage->AddResource(index);
                    FeApp::GetInstance()->GetFeResourceManager().AddAlias(
                            static_cast<const char *>(alias), index);
                }
        ...
        while (p3dModelSubTree->SetCurrentElementByIndex(i)) {
            if
                ...
                *m_tempString += filename;
                ...
                int index = FeApp::GetInstance()->GetFeResourceManager().AddResource(*m_tempString,
                                                                                     NULL,
                                                                                     RT_P3D_OBJECT,
                                                                                     inventoryName);
                pOwningPage->AddResource(index);
                FeApp::GetInstance()->GetFeResourceManager().AddAlias(
                        static_cast<const char *>(alias), index);
                FeApp::GetInstance()->GetFeResourceManager().AddPure3dAttributes(cameraName,
                                                                                 animationName,
                                                                                 index);
            }
...
void FeProjectFileHandler::ParsePure3dObject(XMLTree *pXmlTree, FeOwner *pOwner) {
    ...
    if (pXmlTree->GetAttribute(TAG_AttrName, *m_tempString)) {
        ...
        if (pXmlTree->SetCurrentElementByName(TAG_Position)) {
            if (!pXmlTree->GetAttribute(TAG_PositionX, &x) ||
        ...
        FePure3dObject *pPure3dObject = new FePure3dObject(
                static_cast<const char *>(*m_tempString), x, y);

        // Read and set the bounding box.
        int width = 0;
        int height = 0;
        if (pXmlTree->SetCurrentElementByName(TAG_Dimension)) {
            if (!pXmlTree->GetAttribute(TAG_DimensionWidth, &width) ||
                !pXmlTree->GetAttribute(TAG_DimensionHeight, &height)) {
...
void FeProjectFileHandler::ParseText(XMLTree *pXmlTree, FeOwner *pOwner) {
    ...
    int x = 0, y = 0;
    if (!pXmlTree->SetCurrentElementByName(TAG_Position) ||
        !pXmlTree->GetAttribute(TAG_PositionX, &x) || !pXmlTree->GetAttribute(TAG_PositionY, &y)) {
    ...
    int h = 0, w = 0;
    if (!pXmlTree->SetCurrentElementByName(TAG_Dimension) ||
        !pXmlTree->GetAttribute(TAG_Height, &h) || !pXmlTree->GetAttribute(TAG_Width, &w)) {
    ...
    if (!pXmlTree->SetCurrentElementByName(TAG_TextStyle) ||
        !pXmlTree->GetAttribute(TAG_AttrName, *m_tempString)) {
