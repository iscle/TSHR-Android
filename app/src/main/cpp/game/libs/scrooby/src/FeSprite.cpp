...
void FeSprite::SetIndex(int n) {
    if (n < mAliases.Size()) {
        ...
        if ((mIndex == n) && mSprite) {
            ...
            mIndex = n;
            m_isDirty = true;
            mSprite = dynamic_cast<tSprite *>(FeApp::GetInstance()->GetFeResourceManager().GetResource(
                    *mAliases[mIndex]));
            m_drawable = mSprite;
        }
    } else {
...
void FeSprite::SetRawSprite(tSprite *sprite, bool updateDrawable) {
    mSprite = sprite != NULL ?
              sprite :
              dynamic_cast<tSprite *>(FeApp::GetInstance()->GetFeResourceManager().GetResource(
                      *mAliases[mIndex]));
