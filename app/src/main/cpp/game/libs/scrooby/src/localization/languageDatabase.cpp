...
//-----------------------------------------------------------------------------
bool LanguageDatabase::AddEntry(const PascalCString &rid, const UnicodeString &translation,
                                const char chLanguage) {
...
//
//=============================================================================
void LanguageDatabase::Export(const char language, const _int8 *mapBuffer,
                              const _int8 *stringBuffer) const {
...

    {
        ...
        for (RidToEntryMap::const_iterator rit = ridToEntryMap.begin();
             rit != ridToEntryMap.end(); rit++) {
    ...
    for (i = 0; i < vectorOfAllRidNames.size(); i++) {
        ...
        int hashValue = theString.Hash(
                this->hashingModulo);                        //IMPROVE: use the hash modulo here
    ...
    for (i = 0; i < outputStrings.size(); i++) {
        ...
        theString.MakeWide(((unsigned short *) (stringBuffer + currentOffset)),
                           theString.Length() + 2);
...
bool LanguageDatabase::ImportXlsFile(const char *szFilename) {
    ...
    char *cell2Language = static_cast<char *>(ScroobyGetMemory(ScroobyTempPool,
                                                               headerRowLength));
...
//-----------------------------------------------------------------------------
bool LanguageDatabase::LookupLabel(const UnicodeString &string, const char language,
                                   PascalCString &buffer) const {
    //find the database
    {
        const RidToEntryMap &database = this->m_mapLabelToEntry;
...
//-----------------------------------------------------------------------------
bool LanguageDatabase::LookupTranslation(const PascalCString &label, const char language,
                                         UnicodeString &buffer) const {
    //find the database
    {
        const RidToEntryMap &database = this->m_mapLabelToEntry;
...
//-----------------------------------------------------------------------------
bool
LanguageDatabase::LookupTranslationForAPlatform(const PascalCString &label, const char language,
