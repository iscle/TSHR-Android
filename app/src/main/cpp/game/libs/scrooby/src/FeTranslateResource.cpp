...
bool FeResourceTextBible::Load() {
    ...
    if (buffer == NULL) {
        ...
        FeApp::GetInstance()->GetFeResourceManager().LoadMemoryBuffer(handle,
                                                                      FeApp::GetInstance()->GetFeResourceManager().GetCallback());
        return false;
    } else {
        //buffer is loaded, so all loading of this resource is done - interpret the data
        this->mTranslator[this->currentLanguage].RegisterDatafile(
                reinterpret_cast<XLDatafile *>(buffer));
    }
    return true;
}
