...
//===========================================================================
    tEntity *FeProjectChunkHandler::LoadObject(tChunkFile *file, tEntityStore *store) {
        ...
        FeApp::GetInstance()->GetFeResourceManager().SetCallbackResourceID(
                FeApp::GetInstance()->GetFeResourceManager().AddResource("dummy.p3d", NULL,
            ...
            switch (chunkId) {
                ...
                case Pure3D::Scrooby::RESOURCEIMAGE : {
                    ...
                    if (!FeApp::GetInstance()->UseFullImageNames() ||
                        pfilename.JustExtension().EqualsInsensitive("p3d")) {
                        pfilename = fullFileName;
                    } else {
                        pfilename = tempString;
                    }
                    int index = FeApp::GetInstance()->GetFeResourceManager().AddResource(tempString,
                                                                                         NULL,
                                                                                         RT_IMAGE,
                                                                                         pfilename);
...
//===========================================================================
    FePure3dObject *FeProjectChunkHandler::LoadPure3dObjectChunk(tChunkFile *file) {
        ...
        returnMe->SetVerticalJustification(Scrooby::JustificationEnum(
                justificationY));     //IAN IMPROVE: new style casts please
...
//===========================================================================
    FeScreen *FeProjectChunkHandler::LoadScreenChunk(tChunkFile *file) {
        ...
        while (numPages > 0) {
            ...
            ::radMemorySetCurrentAllocator(old);
            FeApp::GetInstance()->GetFeResourceManager().AddResource(temp, returnMe, RT_PAGE,
                                                                     fullFileName);
            numPages--;
        }
