...
UnicodeString &UnicodeString::operator=(const UnicodeString &right) {
    ...
    if (m_vectorSize > 0) {
        if (right.m_charVector != NULL) {
            memcpy(m_charVector, right.m_charVector,
                   (right.m_vectorSize + 1) * sizeof(UnicodeChar));
        } else {
...
void UnicodeString::MakeAscii(char *buffer, int size) const {
    ...
    for (i = 0; i < Length(); i++) {
        ...
        buffer[i] = ((m_charVector[i] - static_cast<unsigned char>(m_charVector[i])) == 0)
                    ? static_cast<char>(m_charVector[i]) : '?';
    }
    buffer[i] = 0;
}
