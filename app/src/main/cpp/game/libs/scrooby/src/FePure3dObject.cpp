...
void FePure3dObject::Render() {
    ...
    if (!m_alreadyRendered) {
        ...
        //
        int index = FeApp::GetInstance()->GetFeResourceManager().GetIndex(
                static_cast<const char *>(*m_alias));

        tDrawable *drawable = GetDrawable();
        if (drawable == NULL) {
            SetDrawable(
                    dynamic_cast<tDrawable *>(FeApp::GetInstance()->GetFeResourceManager().GetResource(
                            index)));
        ...
        if (GetMultiController() == NULL) {
            tMultiController *multiController = FeApp::GetInstance()->GetFeResourceManager().GetMultiController(
                    index);
...
#endif
        m_Camera->SetState();
        p3d::context->LoadViewMatrix(m_Camera->GetWorldToCameraMatrix(),
                                     m_Camera->GetCameraToWorldMatrix());
    }
