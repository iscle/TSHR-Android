...
//===========================================================================
    void SceneElement::Display(const SceneElementRenderProperties &properties) {
        ...
        SceneElementRenderProperties newProp = this->UpdateProperties(
                properties);//IAN IMPROVE: don't call this twice all the time
...
//    }
        ...
        m_tDrawable = d;
        if (m_tDrawable !=
            NULL) {                               //IAN IMPROVE: you've got it backwards, are you sure?
...
//===========================================================================
    SceneElementRenderProperties
    SceneElement::UpdateProperties(const SceneElementRenderProperties &properties) {
    ...
    P3D_WCHAR defaultCharacters[] =
            {
                    ...
                    'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z',    //65-90
                    'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p',
                    'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z',    //97-122
...
//===========================================================================
    tFont *Core::LoadFontFromBuffer
        ...
        tFileMem *mem = new tFileMem(reinterpret_cast<unsigned char *>(buffer),
                                     bufferSize); //P3D NEW
        mem->AddRef();
        mem->SetFilename(name);

        tTypeFace *typeFace = new tTypeFace(mem,
                                            static_cast<int>(fontSize));             //P3D NEW
        typeFace->AddRef();
        typeFace->SetCharacters(defaultCharacters);
        thisFont = typeFace->MakeTextureFont(name, P3D_SCREEN_FONT,
                                             static_cast<int>(fontSize));      //IAN IMPROVE: casting int to float is bad
...
//===========================================================================
    ...
    unsigned Core::AddText(FeText *pFeText, tFont *pFont, UnicodeChar *string) {
        ...
        //
        tTextString *pTextString = new tTextString(pFont, reinterpret_cast<P3D_WCHAR *>(string),
                                                   1.0f); //P3D NEW
    ...
    void Core::ReplaceTextObject(FeText *pFeText, tFont *pFont, UnicodeChar *string,
                                 unsigned textHandle) {
        //
        // Note: UnicodeChar == P3D_WCHAR == unsigned short
        //
...
//===========================================================================
    ...
    void Core::ReplacePoly(FePolygon *pFePolygon, unsigned polyHandle, int nVer, rmt::Vector *ver,
                           tColour *c) {
...
//===========================================================================
    ...
    void Core::Link(unsigned parent, unsigned child) {
        ...
        } else {// append in the end to preserve bottom to top order
    ...
    void Core::ApplyEffect(unsigned effectHandle, unsigned objectHandle) {
        if (effects[effectHandle] && scene[objectHandle].Used() &&
            scene[objectHandle].GetTDrawable()) {
...
//===========================================================================
    void Core::DisplaySetup() {
        ...
        aspect = static_cast<float>(FeApp::GetInstance()->GetScreenWidth()) /
