...
//----------------------------------------------------------------------------
    ...
    bool
    block_construct_from_stream_synch(block *, stream_info *, pointer *p_first_item, memory_hooks *,
                                      bool reverse_endian);
...
// class
    ...
    void cls_type_add_attribute(const ods::pointer &ptr_cls_type, const char *p_name,
                                const ods::pointer &ptr_type);

    unsigned int cls_type_num_attributes(const ods::pointer &ptr_cls_type);

    void cls_type_attribute_name(const ods::pointer &ptr_cls_type, unsigned int attrib, char *,
                                 unsigned int);
...
//----------------------------------------------------------------------------
    ...
    bool instance_write_text(const ods::pointer &ptr_project, const ods::pointer &ptr_type,
                             const ods::pointer &ptr_instance, stream_info *, unsigned int indent);

    bool
    instance_read_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance, Parser &);

    bool
    instance_compare_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance, Parser &,
                          bool *p_same);
...
// uint32
    ...
    bool uint32_instance_write_text(const ods::pointer &ptr_project, const ods::pointer &ptr_type,
                                    const ods::pointer &ptr_instance, stream_info *,
                                    unsigned int indent);

    bool uint32_instance_read_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                   Parser &);

    bool
    uint32_instance_compare_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                 Parser &, bool *p_same);
...
// float
    ...
    bool float_instance_write_text(const ods::pointer &ptr_project, const ods::pointer &ptr_type,
                                   const ods::pointer &ptr_instance, stream_info *,
                                   unsigned int indent);

    bool float_instance_read_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                  Parser &);

    bool float_instance_compare_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                     Parser &, bool *p_same);
...
// character
    ...
    bool char_instance_write_text(const ods::pointer &ptr_project, const ods::pointer &ptr_type,
                                  const ods::pointer &ptr_instance, stream_info *,
                                  unsigned int indent);

    bool char_instance_read_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                 Parser &);

    bool char_instance_compare_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                    Parser &, bool *p_same);
...
// bool
    ...
    bool bool_instance_write_text(const ods::pointer &ptr_project, const ods::pointer &ptr_type,
                                  const ods::pointer &ptr_instance, stream_info *,
                                  unsigned int indent);

    bool bool_instance_read_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                 Parser &);

    bool bool_instance_compare_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                    Parser &, bool *p_same);
...
// string
    ...
    bool string_instance_write_text(const ods::pointer &ptr_project, const ods::pointer &ptr_type,
                                    const ods::pointer &ptr_instance, stream_info *,
                                    unsigned int indent);

    bool string_instance_read_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                   Parser &);

    bool
    string_instance_compare_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                 Parser &, bool *p_same);
...
// array
    ...
    bool array_instance_write_text(const ods::pointer &ptr_project, const ods::pointer &ptr_type,
                                   const ods::pointer &ptr_instance, stream_info *,
                                   unsigned int indent);

    bool array_instance_read_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                  Parser &);

    bool array_instance_compare_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                     Parser &, bool *p_same);

    bool array_instance_assemble_name(const ods::pointer &ptr_project, const ods::pointer &ptr_type,
                                      const ods::pointer &ptr_object,
                                      const ods::pointer &ptr_instance, char *p_string,
    ...
    ods::pointer array_instance_item_at(const ods::pointer &ptr_array, unsigned int item_size,
                                        unsigned int index);

    void array_instance_remove_item(const ods::pointer &ptr_array, unsigned int item_size,
                                    unsigned int index);
...
// ref
    ...
    bool ref_instance_write_text(const ods::pointer &ptr_project, const ods::pointer &ptr_type,
                                 const ods::pointer &ptr_instance, stream_info *,
                                 unsigned int indent);

    bool ref_instance_read_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                Parser &);

    bool ref_instance_compare_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                   Parser &, bool *p_same);
...
// cls
    ...
    bool cls_instance_write_text(const ods::pointer &ptr_project, const ods::pointer &ptr_type,
                                 const ods::pointer &ptr_instance, stream_info *,
                                 unsigned int indent);

    bool cls_instance_read_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                Parser &);

    bool cls_instance_compare_text(const ods::pointer &ptr_type, const ods::pointer &ptr_instance,
                                   Parser &, bool *same);

    ods::pointer
    cls_instance_find_attribute(const ods::pointer &ptr_type, const ods::pointer &ptr_cls_instance,
                                const char *p_name, const pointer &ptr_expected_type);

    bool cls_instance_assemble_name(const ods::pointer &ptr_project, const ods::pointer &ptr_type,
                                    const ods::pointer &ptr_object,
                                    const ods::pointer &ptr_instance, char *p_string,
                                    unsigned int max_chars);
...
//----------------------------------------------------------------------------
    ...
    ods::pointer
    project_cls_type_find_or_alloc(const ods::pointer &ptr_project, const char *p_name);

    unsigned int project_num_ref_types(const ods::pointer &ptr_project);

    ods::pointer
    project_ref_type_find(const ods::pointer &ptr_project, const ods::pointer &ptr_type);
    ...
    ods::pointer project_ref_type_find_or_alloc(const ods::pointer &ptr_project,
                                                const ods::pointer &ptr_ref_type);

    unsigned int project_num_array_types(const ods::pointer &ptr_project);

    ods::pointer
    project_array_type_find(const ods::pointer &ptr_project, const ods::pointer &ptr_type);

    ods::pointer project_array_type_at(const ods::pointer &ptr_project, unsigned int i);

    void
    project_add_array_type(const ods::pointer &ptr_project, const ods::pointer &ptr_array_type);

    ods::pointer project_array_type_find_or_alloc(const ods::pointer &ptr_project,
                                                  const ods::pointer &ptr_array_type);

    void project_add_instance(const ods::pointer &ptr_project, const char *p_name,
                              const ods::pointer &ptr_object, const ods::pointer &ptr_type);

    unsigned int project_num_instances(const ods::pointer &ptr_project);

    ods::pointer
    project_instance_at(const ods::pointer &ptr_project, unsigned int i, char *, unsigned int,
                        ods::pointer *ptr_type);

    ods::pointer
    project_find_instance(const ods::pointer &ptr_project, const char *, ods::pointer *ptr_type);

    void project_assemble_instance_name(const ods::pointer &ptr_project,
                                        const ods::pointer &ptr_instance, char *, unsigned int);
...
//----------------------------------------------------------------------------
    ...
    template<class T>
    class pointer_templ : public ods::pointer {
    public:
        pointer_templ() {}
