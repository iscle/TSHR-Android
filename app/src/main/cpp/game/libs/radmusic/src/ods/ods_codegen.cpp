...
namespace ods {
    ...
    void _get_type_name(pointer ptr_type, char *p_name, unsigned int len) {
        ...
        switch (id) {
            case g_uint32_id:
                strncpy(p_name, "uint32", len);
                break;
            case g_bool_id:
                strncpy(p_name, "bool", len);
                break;
            case g_float_id:
                strncpy(p_name, "float", len);
                break;
            case g_string_id:
                strncpy(p_name, "string", len);
                break;
            case g_character_id:
                strncpy(p_name, "char", len);
                break;
            case g_array_id: {
                ...
                switch (id) {
                    ...
                    case g_character_id: {
                        ...
                        stream_printf(p_si, "inline %s %s_%s(const %s &, %s);\n", "void", name,
                                      attribute_name, name, attribute_type_name);

                        break;
                    }
                    case g_string_id: {
                        stream_printf(p_si,
                                      "inline %s %s_%s(const %s & , char * p_string, unsigned int len);\n",
                    ...
                    case g_ref_id: {
                        _get_type_name(ref_type_get_type(ptr_attribute_type), attribute_type_name,
                                       256);
    ...
    bool _generate_ref_declarations(
        ...
        for (unsigned int r = 0; r < num_ref_types; r++) {
            ...
            switch (id) {
                ...
                case g_string_id: {
                    stream_printf(p_si,
                                  "inline %s deref(const %s & ptr, char * p_string, unsigned int len);\n",
                                  "void", ref_type_name);
                    break;
                }
                case g_array_id:
    ...
    bool _generate_array_declarations(
        ...
        for (unsigned int a = 0; a < num_array_types; a++) {
            ...
            switch (id) {
                ...
                case g_character_id: {
                    stream_printf(p_si,
                                  "inline const %s %s_item_at(const %s & ptr, unsigned int index);\n",
                ...
                case g_string_id: {
                    stream_printf(p_si,
                                  "inline %s %s_item_at(const %s & ptr, unsigned int index, char * p_string, unsigned int len);\n",
                    ...
                    stream_printf(p_si,
                                  "inline %s %s_add_item(const %s &, const char * p_string);\n",
                                  "void", array_type_name, array_type_name);
                    break;
                }
                case g_ref_id: {
                    ...
                    stream_printf(p_si,
                                  ...
                                  type_name, array_type_name, array_type_name);
                    stream_printf(p_si,
                                  "inline %s %s_add_item(const %s &, const %s & ptr_item);\n",
    ...
    bool _generate_cls_definitions(
        ...
        for (unsigned int t = 0; t < num_types; t++) {
            ...
            stream_printf(p_si, "    %s ptr = (%s) block_alloc(p_memory, %d, \"%s\") ;\n", name,
                          name, instance_size(ptr_cls_type), name);
            ...
            for (unsigned int a = 0; a < num_attributes; a++) {
                ...
                switch (id) {
                    ...
                    case g_string_id: {
                        stream_printf(p_si,
                                      "    %s_instance_construct(pointer(ptr.memory(), ptr.offset() + %d));\n",
                    ...
                    case g_cls_id: {
                        stream_printf(p_si,
                                      "    %s_construct(pointer_templ<_%s>(ptr.memory(), ptr.offset() + %d));\n",
            ...
            for (unsigned int a = 0; a < num_attributes; a++) {
                ...
                switch (id) {
                    ...
                    case g_string_id: {
                        stream_printf(p_si,
                                      "    %s_instance_destroy(pointer(ptr.memory(), ptr.offset() + %d));\n",
                                      attribute_type_name, pos);
                        break;
                    }
                    case g_array_id:
                    ...
                    case g_cls_id: {
                        stream_printf(p_si,
                                      "    %s_destroy(pointer_templ<_%s>(ptr.memory(), ptr.offset() + %d));\n",
                                      attribute_type_name, attribute_type_name, pos);
                        break;
                    }
                    default: {
                ...
                switch (id) {
                    ...
                    case g_character_id: {
                        stream_printf(p_si, "inline %s %s_%s(const %s & ptr)\n{\n",
                                      attribute_type_name, name, attribute_name, name);
                        stream_printf(p_si,
                                      "    return %s_instance_read(pointer(ptr.memory(), ptr.offset() + %d));\n}\n",
                                      attribute_type_name, pos);
                        stream_printf(p_si, "inline void %s_%s(const %s & ptr, %s value)\n{\n",
                                      name, attribute_name, name, attribute_type_name);
                        stream_printf(p_si,
                                      "    %s_instance_write(pointer(ptr.memory(), ptr.offset() + %d), value);\n",
                                      attribute_type_name, pos);
                        break;
                    }
                    case g_string_id: {
                        stream_printf(p_si,
                                      "inline void %s_%s(const %s & ptr, char * p_string, unsigned int len)\n{\n",
                                      name, attribute_name, name);
                        stream_printf(p_si,
                                      "    string_instance_read(pointer(ptr.memory(), ptr.offset() + %d), p_string, len);\n}\n",
                                      pos);
                        stream_printf(p_si,
                                      "inline void %s_%s(const %s & ptr, const char * p_string)\n{\n",
                                      name, attribute_name, name);
                        stream_printf(p_si,
                                      "    string_instance_write(pointer(ptr.memory(), ptr.offset() + %d), p_string);\n",
                                      pos);
                        break;
                    }
                    case g_ref_id: {
                        ...
                        stream_printf(p_si, "inline const %s %s_%s(const %s & ptr)\n{\n",
                                      attribute_type_name, name, attribute_name, name);
                        stream_printf(p_si,
                                      "    return (%s) ref_instance_read(pointer(ptr.memory(), ptr.offset() + %d));\n}\n",
                                      attribute_type_name, pos);
                        stream_printf(p_si,
                                      "inline void %s_%s(const %s & ptr, const %s & ptr_item)\n{\n",
                                      name, attribute_name, name, attribute_type_name);
                        stream_printf(p_si,
                                      "    ref_instance_write(pointer(ptr.memory(), ptr.offset() + %d), ptr_item);\n",
                                      pos);
                        break;
                    }
                    case g_array_id:
                    case g_cls_id: {
                        stream_printf(p_si, "inline const %s %s_%s(const %s & ptr)\n{\n",
                                      attribute_type_name, name, attribute_name, name);
                        stream_printf(p_si,
                                      "    return pointer_templ<_%s>(ptr.memory(), ptr.offset() + %d);\n",
                                      attribute_type_name, pos);
                        break;
                    }
                    default: {
            ...
            switch (id) {
                ...
                case g_character_id: {
                    stream_printf(p_si, "inline %s deref(const %s & ptr)\n{\n", type_name,
                                  ref_type_name);
                    stream_printf(p_si,
                                  "    return %s_instance_read(block_read_reference(ptr)));\n}\n",
                                  type_name);
                    break;
                }
                case g_string_id: {
                    stream_printf(p_si,
                                  ...
                                  "void", ref_type_name);
                    stream_printf(p_si,
                                  "    return string_instance_read(block_read_reference(ptr), p_string, len);\n}\n");
                    break;
                }
                case g_array_id:
                case g_ref_id:
                case g_cls_id: {
                    stream_printf(p_si, "inline %s deref(const %s & ptr)\n{\n", type_name,
                                  ref_type_name);
                    stream_printf(p_si,
                                  "    return pointer_templ<_%s>(block_read_reference(ptr));\n}\n",
                                  type_name);
                    break;
                }
                default:
    ...
    bool _generate_array_definitions(
        ...
        for (unsigned int a = 0; a < num_array_types; a++) {
            ...
            stream_printf(p_si,
                          "inline unsigned int %s_num_items(const %s & ptr)\n{\n    return array_instance_num_items(ptr);\n}\n",
                          array_type_name, array_type_name);
            ...
            switch (id) {
                ...
                case g_character_id: {
                    stream_printf(p_si,
                                  ...
                                  type_name, array_type_name, array_type_name);
                    stream_printf(p_si,
                                  "    return %s_instance_read(array_instance_item_at(ptr, %d, index));\n}\n",
                                  type_name, instance_size(ptr_array_type_type));
                    stream_printf(p_si, "inline void %s_add_item(const %s & ptr, %s item)\n{\n",
                                  array_type_name, array_type_name, type_name);
                    stream_printf(p_si,
                                  "    %s_instance_write(array_instance_add_item(ptr, %d), item);\n}\n",
                                  type_name, instance_size(ptr_array_type_type));
                ...
                case g_string_id: {
                    stream_printf(p_si,
                                  "inline void string_array_item_at(const %s & ptr, unsigned int index, char * p_string, unsigned int len)\n{\n",
                                  array_type_name);
                    stream_printf(p_si,
                                  "    return string_instance_read(array_instance_item_at(ptr, %d, index), p_string, len);\n}\n",
                                  instance_size(ptr_array_type_type));

                    stream_printf(p_si,
                                  "inline void string_array_item_at(const string_array & ptr, unsigned int index, const char * p_new_val)\n{\n");
                    stream_printf(p_si,
                                  "    pointer ptr_item = array_instance_item_at(ptr, %d, index);\n",
                                  instance_size(ptr_array_type_type));
                    ...
                    stream_printf(p_si,
                                  "    pointer ptr_new_item = array_instance_add_item(ptr, %d);\n",
                                  instance_size(ptr_array_type_type));
                    ...
                    stream_printf(p_si,
                                  "        pointer ptr_item = array_instance_item_at(ptr, %d, i);\n",
                                  instance_size(ptr_array_type_type));
                    stream_printf(p_si, "        string_instance_destroy(ptr_item);\n    }\n");
                    stream_printf(p_si, "    array_instance_destroy(ptr, %d);\n}\n",
                                  instance_size(ptr_array_type_type));
                ...
                case g_ref_id: {
                    ...
                    stream_printf(p_si,
                                  "inline void %s_add_item(const %s & ptr, const %s & ptr_item)\n{\n",
                                  array_type_name, array_type_name, ref_type_name);
                    stream_printf(p_si,
                                  "     %s ptr_new_item = (%s) array_instance_add_item(ptr, %d);\n",
                    ...
                    stream_printf(p_si, "    for(unsigned int i = 0; i <items; i ++)\n    {\n");
                    stream_printf(p_si,
                                  "        %s ptr_item = (%s) array_instance_item_at(ptr, %d, i);\n",
                                  type_name, type_name, instance_size(ptr_array_type_type));
                ...
                case g_cls_id: {
                    stream_printf(p_si,
                                  ...
                                  type_name, array_type_name, array_type_name);
                    stream_printf(p_si,
                                  "    return pointer_templ<_%s>(array_instance_item_at(ptr, %d, index));\n}\n",
                                  type_name, instance_size(ptr_array_type_type));
                    ...
                    stream_printf(p_si,
                                  "inline void %s_remove_item(const %s & ptr, unsigned int index)\n{\n",
                                  array_type_name, array_type_name);
                    stream_printf(p_si,
                                  "    %s ptr_item = (%s) array_instance_item_at(ptr, %d, index);\n",
                                  type_name, type_name, instance_size(ptr_array_type_type));
                    stream_printf(p_si, "    %s_destroy(ptr_item);\n", type_name);
                    stream_printf(p_si, "    ptr_item = pointer_null;\n");
                    stream_printf(p_si, "    array_instance_remove_item(ptr, %d, index);\n}\n",
