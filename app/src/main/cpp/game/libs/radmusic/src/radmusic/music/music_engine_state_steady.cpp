...
namespace radmusic {
    void
    _state_steady_init_transition(struct_music_engine *, region &region_target, bool reset_target);
...
namespace radmusic {
    ...
    bool state_steady_set_target_region(
        ...
        if (ptr_region_target == p_se->state_steady.ptr_region_current) {
            rDebugChannelPrintf(debug_channel,
                                "region: ignoring request to play region we are already playing\n");
            return true;
        }
    ...
    void _state_steady_init_transition(
        ...
        if (not_null(ptr_stitch_transition)) {
            ...
            if (not_null(ptr_fade_transition)) {
                ...
                rDebugChannel(debug_channel,
                              "Error: No Default transition defined. Can't transition.\n");
            }
        }
    }
        ...
        if (p_se->state_steady.p_state_stitch != 0) {
            if (state_stitch_initialized(p_se)) {
                if (p_se->state_steady.transition_type == state_steady_transition_type_switch) {
                    ...
                    if (sequence_player_state(p_sequence_player_primary) ==
                        sequence_player_state_finishing ||
                        sequence_player_state(p_sequence_player_primary) ==
            ...
            if (state_fade_initialized(p_se)) {
                if (p_se->state_steady.transition_type == state_steady_transition_type_switch) {
                    ...
                    unsigned int source_time = radSoundFloatToUInt(
                            fade_transition_source_time(ptr_fade_transition));
                    unsigned int target_start_time = radSoundFloatToUInt(
                            fade_transition_target_start(ptr_fade_transition));
                    ...
                    unsigned int remaining_time = sequence_player_remaining_time(
                            p_sequence_player_primary);
