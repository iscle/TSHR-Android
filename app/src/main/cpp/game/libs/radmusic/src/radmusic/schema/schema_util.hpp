...
//-----------------------------------------------------------------------------
    ...
    ods::region
    group_find_region_in_sub_tree(const ods::group &ptr_group, const char *p_region_name);
    ...
    construct_audio_format(const ods::audio_format &, bool targetFormat);

    unsigned int
    audio_format_samples_to_milliseconds(const ods::audio_format &, unsigned int samples);

    unsigned int audio_format_samples_to_frames(const ods::audio_format &, unsigned int samples);

    unsigned int
    audio_format_frames_to_samples(const ods::audio_format &ptr_audio_format, unsigned int frames);
    ...
    bool
    beat_set_beat_in_range(const ods::beat_set &ptr_beat_set, float last_beat, float current_beat);

    float tempo_track_beat(const ods::tempo_track &ptr_tempo_track, unsigned int current_sample,
