    ...
    void resource_manager_initialize(
        ...
        for (unsigned int sg = 0; sg < g_num_stream_graphs; sg++) {
            ...
            stream_graph_construct(&p_n->stream_graph,
                                   ...
                                   radMemorySpaceGetAllocator(p_d->buffered_data_source_space,
                                                              memory::allocator()),
        ...
        for (unsigned int b = 0; b < 2; b++) {
            ...
            if (p_rb->state == resource_buffer_state_unloading) {
                while (p_rb->clip_stack_current_pointer > p_rb->clip_stack_unload_pointer) {
                    if (p_rb->p_clips[p_rb->clip_stack_current_pointer -
                                      1].p_IRadSoundclip->WillRelease()) {
                        p_rb->p_clips[p_rb->clip_stack_current_pointer -
                                      1].p_IRadSoundclip->Release();
                ...
                while (p_rb->cache_stack_current_pointer > p_rb->cache_stack_unload_pointer) {
                    if (p_rb->p_data_caches[p_rb->cache_stack_current_pointer -
                                            1].p_IRadSoundDataCache->WillRelease()) {
                        p_rb->p_data_caches[p_rb->cache_stack_current_pointer -
                                            1].p_IRadSoundDataCache->Release();
                        p_rb->p_data_caches[p_rb->cache_stack_current_pointer -
                                            1].p_IRadSoundDataCache = 0;
            ...
            if (p_rb->state == resource_buffer_state_loading) {
                while (p_rb->clip_stack_current_pointer < p_rb->clip_stack_load_pointer) {
                    ...
                    if (p_rb->p_clips[p_rb->clip_stack_current_pointer].p_IRadSoundclip->GetState() !=
                        IRadSoundClip::Initialized) {
                        return;
                    }

                    rAssert(p_rb->p_clips[p_rb->clip_stack_current_pointer].p_IRadSoundclip->GetState() ==
                            IRadSoundClip::Initialized);
                ...
                while (p_rb->cache_stack_current_pointer < p_rb->cache_stack_load_pointer) {
                    ...
                    if (*pp_IRadSoundDataCache == 0) {
                        ...
                        *pp_IRadSoundDataCache = radSoundDataCacheCreate(p_rm->ai_allocator);
                        ...
                        (*pp_IRadSoundDataCache)->InitializeAsync(
                                ...
                                p_rb->p_IRadMemoryHeap_Cache,
                                _construct_file_data_source(p_rm, stream_rsd_file_ref(ptr_stream),
                                                            0),
                                data_cache_nom);
                    }
                    ...
                    rAssert((*pp_IRadSoundDataCache)->GetState() ==
                            IRadSoundDataCache::Initialized);
...
// set load pointer to parent before calling this function
    ...
    void _queue_items_to_load(resource_buffer *p_rb, const group &ptr_group_child,
                              const group &ptr_group_parent) {
        ...
        while (ptr_group_search != ptr_group_parent && (not_null(ptr_group_search))) {
            ...
            for (unsigned int c = 0; c < clip_ref_array_num_items(ptr_clip_ref_array); c++) {
                ...
                p_rb->p_clips[p_rb->clip_stack_load_pointer +
                              (num_clips_to_load - 1 - clips_queued)].ptr_clip = ptr_clip;
                clips_queued++;
            }
            ...
            for (unsigned int s = 0; s < stream_ref_array_num_items(ptr_stream_ref_array); s++) {
                ...
                if (false == stream_streamed(ptr_stream)) {
                    p_rb->p_data_caches[p_rb->cache_stack_load_pointer +
                                        (num_data_caches_to_load - 1 -
                                         caches_queued)].ptr_stream = ptr_stream;
                    caches_queued++;
                }
            }
    ...
    void _resource_buffer_move_to_group(resource_manager *p_rm, unsigned int buffer_number,
                                        const group &ptr_group_target) {
        ...
        if (is_null(p_rb->ptr_group_current) ||
            group_is_child_of(ptr_group_target, p_rb->ptr_group_current)) {
            rDebugChannelPrintf(debug_channel,
            ...
            if (group_tree_depth(ptr_group_to_move_up) !=
                group_tree_depth(ptr_group_to_move_up_to)) {
                // Get to common depth

                if (group_tree_depth(ptr_group_target) >
                    group_tree_depth(p_rb->ptr_group_current)) {
    ...
    bool resource_manager_group_loaded(resource_manager *p_rm, unsigned int buffer,
