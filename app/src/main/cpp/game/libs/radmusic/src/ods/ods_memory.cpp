...
namespace ods {
    ...
    bool block_construct_from_stream_synch(
        ...
        if (true != p_si->p_sf->p_read(p_si->p_stream, &p_m->num_references,
                                       sizeof(p_m->num_references))) { return false; }
        ...
        if (true != p_si->p_sf->p_read(p_si->p_stream, p_m->p_references,
                                       sizeof(offset_t) * p_m->num_references)) {
            return false;
        }
        ...
        if (true !=
            p_si->p_sf->p_read(p_si->p_stream, &p_m->size, sizeof(p_m->size))) { return false; }
    ...
    void block_read(bool check_endian, const pointer &p, offset_t bit_offset, void *p_bits,
                    offset_t bits) {
        rAssert(bit_offset % 8 == 0);
        rAssert(bits % 8 == 0);
    ...
    void block_insert(const pointer &p, offset_t bit_offset, offset_t bits) {
        ...
        ::memmove(p.memory()->p_mem + total_byte_offset + bytes_to_insert,
                  p.memory()->p_mem + total_byte_offset,
                  (p.memory()->size / 8) - total_byte_offset);
    ...
    void block_insert(bool check_endian, const pointer &p, offset_t bit_offset, const void *p_bits,
                      offset_t bits) {
...
// array
    ...
    void array_destroy(pointer p) {
        ...
        block_read(true, p, _array_item_size_offset, &list_item_size_in_bits,
                   (sizeof(list_item_size_in_bits) * 8));
    ...
    pointer array_get_pointer_at(pointer ptr_a, unsigned int index) {
        ...
        block_read(true, ptr_a, _array_item_size_offset, &list_item_size_in_bits,
                   (sizeof(list_item_size_in_bits) * 8));

        return pointer(ptr_a.memory(),
                       ptr_a.offset() + _array_start_offset + (list_item_size_in_bits * index));
    }
    ...
    pointer array_add_tail(const pointer &ptr_array) {
        ...
        block_read(true, ptr_array, _array_item_size_offset, &list_item_size_in_bits,
                   (sizeof(num_items) * 8));
        ...
        pointer ptr_new_item(ptr_array.memory(), ptr_array.offset() + _array_start_offset +
                                                 (list_item_size_in_bits * num_items));
    ...
    void array_remove_at(const pointer &ptr_array, unsigned int index) {
        ...
        block_delete(
                pointer(
                        ptr_array.memory(),
                        ptr_array.offset() + _array_start_offset +
                        (index * list_item_size_in_bits)),
                list_item_size_in_bits);
