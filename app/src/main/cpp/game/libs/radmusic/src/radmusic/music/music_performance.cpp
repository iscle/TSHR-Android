...
namespace radmusic {
    ...
    void
    performance_state_name(performance *p, unsigned int index, char *p_string, unsigned int len) {
        return state_name(
                state_array_item_at(
    ...
    void
    performance_state_value_name(performance *p, unsigned int e, unsigned int i, char *p_string,
                                 unsigned int len) {
        state ptr_state = state_array_item_at(
    ...
    void performance_trigger_event(performance *p_performance, unsigned int ei) {
        ...
        p_performance->event_queue[
                (p_performance->event_queue_head + p_performance->event_queue_num_items) %
                EVENT_QUEUE_SIZE] = ei;
    ...
    void _do_action_rand_state(performance *p_performance,
                               const rand_state_action &ptr_rand_state_action) {
    ...
    void _do_event_actions(performance *p_performance, unsigned int i) {
        event ptr_event = event_array_item_at(
                comp_events(p_performance->p_composition->ptr_composition), i);
        ...
        for (unsigned int a = 0; a < num_actions; a++) {
            ...
            switch ((enum_action_type) action_type(ptr_action)) {
                ...
                case action_type_rand_state:
                    _do_action_rand_state(p_performance, (rand_state_action)(pointer)
                    ptr_action);
                    break;
                default:
                    rAssert(false);
