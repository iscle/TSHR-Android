...
//----------------------------------------------------------------------------
    ...
    struct stream_graph_callback : public IRadSoundStitchCallback {
        ...
        virtual void
        OnStitch(IRadSoundHalDataSource **pp_ds, unsigned int frame_count, void *p_user_data) {
...
//----------------------------------------------------------------------------
    ...
    static void
    _on_stitch(stream_graph *p_sg, IRadSoundHalDataSource **pp_ds, unsigned int frame_count) {
    ...
    static void _flush_queue(stream_graph *p_sg) {
        while (p_sg->queue_items > 0) {
            struct_queue_item *p_item =
                    p_sg->queue +
                    ((p_sg->queue_head + p_sg->queue_items - 1) % STREAM_GRAPH_QUEUE_SIZE);
