...
namespace radmusic {
        ...
        p_rc->p_layer_controllers = (layer_controller *)
                memory::malloc(
                        ...
                        layer_ref_array_num_items(
                                region_layer_refs(ptr_region)),
                        "struct_region_controller sequence players");

        for (unsigned int t = 0;
             t < layer_ref_array_num_items(region_layer_refs(p_rc->ptr_region)); t++) {
            ...
            sprintf(sequence_player_name, "struct_region_controller: [%s] layer: [%s]", region_nom,
                    layer_nom);
    ...
    void region_controller_destroy(struct_region_controller *p_rc) {
        for (unsigned int s = 0;
             s < layer_ref_array_num_items(region_layer_refs(p_rc->ptr_region)); s++) {
    ...
    void region_controller_queue_stream(
        ...
        for (unsigned i = 1;
             i < layer_ref_array_num_items(region_layer_refs(p_rc->ptr_region)); i++) {
            sequence_player_queue(
    ...
    void region_controller_kill(struct_region_controller *p_rc) {
        for (unsigned int l = 0;
             l < layer_ref_array_num_items(region_layer_refs(p_rc->ptr_region)); l++) {
