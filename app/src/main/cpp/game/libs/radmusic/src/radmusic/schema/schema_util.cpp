...
//-----------------------------------------------------------------------------
        ...
        if (num_children == 0) {
            ...
            for (unsigned int c = 0; c < num_children; c++) {
                group_memory_sizes_recurse(
                        ...
                        p_num_sound_resources, p_sound_memory_size,
                        p_ptr_group_largest_sound_memory,
                        p_num_cache_resources, p_cache_memory_size,
                        p_ptr_group_largest_cache_memory);
            }
        }
    }

    bool composition_check_constraints(const comp &ptr_composition, char *p_error_msg,
                                       unsigned int len) {
        bool ok = true;
        char sound_msg[2048];
        char cache_msg[2048];
        ...
        if (cacheMemorySize > comp_cache_memory_max(ptr_composition)) {
            sprintf(cache_msg,
                    "Composition uses more cache memory than is budgeted:\n    Budget: H:[0x%x]B D:[%d]B [%.2f]K [%.2f]M\n    Used: H:[0x%x]B D:[%d]B [%.2f]K [%.2f]M\n",
                    comp_cache_memory_max(ptr_composition),
                    comp_cache_memory_max(ptr_composition),
                    radSoundFloatToDouble(comp_cache_memory_max(ptr_composition) / 1024.0f),
                    radSoundFloatToDouble(
                            (float) comp_cache_memory_max(ptr_composition) / (1024.0f * 1024.0f)),
                    cacheMemorySize,
                    cacheMemorySize,
                    radSoundFloatToDouble((float) cacheMemorySize / 1024.0f),
        ...
        if (!ok || !streamsOk) {
            strcpy(p_error_msg, ok && streamsOk ? ""
                                                : "Your comp does not meet minimum memory and/or disk constraints:\n");
    ...
    void group_check_stream_constraints_recursive(
        ...
        for (unsigned int c = 0; c < num_children; c++) {
            ...
            group_check_stream_constraints_recursive(ptr_group_new, min_length_in_ms,
                                                     p_error_string, len, ok);
        }
    }
    ...
    unsigned int rsd_file_length_in_ms(const rsd_file &ptr_rsd_file) {
        ...
        return (rsd_file_length_in_samples(ptr_rsd_file) * 1000) /
               audio_format_sampling_rate(ptr_audio_format);
    }

    unsigned int rsd_file_length_in_samples(const rsd_file &ptr_rsd_file) {
        audio_format ptr_audio_format = rsd_file_audio_format(ptr_rsd_file);
        IRadSoundHalAudioFormat::Encoding e = (IRadSoundHalAudioFormat::Encoding) audio_format_encoding(
                ptr_audio_format);
    ...
    unsigned int rsd_file_length_in_frames(const rsd_file &ptr_rsd_file) {
        ...
        unsigned int frame_size =
                g_audio_format_info[e].mono_8_bit_frame_size_bytes * num_channels * byte_resolution;
    ...
    unsigned int
    audio_format_frames_to_samples(const audio_format &ptr_audio_format, unsigned int frames) {
        IRadSoundHalAudioFormat::Encoding e = (IRadSoundHalAudioFormat::Encoding) audio_format_encoding(
                ptr_audio_format);
    ...
    unsigned int
    audio_format_samples_to_frames(const audio_format &ptr_audio_format, unsigned int samples) {
        IRadSoundHalAudioFormat::Encoding e = (IRadSoundHalAudioFormat::Encoding) audio_format_encoding(
                ptr_audio_format);
    ...
    bool composition_check_stream_constraints(
        ...
        group_check_stream_constraints_recursive(ptr_group_1, comp_stream_size_min(ptr_composition),
                                                 p_error_message, len, &ok);

        return ok;
    }

    ref <IRadSoundHalAudioFormat>
    construct_audio_format(const audio_format &ptr_audio_format, bool targetFormat) {
    ...
    stitch_transition composition_find_stitch_transition(
            ...
            const region &ptr_region_source_in,
            const region &ptr_region_target_in) {
        stitch_transition_array ptr_stitch_transition_array = comp_stitch_transitions(
                ptr_composition);
        unsigned num_stitch_transitions = stitch_transition_array_num_items(
                ptr_stitch_transition_array);
