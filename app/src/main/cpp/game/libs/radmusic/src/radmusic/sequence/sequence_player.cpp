...
namespace radmusic {
    ...
    static unsigned int
    _event_remaining_time_silence(sequence_player *, sequence_player_event *p_e);
    ...
    event_handler g_p_event_handlers[sequence_event_max] =
            {
                    ...
                    /* ods::stream     */
                                                      {_command_stream,       _event_stream,              _event_dump_stream,              _event_reset_stream,  _event_remaining_time_stream},
                    /* silence    */
                                                      {_command_silence,      _event_silence,             _event_dump_silence,             _event_reset_silence, _event_remaining_time_silence},
                    /* var_volume */
                                                      {_command_var,          _event_var_volume,          _event_dump_var_volume,          0,                    0,},
                    /* var_pitch  */
                                                      {_command_var,          _event_var_pitch,           _event_dump_var_pitch,           0,                    0,},
                    /* var_volume_rand_min */
                                                      {_command_var,          _event_var_volume_rand_min, _event_dump_var_volume_rand_min, 0,                    0,},
                    /* var_volume_rand_max */
                                                      {_command_var,          _event_var_volume_rand_max, _event_dump_var_volume_rand_max, 0,                    0,},
                    /* var_pitch_rand_min */
                                                      {_command_var,          _event_var_pitch_rand_min,  _event_dump_var_pitch_rand_min,  0,                    0,},
                    /* var_pitch_rand_max */
                                                      {_command_var,          _event_var_pitch_rand_max,  _event_dump_var_pitch_rand_max,  0,                    0,},
                    /* var_aux_gain */
                                                      {_command_var,          _event_var_aux_gain,        _event_dump_var_aux_gain,        0,                    0,},
                    /* var_positional */
                                                      {_command_var,          _event_var_positional,      _event_dump_var_positional,      0,                    0,},
                    /* var_pos_fall_off */
                                                      {_command_var,          _event_var_pos_fall_off,    _event_dump_var_pos_fall_off,    0,                    0,},
                    /* var_pos_dist_min */
                                                      {_command_var,          _event_var_pos_dist_min,    _event_dump_var_pos_dist_min,    0,                    0,},
                    /* var_pos_dist_max */
                                                      {_command_var,          _event_var_pos_dist_max,    _event_dump_var_pos_dist_max,    0,                    0,},
                    /* callback */
                                                      {_command_var,          _event_var_callback,        _event_dump_var_callback,        0,                    0,},
                    /* logic_and */
                                                      {_command_logic_and,    0,                          0,                               0,                    0,},
                    /* logic_or */
                                                      {_command_logic_or,     0,                          0,                               0,                    0,},
                    /* logic_repeat */
                                                      {_command_logic_repeat, 0,                          0,                               0,                    0,}
            };
...
//----------------------------------------------------------------------------
    ...
    void sequence_player_dequeue(sequence_player *p_sp, stream_graph **pp_sg) {
        ...
        while (_event_queue_window_queued_items(p_sp) > 0) {
            ...
            enum_sequence_event_type id = (enum_sequence_event_type) sequence_event_type(
                    p_e->ptr_sequence_event);
...
//----------------------------------------------------------------------------
    ...
    void _update_event_queue(sequence_player *p_sp) {
        ...
        while (true) {
            ...
            enum_sequence_event_type id_head = (enum_sequence_event_type) sequence_event_type(
                    p_e_head->ptr_sequence_event);
...
//----------------------------------------------------------------------------
    ...
    bool _event_stream(sequence_player *p_sp, sequence_player_event *p_e_head) {
        ...
        if (_event_queue_window_unqueued_items(p_sp) > 0) {
            ...
            enum_sequence_event_type id_window = (enum_sequence_event_type) sequence_event_type(
                    p_e_window->ptr_sequence_event);
        ...
        if (eos) {
            ...
            ods::stream ptr_stream_window = event_stream_stream_ref(
                    (event_stream) p_e_window->ptr_sequence_event);

            if (p_e_window->stream.queued_sample == NULL_SAMPLE) {
                if (p_sp->p_stream_graph == 0 ||
                    stream_graph_queue_full(p_sp->p_stream_graph) == false) {
                    audio_format ptr_audio_format = rsd_file_audio_format(
                    ...
                    unsigned int stream_frames_to_play =
                            stream_total_length_in_frames - stream_start_frame;
        ...
        if (stream_graph_state(p_sp->p_stream_graph) == stream_graph_state_running) {
            ...
            {
                unsigned int stream_total_samples = rsd_file_length_in_samples(
                        stream_rsd_file_ref(ptr_stream_head));
                unsigned int stream_samples_to_play =
                        stream_total_samples - p_e_head->stream.start_sample;
                ...
                if (current_pos >= p_e_head->stream.queued_sample + stream_samples_to_play) {
                    ...
                    p_e_head->stream.current_sample = current_pos - p_e_head->stream.queued_sample +
                                                      p_e_head->stream.start_sample;

                    p_sp->beat = tempo_track_beat(
                            stream_tempo_track(ptr_stream_head),
                            p_e_head->stream.current_sample,
                            audio_format_sampling_rate(
                                    rsd_file_audio_format(stream_rsd_file_ref(ptr_stream_head))));
...
//----------------------------------------------------------------------------
    ...
    static bool _event_var_aux_gain(sequence_player *p_sp, sequence_player_event *p_e) {
        ...
        unsigned char aux_number = event_var_aux_gain_aux_number(
                (event_var_aux_gain) p_e->ptr_sequence_event);
...
//----------------------------------------------------------------------------
    ...
    static unsigned int
    _event_remaining_time_clip(sequence_player *p_sp, sequence_player_event *p_e) {
...
//----------------------------------------------------------------------------
    ...
    static unsigned int
    _event_remaining_time_stream(sequence_player *p_sp, sequence_player_event *p_e) {
        ods::stream ptr_stream = event_stream_stream_ref((event_stream) p_e->ptr_sequence_event);

        unsigned int length_in_samples = rsd_file_length_in_samples(
...
//----------------------------------------------------------------------------
    ...
    sequence_player_event *_event_queue_window_peek(sequence_player *p_sp) {
        if (p_sp->event_queue_window_items == p_sp->event_queue_items) {
            ...
            event_queue_type i = _event_queue_wrap(
                    p_sp->event_queue_head + p_sp->event_queue_window_items);
...
//----------------------------------------------------------------------------
    ...
    void _event_dump_var_positional(sequence_player *p_sp, sequence_player_event *p_e) {
        bool positional = event_var_positional_positional(
                (event_var_positional) p_e->ptr_sequence_event);
...
//----------------------------------------------------------------------------
    ...
    void _event_dump_var_pos_fall_off(sequence_player *p_sp, sequence_player_event *p_e) {
        float pos_fall_off = event_var_pos_fall_off_pos_fall_off(
                (event_var_pos_fall_off) p_e->ptr_sequence_event);
...
//----------------------------------------------------------------------------
    ...
    void _event_dump_var_pos_dist_min(sequence_player *p_sp, sequence_player_event *p_e) {
        float pos_dist_min = event_var_pos_dist_min_pos_dist_min(
                (event_var_pos_dist_min) p_e->ptr_sequence_event);
...
//----------------------------------------------------------------------------
    ...
    void _event_dump_var_pos_dist_max(sequence_player *p_sp, sequence_player_event *p_e) {
        float pos_dist_max = event_var_pos_dist_max_pos_dist_max(
                (event_var_pos_dist_max) p_e->ptr_sequence_event);
