...
public:
    ...
    virtual void Initialize(tBaseEmitter *emitter) {
        ...
        if (!emitter->GetFactory()->GetInfiniteLife()) {
            lifeInit = rmt::Max(ParticleSystemRandomFloat(emitter->currLife, emitter->currLifeVar),
                                0.0f);
        } else {
            lifeInit = -1.0f;
        }
        weight = weightInit = rmt::Max(
                ParticleSystemRandomFloat(emitter->currWeight, emitter->currWeightVar), 0.0f);
...
public:
    ...
    virtual void Initialize(tBaseEmitter *emitter) {
        ...
        switch (spriteEmitterFactory->GetTextureAnimationMode()) {
            ...
            case p3dParticleSystemConstants::TextureAnimMode::RANDOM: {
                ...
                if (numTexFrames > 1) {
                    texFrameInit = texFrame = rmt::FtoL(
                            ((float) numTexFrames) * ParticleSystemRandomFloat(0.5f, 0.5f));
                } else {
        ...
        size = sizeInit = rmt::Max(
                ParticleSystemRandomFloat(spriteEmitter->currSize, spriteEmitter->currSizeVar),
                0.0f);
        switch (spriteEmitterFactory->GetAngleMode()) {
            case p3dParticleSystemConstants::ParticleAngleMode::RANDOM: {
                spinInit = ParticleSystemRandomFloat(spriteEmitter->currSpin,
                                                     spriteEmitter->currSpinVar);
    ...
    virtual void Update(float deltaTime, float deltaFrame, tBaseEmitter *emitter) {
        ...
        if (life <= lifeInit) {
            if (spriteEmitterFactory->GetTextureAnimationMode() ==
                ...
                texFrame =
                        rmt::FtoL(life / rmt::LtoF(spriteEmitterFactory->GetTextureFrameRate())) +
                        texFrameInit;
