    ...
    if (numAnimations > 0) {
        ...
        for (unsigned int i = 0; i < numAnimations; i++) {
            ...
            if (poseControllers[i] != -1) {
                ((tPoseAnimationController *) object->animations[i]->controllers[poseControllers[i]])->SetPose(
                        object->baseObject->GetPose());
            }
            ...
            if (visibilityControllers[i] != -1) {
                ((tCompDrawVisibilityAnimationController * )(
                        object->animations[i]->controllers[visibilityControllers[i]]))->SetDrawable(
                        object->baseObject);
            }
            ...
            for (unsigned int j = 0; j < numEffects[i]; j++) {
                ...
                if (i == 0) {
                    ...
                    tEffect *origObj = (tEffect *) object->baseObject->GetDrawableElement(
                            effects[i][j])->GetDrawable();
                    // create the effect (and attach it to it's new controller
                    tEffect *newObj = origObj->GetFactory()->CreateEffect((tEffectController * )(
                            object->animations[i]->controllers[effectControllers[i][j]]));
                    ...
                    ((tEffectController * )(
                            object->animations[i]->controllers[effectControllers[i][j]]))->SetEffect(
                            (tEffect *) object->baseObject->GetDrawableElement(
                                    effects[i][j])->GetDrawable());
                }
            }
        }
        ...
        switch (f->BeginChunk()) {
            case Pure3D::AnimatedObject::ANIMATION: {
                if (currAnim < factory->numAnimations) {
                    ...
                    if (animation) {
                        for (unsigned int i = 0; i < animation->numControllers; i++) {
                            if ((factory->poseControllers[currAnim] == -1) &&
                                (dynamic_cast<tPoseAnimationController *>(animation->controllers[i]))) {
                                factory->poseControllers[currAnim] = i;
                                continue;
                            }
                            if ((factory->visibilityControllers[currAnim] == -1) &&
                                (dynamic_cast<tCompDrawVisibilityAnimationController *>(animation->controllers[i]))) {
                                factory->visibilityControllers[currAnim] = i;
                                continue;
                            }
                            tEffectController *ec = dynamic_cast<tEffectController *>(animation->controllers[i]);
                            if (ec) {
                                effects[factory->numEffects[currAnim]] = factory->baseObject->FindNodeIndex(
                                        ec->GetEffect()->GetUID());
                        ...
                        memcpy(factory->effects[currAnim], effects,
                               factory->numEffects[currAnim] * sizeof(int));
                        memcpy(factory->effectControllers[currAnim], effectControllers,
                               factory->numEffects[currAnim] * sizeof(int));
...
//-------------------------------------------------------------------------
tAnimatedObjectAnimation *
tAnimatedObjectAnimationLoader::LoadObject(tChunkFile *f, tEntityStore *store) {
    unsigned int version = f->GetLong();
    ...
    tFrameControllerLoader *loader = dynamic_cast<tFrameControllerLoader *>(p3d::loadManager->GetP3DHandler()->GetHandler(
    ...
    for (unsigned int i = 0; i < animation->numControllers; i++) {
        ...
        if (ac) {
            ...
            if (anim) {
                animation->numFrames =
                        anim->GetNumFrames() > animation->numFrames ? anim->GetNumFrames()
                                                                    : animation->numFrames;
            }
        }
    }

    rWarningMsg(currController == animation->numControllers,
                "Warning: controllers missing for animated object");
