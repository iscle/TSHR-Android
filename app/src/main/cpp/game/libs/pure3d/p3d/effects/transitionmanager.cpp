...
// The display code is directly taken from the Scrooby FeScreen::Display()
    ...
    aspect = static_cast<float>(p3d::display->GetWidth()) /
             static_cast<float>(p3d::display->GetHeight());
...
// Draw our screen overlay
    ...
    pddiPrimStream *stream = p3d::pddi->BeginPrims(mShader->GetShader(), PDDI_PRIM_TRIANGLES,
                                                   PDDI_V_CT, mXSize * mYSize * 6);
    int i = 0;
    for (i; i < mXSize * mYSize; i++) {
        stream->Vertex(&mPositions[i].tri[0].point[0].pos, mColour,
                       &mPositions[i].tri[0].point[0].uv);
        stream->Vertex(&mPositions[i].tri[0].point[1].pos, mColour,
                       &mPositions[i].tri[0].point[1].uv);
        stream->Vertex(&mPositions[i].tri[0].point[2].pos, mColour,
                       &mPositions[i].tri[0].point[2].uv);
        stream->Vertex(&mPositions[i].tri[1].point[0].pos, mColour,
                       &mPositions[i].tri[1].point[0].uv);
        stream->Vertex(&mPositions[i].tri[1].point[1].pos, mColour,
                       &mPositions[i].tri[1].point[1].uv);
        stream->Vertex(&mPositions[i].tri[1].point[2].pos, mColour,
                       &mPositions[i].tri[1].point[2].uv);
    }
    ...
    if (!mPositions) {
        ...
        for (y; y < mYSize; y++) {
            ...
            for (x; x < mXSize; x++) {
                ...
                mPositions[y * mYSize + x].tri[0].point[1].pos.Set((x + 1) * xUnit, y * yUnit,
                                                                   0.0f);
                mPositions[y * mYSize + x].tri[0].point[1].uv.Set((x + 1) * xUnit,
                                                                  (mYSize - y) * vUnit);
                mPositions[y * mYSize + x].tri[0].point[2].pos.Set(x * xUnit, (y + 1) * yUnit,
                                                                   0.0f);
                mPositions[y * mYSize + x].tri[0].point[2].uv.Set(x * xUnit,
                                                                  (mYSize - y - 1) * vUnit);

                mPositions[y * mYSize + x].tri[1].point[0].pos.Set((x + 1) * xUnit, y * yUnit,
                                                                   0.0f);
                mPositions[y * mYSize + x].tri[1].point[0].uv.Set((x + 1) * xUnit,
                                                                  (mYSize - y) * vUnit);
                mPositions[y * mYSize + x].tri[1].point[1].pos.Set(x * xUnit, (y + 1) * yUnit,
                                                                   0.0f);
                mPositions[y * mYSize + x].tri[1].point[1].uv.Set(x * xUnit,
                                                                  (mYSize - y - 1) * vUnit);
                mPositions[y * mYSize + x].tri[1].point[2].pos.Set((x + 1) * xUnit, (y + 1) * yUnit,
                                                                   0.0f);
                mPositions[y * mYSize + x].tri[1].point[2].uv.Set((x + 1) * xUnit,
                                                                  (mYSize - y - 1) * vUnit);
            }
        }
    }
    switch (mType) {
        ...
        case TRANSITION_EXPLODE: {
            ...
            for (i; i < mYSize * mXSize; i++) {
                ...
                mPositions[i].tri[0].point[0].pos.x +=
                        (mPositions[i].tri[0].point[0].pos.x - 0.49f) * percent + rand;
                mPositions[i].tri[0].point[1].pos.x +=
                        (mPositions[i].tri[0].point[0].pos.x - 0.49f) * percent + rand;
                mPositions[i].tri[0].point[2].pos.x +=
                        (mPositions[i].tri[0].point[0].pos.x - 0.49f) * percent + rand;
                rand = mRand.Float();
                mPositions[i].tri[1].point[0].pos.x +=
                        (mPositions[i].tri[0].point[0].pos.x - 0.49f) * percent + rand;
                mPositions[i].tri[1].point[1].pos.x +=
                        (mPositions[i].tri[0].point[0].pos.x - 0.49f) * percent + rand;
                mPositions[i].tri[1].point[2].pos.x +=
                        (mPositions[i].tri[0].point[0].pos.x - 0.49f) * percent + rand;

                rand = mRand.Float();
                mPositions[i].tri[0].point[0].pos.y +=
                        (mPositions[i].tri[0].point[0].pos.y - 0.49f) * percent + rand;
                mPositions[i].tri[0].point[1].pos.y +=
                        (mPositions[i].tri[0].point[0].pos.y - 0.49f) * percent + rand;
                mPositions[i].tri[0].point[2].pos.y +=
                        (mPositions[i].tri[0].point[0].pos.y - 0.49f) * percent + rand;
                rand = mRand.Float();
                mPositions[i].tri[1].point[0].pos.y +=
                        (mPositions[i].tri[0].point[0].pos.y - 0.49f) * percent + rand;
                mPositions[i].tri[1].point[1].pos.y +=
                        (mPositions[i].tri[0].point[0].pos.y - 0.49f) * percent + rand;
                mPositions[i].tri[1].point[2].pos.y +=
                        (mPositions[i].tri[0].point[0].pos.y - 0.49f) * percent + rand;
            }
        }
            break;
        case TRANSITION_SHATTER: {
            ...
            for (i; i < mYSize * mXSize; i++) {
                ...
                rand = (mRand.Float() - 0.005f);
                mPositions[i].tri[0].point[0].pos.y +=
                        ((mPositions[i].tri[0].point[0].pos.y - 1.0f) * percent) + rand;
                mPositions[i].tri[0].point[1].pos.y +=
                        ((mPositions[i].tri[0].point[0].pos.y - 1.0f) * percent) + rand;
                mPositions[i].tri[0].point[2].pos.y +=
                        ((mPositions[i].tri[0].point[0].pos.y - 1.0f) * percent) + rand;
