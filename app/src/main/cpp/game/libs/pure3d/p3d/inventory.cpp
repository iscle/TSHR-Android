...
void tInventory::SetSectionPath(const char *p0, const char *p1, const char *p2, const char *p3,
    ...
    nSectionPath = 0;
    if (p0) {
        sectionPath[0] = tEntity::MakeUID(p0);
        nSectionPath++;
    }
    if (p1) {
        sectionPath[1] = tEntity::MakeUID(p1);
        nSectionPath++;
    }
    if (p2) {
        sectionPath[2] = tEntity::MakeUID(p2);
        nSectionPath++;
    }
    if (p3) {
        sectionPath[3] = tEntity::MakeUID(p3);
        nSectionPath++;
    }
    if (p4) {
        sectionPath[4] = tEntity::MakeUID(p4);
        nSectionPath++;
    }
    if (p5) {
        sectionPath[5] = tEntity::MakeUID(p5);
        nSectionPath++;
    }
    if (p6) {
        sectionPath[6] = tEntity::MakeUID(p6);
        nSectionPath++;
    }
    if (p7) {
        sectionPath[7] = tEntity::MakeUID(p7);
        nSectionPath++;
    }
}
...
tEntity *
tInventory::SearchSection(unsigned s, radLoadInventory::SafeCastBase &caster, const tName &name) {
