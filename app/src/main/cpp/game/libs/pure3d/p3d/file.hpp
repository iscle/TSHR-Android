...
public:
    ...
    inline P3D_U8 GetByte(void) {
        P3D_U8 tmp;
        GetData(&tmp, 1, BYTE);
        return tmp;
    }

    inline P3D_U16 GetWord(void) {
        P3D_U16 tmp;
        GetData(&tmp, 1, WORD);
        return tmp;
    }

    inline P3D_U32 GetDWord(void) {
        P3D_U32 tmp;
        GetData(&tmp, 1, DWORD);
        return tmp;
    }

    inline P3D_U64 GetQWord(void) {
        P3D_U64 tmp;
        GetData(&tmp, 1, QWORD);
        return tmp;
    }

    inline bool Read(void *data, unsigned count, unsigned size) {
        return GetData(data, count, static_cast<DataType>(size));
    }
...
protected:
    ...
    void UncompressBlock(unsigned char *input, unsigned int inputsize, unsigned char *output,
...
public :
    ...
    virtual bool SetEndianSwap(bool swap) {
        m_endianSwap = swap;
        return dataStream->SetEndianSwap(swap);
    }
