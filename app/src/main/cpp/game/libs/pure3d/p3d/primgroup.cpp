...
void tPrimGroupStreamed::Display() {
    ...
    for (int i = 0; i < numPasses; ++i) {
        pddiPrimStream *stream = p3d::pddi->BeginPrims(mShader->GetShader(), mPrimType,
                                                       vertexFormat, (mIndexCount > 0) ? mIndexCount
                                                                                       : mVertexList->GetNumVertices(),
                                                       i);
...
//---------------------------------------------------------------
tPrimGroupSkinnedStreamed::tPrimGroupSkinnedStreamed(int nVertex, unsigned format, int nIndex,
                                                     bool allocate) :
...
void tPrimGroupSkinnedStreamed::Display(void) {
    ...
    if (mVertexFormat & PDDI_V_NORMAL) {
        for (int i = 0; i < count; i++) {
            ...
            normal.x =
                    m->m[0][0] * oldNormal.x + m->m[1][0] * oldNormal.y + m->m[2][0] * oldNormal.z;
            normal.y =
                    m->m[0][1] * oldNormal.x + m->m[1][1] * oldNormal.y + m->m[2][1] * oldNormal.z;
            normal.z =
                    m->m[0][2] * oldNormal.x + m->m[1][2] * oldNormal.y + m->m[2][2] * oldNormal.z;
            ...
            if (weight != 1.0f) {
                ...
                vertex.x +=
                        (m->m[0][0] * sv.x + m->m[1][0] * sv.y + m->m[2][0] * sv.z + m->m[3][0]) *
                        weight;
                vertex.y +=
                        (m->m[0][1] * sv.x + m->m[1][1] * sv.y + m->m[2][1] * sv.z + m->m[3][1]) *
                        weight;
                vertex.z +=
                        (m->m[0][2] * sv.x + m->m[1][2] * sv.y + m->m[2][2] * sv.z + m->m[3][2]) *
                        weight;
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (weight != 1.0f) {
                weight = verts[i].weights[1];
                m = matrixPalette[verts[i].indices[1]];
                vertex.x += (m->m[0][0] * sv->x + m->m[1][0] * sv->y + m->m[2][0] * sv->z +
                             m->m[3][0]) * weight;
                vertex.y += (m->m[0][1] * sv->x + m->m[1][1] * sv->y + m->m[2][1] * sv->z +
                             m->m[3][1]) * weight;
                vertex.z += (m->m[0][2] * sv->x + m->m[1][2] * sv->y + m->m[2][2] * sv->z +
                             m->m[3][2]) * weight;
...
void tPrimGroupSkinnedPC::Display(void) {
    ...
    if (mVertexFormat & PDDI_V_NORMAL) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            // note that we only use the first bone index - normals aren't weighted
            Vector normal;
            normal.x = m->m[0][0] * verts[i].normal.x + m->m[1][0] * verts[i].normal.y +
                       m->m[2][0] * verts[i].normal.z;
            normal.y = m->m[0][1] * verts[i].normal.x + m->m[1][1] * verts[i].normal.y +
                       m->m[2][1] * verts[i].normal.z;
            normal.z = m->m[0][2] * verts[i].normal.x + m->m[1][2] * verts[i].normal.y +
                       m->m[2][2] * verts[i].normal.z;
...
template<class T>
inline static void
IndexedDisplayInternal(tVertexList *vertexList, T *stream, unsigned short *indices, int nIndex) {
    int i;
    ...
    switch (vertexFormat) {
        ...
        default:
            ...
            for (i = 0; i < nIndex; i++) {
                if (normal)
                    stream->Normal(normal[indices[i]].x, normal[indices[i]].y,
                                   normal[indices[i]].z);
                int j;
...
template<class T>
inline static void
IndexedDisplayWithOffsetInternal(tVertexList *vertexList, T *stream, unsigned short *indices,
                                 int nIndex) {
    int i;
    ...
    pddiVector *op = (pddiVector * )(
            offset->mMask & tVertexOffsetList::eOffsetCoord ? offset->mCoord : NULL);
    tColourOffset *oc = (tColourOffset * )(
            offset->mMask & tVertexOffsetList::eOffsetColour ? offset->mColour : NULL);
    pddiVector *on = (pddiVector * )(
            offset->mMask & tVertexOffsetList::eOffsetNormal ? offset->mNormal : NULL);
    pddiVector2 *ot0 = (pddiVector2 * )(
            offset->mMask & tVertexOffsetList::eOffsetUV0 ? offset->mUV[0] : NULL);
    pddiVector2 *ot1 = (pddiVector2 * )(
            offset->mMask & tVertexOffsetList::eOffsetUV1 ? offset->mUV[1] : NULL);
    pddiVector2 *ot2 = (pddiVector2 * )(
            offset->mMask & tVertexOffsetList::eOffsetUV2 ? offset->mUV[2] : NULL);
    pddiVector2 *ot3 = (pddiVector2 * )(
            offset->mMask & tVertexOffsetList::eOffsetUV3 ? offset->mUV[3] : NULL);
    ...
    switch (vertexFormat) {
        ...
        case PDDI_V_CT:
            if (oc && ot0 && op) {         //with color, uv and position offset
                ...
                for (i = 0; i < nIndex; i++) {
                    ...
                    stream->Vertex(&p[indices[i]], c0[indices[i]], /* &n[indices[i]], */
                                   &t0[indices[i]]);
                }
            }
        ...
        default:
            ...
            for (i = 0; i < nIndex; i++) {
                if (n) {
                    if (on)
                        stream->Normal(n[indices[i]].x + on[indices[i]].x,
                                       n[indices[i]].y + on[indices[i]].y,
                                       n[indices[i]].z + on[indices[i]].z);
                    else
                        stream->Normal(n[indices[i]].x, n[indices[i]].y, n[indices[i]].z);
                }
                ...
                if (t0) {
                    if (ot0)
                        stream->UV(t0[indices[i]].u + ot0[indices[i]].u,
                                   t0[indices[i]].v + ot0[indices[i]].v, 0);
                    else
                        stream->UV(t0[indices[i]].u, t0[indices[i]].v, 0);
                }
                ...
                if (t1) {
                    if (ot1)
                        stream->UV(t1[indices[i]].u + ot1[indices[i]].u,
                                   t1[indices[i]].v + ot1[indices[i]].v, 1);
                    else
                ...
                if (t2) {
                    if (ot2)
                        stream->UV(t2[indices[i]].u + ot2[indices[i]].u,
                                   t2[indices[i]].v + ot2[indices[i]].v, 2);
                    else
                ...
                if (t3) {
                    if (ot3)
                        stream->UV(t3[indices[i]].u + ot3[indices[i]].u,
                                   t3[indices[i]].v + ot3[indices[i]].v, 3);
                    else
                        stream->UV(t3[indices[i]].u, t3[indices[i]].v, 3);
                }

                if (op)
                    stream->Coord(p[indices[i]].x + op[indices[i]].x,
                                  p[indices[i]].y + op[indices[i]].y,
                                  p[indices[i]].z + op[indices[i]].z);
                else
                    stream->Coord(p[indices[i]].x, p[indices[i]].y, p[indices[i]].z);
...
tPrimGroup *tPrimGroupLoader::LoadPCSkin(tChunkFile *f, tEntityStore *store, rmt::Matrix *bones) {
    ...
    mVertexFormat =
            mVertexFormat & ~(PDDI_V_WEIGHTS | PDDI_V_INDICES | PDDI_V_BINORMAL | PDDI_V_TANGENT);
    ...
    while (f->ChunksRemaining()) {
        ...
        switch (f->GetCurrentID()) {
            ...
            case Pure3D::Mesh::INDEXLIST: {
                ...
                unsigned short *tempIndexList = (unsigned short *) p3d::MallocTemp(
...
tPrimGroup *tPrimGroupLoader::LoadStreamed(tChunkFile *f, tEntityStore *store, rmt::Matrix *bones) {
    ...
    if (bones) {
        ...
        skinPrimGroup = new tPrimGroupSkinnedStreamed(mVertexCount, mVertexFormat, mIndexCount,
                                                      true);
        primGroup = skinPrimGroup;
...
//-------------------------------------------------------------------------------------------------------------------
tPrimGroup *
tPrimGroupLoader::Load(tChunkFile *f, tEntityStore *store, rmt::Matrix *bones, bool optimise,
                       bool deform) {
    pddiExtHardwareSkinning *hwSkin = p3d::context->GetHardwareSkinning();
