...
public:
    ...
    void
    SetClearMask(unsigned mask) { clearMask = mask; } // some combination of PDDI_BUFFER_* flags
    ...
    void SetFogPlanes(float fnear, float ffar) {
        fogNear = fnear;
        fogFar = ffar;
    }

    void GetFogPlanes(float *fnear, float *ffar) {
        *fnear = fogNear;
        *ffar = fogFar;
    }
