...
tImageFont::DisplayLine(tImage *dest, int x, int y, const P3D_UNICODE *text, int formatting) const {
    ...
    while ((text[currChar]) && (text[currChar] != (P3D_UNICODE) '\n')) {
        ...
        dest->CopyRegion(image, glyphs[currGlyph].xOrigin, 0, glyphs[currGlyph].width, fontHeight,
                         x, y);
...
tImageFont::FindGlyph(P3D_UNICODE code) const {
    ...
    while (((endPos - startPos) > 1) &&
           ((glyphs[startPos].code < code) && (glyphs[endPos].code > code))) {
...
tImageFontLoader::LoadObject(tChunkFile *f, tEntityStore *store) {
    ...
    tImageLoader *loader = dynamic_cast<tImageLoader *>(p3d::loadManager->GetP3DHandler()->GetHandler(
