...
namespace p3d {
    ...
    inline bool load(unsigned char *buffer, unsigned size, unsigned pool = 0,
                     const char *filename = "tmp.p3d") {
    ...
    inline tLoadRequest *
    loadAsync(const char *name, const char *section = NULL, tLoadRequest::Callback *done = NULL,
              void *user = NULL, unsigned pool = 0) {
        radMemoryAllocator old = ::radMemorySetCurrentAllocator(RADMEMORY_ALLOC_TEMP);
    ...
    inline tLoadRequest *
    loadAsync(tFile *file, const char *section = NULL, tLoadRequest::Callback *done = NULL) {
        tLoadRequest *load = new tLoadRequest(file);
        load->SetAsync(true);
        load->SetInventorySection(section);
        load->SetCallback(done);
    ...
    template<class T>
    inline T *find(tEntityStore *inventory, const tName &name) {
        if (name.GetUID() == tName::MakeUID("") ||
            name.GetUID() == tName::MakeUID("NO_NAME_TEXT")) {
            return NULL;
        } else {
            T *obj = NULL;
    ...
    template<class T>
    inline T *find(tEntityStore *inventory, const char *name) {
        tName tname = name;
