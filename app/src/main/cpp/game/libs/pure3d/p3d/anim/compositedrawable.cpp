        ...
        if (elements[i]) {
            switch (elements[i]->GetType()) {
                case DrawableElement::PROP_ELEMENT: {
                    ...
                    DrawablePropElement *newProp = compDraw->AddProp(prop->GetDrawable(),
                                                                     prop->poseIndex);
                ...
                case DrawableElement::SKIN_ELEMENT: {
                    ...
                    DrawablePoseElement *newPose = compDraw->AddPose(
                            (tDrawablePose *) pose->GetDrawable());
                ...
                case DrawableElement::EFFECT_ELEMENT: {
                    ...
                    DrawableEffectElement *newEffect = compDraw->AddEffect(
                            (tEffect *) effect->GetDrawable(), effect->poseIndex);
        ...
        if (elements[i]) {
            ...
            if (elements[i]->IsVisible()) {
                if (elements[i]->IsTranslucent()) {
                    ...
                    //    elements[i]->Display();
                    //else 
                    translucentObjects.Add(elements[i]->GetDrawable(),
                                           elements[i]->GetWorldMatrix(), elements[i]->SortOrder());
                } else {
                    elements[i]->Display();
                }
            }
...
tCompositeDrawable::AddProp(tDrawable *prop, int poseIndex) {
...
// For effects
//
tCompositeDrawable::DrawableEffectElement *
tCompositeDrawable::AddEffect(tEffect *effect, int poseIndex) {
...
tEntity *tCompositeDrawableLoader::LoadObject(tChunkFile *f, tEntityStore *store) {
        ...
        if (f->GetCurrentID() == P3D_COMPOSITE_DRAWABLE_SKIN_LIST) {
            ...
            for (int i = 0; i < NumElements; i++) {
                ...
                if (f->GetCurrentID() == P3D_COMPOSITE_DRAWABLE_SKIN) {
                    ...
                    if (skin) {
                        tCompositeDrawable::DrawablePoseElement *compDrawSkin = compositeDraw->AddPose(
                                skin);
                        ...
                        compositeDraw->boundingBox.high.x = rmt::Max((skinBox.high.x + mid.x),
                                                                     (compositeDraw->boundingBox.high.x));
                        compositeDraw->boundingBox.high.y = rmt::Max((skinBox.high.y + mid.y),
                                                                     (compositeDraw->boundingBox.high.y));
                        compositeDraw->boundingBox.high.z = rmt::Max((skinBox.high.z + mid.z),
                                                                     (compositeDraw->boundingBox.high.z));

                        compositeDraw->boundingBox.low.x = rmt::Min((skinBox.low.x + mid.x),
                                                                    (compositeDraw->boundingBox.low.x));
                        compositeDraw->boundingBox.low.y = rmt::Min((skinBox.low.y + mid.y),
                                                                    (compositeDraw->boundingBox.low.y));
                        compositeDraw->boundingBox.low.z = rmt::Min((skinBox.low.z + mid.z),
                                                                    (compositeDraw->boundingBox.low.z));
            ...
            for (int i = 0; i < NumElements; i++) {
                ...
                if (f->GetCurrentID() == P3D_COMPOSITE_DRAWABLE_EFFECT) {
                    ...
                    if (effect) {
                        tCompositeDrawable::DrawableEffectElement *compDrawEffect = compositeDraw->AddEffect(
                                effect, skeletonID);
                        ...
                        compositeDraw->boundingBox.high.x = rmt::Max(v,
                                                                     compositeDraw->boundingBox.high.x);
                        v = rmt::Min(effectBox.low.x, effectBox.high.x);
                        compositeDraw->boundingBox.low.x = rmt::Min(v,
                                                                    compositeDraw->boundingBox.low.x);
                        ...
                        compositeDraw->boundingBox.high.y = rmt::Max(v,
                                                                     compositeDraw->boundingBox.high.y);
                        v = rmt::Min(effectBox.low.y, effectBox.high.y);
                        compositeDraw->boundingBox.low.y = rmt::Min(v,
                                                                    compositeDraw->boundingBox.low.y);
                        ...
                        compositeDraw->boundingBox.high.z = rmt::Max(v,
                                                                     compositeDraw->boundingBox.high.z);
                        v = rmt::Min(effectBox.low.z, effectBox.high.z);
                        compositeDraw->boundingBox.low.z = rmt::Min(v,
                                                                    compositeDraw->boundingBox.low.z);
            ...
            for (int i = 0; i < NumElements; i++) {
                ...
                if (f->GetCurrentID() == P3D_COMPOSITE_DRAWABLE_PROP) {
                    ...
                    if (prop) {
                        tCompositeDrawable::DrawablePropElement *compDrawProp = compositeDraw->AddProp(
                                prop, skeletonID);
    ...
    // really hacky sphere 
    compositeDraw->boundingSphere.centre.x =
            (compositeDraw->boundingBox.high.x + compositeDraw->boundingBox.low.x) / 2;
    compositeDraw->boundingSphere.centre.y =
            (compositeDraw->boundingBox.high.y + compositeDraw->boundingBox.low.y) / 2;
    compositeDraw->boundingSphere.centre.z =
            (compositeDraw->boundingBox.high.z + compositeDraw->boundingBox.low.z) / 2;
    rmt::Vector v;
