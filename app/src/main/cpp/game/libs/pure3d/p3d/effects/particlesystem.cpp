...
void tParticleSystem::Update(const rmt::Matrix *worldMatrix) {
    ...
    if (((cycleMode == FORCE_CYCLIC) ||
         ((cycleMode == DEFAULT_CYCLE_MODE) && (factory->GetCycleAnim()))) &&
        (newFrame >= endFrame)) {
        numCycles++;
    }
...
void tParticleSystem::SortEmitters() {
    ...
    for (i = 0; i < numEmitters; i++) {
        ...
        zValues[i] = worldToCamera.m[0][2] * tempPos.x + worldToCamera.m[1][2] * tempPos.y +
                     worldToCamera.m[2][2] * tempPos.z + worldToCamera.m[3][2];
    }
