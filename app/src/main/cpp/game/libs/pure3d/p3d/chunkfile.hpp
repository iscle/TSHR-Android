...
public:
    ...
    bool GetData(void *buf, unsigned count,
                 tFile::DataType type = tFile::BYTE) { return realFile->GetData(buf, count, type); }

    virtual bool Read(void *data, unsigned count, unsigned size) {
        return GetData(data, count, static_cast<tFile::DataType>(size));
    }
