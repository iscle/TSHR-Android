...
tTexture *tImageConverter::ImageToTexture(tImage *image, bool linear) {
    ...
    if (!texture->Create(image->GetWidth(), image->GetHeight(), image->GetDepth(),
                         image->GetAlphaDepth(), 0, requestedType, PDDI_USAGE_STATIC)) {
...
//-------------------------------------------------------------------
tTexture *tImageConverter::ImageInToVolumeTexture(tImage **images, int numImages, tTexture *texture,
                                                  int mipLevel) {
#ifdef RAD_XBOX
    P3DASSERT(texture);
...
tTexture *tImageConverter::ImageToVolumeTexture(tImage **images, int numImages, int numMipMaps,
...
void tImageConverter::UpdateSurface(tImage *image, pddiLockInfo *lock) {
    ...
    switch (format) {
        ...
        default:
            P3DVERIFY(false, "Pixel format for texture is not supported, ",
                      image->GetNameDangerous());
            break;
    }
}
        ...
        case 16: {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    unsigned r =
                            ((ri >> lock->rgbaLShift[0]) << lock->rgbaRShift[0]) | (ri >> rFrac);
                    unsigned gi = pixel & lock->rgbaMask[1];
                    unsigned g =
                            ((gi >> lock->rgbaLShift[1]) << lock->rgbaRShift[1]) | (gi >> gFrac);
                    unsigned bi = pixel & lock->rgbaMask[2];
                    unsigned b =
                            ((bi >> lock->rgbaLShift[2]) << lock->rgbaRShift[2]) | (bi >> bFrac);
