...
void tMultiController::SetFrame(float f) {
    ...
    if ((f != 0.0f) && first) {
        p3d::print(
                "WARNING : tMultiController::SetFrame was called, results are probably not what you expect!\n");
        first = false;
    }
...
void tMultiController::SetFrameRange(float minframe, float maxframe) {
    static bool first = true;
    p3d::print(
            "WARNING : tMultiController::SetFrameRange was called, results are probably not what you expect!\n");
    first = false;
