...
static void LoadTargaTRUECOLOUR(TargaHeader &header, tFile *file, tImageHandler::Builder *builder) {
    builder->BeginImage(header.width, header.height, header.bpp, tImageHandler::Builder::BOTTOM,
                        NULL);
...
void tTargaHandler::CreateImage(tFile *file, tImageHandler::Builder *builder) {
    ...
    if (header.imageType != TGA_UNCOMPRESSED_CMAP &&
        ...
        char buf[256];
        sprintf(buf,
                "Attempted to load an unsuported TARGA format from file %s\n (Ignore will continue safely)",
                file->GetFilename());
    ...
    } else if (header.imageType == TGA_UNCOMPRESSED_TRUECOLOUR) {
