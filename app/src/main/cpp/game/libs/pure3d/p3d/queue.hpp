...
protected:
    ...
    void TrickleDown(unsigned node) {
        ...
        if ((buffer[node].priority > buffer[LeftChild(node)].priority) &&
            (buffer[node].priority > buffer[RightChild(node)].priority))
            return;
...
template<class T>
class tOrderedPriorityQueue : public tPriorityQueue<T> {
public:
    ...
    void Enqueue(T t, int priority = 0) {
        ...
        if (sequenceNumber == 0) {
            p3d::print(
                    "warning : sequence number wrapped in tOrderedPriorityQuere, bad ordering may result\n");
