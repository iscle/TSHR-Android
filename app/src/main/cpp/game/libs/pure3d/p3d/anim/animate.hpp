...
public:
    ...
    public:
        ...
        virtual tFrameController *
        LoadFC(char *name, unsigned type, tChunkFile *file, tEntityStore *store) = 0;
    };
...
protected:
    ...
    virtual tFrameController *
    LoadFC(char *name, unsigned type, tChunkFile *file, tEntityStore *store);
...
public:
    ...
    float MakeValidFrame(float frame, float rangeMin = -1.0f, float rangeMax = -1.0f,
                         p3dCycleMode mode = DEFAULT_CYCLE_MODE) const;
...
protected:
    ...
    tAnimationGroup *
    LoadAnimationGroup(tChunkFile *file, tEntityStore *store, tAnimationMemoryBlock *block = NULL);
