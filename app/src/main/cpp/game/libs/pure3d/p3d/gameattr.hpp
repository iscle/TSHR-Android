...
public:
    ...
    const rmt::Vector &GetVector(const char *paramName) {
        return GetVector(tEntity::MakeUID(paramName));
    }

    const rmt::Matrix &GetMatrix(const char *paramName) {
        return GetMatrix(tEntity::MakeUID(paramName));
    }
