...
void tBaseEmitterFactory::SetUpEmitter(tBaseEmitter *emitter) {
    ...
    emitter->visibilityChannel = emitterAnim->GetGroupByIndex(0)->GetBoolChannel(
            Pure3DAnimationChannels::ParticleSystem::Emitter::VISIBILITY_VIS);
    emitter->translationChannel = emitterAnim->GetGroupByIndex(0)->GetVectorChannel(
            Pure3DAnimationChannels::ParticleSystem::Emitter::TRANSLATION_TRA);
    emitter->rotationChannel = emitterAnim->GetGroupByIndex(0)->GetRotationChannel(
            Pure3DAnimationChannels::ParticleSystem::Emitter::ROTATION_ROT);
...
void tBaseEmitter::Update(float deltaTime, float deltaFrame, const rmt::Matrix *matrix) {
    ...
    float numAvailParticles = rmt::Max(
            (float) (currNumParticles - particles->GetNumLiveParticles()), 0.0f);
    ...
    if (numParticlesQueued >= 1.0f) {
        ...
        numParticlesQueued -= (float) generator->Generate(frame,
                                                          rmt::FtoL(rmt::Floor(numParticlesQueued)),
                                                          this);
    }
...
void tBaseEmitter::ReconnectAnimations() {
    ...
    if (state) {
        numParticlesChannel[0] = state->GetIntChannel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::NUMBER_PARTICLES_NUM);
        emissionRateChannel[0] = state->GetFloat1Channel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::EMISSION_RATE_RAT);
        lifeChannel[0] = state->GetFloat1Channel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::LIFE_LIF);
        lifeVarChannel[0] = state->GetFloat1Channel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::LIFE_VAR_LIFV);
        speedChannel[0] = state->GetFloat1Channel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::SPEED_SPD);
        speedVarChannel[0] = state->GetFloat1Channel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::SPEED_VAR_SPDV);
        weightChannel[0] = state->GetFloat1Channel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::WEIGHT_WGT);
        weightVarChannel[0] = state->GetFloat1Channel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::WEIGHT_VAR_WGTV);
        gravityChannel[0] = state->GetFloat1Channel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::GRAVITY_GRA);
        dragChannel[0] = state->GetFloat1Channel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::DRAG_DRA);
        if (blendState2 != blendState1) {
            state = factory->GetEmitterAnimation()->GetGroupByIndex(blendState2);
            numParticlesChannel[1] = state->GetIntChannel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::NUMBER_PARTICLES_NUM);
            emissionRateChannel[1] = state->GetFloat1Channel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::EMISSION_RATE_RAT);
            lifeChannel[1] = state->GetFloat1Channel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::LIFE_LIF);
            lifeVarChannel[1] = state->GetFloat1Channel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::LIFE_VAR_LIFV);
            speedChannel[1] = state->GetFloat1Channel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::SPEED_SPD);
            speedVarChannel[1] = state->GetFloat1Channel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::SPEED_VAR_SPDV);
            weightChannel[1] = state->GetFloat1Channel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::WEIGHT_WGT);
            weightVarChannel[1] = state->GetFloat1Channel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::WEIGHT_VAR_WGTV);
            gravityChannel[1] = state->GetFloat1Channel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::GRAVITY_GRA);
            dragChannel[1] = state->GetFloat1Channel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::DRAG_DRA);
        } else {
            numParticlesChannel[1] = NULL;
            emissionRateChannel[1] = NULL;
...
void tSpriteEmitter::Display() {
    ...
    if (particles->GetNumLiveParticles() > 0) {
        ...
        if (((tSpriteEmitterFactory *) factory)->GetAngleMode() ==
            ...
            for (int i = 0; i < numLiveParticles; i++) {
                ...
                current->spin = rmt::ATanCoords(angleVector.x, angleVector.y) +
                                ((tSpriteEmitterFactory *) factory)->GetAngle();
            }
        }
        ...
        for (int i = 0; i < numLiveParticles; i++) {
            ...
            currPos.x = transform.m[0][0] * temp.x + transform.m[1][0] * temp.y +
                        transform.m[2][0] * temp.z + transform.m[3][0];
            currPos.y = transform.m[0][1] * temp.x + transform.m[1][1] * temp.y +
                        transform.m[2][1] * temp.z + transform.m[3][1];
            currPos.z = transform.m[0][2] * temp.x + transform.m[1][2] * temp.y +
                        transform.m[2][2] * temp.z + transform.m[3][2];
...
void tSpriteEmitter::ReconnectAnimations() {
    const tAnimationGroup *state = factory->GetEmitterAnimation()->GetGroupByIndex(blendState1);
    if (state) {
        colourChannel[0] = state->GetColourChannel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::COLOUR_CLR);
        colourVarChannel[0] = state->GetIntChannel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::COLOUR_VAR_CLRV);
        alphaChannel[0] = state->GetIntChannel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::ALPHA_ALP);
        alphaVarChannel[0] = state->GetIntChannel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::ALPHA_VAR_ALPV);
        sizeChannel[0] = state->GetFloat1Channel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::SIZE_SIZ);
        sizeVarChannel[0] = state->GetFloat1Channel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::SIZE_VAR_SIZV);
        spinChannel[0] = state->GetFloat1Channel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::SPIN_SPI);
        spinVarChannel[0] = state->GetFloat1Channel(
                Pure3DAnimationChannels::ParticleSystem::Emitter::SPIN_VAR_SPIV);
        if (blendState2 != blendState1) {
            state = factory->GetEmitterAnimation()->GetGroupByIndex(blendState2);
            colourChannel[1] = state->GetColourChannel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::COLOUR_CLR);
            colourVarChannel[1] = state->GetIntChannel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::COLOUR_VAR_CLRV);
            alphaChannel[1] = state->GetIntChannel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::ALPHA_ALP);
            alphaVarChannel[1] = state->GetIntChannel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::ALPHA_VAR_ALPV);
            sizeChannel[1] = state->GetFloat1Channel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::SIZE_SIZ);
            sizeVarChannel[1] = state->GetFloat1Channel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::SIZE_VAR_SIZV);
            spinChannel[1] = state->GetFloat1Channel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::SPIN_SPI);
            spinVarChannel[1] = state->GetFloat1Channel(
                    Pure3DAnimationChannels::ParticleSystem::Emitter::SPIN_VAR_SPIV);
        } else {
            colourChannel[1] = NULL;
            colourVarChannel[1] = NULL;
...
void tSpriteEmitter::UpdateEmitterAttributes() {
    if ((((tSpriteEmitterFactory *) factory)->GetColourAnimationMode() ==
         p3dParticleSystemConstants::ColourAnimMode::NONE) ||
        (((tSpriteEmitterFactory *) factory)->GetColourAnimationMode() ==
         p3dParticleSystemConstants::ColourAnimMode::ALPHA)) {
        if (colourChannel[0]) {
    ...
    if ((((tSpriteEmitterFactory *) factory)->GetColourAnimationMode() ==
         p3dParticleSystemConstants::ColourAnimMode::NONE) ||
        (((tSpriteEmitterFactory *) factory)->GetColourAnimationMode() ==
         p3dParticleSystemConstants::ColourAnimMode::COLOUR)) {
        if (alphaChannel[0]) {
