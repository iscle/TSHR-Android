...
public:
    tVectorChannel(int nKeys, tAnimationMemoryBlock *block = NULL) : tChannel(nKeys,
                                                                              block) { dataType = VECTOR; }
...
protected:
    tVector1DOFChannel()
            : tVectorChannel(), dynamicIndex(0), constants(0.0f, 0.0f, 0.0f), values(NULL) {}
...
protected:
    tVector2DOFChannel()
            : tVectorChannel(), staticIndex(0), constants(0.0f, 0.0f, 0.0f), values(NULL) {}
...
public:
    tRotationChannel(int nKeys, tAnimationMemoryBlock *block = NULL) : tChannel(nKeys,
                                                                                block) { dataType = ROTATION; }
...
public:
    ...
    void FindEndKeys(float frame, float *beginframe, tEntity **beginkey, float *endframe,
...
public:
    ...
    tChannel *
    LoadChannel(tChunkFile *file, tEntityStore *store, tAnimationMemoryBlock *block = NULL);

    tIntChannel *
    LoadIntChannel(tChunkFile *file, tEntityStore *store, tAnimationMemoryBlock *block = NULL);

    tFloat1Channel *
    LoadFloat1Channel(tChunkFile *file, tEntityStore *store, tAnimationMemoryBlock *block = NULL);

    tFloat2Channel *
    LoadFloat2Channel(tChunkFile *file, tEntityStore *store, tAnimationMemoryBlock *block = NULL);

    tVectorChannel *
    LoadVectorChannel(tChunkFile *file, tEntityStore *store, tAnimationMemoryBlock *block = NULL);

    tRotationChannel *
    LoadRotationChannel(tChunkFile *file, tEntityStore *store, tAnimationMemoryBlock *block = NULL);

    tStringChannel *
    LoadStringChannel(tChunkFile *file, tEntityStore *store, tAnimationMemoryBlock *block = NULL);

    tEntityChannel *
    LoadEntityChannel(tChunkFile *file, tEntityStore *store, tAnimationMemoryBlock *block = NULL);

    tBoolChannel *
    LoadBoolChannel(tChunkFile *file, tEntityStore *store, tAnimationMemoryBlock *block = NULL);

    tColourChannel *
    LoadColourChannel(tChunkFile *file, tEntityStore *store, tAnimationMemoryBlock *block = NULL);

    tEventChannel *
    LoadEventChannel(tChunkFile *file, tEntityStore *store, tAnimationMemoryBlock *block = NULL);
