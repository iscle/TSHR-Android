...
///  
    ...
    bool ok = builder->BeginImage(header.dwWidth, header.dwHeight, 2, tImageHandler::Builder::TOP,
                                  NULL);
...
//    file->SetPosition(sizeof(long) + sizeof(DDSURFACEDESC2));
    ...
    if (header.dwFlags & DDSD_LINEARSIZE) {
        ...
        unsigned char *scanline_in = (unsigned char *) p3d::MallocTemp(
                header.lPitch * header.dwHeight);
