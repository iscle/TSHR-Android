...
public:
    ...
    public:
        enum {
            BASE_ELEMENT, PROP_ELEMENT, SKIN_ELEMENT, EFFECT_ELEMENT
        };

        DrawableElement() :
                visible(true), lockVisibility(false), isTranslucent(false), sortOrder(0.5f),
                pose(NULL) { type = BASE_ELEMENT; }
    ...
    public:
        ...
        const virtual rmt::Matrix *GetWorldMatrix() {
            return &(pose->GetJoint(poseIndex))->worldMatrix;
        }
    ...
    public:
        DrawableEffectElement()
                : DrawableElement(), effect(NULL), poseIndex(0) { type = EFFECT_ELEMENT; }
        ...
        virtual void SetVisibility(bool vis) {
            DrawableElement::SetVisibility(vis);
            if (effect) effect->SetVisibility(vis);
        }
