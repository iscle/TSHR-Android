...
tTexture *tTextureLoader::LoadImage(tChunkFile *f, tImageFactory *factory, tTexture *buildTexture,
                                    int mipLevel) {
    if (!factory)
        ...
        switch (f->BeginChunk()) {
            case Pure3D::Texture::IMAGE_DATA: {
                ...
                if (texture == NULL) {
                    ...
                    texture = factory->ParseAsTexture(file, name, size,
                                                      (tImageHandler::Format) format);
                    f->EndInset(file);
                } else {
                    tFile *file = f->BeginInset();
                    factory->ParseIntoTexture(file, texture, (tImageHandler::Format) format,
                                              mipLevel);
                    f->EndInset(file);
                }
...
tTexture *
tTextureLoader::LoadVolumeImage(tChunkFile *f, tImageFactory *factory, tTexture *buildTexture,
                                int mipLevel, int numMipMaps) {
    if (!factory)
        factory = imageFactory;
