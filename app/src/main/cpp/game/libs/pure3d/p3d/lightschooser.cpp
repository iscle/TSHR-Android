...
float tLightsChooser::TargetPntLights(const rmt::Vector &target, unsigned mark) {
    ...
    for (i = mPntLightList->GetNumLights() - 1; i >= 0; i--) {
        ...
        if (attenuation > 0.0f) {
            ...
            intensity =
                    attenuation * rmt::LtoF(rmt::Max(col.Red(), rmt::Max(col.Green(), col.Blue())));
...
void tLightsChooser::SelectShadowCasters(tLightGroup *bestShadowCasters,
    ...
    for (i = 0; i < numLights; i++) {
        ...
        // Get the next directional light to update.
        tDirectionalLight *dLight = dynamic_cast<tDirectionalLight *>(bestShadowCasters->GetLight(
                i));
        if (dLight == NULL) {
            dLight = new tDirectionalLight; // Create a new directional light if needed.
...
bool
tLightsChooser::GetCacheShadowCasters(tLightGroup *bestShadowCasters, const rmt::Vector &target,
...
void tLightsChooser::LightList::SetLight(int n, tLight *light) {
    ...
    bool isShadowCaster =
            (light->GetIlluminationType() != NEGATIVE_ILLUMINANT) && light->IsShadowCaster();
