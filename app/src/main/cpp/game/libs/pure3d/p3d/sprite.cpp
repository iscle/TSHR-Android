...
tSprite::tSprite(tTexture **images, int w, int h, int count, tShader *mat, int border, int nx,
                 int ny, tImageConverter *conv) {
    linear = false;
...
void tSprite::BuildSections(tImage *image, tImageConverter *conv) {
    ...
    for (xSect = 0; xSect < numXSect; xSect++) {
        for (ySect = 0; ySect < numYSect; ySect++) {
            int newX =
                    sections[xSect][ySect].width > minSize ? sections[xSect][ySect].width + border
                                                           : minSpriteSize;
            int newY =
                    sections[xSect][ySect].height > minSize ? sections[xSect][ySect].height + border
                                                            : minSpriteSize;
...
void
tSprite::BuildTexture(int texNum, tRect &src, int newX, int newY, tImage *image, tImage *buffer,
                      tImageConverter *conv) {
...
void tSprite::BuildPoly(int polyNum, tRect &src, int newX, int newY) {
    ...
    polys[(polyNum * 4) + 0].v.Set(float(src.right) * scaleX, float(height - src.bottom) * scaleY,
                                   0.0f);
    polys[(polyNum * 4) + 0].uv0.u = u1;
    polys[(polyNum * 4) + 0].uv0.v = v1;

    polys[(polyNum * 4) + 1].v.Set(float(src.left) * scaleX, float(height - src.bottom) * scaleY,
                                   0.0f);
    polys[(polyNum * 4) + 1].uv0.u = u0;
    polys[(polyNum * 4) + 1].uv0.v = v1;

    polys[(polyNum * 4) + 2].v.Set(float(src.right) * scaleX, float(height - src.top) * scaleY,
                                   0.0f);
    polys[(polyNum * 4) + 2].uv0.u = u1;
    polys[(polyNum * 4) + 2].uv0.v = v0;

    polys[(polyNum * 4) + 3].v.Set(float(src.left) * scaleX, float(height - src.top) * scaleY,
                                   0.0f);
...
//      if(tDrawable2D::RectClip(scratch,uv,colours))
        {
            ...
            pddiPrimStream *stream = p3d::pddi->BeginPrims(Shader->GetShader(), PDDI_PRIM_TRISTRIP,
                                                           PDDI_V_CT, 4);
...
tEntity *tSpriteLoader::LoadObject(tChunkFile *f, tEntityStore *store) {
    ...
    if (image) {
        ...
        tShader *mat = p3d::find<tShader>(store, shader);
        tSprite *sprite = new tSprite(images, imageWidth, imageHeight, imageCount, mat, 1, nativeX,
                                      nativeY, converter);
        sprite->SetName(name);
...
tTexture *tSpriteLoader::LoadTexture(tChunkFile *f, int depth /*=32*/) {
    ...
    switch (format) {
        case tImageHandler::IMG_PS2_4BIT:
            imageFactory->SetTextureHints(8, 0, PDDI_TEXTYPE_PS2_4BIT, PDDI_USAGE_STATIC);
            break;
        case tImageHandler::IMG_PS2_8BIT:
            imageFactory->SetTextureHints(8, 0, PDDI_TEXTYPE_PS2_8BIT, PDDI_USAGE_STATIC);
            break;
        case tImageHandler::IMG_PS2_16BIT:
            imageFactory->SetTextureHints(8, 0, PDDI_TEXTYPE_PS2_16BIT, PDDI_USAGE_STATIC);
            break;
        case tImageHandler::IMG_PS2_32BIT:
            imageFactory->SetTextureHints(8, 0, PDDI_TEXTYPE_PS2_32BIT, PDDI_USAGE_STATIC);
            break;
        default:
            break;
    }
    ...
    while (f->ChunksRemaining()) {
        switch (f->BeginChunk()) {
            case Pure3D::Texture::IMAGE_DATA: {
                unsigned size = f->GetLong();
                tFile *file = f->BeginInset();
                image = imageFactory->ParseAsTexture(file, name, size,
