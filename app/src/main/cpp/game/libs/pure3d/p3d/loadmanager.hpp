...
public:
    ...
    public:
        ...
        void Done() {
            Done(((request == NULL) || (request->GetState() == COMPLETE)) ? LOAD_OK : LOAD_ERROR,
                 request);
    ...
    void SetCallback(
            Callback *);            // assign a calback to be triggered when the load is complete

    void SetAsync(bool);                    // request that the load be completed asynchronously

    void SetMemoryAllocator(
            int);           // set a memory allocator to be used for data created byt this load

    void SetInventorySection(
            const char *);  // set an inventory section that the loaded data should be put in
    void SetInventorySection(
            tUID);         //      note : these functions won't have any effect if SetStore has been called
    //      also note : loads go into the main inventory "default" section
    //          (not the current section) unless this is set


    void SetStore(
            tEntityStore *);           // set an entity store to place loaded data in rather than the main inventory

    void SetPriority(
            int);                  // set the priority of the request (larger numbers are loaded first);

    void SetUserData(void *);                // set a arbitrary piece of data
    void *GetUserData(void) { return userData; }

    void SetDummy(
            bool);                    // if true request is not an actual load, but just used in queue for synchronisation
    ...
    bool
    IsDummy(void);  // true if reuest is not intended to represent an actual load (for syncronisation only)
