...
public:
    ...
    void SetCurrentAnimation(unsigned int anim) {
        if (numAnimations > 0)
            currentAnimation = anim >= numAnimations ? numAnimations - 1 : anim;
    }

    tAnimatedObjectAnimation *GetCurrentAnimation() const { return animations[currentAnimation]; }

    tAnimatedObjectAnimation *GetAnimation(unsigned int index) const {
        if (index < numAnimations)return animations[index];
        return NULL;
    }
...
public:
    ...
    unsigned int GetNumAnimations() const {
        if (object) return object->GetNumAnimations();
        return 0;
    }

    void SetCurrentAnimation(unsigned int anim) {
        if (object) object->SetCurrentAnimation(anim);
        SetFrame(0.0f);
    }
    ...
    virtual float GetNumFrames() {
        if (object) return object->GetCurrentAnimation()->GetNumFrames();
        return 0.0f;
    }
