...
tTextureFont::DisplayLine(const P3D_UNICODE *text, int formatting) const {
    ...
    p3d::stack->Push();
    p3d::stack->Translate((formatting & CENTRE_X) ? -GetLineWidth(text) / 2.0f : 0.0f,
                          fontBaseLine - fontHeight, 0.0f);
...
tTextureFontLoader::LoadObject(tChunkFile *f, tEntityStore *store) {
    ...
    tTextureLoader *loader = dynamic_cast<tTextureLoader *>(p3d::loadManager->GetP3DHandler()->GetHandler(
    ...
    font->SetMissingLetter(
            0x7f7f); // special character rendered as rectangle by the font for srr2 xbox tcr
    return font;
}
