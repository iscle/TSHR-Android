...
int tExpression::GetKeyIndex(float keyVal) {
    void *ptr = bsearch((const void *) &keyVal, (const void *) keys, nKeys, sizeof(float),
                        compareFloat);
...
tEntity *tExpressionLoader::LoadObject(tChunkFile *f, tEntityStore *) {
    ...
    for (i = 0; i < expr->nKeys; i++) {
        ...
        if (expr->keys[i] > 0) numPos++;
        expr->posKeysExist = true;
    }
...
IRefCount *
tExpressionGroupLoader::tExpressionStore::Find(radLoadInventory::SafeCastBase &c, const tUID uid) {
