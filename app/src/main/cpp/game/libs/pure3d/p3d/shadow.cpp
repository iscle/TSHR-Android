...
static inline void SetupPass(ShadowVolPass pass, tShader *shader) {
    ...
    p3d::pddi->SetStencilOp(PDDI_STENCIL_KEEP, PDDI_STENCIL_KEEP,
                            (pass == PASS_FRONT) ? PDDI_STENCIL_INCR : PDDI_STENCIL_DECR);
#endif
...
//    pddi->SetZCompare(PDDI_COMPARE_LESSEQUAL);
    ...
    // Draw the wash
    pddiPrimStream *stream = pddi->BeginPrims(washShader->GetShader(), PDDI_PRIM_TRISTRIP, PDDI_V_C,
                                              4);

    stream->Colour(washColour);
    stream->Coord(0, 0, 1);
    stream->Colour(washColour);
    stream->Coord(1, 0, 1);
    stream->Colour(washColour);
...
void tShadowMesh::DrawShadowVolume() {
    ...
    stream = p3d::pddi->BeginPrims(s_volumeShader->GetShader(), PDDI_PRIM_TRIANGLES, PDDI_V_C,
                                   6 * s_NumDrawEdges);
...
void tShadowMesh::DrawNormals(void) {
    pddiPrimStream *stream = p3d::pddi->BeginPrims(NULL, PDDI_PRIM_LINES, PDDI_V_C,
                                                   2 * numTriangles);
...
void tShadowMesh::DrawShell(void) {
    pddiPrimStream *stream = p3d::pddi->BeginPrims(NULL, PDDI_PRIM_TRIANGLES, PDDI_V_C,
                                                   3 * numTriangles);
    tColour blue(0, 0, 255, 255);

    for (unsigned i = 0; i < numTriangles; i++) {
        stream->Colour(blue);
        stream->Coord(vertices[topology[i].v0].x, vertices[topology[i].v0].y,
                      vertices[topology[i].v0].z);

        stream->Colour(blue);
        stream->Coord(vertices[topology[i].v1].x, vertices[topology[i].v1].y,
                      vertices[topology[i].v1].z);

        stream->Colour(blue);
        stream->Coord(vertices[topology[i].v2].x, vertices[topology[i].v2].y,
                      vertices[topology[i].v2].z);
    }
...
tEntity *tShadowSkinLoader::LoadObject(tChunkFile *f, tEntityStore *store) {
    ...
    tShadowSkin::VertexThree *sourceVertexThree = (tShadowSkin::VertexThree *) p3d::MallocTemp(
            sizeof(tShadowSkin::VertexThree) * numVertices);
