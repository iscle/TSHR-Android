...
public:
    ...
    void SetState(tSequencerState *new_state, float baseSpeed, float start_frame = 0.0F,
...
public:
    ...
    inline tUID GetCurrentSubPartStateUID(
            int subPartIndex) { return (mSubParts[subPartIndex].State()->GetUID()); }

    void TransitionOn(char *transitionName) {
        mSubParts[0].State()->TransitionOn(tEntity::MakeUID(transitionName));
    }
