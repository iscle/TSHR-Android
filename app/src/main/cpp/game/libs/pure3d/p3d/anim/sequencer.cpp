...
//--------------------------------------------------
tStateSequencerCommand *
tSequencerState::TransitionTo(tSequencerState *new_state, bool queryTransition) {
    if (new_state == NULL) return NULL;

    int a;
    for (a = 0; a < this->mTransitionCount; a++) {
        if ((this->mTransitions[a].GetSequencerState() == new_state) &&
            (mTransitions[a].mTransitionDirection == 1)) {
            ...
            if (this->mTransitions[a].mOutFrameMin < 0.0F) {
                ...
                float startframe = this->mTransitions[a].mInFrame +
                                   (this->mTransitions[a].mInFrameRangeScale * framediff);
    ...
    for (a = 0; a < new_state->mTransitionCount; a++) {
        if ((new_state->mTransitions[a].GetSequencerState() == this) &&
            (new_state->mTransitions[a].mTransitionDirection == 0)) {
            ...
            if (new_state->mTransitions[a].mOutFrameMin < 0.0F) {
                // Compute where the new state's animation should start
                float framediff = mCurrentFrame - minframe;
                float startframe = new_state->mTransitions[a].mInFrame +
                                   (new_state->mTransitions[a].mInFrameRangeScale * framediff);
...
tStateSequencerCommand *tSequencerState::TransitionOn(tUID transitionNameUID) {
    ...
    int a;
    for (a = 0; a < this->mTransitionCount; a++) {
        if ((mTransitions[a].mName == transitionNameUID) &&
            (mTransitions[a].mTransitionDirection == 1)) {
            ...
            if (this->mTransitions[a].mOutFrameMin < 0.0F) {
                ...
                float framediff = mCurrentFrame - minframe;
                float startframe =
                        mTransitions[a].mInFrame + (mTransitions[a].mInFrameRangeScale * framediff);
...
//--------------------------------------------------
void tSubPartState::SetState(tSequencerState *new_state, float baseSpeed, float start_frame,
                             float blendframes) {
...
tSequencer *tSequencerLoader::ParseSequencer(TextDataParser *parser) {
    ...
    while (!parser->Eof()) {
        ...
        if (!strcmp("animtarget", token)) {
            parser->Advance();
            parser->CurrentToken(value, sizeof(value));
            tBlendFrameController *animationTarget = p3d::find<tBlendFrameController>(mStore,
                                                                                      value);
            if (animationTarget) {
...
bool tSequencerLoader::ParseStates(TextDataParser *parser, tSequencer *seq) {
    ...
    while (!parser->Eof()) {
        ...
        if (!strcmp("state", token)) {
            ...
            if (current_state < 0) {
                p3d::printf(
                        "Error parsing states line %d, must set count before defining a state.\n",
                        parser->CurrentLine());
                continue;
            }
            ...
            if (current_state >= seq->mStateCount) {
                p3d::printf(
                        "Error parsing states line %d, more states have been defined thet the count specifies.\n",
                        parser->CurrentLine());
                continue;
            }
...
bool tSequencerLoader::ParseState(TextDataParser *parser, tSequencerState *state) {
    ...
    while (!parser->Eof()) {
        ...
        if (!strcmp("anim", token)) {
            ...
            P3DASSERTMSG(state->mObjectAnimation, "Sequencer loader failed to find animation",
                         value);
            continue;
        }
        ...
        if (!strcmp("event", token)) {
            ...
            P3DASSERTMSG(state->mEventAnimation, "Sequencer loader failed to find event animation",
                         value);
            continue;
        }
...
bool tSequencerLoader::ParseTransition(TextDataParser *parser, tStateTransition *dest) {
    ...
    while (!parser->Eof()) {
        ...
        if (!strcmp("command", token)) {
            ...
            else if (!strcmp("crossfade", value))
                dest->mCommand = tStateSequencerCommand::CrossFade;
            else if (!strcmp("usedefault", value))
                dest->mCommand = tStateSequencerCommand::UseDefault;
            else {
...
bool tSequencerLoader::FixupTransitions(tSequencer *seq) {
    ...
    for (a = 0; a < seq->mStateCount; a++) {
        ...
        if (state->mOutDest != NULL) {
            ...
            P3DASSERTMSG(state->mOutDest->mStateParam, "Sequencer couldn'd find sequencerState",
                         name);
            delete[] name;
        }
