...
void tVertexAnimController::Update() {
    ...
    for (int i = 0; i < animation->GetNumGroups(); ++i) {
        ...
        if (animGroup) {
            ...
            const tEntityChannel *keys = animGroup->GetEntityChannel(
                    Pure3DAnimationChannels::Vertex::VERTEX_VRTX);
            ...
            if (keys) {
                ...
                if (k1 !=
                    k2) {     //if the end keys are differet, update the offset list with the right end key
