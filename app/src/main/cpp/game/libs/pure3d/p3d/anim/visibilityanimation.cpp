...
void tSimpleVisibilityAnimationController::Update() {
    ...
    if (HasHirarchy()) {
        for (i = 0; i < iChannels; i++) {
            ...
            pVisChan = animation->GetGroupByIndex(i)->GetBoolChannel(
                    Pure3DAnimationChannels::Visibility::VISIBILITY_VIS);
...
//-----------------------------------------------------------------------------
tSceneGraphVisibilityAnimationController::tSceneGraphVisibilityAnimationController(
        tSceneGraphVisibilityAnimationController *c) :
...
//-----------------------------------------------------------------------------
void tSceneGraphVisibilityAnimationController::SetAnimation(tAnimation *anim, float startFrame,
                                                            float blendFrames) {
    SetAnimation(anim);
    P3DASSERTMSG(startFrame == 0.0f, "VisibilityAnimation wont work if you set the frame",
                 "visibilityanimation.cpp");
}
...
void tSceneGraphVisibilityAnimationController::RebindAnimationAndDrawable() {
    ...
    if ((animation != NULL) && (pScene != NULL)) {
        ...
        for (i = 0; i < iNumNodes; i++) {
            Scenegraph::Visibility *visibilityNode = dynamic_cast<Scenegraph::Visibility *>(pScene->Find(
                    animation->GetGroupByIndex(i)->GetUID()));
...
//-----------------------------------------------------------------------------   
tCompDrawVisibilityAnimationController::tCompDrawVisibilityAnimationController(
        tCompDrawVisibilityAnimationController *c) :
        tSimpleVisibilityAnimationController(c),
        pCompDraw(NULL),
        visNodes(NULL) {
    SetDrawable(c->GetDrawable());
...
//-----------------------------------------------------------------------------   
void tCompDrawVisibilityAnimationController::SetAnimation(tAnimation *anim, float startFrame,
                                                          float blendFrames) {
    SetAnimation(anim);
    P3DASSERTMSG(startFrame == 0.0f, "VisibilityAnimation wont work if you set the frame",
                 "visibilityanimation.cpp");
...
void tCompDrawVisibilityAnimationController::RebindAnimationAndDrawable() {
    ...
    if ((animation != NULL) && (pCompDraw != NULL)) {
        ...
        // store pointer to scene node that matches the stored UID
        for (i = 0; i < iNumNodes; i++) {
            tCompositeDrawable::DrawableElement *drawable = pCompDraw->FindNode(
                    animation->GetGroupByIndex(i)->GetUID());
            P3DASSERT(drawable);
            visNodes[i] = drawable;
        }
