...
static void
p3dBlockCopy(unsigned char *src, unsigned char *dest, int width, int height, int srcStride,
             int destStride);

static void
ImageBlockCopy8(tImage8 *srcImg, unsigned char *destBase, int x0, int y0, int width, int height,
                int destStride);

static void
ImageBlockCopy8Mask(tImage8 *srcImg, unsigned char *destBase, int x0, int y0, int width, int height,
                    int destStride, unsigned char mask);

static void
BlockCopy32Mask(unsigned *src, unsigned *dest, int width, int height, int srcStride, int destStride,
                unsigned mask);

static void ImageBlockCopy32(tImage *src, unsigned *dest, int x0, int y0, int width, int height,
                             int destStride);

static void
ImageBlockCopy32Mask(tImage *src, unsigned *destBase, int x0, int y0, int width, int height,
                     int destStride, unsigned mask);
...
void tImage8::CopyRegion(tImage *srcImg, int sx, int sy, int sw, int sh, int dx, int dy) {
    ...
    if (srcImg->GetColourChannel() && image) {
        ...
        if (srcImg->GetColourKeyStatus()) {
            ImageBlockCopy8Mask((tImage8 *) srcImg, dest, sx, sy, sw, sh, width,
                                (unsigned char) srcImg->GetColourKey());
        } else {
    ...
    if (srcImg->GetColourChannel() && image) {
        if (srcImg->GetDepth() == 32) {
            ...
            if (srcImg->GetColourKeyStatus()) {
                ...
                p3dBlockCopy((unsigned char *) src, (unsigned char *) dest, sw * 4, sh,
                             srcPitch * 4, width * 4);
            }
        } else {
...
void p3dBlockCopy(unsigned char *src, unsigned char *dest, int width, int height, int srcStride,
                  int destStride) {
...
void BlockCopy32Mask(unsigned *srcBase, unsigned *destBase, int width, int height, int srcStride,
                     int destStride, unsigned mask) {
    for (int i = 0; i < height; i++) {
...
void ImageBlockCopy32(tImage *src, unsigned *destBase, int x0, int y0, int width, int height,
                      int destStride) {
    int x1 = x0 + width;
    int y1 = y0 + height;
    for (int y = y0; y < y1; y++) {
...
void ImageBlockCopy32Mask(tImage *src, unsigned *destBase, int x0, int y0, int width, int height,
                          int destStride, unsigned mask) {
    int x1 = x0 + width;
    int y1 = y0 + height;
    for (int y = y0; y < y1; y++) {
            ...
            if ((colour & 0x00ffffff) != mask) {
                ...
                if (srcColour.Alpha() == 255) {
                    *dest = srcColour;
                    ...
                    tColour destColour(*dest);
                    int red = (((srcColour.Red() - destColour.Red()) * srcColour.Alpha()) >> 8) +
                              destColour.Red();
                    int green =
                            (((srcColour.Green() - destColour.Green()) * srcColour.Alpha()) >> 8) +
                            destColour.Green();
                    int blue = (((srcColour.Blue() - destColour.Blue()) * srcColour.Alpha()) >> 8) +
                               destColour.Blue();
                    int alpha = srcColour.Alpha() + destColour.Alpha();
                    *dest = tColour(red, green, blue, alpha > 255 ? 255 : alpha);
                }
