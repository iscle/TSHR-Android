...
void gcRasterizerState::SetRasterizer(GXChannelID channel) {
    ...
    if (mEnableLights)
        GXSetChanCtrl(channel, true, mAmbientSource, mMaterialSource, mLightMask, GX_DF_CLAMP,
                      mAttenuation);
    else
        GXSetChanCtrl(channel, false, mAmbientSource, mMaterialSource, GX_LIGHT_NULL, GX_DF_NONE,
                      GX_AF_NONE);
}
