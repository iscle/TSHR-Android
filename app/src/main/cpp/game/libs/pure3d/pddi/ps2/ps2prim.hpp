...
public:
    ...
    static void
    ComputeDMASize(pddiPrimBufferDesc &desc, VUMemLayout &vuLayout, unsigned &sizeInBytes,
...
private:
    ...
    void WriteComponent(VifStream &vif, pddiVector *src, int count, unsigned startAddr,
                        VIF::UnpackFormat format);

    void WriteComponent(VifStream &vif, pddiVector2 *src, int count, unsigned startAddr,
                        VIF::UnpackFormat format);

    void WriteComponent(VifStream &vif, pddiColour *src, int count, unsigned startAddr,
                        VIF::UnpackFormat format, bool shift);

    void WritePositions(VifStream &vif, Position *src, int count, unsigned startAddr);

    void WriteMatrixIndices(VifStream &vif, MatrixIndices *src, int count, unsigned startAddr,
...
public:
    ...
    int GetPrimCount() {
        return ps2VertexList::VertsToPrims(desc.GetPrimType(), desc.GetVertexCount());
    }
    ...
    virtual unsigned char *LockIndexBuffer() {
        PDDIASSERT(0 && "PDDI ERROR::LockIndexBuffer() not supported on PS2!");
        return 0;
    }

    virtual void UnlockIndexBuffer(int count) {
        PDDIASSERT(0 && "PDDI ERROR::UnlockIndexBuffer() not supported on PS2!");
    }
    ...
    inline unsigned GetProgramAddressOffset() const {
        return (unsigned) programAddress - (unsigned) dmaBuffer;
    }
