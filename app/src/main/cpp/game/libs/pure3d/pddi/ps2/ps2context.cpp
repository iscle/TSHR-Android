...
ps2Context::ps2Context(pddiDisplay *display, pddiDevice *device) :
    ...
    PDDIASSERT(g_display->GetDepth() == 32 &&
               "PDDI only supports 32 bit rendering on the Playstation2!");
    ...
    SetGSRegister(SCE_GS_DIMX,
                  SCE_GS_SET_DIMX(0x4, 0x2, 0x5, 0x3, 0x0, 0x6, 0x1, 0x7, 0x5, 0x3, 0x4, 0x2, 0x1,
                                  0x7, 0x0, 0x6));
    mfifo->Flush();
}
...
void ps2Context::DisplayReset() {
    ...
    if (vram) {
        ...
        vram->SetFrameAddr(g_display->GetFrameAddr(0), SCE_GS_PSMCT16, g_display->GetFrameAddr(1),
                           SCE_GS_PSMCT32);
        vram->SetZbufferAddr(g_display->GetZBufferAddress(), g_display->GetZBufferFormat());
        vram->SetTexMemory(g_display->GetTextureMemoryStart(),
                           g_display->GetTextureMemoryEnd() - g_display->GetTextureMemoryStart());
    }
}
...
void ps2Context::ClearVRAM(u_char r, u_char g, u_char b, u_char a) {
    ...
    gifPkt.A_D(SCE_GS_XYZF2, SCE_GS_SET_XYZ(0, 0, 0));
    gifPkt.A_D(SCE_GS_XYZF2, SCE_GS_SET_XYZ(1024 << 4, 1024 << 4, 0));
    gifPkt.A_D(SCE_GS_ZBUF_1,
               SCE_GS_SET_ZBUF(zbp, zsm, !state.renderState->zWrite)); // restore z-writes
...
void ps2Context::Clear(unsigned bufferMask) {
    ...
    */
    ...
    gifPkt.A_D(SCE_GS_TEST_1,
               SCE_GS_SET_TEST(0, 1, 0, 0, 0, 1, 1, SCE_GS_ZALWAYS)); // disable z, alpha test
    ...
    if (mask == PDDI_BUFFER_DEPTH) {
        ...
        gifPkt.A_D(SCE_GS_RGBAQ,
                   SCE_GS_SET_RGBAQ(colour.Red(), colour.Green(), colour.Blue(), colour.Alpha(),
                                    0x3f800000));
    }
    ...
    gifPkt.A_D(SCE_GS_ZBUF_1, SCE_GS_SET_ZBUF(zbp, zsm, !state.renderState->zWrite));
    // restore z-test
    gifPkt.A_D(SCE_GS_TEST_1, SCE_GS_SET_TEST(0, 1, 0, 0, 0, 1, 1,
                                              SHADER::zCompareTable[state.renderState->zCompare]));
    gifPkt.EndGifAD();
...
void ps2Context::SyncGSPath() {
    ...
    sanity = 0;
    while ((DGET_GS_CSR() & GS_CSR_FIFO_M) !=
           (1 << GS_CSR_FIFO_O))  // CSR.FIFO == 0x01: GS fifo empty
    {
...
void ps2Context::LoadHardwareMatrix(pddiMatrixType id) {
    PDDIASSERT(id == PDDI_MATRIX_MODELVIEW &&
               "PDDI ERROR:  Attempt to modify unsupported matrix type on the PS2.");
    transformStateChanged = true;
}
...
void ps2Context::LazyStateSet(void) {
    if (transformStateChanged) {
        ADD_STAT(PDDI_STAT_MATRIX_OPS, 1);
        EE::QwordCopy(&vu_context->transform.matrix,
                      state.matrixStack[PDDI_MATRIX_MODELVIEW]->Top(), 4);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (lightStateChanged[i] || transformStateChanged) {
                ...
                if (light.enabled) {
                    ...
                    if (light.type == PDDI_LIGHT_DIRECTIONAL) {
                        // directional light
                        v.Set(-light.viewDirection.x, -light.viewDirection.y,
...
void ps2Context::DrawPrimBuffer(pddiShader *s, pddiPrimBuffer *b) {
    ...
    PDDIASSERT(buffer->GetDMABuffer() &&
               "PDDI ERROR:  Call pddiPrimBuffer::Finalize() before rendering.");
...
pddiPrimStream *
ps2Context::BeginPrims(pddiShader *s, pddiPrimType primType, unsigned vertexType, int vertexCount,
                       unsigned pass) {
    ps2Shader *shader = s ? (ps2Shader *) s : (ps2Shader *) defaultShader;
    ...
    if (lp->specular > 0.0f) {
        for (int i = 0; i < 4; i++) {
            if (state.lightingState->light[i].enabled) {
                // diffuse case
                vu_context->light.lightColour[i].r =
                        state.lightingState->light[i].red * lp->diffuse.r;
                vu_context->light.lightColour[i].g =
                        state.lightingState->light[i].green * lp->diffuse.g;
                vu_context->light.lightColour[i].b =
                        state.lightingState->light[i].blue * lp->diffuse.b;
                vu_context->light.lightColour[i].a = 0;

                // specular case (Dl*Sm)
                vu_context->light.specular[i] =
                        state.lightingState->light[i].intensity * lp->specular;
            }
        }
...
int ps2Context::ClearDestAlpha(const pddiRect *rc) {
    ...
    gifPkt.A_D(SCE_GS_TEST_2,
               SCE_GS_SET_TEST(1, 0, 0, 1, 0, 0, 1, SCE_GS_ZALWAYS)); // disable ztest, zwrite
    gifPkt.A_D(SCE_GS_ALPHA_2,
               SCE_GS_SET_ALPHA(1, 0, 2, 2, 0x80)); // (Cd - Cs)*0x80 + 0 , subtractive blend
    gifPkt.A_D(SCE_GS_RGBAQ,
               SCE_GS_SET_RGBAQ(0, 0, 0, 0xff, 0x3f80000)); // subtract 0xff from alpha
...
int ps2Context::TransformToScreen(const pddiVector *a, pddiVector *out) {
    ...
    if (output.w > 0.0f) {
        ...
        out->x = (output.x * Q) * vu_context->transform.viewScaleX +
                 vu_context->transform.viewOffsetX;
        out->y = (output.y * Q) * vu_context->transform.viewScaleY +
                 vu_context->transform.viewOffsetY;
...
void ps2Context::MotionBlur(unsigned src, unsigned dst, const pddiRect *srcRect,
                            const pddiRect *destRect) {
    PDDIASSERT(mfifo);
    ...
    if (motionBlurTheta > 0.01f) {
        ...
        gifPkt.A_D(SCE_GS_TEST_1, SCE_GS_SET_TEST_1(0, 1, 0, 0, 0, 0, 1,
                                                    SCE_GS_ZALWAYS));   // disable z, alpha test
        ...
        for (int i = 0; i < 3; i++) {
            ...
            gifPkt.A_D(SCE_GS_ZBUF_1,
                       SCE_GS_SET_ZBUF(zbp, zsm, !state.renderState->zWrite)); // enable zwrites
            gifPkt.EndGifAD();
            mfifo->CloseVifStreamAdd(gifPkt);
        }
    } else {
