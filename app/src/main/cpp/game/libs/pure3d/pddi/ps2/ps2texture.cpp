...
int ps2Texture::NextVramOffset(unsigned w, unsigned h, unsigned bpp) {
    ...
    switch (bpp) {
        case 4 :
            lut = lut4;
            lutSize = sizeof(lut4) / sizeof(lut4[0]);
            break;
        case 8 :
            lut = lut8;
            lutSize = sizeof(lut8) / sizeof(lut8[0]);
            break;
        case 32:
            lut = lut32;
            lutSize = sizeof(lut32) / sizeof(lut32[0]);
            break;
    };
...
int ps2Texture::VramAlloc(unsigned w, unsigned h, unsigned bpp, unsigned mipmaps, u_short *tbp,
                          u_short *cbp, bool alignToPage) {
    const unsigned blockWidth = blockSizes[bpp >> 3][0];
    const unsigned blockHeight = blockSizes[bpp >> 3][1];
...
bool
ps2Texture::Create(int xSize, int ySize, int bitDepth, int alpha, int nMipMap, pddiTextureType type,
                   pddiTextureUsageHint usageHint) {
    PDDIASSERT(
            (nMipMap < (PS2_MAX_MIP_LEVELS)) && "PDDI ERROR: Too many miplevels! Use 6 or less.");
    ...
    switch (bpp) {
        case 32: {
            ...
            pixelFormat = (type == PDDI_TEXTYPE_PS2_32BIT) ? PDDI_PIXEL_PS2_32BIT
                                                           : PDDI_PIXEL_ARGB8888;
            break;
        }
        case 16: {
            tex0.PSM = SCE_GS_PSMCT16;
            pixelFormat = (type == PDDI_TEXTYPE_PS2_16BIT) ? PDDI_PIXEL_PS2_16BIT
                                                           : PDDI_PIXEL_ARGB1555;
            break;
        }
        case 8: {
            ...
            nPaletteEntries = 256;
            palette = (pddiColour *) MallocAligned(128,
                                                   256 * sizeof(pddiColour)); // 128 byte aligned
            break;
        }
        case 4: {
            ...
            nPaletteEntries = 16;
            palette = (pddiColour *) MallocAligned(128,
                                                   16 * sizeof(pddiColour)); // 128 byte aligned
            break;
        }
        default :
            PDDIASSERT(0);
    ...
    for (int mipLevel = 0; mipLevel <= nMip; mipLevel++) {
        ...
        bits[mipLevel] = (char *) MallocAligned(128, (mipWidth * mipHeight * bpp) /
                                                     8); // 128 byte aligned
...
unsigned ps2Texture::AddToPacket(unsigned *packet) const {
    ...
    if (palette) {
        ...
        dmaPkt.A_D(SCE_GS_BITBLTBUF,
                   SCE_GS_SET_BITBLTBUF(0, 0, 0, cbp + vramaddress, 1, SCE_GS_PSMCT32));
    ...
    for (int mipLevel = 0; mipLevel <= nMip; mipLevel++) {
        ...
        unsigned lines_per_pass =
                32767 / quadwords_per_line;  // 32768 quadwords per GIF Tag restriction
        ...
        while (lines_left > 0) {
            ...
            dmaPkt.A_D(SCE_GS_BITBLTBUF, SCE_GS_SET_BITBLTBUF(0, 0, 0, tbp[mipLevel] + vramaddress,
                                                              (tbw >> mipLevel) ? (tbw >> mipLevel)
                                                                                : 1, psm));
