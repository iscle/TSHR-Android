...
d3dPrimBufferStream::d3dPrimBufferStream() :
        ...
        coord(NULL), normal(NULL), binormal(NULL), tangent(NULL), compressedNormal(NULL),
        colour(NULL), specular(NULL), uv(NULL) {
#ifdef _XBOX
    size = NULL;
    w = NULL;
#endif
...
void
d3dPrimBufferStream::SetBasePtr(void *base, d3dVertexProgram *vs, int bufferCount, int offset) {
    basePtr = base;
    nVertex = 0;
...
d3dPrimBuffer::~d3dPrimBuffer() {
    ...
    context->ADD_STAT(PDDI_STAT_BUFFERED_ALLOC,
                      -float((vertexProgram->GetStride() * maxVertex) + (nIndex * 2)) / 1024.0f);
}

void d3dPrimBuffer::Create(pddiPrimType type, unsigned format, unsigned h, int nVertex, int nIndex,
                           const char *vertexProgramName) {
    static char dummy[] = "";
...
void d3dPrimBuffer::CreateStripList(pddiPrimType type, unsigned fmt, int maxVertex, int nStrip,
                                    int *strLen) {
    PDDIASSERT(0);
}
...
void d3dPrimBuffer::SetNumIndices(int count) {
    ...
    if (count) {
        HRESULT res = d3d->CreateIndexBuffer(count * sizeof(unsigned short), D3DUSAGE_WRITEONLY,
                                             D3DFMT_INDEX16, D3DPOOL_MANAGED, &indices);
        DDVERIFY(res);
...
void d3dPrimBuffer::Display() {
    ...
    if (nIndex > 0) {
        ...
        HRESULT res = d3d->DrawIndexedPrimitive(primitiveType, 0, maxVertex, 0,
                                                VertsToPrims(primTypePDDI, nIndex));
        DDVERIFY(res);
    } else {
        d3d->SetVertexShader(vertexProgram->GetD3DVS());
        d3d->SetStreamSource(0, buffer, vertexProgram->GetStride());
