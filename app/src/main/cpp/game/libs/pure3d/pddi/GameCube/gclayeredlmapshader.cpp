...
static gcColourBlendStruct SpecularColourBlendTable =
        {  // dest = CLAMP((((A * (1.0 - C) + B * C) * TevOp) + D + bias) * scale)
                //   A           B           C           D          TevOp       Scale
                GX_CC_RASC, GX_CC_ZERO, GX_CC_ZERO, GX_CC_CPREV, GX_TEV_ADD, GX_CS_SCALE_1,
                GX_ENABLE
        };
...
static gcAlphaBlendStruct SpecularAlphaBlendTable =
        {  // dest = CLAMP((((A * (1.0 - C) + B * C) * ColourTevOp) + D + bias) * ColourScale)
                //   A           B           C           D
                GX_CA_RASA, GX_CA_ZERO, GX_CA_ZERO, GX_CA_APREV, GX_TEV_ADD, GX_CS_SCALE_1,
                GX_ENABLE
        };
...
static gcColourBlendStruct TTAlphaColourBlendTable =
        {  // dest = CLAMP((((A * (1.0 - C) + B * C) * TevOp) + D + bias) * scale)
                //   A           B           C           D          TevOp       Scale
                GX_CC_TEXC, GX_CC_CPREV, GX_CC_TEXA, GX_CC_ZERO, GX_TEV_ADD, GX_CS_SCALE_1,
                GX_ENABLE
        };
...
static gcColourBlendStruct TTAddColourBlendTable =
        {  // dest = CLAMP((((A * (1.0 - C) + B * C) * TevOp) + D + bias) * scale)
                //   A           B           C           D          TevOp       Scale
                GX_CC_TEXC, GX_CC_ZERO, GX_CC_ZERO, GX_CC_CPREV, GX_TEV_ADD, GX_CS_SCALE_1,
                GX_ENABLE
        };
...
static gcColourBlendStruct TTSubtractColourBlendTable =
        {  // dest = CLAMP((((A * (1.0 - C) + B * C) * TevOp) + D + bias) * scale)
                //   A           B           C           D          TevOp       Scale
                GX_CC_TEXC, GX_CC_ZERO, GX_CC_ZERO, GX_CC_CPREV, GX_TEV_SUB, GX_CS_SCALE_1,
                GX_ENABLE
        };
...
static gcColourBlendStruct TTModulateColourBlendTable =
        {  // dest = CLAMP((((A * (1.0 - C) + B * C) * TevOp) + D + bias) * scale)
                //   A           B           C           D          TevOp       Scale
                GX_CC_ZERO, GX_CC_TEXC, GX_CC_CPREV, GX_CC_ZERO, GX_TEV_ADD, GX_CS_SCALE_1,
                GX_ENABLE
        };
...
static gcAlphaBlendStruct TTModulateAlphaBlendTable =
        {  // dest = CLAMP((((A * (1.0 - C) + B * C) * TevOp) + D + bias) * scale)
                //   A           B           C           D          TevOp       Scale
                GX_CA_ZERO, GX_CA_TEXA, GX_CA_APREV, GX_CA_ZERO, GX_TEV_ADD, GX_CS_SCALE_1,
                GX_ENABLE
        };
...
static gcColourBlendStruct TTModulate2ColourBlendTable =
        {  // dest = CLAMP((((A * (1.0 - C) + B * C) * TevOp) + D + bias) * scale)
                //   A           B           C           D          TevOp       Scale
                GX_CC_ZERO, GX_CC_TEXC, GX_CC_CPREV, GX_CC_ZERO, GX_TEV_ADD, GX_CS_SCALE_2,
                GX_ENABLE
        };
...
static gcAlphaBlendStruct TTModulate2AlphaBlendTable =
        {  // dest = CLAMP((((A * (1.0 - C) + B * C) * TevOp) + D + bias) * scale)
                //   A           B           C           D          TevOp       Scale
                GX_CA_ZERO, GX_CA_TEXA, GX_CA_APREV, GX_CA_ZERO, GX_TEV_ADD, GX_CS_SCALE_2,
                GX_ENABLE
        };
...
static gcColourBlendStruct TTAddModAlphaColourBlendTable =
        {  // dest = CLAMP((((A * (1.0 - C) + B * C) * TevOp) + D + bias) * scale)
                //   A           B           C           D          TevOp       Scale
                GX_CC_ZERO, GX_CC_APREV, GX_CC_TEXC, GX_CC_CPREV, GX_TEV_ADD, GX_CS_SCALE_1,
                GX_ENABLE
        };
...
pddiBaseShader *
gcLayeredLMapShader::Allocate(pddiRenderContext *c, const char *name, const char *aux) {
...
void gcLayeredLMapShader::SetDevPass(unsigned pass) {
    ...
    GXSetBlendMode(gcBlendModeTable[m].Type, gcBlendModeTable[m].Source, gcBlendModeTable[m].Dest,
                   gcBlendModeTable[m].Op);
    ...
    if (mIsLit) {
        ...
        mRasStates[1].mLightMask = mIsSpecular ? (GXLightID)(lightmask << _GC_MAX_LIGHTS)
                                               : GX_LIGHT_NULL;
    }
