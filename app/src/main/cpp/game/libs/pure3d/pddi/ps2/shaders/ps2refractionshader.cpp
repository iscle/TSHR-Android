...
ps2RefractionShaderAllocate(pddiRenderContext *c, const char *, const char *) {
...
void ps2RefractionShader::Validate(unsigned format) {
    PDDIASSERT(((format & 0xf) == 1) &&
               "PDDI ERROR: ps2RefractionShader: Invalid vertex format (texture co-ord count != 1)");
    PDDIASSERT((format & PDDI_V_POSITION) &&
               "PDDI ERROR: ps2RefractionShader:  Invalid vertex format (no positions");
    PDDIASSERT((format & PDDI_V_NORMAL) &&
               "PDDI ERROR: ps2RefractionShader:  Invalid Vertex Format (requires normals)");
}
