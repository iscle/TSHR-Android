...
void d3dToonVertexShader::SetPass(int pass) {
    ...
    if (pass == 0) {     //first pass
        float vColor[4] = {shaderInfo.diffuse.Red() / 255.f, shaderInfo.diffuse.Green() / 255.f,
                           shaderInfo.diffuse.Blue() / 255.f, shaderInfo.diffuse.Alpha() / 255.f};
        ...
        if (InSkinMode()) {
            ...
            m_pd3dDevice->SetVertexShaderConstant(12, (pick[0] == -1) ? (D3DVECTOR * ) & vDarkLight
                                                                      : &light0.Direction, 1);
            m_pd3dDevice->SetVertexShaderConstant(13, (pick[1] == -1) ? (D3DVECTOR * ) & vDarkLight
                                                                      : &light1.Direction, 1);
            m_pd3dDevice->SetVertexShaderConstant(14, (pick[2] == -1) ? (D3DVECTOR * ) & vDarkLight
                                                                      : &light2.Direction, 1);
        ...
        float vColor[4] = {(float) silhoutte.Red() / 255.f, (float) silhoutte.Green() / 255.f,
                           (float) silhoutte.Blue() / 255.f, 1.f};
...
void d3dToonVertexShader::CreateShadeMap() {
    ...
    d3dok = d3d->CreateTexture((unsigned int) TOON_SHADE_MAP_DIM, 1, 1, NULL, descBack.Format,
                               D3DPOOL_MANAGED, &d3dShadeMap);
