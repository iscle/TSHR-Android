...
//------------------------------------------------------------------------------------
void gcMISkinBuffer::Display(gcContext *context, pddiShader *shader, pddiMatrix *palette,
    ...
    for (a = 1; a < mComponentCount; a++) {
        ...
        GXSetVtxAttrFmt(GX_VTXFMT0, (GXAttr) desc->item, (GXCompCnt) desc->count,
                        (GXCompType) desc->style, (u8) desc->fraction);
...
lbz r8,
0x00(r5)         // Load weight count

vertlooptop:
ps_sub fp2, fp2, fp2         // zero the output vert XY
psq_l fp4,
0x04(r5), 0, 7   // Load weight pair

ps_sub fp3, fp3, fp3         // zero the output vert ZW
psq_l fp0,
0x00(r6), 0, 4   // load position XY

cmplwi cr1, r8,
0x01         // check for one bone case
lbzx r9, r5, r8            // put matrix palette index into r9

cmplwi cr2, r8,
0x02         // check for two bone case
slwi r9, r9,
6             // scale to offest in matrix array

psq_l fp1,
0x04(r6), 1, 4   // load position Z1
add r8, r4, r9            // matrix pointer to r8

psq_l fp5,
0x00(r8), 0, 0   // fp5  is m00 m01
addi r6, r6,
0x06          // Increment the vertex pointer

psq_l fp6,
0x08(r8), 0, 0   // fp6  is m02 m03
...
lbz r9,
0x01(r5)          // start building weight #1 matrix pointer
...
ps_add fp7, fp5, fp7         // add m30 m31 to fp7
slwi r9, r9,
6             // cont building matrix pointer #1, mult by size of pddiMatrix
...
addi r5, r5,
0x08          // Increment the weight pointer
...
ps_add fp7, fp5, fp7         // add m30 m31 to fp7
dcbt        0,
r5                 // cache touch the next weight

// out ZW trans
ps_add
fp8, fp6,
fp8         // add m32 m33 to fp8
...
fp2
        lbz
r8,  0x00(r5)         // Load next weight count
...
psq_l fp1,
0x02(r6), 1, 5   // load normal Z1
add r8, r4, r9            // matrix pointer to r8

psq_l fp5,
0x00(r8), 0, 0   // fp5  is m00 m01
addi r6, r6,
0x03          // Increment the normal pointer
...
// output vertex to fifo
psq_st fp2,
0x00(r12), 0, 5  // normal xy
psq_st fp3,
0x00(r12), 1, 5  // normal z
