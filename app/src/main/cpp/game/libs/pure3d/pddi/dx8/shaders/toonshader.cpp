...
int d3dToonShader::GetPasses() {
    return (m_style == TOON_SILHOUETTE ? 1
                                       : 2);    //a silhouette only takes 1 pass , depth & normal take 2
}
...
void d3dToonShader::SetPass(int pass) {
    ...
    switch (tmp) {
        case TOON_DEFAULT: {
            D3DCOLOR color = D3DCOLOR_ARGB(shaderInfo.diffuse.Alpha(), shaderInfo.diffuse.Red(),
                                           shaderInfo.diffuse.Green(), shaderInfo.diffuse.Blue());
        ...
        case TOON_SILHOUETTE: {
            ...
            d3d->SetAlphaBlend(true, D3DBLENDOP_ADD, PDDI_BF_ZERO,
                               PDDI_BF_ZERO); //we don't draw anything but black
        ...
        case TOON_DEPTH: {
            if (pass == 0) {
                ...
                d3d->SetAlphaTest(false);
                d3d->SetAlphaBlend(true, D3DBLENDOP_ADD, PDDI_BF_ONE,
                                   PDDI_BF_ZERO); //we don't draw anything but black
                d3d->SetMaterial(false, twoSided, NULL);
            }  //end pass 0
            else if (pass == 1) {
                D3DCOLOR color = D3DCOLOR_ARGB(shaderInfo.diffuse.Alpha(), shaderInfo.diffuse.Red(),
                                               shaderInfo.diffuse.Green(),
                                               shaderInfo.diffuse.Blue());
...
void d3dToonShader::GetCubeVector(int i, int cubesize, int x, int y, pddiVector *vector) {
    ...
    mag = 1.0f /
          (float) sqrt(vector->x * vector->x + vector->y * vector->y + vector->z * vector->z);
    vector->x *= mag;
    vector->y *= mag;
    vector->z *= mag;
}
...
void d3dToonShader::CreateNormalMap() {
    ...
    d3dok = d3d->CreateTexture((unsigned int) TOON_CUBE_MAP_RESOLUTION,
                               (unsigned int) TOON_CUBE_MAP_RESOLUTION, 1, NULL, descBack.Format,
                               D3DPOOL_MANAGED, &d3dNormalMap);
...
void d3dToonShader::CreateCubeMap() {
    ...
    d3dok = d3d->CreateCubeTexture((unsigned int) TOON_CUBE_MAP_RESOLUTION, 1, NULL,
                                   descBack.Format, D3DPOOL_MANAGED, &d3dCubeMap);
    PDDIASSERT(d3dok == D3D_OK);
