...
const unsigned PDDI_SP_TOPTEX = PDDI_FOURCC('T', 'T', 'E',
                                            'X');  // used by layered shader; specify the top texture
const unsigned PDDI_SP_TWOLAYERCBV = PDDI_FOURCC('C', 'L', 'Y',
                                                 'R');  // apply cbv to both layers of layered shader
...
const unsigned PDDI_SP_EMISSIVEALPHA = PDDI_FOURCC('E', 'M', 'S',
                                                   'A');  // alpha component of emissive colour (int)
...
const unsigned PDDI_SP_BMSPECULAR = PDDI_FOURCC('B', 'M', 'S',
                                                'P');  // [en|dis]able specular bump mapping
const unsigned PDDI_SP_BMLIGHTDEP = PDDI_FOURCC('B', 'M', 'L',
                                                'D');  // do or don't use a constant light vector for DP3 bumpmapping
...
const unsigned PDDI_SP_TOONSTYLE = PDDI_FOURCC('T', 'N', 'S',
                                               'T');  // 0 = default, 1 = silhouette, 2 = depth/metal
...
const unsigned PDDI_SP_PROJTEX = PDDI_FOURCC('P', 'R', 'J',
                                             'T');  // map for texture projection shader
const unsigned PDDI_SP_PROJTEX0 = PDDI_FOURCC('P', 'R', 'J',
                                              0);  // map for texture projection shader
const unsigned PDDI_SP_PROJTEX1 = PDDI_FOURCC('P', 'R', 'J',
                                              '1');  // map for texture projection shader
const unsigned PDDI_SP_SPECMAP = PDDI_FOURCC('S', 'P', 'M', 'P');
const unsigned PDDI_SP_PROJMATRIX = PDDI_FOURCC('P', 'R', 'M',
                                                'T');  // matrix for texture projection shader
const unsigned PDDI_SP_PROJID = PDDI_FOURCC('P', 'R', 'J',
                                            'N');  // Identify which class of projected textures to use

const unsigned PDDI_SP_TWOSIDED = PDDI_FOURCC('2', 'S', 'I', 'D');

const unsigned PDDI_SP_SHADOWTEXTURE = PDDI_FOURCC('S', 'H', 'T',
                                                   'X');  // Depth map from light view for shadows
const unsigned PDDI_SP_SHADOWCOLOUR = PDDI_FOURCC('S', 'H', 'C',
                                                  'L');  // modulate colour for shadowed area
