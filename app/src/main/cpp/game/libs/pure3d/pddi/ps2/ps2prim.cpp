...
inline unsigned ComponentCount(unsigned vertexFormat) {
    ...
    unsigned mask = (PDDI_V_POSITION | PDDI_V_COLOUR | PDDI_V_NORMAL | PDDI_V_INDICES |
                     PDDI_V_WEIGHTS);
    vertexFormat &= mask;
...
void ps2VertexList::WriteComponent(VifStream &vif, pddiVector *src, int count, unsigned startAddr,
                                   VIF::UnpackFormat format) {
    switch (format) {
        case VIF::V3_32: {
...
void ps2VertexList::WriteComponent(VifStream &vif, pddiVector2 *src, int count, unsigned startAddr,
                                   VIF::UnpackFormat format) {
    switch (format) {
        case VIF::V2_32: {
...
void ps2VertexList::WriteComponent(VifStream &vif, pddiColour *src, int count, unsigned startAddr,
                                   VIF::UnpackFormat format, bool shift) {
    switch (format) {
        case VIF::V4_8: {
            vif.StMask(0xffffffff);
            pddiColour *v = (pddiColour *) vif.Unpack(startAddr, count, VIF::V4_8, NULL,
                                                      VIF::TopsRelative);
            if (shift) {
                for (int i = 0; i < count; i++) {
                    v[i].Set(src[i].Blue() >> 1, src[i].Green() >> 1, src[i].Red() >> 1,
                             src[i].Alpha() >> 1);    // swizzle colours
                }
            } else {
                for (int i = 0; i < count; i++) {
                    v[i].Set(src[i].Blue(), src[i].Green(), src[i].Red(),
                             src[i].Alpha() >> 1);    // swizzle colours
                }
            }
            break;
        ...
        case VIF::V4_5: {
            vif.StMask(0xffffffff);
            unsigned short *v = (unsigned short *) vif.Unpack(startAddr, count, VIF::V4_5, NULL,
                                                              VIF::TopsRelative);
            for (int i = 0; i < count; i++) {
...
void
ps2VertexList::WriteMatrixIndices(VifStream &vif, MatrixIndices *src, int count, unsigned startAddr,
...
void
ps2VertexList::WriteSkinWeights(VifStream &vif, SkinWeights *src, int count, unsigned startAddr) {
    vif.StMask(0xffffffff);
    vif.Unpack(startAddr, count, VIF::V4_16, src, VIF::TopsRelative);
}

void ps2VertexList::QuadPartitionVU(ps2VertexList::VUMemDescription *desc,
    ...
    layout->outputAddress[0] = layout->inputAddress[0] + desc->inputHeaderSize +
                               (layout->nVerticesPerBuffer * desc->inputVertexSize);
    layout->outputAddress[1] = layout->inputAddress[1] + desc->inputHeaderSize +
                               (layout->nVerticesPerBuffer * desc->inputVertexSize);
}
...
static void Dump_VUMemLayout(ps2VertexList::VUMemLayout *layout) {
    scePrintf("base               = 0x%x (%d)\n"
              ...
              layout->scratchAreaAddress, layout->scratchAreaAddress, layout->matrixAreaAddress,
              layout->matrixAreaAddress,
              layout->inputAddress[0], layout->inputAddress[0], layout->inputVertexAddress[0],
              layout->inputVertexAddress[0],
              layout->outputAddress[0], layout->outputAddress[0],
              layout->inputAddress[1], layout->inputAddress[1], layout->inputVertexAddress[1],
              layout->inputVertexAddress[1],
...
void ps2VertexList::DeIndex() {
    ...
    if (matrixIndices) {
        MatrixIndices *di_indices = (MatrixIndices *) EE::malloc_temp(
                sizeof(MatrixIndices) * nIndices);
        for (int i = 0; i < nIndices; i++) {
...
void ps2VertexList::ComputeDMASize(pddiPrimBufferDesc &desc, VUMemLayout &vuLayout,
                                   unsigned &sizeInBytes, unsigned &verticesPerPacket) {
    unsigned format = desc.GetVertexFormat();
...
//    Dump_VUMemLayout(&vuLayout);
    ...
    if (nVertices <= verticesPerPacket) {
        nPacket = 1;
    } else if (desc.GetPrimType() == PDDI_PRIM_TRISTRIP) {
    ...
    if (format & PDDI_V_INDICES) {
        dmaBufferSize +=
                sizeof(MatrixIndices) * nVertices + sizeof(MatrixIndexComponentHeader) * nPacket;
    }
    if (format & PDDI_V_WEIGHTS) {
...
void *
ps2VertexList::CreateDMABuffer(pddiPrimBufferDesc &desc, void *dmaBuffer, unsigned &dmaBufferSize) {
    if (indices) {
...
//        scePrintf("dmaBufferSize=%d bytes\n", dmaBufferSize);
        PDDIASSERT((dmaBufferSize / 16 < 65536) &&
                   "PDDI ERROR: PS2 Prim DMA buffer to large (too many vertices) Sorry!\n");
    ...
    while (verticesRemaining) {
        ...
        for (int i = 0; i < MAX_UV_CHANNELS; i++) {
            if (st[i]) {
                WriteComponent(vif, &st[i][currentVertex], verticesThisPacket, startAddr + offset,
                               stFormat[i]);
                offset++;
            }
        }
        ...
        if (colour) {
            WriteComponent(vif, &colour[currentVertex], verticesThisPacket, startAddr + offset,
                           colourFormat, shiftColours);
            offset++;
        }

        if (normal) {
            WriteComponent(vif, &normal[currentVertex], verticesThisPacket, startAddr + offset,
                           normalFormat);
            offset++;
        }
        ...
        if (matrixIndices) {
            ...
            WriteMatrixIndices(vif, &matrixIndices[currentVertex], verticesThisPacket,
                               startAddr + offset, desc.GetMatrixCount());
            offset++;
        }

        if (skinWeights) {
            WriteSkinWeights(vif, &skinWeights[currentVertex], verticesThisPacket,
                             startAddr + offset);
            offset++;
        }
...
ps2PrimBuffer::ps2PrimBuffer(ps2Context *c, pddiPrimBufferDesc *d) : desc(*d), context(c) {
    ...
    PDDIASSERT((desc.GetPrimType() == PDDI_PRIM_TRIANGLES) ||
               (desc.GetPrimType() == PDDI_PRIM_TRISTRIP));
    ...
    if (!(format & PDDI_V_COLOUR) &&
        !(format & PDDI_V_NORMAL))  // hack to force one of colours or normals
    {
...
void *ps2PrimBuffer::LockMemImage(unsigned memImageLength) {
    ...
    PDDIASSERT(dmaBufferSize == memImageLength &&
               "PDDI ERROR:  Memory image size != Memory image file size.");
    PDDIASSERT((dmaBufferSize / 16 < 65536) &&
               "PDDI ERROR: PS2 Prim DMA buffer to large (too many vertices) Sorry!\n");
...
void ps2PrimBuffer::Finalize() {
    ...
    if (inputVertices->skinWeights != NULL) {
        for (a = 0; a < desc.GetVertexCount(); a++) {
            ...
            if (weightcount == 2)
                inputVertices->matrixIndices[a].i3 = inputVertices->matrixIndices[a].i2;
            else if (weightcount == 1)
                inputVertices->matrixIndices[a].i3 = inputVertices->matrixIndices[a].i1;
            else if (weightcount == 0)
                inputVertices->matrixIndices[a].i3 = inputVertices->matrixIndices[a].i0;
        }
