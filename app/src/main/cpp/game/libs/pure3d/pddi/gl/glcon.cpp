...
pglContext::pglContext(pglDevice *dev, pglDisplay *disp) : pddiBaseContext((pddiDisplay *) disp,
                                                                           (pddiDevice *) dev) {
    device = dev;
    display = disp;
...
void pglContext::SetupHardwareProjection(void) {
    switch (state.viewState->projectionMode) {
        ...
        case PDDI_PROJECTION_ORTHOGRAPHIC :
            ...
            glLoadIdentity();
            glOrtho(-0.5, 0.5,
                    -((1 / state.viewState->camera.aspect) / 2),
                    ((1 / state.viewState->camera.aspect) / 2),
                    (state.viewState->camera.nearPlane), (state.viewState->camera.farPlane));
            glViewport(int(state.viewState->viewWindow.left * display->GetWidth()),
                       ...
                       int((state.viewState->viewWindow.right - state.viewState->viewWindow.left) *
                           display->GetWidth()),
                       int((state.viewState->viewWindow.bottom - state.viewState->viewWindow.top) *
                           display->GetHeight()));
            glMatrixMode(GL_MODELVIEW);
            break;
        ...
        case PDDI_PROJECTION_PERSPECTIVE : {
            ...
            glMatrixMode(GL_PROJECTION);
            glLoadIdentity();
            glFrustum(-halfX, halfX, -halfY, halfY, state.viewState->camera.nearPlane,
...
void pglContext::SetScissor(pddiRect *rect) {
    ...
    if (!rect) {
        ...
        glScissor(rect->left, display->GetHeight() - rect->bottom, rect->right - rect->left,
...
pddiPrimStream *
pglContext::BeginPrims(pddiShader *mat, pddiPrimType primType, unsigned vertexType, int vertexCount,
                       unsigned pass) {
    if (!mat)
        mat = defaultShader;
...
pglPrimBuffer::pglPrimBuffer(pglContext *c, pddiPrimType type, unsigned vertexFormat, int nVertex,
                             int nIndex) : context(c) {
