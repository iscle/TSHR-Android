...
static volatile ps2Texture *textureUploadQueue[UPLOAD_QUEUE_ENTRIES] __attribute__((aligned(16))) = {
        NULL};
...
int ps2TexCache::AllocateCache(ps2Texture *tex) {
    ...
    PDDIASSERT((texMemAllocate + tex->GetVramFootprint()) <= texMemEnd &&
               "Texture is too large to fit into VRAM");
...
bool ps2TexCache::Insert(ps2Texture *tex) {
    ...
    if ((tex->cacheInfo.state == ps2Texture::PS2_TEX_CACHED) &&
        (tex->GetVideoMemoryAddress() > 0) && (!forceUploads)) {
        doUpload = false;
    } else {
        doUpload = true;
    }
    ...
    if ((tex->GetVideoMemoryAddress() < texMemStart) ||
        ((tex->GetVideoMemoryAddress() + tex->GetVramFootprint()) > texMemEnd)) {
...
void ps2TexCache::ResolveCacheConflict(ps2Texture *tex, ps2Texture *tex2) {
    ...
    PDDIASSERT((texMemAllocate >= texMemStart && texMemAllocate < texMemEnd) &&
               "Invalid texMem address");
...
void ps2TexCache::EnsureResident(VifStream &vif, ps2Texture *texture) {
    ...
    if (needToUpload) {
        PDDIASSERT(
                (textureUploadQueue[textureUploadQueueHead] == NULL) && "Upload queue too small!!");
