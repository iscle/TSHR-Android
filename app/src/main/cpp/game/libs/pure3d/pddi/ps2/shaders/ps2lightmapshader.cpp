...
void ps2LightmapShader::Validate(unsigned format) {
    PDDIASSERT(((format & 0xf) == 2) &&
               "PDDI ERROR: ps2LightmapShader: Invalid vertex format (texture co-ord count != 2)");
    PDDIASSERT((format & PDDI_V_POSITION) &&
               "PDDI ERROR: ps2LightmapShader:  Invalid vertex format (no positions");
    PDDIASSERT((format & PDDI_V_COLOUR) &&
               "PDDI ERROR:  ps2LightmapShader: Invalid vertex format (no colours)");
    PDDIASSERT(!(format & PDDI_V_NORMAL) &&
               "PDDI ERROR:  ps2LightmapShader: Invalid vertex format (normals present, colours required)");
    PDDIASSERT(!(format & PDDI_V_INDICES) &&
               "PDDI ERROR:  ps2LightmapShader: Invalid vertex format (skinning not supported)");
...
void ps2LightmapShader::Bind(GSContext *gsContext, VifStream &vif) {
    ...
    qwords++;
    pkt.ad(SCE_GS_MIPTBP1_1, &pass[0].miptbp1);
    qwords++;
    pkt.ad(SCE_GS_MIPTBP2_1, &pass[0].miptbp2);
    qwords++;
    pkt.ad(SCE_GS_CLAMP_1, &pass[0].clamp);
    qwords++;
