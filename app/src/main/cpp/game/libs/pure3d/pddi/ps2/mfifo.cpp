...
ps2MFIFO::ps2MFIFO(const int size) {
    ...
    rb_size = size;
    rb_addr = (unsigned *) radMemoryAllocAligned(0, rb_size,
                                                 rb_size);//this is rerouted due to the huge gap that would be left were this to be alloc'd to peristent//MallocAligned(rb_size, rb_size);
...
void ps2MFIFO::Enable(const bool e) {
    ...
    if (e) {
        ...
        *D1_CHCR = 1 | (1 << 2) | (1 << 6) |
                   (1 << 8); // from memory | chain mode | tag transfer | start
    } else {
        enabled = false;
