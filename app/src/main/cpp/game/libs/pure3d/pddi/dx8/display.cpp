...
bool d3dDisplay::InitDisplay(const pddiDisplayInit *init) {
    ...
    if (!(d3dCaps.Caps2 & D3DCAPS2_CANRENDERWINDOWED) && (mode == PDDI_DISPLAY_WINDOW)) {
        ...
        OutputDebugString(
                "PDDI: Cannot use a selected adapter in a window, switching to fullscreen.\n");
    }
    ...
    {
        D3DFORMAT *testDepthFormats = depthFormatTable[((displayDepth == 32) ? 1 : 0) |
                                                       ((bufferMask & PDDI_BUFFER_STENCIL) ? 2
                                                                                           : 0)];
    ...
    */
    ...
    d3dpp.FullScreen_PresentationInterval = (mode == PDDI_DISPLAY_WINDOW)
                                            ? D3DPRESENT_INTERVAL_DEFAULT : (lockToVsync
...
/*      if(rc != D3DERR_DEVICELOST)
    ...
    if (init->enableSnapshot) {
        ...
        DDVERIFY(d3dDevice->CreateTexture(descRenderTarget.Width, descRenderTarget.Height, 1,
                                          D3DUSAGE_RENDERTARGET, /*descRenderTarget.Format*/
                                          D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &snapshot));
        ...
        DDVERIFY(d3dDevice->CreateImageSurface(dm.Width, dm.Height, D3DFMT_A8R8G8B8,
                                               &frontTmpBuffer));
    }
...
unsigned d3dDisplay::Screenshot(pddiColour *buffer, int nBytes, const pddiRect &rectangle,
                                pddiBuffer source) {
    ...
    if (source == PDDI_BUFFER_BACK) {
        ...
        DDVERIFY(d3dDevice->CreateImageSurface(width, height, colourBufferFormat,
                                               &dest)); // TODO : breaks in 16bpp
