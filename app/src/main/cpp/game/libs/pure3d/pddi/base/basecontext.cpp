...
void pddiBaseContext::ComputeFrameStats() {
    stats[PDDI_STAT_BUFFERED_PRIM_AVG] =
            stats[PDDI_STAT_BUFFERED_PRIM_CALLS] > 0.0f ?
            (stats[PDDI_STAT_BUFFERED_PRIM] + stats[PDDI_STAT_BUFFERED_INDEXED_PRIM]) /
            stats[PDDI_STAT_BUFFERED_PRIM_CALLS] : 0.0f;
...
pddiPrimStream *
pddiBaseContext::BeginPrims(pddiShader *shader, pddiPrimType primType, unsigned vertexType,
                            int vertexCount, unsigned pass) {
    PDDIASSERT(shader);
    return 0;
}
...
void
pddiBaseContext::SetStencilOp(pddiStencilOp failOp, pddiStencilOp zFailOp, pddiStencilOp zPassOp) {
    state.stencilState->failOp = failOp;
    state.stencilState->zFailOp = zFailOp;
    state.stencilState->zPassOp = zPassOp;
}

void pddiBaseContext::GetStencilOp(pddiStencilOp *failOp, pddiStencilOp *zFailOp,
...
bool pddiAssertFailed(const char *file, int line, const char *cond, const char *msg,
                      const char *module) {
    char msgbuf[1024];
    if (!msg && !module) {
