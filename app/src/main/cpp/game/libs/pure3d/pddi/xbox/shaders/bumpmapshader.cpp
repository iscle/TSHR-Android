...
pddiShadeVectorTable d3dBumpMapShader::vectorTable[] =
        {
                {PDDI_SP_LIGHTV,  SHADE_VECTOR(
                                          SetLightVector)},       //must set the light vector for both diffuse and specular mode
                {PDDI_SP_CAMERAV, SHADE_VECTOR(
                                          SetCameraVector)},     //only need to set the camera vector for specular mode
                {PDDI_SP_NULL,    NULL}
        };
...
void d3dBumpMapShader::CreateCubeMap() {
    ...
    d3dok = d3d->CreateCubeTexture(CUBE_MAP_RESOLUTION, 1, NULL, descBack.Format, D3DPOOL_MANAGED,
                                   &d3dCubeMap);
...
bool d3dBumpMapShader::diff(pddiVector a, pddiVector b) {
    ...
    a.x -= b.x;
    a.y -= b.y;
    a.z -= b.z;      //translate a - b to origin
