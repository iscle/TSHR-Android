...
static float convertFixedPointToFloat(u_int fpr, u_int sign, u_int integer, u_int fractional) {
    ...
    val = (float) (fpr &
                   ((1 << (integer + fractional)) - 1));    // obtain the value (without the sign)
    val /= (float) (1 << fractional);

    if (sign > 0 &&
        (fpr >> (integer + fractional)) & 1) {    // if sign matters, and if sign was set...
        val = -val;
    }
    return val;
}
...
static char *disassemblePrim(u_int p, BOOL outputPrimType, BOOL verbose) {
    ...
    for (i = 0; i < 9; i++) {
        if ((p >> 3) & (1 << i)) {
            if (verbose) {
                DEBUG_addField(ENTRY_FIELD, o, DATA_STR
                "("
                VERBOSE
                "%s"
                NORM
                ")", gPrimFields[i], gPrimFieldsVerbose[i]);
            } else {
...
static int disassembleRegister(int regnum, u_int *d, char *s) {
    ...
    if (i == NUMGIFREGS) {
        sprintf(gTempString, ERRORCOL
        "Invalid GS register encountered : %02x"
        NORM, regnum);
    ...
    DEBUG_addField(ENTRY_TITLE, s, GSREGISTERTITLE
    "GS %-10s"
    NORM
    " "
    DATASOURCEADDRESS, gRegs[i].regname, (int) gGifInput);

    if (gVerbose) {
        DEBUG_addField(ENTRY_FIELD, s, "["
        MEMCONTENTS_64
        "]", d[1], d[0]);
    }
    ...
    if (gVerbose) {
        DEBUG_addField(ENTRY_FIELD, s, VERBOSE
        "%s"
        NORM, gRegs[i].description);
    }
    ...
    switch (gRegs[i].address) {
        ...
        case 0x01:        // RGBAQ
        {
            ...
            float q = *(float *) &d[1];
            DEBUG_addField(ENTRY_FIELD, s, "RGBA=("
            DATA_2HEX
            ", "
            DATA_2HEX
            ", "
            ...
            DEBUG_addField(ENTRY_FIELD, s, "Q="
            DATA_FLOAT
            " ["
            DATA_8HEX
            "]", gTempString, d[1]);
        ...
        case 0x0c:        // XYZF3 (no Kick)
        {
            ...
            if (gRegs[i].address == 0x0c) {
                DEBUG_addField(ENTRY_FIELD, s, COLOUR_BOLD
                "(No Kick) "
                NORM);
            }

            DEBUG_addField(ENTRY_FIELD, s, "XYZF=(%s, "
            DATA_HEX
            ", "
            DATA_HEX
            ")",
        ...
        case 0x0d:        // XYZ3 (no Kick)
        {
            ...
            DEBUG_addField(ENTRY_FIELD, s, "XYZ=(%s, "
            DATA_HEX
            ")", gTempString);
            DEBUG_addField(ENTRY_FIELD, s, "=("
            DATA_4HEX
        ...
        case 0x07: {
            ...
            if (psmstr == NULL) psmstr = ERRORCOL
            "*UNKNOWN*"
            NORM;
            if (cpsmstr == NULL) cpsmstr = ERRORCOL
            "*UNKNOWN*"
            NORM;

            if ((gRegs[i].address & 0x10) == 0) {
                DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_4HEX
                " [TBP*64="
                DATA_6HEX
                "]", fname[TBP0], f[TBP0], f[TBP0] * 64);
                DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_2HEX, fname[TBW], f[TBW]);
            }

            DEBUG_addField(ENTRY_FIELD_NL, s, NAME_DATA_HEX
            VERBOSEA, fname[PSM], f[PSM], psmstr);

            if ((gRegs[i].address & 0x10) == 0) {
                DEBUG_addField(ENTRY_FIELD_NL, s, NAME_DATA_STR, fname[TCC],
                               f[TCC] ? "RGBA" : "RGB+TEXA");
                DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_STR, fname[TFX], texturefunction[f[TFX]]);
                DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_HEX
                " (="
                DATA_DEC
                " txls)", fname[TW], f[TW], 1 << f[TW]);
                DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_HEX
                " (="
                DATA_DEC
                " txls)", fname[TH], f[TH], 1 << f[TH]);
            }

            DEBUG_addField(ENTRY_FIELD_NL, s, NAME_DATA_4HEX
            " [CBP*64="
            DATA_6HEX
            "]", fname[CBP], f[CBP], f[CBP] * 64);
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_HEX
            VERBOSEA, fname[CPSM], f[CPSM], cpsmstr);
            DEBUG_addField(ENTRY_FIELD, s, "%s="
            DATA
            "CSM%d"
            NORM, fname[CSM], f[CSM]);
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_2HEX
            " [CSA*16="
            DATA_4HEX
            "]", fname[CSA], f[CSA], f[CSA] * 16);
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_HEX
            VERBOSEA, fname[CLD], f[CLD], loadcontrol[f[CLD]]);

            if (f[CLD] == 0x6 || f[CLD] == 0x7) {
                err |= AddGifError(WARN_GIF_INV_FIELD, ERROR_FATAL_CONT,
                                   "Invalid load control parameter");
            }
            if (f[CSA] == 0x1 && f[CSA] != 0) {
                err |= AddGifError(WARN_GIF_INV_FIELD, ERROR_FATAL_CONT,
                                   "CSM is 1, but CSA is not zero!!");
            }
            if (getPixelStorageFormatString(f[PSM]) == NULL || f[PSM] == 0x12) {
                err |= AddGifError(WARN_GIF_INV_FIELD, ERROR_FATAL_CONT,
                                   "Invalid pixel storage format");
            }
            if (f[CPSM] != 0 && f[CPSM] != 2 && f[CPSM] != 0xa) {
                err |= AddGifError(WARN_GIF_INV_FIELD, ERROR_FATAL_CONT,
                                   "Invalid clut storage format");
            }
        }
            break;
        ...
        case 0x09: {
            ...
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_HEX
            VERBOSEA, fname[WMS], f[WMS], fieldu[f[WMS]]);
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_HEX
            VERBOSEA, fname[WMT], f[WMT], fieldu[f[WMT]]);
        ...
        case 0x15: {
            ...
            char *mminfields[] = {"NEAREST", "LINEAR", "NEAREST_MIPMAP_NEAREST",
                                  "NEAREST_MIPMAP_LINEAR",
                                  "LINEAR_MIPMAP_NEAREST", "LINEAR_MIPMAP_LINEAR",
                                  ERRORCOL"*RESERVED*"NORM, ERRORCOL"*RESERVED*"NORM};
            char *mtbafields[] = {"Value specified by TBP1 & TBP2 used",
                                  "Base address of TBP1-3 automatically set"};
            float k = convertFixedPointToFloat(f[K], 1, 7, 4);

            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_STR, fname[LCM],
                           f[LCM] ? "K" : "(log2(1/|Q|)<<L)+K");
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_DEC, fname[MXL], f[MXL]);
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_STR, fname[MMAG],
                           f[MMAG] ? "LINEAR" : "NEAREST");
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_STR, fname[MMIN], mminfields[f[MMIN]]);
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_DEC
            VERBOSEA, fname[MTBA], f[MTBA], mtbafields[f[MTBA]]);
            ...
            if (f[MMIN] > 5) {
                err |= AddGifError(WARN_GIF_INV_FIELD, ERROR_FATAL_CONT,
                                   "MMIN is set to a reserved value");
            }
        }
            break;
        ...
        case 0x17: {
            ...
            char *loadcontrol[] = {"Temp buff not changed", "Load to CSA position",
                                   "Load to CSA position, CBP0<-CBP",
                                   "Load to CSA position, CBP1<-CBP",
                                   "If CBP0<>CBP, then load and CBP0<-CBP",
                                   "If CBP1<>CBP, then load and CBP1<-CBP",
        ...
        case 0x1a:        // PRMODECONT
        {
            ...
            if (gVerbose) {
                DEBUG_addField(ENTRY_FIELD, s, VERBOSE
                "%s used"
                NORM, f[AC] ? "Enabled - PRIM" : "Disabled - PRMODE");
            }
        }
            break;
        ...
        case 0x1c:        // TEXCLUT
        {
            ...
            " [*64="
            DATA_4HEX
            "]", fname[CBW], f[CBW], f[CBW] * 64);
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_2HEX
            " [*16="
            DATA_4HEX
            "]", fname[COU], f[COU], f[COU] * 16);
        ...
        case 0x22:        // SCANMSK
        {
            ...
            char *fieldmsk[] = {"Not masked", "*RESERVED*",
                                "Drawing with even Y coord is prohibited",
        ...
        case 0x37: {
            typedef enum {
                TBP1, TBW1, TBP2, TBW2, TBP3, TBW3
            } MIPTBP1;
            int startReg = (gRegs[i].address == 0x36 || gRegs[i].address == 0x37) ? 4 : 1;

            DEBUG_addField(ENTRY_FIELD, s, "TBP%d="
            DATA_4HEX, startReg, f[TBP1]);
            DEBUG_addField(ENTRY_FIELD, s, "TBW%d="
            DATA_4HEX, startReg, f[TBW1]);
            startReg++;
            DEBUG_addField(ENTRY_FIELD, s, "TBP%d="
            DATA_4HEX, startReg, f[TBP2]);
            DEBUG_addField(ENTRY_FIELD, s, "TBW%d="
            DATA_4HEX, startReg, f[TBW2]);
            startReg++;
            DEBUG_addField(ENTRY_FIELD, s, "TBP%d="
            DATA_4HEX, startReg, f[TBP3]);
            DEBUG_addField(ENTRY_FIELD, s, "TBW%d="
            DATA_4HEX, startReg, f[TBW3]);
        }
            break;
        ...
        case 0x3b:        // TEXA
        {
            ...
            if (gVerbose) {
                DEBUG_addField(ENTRY_FIELD, s, VERBOSEA,
                               f[AEM] ? "A=0 when R=G=B=0" : "Processed normally when R=G=B=0");
            }
        }
            break;
        ...
        case 0x3d:        // FOGCOL
        {
            ...
            DEBUG_addField(ENTRY_FIELD, s, "RGB=("
            DATA_2HEX
            ", "
            DATA_2HEX
            ", "
        ...
        case 0x43: {
            ...
            if (f[A] == 3 || f[B] == 3 || f[C] == 3 || f[D] == 3) {
                err |= AddGifError(WARN_GIF_INV_FIELD, ERROR_FATAL_CONT,
                                   "Reserved value used in alpha formula!");
            }
        }
            break;
        ...
        case 0x46:        // COLCLAMP
        {
            ...
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_STR, fname[CLAMP],
                           f[CLAMP] ? "CLAMP" : "MASK");
        }
            break;
        ...
        case 0x4d: {
            ...
            " [*2048="
            DATA_8HEX
            "]", fname[FBP], f[FBP], f[FBP] * 2048);
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_2HEX
            " (="
            DATA_DEC
            " pixels)", fname[FBW], f[FBW], f[FBW] * 64);
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_DEC
            VERBOSEA, fname[PSM], f[PSM], psmstr);
        ...
        case 0x50:        // BITBLTBUF
        {
            ...
            if (spsmstr == NULL) spsmstr = ERRORCOL
            "*UNKNOWN*"
            NORM;
            if (dpsmstr == NULL) dpsmstr = ERRORCOL
            "*UNKNOWN*"
            NORM;
        ...
        case 0x53:        // TRXDIR
        {
            ...
            char *xdirfields[] = {"Host -> Local", "Local -> Host", "Local -> Local",
                                  "Trasnmission disabled"};

            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_DEC
            VERBOSEA, fname[XDIR], f[XDIR], xdirfields[f[XDIR]]);
        }
            break;

        case 0x54:        // HWREG
        {
            DEBUG_addField(ENTRY_FIELD, s, "DATA="
            DATA
            "%08x.%08x"
            NORM, d[1], d[0]);
        }
        ...
        case 0x62:        // LABEL
        {
            ...
            DEBUG_addField(ENTRY_FIELD, s, "(ID & IDMSK)="
            DATA_8HEX, f[ID] & f[IDMSK]);
        }
            break;
        ...
        case 0x80:        // PMODE
        {
            ...
            if (f[CRTMD] != 0) {
                err |= AddGifError(WARN_GIF_INV_FIELD, ERROR_FATAL_CONT,
                                   "Invalid CRTMD value (should be 0)");
            }
        }
            break;
        ...
        case 0x82:        // SMODE2
        {
            ...
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_DEC
            VERBOSEA, fname[INTT], f[INTT], f[INTT] ? "INTERLACE" : "NON-INTERLACE");
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_DEC
            VERBOSEA, fname[FFMD], f[FFMD], f[FFMD] ? "FRAME" : "FIELD");
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_DEC
            VERBOSEA, fname[DPMS], f[DPMS], dpmsnames[f[DPMS]]);
        }
            break;
        ...
        case 0x89: {
            ...
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_2HEX
            VERBOSEA, fname[PSM], f[PSM], psmstr);
            DEBUG_addField(ENTRY_FIELD, s, "DBX/Y=("
            DATA_4HEX
            ", "
            DATA_4HEX
            ") = ("
        ...
        case 0x8a: {
            typedef enum {
                DX, DY, DW, DH, MAGH, MAGV
            } DISPLAY;

            DEBUG_addField(ENTRY_FIELD, s, "DX/Y=("
            DATA_4HEX
            ", "
            DATA_4HEX
            ")", f[DX], f[DY]);
            DEBUG_addField(ENTRY_FIELD, s, "DW/H=("
            DATA_4HEX
            ", "
            DATA_4HEX
            ")", f[DW], f[DH]);
        ...
        {
            typedef enum {
                EXBP, EXBW, FBIN, WFFMD, EMODA, EMODC, WDX, WDY
            } EXTBUF;
            char *emodafield[] = {"Input alpha written as is", "Value from RGB->LumY written",
                                  "Value from RGB->LumY/2 written", "Always 0"};
            char *emodcfield[] = {"Input alpha written as is",
                                  "Value from RGB->LumY written to RGB",
                                  "Value from RGB->YCbCr written",
                                  "Input Alpha written to RGB resp."};
            ...
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_DEC
            VERBOSEA, fname[FBIN], f[FBIN], f[FBIN] ? "OUT2" : "OUT1");
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_DEC
            VERBOSEA, fname[WFFMD], f[WFFMD], f[WFFMD] ? "FRAME" : "FIELD");
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_DEC
            VERBOSEA, fname[EMODA], f[EMODA], emodafield[f[EMODA]]);
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_DEC
            VERBOSEA, fname[EMODC], f[EMODC], emodcfield[f[EMODC]]);
            DEBUG_addField(ENTRY_FIELD, s, "WDX/Y=("
            DATA_4HEX
            ", "
            DATA_4HEX
            ")", f[WDX], f[WDY]);
        ...
        {
            typedef enum {
                SX, SY, SMPH, SMPV, WW, WH
            } EXTDATA;
            DEBUG_addField(ENTRY_FIELD, s, "SX/Y=("
            DATA_4HEX
            ", "
            DATA_4HEX
            ")", f[SX], f[SY]);
            DEBUG_addField(ENTRY_FIELD, s, "WW/H=("
            DATA_4HEX
            ", "
            DATA_4HEX
            ")", f[WW], f[WH]);
        ...
        {        // We don't disassemble any field beyond NFIELD, since these are read only
            typedef enum {
                SIGNAL, FINISH, HSINT, VSINT, FLUSH, RESET
            } CSR;
        ...
        case 0xc4:        // BUSDIR
        {
            ...
            DEBUG_addField(ENTRY_FIELD, s, NAME_DATA_DEC
            VERBOSEA, fname[DIR], f[DIR], f[DIR] ? "Local -> Host" : "Host->Local");
        }
            break;
        ...
        {
            DEBUG_addField(ENTRY_FIELD, s, "*GIFTAG* REGS field = 0xb ("
            ERRORCOL
            "*RESERVED*"
            NORM
            "). Skipping this field.");
...
static int RegDesc2GSRegister(int regDesc, u_int *d, BOOL packedMode) {
    if (regDesc <= REGDESC_FOG || regDesc == REGDESC_XYZF3 || regDesc == REGDESC_XYZ3) {
        ...
        if (packedMode && (regDesc == REGDESC_XYZF2 || regDesc == REGDESC_XYZ2) &&
            (d[3] & 0x8000)) {
            return regDesc + 0x08;
        }
        return regDesc;
...
static int disassemblePacked(int regidx, u_int *d, char *s) {
    ...
    switch (regidx) {
        ...
        case REGDESC_ST:
            ...
            q = *(float *) &d[2];
            s += sprintf(s, "  STQ encountered - setting internal GIF Q to "
            DATA
            "%f"
            NORM
            "\n", q);
...
//    printf("reg count = %d, regindex = %d, loopcnt = %d\n", g_cnt.REGCNT, regindex, g_cnt.LOOPCNT);
                ...
                if (g_cnt.LOOPCNT != 0) {
                    ...
                    ASSERT((offset != 0),
                           "Logic error - came across a NOP in the first half of a quadword.\n");
...
static int parseGifTag(char *s) {
    ...
    DEBUG_addField(ENTRY_TITLE, s, GIFTAGTITLE
    "GIFTAG"
    NORM
    "%7s "
    DATASOURCEADDRESS, "", gt.tagaddr);
    DEBUG_addField(ENTRY_FIELD, s, "["
    MEMCONTENTS_128
    "]", g_tag3, g_tag2, g_tag1, g_tag0);
    DEBUG_addField(ENTRY_FIELD, s, "Expects "
    DATA_HEX
    " regs", tsize);
    ...
    if (gt.flg == GIFTAG_PACKED) {
        ...
        if (gt.pre) {
            DEBUG_addField(ENTRY_FIELD_NL, s, NAME_DATA_STR, "PRIM",
                           disassemblePrim(gt.prim, TRUE, gVerbose));
        }
    }
    ...
    if (gt.flg == GIFTAG_PACKED || gt.flg == GIFTAG_REGLIST) {
        ...
        for (i = 0; i < gt.nreg; i++) {
            DEBUG_addField(ENTRY_FIELD, s, "%d:"
            DATA
            "%s"
            NORM
            "  ", i, gGTRegisterNames[(int) gt.regidx[i]]);
        }
    }
    ...
    if (gt.flg != GIFTAG_PACKED) {
        if (gt.pre != 0) {
            sprintf(gTempString,
                    "Warning, PRE bit is set, but FLG is not PACKED. PRIM is ignored.");
        ...
        for (i = 0; i < 16; i++) {
            ...
            if (i >= gt.nreg && gt.regidx[i] != 0) {
                sprintf(gTempString,
                        "Regs fields are set, but NREGS is not large enough to use them.");
...
int GIF_emulateGifCycle(void) {
    ...
    if (((int) gGifInput & 0xf) != 0) {
        sprintf(gTempString,
                "DMA List is not aligned on a 4 byte boundary. (Address = %08x)",
                (u_int) gGifInput);
