...
bool SupportsMode(unsigned mode) {
    ...
    if (sDisplayModes[mode].height == 480 && sDisplayModes[mode].widescreen &&
        !(flags & XC_VIDEO_FLAGS_WIDESCREEN))
        return FALSE;

    // Explicit check for 480p
    if (sDisplayModes[mode].height == 480 && sDisplayModes[mode].progressive &&
        !(flags & XC_VIDEO_FLAGS_HDTV_480p))
        return FALSE;
...
bool d3dDisplay::InitDisplay(const pddiDisplayInit *init) {
    ...
    if (displayInit.allowHDTV) {
        ...
        while (!done && (useMode < NUM_MODES)) {
            if (SupportsMode(useMode) &&
                ...
                ((sDisplayModes[useMode].widescreen && displayInit.allowWidescreen) ||
                 !sDisplayModes[useMode].widescreen)) {
                done = true;
                displayInit.xsize = sDisplayModes[useMode].width;
                displayInit.ysize = sDisplayModes[useMode].height;
                hdtvFlags = sDisplayModes[useMode].progressive ? D3DPRESENTFLAG_PROGRESSIVE
    ...
    d3dpp.FullScreen_PresentationInterval = lockToVsync ? D3DPRESENT_INTERVAL_ONE
