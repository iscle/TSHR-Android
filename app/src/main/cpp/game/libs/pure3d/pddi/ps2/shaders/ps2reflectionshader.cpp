...
static pddiBaseShader *
ps2ReflectionShaderAllocateEnv(pddiRenderContext *c, const char *shaderName, const char *aux) {
    return new ps2ReflectionShader((ps2Context *) c, ps2ReflectionShader::Environment);
}

static pddiBaseShader *
ps2ReflectionShaderAllocateSph(pddiRenderContext *c, const char *shaderName, const char *aux) {
    return new ps2ReflectionShader((ps2Context *) c, ps2ReflectionShader::SphereMap);
}

static pddiBaseShader *
ps2ReflectionShaderAllocatePla(pddiRenderContext *c, const char *shaderName, const char *aux) {
    return new ps2ReflectionShader((ps2Context *) c, ps2ReflectionShader::PlanarMap);
}

static pddiBaseShader *
ps2ReflectionShaderAllocateRef(pddiRenderContext *c, const char *shaderName, const char *aux) {
...
void ps2ReflectionShader::Validate(unsigned format) {
    PDDIASSERT(((format & 0xf) == 1) &&
               "PDDI ERROR: ps2ReflectionShader: Invalid vertex format (texture co-ord count != 1)");
    PDDIASSERT((format & PDDI_V_POSITION) &&
               "PDDI ERROR: ps2ReflectionShader:  Invalid vertex format (no positions");
    PDDIASSERT(!(format & PDDI_V_INDICES) &&
               "PDDI ERROR:  ps2ReflectionShader: Invalid vertex format (skinning not supported)");
    PDDIASSERT(!(format & PDDI_V_WEIGHTS) &&
               "PDDI ERROR:  ps2ReflectionShader: Invalid vertex format (skinning not supported)");
}
...
void ps2ReflectionShader::Bind(GSContext *gsContext, VifStream &vif) {
    ...
    pkt.BeginTag();
    qwords++;
    pkt.ad(SCE_GS_PRMODE, &pass[0].prmode);
    qwords++;
    pkt.ad(SCE_GS_TEX0_1, &pass[0].tex0);
    qwords++;
    pkt.ad(SCE_GS_TEX1_1, &pass[0].tex1);
    qwords++;
    pkt.ad(SCE_GS_ALPHA_1, &pass[0].alpha);
    qwords++;
    pkt.ad(SCE_GS_TEST_1, &pass[0].test);
    qwords++;
    pkt.EndTag();
    ...
    pkt.BeginTag();
    qwords++;
    pkt.ad(SCE_GS_PRMODE, &pass[1].prmode);
    qwords++;
    pkt.ad(SCE_GS_TEX0_1, &pass[1].tex0);
    qwords++;
    pkt.ad(SCE_GS_TEX1_1, &pass[1].tex1);
    qwords++;
    pkt.ad(SCE_GS_TEST_1, &pass[1].test);
    qwords++;
    pkt.ad(SCE_GS_ALPHA_1, &pass[1].alpha);
    qwords++;
    pkt.EndTag();
