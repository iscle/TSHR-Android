...
void CalculateDepthMapping(float fDepth0, float fDepth1, float *pfAlphaOffset, float *pfAlphaSlope,
...
void d3dExtFocusBlurEffect::BeginFocusBlur() {
    if (!m_dwFocusPixelShader) {
        ...
        m_pContext->GetDisplay()->GetD3DDevice()->CreatePixelShader(&foo->Psd,
                                                                    &m_dwFocusPixelShader);
    }
...
void d3dExtFocusBlurEffect::BlurBlurryTexture(FilterSample fSample[], int filterSize) {
    ...
    XGSetTextureHeader(descRenderTarget.Width, descRenderTarget.Height, 1, 0,
                       descRenderTarget.Format, 0, &SourceTexture, pRenderTarget->Data,
    ...
    pD3DDevice->SetRenderState(D3DRS_BLENDOP,
                               D3DBLENDOP_ADD);    // setup subsequent renderings to add to previous value
    ...
    for (xx = 0; xx < 4; xx++) {
        ...
        pD3DDevice->SetTextureStageState(xx, D3DTSS_TEXTURETRANSFORMFLAGS,
                                         D3DTTFF_DISABLE);  // pass texture coords without transformation
        pD3DDevice->SetTextureStageState(xx, D3DTSS_TEXCOORDINDEX,
                                         xx); // each texture has different tex coords
    ...
    pD3DDevice->SetVertexShader(
            D3DFVF_XYZRHW | D3DFVF_TEX4);   // for screen-space texture-mapped quadrilateral
    ...
    for (int iSample = 0; iSample < filterSize; iSample++) {
        ...
        if (fWeight < 0.f) {
            ...
            rPSInput[xx] =
                    PS_INPUTMAPPING_SIGNED_NEGATE | ((xx % 2) ? PS_REGISTER_C1 : PS_REGISTER_C0);
        } else {
            filterWeights[xx] = D3DXCOLOR(fWeight, fWeight, fWeight, fWeight);
            rPSInput[xx] =
                    PS_INPUTMAPPING_SIGNED_IDENTITY | ((xx % 2) ? PS_REGISTER_C1 : PS_REGISTER_C0);
        }
        ...
        if (xx == 4 || iSample == filterSize - 1)  // max texture stages or last sample
        {
            ...
            pD3DDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, aQuad,
                                        sizeof(quad)); // one quad blends 4 textures
            pD3DDevice->SetRenderState(D3DRS_ALPHABLENDENABLE,
                                       TRUE); // on subsequent renderings, add to what's in the render target
            xx = 0;
        }
    }
...
void d3dExtFocusBlurEffect::DrawFocusRange() {
    ...
    XGSetTextureHeader(desc.Width, desc.Height, 1, 0, fmtOverride, 0, &ZBufferTexture,
                       pZBuffer->Data, desc.Width * 4);
    pZBuffer->Release();
    ...
    v[0].tu0 = 0.0f;
    v[0].tv0 = 0.0f;
    v[1].tu0 = (float) desc.Width;
    v[1].tv0 = 0.0f;
    v[2].tu0 = 0.0f;
    v[2].tv0 = (float) desc.Height;
    v[3].tu0 = (float) desc.Width;
    v[3].tv0 = (float) desc.Height;
    v[0].tu1 = fOffsetX;
    v[0].tv1 = fOffsetY;
    v[1].tu1 = fOffsetX + (float) descBlur.Width;
    v[1].tv1 = fOffsetY;
    v[2].tu1 = fOffsetX;
    v[2].tv1 = fOffsetY + (float) descBlur.Height;
    v[3].tu1 = fOffsetX + (float) descBlur.Width;
    v[3].tv1 = fOffsetY + (float) descBlur.Height;
    ...
    CalculateDepthMapping(m_fFocusNear, m_fFocusFar, &fAlphaOffset, &fAlphaSlope, &fBlueOffset,
                          &fBlueSlope);
    float rfConstants[] = {
...
void CalculateDepthMapping(float fDepth0,
    ...
    if (fScale > maximumSlope) {
        ...
        fDepth0 = 0.5f * (fDepth0 + fDepth1) -
                  0.5f / fScale; // move start so that peak is in middle of fDepth0 and fDepth1
