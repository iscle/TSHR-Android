...
#endif
    {
        ...
        for (unsigned i = start; i <= end; i += 16) {
            printf("%04x: %08x %08x %08x %08x -- %.2f %.2f %.2f %.2f -- %.4f %.4f\n",
                   ...
                   PS2ftod(legalizefloat(ptr[3])), PS2ftod(legalizefloat(ptr[2])),
                   PS2ftod(legalizefloat(ptr[1])), PS2ftod(legalizefloat(ptr[0])),
    ...
    void DumpVU1RegsF() {
        ...
        for (i = 0; i < 32; i++) {
            ...
            printf("vf%02d: %f %f %f %f -- %08x %08x %08x %08x\n", i,
                   vf[3], vf[2], vf[1], vf[0],
                   *(unsigned *) &vf[3], *(unsigned *) &vf[2], *(unsigned *) &vf[1],
                   *(unsigned *) &vf[0]);
        }
    }
