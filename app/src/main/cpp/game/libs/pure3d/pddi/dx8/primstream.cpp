...
d3dPrimStream::d3dPrimStream(d3dContext *c) :
    ...
    for (int i = 0; i < PRIM_STREAM_BUFFER_COUNT; i++) {
        HRESULT res = d3d->CreateVertexBuffer(VERTEX_BUFFER_SIZE, 0 /*D3DUSAGE_WRITEONLY*/, 0,
                                              D3DPOOL_MANAGED, &bufferRing[i]);
        DDVERIFY(res);
    }
...
void d3dPrimStream::Flush() {
    ...
    if ((nVertex <= 0) || ((primitiveType == D3DPT_TRIANGLESTRIP) && (nVertex <= 2)) ||
        ((primitiveType == D3DPT_LINESTRIP) && (nVertex <= 1))) {
        expected -= nVertex;
        nVertex = 0;
        return;
    }
