...
public:
    ...
    inline int EnableInterrupts(bool enable) {
        intr = enable;
        return 0;
    }
    ...
    inline int Nop() {
        *buf++ = VIF1_SET_NOP(intr);
        return 0;
    }

    inline int StCycl(int wl, int cl) {
        *buf++ = VIF1_SET_STCYCL(wl, cl, intr);
        return 0;
    }

    inline int Offset(int offset) {
        *buf++ = VIF1_SET_OFFSET(offset, intr);
        return 0;
    }

    inline int Base(int base) {
        *buf++ = VIF1_SET_BASE(base, intr);
        return 0;
    }

    inline int ITop(int addr) {
        *buf++ = VIF1_SET_ITOP(addr, intr);
        return 0;
    }

    inline int StMod(int m) {
        *buf++ = VIF1_SET_STMOD(m, intr);
        return 0;
    }

    inline int MskPath3(bool mask) {
        *buf++ = VIF1_SET_MSKPATH3(mask, intr);
        return 0;
    }

    inline int Mark(int val) {
        *buf++ = VIF1_SET_MARK(val, intr);
        return 0;
    }

    inline int FlushE() {
        *buf++ = VIF1_SET_FLUSHE(intr);
        return 0;
    }

    inline int Flush() {
        *buf++ = VIF1_SET_FLUSH(intr);
        return 0;
    }

    inline int FlushA() {
        *buf++ = VIF1_SET_FLUSHA(intr);
        return 0;
    }

    //inline int FlushA_Safe()           { *buf++ = VIF1_SET_FLUSHA(intr); return 0; }
    inline int MsCal(int addr) {
        *buf++ = VIF1_SET_MSCAL(addr, intr);
        return 0;
    }

    inline int MsCnt() {
        *buf++ = VIF1_SET_MSCNT(intr);
        return 0;
    }

    inline int MsCalF(int addr) {
        *buf++ = VIF1_SET_MSCALF(addr, intr);
        return 0;
    }

    inline int StMask(unsigned mask) {
        *buf++ = VIF1_SET_STMASK(intr);
        *buf++ = mask;
        return 0;
    }

    inline int Mpg(int addr, int size) {
        *buf++ = VIF1_SET_MPG(addr, size, intr);
        return 0;
    }
    ...
    void *Unpack(unsigned vuDest, unsigned qword, unsigned format, void *data = 0,
                 VIF::UnpackAddressMode addrMode = VIF::Absolute);
