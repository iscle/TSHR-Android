...
static gcAlphaBlendStruct SpecularAlphaBlendTable =
        {  // dest = CLAMP((((A * (1.0 - C) + B * C) * ColourTevOp) + D + bias) * ColourScale)
                //   A           B           C           D
                GX_CA_ZERO, GX_CA_ZERO, GX_CA_ZERO, GX_CA_APREV, GX_TEV_ADD, GX_CS_SCALE_1,
                GX_ENABLE
        };
...
//    GX_CC_TEXC, GX_CC_ZERO, GX_CC_ZERO, GX_CC_CPREV, GX_TEV_ADD, GX_CS_SCALE_1, GX_ENABLE
                GX_CC_ZERO, GX_CC_TEXC, GX_CC_KONST, GX_CC_CPREV, GX_TEV_ADD, GX_CS_SCALE_1,
                GX_ENABLE
        };
...
pddiBaseShader *
gcEnvironmentShader::Allocate(pddiRenderContext *c, const char *name, const char *aux) {
...
void gcEnvironmentShader::SetDevPass(unsigned pass) {
    ...
    if (mEnvironment) {
        ...
        GXSetTexCoordGen((GXTexCoordID) nTexCoord, GX_TG_MTX3x4, GX_TG_NRM,
                         GX_TEXMTX5/*GX_TEXMTX1*/);
