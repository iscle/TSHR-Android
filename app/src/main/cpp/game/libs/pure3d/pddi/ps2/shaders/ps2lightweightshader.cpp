...
ps2LightweightShaderAllocate(pddiRenderContext *c, const char *, const char *) {
...
void ps2LightweightShader::Validate(unsigned format) {
    PDDIASSERT(((format & 0xf) == 1) &&
               "PDDI ERROR: ps2LightweightShader: Invalid vertex format (texture co-ord count != 1)");
    PDDIASSERT((format & PDDI_V_POSITION) &&
               "PDDI ERROR: ps2LightweightShader:  Invalid vertex format (no positions)");
    PDDIASSERT((format & PDDI_V_COLOUR) &&
               "PDDI ERROR: ps2LightweightShader:  Invalid vertex format (no colours)");
    PDDIASSERT(!(format & PDDI_V_NORMAL) &&
               "PDDI ERROR: ps2LightweightShader:  Invalid vertex format (normals present)");
}
