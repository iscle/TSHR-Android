...
void ps2SimpleShader::Validate(unsigned format) {
    PDDIASSERT(((format & 0xf) == 1) &&
               "PDDI ERROR: ps2SimpleShader: Invalid vertex format (texture co-ord count != 1)");
    PDDIASSERT((format & PDDI_V_POSITION) &&
               "PDDI ERROR: ps2SimpleShader:  Invalid vertex format (no positions");
    PDDIASSERT(!((format & PDDI_V_NORMAL) && (format & PDDI_V_COLOUR)) &&
               "PDDI ERROR: ps2SimpleShader:  Invalid Vertex Format (normals & colour mutually exclusive)");

    if (isLit) {
        PDDIASSERT((format & PDDI_V_NORMAL) &&
                   "PDDI ERROR:  ps2SimpleShader: Invalid vertex format (isLit = 1, no normals)");
    } else {
        PDDIASSERT((format & PDDI_V_COLOUR) &&
                   "PDDI ERROR:  ps2SimpleShader: Invalid vertex format (isLit = 0, no colours)");
    }
}
