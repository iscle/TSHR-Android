...
void d3dUberShader::SetPass(int pass) {
    ...
    for (int i = 0; i < stageData.stageNum; ++i) {
        ...
        d3d->SetTextureColourCombine(i, stageData.colourOp[i], stageData.colourArg1[i],
                                     stageData.colourArg2[i]);
        d3d->SetTextureAlphaCombine(i, stageData.alphaOp[i], stageData.alphaArg1[i],
                                    stageData.alphaArg2[i]);
    }
