...
void ps2ToonShader::Bind(GSContext *gsContext, VifStream &vif) {
    ...
    if (isLit) {
        ...
        for (int i = 0; i < 4; i++) {
            vu->light.lightColour[i].a =
                    max(max(vu->light.lightColour[i].r, vu->light.lightColour[i].g),
                        vu->light.lightColour[i].b) * g_alphaScale;
        ...
        vu->light.additive.a =
                max(max(vu->light.additive.r, vu->light.additive.g), vu->light.additive.b) *
                g_ambientScale;
...
void ps2ToonShader::Validate(unsigned format) {
    PDDIASSERT(((format & 0xf) == 1) &&
               "PDDI ERROR: ps2ToonShader: Invalid vertex format (texture co-ord count != 1)");
    PDDIASSERT((format & PDDI_V_POSITION) &&
               "PDDI ERROR: ps2ToonShader:  Invalid vertex format (no positions");
    PDDIASSERT((format & PDDI_V_NORMAL) &&
               "PDDI ERROR: ps2ToonShader:  Model must be have normals (lit)");
}
