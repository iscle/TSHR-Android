...
void d3dCSRPShader::SetPass(int pass) {
    ...
    bool projection = allowProjection && ((pddiExtGlobalProjectedTexture *) (context->GetExtension(
            PDDI_EXT_GLOBAL_PROJECTED_TEXTURE)))->GetProjection(m_ProjectionId, &projectionMatrix,
                                                                (pddiTexture **) &projectionTexture);
