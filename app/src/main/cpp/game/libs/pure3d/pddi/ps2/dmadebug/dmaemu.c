...
static int AddSysError(int errnum, int severity, u_int *addr, char *error) {
    ...
    if (addr != NULL) {
        ...
        sprintf(err + strlen(err), ADDRESS_HEX
        ":["
        MEMCONTENTS_64
        "] ",
                (u_int) addr,
...
BOOL DMA_isValidAddress(void *addr) {
    ...
    return
            (a >= 0x00100000 && a <= gUpperMainMemLimit) ||
            // Main memory (not including kernal area)
...
void *DMA_convertToMainMemAddress(void *addr) {
    ...
    if ((a >> 28) == 0 ||
        (a >> 28) == 3) {        // If Cached or Uncached Accelerated, change to Uncached.
...
static int validateDmaAddress(void *address, char *description) {
    ...
    if (((u_int) address & 0xf0000000) == 0x70000000) {
        sprintf(gTempString, "%s at %p looks like a scratchpad address, but bit 31 is not set.",
                description, address);
    ...
    if (!DMA_isValidAddress(DMA_convertToMainMemAddress(address))) {
        sprintf(gTempString, "%s (0x%08x) is not in a valid address range", description,
                (u_int) address);
...
static void endDmaTransfer(void) {
    ...
    if (gDumpDmaStats &&
        d->chcr.MOD == 1) {        // Only worth outputting stats if we're in Source Chain mode
        outputDmaStatistics();
    }
}
...
static int interpretSourceChainTag(void) {
    ...
    DEBUG_addField(ENTRY_TITLE, s, DMATITLE
    "DMA %s%s %-4s"
    NORM
    " "
    DATASOURCEADDRESS,
    ...
    MEMCONTENTS_64
    "]", hi, lo);            // Tag contents (Hi/Lo)
    DEBUG_addField(ENTRY_FIELD, s, "CHAN="
    DATA_STR, gDmaChanNames[gChan]);        // Channel name
    DEBUG_addField(ENTRY_FIELD, s, "TTE="
    DATA_DEC, tte);                                    // TTE status
    DEBUG_addField(ENTRY_FIELD, s, "IRQ="
    DATA_DEC, irq);                                    // 'interrupt'
    DEBUG_addField(ENTRY_FIELD, s, "PCE="
    DATA
    "%11s"
    NORM, gPceDescription[pce]);        // PCE status
    DEBUG_addField(ENTRY_FIELD, s, "QWC:"
    DATA_4HEX
    NORM, qwc);                        // QuadWordCount

    // ---------------- Tag field boundary checks

    if (pce == 1) {
        err |= AddSysError(ERR_RESERVED_PCE, ERROR_FATAL_CONT, tagAddress,
                           "PCE field is 1, which is reserved.");
    }
    ...
    if (id == DMA_REFS &&
        !(gChan == SCE_DMA_VIF1 || gChan == SCE_DMA_SIF1 || gChan == SCE_DMA_GIF)) {
    ...
    if ((id == DMA_CALL || id == DMA_RET) &&
        !(gChan == SCE_DMA_VIF0 || gChan == SCE_DMA_VIF1 || gChan == SCE_DMA_GIF)) {
        err |= AddSysError(ERR_INVALID_TAG_FOR_CHANNEL, ERROR_FATAL_CONT, tagAddress,
    ...
    if (!(id == DMA_CNT || id == DMA_RET || id == DMA_END)) {
        ...
        addr = DMA_convertToMainMemAddress(addr);
        if (FATAL(err)) {
            err |= AddSysError(ERR_DMA_INVALID_ADDRESS, ERROR_FATAL_CONT, tagAddress,
                               "DMA tag ADDR field is invalid");
        }
    }
    ...
    for (i = 0; i < numBlocks; i++) {
        ...
        error = NO_ERROR;
        error |= validateDmaAddress(convertToDmaAddress(startTransAddr[i]),
                                    "Transfer start address");
        error |= validateDmaAddress(convertToDmaAddress(endTransAddr[i] - 16),
                                    "Transfer end address");
        if (FATAL(error)) {
            sprintf(gTempString, "Data to be transferred falls outside main mem/scratch pad RAM. "
                                 "[%08x -> %08x]", (u_int) startTransAddr[i],
                    (u_int) endTransAddr[i]);
    ...
    if (numBlocks == 0) {
        DEBUG_addField(ENTRY_FIELD_NL, s, "Transfers from: ["
        ADDRESS
        ...
        dputs("++++blocks");
        for (i = 0; i < numBlocks; i++) {
            DEBUG_addField(ENTRY_FIELD_NL, s, "Transfers: ["
            ADDRESS_HEX
            "->"
            ADDRESS_HEX
            ") [len(qwords):"
            DATA_HEX
            "]%s",
                    (u_int) startTransAddr[i], (u_int) endTransAddr[i],
                    ((u_int) endTransAddr[i] - (u_int) startTransAddr[i]) >> 4,
...
typedef enum {
    INTERPRET_TAG, CHECK_DMA_END_TRANSFER
} PROCESSING_STAGE;
