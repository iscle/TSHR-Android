...
void d3dToon2VertexShader::LoadVSConstants(void) {
    ...
    float vColor[4] = {shaderInfo.diffuse.Red() / 255.f, shaderInfo.diffuse.Green() / 255.f,
                       shaderInfo.diffuse.Blue() / 255.f, 1.f};
    ...
    m_pd3dDevice->SetVertexShaderConstant(17, (pick[0] == -1) ? (D3DVECTOR * ) & vDarkLight
                                                              : &light0.Direction, 1);
    m_pd3dDevice->SetVertexShaderConstant(18, (pick[1] == -1) ? (D3DVECTOR * ) & vDarkLight
                                                              : &light1.Direction, 1);
    m_pd3dDevice->SetVertexShaderConstant(19, (pick[1] == -1) ? (D3DVECTOR * ) & vDarkLight
                                                              : &light2.Direction, 1);
...
void d3dToon2VertexShader::CreateSilhoutteMap() {
    ...
    d3dok = d3d->CreateTexture((unsigned int) TOON_EDGE_MAP_DIM, 1, 1, NULL, descBack.Format,
                               D3DPOOL_MANAGED, &d3dsilhoutteMap);
