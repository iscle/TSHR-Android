...
int PageConv4to32(int width, int height, u_char *p_input, u_char *p_output) {
    ...
    for (i = 0; i < n_height; i++) {
        ...
        for (j = 0; j < n_width; j++) {
            ...
            po0 = output_block;
            po1 = p_output + 8 * index32_v[in_block_nb] * output_page_line_size +
                  index32_h[in_block_nb] * 32;
...
int PageConv8to32(int width, int height, u_char *p_input, u_char *p_output) {
    ...
    for (i = 0; i < n_height; i++) {
        ...
        for (j = 0; j < n_width; j++) {
            ...
            pi0 = input_block;
            pi1 = p_input + PSMT8_BLOCK_HEIGHT * i * input_page_line_size +
                  j * PSMT8_BLOCK_WIDTH; // byte
...
int PageConv16to32(int width, int height, u_char *p_input, u_char *p_output) {
    ...
    for (i = 0; i < n_height; i++) {
        ...
        for (j = 0; j < n_width; j++) {
            ...
            pi0 = input_block;
            pi1 = p_input + BLOCK_HEIGHT_16 * i * input_page_line_size +
                  j * BLOCK_WIDTH_16 * 2; // byte
