...
void ps2Vram::BlitBackToFront() {
    ...
    gifPkt.A_D(SCE_GS_DIMX,
               SCE_GS_SET_DIMX(0x4, 0x2, 0x5, 0x3, 0x0, 0x6, 0x1, 0x7, 0x5, 0x3, 0x4, 0x2, 0x1, 0x7,
                               0x0, 0x6));
    gifPkt.A_D(SCE_GS_PRMODE,
               SCE_GS_SET_PRMODE(0, 1, 0, 0, 0, 1, 1, 0));  // TME=1 , UV = 1, context1
    gifPkt.A_D(SCE_GS_TEST_2, SCE_GS_SET_TEST_1(0, 0, 0, 0, 0, 0, 1,
                                                SCE_GS_ZALWAYS));  // ZTE is on because of TRC (TRC V2.1e, 16.2.1)
...
void ps2Vram::BlitBackToTexture(unsigned fbp, unsigned psm, pddiColour tint) {
    ...
    gifPkt.A_D(SCE_GS_TEX1_2, SCE_GS_SET_TEX1_1(0, 0, filterLinear, filterLinear, 0, 0, 0));
    gifPkt.A_D(SCE_GS_TEXA, SCE_GS_SET_TEXA(128, 0, 128));
    gifPkt.A_D(SCE_GS_RGBAQ, SCE_GS_SET_RGBAQ((tint.Red() + 1) >> 1, (tint.Green() + 1) >> 1,
                                              (tint.Blue() + 1) >> 1, 0x80,
                                              0x3f800000)); // tint it a bit
    BuildGifStrips(gifPkt, true);
...
void ps2Vram::DownsampleBackbuffer() {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (i == 0)
            gifPkt.A_D(SCE_GS_TEX0_2,
                       SCE_GS_SET_TEX0_1(srcFbp, fbw, srcPsm, 10, 10, 1, 0, 0, 0, 0, 0, 0));
        else
            gifPkt.A_D(SCE_GS_TEX0_2,
                       SCE_GS_SET_TEX0_1(dstFbp, fbw, dstPsm, 10, 10, 1, 0, 0, 0, 0, 0, 0));
    ...
    gifPkt.A_D(SCE_GS_PRMODE,
               SCE_GS_SET_PRMODE(0, 0, 0, 1, 0, 1, 1, 0));  // ABE=1 , UV = 1, context1
    ...
    gifPkt.A_D(SCE_GS_PRMODE,
               SCE_GS_SET_PRMODE(0, 1, 0, 1, 0, 1, 1, 0));  // ABE=1 , UV = 1, context1
