...
void ps2Display::SetupFS_AA(short dispW, short dispH, short dispPSM, short ztest, short zPSM) {
    ...
    if (dtv480) {
        ...
        gsPrivReg[0].DISPLAY1.MAGH = (1550 / dispW -
                                      1);//magh;     // pixel magnification value (specifies horizontal resolution)
        gsPrivReg[0].DISPLAY1.MAGV = 0;
        gsPrivReg[0].DISPLAY1.DW = (dispW * (1550 / dispW) -
                                    1);       // magic value from libgraph docs (639.75 * 4)
        gsPrivReg[0].DISPLAY1.DH = (dispH - s);  // display height          
    } else {
        gsPrivReg[0].DISPLAY1.DX = (dispMode == SCE_GS_NTSC) ? 636
                                                             : 656; // magic values taken from libgraph docs
    ...
    gsPrivReg[0].DISPLAY2.DX = gsPrivReg[0].DISPLAY1.DX +
                               2; // offset second read circuit for slight blurring effect (units=VCK=4/pixel)
