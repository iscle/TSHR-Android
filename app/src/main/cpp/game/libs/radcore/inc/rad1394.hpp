...
struct IRad1394Slave : public IRefCount {
    ...
    //
    virtual void
    ReadMemoryAsync(unsigned int address, unsigned int size, void *dest, bool atomic = false) = 0;

    virtual void
    WriteMemoryAsync(unsigned int address, unsigned int size, void *source, bool atomic = false,
                     bool localWrite = false) = 0;
