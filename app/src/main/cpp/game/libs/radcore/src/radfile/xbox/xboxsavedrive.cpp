...
union DeviceState {
    ...
    void update(void) {
        ...
        //
        unsigned char insertions = (unsigned char) ((insertMask & 0xF) |
                                                    ((insertMask >> 12) & 0xF0) & 0xFF);
        unsigned char removals = (unsigned char) ((removeMask & 0xF) |
                                                  ((removeMask >> 12) & 0xF0) & 0xFF);
...
radXboxSaveDrive::radXboxSaveDrive(const char *pdrivespec, radMemoryAllocator alloc)
    ...
    if (!g_IsInitialized) {
        ...
        s_TransferBuffer = (unsigned char *) ::radMemoryRoundUp(
                (unsigned int) s_TransferBufferSpace, XBOXSAVEDRIVE_ALIGNMENT);
    }
    ...
    if (strcmp(pdrivespec, "U:") == 0) {
        ...
        // It's a memory unit.
        //
        m_Capabilities = (radDriveRemovable | radDriveWriteable | radDriveSaveGame |
                          radDriveEnumerable);
    }
...
radDrive::CompletionStatus radXboxSaveDrive::OpenSaveGame
    ...
    bool create = !(error != ERROR_PATH_NOT_FOUND &&
                    (flags == OpenExisting || flags == OpenAlways));
    ...
    if (create) {
        ...
        if (memcardInfo->m_Icon != NULL) {
            ...
            HANDLE handle = ::CreateFile(savePath, GENERIC_READ | GENERIC_WRITE,
            ...
            if (
                    ::SetFilePointer(handle, (LONG) memcardInfo->m_IconSize, NULL, FILE_BEGIN) !=
                    memcardInfo->m_IconSize ||
                    !::SetEndOfFile(handle) ||
                    ::SetFilePointer(handle, (LONG) 0, NULL, FILE_BEGIN) != 0 ||
                    !::WriteFile(handle, memcardInfo->m_Icon, memcardInfo->m_IconSize,
                                 &bytesWritten, NULL)
                    ) {
                ::CloseHandle(handle);
                HandleError(::GetLastError());
    ...
    //
    strcpy(&savePath[saveLen],
           "05"); // hardcode the 'real' filename, since simpson2 descriptive name contains ':' and '/'

    //
    // Open the file for resizing and initializing.
    //
    *pHandle = ::CreateFile(savePath, GENERIC_READ | GENERIC_WRITE,
    ...
    if (*pHandle != INVALID_HANDLE_VALUE) {
        if (create) {
            ...
            unsigned int numBlocks =
                    (maxSize + XBOXSAVEDRIVE_TRANSFER_BUFFER_SIZE - 1) /
                    XBOXSAVEDRIVE_TRANSFER_BUFFER_SIZE;
            unsigned int fsize =
                    maxSize + numBlocks * sizeof(XCALCSIG_SIGNATURE) + sizeof(unsigned int);
...
radXboxSaveDrive::CommitFile(radFileHandle handle, const char *fileName) {
    // Mount the drive.
    //
    if (!Mount()) {
        return Error;
...
radDrive::CompletionStatus radXboxSaveDrive::WriteSignedBlock
    ...
    rAssertMsg(
            block * (XBOXSAVEDRIVE_TRANSFER_BUFFER_SIZE - sizeof(XCALCSIG_SIGNATURE)) + position +
            numBytes <= size,
...
radDrive::CompletionStatus radXboxSaveDrive::FindFirst
    ...
    *pHandle = (radFileDirHandle) handle;
    DirResult dirResult = TranslateDirInfo(pDirectoryInfo, &FindData, m_HandlePool[handle].m_Handle,
                                           m_HandlePool[handle].m_pSpec);
    if (dirResult == Done) {
        return Complete;
...
radDrive::CompletionStatus
radXboxSaveDrive::FindNext(radFileDirHandle *pHandle, IRadDrive::DirectoryInfo *pDirectoryInfo) {
    //
    ...
    while (dirResult == NoMatch) {
        ...
        if (!::XFindNextSaveGame(m_HandlePool[handle].m_Handle, &FindData)) {
            dirResult = TranslateDirInfo(pDirectoryInfo, &FindData, INVALID_HANDLE_VALUE,
                                         m_HandlePool[handle].m_pSpec);
        } else {
            dirResult = TranslateDirInfo(pDirectoryInfo, &FindData, m_HandlePool[handle].m_Handle,
                                         m_HandlePool[handle].m_pSpec);
        }
...
unsigned int radXboxSaveDrive::GetCreationSize(radMemcardInfo *memcardInfo, unsigned int size) {
    ...
    csize += ::radMemoryRoundUp(
            size + numBlocks * sizeof(XCALCSIG_SIGNATURE) + sizeof(unsigned int), 16 * 1024);
...
void radXboxSaveDrive::SetMediaInfo(void) {
    ...
    if (m_DriveLetter != 'U') {
        ...
        error = TranslateError(
                ::XMUNameFromDriveLetter(m_DriveLetter, (WCHAR *) m_MediaInfo.m_VolumeName,
                                         sizeof(m_MediaInfo.m_VolumeName) / sizeof(WCHAR))
        );
...
void radXboxSaveDrive::BuildFileSpec(const char *fileName, char *fullName, unsigned int size,
                                     char **pName) {
    //
...
bool radXboxSaveDrive::InitializeFile(radFileHandle handle, unsigned int size, bool validHeader) {
    ...
    while (fullSize > XBOXSAVEDRIVE_TRANSFER_BUFFER_SIZE) {
        if (!::WriteFile(handle, s_TransferBuffer, XBOXSAVEDRIVE_TRANSFER_BUFFER_SIZE,
                         &bytesWritten, NULL)) {
            HandleError(::GetLastError());
            return false;
        }
