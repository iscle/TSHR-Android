...
//
extern "C" void
radStackTracePs2Get(unsigned int *results, int max, void *stackPointer, void *returnAddress) {
    extern int main();
    ...
    while (ra && max) {
        ...
        if ((unsigned) ra < (unsigned int) &_codestart ||
            (unsigned) ra > (unsigned int) &_codeend) {
            *results++ = 0;
            break;
        }
