...
radCrashHandler::radCrashHandler(void)
    ...
    m_pRadCrashRecordXBox = static_cast<radCrashRecordXBox *>(::radMemoryAllocAligned(
            GetThisAllocator(),
    ...
    for (unsigned int i = 0;
         i < 100000000; i++) { ;;  // This seems to be the only way to find the memory card always
    }
    ...
    if (m_MemCardLetter != 0x00) {
        ...
        switch (m_State) {
            case NO_CRASH: {
                ...
                sprintf(m_pRadCrashRecordXBox->m_RadCrashRecordInfo.m_Title,
                        RAD_CRASH_RECORD_TITLE);
                sprintf(m_pRadCrashRecordXBox->m_RadCrashRecordInfo.m_Platform,
                        RAD_CRASH_RECORD_XBOX_PLATFORM);
                sprintf(m_pRadCrashRecordXBox->m_RadCrashRecordInfo.m_Version,
                        RAD_CRASH_RECORD_XBOX_VERSION);
                ...
                unsigned int stackDumpSize =
                        RAD_CRASH_RECORD_XBOX_STACK_DUMP_SIZE * sizeof(unsigned int);
                ...
                while (IsBadReadPtr((void *) m_pRadCrashRecordXBox->m_Context.Esp, stackDumpSize)) {
                    ...
                    if (((stackDumpSize + m_pRadCrashRecordXBox->m_Context.Esp) & 0x0fff) != 0) {
                        stackDumpSize -=
                                (stackDumpSize + m_pRadCrashRecordXBox->m_Context.Esp) & 0x0fff;
                    } else {
                ...
                int thisPointerData = (m_pRadCrashRecordXBox->m_Context.Ecx) -
                                      (RAD_CRASH_RECORD_XBOX_THIS_POINTER_DUMP_POINTER_OFFSET * 4);
                ...
                unsigned int stackTraceSize =
                        RAD_CRASH_RECORD_XBOX_THIS_POINTER_DUMP_SIZE * sizeof(unsigned int);
                ...
                ret = WriteFile(handle, m_pRadCrashRecordXBox, sizeof(radCrashRecordXBox), &written,
                                NULL);
            ...
            default: {
                rDebugPrintf(
                        "ERROR: radCrashHandler FAILED. radCrashHandler generated an exception\n");
                break;
            }
        }
    }
