...
CHostHioSocket::HIOSocketThread
        (
...
bool CHostHioSocket::DoTargetCommunication(bool doingTargetCommunication) {
    ...
    for (i = 0; i < HIO_BUFFER_COUNT; i++) {
        if (controlBlock.m_TargetTransitBufferOwner[i] != 0xffffffff) {
            ...
            if ((socketIndex != HIO_CHANNEL_COUNT) &&
                m_pSharedData->m_SocketArray[socketIndex].m_State == Connected) {
                unsigned int bytesInReceiver = controlBlock.m_TargetTransmitCount[channelIndex];

                if ((bytesInReceiver != 0) &&
                    (m_pSharedData->m_SocketArray[socketIndex].m_BytesInRxBuffer == 0)) {
                    //
                    ...
                    m_HioRead(&m_pHioMemory->m_TargetTransmitBuffer[i][0],
                              m_pSharedData->m_SocketArray[socketIndex].m_RxBuffer,
                    ...
                    //
                    PostMessage(m_pSharedData->m_SocketArray[socketIndex].m_hWnd,
                                m_pSharedData->m_SocketArray[socketIndex].m_WndMsg,
                                (unsigned int) m_pSharedData->m_SocketArray[socketIndex].m_Socket,
                                FD_READ);
    ...
    for (i = 0; i < HIO_CHANNEL_COUNT; i++) {
        if ((m_pSharedData->m_SocketArray[socketIndex].m_State == Connected) ||
            (m_pSharedData->m_SocketArray[socketIndex].m_State == PendingDisconnect)) {
            ...
            if ((bytesInTransmitter == 0) &&
                ...
                if (bufferIndex != 0xffffffff) {
                    ...
                    m_HioWrite(&m_pHioMemory->m_HostTransmitBuffer[bufferIndex][0],
                               m_pSharedData->m_SocketArray[socketIndex].m_TxBuffer, RoundUpTo32(
