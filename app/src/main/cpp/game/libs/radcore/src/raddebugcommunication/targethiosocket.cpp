...
//    }
    ...
    if (s_Channel == -1) {
        rDebugPrintf(
                "Warning: Failed to initialize Host I/O commumication with channel %d. Check connection.\n",
                channel);
    }
...
void CTargetHIOSocket::DoHIOCommunication(void) {
    ...
    switch (m_ComState) {
        ...
        case ReadingData : {
            ...
            //
            unsigned int channelIndex = radPlatformEndian32(
                    m_ChannelControlBlock.m_HostTransitBufferOwner[m_ReadWriteIndex]);
...
bool CTargetHIOSocket::ReadData(void) {
    ...
    for (unsigned int i = m_ReadWriteIndex; i < HIO_BUFFER_COUNT; i++) {
        if (m_ChannelControlBlock.m_HostTransitBufferOwner[i] != 0xffffffff) {
            ...
            //
            unsigned int channelIndex = radPlatformEndian32(
                    m_ChannelControlBlock.m_HostTransitBufferOwner[i]);
            ...
            if ((socketIndex != HIO_CHANNEL_COUNT) &&
                m_SocketArray[socketIndex].m_State == Connected) {
                unsigned int bytesInReceiver = radPlatformEndian32(
                        m_ChannelControlBlock.m_HostTransmitCount[channelIndex]);
                ...
                if ((bytesInReceiver != 0) && (m_SocketArray[socketIndex].m_BytesInRxBuffer == 0)) {
                    ...
                    //
                    DCInvalidateRange(m_SocketArray[socketIndex].m_RxBuffer,
                                      RoundUpTo32(bytesInReceiver));

                    BOOL result = HIOReadAsync((u32) & m_pHioMemory->m_HostTransmitBuffer[i][0],
                                               m_SocketArray[socketIndex].m_RxBuffer,
                                               RoundUpTo32(bytesInReceiver),
                                               ReadWriteCallback);
...
bool CTargetHIOSocket::WriteData(void) {
    for (unsigned int i = m_ReadWriteIndex; i < HIO_CHANNEL_COUNT; i++) {
        if ((m_SocketArray[m_CurrentSendIndex].m_State == Connected) ||
            (m_SocketArray[m_CurrentSendIndex].m_State == PendingDisconnect)) {
            unsigned int bytesInTransmitter = radPlatformEndian32(
                    m_ChannelControlBlock.m_TargetTransmitCount[m_SocketArray[m_CurrentSendIndex].m_HioChannel]);

            if ((bytesInTransmitter == 0) &&
                (m_SocketArray[m_CurrentSendIndex].m_BytesInTxBuffer != 0)) {
                //
                ...
                if (bufferIndex != 0xffffffff) {
                    ...
                    m_ChannelControlBlock.m_TargetTransitBufferOwner[bufferIndex] = radPlatformEndian32(
                            m_SocketArray[m_CurrentSendIndex].m_HioChannel);
                    m_LastSendIndex = m_CurrentSendIndex;

                    //
                    // Flush cache before writing the data.
                    //
                    DCFlushRange(m_SocketArray[m_CurrentSendIndex].m_TxBuffer,
                                 RoundUpTo32(m_SocketArray[m_CurrentSendIndex].m_BytesInTxBuffer));

                    BOOL result = HIOWriteAsync(
                            (u32) & m_pHioMemory->m_TargetTransmitBuffer[bufferIndex][0],
                            m_SocketArray[m_CurrentSendIndex].m_TxBuffer,
                            RoundUpTo32(m_SocketArray[m_CurrentSendIndex].m_BytesInTxBuffer),
                            ReadWriteCallback);
...
void CTargetHIOSocket::ResolveState(void) {
    ...
    for (unsigned int i = 0; i < HIO_CHANNEL_COUNT; i++) {
        if ((m_SocketArray[i].m_State == Connected) ||
            ...
            HioChannelControlBlock::ControlMessage message =
                    (HioChannelControlBlock::ControlMessage) radPlatformEndian32(
                            m_ChannelControlBlock.m_MessageMap[channel]);
            rAssert((message == HioChannelControlBlock::NullMessage) ||
                    (message == HioChannelControlBlock::DisconnectMessage));
            ...
            if (message == HioChannelControlBlock::DisconnectMessage) {
                ...
                // 
                m_ChannelControlBlock.m_MessageMap[channel] = (HioChannelControlBlock::ControlMessage) radPlatformEndian32(
                        HioChannelControlBlock::NullMessage);
                ...
                if ((m_SocketArray[i].m_State == Disconnected) ||
                    ((m_SocketArray[i].m_State == PendingDisconnect) &&
                     (m_SocketArray[i].m_BytesInTxBuffer == 0))) {
                    m_ChannelControlBlock.m_MessageMap[channel] = (HioChannelControlBlock::ControlMessage) radPlatformEndian32(
                            HioChannelControlBlock::DisconnectMessage);
                    m_SocketArray[i].m_State = Free;
                }
            }
        }
    ...
    for (unsigned int channel = 0; channel < HIO_CHANNEL_COUNT; channel++) {
        if (m_ChannelControlBlock.m_ChannelAllocationMap[channel] != 0) {
            if (HioChannelControlBlock::ConnectRequestMessage ==
                (HioChannelControlBlock::ControlMessage) radPlatformEndian32(
                        m_ChannelControlBlock.m_MessageMap[channel])) {
                //
                ...
                for (unsigned int i = 0; i < HIO_CHANNEL_COUNT; i++) {
                    if (m_SocketArray[i].m_State == Listen) {
                        ...
                        m_ChannelControlBlock.m_MessageMap[channel] = (HioChannelControlBlock::ControlMessage) radPlatformEndian32(
                                HioChannelControlBlock::ConnectAcceptMessage);
                        break;
                    }
                }
                ...
                if (HioChannelControlBlock::ConnectRequestMessage ==
                    (HioChannelControlBlock::ControlMessage) radPlatformEndian32(
                            m_ChannelControlBlock.m_MessageMap[channel])) {
                    m_ChannelControlBlock.m_MessageMap[channel] = (HioChannelControlBlock::ControlMessage) radPlatformEndian32(
                            HioChannelControlBlock::ConnectRejectMessage);
                }
            }
        }
    }
