...
radCementLibrary::radCementLibrary(radDrive *pDrive, radFile *pDataFile,
                                   radCementLibraryPriority priority)
        :
        radSafeRefCount(1),
        m_pDrive(pDrive),
...
void radCementLibrary::OnFileOperationsComplete(void *pUserData) {
    ...
    if (whatToDo == ReadFileInfo) {
        ...
        if (!m_pDataFile->IsOpen()) {
            ...
            //
            unsigned int readSize = ::radMemoryRoundUp(sizeof(radCFFileInfo),
                                                       m_pDataFile->GetOptimalSize());
            unsigned int size = ::radMemoryRoundUp(sizeof(radCFHeader),
                                                   m_pDataFile->GetOptimalSize());
            size = size > readSize ? size : readSize;
            m_TmpBuffer = (char *) ::radMemoryAllocAligned(RADMEMORY_ALLOC_TEMP, size,
        ...
        if (
                strcmp(pFileInfo->m_CementLibraryIdentification,
                       RAD_CEMENT_LIBRARY_IDENTIFICATION) != 0
                ) {
...
#       endif
        ...
        unsigned int totalSize =
                ::radMemoryRoundUp(sizeof(radCFHeader) + sizeof(radCFHeader::HFE) * numFiles,
                                   m_pDataFile->GetOptimalSize());

        m_Header = (radCFHeader *)
                ::radMemoryAllocAligned(GetThisAllocator(), totalSize,
                                        radFileOptimalMemoryAlignment);
...
radCementLibraryList::~radCementLibraryList(void) {
    ...
    for (unsigned int i = 0; i < radCementLibraryNumPriorities; i++) {
        if (m_pHead[i] != NULL) {
            ...
            rDebugPrintf(
                    "The following cement libraries have not been unregistered at priority [%u]: \n",
                    i);
            while (ptr != NULL) {
