...
unsigned char s_TransferBufferSpace[
        GCNMEMCARDDRIVE_TRANSFER_BUFFER_SIZE + GCNMEMCARDDRIVE_ALIGNMENT];
...
radGcnMemcardDrive::radGcnMemcardDrive(const char *pdrivespec, radMemoryAllocator alloc)
    ...
    if (!g_IsInitialized) {
        ...
        s_TransferBuffer = (unsigned char *) ::radMemoryRoundUp(
                (unsigned int) s_TransferBufferSpace, GCNMEMCARDDRIVE_ALIGNMENT);
...
radDrive::CompletionStatus radGcnMemcardDrive::OpenSaveGame
    ...
    unsigned int numBlocks =
            (maxSize + headerSize + GCNMEMCARDDRIVE_TRANSFER_BUFFER_SIZE - 1) /
            GCNMEMCARDDRIVE_TRANSFER_BUFFER_SIZE;
    ...
    if (error == FileNotFound) {
        if (flags == OpenExisting) {
            ...
            rAssertMsg(flags == CreateAlways || flags == OpenAlways,
                       "PS2MemcardDrive: invalid open flags.");
...
radGcnMemcardDrive::CloseFile(radFileHandle handle, const char *fileName) {
    //
...
radDrive::CompletionStatus
radGcnMemcardDrive::CommitFile(radFileHandle handle, const char *fileName) {
    //
...
radDrive::CompletionStatus radGcnMemcardDrive::WriteSignedBlock
    ...
    rAssertMsg(
            block * (GCNMEMCARDDRIVE_TRANSFER_BUFFER_SIZE - sizeof(radCrc)) + position + numBytes <=
            size,
...
unsigned int radGcnMemcardDrive::GetCreationSize(radMemcardInfo *memcardInfo, unsigned int size) {
    ...
    unsigned int headerSize = HeaderSize(memcardInfo);
    unsigned int numBlocks =
            (size + headerSize + GCNMEMCARDDRIVE_TRANSFER_BUFFER_SIZE - 1) /
            GCNMEMCARDDRIVE_TRANSFER_BUFFER_SIZE;
...
IRadDrive::MediaInfo::MediaState radGcnMemcardDrive::ErrorToMediaState(int result) {
    switch (result) {
        case CARD_RESULT_FATAL_ERROR:
            rDebugString(
                    "GCNMemcardDrive: getting media info failed. This shouldn't have happened.\n");
...
void radGcnMemcardDrive::BuildFileSpec(const char *fileName, char *fullName, unsigned int size,
                                       char **pName) {
    ...
    *pName = &fullName[len];
    ...
    rAssertMsg(strchr(fullName, '\\') == NULL,
               "GCNMemcardDrive: subdirectories are not supported.");
}
...
unsigned int radGcnMemcardDrive::HeaderSize(const radMemcardInfo *memcardInfo) {
    ...
    if (memcardInfo != NULL) {
        ...
        //
        bool isC8 = false;
        rAssertMsg(memcardInfo->m_NumFrames <= CARD_ICON_MAX,
                   "GcnMemcardDrive: invalid memcard info.");
        for (unsigned int i = 0; i < memcardInfo->m_NumFrames; i++) {
            rAssertMsg(memcardInfo->m_pIcon[i].m_Data != NULL,
                       "GcnMemcardDrive: invalid memcard info.");
            ...
            switch (memcardInfo->m_pIcon[i].m_Format) {
                ...
                case radMemcardIconData::C8:
                    ...
                    isC8 = true;
                    rAssertMsg(memcardInfo->m_pIcon[i].m_CLUT != NULL,
                               "GcnMemcardDrive: invalid memcard info.");
                    break;
...
void radGcnMemcardDrive::PackHeader(const radMemcardInfo *memcardInfo, CARDStat *cardStat,
                                    unsigned int fileSize) {
    rAssertMsg(memcardInfo != NULL,
               "radFile: GcnMemcardDrive: radMemcardInfo required for creation.");
...
bool radGcnMemcardDrive::InitializeFile
    ...
    //
    unsigned int firstSize = GCNMEMCARDDRIVE_TRANSFER_BUFFER_SIZE > fileSize ? fileSize
                                                                             : GCNMEMCARDDRIVE_TRANSFER_BUFFER_SIZE;
    ...
    while (fullSize > GCNMEMCARDDRIVE_TRANSFER_BUFFER_SIZE) {
        if (!DoFun(::CARDWrite(pCardInfo, s_TransferBuffer, GCNMEMCARDDRIVE_TRANSFER_BUFFER_SIZE,
                               offset))) {
            return false;
        }
...
bool radGcnMemcardDrive::Copy(CARDFileInfo *pDestInfo, CARDFileInfo *pSrcInfo) {
    ...
    while (length >= GCNMEMCARDDRIVE_TRANSFER_BUFFER_SIZE) {
        if (
                !DoFun(::CARDRead(pSrcInfo, s_TransferBuffer, GCNMEMCARDDRIVE_TRANSFER_BUFFER_SIZE,
                                  offset)) ||
                !DoFun(::CARDWrite(pDestInfo, s_TransferBuffer,
                                   GCNMEMCARDDRIVE_TRANSFER_BUFFER_SIZE, offset))
                ) {
            return false;
        }
