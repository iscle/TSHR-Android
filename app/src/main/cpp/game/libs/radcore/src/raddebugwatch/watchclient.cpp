...
DebugWatchClient::DebugWatchClient(radMemoryAllocator alloc)
    ...
    rAssertMsg(m_Channel != NULL,
               "Debug Watch Failed: Make sure debug communication system initialized\n");
...
void DebugWatchClient::AddFunction
    ...
    //
    if (NULL != (pPacket = DoCreatePacketPreamble((void *) callback, pName, pNameSpace, callback,
                                                  userData))) {
        //
        // Add stuff specific to the type.
        //
...
void DebugWatchClient::OnReceiveComplete
    ...
    if (successful) {
            ...
            switch (radPlatformEndian32(*((WPCommand *) pRxAddress))) {
                ...
                case WPCRequestValue : {
                    ...
                    switch (radPlatformEndian32(pCommand->m_DataType)) {
                        case WPDUnsignedInt : {
                            pReply->m_Attrib.m_ui.m_Value = radPlatformEndian32(
                                    *((unsigned int *) (pCommand->m_Address)));
                            break;
                        }

                        case WPDInt : {
                            pReply->m_Attrib.m_i.m_Value = radPlatformEndian32(
                                    *((int *) (pCommand->m_Address)));
                            break;
                        }

                        case WPDUnsignedShort : {
                            pReply->m_Attrib.m_us.m_Value = radPlatformEndian16(
                                    *((unsigned short *) (pCommand->m_Address)));
                            break;
                        }

                        case WPDShort : {
                            pReply->m_Attrib.m_s.m_Value = radPlatformEndian16(
                                    *((short *) (pCommand->m_Address)));
                            break;
                        }
                        ...
                        case WPDBoolean : {
                            pReply->m_Attrib.m_bool.m_Value = radPlatformEndian32(
                                    *((bool *) (pCommand->m_Address)));
                            break;
                        }

                        case WPDFloat : {
                            pReply->m_Attrib.m_f.m_Value = radPlatformEndianFloat(
                                    *((float *) (pCommand->m_Address)));
                            break;
                        }
                        ...
                        case WPDVector : {
                            pReply->m_Attrib.m_vector.m_x = radPlatformEndianFloat(
                                    *((float *) (pCommand->m_Address) + 0));
                            pReply->m_Attrib.m_vector.m_y = radPlatformEndianFloat(
                                    *((float *) (pCommand->m_Address) + 1));
                            pReply->m_Attrib.m_vector.m_z = radPlatformEndianFloat(
                                    *((float *) (pCommand->m_Address) + 2));
                            break;
                        }
                        ...
                        case WPDString : {
                            ...
                            strcpy((char *) m_CurrentTxAddress + sizeof(WCValue),
                                   (const char *) pCommand->m_Address);
                            m_CurrentTxAddress += length;
                            m_TxBytesQueued += length;
                ...
                case WPCWriteValue: {
                    ...
                    switch (radPlatformEndian32(pCommand->m_DataType)) {
                        case WPDUnsignedInt : {
                            *((unsigned int *) (pCommand->m_Address)) = radPlatformEndian32(
                                    pCommand->m_Attrib.m_ui.m_Value);
                            break;
                        }

                        case WPDInt : {
                            *((int *) (pCommand->m_Address)) = radPlatformEndian32(
                                    pCommand->m_Attrib.m_i.m_Value);
                            break;
                        }

                        case WPDUnsignedShort : {
                            *((unsigned short *) (pCommand->m_Address)) = radPlatformEndian16(
                                    pCommand->m_Attrib.m_us.m_Value);
                            break;
                        }

                        case WPDShort : {
                            *((short *) (pCommand->m_Address)) = radPlatformEndian16(
                                    pCommand->m_Attrib.m_s.m_Value);
                            break;
                        }
                        ...
                        case WPDBoolean : {
                            *((bool *) (pCommand->m_Address)) =
                                    pCommand->m_Attrib.m_bool.m_Value != 0;
                            break;
                        }

                        case WPDFloat : {
                            *((float *) (pCommand->m_Address)) = radPlatformEndianFloat(
                                    pCommand->m_Attrib.m_f.m_Value);
                            break;
                        }
                        ...
                        case WPDVector : {
                            *((float *) (pCommand->m_Address) + 0) = radPlatformEndianFloat(
                                    pCommand->m_Attrib.m_vector.m_x);
                            *((float *) (pCommand->m_Address) + 1) = radPlatformEndianFloat(
                                    pCommand->m_Attrib.m_vector.m_y);
                            *((float *) (pCommand->m_Address) + 2) = radPlatformEndianFloat(
                                    pCommand->m_Attrib.m_vector.m_z);
                            break;
                        }

                        case WPDString : {
                            strcpy((char *) pCommand->m_Address,
                                   (const char *) pRxAddress + sizeof(WCWriteValue));
                            bytesReceived -= radPlatformEndian32(
                                    pCommand->m_Attrib.m_string.m_Length);
        ...
        m_Channel->ReceiveAsync(m_RxBuffer + bytesReceived, sizeof(m_RxBuffer) - bytesReceived,
                                this);
    }
}
