...
void rDbgComSocketTargetChannel::OnDispatchCallack(void *userData) {
    ...
    if (m_CurrentBytesToRead != 0) {
        ...
        //
        ReadMore:
        int bytesRead = m_ParentTarget->m_SocketImp->recv(m_Socket, (char *) m_CurrentReadAddress,
        ...
        if (bytesRead <= 0) {
            ...
            if (m_CurrentBytesToRead == 0) {
                ...
                if (m_ReadingHeader) {
                    ...
                    rAssert(sizeof(m_RxBuffer) >=
                            radPlatformEndian16(((DECI2_HDR *) m_RxBuffer)->length));
                    m_CurrentBytesToRead = radPlatformEndian16(((DECI2_HDR *) m_RxBuffer)->length) -
                                           sizeof(DECI2_HDR);
    ...
    if (m_CurrentBytesToWrite != 0) {
        ...
        //
        int bytesWritten = m_ParentTarget->m_SocketImp->send(m_Socket,
...
bool rDbgComSocketTargetChannel::ProcessReceiveComplete(void) {
    ...
    while (radPlatformEndian32(pData->m_DataSize) > 0) {
        ...
        pData->m_DataSize = radPlatformEndian32(
                radPlatformEndian32(pData->m_DataSize) - bytesCopied);
