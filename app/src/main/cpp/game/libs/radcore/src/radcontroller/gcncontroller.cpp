...
u32 g_WheelHandles[SI_MAX_CHAN] = {LG_INVALID_DEVICE, LG_INVALID_DEVICE, LG_INVALID_DEVICE,
                                   LG_INVALID_DEVICE};
...
//					sShort = pData->substickX;
                        newValue = (((float) pData->substickX / PAD_SUB_STICK_MAX_RADIUS) + 1.0f) /
                                   2.0f;
                        break;

                    case PAD_RIGHT_STICK_Y:
//					sShort = pData->substickY;
                        newValue = (((float) pData->substickY / PAD_SUB_STICK_MAX_RADIUS) + 1.0f) /
                                   2.0f;
                        break;
    ...
    float CalculateNewValue(LGPosition *pData) {
        ...
        if (pData != NULL) {
            if (m_pType == g_Gcnipt[0]) // Button
            {
                newValue = ((unsigned int) (pData->button) & m_Offset) ? 1.0f : 0.0f;
                //sprintf(outbuf, "button name = [%s], button value = [%f]\n", GetName(), newValue);
            } else if ((m_pType == g_Gcnipt[1]) ||
                       (m_pType == g_Gcnipt[2])) //Wheel or combined brake/gas
            {
                s8 sShort = 0;
            ...
            if (m_hController != -1) {
                if (m_SIType == SI_GC_CONTROLLER || m_SIType == SI_GC_WAVEBIRD) {
                    if (g_PadStatus[m_hController].err == PAD_ERR_NO_CONTROLLER) {
                        ...
                        //
                        IRadControllerOutputPoint *pICop = reinterpret_cast<IRadControllerOutputPoint *>(m_xIOl_OutputPoints->GetAt(
                                0));
    ...
    radControllerGcn
        ...
        for (unsigned int input = 0; input < (pointSize / sizeof(GcnInputPoint)); input++) {
            IRefCount *pObject = static_cast<IRefCount *>(new(
                    g_CtlrAllocator) radControllerInputPointGcn(points[input].m_pType,
                                                                points[input].m_pName,
                                                                points[input].m_Mask)
            );
        ...
        if (m_SIType == SI_GC_CONTROLLER || m_SIType == SI_GC_WAVEBIRD) {
            ...
            if (m_xIOl_OutputPoints != NULL) {
                radControllerOutputPointGcn *pMotor = new(
                        g_CtlrAllocator) radControllerOutputPointGcn("Motor");
            ...
            if (openStatus == LG_SUCCESS) {
                ...
                //
                if (m_xIOl_OutputPoints != NULL) {
                    radControllerOutputPointGcnLG *pSpring = new(
                            g_CtlrAllocator) radControllerOutputPointGcnLG("CenterSpring",
                                                                           LG_TYPE_SPRING);

                    m_xIOl_OutputPoints->AddObject(static_cast<IRefCount *>(pSpring));

                    radControllerOutputPointGcnLG *pDamper = new(
                            g_CtlrAllocator) radControllerOutputPointGcnLG("BaseDamper",
                                                                           LG_TYPE_DAMPER);

                    m_xIOl_OutputPoints->AddObject(static_cast<IRefCount *>(pDamper));

                    radControllerOutputPointGcnLG *pConstant = new(
                            g_CtlrAllocator) radControllerOutputPointGcnLG("Constant",
                                                                           LG_TYPE_CONSTANT);

                    m_xIOl_OutputPoints->AddObject(static_cast<IRefCount *>(pConstant));

                    radControllerOutputPointGcnLG *pRumble = new(
                            g_CtlrAllocator) radControllerOutputPointGcnLG("Rumble",
                                                                           LG_TYPE_TRIANGLE);

                    m_xIOl_OutputPoints->AddObject(static_cast<IRefCount *>(pRumble));

                    radControllerOutputPointGcnLG *pHeavyRumble = new(
                            g_CtlrAllocator) radControllerOutputPointGcnLG("HeavyRumble",
                                                                           LG_TYPE_SQUARE);
