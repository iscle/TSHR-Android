...
void radRequestPoolInitialize
    ...
    //
    radMemoryCreatePool(&s_RequestPool, sizeof(radRequestUnion), maxOutstandingRequests, 0, false,
                        None, alloc, "radFileRequest");
...
radDrive::CompletionStatus FileReadRequest::DoRequest(void) {
    ...
    if (bytesToRead > 0) {
        ...
        //
        if (m_pFile->m_FileCache.ShouldUseCache(m_pFile->m_Position + m_pFile->m_BaseOffset,
                                                bytesToRead)) {
            status =
...
radDrive::CompletionStatus FileSetPositionRequest::DoRequest(void) {
    rAssertMsg(m_pFile->m_IsOpen, rNullStringWrap(m_pFile->m_Filename));
    rAssertMsg(m_Position <= m_pFile->m_Size,
               "radFileSystem: setting file pointer past end of file.");
...
radDrive::CompletionStatus DriveFindFirstRequest::DoRequest(void) {
    ...
    radDrive::CompletionStatus status =
            pDrive->FindFirst(m_pSearchSpec, m_pDirInfo, &m_pInstDrive->m_DirHandle,
                              !m_pInstDrive->m_DirSearch);
