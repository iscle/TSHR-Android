...
radPs2CdDrive::FindNext(radFileDirHandle *pHandle, IRadDrive::DirectoryInfo *pDirectoryInfo) {
    //
    // Check if the disk changed.
    //
    radFileError chError = DiskChanged();
...
radFileError radPs2CdDrive::BuildDirectory(unsigned int dirAddress, unsigned int level, char *pName,
                                           unsigned int key) {
...
radFileError radPs2CdDrive::FindDirectorySector
    ...
    while (curr < dirNameEnd) {
        ...
        //
        unsigned int numSectors =
                (extentLen + m_MediaInfo.m_SectorSize - 1) / m_MediaInfo.m_SectorSize;
...
void radPs2CdDrive::BuildFileSpec(const char *fileName, char *fullName, unsigned int size,
                                  char **pName) {
