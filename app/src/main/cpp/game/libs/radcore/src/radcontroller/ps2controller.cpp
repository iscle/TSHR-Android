        ...
        do {
            ...
            for (s = 0; s < CL_MAXIMUM_SLOTSONPORT; s++) {
                if (m_LocationTable[s] != NULL) {
                    if
                            (
                            m_LocationTable[s]->GetControllerState() !=
                            iControllerLocation2::CL_STATE_DISCONNECTED &&
                            m_LocationTable[s]->GetControllerState() !=
                            iControllerLocation2::CL_STATE_READY
                            ) {
                        done = false;
                    }
                }
...
IRadController *rControllerSystem2::GetControllerAtLocation(const char *pLocation) {
    ...
    if (strncmp("USB", pLocation, 3) == 0) {
        ...
        if (m_xIOl_Controllers != NULL) {
            ...
            while (NULL !=
                   (pIRadController = reinterpret_cast<IRadController *>(m_xIOl_Controllers->GetNext()))) {
...
void rControllerSystem2::SetBufferTime(unsigned int milliseconds) {
    ...
    if (m_xIOl_Controllers != NULL) {
        ...
        while (NULL !=
               (pIController2 = reinterpret_cast<iIController2 *>(m_xIOl_Controllers->GetNext()))) {
...
void rControllerSystem2::RegisterConnectionChangeCallback(
...
void rControllerSystem2::UnRegisterConnectionChangeCallback(
        IRadControllerConnectionChangeCallback *pCallback) {
...
void rControllerSystem2::OnTimerDone(unsigned int elapsedtime, void *pUserData) {
    ...
    for (int p = 0; p < CL_MAXIMUM_PORTS; p++) {
        for (int s = 0; s < CL_MAXIMUM_SLOTSONPORT; s++) {
            if (m_LocationTable[i] != NULL) {
                int state = scePadGetState(m_LocationTable[i]->GetPort(),
                                           m_LocationTable[i]->GetSlot());
                ...
                if (state == scePadStateDiscon) {
                    ...
                    if (controller != NULL) {
                        ...
                        if (controller->IsConnected()) {
                            m_LocationTable[i]->SetControllerState(
                                    iControllerLocation2::CL_STATE_DISCONNECTED);
                    ...
                    if (controller != NULL) {
                        ...
                        int len = scePadRead(m_LocationTable[i]->GetPort(),
                                             m_LocationTable[i]->GetSlot(), buffer);
                    ...
                    if (m_LocationTable[i]->GetControllerState() ==
                        iControllerLocation2::CL_STATE_DISCONNECTED) {
                        m_LocationTable[i]->SetControllerState(
                                iControllerLocation2::CL_STATE_STARTINIT);
                    }
                    //
        ...
        case LGHINT_ENUMNEEDED: {
                ...
                switch (result) {
                    ...
                    case LG_SUCCESS: {
                        ...
                        for (wheelIndex = 0; wheelIndex < CL_MAX_WHEELS; ++wheelIndex) {
                            if (m_WheelDeviceDesc[wheelIndex].desc.ExistingHandle !=
                                LG_INVALID_DEVICE &&
                                memcmp(&deviceDesc.Topology,
                                       &(m_WheelDeviceDesc[wheelIndex].desc.Topology), 8) == 0) {
                                //we've got this one.
                        ...
                        if (!found) {
                            ...
                            memcpy(&newDevices[savedDeviceCount], &deviceDesc,
                                   sizeof(lgDevDeviceDesc));
            ...
            for (deviceIndex = 0; deviceIndex < CL_MAX_WHEELS; ++deviceIndex) {
                if (m_WheelDeviceDesc[deviceIndex].desc.ExistingHandle != LG_INVALID_DEVICE &&
                    ...
                    //Remove this wheel.
                    char location[64];
                    sprintf(location, "USB%d",
                            m_WheelDeviceDesc[deviceIndex].desc.Topology[0] - 0x01);
                ...
                } else if (m_WheelDeviceDesc[deviceIndex].desc.ExistingHandle !=
                           LG_INVALID_DEVICE) {
                    ...
                    for (i = 0; i < savedDeviceCount; ++i) {
                        if (newDevices[i].ExistingHandle != LG_INVALID_DEVICE) {
                            ...
                            if (result == LG_SUCCESS) {
                                ...
                                result = lgDevSetDeviceProperty(newDeviceHandle,
                                                                LGPROP_CENTERINGENABLED, LG_FALSE);
                                rAssert(result == LG_SUCCESS);
                                ...
                                m_WheelDeviceDesc[deviceIndex].controller = new(
                                        g_CtlrAllocator) LGDevWheelController(&newDevices[i]);
                                ...
                                m_xIOl_Controllers->AddObject(
                                        m_WheelDeviceDesc[deviceIndex].controller);

                                //Copy this to our list of official wheels.
                                memcpy(&m_WheelDeviceDesc[deviceIndex].desc, &newDevices[i],
                                       sizeof(lgDevDeviceDesc));
                                ...
                                if (m_StatusCallback != NULL) {
                                    IRadController *pController = NULL;
                                    pController = static_cast<IRadController *>(m_WheelDeviceDesc[deviceIndex].controller);
                                    m_StatusCallback->OnControllerConnectionStatusChange(
                                            pController);
                                }
                                break;
                            }
...
void rControllerSystem2::InitializeLocations(void) {
    for (int port = 0; port < CL_MAXIMUM_PORTS; port++) {
        ...
        for (int slot = 0; slot < nSlots; slot++) {
            int ret = scePadPortOpen(port, slot,
                                     g_ControllerPadDMABuffer[port * CL_MAXIMUM_SLOTSONPORT +
                                                              slot].m_PadDMABuffer);

            m_LocationTable[port * CL_MAXIMUM_SLOTSONPORT + slot] = new(
                    g_CtlrAllocator) iControllerLocation2(port, slot);
        }
    }
}
...
void rControllerSystem2::CreateController(int id, iControllerLocation2 *location,
                                          rController2 **pprController) {
...
void
rControllerSystem2::_ControllerAlmostReady(iControllerLocation2 *location, int state, int ltime) {
    int p = location->GetPort();
    int s = location->GetSlot();
    ...
    if (state == scePadStateStable || state == scePadStateFindCTP1) {
        ...
        bool bConnectOld = location->ReconnectIfControllerMatches(buffer[1], &controller,
                                                                  (m_BufferingTime / pollingRate) +
                                                                  2);
...
bool iControllerLocation2::ReconnectIfControllerMatches(int id, rController2 **controller,
...
rController2::rController2(int port, int slot, int id, IRadString *locationName,
                           unsigned char *pActAlignBuffer, unsigned char *pActDirectBuffer,
                           int numActuators, unsigned int bufferSize)
        :
        m_ReferenceCount(1),
        m_Port(port),
        m_Slot(slot),
    ...
    if (m_xIOl_InputPoints != NULL && m_xIOl_OutputPoints != NULL) {
        ...
        if (len > 0) {
            ...
            memcpy((void *) (radBuffer + sizeof(unsigned int)), &sonyBuffer,
            ...
            if (m_xIOl_InputPoints != NULL) {
                ...
                while (NULL !=
                       (pIControllerInputPoint2 = reinterpret_cast<iIControllerInputPoint2 *>(m_xIOl_InputPoints->GetNext()))) {
...
unsigned int rController2::GetNumberOfInputPointsOfType(const char *pType) {
    ...
    if (m_xIOl_InputPoints != NULL) {
        ...
        while (NULL !=
               (pIRadControllerInputPoin = reinterpret_cast<IRadControllerInputPoint *>(m_xIOl_InputPoints->GetNext()))) {
...
unsigned int rController2::GetNumberOfOutputPointsOfType(const char *pType) {
    ...
    if (m_xIOl_OutputPoints != NULL) {
        ...
        while (NULL !=
               (pIRadControllerInputPoint = reinterpret_cast<IRadControllerOutputPoint *>(m_xIOl_OutputPoints->GetNext()))) {
...
IRadControllerInputPoint *rController2::GetInputPointByTypeAndIndex
    ...
    if (m_xIOl_InputPoints != NULL) {
        ...
        while (NULL !=
               (pIRadControllerInputPoint = reinterpret_cast<IRadControllerInputPoint *>(m_xIOl_InputPoints->GetNext()))) {
...
IRadControllerOutputPoint *rController2::GetOutputPointByTypeAndIndex
    ...
    if (m_xIOl_OutputPoints != NULL) {
        ...
        while (NULL !=
               (pIRadControllerOutputPoint = reinterpret_cast<IRadControllerOutputPoint *>(m_xIOl_OutputPoints->GetNext()))) {
    ...
    if (m_ReferenceCount > 1) {
        ...
        while (m_pIControllerBuffer2->PeekNextPacket((void *) &timeStamp, sizeof(timeStamp))) {
            ...
            unsigned int packetAge =
                    g_theCS->iGetVirtualTime() - (timeStamp + g_theCS->iGetVirtualTimeAdjust());
...
void rController2::iPoll(unsigned int timerManagerMs) {
    ...
    if (m_ReferenceCount > 1) {
        ...
        memcpy((void *) (tempBuffer + sizeof(unsigned int)), GetDataBuffer(),
               sizeof(unsigned int) + 32);
...
iControllerOutputHandler2::iControllerOutputHandler2(int port, int slot,
                                                     unsigned char *pActDirectBuffer)
        :
        m_ReferenceCount(1),
        m_Connected(true),
        m_Port(port),
...
rControllerOutputPoint2::rControllerOutputPoint2(iRIControllerOutputPointCallback2 *handler,
                                                 IRadString *name, IRadString *type)
        :
        m_ReferenceCount(1),
        m_Value(0.0f),
        m_Name(name),
...
void rControllerInputPoint2::RegisterControllerInputPointCallback(
        IRadControllerInputPointCallback *pCallback, unsigned int userData) {
    rAssert(pCallback != NULL);

    if (pCallback != NULL) {
        rAssertMsg(m_pWIControllerInputPointCallback2 == NULL,
                   "Only one controller input point callback supported at this time");
    }
...
void rControllerInputPoint2::UnRegisterControllerInputPointCallback(
...
bool rControllerInputPoint2::iHandleEvent(unsigned char *buffer, unsigned int ltime) {
    ...
    if (newValue != m_Value) {
        if (((newValue - m_Value) >= m_Tolerance) || ((m_Value - newValue) >= m_Tolerance)) {
            ...
            if (m_pWIControllerInputPointCallback2 != NULL) {
                m_pWIControllerInputPointCallback2->OnControllerInputPointChange(m_UserData,
                                                                                 m_Value);
            }
            return true;
        }
...
float LGDevWheelInputPoint::CalculateNewValue(lgDevPosition *position) {
    ...
    if (position != NULL) {
        ...
        } else if (m_Data->m_pType == InputPointTypes[2] ||
                   m_Data->m_pType == InputPointTypes[3])//Some kind of axis.
        {
            newValue = (float) ((unsigned char) ((char *) position)[m_Data->m_Offset]) /
                       255.0f;  //They're 0 - 255
        } else //POV
        {
            //TODO: Better way?
...
bool LGDevWheelInputPoint::HandleEvent(lgDevPosition *position, unsigned int time) {
    ...
    if (newValue != m_Value) {
        if (((newValue - m_Value) >= m_Tolerance) || ((m_Value - newValue) >= m_Tolerance)) {
            ...
            if (NULL !=
                (pIWcr = reinterpret_cast<IRadWeakCallbackWrapper *>(m_xIOl_Callbacks->GetNext()))) {
...
//
//=============================================================================
void LGDevWheelInputPoint::RegisterControllerInputPointCallback(
        IRadControllerInputPointCallback *pCallback,
...
//
//=============================================================================
void LGDevWheelInputPoint::UnRegisterControllerInputPointCallback(
        IRadControllerInputPointCallback *pCallback) {
    //
...
LGDevWheelController::LGDevWheelController(lgDevDeviceDesc *desc) :
    ...
    for (unsigned int input = 0; input < (pointSize / sizeof(LGDevInputPointData)); input++) {
        IRefCount *pObject = static_cast<IRefCount *>(new(g_CtlrAllocator) LGDevWheelInputPoint(
                &points[input]));
    ...
    //
    if (m_xIOl_OutputPoints != NULL) {
        LGDevWheelOutputPoint *pSpring = new(g_CtlrAllocator) LGDevWheelOutputPoint("CenterSpring",
                                                                                    LGTYPE_SPRING);

        m_xIOl_OutputPoints->AddObject(static_cast<IRefCount *>(pSpring));

        LGDevWheelOutputPoint *pDamper = new(g_CtlrAllocator) LGDevWheelOutputPoint("BaseDamper",
                                                                                    LGTYPE_DAMPER);

        m_xIOl_OutputPoints->AddObject(static_cast<IRefCount *>(pDamper));

        LGDevWheelOutputPoint *pConstant = new(g_CtlrAllocator) LGDevWheelOutputPoint("Constant",
                                                                                      LGTYPE_CONSTANT);

        m_xIOl_OutputPoints->AddObject(static_cast<IRefCount *>(pConstant));

        LGDevWheelOutputPoint *pRumble = new(g_CtlrAllocator) LGDevWheelOutputPoint("Rumble",
                                                                                    LGTYPE_TRIANGLE);

        m_xIOl_OutputPoints->AddObject(static_cast<IRefCount *>(pRumble));

        LGDevWheelOutputPoint *pHeavyRumble = new(g_CtlrAllocator) LGDevWheelOutputPoint(
                "HeavyRumble", LGTYPE_SQUARE);
...
void LGDevWheelController::iPoll(unsigned int timerManagerMs) {
    ...
    if (result == LG_SUCCESS) {
        ...
        while (NULL !=
               (pICop = reinterpret_cast<LGDevWheelOutputPoint *>(m_xIOl_OutputPoints->GetNext()))) {
        ...
        if (packetAge < 0x7FFFFFFF) {
            ...
            if (gotPacket) {
                if (m_xIOl_InputPoints != NULL) {
                    ...
                    while (NULL !=
                           (pLGDevWheelInputPoint = reinterpret_cast<LGDevWheelInputPoint *>(m_xIOl_InputPoints->GetNext()))) {
...
unsigned int LGDevWheelController::GetNumberOfInputPointsOfType(const char *pType) {
    ...
    while (NULL !=
           (pICip = reinterpret_cast<IRadControllerInputPoint *>(m_xIOl_InputPoints->GetNext()))) {
...
unsigned int LGDevWheelController::GetNumberOfOutputPointsOfType(const char *pType) {
    ...
    while (NULL !=
           (pICop = reinterpret_cast<IRadControllerOutputPoint *>(m_xIOl_OutputPoints->GetNext()))) {
...
//
//=============================================================================
IRadControllerInputPoint *
LGDevWheelController::GetInputPointByTypeAndIndex(const char *pType, unsigned int index) {
    //
...
//
//=============================================================================
IRadControllerOutputPoint *
LGDevWheelController::GetOutputPointByTypeAndIndex(const char *pType, unsigned int index) {
    //
