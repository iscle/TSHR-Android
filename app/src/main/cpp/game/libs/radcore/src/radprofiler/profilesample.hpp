...
public:
    ...
    virtual void
    EndMeasure(radTime64 *uTotalTimeInMicroSec = NULL, radTime64 *uAveTimeInMicroSec = NULL,
               radTime64 *uThisTimeInMicroSec = NULL, unsigned int *uExecutionCount = NULL);

    virtual void StartFrame();

    virtual void EndFrame(radTime64 uTotalFrameTimeInMicroSec, radTime64 uAveFrameTimeInMicroSec,
                          radTime64 uThisFrameTimeInMicroSec, unsigned int uFrameCount);
...
inline void *radProfileSample::operator new(size_t size, radMemoryAllocator allocator) {
    if (s_pProfileSampleAllocator != NULL) {
        ...
        //
        rTuneString(
                "radProfiler: Warning: Performance warning, specialized memory allocator not found.\n");
