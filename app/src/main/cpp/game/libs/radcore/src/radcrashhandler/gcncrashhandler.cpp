...
static unsigned int radCrashHandlerGcnThreadEntry(void *userData) {
    ...
    void *pCardWorkArea = ::radMemoryAllocAligned(
            g_RadCrashHandlerGcnAllocator, CARD_WORKAREA_SIZE,
            64);  // only requires 32 bytes alignment
    ...
    g_pRadCrashRecordGcn = static_cast<radCrashRecordGcn *>(::radMemoryAllocAligned(
            g_RadCrashHandlerGcnAllocator, fileSize, 1024));
    ...
    unsigned int *pMemStackDump = (unsigned int *) (g_pRadCrashRecordGcn->m_OSContext.gpr[SP] -
                                                    RAD_CRASH_RECORD_GCN_STACK_DUMP_SP_OFFSET *
                                                    sizeof(unsigned int));

    if (g_pRadCrashRecordGcn->m_OSContext.gpr[SP] -
        RAD_CRASH_RECORD_GCN_STACK_DUMP_SP_OFFSET * sizeof(unsigned int) < 0) {
        pMemStackDump = (unsigned int *) (0x00000000);  // stay valid
    }

    // Record the contents of the stack

    for (unsigned int dumpIndex = 0;
         dumpIndex < RAD_CRASH_RECORD_GCN_STACK_DUMP_SIZE; dumpIndex++) {
