...
inline unsigned int radMemoryBinAllocator::InternalBin::GetPaddedElementSize(void) {
    return m_ElementSizeInBytes + RADMEMORY_BIN_ALLOCATOR_PRE_PAD +
           RADMEMORY_BIN_ALLOCATOR_POST_PAD;
}
...
inline void *radMemoryBinAllocator::InternalBin::UnAlignMemory(void *pMemory) {
    unsigned int offset = (((char *) pMemory - GetUnPaddedMemoryStart()) / GetPaddedElementSize()) *
                          GetPaddedElementSize();
...
inline radMemoryBinAllocator::InternalBin::FreeBlock *
radMemoryBinAllocator::InternalBin::IndexToPtr(
    ...
    if (index == NULL_INDEX) {
        ...
        rAssert((char *) pFreeBlock >= GetUnPaddedMemoryStart() &&
                (char *) pFreeBlock < GetUnPaddedMemoryEnd());
    }
...
inline FREE_BLOCK_INDEX_TYPE radMemoryBinAllocator::InternalBin::PtrToIndex(void *pMemory) {
    FREE_BLOCK_INDEX_TYPE index =
            ((char *) pMemory - GetUnPaddedMemoryStart()) / GetPaddedElementSize();
