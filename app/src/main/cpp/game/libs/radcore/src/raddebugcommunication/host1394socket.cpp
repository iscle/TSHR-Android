...
CHost1394Socket::CHost1394Socket(void)
    ...
    if ((result == ERROR_SUCCESS) || (result == REG_OPENED_EXISTING_KEY)) {
        ...
        LONG result = RegQueryValueEx(RegKey, SOCKET_TIMEOUT_FIELD, 0, NULL,
                                      (unsigned char *) &timeoutValue, &tmpLength);
...
bool CHost1394Socket::DoTargetCommunication(bool doingTargetCommunication) {
    ...
    if (!m_pSharedData->m_1394DataInitialized) {
        ...
        //
        bool result = m_pIRad1394Master->WriteMemory(BUS_NUMBER, NODE_NUMBER,
                                                     (unsigned int) &m_p1394Memory->m_ChannelControlBlock,
    ...
    ReRead:
    bool result = m_pIRad1394Master->ReadMemory(BUS_NUMBER, NODE_NUMBER,
                                                (unsigned int) &m_p1394Memory->m_ChannelControlBlock,
                                                sizeof(controlBlock), &controlBlock);
        ...
        for (i = 0; i < FW1394_CHANNEL_COUNT; i++) {
            if ((m_pSharedData->m_SocketArray[i].m_State != Connecting) &&
                ...
                if (m_pSharedData->m_SocketArray[i].m_State == Connected) {
                    ...
                    //
                    PostMessage(m_pSharedData->m_SocketArray[i].m_hWnd,
                                m_pSharedData->m_SocketArray[i].m_WndMsg,
                ...
                if (controlBlock.m_MemoryOwner == FW1394ChannelControlBlock::TARGET_RESTARTED) {
                    ...
                    PostMessage(m_pSharedData->m_SocketArray[i].m_hWnd,
                                m_pSharedData->m_SocketArray[i].m_WndMsg,
                                (unsigned int) m_pSharedData->m_SocketArray[i].m_Socket,
                                0xFFFF0000 | FD_CONNECT);
                }
            }
        }
    ...
    for (i = 0; i < FW1394_CHANNEL_COUNT; i++) {
        if (m_pSharedData->m_SocketArray[i].m_State == Connected) {
            ...
            if ((bytesInReceiver != 0) &&
                ...
                //
                tryAgain:
                if (m_pIRad1394Master->ReadMemory(BUS_NUMBER, NODE_NUMBER,
                                                  (unsigned int) &m_p1394Memory->m_Channel[m_pSharedData->m_SocketArray[i].m_1394Channel].m_TargetTransmitBuffer,
                                                  RoundUpTo16(bytesInReceiver),
                                                  m_pSharedData->m_SocketArray[i].m_RxBuffer)) {
    ...
    for (i = 0; i < FW1394_CHANNEL_COUNT; i++) {
        if ((m_pSharedData->m_SocketArray[i].m_State == Connected) ||
            ...
            if ((bytesInTransmitter == 0) &&
                ...
                //
                if (m_pIRad1394Master->WriteMemory(BUS_NUMBER, NODE_NUMBER,
                                                   (unsigned int) &m_p1394Memory->m_Channel[m_pSharedData->m_SocketArray[i].m_1394Channel].m_HostTransmitBuffer,
                                                   RoundUpTo16(
                                                           m_pSharedData->m_SocketArray[i].m_BytesInTxBuffer),
                                                   m_pSharedData->m_SocketArray[i].m_TxBuffer)) {
                    controlBlock.m_HostTransmitCrc[m_pSharedData->m_SocketArray[i].m_1394Channel] =
                            MakeCrc(m_pSharedData->m_SocketArray[i].m_TxBuffer,
                                    m_pSharedData->m_SocketArray[i].m_BytesInTxBuffer);
        ...
        if (m_pSharedData->m_SocketArray[i].m_State == Connecting) {
            ...
            if (m_pSharedData->m_SocketArray[i].m_1394Channel == FW1394_CHANNEL_COUNT) {
                ...
                if (message == FW1394ChannelControlBlock::ConnectAcceptMessage) {
                    ...
                    //
                    if (GetTickCount() - m_pSharedData->m_SocketArray[i].m_ConnectTime >
                        CONNECTION_TIMEOUT) {
                        //
    ...
    // Lets write the data into the shared memory.
    //
    m_pIRad1394Master->WriteMemory(BUS_NUMBER, NODE_NUMBER,
                                   (unsigned int) &m_p1394Memory->m_ChannelControlBlock,
                                   sizeof(controlBlock), &controlBlock);
