...
extern "C" void radCrashHandlerPs2WriteReport(void) {
    ...
    while (temp < 1) {
        ...
        sprintf(g_pTheRadCrashRecordPs2->m_RadCrashRecordInfo.m_Platform,
                RAD_CRASH_RECORD_PS2_PLATFORM);
        sprintf(g_pTheRadCrashRecordPs2->m_RadCrashRecordInfo.m_Version,
                RAD_CRASH_RECORD_PS2_VERSION);
        ...
        radCrashRecordPs2 *pReport = static_cast<radCrashRecordPs2 *>(::radMemoryAllocAligned(
                g_RadCrashHandlerAllocator, g_pTheRadCrashRecordPs2FileSize,
                256));
...
void radCrashHandlerInitialize(radMemoryAllocator allocator) {
    ...
    g_pTheRadCrashRecordPs2FileSize = g_pTheRadCrashRecordPs2FileSize -
                                      (g_pTheRadCrashRecordPs2FileSize % mediaInfo.m_SectorSize) +
                                      mediaInfo.m_SectorSize;

    //
    // Allocate memory for the crash record.  Make it a very safe alignment.
    //

    g_pTheRadCrashRecordPs2 = static_cast<radCrashRecordPs2 *>(::radMemoryAllocAligned(
            g_RadCrashHandlerAllocator, g_pTheRadCrashRecordPs2FileSize,
            256));
    ...
    ::radCrashHandlerInstallHook(RAD_CRASH_TLB_REFILL_VECTOR,
                                 (unsigned int) radCrashHandlerPs2ExceptionHandler_V_TLB_REFILL);
    ::radCrashHandlerInstallHook(RAD_CRASH_COMMON_VECTOR,
                                 (unsigned int) radCrashHandlerPs2ExceptionHandler_V_COMMON);
}
