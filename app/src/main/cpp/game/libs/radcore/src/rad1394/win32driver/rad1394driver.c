...
NTSTATUS
rad1394_SynchCompletionRoutine(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PKEVENT Event);
...
NTSTATUS rad1394_AsyncRead(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN ULONG bRawMode, IN ULONG
                           bGetGeneration,
                           IN IO_ADDRESS DestinationAddress, IN ULONG nNumberOfBytesToRead, IN ULONG
                           nBlockSize,
                           IN ULONG fulFlags, IN ULONG ulGeneration, IN ULONG localRead, IN OUT
                           PULONG Data);

NTSTATUS rad1394_AsyncWrite(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN ULONG bRawMode, IN ULONG
                            bGetGeneration,
                            IN IO_ADDRESS DestinationAddress, IN ULONG nNumberOfBytesToWrite,
                            IN ULONG nBlockSize,
                ...
                case RAD1394_IOCTL_READ: {
                    ...
                    if (inputBufferLength < (sizeof(ASYNC_READ) - RAD1394_MAX_READ_SIZE)) {
                        ...
                        if ((outputBufferLength < (sizeof(ASYNC_READ) - RAD1394_MAX_READ_SIZE)) ||
                            (outputBufferLength - (sizeof(ASYNC_READ) - RAD1394_MAX_READ_SIZE) <
...
NTSTATUS rad1394_AsyncRead
    ...
    if ((deviceExtension->AddressRangeData != NULL) && bLocalRead) {
        void *srcAddress = (unsigned char *) (deviceExtension->AddressRangeData->Buffer) +
...
NTSTATUS rad1394_AsyncWrite
    ...
    if (deviceExtension->AddressRangeData != NULL) {
        void *destAddress = (unsigned char *) (deviceExtension->AddressRangeData->Buffer) +
                            DestinationAddress.IA_Destination_Offset.Off_Low;
