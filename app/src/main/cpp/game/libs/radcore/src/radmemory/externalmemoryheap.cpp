...
)
{
    ...
    while ( ( pEmo_Free != NULL ) &&
            ( pEmo_Free->m_ClientAddress != static_cast< unsigned int >( (uintptr_t)pMemory ) ) )
    {
...
)
{
    ...
    while( ( pEmo_Free != NULL ) && ( pEmo_Free->m_ClientAddress != static_cast< unsigned int >( (uintptr_t)pMemory ) ) )
    {
        pEmo_Free = pEmo_Free->m_pEmo_Next;
    }
...
)
{
    ...
    pEmo_FreeMem->Initialize(
        this,
        0,
        static_cast< unsigned int >( (uintptr_t)pStartOfExternalMemory ),
        sizeOfExternalMemory,
        NULL,
        NULL
    );
