...
namespace sim {
    ...
    SimStateArticulated *SimStateArticulated::CreateSimStateArticulated(poser::Pose *inPose,
                                                                        SimStateArticulated *inSimState,
                                                                        const char *inName) {
        ...
        SimStateArticulated *simState = CreateSimStateArticulated(inPose,
                                                                  inSimState->GetCollisionObject(),
                                                                  simObj);
    ...
    SimStateArticulated *
    SimStateArticulated::CreateSimStateArticulated(tUID inUid, poser::Pose *inPose,
                                                   SimStateAttributes inAttrib,
    ...
    SimStateArticulated *
    SimStateArticulated::CreateSimStateArticulated(const char *inName, poser::Pose *inPose,
                                                   SimStateAttributes inAttrib,
                                                   tEntityStore *inStore) {
    ...
    SimStateArticulated *
    SimStateArticulated::CreateSimStateArticulated(poser::Pose *inPose, CollisionObject *collObj,
                                                   SimulatedObject *simObj) {
...
//
    ...
    void SimStateArticulated::SetCollisionObject(CollisionObject *inObject) {
        ...
        for (int i = 0; i < mCollisionObject->GetNumJoint(); i++) {
            CollisionVolume *localVolume = mCollisionObject->GetCollisionVolume()->GetSubCollisionVolume(
                    i);
            if (localVolume) {
...
// that don't require it
    ...
    void SimStateArticulated::AddCacheToVirtualCM(const rmt::Vector &inV, const rmt::Vector &inW,
                                                  int inJointIndex) {
        if (inJointIndex >= 0) {
            for (int i = 0; i < mVirtualCMList.GetSize(); i++) {
                if (mVirtualCMList[i]->GetIndex() == inJointIndex) {
    ...
    SimState *SimStateArticulated::CreateJointSimState(int inJoint, const char *inName) {
        ...
        if (mCollisionObject) {
            CollisionVolume *vol = mCollisionObject->GetCollisionVolume()->GetSubCollisionVolume(
                    inJoint, false);
    ...
    void SimStateArticulated::UpdateHasMoved() {
        if (mCollisionObject && !mCollisionObject->HasMoved()) {
            ...
            Vector dp;
            dp.Sub(mJointState[mMinJointCollisionIndex].GetPosition(), *((Vector * )(
                    &mJointState[mMinJointCollisionIndex].GetPreviousTransform().m[3])));

            if (dp.DotProduct(dp) > 0) {
                mCollisionObject->Moved(dp);
            } else {
                mCollisionObject->SetHasMoved(
                        true); // root might not be moving but animation still runs on it.
            }
        }
        if (mArticulated) {
    ...
    void SimStateArticulated::GetVelocity(const rmt::Vector &inPosition, rmt::Vector &oVelocity,
                                          int inIndex) {
...
//
    ...
    void SimStateArticulated::JointState::GetVelocity(const rmt::Vector &inPosition,
                                                      rmt::Vector &oVelocity) {
        UpdateVelocity();
    ...
    void SimStateArticulated::JointState::UpdateVelocity() {
        if (!mVelocityUpdated) {
            ...
            ExtractVelocityFromMatrix(mPreviousTransform, mJoint->GetWorldTransform().GetMatrix(),
                                      scale, mPreviousDt, mVelocityState);
        }
    }
