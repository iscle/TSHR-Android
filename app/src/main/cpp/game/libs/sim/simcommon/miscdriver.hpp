...
// applies simple oscillation transform to a given joint
    ...
    public:
        OscillatorDriver(int minJoint = 0, float inMax = 5.0f) : mMinJoint(minJoint),
                                                                 mMaxTranslation(inMax) {
    ...
    public:
        RotationDriver(int minJoint = 0.0f, int inAxis = 0, float inSpeed = 5.0f) : mMinJoint(
                minJoint) {
            mSpeedAngle = inSpeed;
            if (inAxis == 0)
    ...
    public:
        OrbitDriver(int minJoint = 0.0f, int inAxis = 0, float inSpeed = 5.0f) : mMinJoint(
                minJoint) {
            mSpeedAngle = inSpeed;
            if (inAxis == 0)
                mRotationAxis.Set(1, 0, 0);
...
// this should be the last thing done, after any IK or physics etc drivers
    ...
    public:
        ...
        virtual void Update(poser::Pose *pose) {
            pose->GetJoint(mDstJntIndex)->SetObjectMatrixRotation(
                    pose->GetJoint(mSrcJntIndex)->GetObjectMatrix());
        }
