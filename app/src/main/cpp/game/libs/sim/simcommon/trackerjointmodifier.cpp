            ...
            if (!mSeeTarget) // if obj is behind relax the current mTrackerJointDef
            {
                if (Fabs(mTrackerJointDef) < speedHdt) {
                    ...
                    mTrackerJointDef -= (mTrackerJointDef > 0 ? speedHdt : -speedHdt);
                       ...
                       mGoodEnough) // do nothing if trackerJoint already look at the right place
            {
                float ang = ACos(dot);
                rAssert(ang > 0);
            ...
            if (mTrackVertical) {
                if (!mSeeTarget) // if obj is behind relax the current mTrackerJointDef
                {
                    if (Fabs(mTrackerJointDefVertical) < speedVdt) {
                        ...
                        mTrackerJointDefVertical -= (mTrackerJointDefVertical > 0 ? speedVdt
                                                                                  : -speedVdt);
                    }
                } else {
                    dr.Normalize();
                        ...
                        mGoodEnough) // do nothing if trackerJoint already look at the right place
                    {
                        ...
                        if (Fabs(mTrackerJointDefVertical) > mMaxangleV) {
                            mTrackerJointDefVertical = (mTrackerJointDefVertical > 0 ? mMaxangleV
                                                                                     : -mMaxangleV);
                        }
                    }
                }
