    ...
    float SimpleFilter1D::Estimate(float newMeasurement, float in_dt /* = 0*/) {
        ...
        if (in_dt != mDt) {
            ...
            if (mDt == 0.0f) {
                ...
                mPrevMeasurement =
                        (2.0f * mPrevMeasurement - newMeasurement * mDeltaScl) / (1.0f - mDeltaScl);
            }
...
// Pass in the threshold value that we want to detect for measUncert.
    bool SimpleFilter3D::Init(const Vector *firstGuess, const Vector *measUncert,
                              const Vector * /*ValueUncert*/) {
    ...
    bool SimpleFilterAbs3D::Init(const Vector *firstGuess, const Vector *measUncert,
                                 const Vector *ValueUncert) {
...
//
    ...
    bool
    DoubleKalmann1D::Init(float firstGuess, float measUncert, float valueUncert, float frstGuess2,
                          float measUncert2, float valueUncert2) {
