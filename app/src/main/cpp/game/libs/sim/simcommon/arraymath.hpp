...
#define ARRAY_MATH_VERY_SMALL 0.00000001f
    ...
    inline void
    AddLinearVector(const float a[], const float sa, const float b[], const float sb, float r[]) {
        r[0] = a[0] * sa + b[0] * sb;
        r[1] = a[1] * sa + b[1] * sb;
        r[2] = a[2] * sa + b[2] * sb;
    }

    inline void AddAddLinearVector(const float a[], const float sa, const float b[], const float sb,
                                   float r[]) {
        r[0] += a[0] * sa + b[0] * sb;
        r[1] += a[1] * sa + b[1] * sb;
        r[2] += a[2] * sa + b[2] * sb;
    ...
    void
    AddLinearMatrix(const float m1[][3], float s1, const float m2[][3], float s2, float mt[][3]);
    ...
    bool
    AreMatricesEqual(const float j[][3], const float k[][3], float in_eps = ARRAY_MATH_VERY_SMALL);

    bool AreMatricesEqualTranspose(const float j[][3], const float k[][3],
                                   float in_eps = ARRAY_MATH_VERY_SMALL);
    ...
    inline void
    AddScaleVectorN(float *res, const float *v_1, const float *v_2, const float f, int size) {
        rAssert(size % 4 == 0);
