...
namespace sim {
    ...
    class SimStateArticulated
            : public SimState {
    public:
        static SimStateArticulated *CreateSimStateArticulated(tUID inUID, poser::Pose *inPose,
                                                              SimStateAttributes inAttrib = SimStateAttribute_Default,
                                                              tEntityStore *inStore = NULL);

        static SimStateArticulated *
        CreateSimStateArticulated(const char *inName, poser::Pose *inPose,
                                  SimStateAttributes inAttrib = SimStateAttribute_Default,
                                  tEntityStore *inStore = NULL);

        static SimStateArticulated *
        CreateSimStateArticulated(poser::Pose *inPose, SimStateArticulated *inSimState,
                                  const char *inName = NULL);

        static SimStateArticulated *
        CreateSimStateArticulated(poser::Pose *inPose, CollisionObject *inColl,
                                  SimulatedObject *inSim);
        ...
        void UpdateJointState(
                float inDt); // this should be called after SetTransform has been called and poser::Pose has been updated
        ...
        public:
            ...
            SimVelocityState &VelocityState() {
                UpdateVelocity();
                return mVelocityState;
            }

            rmt::Vector &GetLinearVelocity() {
                UpdateVelocity();
                return mVelocityState.mLinear;
            }

            rmt::Vector &GetAngularVelocity() {
                UpdateVelocity();
                return mVelocityState.mAngular;
            }

            void GetVelocity(const rmt::Vector &inPosition, rmt::Vector &oVelocity);

            const rmt::Vector &
            GetPosition() const { return mJoint->GetWorldTransform().GetMatrix().Row(3); }
        ...
        GetArtPhysObject() { return (ArticulatedPhysicsObject *) mSimulatedObject; }
    ...
    private:
        SimStateArticulated &operator=(SimStateArticulated &inObj) {
            rAssert(false);
            return *this;
        }
