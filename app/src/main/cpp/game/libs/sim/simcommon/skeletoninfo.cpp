...
namespace sim {
    ...
    SkeletonInfo::SkeletonInfo(tSkeleton *inSkeleton) {
        ...
        for (i = 1; i < mNumJoints; i++) // add the contribution of the childs to the axis
        {
            ...
            if (numChild[parent] ==
                0) // addis all the contribution doesn't work because of some weird joints in the skeleton
            {
        ...
        for (i = 1; i < mNumJoints; i++) // transform rotaxis in joint space
        {
            ...
            inSkeleton->GetJoint(parentIndex)->inverseWorldMatrix.RotateVector(tmp,
                                                                               &mJointInfo[i].mRotAxis);
        }
