...
*/
    ...
    float ComputeScaleFromMatrix(const Matrix &m, float epsilon) {
        ...
        if (Fabs(f_0 - f_1) / f_0 < epsilon && Fabs(f_0 - f_2) / f_0 < epsilon) {
            ...
            rAssertMsg(false,
                       "SimStateMatrix::ExtractScaleFromTransform: Matrix anisotropic, can't do it");
        }
        return scale;
    }
...
//    rotation must be set. Identity is set and false is returned.
    ...
    bool BuildRotationFromTo(const rmt::Vector &in_vctFrom, const rmt::Vector &in_vctTo,
                             rmt::Quaternion &o_quat, bool &o_Flip, float inMinAng,
                             float inMaxAng) {
    ...
    bool BuildRotationFromTo(const rmt::Vector &in_vctFrom, const rmt::Vector &in_vctTo,
                             rmt::Matrix &o_mat, bool &o_Flip, float inMinAng, float inMaxAng) {
        Quaternion l_quat;
