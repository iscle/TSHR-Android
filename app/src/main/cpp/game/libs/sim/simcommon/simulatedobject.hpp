...
namespace sim {
    ...
    enum SimulatedObjectTypeEnum {
        RigidObjectType, ArticulatedObjectType, FlexibleObjectType, MaxPhObjEnum
    };
    ...
    public:
        ...
        AddImpulse(const rmt::Vector &inPos, const rmt::Vector &inImp, const SymMatrix &inSymMatrix,
                   SimulatedObject *otherObj = NULL, int index = -1, int otherIndex = -1) = 0;
        ...
        GetMassMatrix(const rmt::Vector &inPos, SymMatrix &oM, SimulatedObject *o = NULL,
                      int index = -1, int otherIndex = -1) = 0;

        virtual float InvMass() const { return 1.0f; }

        virtual void ResolveInterpenetration(int in_index, float &io_dist, const rmt::Vector &inN,
                                             rmt::Vector &o_imp, bool addconstrain,
                                             const SimulatedObject &otherObj);

        virtual void AddSolverConstraint(const rmt::Vector &inDir, int in_index) {};

        virtual bool SpeedAdjustmentFromCache(const rmt::Vector &, rmt::Vector &,
                                              int inIndex = -1) { return false; }

        // access to collision cache data
        virtual bool GetCollisionCache(rmt::Vector &o_cacheV, rmt::Vector &o_cacheW,
                                       int inIndex = -1) const { return false; }//default simulated objects do not have caches;
        ...
        virtual bool
        UpdateConstraintTarget(const LocalTarget &in_target, rmt::Vector *o_VelocityTarget,
                               SimTarget &io_target) const { return false; }

        virtual bool UpdateConstraint(const LocalTarget &in_LocalTarget, const SimTarget &in_Target,
                                      const rmt::Vector *in_VelocityTarget,
                                      float in_dt) { return false; }
    ...
    private:
        SimulatedObject &operator=(SimulatedObject &inObj) {
            rAssert(false);
            return *this;
        }
