...
namespace sim {
    ...
    class pm {     // Represents the interaction of particle row with particle column
    public:
        pm();
    ...
    public:
        ...
        void Clear() {
            mArray.Clear();
            memset(mCache, 0, sizeof(ppm) * mNbCol); /*TestBlockArray();*/ }
        ...
        void TestBlockArray() {
            for (int i = 0; i < mNbCol; i++) {
                ...
                rAssert((isIn == false && mCache[i] == NULL) ||
                        (isIn == true && mCache[i] != NULL && mCache[i]->column == i));
            }
        }
...
//
    ...
    public:
        ...
        virtual void
        modified_pcg(const float *b, /*const*/ BlockArray *Sfilter, const float *z, float *dv);
