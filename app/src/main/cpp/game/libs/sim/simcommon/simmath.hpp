...
// epsilon represent a power of 10
    inline float TruncateToEpsilon(float mScale, float epsilon = 0.001f) {
        return rmt::Floor(mScale / epsilon) * epsilon;
    }
    ...
    bool BuildRotationFromTo(const rmt::Vector &in_vctFrom, const rmt::Vector &in_vctTo,
                             rmt::Quaternion &o_quat, bool &o_Flip, float inMinAng = 0.0f,
                             float inMaxAng = rmt::PI_2);

    bool BuildRotationFromTo(const rmt::Vector &in_vctFrom, const rmt::Vector &in_vctTo,
                             rmt::Matrix &o_mat, bool &o_Flip, float inMinAng = 0.0f,
                             float inMaxAng = rmt::PI_2);
