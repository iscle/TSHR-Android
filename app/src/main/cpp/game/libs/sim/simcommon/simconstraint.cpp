...
namespace sim {
    ...
    SimConstraint::SimConstraint(SimState *in_constrainingObject,
                                 LocalTarget &in_constrainingLacalTarget,
                                 SimState *in_constrainedObject,
                                 LocalTarget &in_constrainedLacalTarget) :
    ...
    void SimConstraint::Update(float in_dt) {
        ...
        mState1->GetSimulatedObject()->UpdateConstraintTarget(mLocTarget1, &l_VelocyTarget,
                                                              *mTarget1);

        //Ask to the constrained object to satisfied the constraint.
        mState2->GetSimulatedObject()->UpdateConstraint(mLocTarget2, *mTarget1, &l_VelocyTarget,
                                                        in_dt);
