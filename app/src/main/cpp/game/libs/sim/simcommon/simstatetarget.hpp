...
namespace sim {
    ...
    public:
        ...
        virtual void GetPos(rmt::Vector &v, float dt = 0) const {
            mSimState->GetTransform().Transform(offset, &v);
        }

        virtual void GetTransform(rmt::Matrix &m, float dt = 0) const {
            m = mSimState->GetTransform();
            GetPos(*(rmt::Vector *) m.m[3]);
        }
