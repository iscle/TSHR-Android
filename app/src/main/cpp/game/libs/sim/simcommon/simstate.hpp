...
//
    ...
    void ExtractVelocityFromMatrix(const rmt::Matrix &oldMatrix, const rmt::Matrix &newMatrix,
                                   float scale, float dt, SimVelocityState &velocity);
...
//
    ...
    public:
        ...
        static SimState *
        CreateSimState(tUID inUID, SimStateAttributes inAttrib = SimStateAttribute_Default,
                       tEntityStore *inStore = NULL);

        static SimState *
        CreateSimState(const char *inName, SimStateAttributes inAttrib = SimStateAttribute_Default,
                       tEntityStore *inStore = NULL);
        ...
        static SimState *
        CreateSimState(CollisionVolume *inCollisionVolume, const char *inName = NULL,
                       tEntityStore *inStore = NULL);

        // this simstate cannot be updated
        static SimState *
        CreateManualSimState(CollisionVolume *inCollisionVolume, const char *inName = NULL);
        ...
        virtual void
        AddCacheToVirtualCM(const rmt::Vector &inV, const rmt::Vector &inW, int inIndex = -1);
        ...
        virtual void
        GetVelocity(const rmt::Vector &inPosition, rmt::Vector &oVelocity, int inIndex = -1);
        ...
        */
        ...
        float UpApproxSpeedMagnitude() const {
            return mApproxSpeedMagnitude * sApproxSpeedMagnitudeFactor;
        }

        float DownApproxSpeedMagnitude() const {
            return mApproxSpeedMagnitude / sApproxSpeedMagnitudeFactor;
        }

        static void SetApproxSpeedMagnitudeFactor(float inNewFactor) {
            if (inNewFactor > 0)sApproxSpeedMagnitudeFactor = inNewFactor;
        }
        ...
        virtual void
        DynamicPositionAdjustment(const rmt::Vector &inDeltaPosition, float inDt, bool adjustSpeed);
    ...
    protected:
        ...
        void
        MoveCollisionObject(const rmt::Matrix &previousTransform, const rmt::Matrix &newTransform);
    ...
    private:
        SimState &operator=(SimState &inObj) {
            rAssert(false);
            return *this;
        }
    ...
    public:
        ...
        virtual const rmt::Vector &
        GetPosition(int inIndex) const { return *((rmt::Vector *) sIdentity.m[3]); }
