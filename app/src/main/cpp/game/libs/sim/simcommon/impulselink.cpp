...
namespace sim {
    ...
    ImpulseLink::ImpulseLink(SimState *inSimState, int inIndex, SimTarget *inTarget,
                             float inIntensity)
        ...
        if (mSimState->GetCollisionObject()) {
            CollisionVolume *cv = mSimState->GetCollisionObject()->GetCollisionVolume()->GetSubCollisionVolume(
                    mIndex, true);
            if (cv) {
                cv->UpdateAll();
...
//
    ...
    ImpulseLinkDual::ImpulseLinkDual(SimState *inSimState, int inIndex, SimState *inSimState2,
                                     int inIndex2, float inIntensity) {
        mSimState = inSimState;
        ...
        if (mSimState2->GetCollisionObject()) {
            CollisionVolume *cv = mSimState2->GetCollisionObject()->GetCollisionVolume()->GetSubCollisionVolume(
                    mIndex2, true);
            if (cv) {
                cv->UpdateAll();
                mMinDist += cv->mSphereRadius;
