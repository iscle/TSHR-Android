...
namespace sim {
    ...
    class SimStateFlexible
            : public SimState {
    public:
        static SimStateFlexible *
        CreateSimStateFlexible(tUID inUID, SimStateAttributes inAttrib = SimStateAttribute_Default,
                               tEntityStore *inStore = NULL);

        static SimStateFlexible *CreateSimStateFlexible(const char *inName,
    ...
    private:
        SimStateFlexible &operator=(SimStateFlexible &inObj) {
            rAssert(false);
            return *this;
        }
