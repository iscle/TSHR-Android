...
//
    ...
    public:
        void SetUnits(float l, float m, float t) {
            rAssertMsg(sUnitSet == false,
                       "SimUnitsManager::SetUnits: SetUnits has already been called by someone or SimUnits::Reset has not been called\n");

            // the 1 is 'this'
            rAssertMsg(sList->GetSize() == 1,
                       "SimUnitsManager::SetUnits: SetUnits must only be called at the beginning");
...
//
...
#ifndef RAD_RELEASE
        SimEnvironment::sListOfActiveSimulatedObject = new TList<SimulatedObject *>(
                SimulatedObjectListSize);
#endif
    ...
    void ResetSimulation() {
#ifndef RAD_RELEASE
        rAssert(SimEnvironment::sListOfActiveSimulatedObject == NULL ||
                SimEnvironment::sListOfActiveSimulatedObject->GetSize() == 0);
        ...
        tPlatform::GetPlatform()->GetActiveContext()->GetInventory()->Remove(
                PhysicsProperties::DefaultPhysicsProperties());
    ...
    void SetupLineDrawing() {
        ...
        g_pLineElemArray = new TArray<LineElem>(
                128);  // icky magic number; also no checking for overflow on this array
    }
