...
namespace sim {
    ...
    void IKDriver::Init() {
        ...
        tSkeleton::Joint *sk_j1 = mPose->GetSkeleton()->GetJoint(
                mStartIndex); // shoulder or upper leg
    ...
    void IKDriver::SetProjParented() {
        ...
        mProjAxis.Rotate(mProjAxis0, m);
        mPosAxis.Rotate(mPosAxis0, m);
        SetSolverAxes();
    }
    ...
    void IKDriver::UpdateJoints() {
        ...
        updateR1.Identity();
        updateRmid.Identity(), updateR2.Identity();
        temp.Identity();
    ...
    void IKDriver::DebugDisplay() {
        ...
        tmp = mProjAxis;
        tmp.Rotate(mShiftedParent);
        dLineRel(mPose->GetJoint(mEndIndex)->GetWorldTranslation(), tmp,
                 tColour(220, 100, 40));   // projAxis redish
        tmp = mPosAxis;
        tmp.Rotate(mShiftedParent);
