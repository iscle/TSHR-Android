...
// IKLimbType is used to calculate the restPose poleVector if the limb is hyper-extended
    enum IKLimbType {
        IKLimbElbow = 0, IKLimbKnee
    };

// GoalType determines which Solve method is used
    enum GoalType {
        GPosition = 0, GOrientation, GAim
    };

// ConstraintType determines how the PoleVector is calculated 
    enum ConstraintType {
        CPosition, CPoleVector, COrientation, CAim
    };

    enum ModifierType {
        MParented = 0, MPoleFromPose, MPoleAndGoalFromPose, MPoleAndGoalFromMaya
    };
    ...
    public:
        ...
        // Types of solvers
        bool
        SolveForGoal(const rmt::Vector &goal); // solve shoulder, and elbow or equivalent for legs.
    ...
    private:
        ...
        IKDriver &operator=(IKDriver &inObj) {
            rAssert(false);
            return *this;
        }
