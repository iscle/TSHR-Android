...
namespace sim {
    ...
    enum Solving_Answer {
        Solving_Continue = 0, Solving_Aborted
    };
    ...
    protected:
        ...
        void SetCollisionDistance(float inCollDist) {
            rAssert(inCollDist >= 0);
            mCollisionDistance = inCollDist;
        }

        void SetGravityNorm(float inGravityNorm) {
            rAssert(mGravityNorm >= 0);
            mGravityNorm = inGravityNorm;
        }
        ...
        void
        StartObj(SimulatedObject *inObject, SimulatedObject *otherObject, const rmt::Vector &pos,
                 SymMatrix &inSymMatrix, int index, int otherIndex);

        Solving_Answer ComputeImpulse();

        virtual bool AddImpulse(SimulatedObject *inObject, SimulatedObject *otherObject,
                                const rmt::Vector &inPosition, SymMatrix &inSymMatrix, int index,
                                int otherIndex);
