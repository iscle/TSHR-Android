...
// return true if interference
    static inline bool IsBBIn(CollisionVolume *thisEl, CollisionVolume *el, float precision) {
        if (thisEl->Type() == WallVolumeType) {
            ...
            if (Fabs(el->mPosition.x - thisEl->mPosition.x) >
                thisEl->mBoxSize.x + el->mBoxSize.x + precision)
                return false;
            if (Fabs(el->mPosition.z - thisEl->mPosition.z) >
                thisEl->mBoxSize.z + el->mBoxSize.z + precision)
                return false;
            if (Fabs(el->mPosition.y - thisEl->mPosition.y) >
                thisEl->mBoxSize.y + el->mBoxSize.y + precision)
                return false;
        }
        return true;
    }

    void CollisionDetector::DetectCollision(TArray <CollisionObjectPair> *inPList,
                                            TArray <Collision> *inCList, float inDt,
                                            const bool printOut) {
        mDt = inDt;
...
#ifndef DEEPEST_ONLY
                        dist = nt - Fabs(f_1) +
                               Fabs(f_2); // if axis of cylinder is parallel to wall surface.
                        if (dist < mCollisionDistance) {
                            Vector posa, posb;
                            posa.ScaleAdd(vol->mPosition, -Sign(f_1) * vol->mCylinderRadius, pa);
                            posa.ScaleAdd(posa, Sign(f_2) * vol->mLength, vol->mAxis);
        ...
        if (nn > VERY_SMALL) // else edges are parallel...
        {
            ...
            if (Fabs(nt) > VERY_SMALL) // else no collision possible between these edges
            {
                ...
                if (dist < mCollisionDistance) {
                    if (!volA->HasSubVolume() && !volB->HasSubVolume()) {
                        bool dummy = false;
                        if (EdgeEdge(dummy, pa, volA->mAxis, volA->mLength, pb, volB->mAxis,
                                     volB->mLength, N, nn, dist, volA, volB)) {
