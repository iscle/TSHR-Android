...
namespace sim {
    ...
    Collision::Collision(CollisionVolume *a, const rmt::Vector &pa, CollisionVolume *b,
                         const rmt::Vector &pb, float inDist, const rmt::Vector &inNormal) {
        Set(a, pa, b, pb, inDist, inNormal);
    }

    void Collision::Set(CollisionVolume *a, const rmt::Vector &pa, CollisionVolume *b,
...
//
    ...
    float CollisionObjectPair::SafeCollisionDistance(float dt) {
        ...
        if (vn > VERY_SMALL) {
            ...
            float d =
                    vol->Type() == WallVolumeType ? VERY_LARGE : Fabs(v.DotProduct(vol->mBoxSize));
