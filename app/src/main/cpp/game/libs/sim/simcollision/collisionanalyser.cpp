...
namespace sim {
    ...
    void
    CollisionAnalyser::StartSolvingCollisionList(TArray <Collision> *inList, float current_time) {
        mCachedTime = current_time;
        mCollectingData = true;
    }

    void
    CollisionAnalyser::EndSolvingCollisionList(TArray <Collision> *inList, float current_time) {
    ...
    CollisionAnalyzer_Answer
    CollisionAnalyser::PreCollisionEvent(Collision &inCollision, int inPass) {
...
// this method allows to trigger sounds, animation and modify the objects state.
    CollisionAnalyzer_Answer CollisionAnalyser::CollisionEvent(
        ...
        if (data == NULL) {
            ...
            data = CreateNewData();
            data->Init(inSimStateA, indexA, inSimStateB, indexB,
                       mCachedPossibleEvents | (mCachedPossibleEventsA << 8) |
                       (mCachedPossibleEventsB << 16));
