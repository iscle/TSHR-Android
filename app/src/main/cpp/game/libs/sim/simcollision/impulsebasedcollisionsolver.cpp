...
namespace sim {
    ...
    void ImpulseBasedCollisionSolver::SolveCollisions(TArray <Collision> *inList, float dt_sec,
                                                      float gameTime) {
        rAssert(mAgent != NULL);
        if (mAgent == NULL) {
            return;
    ...
    void ImpulseBasedCollisionSolver::SolveCollisionImpulse() {
        ...
        StartObj(mSimObjectA, mSimObjectB, mCollision.GetPositionA(), mMassMatrixA,
                 mCollision.mIndexA, mCollision.mIndexB);
        StartObj(mSimObjectB, mSimObjectA, mCollision.GetPositionB(), mMassMatrixB,
                 mCollision.mIndexB, mCollision.mIndexA);
        ...
        do {
            ...
            oka = AddImpulse(mSimObjectA, mSimObjectB, mCollision.GetPositionA(), mMassMatrixA,
                             mCollision.mIndexA, mCollision.mIndexB);
            if (oka) {
                if (testCacheA) {
                    oka = mAgent->TestCache(mSimStateA, mCollision.mIndexA) == Solving_Aborted
                          ? false : true;
                    testCacheA = false;
                }
                if (oka) {
                    mImpulse.Scale(-1.0f);
                    okb = AddImpulse(mSimObjectB, mSimObjectA, mCollision.GetPositionB(),
                                     mMassMatrixB, mCollision.mIndexB, mCollision.mIndexA);
                    if (okb) {
                        if (testCacheB) {
                            okb = mAgent->TestCache(mSimStateB, mCollision.mIndexB) ==
                                  Solving_Aborted ? false : true;
                            testCacheB = false;
                        }
                    } else if (mSimObjectB->SpeedAdjustmentFromCache(mCollision.GetPositionB(),
                                                                     mVelocityB,
                                                                     mCollision.mIndexB)) {
                        mDVelocity.Sub(mVelocityA, mVelocityB);
                        mNormDVelocityN = mDVelocity.DotProduct(mCollision.mNormal);
            ...
            } else if (mSimObjectA->SpeedAdjustmentFromCache(mCollision.GetPositionA(), mVelocityA,
                                                             mCollision.mIndexA)) {
                mDVelocity.Sub(mVelocityA, mVelocityB);
                mNormDVelocityN = mDVelocity.DotProduct(mCollision.mNormal);
            }
    ...
    void
    ImpulseBasedCollisionSolver::StartObj(SimulatedObject *inObject, SimulatedObject *otherObject,
                                          const Vector &inPosition, SymMatrix &inMassMatrix,
                                          int inIndex, int otherIndex) {
        if (inObject) {
            inObject->StartCollision(inPosition, mCollision.mNormal, otherObject, inIndex,
                                     otherIndex);
    ...
    bool
    ImpulseBasedCollisionSolver::AddImpulse(SimulatedObject *inObject, SimulatedObject *otherObject,
                                            const Vector &inPosition, SymMatrix &inMassMatrix,
                                            int inIndex, int otherIndex) {
        bool ret = true;

        if (inObject) {
            if (!inObject->AddImpulse(inPosition, mImpulse, inMassMatrix, otherObject, inIndex,
                                      otherIndex)) {
                inObject->GetMassMatrix(inPosition, inMassMatrix, otherObject, inIndex, otherIndex);
                ret = false;
            }
    ...
    void ImpulseBasedCollisionSolver::EndObj(SimulatedObject *inObject, int inIndex, int otherIndex,
                                             bool fdyn) {
        rAssert(mAgent != NULL);
...
//Compute impulse with the help of Chatterjee formulas.
    Solving_Answer ImpulseBasedCollisionSolver::ComputeImpulse() {
        ...
        float e = RestCoeff() -
                  1.0f; //Normally e should be between 0 and 1 however we can use the ACR.
    ...
    float ImpulseBasedCollisionSolver::RestCoeff() {
        ...
        */
        ...
        float trueRest = PhysicsProperties::CombinedRestCoeff(
                mCollision.mCollisionObjectA->GetPhysicsProperties(),
                mCollision.mCollisionObjectB->GetPhysicsProperties());
    ...
    float ImpulseBasedCollisionSolver::FrictCoeff() {
        if (mSelfCollision)
            ...
            return PhysicsProperties::CombinedFrictCoeff(
                    mCollision.mCollisionObjectA->GetPhysicsProperties(),
                    mCollision.mCollisionObjectB->GetPhysicsProperties());
        }
    }
    ...
    float ImpulseBasedCollisionSolver::TangRestCoeff() {
        if (mSelfCollision)
            ...
            if (mSimObjectA && mSimObjectB) {
                return PhysicsProperties::CombinedTangRestCoeff(
                        mCollision.mCollisionObjectA->GetPhysicsProperties(),
                        mCollision.mCollisionObjectB->GetPhysicsProperties());
            } else if (mSimObjectA) {
    ...
    bool ImpulseBasedCollisionSolver::PositionCorrectionForInterpenetration(float &io_dist) {
        ...
        if ((mSimObjectA != NULL && mSimObjectA->Type() == FlexibleObjectType) &&
            (mSimObjectB != NULL && mSimObjectB->Type() == FlexibleObjectType)) {
            static int error = 1;
...
//
    ...
    void CollisionSolverAgent::StartSolvingCollisionList(TArray <Collision> *inList,
                                                         float current_time) {
        if (mAnalyser)
    ...
    void
    CollisionSolverAgent::EndSolvingCollisionList(TArray <Collision> *inList, float current_time) {
        if (mAnalyser)
    ...
    Solving_Answer CollisionSolverAgent::PreCollisionEvent(Collision &inCollision, int inPass) {
        ...
        if (inCollision.GetSimulatedObjectA() == NULL &&
            inCollision.GetSimulatedObjectB() == NULL) {
    ...
    Solving_Answer CollisionSolverAgent::CollisionEvent(SimState *inSimStateA, int indexA,
                                                        ...
                                                        SimulatedObject **simA,
                                                        SimulatedObject **simB) {
        ...
        if (mAnalyser)
            mAnalyser->CollisionEvent(inSimStateA, indexA, inSimStateB, indexB, inPos, inDvN,
                                      inDvT);
    ...
    Solving_Answer
    CollisionSolverAgent::TestImpulse(rmt::Vector &mImpulse, Collision &inCollision) {
    ...
    Solving_Answer CollisionSolverAgent::TestCache(SimState *inSimState, int inIndex) {
        ...
        if (simobj &&
            (simobj->Type() == RigidObjectType))//|| simobj->Type() == ArticulatedObjectType))
        {
            ...
            simobj->GetCollisionCache(cachev, cachew,
                                      -1); //retrieve the collision cache of the physical object.
        ...
        if (simobj && simobj->Type() == ArticulatedObjectType) {
            ...
            for (int i = 0; i < vcma->GetSize(); i++) {
                ...
                if (vcm->GetIndex() == inIndex) {
                    ...
                    if (cachev.DotProduct(cachev) >
                        thresholdfactor2 * currentv.DotProduct(currentv)) {
        ...
        if (object->Type() != FlexibleObjectType) {
            if (inSimState->GetControl() == simSimulationCtrl) {
                if (object->IsAtRest() && inIndex <= 0) {
                    ...
                    if (v.DotProduct(v) <= Sqr(2.0f * object->GetMinimumLinSpeed()) &&
                        w.DotProduct(w) <= Sqr(2.0f * object->GetMinimumAngSpeed())) {
                        object->ResetCache();
                        return Solving_Aborted;
                    }
                }
