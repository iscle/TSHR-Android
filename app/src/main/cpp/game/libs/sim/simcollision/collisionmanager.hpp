...
namespace sim {
    ...
    public:
        ...
        GetImpulseBasedCollisionSolver() { return &mImpulseBasedCollisionSolver; }
        ...
        TArray <Collision> *
        GetCollisionList(int inArea = 0) { return &mAreaArray[inArea].mCollisionList; }

        TList<CollisionObject *> *
        GetCollisionObjectList(int inArea = 0) { return &mAreaArray[inArea].mObjectList; }

        TArray <CollisionObjectPair> *
        GetCollisionObjectPairList(int inArea = 0) { return &mAreaArray[inArea].mPairList; }
        ...
        bool DetectRayIntersection(TArray <RayIntersectionInfo> &inList, const rmt::Vector &p0,
                                   const rmt::Vector &p1, bool inStaticObjectOnly = false,
                                   int inArea = 0);
        ...
        bool
        DetectApproxRayIntersection(ApproxRayIntersectionInfo &info, const rmt::Vector &sourceRay,
                                    const rmt::Vector &endRay, bool inStaticObjectOnly = false,
                                    int inArea = 0);

        void FindClosestPointOnObject(const rmt::Vector &inPos, CollisionObject *inObject,
                                      rmt::Vector &outNormal, float &outDist) {
            this->FindClosestPointOnVolume(inPos, inObject->GetCollisionVolume(), outNormal,
                                           outDist);
        }

        void FindClosestPointOnVolume(const rmt::Vector &inPos, CollisionVolume *inVolume,
                                      rmt::Vector &outNormal, float &outDist);
