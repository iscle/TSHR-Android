...
namespace sim {
    ...
    void CollisionVolume::DeOptimize() {
        switch (mType) {
            ...
            case CylinderVolumeType:
                ((CylinderVolume * )
                this)->mAxisOrientation = VolAxisNotOriented;
                break;
            case WallVolumeType:
                ((WallVolume * )
                this)->mAxisOrientation = VolAxisNotOriented;
    ...
    void CollisionVolume::UpdatePos() {
        if (mCollisionObject != NULL) {
            ...
            if (mType != BBoxVolumeType &&
                !(mCollisionObject->IsManualUpdate()))  // new, greg - I think this is correct
            {
...
//
    ...
    CylinderVolume::CylinderVolume(const Vector &centre, const Vector &o, float len, float r,
                                   bool flatend)
            : CollisionVolume(),
              mAxis(o),
              mLength(len),
...
// OBBoxVolume class
    void
    OBBoxVolume::Set(const Vector &center, const Vector &o0, const Vector &o1, const Vector &o2,
    ...
    void OBBoxVolume::UpdateBBox() {
        if (mAxisOrientation == VolAxisNotOriented) {
            mBoxSize.x = mLength[0] * Fabs(mAxis[0].x) + mLength[1] * Fabs(mAxis[1].x) +
                         mLength[2] * Fabs(mAxis[2].x);
            mBoxSize.y = mLength[0] * Fabs(mAxis[0].y) + mLength[1] * Fabs(mAxis[1].y) +
                         mLength[2] * Fabs(mAxis[2].y);
            mBoxSize.z = mLength[0] * Fabs(mAxis[0].z) + mLength[1] * Fabs(mAxis[1].z) +
                         mLength[2] * Fabs(mAxis[2].z);
        } else {
        ...
        if (mVisible) {
            if (mSubVolumeList != NULL && mSubVolumeList->GetSize() > 0) {
                ...
                if (mCollisionObject && mCollisionObject->GetCollisionVolume() == this) {
                    mSphereRadius = 1.732f * Max(mBoxSize.x, Max(mBoxSize.y,
                                                                 mBoxSize.z)); // sqrt(3)*longest edge, worst case
                }
            }
        }
    }
...
*/
    ...
    void CylinderVolume::InertiaSelf(SymMatrix &m) const {
        ...
        if (mFlatEnd)
            ...
            h = 2.0f * (mLength + mCylinderRadius * 2.0f /
                                  3.0f); // approx. of an height to include the capsule's half spheres
