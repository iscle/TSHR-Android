...
namespace sim {
    ...
    public:
        ...
        const rmt::Matrix &GetTransform(int objref) const {
            return mSimState->GetTransform(objref);
        };
        ...
        CollisionAnalyserPossibleEvent
        PossibleCollisionEvents() { return mPossibleCollisionEvents; }

        void SetPossibleCollisionEvents(
                CollisionAnalyserPossibleEvent in_val) { mPossibleCollisionEvents = in_val; }

        int GetNumJoint() const { return mNumJoint; }

        tName mStringData;                           // public string/uid for the user

        float RestCoeff() const {
            rAssert(mPhysicsProperties);
            return mPhysicsProperties->RestCoeff();
        }

        float TangRestCoeff() const {
            rAssert(mPhysicsProperties);
            return mPhysicsProperties->TangRestCoeff();
        }

        float FrictCoeff() const {
            rAssert(mPhysicsProperties);
            return mPhysicsProperties->FrictCoeff();
        }
    ...
    private:
        ...
        CollisionObject &operator=(CollisionObject &inVolume) {
            rAssert(false);
            return *this;
        }
