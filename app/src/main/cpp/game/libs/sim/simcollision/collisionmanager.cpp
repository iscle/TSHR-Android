...
namespace sim {
    ...
    void
    CollisionManager::SetCollisionManagerAttributes(CollisionManagerAttributes inCMAttributes) {
    ...
    void
    CollisionManager::RemovePair(CollisionObject *inObjectA, CollisionObject *inObjectB, int area) {
        rAssert((area >= 0) && (area < mNbArea));
        mAreaArray[area].RemovePair(inObjectA, inObjectB);
    }

    void
    CollisionManager::AddPair(CollisionObject *inObjectA, CollisionObject *inObjectB, int area) {
        rAssert((area >= 0) && (area < mNbArea));
        mAreaArray[area].AddPair(inObjectA, inObjectB);
    }

    void
    CollisionManager::DetectCollision(int area, float inDt, float gameTime, const bool printOut) {
        if (mAreaArray[area].mActivated) {
            ...
            mCollisionDetector.DetectCollision(&mAreaArray[area].mPairList,
                                               &mAreaArray[area].mCollisionList, inDt, printOut);
            ...
            for (unsigned int i = 0; i < objectListSize; ++i) {
                ...
                if (object->GetSelfCollisionEnabled()) {
                    for (int j = 0; j < object->GetSelfCollisionList().GetSize(); j++) {
                        ...
                        mCollisionDetector.DetectCollision(sc.mCollisionVolume1,
                                                           sc.mCollisionVolume2,
                                                           &mAreaArray[area].mCollisionList);
                    }
                }
            }
    ...
    void CollisionManager::SolveCollision(int area, float inDt, float gameTime) {
        if (mAreaArray[area].mActivated) {
            ...
            mImpulseBasedCollisionSolver.SolveCollisions(&mAreaArray[area].mCollisionList, inDt,
                                                         gameTime);
    ...
    void
    CollisionManager::UpdateSelfCollision(CollisionObject *inObject, float inDt, float gameTime) {
        if (inObject->GetSelfCollisionEnabled()) {
            ...
            for (int j = 0; j < inObject->GetSelfCollisionList().GetSize(); j++) {
                ...
                sc.mCollisionVolume1->UpdateAll();
                sc.mCollisionVolume2->UpdateAll();
                mCollisionDetector.DetectCollision(sc.mCollisionVolume1, sc.mCollisionVolume2,
                                                   &list);
            }
            if (list.GetSize() > 0)
    ...
    bool CollisionManager::DetectRayIntersection(TArray <RayIntersectionInfo> &inList,
        ...
        if (RayIntersectionInfo::ReturnClosestOnly() && inList.GetSize() > 0) {
            ...
            if (info.mCollisionObject->GetRayCastingEnabled() &&
                (!inStaticObjectOnly || info.mCollisionObject->IsStatic()) &&
        ...
        if (inArea == -1) {
            for (int i = 0; i < mNbArea; ++i) {
                mAreaArray[i].DetectRayIntersection(inList, sourceOfRay, ray, sray, rayLen,
                                                    inStaticObjectOnly);
            }
        } else if (inArea >= 0 && inArea < mNbArea) {
            mAreaArray[inArea].DetectRayIntersection(inList, sourceOfRay, ray, sray, rayLen,
                                                     inStaticObjectOnly);
        }
    ...
    bool CollisionManager::DetectApproxRayIntersection(ApproxRayIntersectionInfo &info,
                                                       const rmt::Vector &sourceOfRay,
                                                       const rmt::Vector &endOfRay,
                                                       bool inStaticObjectOnly, int inArea) {
        SimStats::StartRCasSimulation();
    ...
    void
    CollisionManager::FindClosestPointOnVolume(const rmt::Vector &inPos, CollisionVolume *inVolume,
...
//
    ...
    void
    CollisionManager::Area::RemovePair(CollisionObject *inObjectA, CollisionObject *inObjectB) {
    ...
    void CollisionManager::Area::PrepareForCollisionDetection(
            CollisionManagerAttributes inCMAttributes) {
        ClearCollisions();
        ...
        if (inCMAttributes & CM_DetectAll) {
            for (i = 0; i < mPairList.GetSize(); i++) {
                ...
                if (!mPairList[i].mObjectA->IsStatic() &&
                    !mPairList[i].mObjectA->IsManualUpdate()) {
                    mPairList[i].mObjectA->Relocated();
                }
                if (!mPairList[i].mObjectB->IsStatic() &&
                    !mPairList[i].mObjectB->IsManualUpdate()) {
            ...
            if (sUpdateSimStateSTBC) {
                ...
                for (i = 0; i < mPairList.GetSize(); i++) {
                    if (mPairList[i].mNeedsUpdate) {
                        ...
                        simState->SafeTimeBeforeCollision() = rmt::Min(
                                simState->SafeTimeBeforeCollision(),
                                mPairList[i].mSafeTimeBeforeCollision);
