...
// returns true if position is in box
    inline bool PointInBox(const rmt::Vector &position, const rmt::Vector &size) {
        return (rmt::Fabs(position.x) <= size.x) && (rmt::Fabs(position.y) <= size.y) &&
               (rmt::Fabs(position.z) <= size.z);
    }
...
// ds/de is ray source/end location in box frame
    ...
    inline float
    TestRayInBBox(const rmt::Vector &source, const rmt::Vector &sray, const rmt::Vector &bsize) {
        ...
        if (sray.x * source.x < 0) {
            ...
            if (tmpt >= 0.0f && tmpt <= 1.0f) {
                if (rmt::Fabs(sray.y * tmpt + source.y) < bsize.y &&
                    rmt::Fabs(sray.z * tmpt + source.z) < bsize.z) {
                    t = tmpt;
                }
            }
        }
        if (sray.y * source.y < 0) {
            ...
            if (tmpt >= 0.0f && tmpt <= 1.0f) {
                if (rmt::Fabs(sray.x * tmpt + source.x) < bsize.x &&
                    rmt::Fabs(sray.z * tmpt + source.z) < bsize.z) {
                    t = rmt::Min(t, tmpt);
                }
            }
        ...
        if (sray.z * source.z < 0) {
            ...
            if (tmpt >= 0.0f && tmpt <= 1.0f) {
                if (rmt::Fabs(sray.x * tmpt + source.x) < bsize.x &&
                    rmt::Fabs(sray.y * tmpt + source.y) < bsize.y) {
                    t = rmt::Min(t, tmpt);
                }
            }
        }
    ...
    bool RayIntersectSphere(const rmt::Vector &source,
        ...
        if (t2 < radius2) {
            ...
            t.ScaleAdd(-crossLen,
                       ray); // closest intersection point with sphere for sphere at origin
...
*/
    bool RayIntersectVolume(const rmt::Vector &sourceOfRay,
        ...
        if (info.mCollisionVolume->Type() == BBoxVolumeType) {
            float value;
            bool isRootVolume = info.mCollisionVolume->GetCollisionObject()->GetCollisionVolume() ==
                                info.mCollisionVolume;
        ...
        } else if (info.mCollisionVolume->Type() == OBBoxVolumeType &&
                   info.sMethod == RayIntersectionVolume) {
            float value;
        ...
        } else if (info.mCollisionVolume->Type() == CylinderVolumeType &&
                   info.sMethod == RayIntersectionVolume) {
        ...
        } else if ((info.mCollisionVolume->Type() == SphereVolumeType &&
                    info.sMethod == RayIntersectionVolume) ||
                   info.sMethod == RayIntersectionBSphere) {
            ret = RayIntersectSphere(source,
                                     ray,
                                     rayLen,
                                     info.mCollisionVolume->mSphereRadius + info.sRayThickness,
    ...
    bool ApproxRayIntersectVolume(const rmt::Vector &sourceOfRay,
        ...
        } else if (inVolume->Type() == CylinderVolumeType &&
                   info.mMethod == RayIntersectionVolume) {
...
//
    ...
    inline void
    FindBoxCoordAndInOutBool(float point, float sidelength, float &coordinate, bool &inout) {
        if (point < -sidelength) {
    ...
    CollisionVolume *FindClosestPointOnVolume(const rmt::Vector &inPos, CollisionVolume *inVolume,
        ...
        switch (inVolume->Type()) {
            ...
            case BBoxVolumeType: {
                ...
                bool firstTime = true;
                for (int i = 0; i < inVolume->SubVolumeList()->GetSize(); i++) {
                    CollisionVolume *pOutSubVolume = FindClosestPointOnVolume(inPos,
...
//
    ...
    void RayIntersectionInfo::Add(TArray <RayIntersectionInfo> &inList, RayIntersectionInfo &info,
                                  const rmt::Vector &ray, rmt::Vector &sray, float &rayLen) {
