...
namespace sim {
    ...
    void CollisionObject::SetIsStatic(bool inIsStatic) {
        rAssert(!(CollisionManager::GetCurrentInstance() &&
                  CollisionManager::GetCurrentInstance()->Contains(this)));
    ...
    void CollisionObject::SetCollideWithStatic(bool inCollideWithStatic) {
        ...
        if (CollisionManager::GetCurrentInstance()) {
            for (int i = 0; i < CollisionManager::GetCurrentInstance()->NbArea(); i++) {
                if (CollisionManager::GetCurrentInstance()->GetCollisionObjectList(i)->Contains(
                        this)) {
        ...
        if (mCollisionVolumeOwner.mNumOwner > 0) {
            ...
            for (int i = 0; i < mCollisionVolumeOwner.mNumOwner; i++) {
                ...
                for (int j = 0; j < mCollisionVolumeOwner.mOwnerList[i].GetSize(); j++) {
                    if (inCompositeDrawable->FindNode(
                            mCollisionVolumeOwner.mOwnerList[i][j])->IsVisible())
                        ownerVis = true;
                }
    ...
    void
    CollisionObject::UpdateVisibilityByJoint(CollisionVolume *inVol, int inJoint, bool inVisible) {
    ...
    void CollisionObject::SetSelfCollision(SelfCollision &inSelfColl) {
        inSelfColl.mCollisionVolume1 = mCollisionVolume->GetSubCollisionVolume(inSelfColl.mIndex1,
                                                                               inSelfColl.mSelf1);
        inSelfColl.mCollisionVolume2 = mCollisionVolume->GetSubCollisionVolume(inSelfColl.mIndex2,
                                                                               inSelfColl.mSelf2);
...
//
    ...
    CollisionObject::CollisionVolumeOwner &CollisionObject::CollisionVolumeOwner::operator=(
            const CollisionObject::CollisionVolumeOwner &inCollisionVolumeOwner) {
        ...
        while (f->ChunksRemaining()) {
            ...
            switch (f->GetCurrentID()) {
                case Simulation::Collision::OWNER: {
                    ...
                    if (numNames > 0) {
                        newCollisionObject->mCollisionVolumeOwner.mOwnerList[currentOwner].ResizeArray(
                                numNames);
                        while (f->ChunksRemaining()) {
                            f->BeginChunk();
                            switch (f->GetCurrentID()) {
                                case Simulation::Collision::OWNERNAME: {
                                    ...
                                    newCollisionObject->mCollisionVolumeOwner.mOwnerList[currentOwner].NewElem() = tEntity::MakeUID(
                                            newName);
                                    break;
                                }
                                default:
                        ...
                        rAssert(newCollisionObject->mCollisionVolumeOwner.mOwnerList[currentOwner].GetSize() ==
                                numNames);
                    }
                    currentOwner++;
                    break;
    ...
    CollisionVolume *CollisionObjectLoader::LoadCollisionVolume(tChunkFile *file) {
        ...
        switch (file->GetCurrentID()) {
            case Simulation::Collision::SPHERE: {
                ...
                newCollisionVolume = (CollisionVolume * )
                new SphereVolume(p, radius);
                break;
            }
            case Simulation::Collision::CYLINDER: {
                ...
                newCollisionVolume = (CollisionVolume * )
                new CylinderVolume(p, o, length, radius, flatEnd);
                break;
            }
            case Simulation::Collision::OBBOX: {
                ...
                newCollisionVolume = (CollisionVolume * )
                new OBBoxVolume(p, o0, o1, o2, l0, l1, l2);
                break;
            }
