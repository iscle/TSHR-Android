...
namespace sim {
    ...
    typedef enum {
        NO_INTERSECTION = -1, ALL_INSIDE = 0, ONE_POINT = 1, TWO_POINTS = 2, THREE_POINTS = 3
    } GEOM_CODE;
    ...
    protected:
        ...
        bool OBBoxSepPlane(OBBoxVolume *volA,
                           OBBoxVolume *volB); // return true if a sperarating plane exists
        void
        OBBoxV(OBBoxVolume *volA, OBBoxVolume *volB);        // coll between box a and vertices of b
        void
        OBBoxEE(OBBoxVolume *volA, OBBoxVolume *volB);       // coll between edges of bax a and b
        bool
        EdgeEdge(bool &hasCollided, rmt::Vector &pa, rmt::Vector &oa, float la, rmt::Vector &pb,
                 rmt::Vector &ob, float lb, rmt::Vector &N, float nn, float dist = 0,
                 CollisionVolume *volA = NULL, CollisionVolume *volB = NULL);

        bool
        PointBoxColl(OBBoxVolume *El, rmt::Vector &pp, rmt::Vector *pa, rmt::Vector *n, float &dist,
                     float radius = 0, bool getpos = false);
        ...
        void
        OBBoxBorderCollMove(OBBoxVolume *El, rmt::Vector &pos, float dist2, float dist3, int i);
        ...
        void CircleEdge(CollisionVolume *volA, rmt::Vector &pa, rmt::Vector &oa, float la,
                        CylinderVolume *volB, float nft, float no, bool use_all_inside);
        ...
        bool CylCylEnd(CylinderVolume *volA, CylinderVolume *volB, bool testSphereA = true,
                       bool addColl = true);

        bool CylSphereColl(CylinderVolume *volA, const rmt::Vector &Pb, float rb, rmt::Vector &N,
                           float &dist, rmt::Vector *ppa = NULL, rmt::Vector *ppb = NULL,
                           bool testSphereA = true);

        bool PlanePlaneIntersection(rmt::Vector &in_c1, rmt::Vector &in_c2, rmt::Vector &in_n1,
                                    rmt::Vector &in_n2, rmt::Vector *o_pt, rmt::Vector *io_ns,
                                    bool);

        GEOM_CODE CircleCircleIntersection(const rmt::Vector &in_c1, const rmt::Vector &in_c2,
                                           const rmt::Vector &in_N,
        ...
        void AddCollision(CollisionVolume *volA, const rmt::Vector &pa, CollisionVolume *volB,
                          const rmt::Vector &pb, float dist, const rmt::Vector &N);
