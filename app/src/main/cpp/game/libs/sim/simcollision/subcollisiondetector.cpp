...
// test for collision between the faces of volA with the Corner of volB
        ...
        for (j = 0; j < 4; j++) {
            ...
            for (int ii = 0; ii < 3; ii++) {
                ...
                if (dist <= mCollisionDistance &&
                    (dist > -2 * volA->mLength[i] || nt[i] * pDotAxis < 0.0f)) {
                    ...
                    if (dist2 <= (dist > 0 ? mMinCollisionDistance : 0)) {
                        ...
                        if (dist3 <= (dist > 0 ? mMinCollisionDistance : 0)) {
                            ...
                            // we have a collision...
                            volB->Corner(p,
                                         j);                  // mPosition +/- p correspond to two opposite corner of the OBBox
                            ...
                            if (mCollisionDistance == mMinCollisionDistance &&
                                ...
                                (Sqr(dist) + Sqr(dist2 > 0 ? dist2 : 0) +
                                 Sqr(dist3 > 0 ? dist3 : 0)) > MILLI_EPS) {
    ...
    void SubCollisionDetector::OBBoxBorderCollMove(OBBoxVolume *vol, Vector &pos, float dist2,
                                                   float dist3, int i) {
        Vector tmp;
        float sgn;
        int ni = NXTI(i);
    ...
    void SubCollisionDetector::OBBoxEE(OBBoxVolume *volA, OBBoxVolume *volB) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                if (nn >
                    VERY_SMALL) // else edges are parallel... the vertex-edges collision are handled throw OBBoxV
                {
                    nt = mT.DotProduct(N);
                    ...
                    if (Fabs(nt) > VERY_SMALL) // else no collision possible between these edges
                    {
                        ...
                        for (kl = 3; kl >= 0; kl--) {
                            ...
                            // Strict test
                            if (Fabs(npa[ind]) > VERY_SMALL && Fabs(npb[ind]) > VERY_SMALL &&
                                dist[ind] < mCollisionDistance && dist[ind] >
                                                                  -mindist) // else, no collision possible between these axes
                            {
                                if (nt * npa[ind] < 0)
                                ...
                                EdgeEdge(hasCollided, pa[ind], volA->mAxis[i], volA->mLength[i],
                                         pb[ind], volB->mAxis[j], volB->mLength[j], N, nn,
                                         dist[ind], volA, volB);
                                if (mNewColl > 3)
                                    break;
                            }
        ...
        if (!hasCollided &&
            ((volA->GetCollisionObject()->GetSimState()->mAIRefIndex == 1 &&
              (volB->GetCollisionObject()->GetSimState()->mAIRefIndex == 131072 ||
               volB->GetCollisionObject()->GetSimState()->mAIRefIndex == 262146))
             ||
             (volB->GetCollisionObject()->GetSimState()->mAIRefIndex == 1 &&
              (volA->GetCollisionObject()->GetSimState()->mAIRefIndex == 131072 ||
                ...
                for (int j = 0; j < 3; j++) {
                        ...
                        VERY_SMALL) // else edges are parallel... the vertex-edges collision are handled throw OBBoxV
                    {
                        ...
                        if (Fabs(nt) > VERY_SMALL) // else no collision possible between these edges
                        {
                            ...
                            float invn = 1.0f / nn;
                            nt = nt *
                                 invn; // distance between box center along N, the collision normal
                            // there is 4 parallel edges in a cube, we do 2 at a time on each box
                            int ii[4];
                            Vector pa[4], pb[4];
                            float npa[4], npb[4], dist[4];
                            ...
                            for (kl = 3; kl >= 0; kl--) {
                                ...
                                if (Fabs(npa[ind]) > VERY_SMALL && Fabs(npb[ind]) > VERY_SMALL &&
                                    dist[ind] <
                                    mCollisionDistance) // else, no collision possible between these axes
                                {
                                    if (nt * npa[ind] < 0)
                                        pa[ind].Scale(-1.0f);  // identify the correct side on a
                                    if (-nt * npb[ind] < 0)
...
// detect collision between the two given edges
    bool
    SubCollisionDetector::EdgeEdge(bool &hasCollided, Vector &pa, Vector &oa, float la, Vector &pb,
                                   Vector &ob, float lb, Vector &N, float nn,
        ...
        if (Fabs(kb) <= 1.0f) // or the edges projection on the perpendicular plane don't intersect
        {
            ...
            ka = lb * tmp.DotProduct(ob) *
                 nn;//This is the same thing as projecting dp along obx(oaxob)
            if (Fabs(ka) <=
                1.0f) // or the edges projection on the perpendicular plane don't intersect
            {
                    ...
                    if (d3 <= borderDist) {
                        ...
                        if (dist < mCollisionDistance) {
                            if (getpos) {
                                ...
                                *pa = pp;
                                pa->ScaleAdd(sign * (dst[i] + radius),
                                             vol->mAxis[i]); // position on the face
...
// test for collision between the faces of a box with the cylinder
    void SubCollisionDetector::OBBoxCylF(OBBoxVolume *volA, CylinderVolume *volB) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (dist > 0 && dist < volB->mSphereRadius +
                                   mCollisionDistance) // else, no collision possible on these faces or we're deep into it
            {
                ...
                while (!completed) {
                    Vector pb, tmp1, tmp2;
                    pb.Scale(volB->mLength * sgna * Sign(dotAxis),
                             volB->mAxis); // vector from mPosition to the center of the circle
                    ...
                    if (dist <= mCollisionDistance)// && dist> -volB->mCylinderRadius)
                    {
                        ...
                        if (dist2 <= 0) {
                            ni = NXTI(ni);
                            float dist3 = Fabs(nt[ni] - pb.DotProduct(volA->mAxis[ni])) -
                                          volA->mLength[ni];
                            if (dist3 <= 0) {
                                // we have a collision...
                                pb.Add(volB->mPosition);
                    ...
                    // get to rest faster
                    if (firstSide &&
                        dist + 2.0f * volB->mLength * Fabs(dotAxis) <= mCollisionDistance) {
                        sgna = -sgna;
...
// collision of the box vertices with the cylinder
    void SubCollisionDetector::OBBoxCylV(OBBoxVolume *volA, CylinderVolume *volB) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            if (Fabs(dist) > VERY_SMALL) {
                ...
                dp.Add(mT,
                       pa); // mT = volA->mPosition - volB->mPosition => dp is the vector from the corner to volB->mPosition
                ...
                if (dist3 - rad <= mCollisionDistance) // longitudinal distance
                {
                    ...
                    if (dist <= mCollisionDistance) // radial distance
                    {
                        if (!volB->mFlatEnd && Fabs(dist2) >= volB->mLength) {
                            ...
                            if (volB->mFlatEnd &&
                                dist3 > -mCollisionDistance) // collision with the cyl end
                            {
                                ...
                                dist = dist3;
                                       ...
                                       -volB->mCylinderRadius) // else the vertex is exactly on the axis, don't know the collision orientation, could use the speed...
                            {
                                collide = true; // we have a collision...
                                // collision with the side
                                mNormal.Scale(-1.0f / (dist +
                                                       volB->mCylinderRadius)); // dist+volB->mCylinderRadius is the norme of mNormal
                                pb.Scale(dist, mNormal);
                                pb.Add(pa);
                                if (dist3 > 0) {
    ...
    void SubCollisionDetector::OBBoxCylEE(OBBoxVolume *volA, CylinderVolume *volB) {
        ...
        for (int i = 0; i < 3; i++) {
                ...
                VERY_SMALL) // else edges are parallel... the vertex-edges collision are handled throw OBBoxV
            {
                ...
                if (Fabs(nt) > VERY_SMALL) // else no collision possible between these edges
                {
                    ...
                    float invn = 1.0f / (nn);
                    nt = nt *
                         invn; // distance between box center and cyl axis along N, the collision normal
                    ...
                    for (int k = 0; k < 2; k++) {
                        ...
                        // strict test
                        if (Fabs(npa) > VERY_SMALL /*&& Fabs(npb)> VERY_SMALL*/ &&
                            dist < mCollisionDistance &&
                            dist > -mindist) // else, no collision possible between these axes
                        {
                            if (nt * npa < 0)
                                pa.Scale(-1.0f);  // identify the correct side on a
                            pb.Scale((nt > 0 ? -volB->mCylinderRadius * invn :
                                      volB->mCylinderRadius * invn),
                                     N);   // identify the correct side on b

                            EdgeEdge(hasCollided, pa, volA->mAxis[i], volA->mLength[i], pb,
                                     volB->mAxis, volB->mLength, N, nn, dist, volA, volB);
                        }
                    }
                }
            }
        ...
        if (!hasCollided &&
            ...
            (
                    (!((volB->GetCollisionObject()->GetSimState()->mAIRefIndex == 131073) ||
                       (volB->GetCollisionObject()->GetSimState()->mAIRefIndex == 131074)))

                    ||

                    // we're a vehicle moving at non-trivial speed
                    (volA->GetCollisionObject()->GetSimState()->mAIRefIndex == 1 &&
                     (volA->GetCollisionObject()->GetSimState()->GetLinearVelocity()).MagnitudeSqr() >
                     25.0f)
            )
                    ...
                    VERY_SMALL) // else edges are parallel... the vertex-edges collision are handled throw OBBoxV
                {
                    ...
                    if (Fabs(nt) > VERY_SMALL) // else no collision possible between these edges
                    {
                        ...
                        for (int k = 0; k < 2; k++) {
                            ...
                            // lax test
                            if (Fabs(npa) > VERY_SMALL /*&& Fabs(npb)> VERY_SMALL*/ && dist <
                                                                                       mCollisionDistance)//  && dist> -mindist) // else, no collision possible between these axes
                            {
                                if (nt * npa < 0)
                                    pa.Scale(-1.0f);  // identify the correct side on a
                                pb.Scale((nt > 0 ? -volB->mCylinderRadius * invn :
    ...
    bool SubCollisionDetector::OBBoxCylSepPlane(OBBoxVolume *volA, CylinderVolume *volB) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            projection = a * volB->mLength + Sqrt(1.0f - Sqr(a)) * volB->mCylinderRadius +
                         volA->mLength[i];
            if (Fabs(mT.DotProduct(volA->mAxis[i])) > mCollisionDistance + projection)
                return true;
        }
    ...
    void SubCollisionDetector::CircleEdge(CollisionVolume *volA, Vector &pa, Vector &oa, float la,
                                          CylinderVolume *volB, float nft, float no,
                                          bool use_all_inside) {
        ...
        if (interp <= mCollisionDistance) {
            ...
            if (fOoa >= 1.0f - MILLI_EPS &&
                !all_inside) {  // the edge is parallel to the circle's normal.
                pb.Add(pa, mT);
                ...
                if (dn <= mCollisionDistance) {
                    ...
                    if (nr <= Sqr(volB->mCylinderRadius + mCollisionDistance)) {  // collision
                       ...
                       MILLI_EPS)// The numerical precision make difficult to trap this case. Here, we devide by la but this is not correct.
            {
                ...
                if (dn <= mCollisionDistance) {
                    ...
                    if (dr <= mCollisionDistance + volB->mCylinderRadius) {
                        ...
                        if (sign_t1t2 < 0 || Fabs(t1oa) <= da || Fabs(t2oa) <= da) {
                            ...
                            if ((dr > dn || Fabs(dn / volB->mLength) > 0.1f) &&
                                Fabs(da - volB->mCylinderRadius) >=
                                MICRO_EPS) //Rembering that dr and dn are negative.
                            {  //Use the Normal along a radius.
                                if (sign_t1t2 > 0) {
                                    ...
                                    pb.ScaleAdd(volB->mPosition, Sign(pbfo) * volB->mLength,
                                                volB->mAxis);
                                    pb.ScaleAdd(volB->mCylinderRadius, mNormal);
                                }
                                AddCollision(volA, pa, volB, pb, dr, mNormal);
                            } else {  //Use the Normal along a cylinder's axis.
                                if (Fabs(pboa) <= da) {
                ...
                for (int i = 0; i < 2; i++, la *= -1) {
                    ...
                    if (dp < mCollisionDistance) {
                        rAssert(dn <=
                                mCollisionDistance); //always true because of the "all inside" condition.
                        //collision
                        sign = Sign(tn);
                        pa.Add(volB->mPosition, t1);
                        if (dn > dp) {//Use the normal along the cylinder's axis.
                            mNormal = volB->mAxis;
                            mNormal.Scale(sign);
                            pbc.ScaleAdd(pa, sign * Fabs(dn), volB->mAxis);
                        } else {//Use the normal along a circle's radius.
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    if (p < Sqr(la)) // This length must be smaller than the length of the edge.
                    {
                        ...
                        mNormal.ScaleAdd(pa, -Sign(pa.DotProduct(volB->mAxis)) * volB->mLength,
                                         volB->mAxis);
                        ...
                        if (dr < Sqr(volB->mCylinderRadius + mCollisionDistance)) {
                            ...
                            Vector l_pb;
                            if (c2 <
                                .5f) // Instead of chosing the normal type with depth comparison,
                            {              // use the following case when the edge is more parallel than perpendicular to the circle.
                                ...
                                float oae2, s1, da;
                                oae2 = oa.DotProduct(
                                        e2); // Product oa.e2 guaranty to be positive and none zero.

                                rAssert(Sqr(volB->mCylinderRadius + mCollisionDistance) -
                                        Sqr(pbe1) > 0);
                                if (Sqr(volB->mCylinderRadius + mCollisionDistance) - Sqr(pbe1) <=
                                    0.0f)
                                    continue;

                                //Half length of the projected segment on the disk.
                                da = Sqrt(Sqr(volB->mCylinderRadius + mCollisionDistance) -
                                          Sqr(pbe1)); //Should not be zero.
...
// we then start we the less deep and so on
    void SubCollisionDetector::BoxSphereColl(OBBoxVolume *volA, CollisionVolume *volB, Vector &Pb,
                                             float rad) {
        Vector N, pa, pb = Pb;
        float dist;
...
// where the two cylinders have flat end.
    bool
    SubCollisionDetector::CylCylEnd(CylinderVolume *volA, CylinderVolume *volB, bool testSphereA,
                                    bool addColl) {
        Vector pb, pa, psb;
        Vector *ppa, *ppb;
        bool collide = false;
        ...
        if (!volA->mFlatEnd && !volB->mFlatEnd) {
            ...
            for (i = 0, la = volA->mLength; i < 2; i++, la *= -1.0f) {
                for (j = 0, lb = volB->mLength; j < 2; j++, lb *= -1.0f) {
                    ...
                    dist = mNormal.NormalizeSafe();
                    if (dist <=
                        volA->mCylinderRadius + volB->mCylinderRadius + mCollisionDistance) {
            ...
            for (i = 0, la = volA->mLength; i < 2; i++, la *= -1.0f) {
                for (j = 0, lb = volB->mLength; j < 2; j++, lb *= -1.0f) {
                    ...
                    cb.ScaleAdd(volB->mPosition, lb,
                                volB->mAxis);//world position of the other circle.
                    mT.Sub(ca, cb); //from circle's center b to circle's center a
                    if (la * lb * fnfn > 0)
                        continue; //cercle's outward normal are in the same direction.

                    if (e1_m <
                        mCollisionDistance / Max(volA->mCylinderRadius, volB->mCylinderRadius)) {
                        ...
                        if (depth < mCollisionDistance &&
                            -depth < Min(volA->mLength, volB->mLength) * 2.0f) {
                            //Look for an intersection.
                            GEOM_CODE res;
                            cb.ScaleAdd(depth,
                                        mNormal);//this bring circle b in the same plane as circle a.
                            Vector pa2;
                            if (volA->mCylinderRadius < volB->mCylinderRadius)
                                res = CircleCircleIntersection(ca, cb, volA->mAxis,
                                                               volA->mCylinderRadius,
                                                               volB->mCylinderRadius, true, pa,
                                                               pa2);
                            else
                                res = CircleCircleIntersection(cb, ca, volB->mAxis,
                                                               volB->mCylinderRadius,
                                                               volA->mCylinderRadius, true, pa,
                                                               pa2);
                        ...
                        PlanePlaneIntersection(ca, cb, volA->mAxis, volB->mAxis, &pt, &e1_e1,
                                               false);
                        ...
                        if (dp < mCollisionDistance)//else e1 is a separating axis
                        {
                            ...
                            res = CircleCircleIntersection(*cbp, *cap, Elbp->mAxis,
                                                           Elbp->mCylinderRadius,
                                                           Elap->mCylinderRadius * dp, true, pa,
                                                           pa2);
                            ...
                            if ((res == ONE_POINT || res == TWO_POINTS) &&
                                depth < mCollisionDistance && e1_m < 0.5f) {
                                if (addColl == false)
                                    return true;
                                pb.ScaleAdd(pa, -depth, mNormal);
                            ...
                            delta = Sqr(b) - 4 * a * c;
                            if (Fabs(a * c) <
                                MICRO_EPS)//From numerical receipes, suppose to be more accurate.
                            {
                                if (delta >= 0)
                                ...
                                if (q == 0.0f ||
                                    a == 0.0f) //otherwise e1p and e2p are perpendicular
                                {
                                    f1 = f2 = 0.0f;
            ...
            for (int i = 0; i < 2; i++, length *= -1) {
                ...
                if (CylSphereColl(l_Ela, psb, l_Elb->mCylinderRadius, mNormal, dist, ppa, ppb,
                                  testSphereA)) {
                    collide = true;
                    if (addColl) {
                        if (l_Ela == volA)
...
//This method returns true if the cylinder and the sphere collide.
    bool
    SubCollisionDetector::CylSphereColl(CylinderVolume *volA, const Vector &Pb, float rb, Vector &N,
                                        float &dist, Vector *ppa, Vector *ppb, bool testSphereA) {
        float dl = mT.DotProduct(
                volA->mAxis);  // projection on the axe of the dist between the object center
        float dr = rmt::Max(0.0f, dist - Sqr(dl));  // dr2, projection on the radius
        float rad = (volA->mFlatEnd ? 0 : volA->mCylinderRadius);
        bool collide = false;

        if (Fabs(dl) < volA->mLength + rb + mCollisionDistance + rad &&
            dr < Sqr(volA->mCylinderRadius + rb + mCollisionDistance)) {
            ...
            if (dr < 0.0f)
                return collide;
            dr = Sqrt(dr);
            if (volA->mFlatEnd && Fabs(dl) - volA->mLength > dr - rad &&
                dr < rad) // coll with the end face
            {
...
*/
    GEOM_CODE SubCollisionDetector::CircleCircleIntersection
        ...
        d = Sqrt(d);
        if (d + in_r1 < in_r2 + mMinCollisionDistance ||
            d + in_r2 < in_r1 + mMinCollisionDistance) {
            o_p1 = in_c1;
            return ALL_INSIDE;
        }
                                                                                                                            ...
                                                                                                                            void SubCollisionDetector::TestNormalIntegrity(Collision* c)
{
    ...
    */
    ...
    void SubCollisionDetector::AddCollision(CollisionVolume *volA, const Vector &pa,
                                            CollisionVolume *volB, const Vector &pb, float dist,
                                            const Vector &N) {
        ...
        if (mCollisionFilter && mNewColl > 0) {
            ...
            for (int i = l_nbColl - 1; i >= l_nbColl - mNewColl; i--) {
                ...
                if (identity) {
                    ...
                    if (identity == 1) {
                        if (newcoll.mIndexA != pColl->mIndexA ||
                            newcoll.mIndexB != pColl->mIndexB) {
                            continue;
                        }
                    } else {
                        if (newcoll.mIndexA != pColl->mIndexB ||
                            newcoll.mIndexB != pColl->mIndexA) {
                            continue;
                        }
                    }
                    ...
                    if (mCollisionFilter & FILTER_BY_POSITION) {
                        ...
                        if (l_dist1 < minDist && l_dist2 <
                                                 minDist)//TODO: Have a caracteristic length instead of an hard coded number.
                        {
