...
namespace sim {
    ...
    typedef enum {
        eFlexStretch1D_f = 1, eFlexStretch1D_d,
...
// condition.  This lets us differentiate easily.
    ...
    public:
        ...
        static bool ComputeStretch1DFromLambda(float inStretchFact, float inDampingFact, int inNbp,
                                               float inLength0, float inGravity, float &o_force,
                                               float &o_damping, float inMass = 1.0f);

        static bool ComputeBend1DFromKappa(float inCurvatureFact, float inDampingFact, int inNbp,
                                           float inLength0, float inGravity, float &o_force,
                                           float &o_damping, float inMass = 1.0f);
    ...
    public:
        ...
        inline void Insert(Condition *f) {
            f->mNextCond = mNextCond;
            mNextCond = f;
        }

        void RemoveNext();

        inline virtual bool RelateToParticle(const FlexParticle *p) {
            return (p == mPi || p == mPj);
        }
