...
namespace sim {
    ...
    Stretch2DCondition::Stretch2DCondition(FlexParticle *mPi, FlexParticle *mPj, FlexParticle *pkk,
                                           ConditionParams *p,
                                           float ddu1, float ddv1, float ddu2, float ddv2,
                                           bool inUsePartialDerivative)
            : Condition(mPi, mPj, p) {
        rAssert(false); // need to be fixed
        mUsePartialDerivative = inUsePartialDerivative;
    ...
    void
    Stretch2DCondition::Derivative(const float c0, const float L, const float d1, const float d2,
                                   float dCi[], float dCj[], float dCk[]) {
        if (c0 == 0) {
            ZeroVector(dCk);
            ZeroVector(dCj);
            ZeroVector(dCi);
    ...
    void Stretch2DCondition::Derivative2(float *x1, float *x2, const C2DVal &dC1p2, C2DVal &ddC,
                                         int ind1, int ind2) {
    }
    ...
    void Stretch2DCondition::BuildMatrix(SparseBlockMatrix *A, float *b, float dt) {
        ...
        */
        ...
        AddLinearVector(dCi.u, -mParams->Stretch2Dkf * C0.u, dCi.u, -mParams->Stretch2Dkd * Cdot.u,
                        fi);
        AddAddLinearVector(dCi.v, -mParams->Stretch2Dkf * C0.v, dCi.v,
                           -mParams->Stretch2Dkd * Cdot.v, fi);

        AddLinearVector(dCj.u, -mParams->Stretch2Dkf * C0.u, dCj.u, -mParams->Stretch2Dkd * Cdot.u,
                        fj);
        AddAddLinearVector(dCj.v, -mParams->Stretch2Dkf * C0.v, dCj.v,
                           -mParams->Stretch2Dkd * Cdot.v, fj);

        AddLinearVector(dCk.u, -mParams->Stretch2Dkf * C0.u, dCk.u, -mParams->Stretch2Dkd * Cdot.u,
                        fk);
        AddAddLinearVector(dCk.v, -mParams->Stretch2Dkf * C0.v, dCk.v,
                           -mParams->Stretch2Dkd * Cdot.v, fk);
...
//
    ...
    Shear2DCondition::Shear2DCondition(FlexParticle *pii, FlexParticle *pjj, FlexParticle *pkk,
                                       ConditionParams *p, float a0, bool inUsePartialDerivative)
            : Condition(pii, pjj, p),
              angle0(a0),
              mPk(pkk) {
        mUsePartialDerivative = inUsePartialDerivative;
...
//
    ...
    Bend2DCondition::Bend2DCondition(FlexParticle *pii, FlexParticle *pjj, FlexParticle *pkk,
                                     FlexParticle *pll, ConditionParams *p, float a0,
                                     bool inUsePartialDerivative)
            : Condition(pii, pjj, p),
              angle0(a0),
              mPk(pkk),
    ...
    void Bend2DCondition::BuildMatrix(SparseBlockMatrix *A, float *b, float dt) {
        ...
        // add the force add the damping in f
        float fi[3], fj[3], fk[3], fl[3], kkf = -mParams->Bend2Dkf * C0, kkd =
                -mParams->Bend2Dkd * Cdot;
...
//
    ...
    Wind2DCondition::Wind2DCondition(FlexParticle *pii, FlexParticle *pjj, FlexParticle *pkk,
                                     ConditionParams *p, Vector *w, float inarea)
            : Condition(pii, pjj, p),
              mPk(pkk),
              mWind(w),
              mArea(inarea) {
