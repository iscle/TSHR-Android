...
void FlexibleObject::StartCollision(const rmt::Vector &inPos, const Vector &inDir,
                                    const SimulatedObject *inObj, int index) {
...
bool FlexibleObject::AddImpulse(const rmt::Vector &inPos, const rmt::Vector &inImp,
                                const SymMatrix &inSymMatrix, SimulatedObject *otherObj,
                                int index) {
...
void FlexibleObject::GetMassMatrix(const rmt::Vector &inPos, SymMatrix &oM, SimulatedObject *iObj,
                                   int index) {
    oM.Identity();
    if (iObj) {
...
void FlexibleObject::ResolveInterpenetration(int in_index, float in_dist, const Vector &inN,
                                             Vector &o_imp, bool addconstrain,
    ...
    if (in_dist < 0 && l_part->mFree) {
        ...
        if (sdist * dd > mPsyst->mDeli) {
            dd = mPsyst->mDeli /
                 sdist; //Clamp the moving distance to a fraction of the particles distances.
        }
        Vector dx;
        dx.Scale(dd, inN);
...
bool FlexibleObject::UpdateConstraintTarget(const LocalTarget &in_target, Vector *o_VelocityTarget,
                                            SimTarget &io_target) const {
