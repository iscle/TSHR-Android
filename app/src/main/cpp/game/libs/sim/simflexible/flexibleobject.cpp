...
namespace sim {
    ...
    void FlexibleObject::StartCollision(const rmt::Vector &inPos, const Vector &inDir,
                                        const SimulatedObject *inObj, int inIndex, int) {
        rAssert(inIndex >= 0 && inIndex < mPsyst->mNbp);
        Vector dp;
    ...
    bool FlexibleObject::AddImpulse(const rmt::Vector &inPos, const rmt::Vector &inImp,
                                    const SymMatrix &inSymMatrix, SimulatedObject *, int index,
                                    int otherIndex) {
    ...
    void
    FlexibleObject::GetMassMatrix(const rmt::Vector &inPos, SymMatrix &oM, SimulatedObject *iObj,
                                  int index, int) {
        oM.Identity();
...
//
    ...
    void FlexibleObject::ResolveInterpenetration(int in_index, float &io_dist, const Vector &inN,
    ...
    bool
    FlexibleObject::UpdateConstraintTarget(const LocalTarget &in_target, Vector *o_VelocityTarget,
