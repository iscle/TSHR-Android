...
#define MAX_ITER_CG  20    //The maximum number of iteration in cg.
    ...
    public:
        ...
        inline bool Contains(const FlexParticlep el) const {
            return VoidPList::Contains(voidp(el));
        }
        ...
        inline FlexParticleList &operator+=(const FlexParticleList &list) {
            VoidPList::Add((VoidPList &) list);
            return *this;
        }

        inline FlexParticleList &operator=(const FlexParticleList &list) {
            VoidPList::Set((VoidPList &) list);
            return *this;
        }
    };
    ...
    public:
        ...
        float ExtMassParticle(int i) const {
            rAssert(i >= 0 && i < mNbp);
            return mExtDensity * mMass[i];
        }

        void SetExtDensity(float in_val) { mExtDensity = in_val; }

        void ScaleMassParticle(int i, float f) {
            rAssert(i >= 0 && i < mNbp);
            mMass[i] *= f;
        }
        ...
        virtual bool
        SetKappa(float inCurvatureFact, float inDampingFact, bool inUseLongSide = true);
        ...
        void AddAngularSpeedParticle(int index, const rmt::Vector &angularVelocity,
                                     const rmt::Vector &origin);
        ...
        virtual void BreakAt(int i) {
            rAssertMsg(false, "TestParticleSystem::BreakAt Not Implemented");
        }
