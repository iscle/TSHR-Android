...
//
    ...
    FlexJointModifier::FlexJointModifier(poser::Pose *inPose, FlexibleJointDefinition *inDef,
                                         int item)
        ...
        if (mUseRestingPos) {
            mNumFlexJointRestParticle =
                    mNumFlexJointParticle - mDefinitionItem->mFlexibleFixJointIndex.GetSize();
        ...
        for (i = 0; i < mDefinitionItem->mFlexibleJointIndex.GetSize(); i++) {
            ...
            mFlexJointParticle[i].mFix = mDefinitionItem->mFlexibleFixJointIndex.Contains(
                    currentPoseIndex);
            ...
            if (mUseRestingPos && !mFlexJointParticle[i].mFix) {
                ...
                mFlexJointParticle[i].mRestParticle->mParticleIndex =
                        mNumFlexJointParticle + currentRestParticle - 1;
        ...
        *mParticleSystem->mCparams = mParameters;
        ...
        for (i = 0; i < mDefinitionItem->mFlexibleJointConnection.GetSize(); i++) {
            ...
            ConditionParams *c = (mDefinitionItem->mFlexibleJointConnection[i].mOnBranch
                                  ? &mParameters : &mConnectionParameters);
        ...
        for (i = 0; i < mDefinitionItem->mFlexibleJointHinge.GetSize(); i++) {
            ...
            ((ParticleSystemManual *) mParticleSystem)->AddBending1D(i1, i2, i3, false,
                                                                     &mParameters);
        }
        ...
        if (!mParameters.GetParamSet()) {
            ...
            ((ParticleSystemManual *) mParticleSystem)->SetLengthEstimate(jointLength, i,
                                                                          &mParameters);
            ((ParticleSystemManual *) mParticleSystem)->SetLengthEstimate(jointLength, 2,
                                                                          &mConnectionParameters);
            ((ParticleSystemManual *) mParticleSystem)->SetLengthEstimate(0, 2,
                                                                          &mRestingPosParameters);
    ...
    void
    FlexJointModifier::SetPoseTransform(int flexParticleIndex, const rmt::Matrix &inTransform) {
    ...
    void
    FlexJointModifier::SetPoseTranslation(int flexParticleIndex, const rmt::Vector &inTranslation) {
        rAssert(!mFlexJointParticle[flexParticleIndex].mVirtual);
        poser::Joint *j = mPose->GetJoint(mFlexJointParticle[flexParticleIndex].mPoseIndex);
...
*/
        ...
        for (int i = 0; i < mNumFlexJointParticle; i++) {
            ...
            switch (mFlexJointParticle[i].mFlexibleJointConnection.GetSize()) {
                ...
                default: {
                    ...
                    Vector particlePos;
                    mParticleSystem->GetPosParticle(mFlexJointParticle[i].mParticleIndex,
                                                    particlePos);
    ...
    bool FlexJointModifier::UpdateRestingPosition() {
        ...
        if (sUseRootFilteredSpeed && mRootIndex >= 0) {
            ...
            Vector rootpos, previousrootpos, shiftedrootpos, teleportDisplacement(0.0f, 0.0f,
                                                                                  0.0f), freeDeltaSpeed;
            int fixcount = -1;
            for (int k = 0; k < mNumFlexJointParticle; k++) {
                if (mFlexJointParticle[k].mFix) {
                    ...
                    if (mNewRootSpeed[fixcount].x > 10.0f || mNewRootSpeed[fixcount].y > 10.0f ||
                        mNewRootSpeed[fixcount].z > 10.0f) {
                        mRootFilteredSpeed[fixcount].ScaleAdd(sFilterRootSpeedValue,
                                                              mNewRootSpeed[fixcount]);
                        ...
                        mRootFilteredSpeed[fixcount].Sub(mNewRootSpeed[fixcount],
                                                         mPreviousFilteredSpeed[fixcount]);

                        // new speed = old speed + kappa * delta speed
                        mRootFilteredSpeed[fixcount].ScaleAdd(mPreviousFilteredSpeed[fixcount],
                                                              sFilterRootSpeedValue,
                                                              mRootFilteredSpeed[fixcount]);
                        ...
                        freeDeltaSpeed.Sub(mPreviousFilteredSpeed[fixcount],
                                           mPreviousRootSpeed[fixcount]);
                    }
                ...
                else // free particle code
                {
                    ...
                    mParticleSystem->MoveParticle(k, teleportDisplacement.x, teleportDisplacement.y,
                ...
                if (mFlexJointParticle[k].mRestParticle) {
                    ...
                    mParticleSystem->MoveParticle(
                            mFlexJointParticle[k].mRestParticle->mParticleIndex,
                ...
                default: // default method is non relative
                {
                    for (int i = 0; i < mNumFlexJointParticle; i++) {
                        if (mFlexJointParticle[i].mRestParticle) {
                            ...
                            mParticleSystem->DynMoveParticle(
                                    mFlexJointParticle[i].mRestParticle->mParticleIndex, pos, idt);
    ...
    void FlexJointModifier::ResetParticleSystem(Vector *newSpeed) {
        for (int i = 0; i < mNumFlexJointParticle; i++) {
            ...
            if (mFlexJointParticle[i].mRestParticle) // rest particle initially at the same position
            {
                mParticleSystem->SetPosParticle(mFlexJointParticle[i].mRestParticle->mParticleIndex,
                                                particlepos);
...
*/
        ...
        if (dt == 0) {
            ...
            for (int i = 0; i < mNumFlexJointParticle; i++) {
                ...
                if (mFlexJointParticle[i].mRestParticle) {
                    ...
                    speed.Scale(idt);
                    mParticleSystem->SetSpeedParticle(
                            mFlexJointParticle[i].mRestParticle->mParticleIndex, speed.x, speed.y,
                            speed.z);
                }
            }
        }
    ...
    void FlexJointModifier::DebugDisplay() {
        ...
        for (i = 0; i < mFlexibleJointConnection.GetSize(); i++) {
            mParticleSystem->GetPosParticle(mFlexibleJointConnection[i].mFlexJointParticleIndex1,
                                            pos1);
            mParticleSystem->GetPosParticle(mFlexibleJointConnection[i].mFlexJointParticleIndex2,
                                            pos2);
        ...
        if (mUseRestingPos) {
            for (i = 0; i < mNumFlexJointParticle; i++) {
                if (mFlexJointParticle[i].mRestParticle) {
                    ...
                    mParticleSystem->GetPosParticle(
                            mFlexJointParticle[i].mRestParticle->mParticleIndex, pos2);
...
//
    ...
    FlexibleJointDefinitionLoader::FlexibleJointDefinitionLoader() : tSimpleChunkHandler(
            Simulation::Flexible::JOINT) {};
