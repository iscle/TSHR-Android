...
*/
    ...
    public:
        ...
        void AddLine(int pa, int pb,
                     ConditionParams *c = NULL);                      // a line, not a triangle's edge
        void AddTriangle(int pa, int pb, int pc, ConditionParams *c = NULL);          // a triangle
        void AddBending1D(int pa, int pb, int pc, bool calcRestAngle = false,
                          ConditionParams *c = NULL);         // on two consecutive lines
        void AddBending2D(int pa, int pb, int pc, int pd,
                          ConditionParams *c = NULL); // on two adjascent triangles, pb and pc being the shared edge

        virtual bool
        SetLengthEstimate(float inLength, int inNbPart, ConditionParams *inCparam = NULL);
