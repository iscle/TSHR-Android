...
namespace sim {
    ...
    enum RestPositionMethodEnum {
        RELATIVE1 = 1, RELATIVE2, RELATIVE3, RELATIVE4, RELATIVE5, RELATIVE6
    };
    enum UpdateJointsMethodEnum {
        SIMPLEDEF1 = 1, SIMPLEDEF2, SIMPLEDEF3
    };
    enum SimMethodEnum {
        SIMMETHOD1 = 1
    }; // unused for now
    ...
    public:
        ...
        public:
            ...
            bool Contains(short inIndex) const {
                return (inIndex == mIndex1 || inIndex == mIndex2) ? true : false;
            }
    ...
    public:
        ...
        void SetSpeedDampingFactor(
                float inSpeedDampingFactor) { mSpeedDampingFactor = inSpeedDampingFactor; }
        ...
        void
        SetSimStateArticulated(SimStateArticulated *inSimState); // need that to setup the plugs

        void SetBlendingFactor(float inBlendingFactor);

        float BlendingFactor() const { return mBlendingFactor; }

        bool ContainsJoint(int inIndex) const {
            return mPoseToParticleIndex[inIndex] > 0 ? true : false;
        }
    ...
    protected:
        ...
        void ComputeM(const rmt::Vector &p1, const rmt::Vector &p2, const rmt::Vector &p3,
                      rmt::Matrix &m);
