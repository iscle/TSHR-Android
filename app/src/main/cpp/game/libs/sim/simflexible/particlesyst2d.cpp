...
namespace sim {
    ...
    void ParticleSystem2D::BuildConditions() {
        ...
        switch (solver) {
            case eLargeStepSolver: {
                ...
                for (i = 0; i < mNbi - 1; i++) {
                    for (j = 0; j < mNbj - 1; j++) {
                        ...
                        mConditions->Insert(
                                new Wind2DCondition(p01, p00, p10, mCparams, &mWind, area));
                        mConditions->Insert(
                                new Wind2DCondition(p10, p11, p01, mCparams, &mWind, area));
                        ...
                        if (i == mNbi - 2) {
                            ...
                            mConditions->Insert(
                                    new Bend2DCondition(p01, p11, p10, P(i + 2, j), mCparams));
                        }
                        if (j == mNbj - 2) {
                            mConditions->Insert(new Stretch1DCondition(p01, p11, mCparams, mDeli));
                        } else {
                            mConditions->Insert(
                                    new Bend2DCondition(p10, p11, p01, P(i, j + 2), mCparams));
                        }
                    }
                }
            }
            ...
            case eSemiImplicitSolver2D: {
                ...
                for (i = 0; i < mNbi - 1; i++) {
                    for (j = 0; j < mNbj - 1; j++) {
                        ...
                        p11 = P(i + 1, j + 1);
                        mConditions->Insert(
                                new SpringConditionForStretch(p00, p01, mCparams, mDelj));
                        mConditions->Insert(
                                new SpringConditionForStretch(p00, p10, mCparams, mDeli));
                        if (i == mNbi - 2) {
                            mConditions->Insert(
                                    new SpringConditionForStretch(p10, p11, mCparams, mDelj));
                        }
                        if (j == mNbj - 2) {
                            mConditions->Insert(
                                    new SpringConditionForStretch(p01, p11, mCparams, mDeli));
                        }
                    }
                }
                ...
                for (i = 0; i < mNbi - 1; i++) {
                    for (j = 0; j < mNbj - 1; j++) {
                        ...
                        p11 = P(i + 1, j + 1);
                        mConditions->Insert(
                                new SpringConditionForShear(p00, p11, mCparams, lenth0));
                        mConditions->Insert(
                                new SpringConditionForShear(p10, p01, mCparams, lenth0));
                    }
                }
            ...
            default:
                rAssertMsg(0,
                           "Solver type must be set before to call BuildCondition or invalid sovlver type for this kind of particle system.");
                break;
        }
