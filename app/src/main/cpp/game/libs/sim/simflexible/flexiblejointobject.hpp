...
namespace sim {
    ...
    public:
        ...
        bool
        IsValid() const { return true; }//mFlexJointModifier->mJointState == NULL ? false : true; }
        bool IsActivated() const {
            return (IsValid() && mFlexJointModifier->IsEnabled()) ? true : false;
        }
        ...
        short PoseToParticleIndex(int inIndex) const {
            return mFlexJointModifier->PoseToParticleIndex(inIndex);
        }
        ...
        virtual void StartCollision(const rmt::Vector &inPos, const rmt::Vector &inDir,
                                    const SimulatedObject *inObj = NULL, int inIndex = -1,
                                    int otherIndex = -1) {
            rAssert(IsActivated());
        ...
        AddImpulse(const rmt::Vector &inPos, const rmt::Vector &inImp, const SymMatrix &inSymMatrix,
                   SimulatedObject *otherObj = NULL, int inIndex = -1, int otherIndex = -1) {
        ...
        GetMassMatrix(const rmt::Vector &inPos, SymMatrix &oM, SimulatedObject *o = NULL,
                      int inIndex = -1, int otherIndex = -1) {
            rAssert(mCurrentIndex == PoseToParticleIndex(inIndex) && IsActivated());
            FlexibleObject::GetMassMatrix(inPos, oM, o, mCurrentIndex);
        }

        virtual void ResolveInterpenetration(int inIndex, float &io_dist, const rmt::Vector &inN,
                                             rmt::Vector &o_imp, bool addconstrain,
                                             const SimulatedObject &otherObj) {
            rAssert(mCurrentIndex == PoseToParticleIndex(inIndex) && IsActivated());
            FlexibleObject::ResolveInterpenetration(mCurrentIndex, io_dist, inN, o_imp,
                                                    addconstrain, otherObj);
        }
