...
void ParticleSystemOnGeom1D::CreateVertexMapList(Vector *pos) {
    ...
    for (unsigned j = 0; j < vl->GetNumVertices(); j++) {
        ...
        if (alreadythere) {
            ...
            pos[nbp] = v;
            nbp++;
            rAssertMsg(nbp < MAX_VERTICES_IN_GEOM_PS, "too many vertices in tGeometry",
                       "ParticleSystemOnGeom1D::CreateVertexMapList");
        }
    }
}
            ...
            if (testdegeneratedlines) {
                for (int ii = 0; ii < current - lineMap; ii += 2) {
                    if (lineMap[ii] == mapPtoVL[current[0]] || lineMap[ii] == mapPtoVL[current[1]])
                        if (lineMap[ii + 1] == mapPtoVL[current[0]] ||
                            lineMap[ii + 1] == mapPtoVL[current[1]])
                            found = true;
                }
            }
            if (!found) {
...
void ParticleSystemOnGeom1D::BuildConditions() {
    ...
    for (i = 0; i < nLines; i++) {
        ...
        Conditions->Insert(
                new Stretch1DCondition(lp[lineMap[2 * i]], lp[lineMap[2 * i + 1]], cparams,
                                       lineLength[i]));
        Conditions->Insert(
                new Wind1DCondition(lp[lineMap[2 * i]], lp[lineMap[2 * i + 1]], cparams, wind));
    }
    if (nLines > 1) {
        for (i = 0; i < nLines; i++) {
            Conditions->Insert(new Bend1DCondition(lp[lineMap[2 * i]], lp[lineMap[2 * i + 1]],
                                                   lp[lineMap[2 * i + 1]], cparams));
        }
    }
}
...
void ParticleSystemOnGeom1D::PreStepModification() {
    ...
    while (moved && n < maxiter) {
        ...
        for (unsigned i = 0; i < nLines; i++) {
            if (TestExtension1D(lp[lineMap[2 * i]], lp[lineMap[2 * i + 1]], lineLength[i],
                                maxdef)) {
                moved = true;
            }
        }
