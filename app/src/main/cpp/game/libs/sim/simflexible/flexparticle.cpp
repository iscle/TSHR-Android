    ...
    void FlexParticle::CleanForConstraint(Vector *io_d) const {
        switch (mNbCollConstraint) {
            case 3: //0 dof
            {
                ...
                if (filtered.DotProduct(mCollConstraint[0]) < -MICRO_EPS ||
                    filtered.DotProduct(mCollConstraint[1]) < -MICRO_EPS) {
                    io_d->Clear();
                } else {
        ...
        if (mNbCollConstraint == 0) {
            ...
            if (1.0f - Min(Fabs(dot), 1.0f) > MILLI_EPS) {
                ...
                if (1.0f - Min(Fabs(dot), 1.0f) > MILLI_EPS) {
                    ...
                    dot = axe.DotProduct(inDir) /
                          axe.Magnitude(); //axe is not zero as cns[0] and cns[1] are not parallel
