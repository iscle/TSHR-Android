...
namespace sim {
        ...
        switch (mPSystSolver->GetSolverType()) {
            case eLargeStepSolver: {
                ...
                if (mNbp > 2) {
                    for (i = 0; i < mNbp - 2; i++) {
                        mConditions->Insert(
                                new Bend1DCondition(mP[i], mP[i + 1], mP[i + 2], mCparams));
                    }
                }
            }
                break;
            ...
            case eKinematicSolver: {
                for (i = 0; i < mNbp - 1; i++) {
                    mConditions->Insert(
                            new SpringConditionForStretch(mP[i], mP[i + 1], mCparams, mDeli));
                }
    ...
    void ParticleSystem1D::Display() {
        ...
        for (int i = 0; i < mNbp - 1; i++) {
            if (!mP[i]->mBroken)
                dStreamLine(*((rmt::Vector *) mP[i]->m_x), *((rmt::Vector *) mP[i + 1]->m_x), col,
                            col);
        }
