...
namespace sim {
    ...
    void ParticleSystemOnGeom2D::SetEdgeList(unsigned n, unsigned *map, float *len) {
        ...
        rAssert(mDeli != 0.0f);
        mPSystSolver->SetConvergenceCriteral(Max(Min(MAX_EPS_CG, mDeli / mNbEdge), MIN_EPS_CG),
                                             MAX_ITER_CG);
    }
        ...
        switch (mPSystSolver->GetSolverType()) {
            case eLargeStepSolver: {
                for (i = 0; i < mNbEdge; i++) {
                    ...
                    mConditions->Insert(
                            new Stretch1DCondition(mP[mMapEdge[2 * i]], mP[mMapEdge[2 * i + 1]],
                                                   mCparams, mLenEdge[i]));
                }
                for (i = 0; i < mNbTri; i++) {
                ...
                for (i = 0; i < mNbEdge; i++) {
                    ...
                    if (Find2AdjascentTris(&mMapEdge[2 * i], tri, ep)) {
                        mConditions->Insert(new Bend2DCondition(mP[ep[0]], mP[mMapEdge[2 * i]],
                                                                mP[mMapEdge[2 * i + 1]], mP[ep[1]],
                                                                mCparams));
                    }
                }
            }
    ...
    bool ParticleSystemOnGeom2D::Find2AdjascentTris(unsigned *edge, unsigned *tri,
                                                    unsigned *extrap) const {
        bool found = false;
        unsigned i;
        for (i = 0; i < mNbTri; i++) {
    ...
    bool ParticleSystemOnGeom2D::EdgeOfTriangle(unsigned *edge, unsigned *t,
                                                unsigned &extraPoint) const {
        bool found = false;
    ...
    void ParticleSystemOnGeom2D::PreStepModification() {
        ...
        while (moved && n < maxiter) {
            moved = false;
            for (unsigned i = 0; i < mNbEdge; i++) {
                if (TestExtension1D(mP[mMapEdge[2 * i]], mP[mMapEdge[2 * i + 1]], mLenEdge[i],
                                    mMaxDef)) {
                    moved = true;
                }
            }
    ...
    void ParticleSystemOnGeom2D::ComputeParticleNormal() {
        ...
        for (i = 0; i < mNbTri * 3; i += 3) {
            ComputeTriNormal(mP[mMapTri[i]]->m_x, mP[mMapTri[i + 1]]->m_x, mP[mMapTri[i + 2]]->m_x,
                             &mTriNormal[i]);
        }
        for (i = 0; i < mNbTri * 3; i += 3) {
            int ii = 3 * mMapTri[i];
    ...
    void ParticleSystemOnGeom2D::Display() {
        ...
        if (mDisplayBackFace) {
            ...
            p3d::pddi->SetCullMode(
                    mode == PDDI_CULL_NORMAL ? PDDI_CULL_INVERTED : PDDI_CULL_NORMAL);
    ...
    tEntity *PSystGeomLoader::LoadObject(tChunkFile *f, tEntityStore *store) {
        ...
        if (nDim == 2) {
            ...
            FlexibleObject *obj = new FlexibleObject(
                    PhysicsProperties::DefaultPhysicsProperties(store), psyst);
            obj->SetName(name);
        ...
        if (ret) {
            ...
            for (int i = 0; i < ret->mPsyst->mNbp; i++) {
                CollisionVolume *sphere = new SphereVolume(zeropos,
                                                           ret->mPsyst->mDeli * volumescale);
