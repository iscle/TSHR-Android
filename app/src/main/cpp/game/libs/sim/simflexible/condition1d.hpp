...
namespace sim {
    ...
    public:
        Stretch1DCondition(FlexParticle *pi, FlexParticle *pj, ConditionParams *p, float ddl,
...
//
    ...
    public:
        Bend1DCondition(FlexParticle *pi, FlexParticle *pj, FlexParticle *pk, ConditionParams *p,
                        bool calcRestAngle = false, bool inUsePartialDerivative = true);

        FlexParticle *mPk;

        inline virtual bool RelateToParticle(const FlexParticle *p) {
            return (Condition::RelateToParticle(p) || p == mPk);
        }
