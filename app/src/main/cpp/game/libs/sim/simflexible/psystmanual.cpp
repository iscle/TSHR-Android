...
namespace sim {
    ...
    void ParticleSystemManual::AddBending1D(int pa, int pb, int pc, bool calcRestAngle,
                                            ConditionParams *c) {
        rAssert(pa < mNbp && pb < mNbp && pc < mNbp);
        ConditionParams *cp = c == NULL ? mCparams : c;
        switch (mPSystSolver->GetSolverType()) {
            ...
            case eSemiImplicitSolver2D: {
                mConditions->Insert(
                        new Bend1DCondition(mP[pa], mP[pb], mP[pc], cp, calcRestAngle, false));
            }
                break;
            default:
                rAssertMsg(0, "Invalid solver or solver not implemented");
    ...
    bool ParticleSystemManual::SetLengthEstimate(float inLength, int inNbPart,
                                                 ConditionParams *inCparam) {
...
*/
    bool ParticleSystemManual::UpdateCparamsForLambda() {
        ...
        for (i = 0; i < mSubPartList.GetSize(); i++) {
            ...
            if (mMasterSubPartSet == false || i == 0) {
                result = ConditionParams::ComputeStretch1DFromLambda(part.mCparam->LambdaF,
                                                                     part.mCparam->LambdaD,
                                                                     part.mNbPart, part.mLength,
                                                                     gravity, f, d, mass);
                if (result) {
                ...
                part.mCparam->Stretch1Dkd =
                        2 * part.mCparam->LambdaD * Sqrt(mass * part.mCparam->Stretch1Dkf);
                part.mCparam->Stretch2Dkf = part.mCparam->LambdaF * master.mCparam->Stretch2Dkf;
                part.mCparam->Stretch2Dkd =
                        2 * part.mCparam->LambdaD * Sqrt(mass * part.mCparam->Stretch2Dkf);
                mPSystSolver->SetNeedPrecomputation(true);
            }
        }
        return result;
    ...
    bool ParticleSystemManual::UpdateCparamsForKappa() {
        ...
        for (i = 0; i < mSubPartList.GetSize(); i++) {
            PsystSubPart &part = mSubPartList[i];
            if (mMasterSubPartSet == false || i == 0) {
                result = ConditionParams::ComputeBend1DFromKappa(part.mCparam->KappaF,
                                                                 part.mCparam->KappaD,
                                                                 part.mNbPart, part.mLength,
                                                                 gravity, f, d, mass);
                ...
                part.mCparam->Bend1Dkd =
                        2 * part.mCparam->KappaD * Sqrt(mass * part.mCparam->Bend1Dkf / len) /
                        (nb - 1);
                part.mCparam->Bend2Dkf = part.mCparam->KappaF * master.mCparam->Bend2Dkf;
                part.mCparam->Bend2Dkd =
                        2 * part.mCparam->KappaD * Sqrt(mass * part.mCparam->Bend2Dkf / len) /
                        (nb - 1);
                mPSystSolver->SetNeedPrecomputation(true);
            }
        }
    ...
    bool ParticleSystemManual::UpdateCparamsForIota() {
        ...
        for (i = 0; i < mSubPartList.GetSize(); i++) {
            PsystSubPart &part = mSubPartList[i];
            if (mMasterSubPartSet == false || i == 0) {
                result = ConditionParams::ComputeStretch1DFromLambda(part.mCparam->IotaF,
                                                                     part.mCparam->IotaD,
                                                                     part.mNbPart, part.mLength,
                                                                     gravity, f, d, mass);
                ...
                part.mCparam->Shear2Dkd =
                        2 * part.mCparam->IotaD * Sqrt(mass * part.mCparam->Shear2Dkf);
                mPSystSolver->SetNeedPrecomputation(true);
            }
        }
        return result;
