...
// stabilizer
    ...
    void
    ParticleSystem::SetOneConditionParameter(float inVal, eFlexPhysicalPropertiesEnum inWhich) {
        switch (inWhich) {
    ...
    float ParticleSystem::GetOneConditionParameter(eFlexPhysicalPropertiesEnum inWhich) {
        switch (inWhich) {
            case eFlexStretch1D_f :
                return mCparams->Stretch1Dkf;
            case eFlexStretch1D_d :
                return mCparams->Stretch1Dkd;
            case eFlexStretch2D_f :
                return mCparams->Stretch2Dkf;
            case eFlexStretch2D_d :
                return mCparams->Stretch2Dkd;
            case eFlexBend1D_f    :
                return mCparams->Bend1Dkf;
            case eFlexBend1D_d    :
                return mCparams->Bend1Dkd;
            case eFlexBend2D_f    :
                return mCparams->Bend2Dkf;
            case eFlexBend2D_d    :
                return mCparams->Bend2Dkd;
            case eFlexShear2D_f   :
                return mCparams->Shear2Dkf;
            case eFlexShear2D_d   :
                return mCparams->Shear2Dkd;
            case eFlexWind1D_f    :
                return mCparams->Wind1DKf;
            case eFlexWind2D_f    :
                return mCparams->Wind2DKf;
        }
...
// not affected except that the velocities are rotated (magnitudes are constant).
    ...
    void ParticleSystem::AddAngularSpeedParticle(int i, const Vector &angularVelocity,
                                                 const Vector &origin) {
    ...
    void
    ParticleSystem::AddAngularSpeedSystem(const Vector &angularVelocity, const Vector &origin) {
        ...
        for (int i = 0; i < mNbp; i++) {
            ...
            SubVector(mP[i]->m_x, (float *) &origin, (float *) &radius);
            vel.CrossProduct(radius, angularVelocity);
            SetVector(mP[i]->m_v, vel.x + mP[i]->m_v[0], vel.y + mP[i]->m_v[1],
                      vel.z + mP[i]->m_v[2]);
        }
    }
        ...
        if (mP[index]->mFree) {
            switch (mP[index]->mNbCollConstraint) {
                ...
                case 2:
                    ...
                    mPSystSolver->ConstrainParticle(index, 1, &mP[index]->mCollConstraint[0],
                                                    &mP[index]->mCollConstraint[1]);
                    //Clean the current velocity of the particle along the set of constraining direction.
                    mP[index]->CleanForConstraint((Vector *) mP[index]->m_v);
                    break;
                case 1:
...
*/
    bool ParticleSystem::SetKappa
        ...
        result2 = ConditionParams::ComputeBend1DFromKappa(inCurvatureFact, inDampingFact, nbPart,
                                                          length0, gravity, f, d, mass);
...
*/
    bool ParticleSystem::SetIota(float inFact, float inDampingFact, bool) {
        ...
        float f, d;
        bool result = ConditionParams::ComputeStretch1DFromLambda(inFact, inDampingFact, nbPart,
                                                                  length0, gravity, f, d, mass);
