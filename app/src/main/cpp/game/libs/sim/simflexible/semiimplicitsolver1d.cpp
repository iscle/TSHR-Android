...
*/
        ...
        if (filterConstraints) {
            ...
            for (i = 0; i < mSolverNbp; i++) {
                ...
                if (!mPSyst->mP[i]->mFree) {
                    ScaleVector(m_b + 3 * i, -1.0f,
                                m_b + 3 * i); //Add the constraint on each fixed particle.
                }
            }
            if (mImplicitDamping) {
    ...
    void SemiImplicitSolver1D::SolverAdvanceSystem(float *io_dv) {
        ...
        for (i = 0, i3 = 0; i < mSolverNbp; i++, i3 = 3 * i) {
            if (mPSyst->mMass[i]) {
                ScaleVector(m_z + i3, 1 / mPSyst->mMass[i], io_dv +
                                                            i3);      // Here mDv represents the delta speed of each particles.
            }
        }
    }
