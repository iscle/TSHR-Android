...
// To make the second work right you need to reverse k and j, and swap dv1 and dv2.
    ...
    public:
        ...
        Stretch2DCondition(FlexParticle *pi, FlexParticle *pj, FlexParticle *pkk,
                           ConditionParams *p,
                           float du, float dv, float du22, float dv22,
                           bool inUsePartialDerivative = true);

        FlexParticle *pk;

        void Cond(C2DVal &C);

        void Derivative(const float C0, const float L, const float d1, const float d2, float dCi[],
                        float dCj[], float dCk[]);

        void
        Derivative2(float *x1, float *x2, const C2DVal &dC1p2, C2DVal &ddC, int ind1, int ind2);
...
#define halfPI (3.14159f/2.0f)
    ...
    public:
        Shear2DCondition(FlexParticle *pi, FlexParticle *pj, FlexParticle *pk, ConditionParams *p,
                         float angle0 = halfPI, bool inUsePartialDerivative = true);
...
//
    ...
    public:
        Bend2DCondition(FlexParticle *pi, FlexParticle *pj, FlexParticle *pk, FlexParticle *pl,
                        ConditionParams *p, float angle0 = 0, bool inUsePartialDerivative = true);
    ...
    public:
        Wind2DCondition(FlexParticle *pi, FlexParticle *pj, FlexParticle *pk, ConditionParams *p,
                        rmt::Vector *wind, float area);
