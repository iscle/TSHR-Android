...
// not totally stiff. ABSORB_CACHE_FEEDBACK = false will diseable this behavior => ~30% faster.
    ...
    ArticulatedPhysicsObject::ArticulatedPhysicsObject(PhysicsProperties *inProperties,
                                                       pPhysicsJoint *inJoints, int inNumSimJoints)
    ...
    void ArticulatedPhysicsObject::InitFromPose() {
        ...
        if (mJoints != NULL) // the articulated has been used before.
        {
            ...
            for (int i = 0; i < mNumSimJoints; i++) {
                ...
                mJoints[currentIndex]->SetTransform(
                        mPose->GetJoint(currentIndex)->GetWorldMatrix());
            }
        } else {
            ...
            */
            ...
            for (i = 0; i < mNumSimJoints; ++i) {
                ...
                if (i > 0 && parentIndex == 0 && mJoints[parentIndex] != NULL &&
                    mSimJoints[i]->mParentSimJoint == NULL) {
                    mSimJoints[i]->mParentSimJoint = mJoints[parentIndex];
                    mJoints[parentIndex]->mChildrenJoints.ResizeArray(mNumSimJoints);
                    mJoints[parentIndex]->mChildrenJoints.Add(mSimJoints[i]);
                }

                if (mSimJoints[i]->mInvStiffness > 0 || (mSimJoints[i]->mParentSimJoint &&
                                                         mSimJoints[i]->mParentSimJoint->mSimBranch ==
                                                         true))
    ...
    void ArticulatedPhysicsObject::ComputeJointMasses() {
        ...
        for (i = 0; i < mNumSimJoints; i++) {
            ...
            jnt->mML2 =
                    (jnt->mInertiaMatrix0.xx + jnt->mInertiaMatrix0.yy + jnt->mInertiaMatrix0.zz) *
                    0.33f;
    ...
    void ArticulatedPhysicsObject::JointAxis(int inIndex, rmt::Vector &axis, bool recompute) const {
        if (recompute || mJoints[inIndex] == NULL) {
            mPose->GetJoint(inIndex)->GetWorldMatrix().RotateVector(JointInfo(inIndex).mAxis,
                                                                    &axis);
        } else
    ...
    void ArticulatedPhysicsObject::SetClone(ArticulatedPhysicsObject *clone,
                                            SimState *inSimState) //const
    {
    ...
    void ArticulatedPhysicsObject::UpdatePoseMatrix() {
        if (!ConvertedToRigidBody()) {
            for (int i = 0; i < mNumSimJoints; i++) {
                mSimJoints[i]->SetTransform(
                        mPose->GetJoint(mSimJoints[i]->mIndex)->GetWorldMatrix());
            }
            ComputeInertiaMatrix();
        }
    }
    ...
    void ArticulatedPhysicsObject::ComputeInertiaMatrix() {
        if (mMass > 0 && mTimeSinceComputeInertiaMatrix >= mTimeComputeInertiaMatrix &&
            !ConvertedToRigidBody()) {
            mTimeSinceComputeInertiaMatrix = 0;
...
//
    ...
    void
    ArticulatedPhysicsObject::StartCollision(const rmt::Vector &inPos, const rmt::Vector &inDir,
                                             const SimulatedObject *otherObj, int index,
                                             int otherindex) {
        if (mMass == 0)
            return;
    ...
    void ArticulatedPhysicsObject::GetMassMatrix(const rmt::Vector &inPos, SymMatrix &oSymMatrix,
                                                 SimulatedObject *otherObj, int index,
                                                 int otherindex) {
        if (mMass == 0) {
            oSymMatrix.Zero();
            return;
    ...
    bool
    ArticulatedPhysicsObject::SpeedAdjustmentFromCache(const rmt::Vector &pos, rmt::Vector &speed,
                                                       int inIndex) {
        bool ret = false;
    ...
    bool ArticulatedPhysicsObject::AddImpulse(const rmt::Vector &inPos, const rmt::Vector &inImp,
...
// access to collision cache data
    bool ArticulatedPhysicsObject::GetCollisionCache(Vector &o_cacheV, Vector &o_cacheW,
                                                     int inIndex) const {
        if (inIndex >= 0) {
    ...
    void
    ArticulatedPhysicsObject::StartHandlingCollision(const SimulatedObject *otherObj, int index,
                                                     int otherindex) {
        if (index >= 0) {
...
*/
    ...
    void ArticulatedPhysicsObject::ApplyImpulse(int inJointIndex, float inIntensity,
                                                const rmt::Vector &inDir) {
        rAssert(!ConvertedToRigidBody());
        ...
        GetMassMatrix(cm, m, NULL,
                      inJointIndex); // not sure about that, the mass should include fJoint...
