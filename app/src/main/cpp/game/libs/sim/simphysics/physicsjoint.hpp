...
namespace sim {
    ...
    enum DeformationDissipationMethod {
        LinearDeformationDissipation = 0, AmortizedDeformationDissipation
    };
    ...
    public:
        ...
        static PhysicsJoint *
        CreatePhysicsJoint(int inDOF, const rmt::Vector &inCM, float inVolume, int inIndex);
        ...
        void SetInvStiffness(float inInvStiffness) {
            if (mNumDOF > 0)
                mInvStiffness = inInvStiffness;
        }
    ...
    protected:
        ...
        virtual void UpdateInertialDeformation(float, float, float, float, const rmt::Vector &,
                                               const rmt::Vector &, float) = 0;
        ...
        virtual bool
        AddImpulse(const rmt::Vector &inPos, rmt::Vector &inImp, const SymMatrix &inSymMatrix);
        ...
        virtual void
        VelocityProjection(const rmt::Vector &dr, float ndr, rmt::Vector &dv) const = 0;

        virtual void
        ComputeAbsorbedImpulse(const rmt::Vector &impulse, rmt::Vector &absImpulse) = 0;
...
//
    ...
    protected:
        ...
        void UpdateInertialDeformation(float, float, float, float, const rmt::Vector &,
                                       const rmt::Vector &, float) {}
...
//
    ...
    public:
        ...
        void
        ResetDeformation() { mDeformationSpeed = mDeformationSpeed0 = mDeformation = mDeformation0 = 0; }
    ...
    protected:
        ...
        void PerpAxisProjection(const rmt::Vector &v, rmt::Vector &vp) const {
            vp.ScaleAdd(v, -v.DotProduct(mRotAxis),
                        mRotAxis); // keep only component of imp in the plane perp to the axis
        }
        ...
        void UpdateInertialDeformation(float speedRate, float accelRate, float gravityRate,
        ...
        void SetCache(const rmt::Vector &cache) {
            mDeformationSpeedCache = cache.DotProduct(mRotAxis);
            rAssert(rmt::Sqr(mDeformationSpeedCache) < CRAZYW);
        }
        ...
        void AddToDeformationSpeed(const rmt::Vector &w) {
            mDeformationSpeed += w.DotProduct(mRotAxis);
        }
...
//
    ...
    public:
        ...
        void ResetDeformationSpeed() {
            mDeformationSpeed.Clear();
            mDeformationSpeed0.Clear();
            mDeformationSpeedEmpty = true;
        }

        void ResetDeformation() {
            ResetDeformationSpeed();
            mDeformation.Clear();
            mDeformation0.Clear();
            mDeformationEmpty = true;
        }
    ...
    protected:
        ...
        void SetCache(const rmt::Vector &cache) {
            mDeformationSpeedCache = cache;
            rAssert(cache.DotProduct(cache) < CRAZYW);
        }

        void ComputeAbsorbedImpulse(const rmt::Vector &impulse, rmt::Vector &absImpulse) {
            absImpulse.Scale(mAbsorptionFactor, impulse);
        }
        ...
        void AddCacheToDeformationSpeed() {
            mDeformationSpeed.Add(mDeformationSpeedCache);
            mDeformationSpeedEmpty = false;
        }

        void AddToDeformationSpeed(const rmt::Vector &w) {
            mDeformationSpeed.Add(w);
            mDeformationSpeedEmpty = false;
        }

        void
        SubTryAddDeformationSpeed(const rmt::Vector &w, float B, float C, float maxw, float &f);
    ...
    public:
        ...
        void Update(poser::Pose *pose) {
            if (IsEnabled() && mPhysicsJoint->GetInvStiffness()) {
                ...
                mPhysicsJoint->UpdateInertialDeformation(mInertialSpeedRate, mInertialAccelRate,
                                                         mGravityRate, mCentrifugalRate, s,
                                                         mPreviousSpeed, mDt);
                mPreviousSpeed = s;
            }
        }
