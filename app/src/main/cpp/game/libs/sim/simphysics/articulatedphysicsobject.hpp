...
namespace sim {
    ...
    public:
        ArticulatedPhysicsObject(PhysicsProperties *inProperties, pPhysicsJoint *inJoints,
                                 int inNumJoints);
        ...
        virtual void StartCollision(const rmt::Vector &inPos, const rmt::Vector &inDir,
                                    const SimulatedObject *inObj = NULL, int index = -1,
                                    int otherIndex = -1);

        virtual bool
        AddImpulse(const rmt::Vector &inPos, const rmt::Vector &inImp, const SymMatrix &inSymMatrix,
                   SimulatedObject *otherObj = NULL, int index = -1, int otherIndex = -1);

        virtual void EndCollision(bool fdyn = true, int index = -1, int otherIndex = -1);

        virtual void
        GetMassMatrix(const rmt::Vector &inPos, SymMatrix &oM, SimulatedObject *o = NULL,
                      int index = -1, int otherIndex = -1);

        virtual bool
        SpeedAdjustmentFromCache(const rmt::Vector &pos, rmt::Vector &speed, int inIndex = -1);

        // access to collision cache data
        virtual bool
        GetCollisionCache(rmt::Vector &o_cacheV, rmt::Vector &o_cacheW, int inIndex = -1) const;
        ...
        virtual void
        UpdateForPhysicsProperties(); // Need to update after changed of dymamic parameters.
        ...
        const SkeletonInfo::JointInfo &
        JointInfo(int index) const { return mSkeletonInfo->mJointInfo[index]; }
        ...
        const rmt::Matrix &JointTransform(int inIndex) const {
            return mPose->GetJoint(inIndex)->GetWorldTransform().GetMatrix();
        }

        float TimeStep() const { return mTimeStep; }

        rmt::Vector &GetJointLinearVelocity(int index) {
            return ((SimStateArticulated *) mSimState)->GetJointState(index)->GetLinearVelocity();
        }

        rmt::Vector &GetJointAngularVelocity(int index) {
            return ((SimStateArticulated *) mSimState)->GetJointState(index)->GetAngularVelocity();
        }
        ...
        void SetTimeComputeInertiaMatrix(
                float inTimeComputeInertiaMatrix) { mTimeComputeInertiaMatrix = inTimeComputeInertiaMatrix; }

        void SetDissipationInternalRate(
                float inDissipationInternalRate) { mDissipationInternalRate = inDissipationInternalRate; }

        void SetDissipationDeformationRate(
                float inDissipationDeformationRate) { mDissipationDeformationRate = inDissipationDeformationRate; }

        void SetDissipationDeformationSpeedRate(
                float inDissipationDeformationSpeedRate) { mDissipationDeformationSpeedRate = inDissipationDeformationSpeedRate; }

        bool
        ConvertedToRigidBody() const { return ((SimStateArticulated *) mSimState)->ConvertedToRigidBody(); }
        ...
        void SetImpulseReactionEnabled(
                bool inImpulseReactionEnabled) { mImpulseReactionEnabled = inImpulseReactionEnabled; }
    ...
    protected:
        ...
        ArticulatedPhysicsObject() : mJoints(
                NULL) { mType = ArticulatedObjectType; } // for clone service

    private:
        ArticulatedPhysicsObject &operator=(ArticulatedPhysicsObject &inObj) {
            rAssert(false);
            return *this;
        }
