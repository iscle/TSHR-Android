...
//
    ...
    bool PhysicsObject::TestResting(float dt) {
        if (mUseRestingDetector && !IsAtRest() &&
            TimeSinceCollide() < sThresholdTimeForRestingTest) {
    ...
    void PhysicsObject::WakeUp() {
        rAssertMsg(mSimState->GetControl() != simAICtrl,
                   "PhysicsObject::WakeUp: Control has to be given to the simulator before calling EndCollision.");
    ...
    void PhysicsObject::SolveAppliedForces(float dt) {
        ...
        if (gam > 0.0f && TimeSinceCollide() < sThresholdTimeForContactFriction &&
            Type() == RigidObjectType && mAxisOfRevolution < THREE_AXES_OF_REVOLUTION) {
            Vector axis, dw, dL;
            switch (mAxisOfRevolution) {
                ...
                case ONE_AXIS_OF_REVOLUTION:
                    rAssert(mGeomRotAxis.Magnitude() >
                            MILLI_EPS);//Make sure that the axis of revolution is defined in this case.
    ...
    bool PhysicsObject::AddImpulse(const Vector &inPos, const Vector &inImp, const SymMatrix &,
                                   SimulatedObject *, int, int) {
        mCacheEmpty = false;
    ...
    void PhysicsObject::EndCollision(bool fdyn, int, int) {
        if (!mCacheEmpty) {
            ...
            v.CrossProduct(p,
                           dw); // this is the same as v.CrossProduct(dw,p).Scale(-1) to save the scale call
    ...
    void
    PhysicsObject::GetMassMatrix(const rmt::Vector &inPos, SymMatrix &oM, SimulatedObject *, int,
                                 int) {
...
// still not satified, a translation is applied.
    bool
    PhysicsObject::UpdateConstraint(const LocalTarget &in_LocalTarget, const SimTarget &in_Target,
        ...
        static enum {
            Fix_Vcm, Fix_W, UnConstraintVelocity, other_choose
        } choose = UnConstraintVelocity;
        switch (choose) {
            case Fix_Vcm: {
                //To be done:
        ...
        while (f->ChunksRemaining()) {
            ...
            switch (f->GetCurrentID()) {
                ...
                case Simulation::Physics::JOINT: {
                    ...
                    physicsJoint[numSimJoints] = PhysicsJoint::CreatePhysicsJoint(dof, cm, volume,
                                                                                  index);
