...
//  Next enum describes if an object can roll and if it can about which axes.
    ...
    public:
        PhysicsObject(PhysicsProperties *inProperties, const Vector &inPos,
                      const SymMatrix &inInertia, float inVolume);
        ...
        virtual bool UpdateConstraint(const LocalTarget &in_LocalTarget, const SimTarget &in_Target,
                                      const Vector *in_VelocityTarget, float in_dt);
        ...
        void SetRestingSensitivity(float in_PeriodSensitivity, float inLinSpeedSensitivity = 1.0f,
                                   float inAngSpeedSensitivity = 1.0f) {
        ...
        virtual void StartCollision(const rmt::Vector &inPos, const rmt::Vector &inDir,
                                    ...
                                    int otherIndex = -1) { mCacheEmpty = true; }
        ...
        void SetAxisOfRevolution(
                AXIS_OF_REVOLUTION inAxisOfRevolution) { mAxisOfRevolution = inAxisOfRevolution; }
    ...
    private:
        PhysicsObject &operator=(PhysicsObject &inObj) {
            rAssert(false);
            return *this;
        }
