...
namespace sim {
    ...
    PhysicsJoint *
    PhysicsJoint::CreatePhysicsJoint(int inDOF, const rmt::Vector &inCM, float inVolume,
                                     int inIndex) {
            ...
            if (mInvStiffness > 0) {
                ...
                if (ndr > FRACTSIZE * jointSize) {
                    ...
                    if (mParentSimJoint != NULL &&
                        mParentSimJoint == mParentObject->SelfCollisionCenter()) {
    ...
    void PhysicsJoint::GetMassMatrix(const rmt::Vector &inPos, SymMatrix &oM) {
        if (this != mParentObject->SelfCollisionCenter()) {
            ...
            if (mAbsorptionFactor > RELA_SMALL &&
                mVisible) // add the local contribution to the mass matrix
            {
    ...
    bool PhysicsJoint::AddImpulse(const rmt::Vector &inPos, rmt::Vector &inImp,
                                  const SymMatrix &inSymMatrix) {
        if (this == mParentObject->SelfCollisionCenter()) {
        ...
        if (mAbsorptionFactor > 0 && !mCacheCompleted && mVisible) {
            ...
            if (ndveq >
                maxv) // make sure we don't reach an crazy speed (compare to the distance dr)
            {
            ...
            if (impDot > 0 &&
                ndveq > PhysicsObject::SmallSpeed()) // the joint can absorb some of the imp
            {
...
// to be able to use GetLocalSpeed after some impulses have been added.
    ...
    void PhysicsJoint::IncludeCacheReaction(Vector v, Vector w, bool collancestor) {
        ...
        if (mVisible) {
            if (!mOverFlowCache && !collancestor && GetNumDOF() > 0 &&
                this != mParentObject->CollidingJoint()) {
                Vector dw;
...
//
    ...
    void PhysicsJoint1D::SetTransform(const Matrix &m) {
        ...
        mParentObject->JointTransform(mParentIndex).RotateVector(
                mParentObject->JointInfo(mIndex).mRotAxis, &mRotAxis);
        ComputeJointAngle();
        TestJointAngle();
    }

    void
    PhysicsJoint1D::UpdateDeformation(float deformationRate, float deformationSpeedRate, float dt) {
    ...
    void
    PhysicsJoint1D::UpdateInertialDeformation(float speedRate, float accelRate, float gravityRate,
                                              float centrifugalRate, const rmt::Vector &inSpeed,
                                              const rmt::Vector &inDSpeed, float dt) {
        Vector b;
...
// accelerating, we try to add a new w to compensate for the that dvcm
    ...
    void
    PhysicsJoint1D::ComputeAbsorbedImpulse(const rmt::Vector &impulse, rmt::Vector &absImpulse) {
        PerpAxisProjection(impulse, absImpulse);
        absImpulse.Scale(mAbsorptionFactor);
    }

    void
    PhysicsJoint1D::VelocityProjection(const rmt::Vector &dr, float ndr, rmt::Vector &dv) const {
        Vector axis, vec;
        GetRotAxis(axis);
    ...
    float PhysicsJoint1D::TryAddDeformationSpeed(float addSpeed) {
        ...
        if (addSpeed != 0 && totSpeed != 0) {
            ...
            if (scaleAddSpeed > 0)
                scaleAddSpeed2 = TryAddValue(mDeformationSpeed, addSpeed, -MAXDEFSPEED,
                                             MAXDEFSPEED);
        }
...
//
    ...
    void
    PhysicsJoint3D::UpdateDeformation(float deformationRate, float deformationSpeedRate, float dt) {
    ...
    void
    PhysicsJoint3D::UpdateInertialDeformation(float speedRate, float accelRate, float gravityRate,
                                              float centrifugalRate, const rmt::Vector &inSpeed,
                                              const rmt::Vector &inDSpeed, float dt) {
        Vector axis;
...
// accelerating, we try to add a new w to compensate for the that dvcm
    ...
    void
    PhysicsJoint3D::VelocityProjection(const rmt::Vector &dr, float ndr, rmt::Vector &dv) const {
    ...
    void PhysicsJoint3D::TestJointAngle() {
        ...
        if (LimitJointAngle) {
            if (Fabs(mCurrentTorsionAngle) > MaxTorsionAngle) {
                float delta = (mCurrentTorsionAngle > 0 ? mCurrentTorsionAngle - MaxTorsionAngle :
                               mCurrentTorsionAngle + MaxTorsionAngle);
    ...
    float PhysicsJoint3D::TryAddDeformationSpeed(const Vector &addSpeed) {
        ...
        if (addedtorsion != 0) {
            scaleTorsion = TryAddValue(mCurrentTorsionAngle + torsion * mParentObject->TimeStep(),
                                       addedtorsion * mParentObject->TimeStep(), -MaxTorsionAngle,
                                       MaxTorsionAngle);
            if (scaleTorsion > 0)
        ...
        if (addedangle != 0) {
            scaleAngle = TryAddValue(mCurrentAngle + angle * mParentObject->TimeStep(),
                                     addedangle * mParentObject->TimeStep(), mMinAngle, mMaxAngle);
            if (scaleAngle != 0)
        ...
        if (addedsecangle != 0) {
            scalesecAngle = TryAddValue(secangle * mParentObject->TimeStep(),
                                        addedsecangle * mParentObject->TimeStep(), mMinAngle,
                                        mMaxAngle);
            if (scalesecAngle != 0)
