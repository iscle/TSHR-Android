...
//-------------------------------------------------------
    ...
    public:
        ...
        void Set(Vector min, Vector max) {
            low = min;
            high = max;
        }
        ...
        bool Intersects(const Sphere &sphere) const {
            if ((sphere.centre.x >= (low.x - sphere.radius)) &&
                (sphere.centre.x <= (high.x + sphere.radius))) {
                if ((sphere.centre.y >= (low.y - sphere.radius)) &&
                    (sphere.centre.y <= (high.y + sphere.radius))) {
                    if ((sphere.centre.z >= (low.z - sphere.radius)) &&
                        (sphere.centre.z <= (high.z + sphere.radius))) {
                        return true;
                    }
                }
            }
...
//-------------------------------------------------------
    ...
    public:
        ...
        void Set(const Vector &n, float d) {
            normal = n;
            D = d;
        }
        ...
        Plane(const Vector &norm, const Vector &pointInPlane) : normal(norm), D(-normal.DotProduct(
                pointInPlane)) {}

        void Set(const Vector &norm, const Vector &pointInPlane) {
            normal = norm;
            D = -normal.DotProduct(pointInPlane);
        }

        // build a plane from three points in the plane
        Plane(const Vector &pointInPlane1, const Vector &pointInPlane2,
              const Vector &pointInPlane3) { Set(pointInPlane1, pointInPlane2, pointInPlane3); }

        void
        Set(const Vector &pointInPlane1, const Vector &pointInPlane2, const Vector &pointInPlane3);
        ...
        bool Inside(const Sphere &sphere) const {
            return sphere.centre.DotProduct(normal) < (-D + (sphere.radius));
        }
