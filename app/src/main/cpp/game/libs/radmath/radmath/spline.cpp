...
void
Spline::SetCntrlMesh(const Basis basis, const Vector &cv0, const Vector &cv1, const Vector &cv2,
                     const Vector &cv3) {
    Matrix CVs;
    CVs.Row(0) = cv0;
    CVs.m[0][3] = 1.0f;
...
void
Spline::SetCntrlMesh(const Basis basis, const Vector4 &cv0, const Vector4 &cv1, const Vector4 &cv2,
                     const Vector4 &cv3) {
    Matrix CVs;
    CVs.Row4(0) = cv0;
    CVs.Row4(1) = cv1;
...
SplineCurve::SplineCurve(int n, Spline::Basis b, bool c) {
    assert(n == 0 || n >= 4 || c ==
                               true); // it doesn't make sense to evaluate an open cubic spline if you have less than 4 CVs
...
SplineCurve::LoadEvaluator(Evaluator idx, const float t) {
    ...
    Q[idx].SetCntrlMesh(basis, CVs[s % numCVs], CVs[(s + 1) % numCVs], CVs[(s + 2) % numCVs],
                        CVs[(s + 3) % numCVs]);
    start[idx] = s;
    return true;
}
...
SplineCurve::Forward() {
    ...
    assert(t >= 0.0f &&
           t <= GetEndParam());  // avoid stepping outside of area where curve is not defined

    if (LoadEvaluator(FORWARD_DIFFERENCING,
                      t)) // we may need to reload evaluator if we cross segment boundary
        return Q[FORWARD_DIFFERENCING].InitForwardDifferencing(
                numSteps); // if so we need to restart the forward differencing
    else
...
SplineCurve::GetSegmentLength(unsigned int seg) {
    ...
    Spline derivative;
    derivative.SetCntrlMesh(Spline::Basis(int(basis) + 3), CVs[seg % numCVs],
                            CVs[(seg + 1) % numCVs], CVs[(seg + 2) % numCVs],
                            CVs[(seg + 3) % numCVs]);
...
SplineCurve::GetLength(const float t) {
    ...
    LoadEvaluator(CURVE, tt);
    Spline derivative;
    derivative.SetCntrlMesh(Spline::Basis(int(basis) + 3), CVs[min % numCVs],
                            CVs[(min + 1) % numCVs], CVs[(min + 2) % numCVs],
                            CVs[(min + 3) % numCVs]);
