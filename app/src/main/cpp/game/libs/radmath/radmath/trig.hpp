...
// convert to and from old style unsigned short angles (P3DANGLE)
    template<class T>
    inline float AngleToRadian(T a) {
...
// anglephi is elevation (above/below the XY plane),
    void CartesianToSpherical(float x, float y, float z, float *magnitude, float *angletheta,
                              float *anglephi);

    void SphericalToCartesian(float magnitude, float angletheta, float anglephi, float *x, float *y,
                              float *z);
