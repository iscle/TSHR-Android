...
// 4x4 matrix
    ...
    public:
        ...
        void
        SetPerspective(const float fov, const float aspect, const float pnear, const float pfar);

        // matrix ops
        void Invert(void);       // invert the matrix
        void Invert(const Matrix &i) {
            *this = i;
            Invert();
        }

        void InvertOrtho(void);  // invert, assuming the matrix is orthogonal
        void InvertOrtho(const Matrix &i) {
            *this = i;
            InvertOrtho();
        }

        void OrthoNormal(void);  // make the matrix ortho-normal
        void OrthoNormal(const Matrix &i) {
            *this = i;
            OrthoNormal();
        }
        ...
        inline void Mult(const Matrix &i) {
            Matrix t;
            t.Mult(*this, i);
            *this = t;
        }
